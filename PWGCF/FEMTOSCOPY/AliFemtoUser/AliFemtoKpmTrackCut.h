...
#ifndef ALIFEMTOKPMTRACKCUT_H
#define ALIFEMTOKPMTRACKCUT_H
...
class AliFemtoKpmTrackCut : public AliFemtoTrackCut 
{
  public:
...
#ifdef __ROOT__ 
  ClassDef(AliFemtoKpmTrackCut, 1)
#endif
    };


inline void AliFemtoKpmTrackCut::SetPt(const float& lo, const float& hi){fPt[0]=lo; fPt[1]=hi;}
inline void AliFemtoKpmTrackCut::SetRapidity(const float& lo,const float& hi){fRapidity[0]=lo; fRapidity[1]=hi;}
inline void AliFemtoKpmTrackCut::SetEta(const float& lo,const float& hi){fEta[0]=lo; fEta[1]=hi;}
inline void AliFemtoKpmTrackCut::SetCharge(const int& ch){fCharge = ch;}
inline void AliFemtoKpmTrackCut::SetPidProbElectron(const float& lo,const float& hi){fPidProbElectron[0]=lo; fPidProbElectron[1]=hi;}
inline void AliFemtoKpmTrackCut::SetPidProbPion(const float& lo,const float& hi){fPidProbPion[0]=lo; fPidProbPion[1]=hi;}
inline void AliFemtoKpmTrackCut::SetPidProbKaon(const float& lo,const float& hi){fPidProbKaon[0]=lo; fPidProbKaon[1]=hi;}
inline void AliFemtoKpmTrackCut::SetPidProbProton(const float& lo,const float& hi){fPidProbProton[0]=lo; fPidProbProton[1]=hi;}
inline void AliFemtoKpmTrackCut::SetPidProbMuon(const float& lo,const float& hi){fPidProbMuon[0]=lo; fPidProbMuon[1]=hi;}
inline void AliFemtoKpmTrackCut::SetLabel(const bool& flag){fLabel=flag;}
inline void AliFemtoKpmTrackCut::SetStatus(const long& status){fStatus=status;}
inline void AliFemtoKpmTrackCut::SetminTPCclsF(const short& minTPCclsF){fminTPCclsF=minTPCclsF;}
inline void AliFemtoKpmTrackCut::SetminTPCncls(const short& s){fminTPCncls=s;}
inline void AliFemtoKpmTrackCut::SetminITScls(const int& minITScls){fminITScls=minITScls;}
inline void AliFemtoKpmTrackCut::SetMostProbablePion() { fMostProbable = 2; }
inline void AliFemtoKpmTrackCut::SetMostProbableKaon() { fMostProbable = 3; }
inline void AliFemtoKpmTrackCut::SetMostProbableProton() { fMostProbable = 4; }
inline void AliFemtoKpmTrackCut::SetNoMostProbable() { fMostProbable = 0; }
inline void AliFemtoKpmTrackCut::SetMaxITSChiNdof(const float& maxchi) { fMaxITSchiNdof = maxchi; }
inline void AliFemtoKpmTrackCut::SetMaxTPCChiNdof(const float& maxchi) { fMaxTPCchiNdof = maxchi; }
inline void AliFemtoKpmTrackCut::SetMaxSigmaToVertex(const float& maxsig) { fMaxSigmaToVertex = maxsig; }
inline void AliFemtoKpmTrackCut::SetMaxImpactXY(const float& maximpxy) { fMaxImpactXY = maximpxy; }
inline void AliFemtoKpmTrackCut::SetMaxImpactXYPtDep(const float& maxoff, const float& maxnrm, const float& maxpow) { fMaxImpactXYPtOff = maxoff; fMaxImpactXYPtNrm = maxnrm; fMaxImpactXYPtPow = maxpow; }
inline void AliFemtoKpmTrackCut::SetMaxImpactZ(const float& maximpz) { fMaxImpactZ = maximpz; }
