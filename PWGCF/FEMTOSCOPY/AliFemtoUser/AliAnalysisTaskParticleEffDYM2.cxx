...
#include "AliAnalysisTaskParticleEffDYM2.h"




ClassImp(AliAnalysisTaskParticleEffDYM2)
//ClassImp(AliAnalysisTaskParticleEff)

double fV1M2[3];


void AliAnalysisTaskParticleEffDYM2::SetFB(int fb)
{
  fFB = fb;
}

void AliAnalysisTaskParticleEffDYM2::SetPidMethod(PidMethod method)
{
  fPidMethod = method;
}

int AliAnalysisTaskParticleEffDYM2::GetPidMethod()
{
  return (int)fPidMethod;
}

void AliAnalysisTaskParticleEffDYM2::SetPidMethod(int method)
{
  switch(method){
...
AliAnalysisTaskParticleEffDYM2::AliAnalysisTaskParticleEffDYM2(TString name, int pidMethod, int filterbit) :
...
AliAnalysisTaskParticleEffDYM2::~AliAnalysisTaskParticleEffDYM2()
{
...
void AliAnalysisTaskParticleEffDYM2::UserCreateOutputObjects()
{
...
bool IsPionNSigmaM2(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime)
{
...
bool IsPionNSigmaV0M2(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
bool IsPionNSigmaV0TPC5M2(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
bool IsProtonNSigmaV0TPC5M2(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
...
bool IsPionNSigma3M2(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime)
{
...
bool IsKaonNSigmaM2(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
    if (mom > 0.5) {
...
bool IsKaonNSigma3M2(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
  if (mom > 0.5) {
    //rejection of unwanted contamination
   // if(mom>1 && TOFtime<-400)
...
bool IsProtonNSigmaM2(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
...
bool IsProtonNSigmaV0M2(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
...
bool IsProtonNSigma3M2(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
  if (mom > 0.5) {
...
bool IsElectronM2(float nsigmaTPCe, float nsigmaTPCPi,float nsigmaTPCK, float nsigmaTPCP)
{
...
void AliAnalysisTaskParticleEffDYM2::UserExec(Option_t *)
{
...
//copying pid information for FB 128
  ...
  for (Int_t iTracks = 0; iTracks < aodEvent->GetNumberOfTracks(); iTracks++) {
	...
	isPionNsigma = (IsPionNSigmaM2(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]) && !IsKaonNSigma3M2(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma3M2(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
	isKaonNsigma = (!IsPionNSigma3M2(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && IsKaonNSigmaM2(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma3M2(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
	isProtonNsigma = (!IsPionNSigma3M2(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && !IsKaonNSigma3M2(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && IsProtonNSigmaM2(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
...
if(collect[2]==true){
  ...
  for (Int_t iTracks = 0; iTracks < aodEvent->GetNumberOfTracks(); iTracks++) {
    ...
    if(IsElectronM2(nSigmaTPCe,nSigmaTPCPi,nSigmaTPCK,nSigmaTPCP))
      continue;
    fHistQA[10]->Fill(7);
    ...
    //******** With double counting *******************
      isPionNsigma = (IsPionNSigmaM2(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]));
      isKaonNsigma = (IsKaonNSigmaM2(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]));
      isProtonNsigma = (IsProtonNSigmaM2(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
	...
	isProtonNsigma = (IsProtonNSigmaM2(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      ...
      //******** Exclusive PID ********************
      isPionNsigma = (IsPionNSigmaM2(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]) && !IsKaonNSigmaM2(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigmaM2(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isKaonNsigma = (!IsPionNSigmaM2(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && IsKaonNSigmaM2(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigmaM2(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isProtonNsigma = (!IsPionNSigmaM2(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && !IsKaonNSigmaM2(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && IsProtonNSigmaM2(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
  ...
  for (Int_t i = 0; i < aodEvent->GetNumberOfV0s(); i++) 
    {
      ...
      if (aodv0->CosPointingAngle(fV1M2) < 0.99) continue;
      //if (aodv0->CosPointingAngle(fV1M2) < 0.95) continue;
      fCutsLambda->Fill(cutLam++);
      fCutsAntiLambda->Fill(cutALam++);
      ...
      if(aodv0->DecayLength(fV1M2) > 60) continue;
      fCutsLambda->Fill(cutLam++);
      fCutsAntiLambda->Fill(cutALam++);
      ...
      else
	{
      ...
      isPionNsigmaPos = IsPionNSigmaV0TPC5M2(trackpos->Pt(),nSigmaTPCPiPos,nSigmaTOFPiPos);
      isProtonNsigmaPos = IsProtonNSigmaV0TPC5M2(trackpos->Pt(),nSigmaTPCPPos,nSigmaTOFPPos);
      isPionNsigmaNeg = IsPionNSigmaV0TPC5M2(trackneg->Pt(),nSigmaTPCPiNeg,nSigmaTOFPiNeg);
      isProtonNsigmaNeg = IsProtonNSigmaV0TPC5M2(trackneg->Pt(),nSigmaTPCPNeg,nSigmaTOFPNeg);
      ...
      if(isProtonNsigmaNeg && isPionNsigmaPos)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    ...
	    fAllVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt());
      ...
      if(isProtonNsigmaPos && isPionNsigmaNeg)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    ...
	    fAllVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt());
      ...
      if (MCtrkMother->IsPhysicalPrimary() && Lambda && pdgMother==3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fPrimVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt()); 
      ...
      else if (MCtrkMother->IsPhysicalPrimary() && AntiLambda && pdgMother==-3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fPrimVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt()); 
      ...
      else if(MCtrkMother->IsSecondaryFromWeakDecay() && Lambda && pdgMother==3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecWeakVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt()); 
      ...
      else if(MCtrkMother->IsSecondaryFromWeakDecay() && AntiLambda && pdgMother==-3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecWeakVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt()); 
	...
	fSecMatVsDCA[PARTTYPES*fcent+4][0]->Fill(aodv0->DcaV0ToPrimVertex(),aodv0->Pt());
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecMatVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt()); 
	...
	fSecMatVsDCA[PARTTYPES*fcent+4][1]->Fill(aodv0->DcaV0ToPrimVertex(),aodv0->Pt());
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){ 
	    fSecMatVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt()); 
      ...
      else if(Lambda)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fFakeVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt()); 
      ...
      else if(AntiLambda)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fFakeVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1M2),aodv0->Pt()); 
