...
#include "AliAnalysisEfficiencyPbPbExclusivePID.h"




ClassImp(AliAnalysisEfficiencyPbPbExclusivePID)
//ClassImp(AliAnalysisTaskParticleEff)

double fV1PbPb[3];


void AliAnalysisEfficiencyPbPbExclusivePID::SetFB(int fb)
{
  fFB = fb;
}

void AliAnalysisEfficiencyPbPbExclusivePID::SetPidMethod(PidMethod method)
{
  fPidMethod = method;
}

int AliAnalysisEfficiencyPbPbExclusivePID::GetPidMethod()
{
  return (int)fPidMethod;
}

void AliAnalysisEfficiencyPbPbExclusivePID::SetPidMethod(int method)
{
  switch(method){
...
AliAnalysisEfficiencyPbPbExclusivePID::AliAnalysisEfficiencyPbPbExclusivePID(TString name, int pidMethod, int filterbit) :
  AliAnalysisTaskSE(name), centrality(0), fHistoList(0),  fMassInvLambdaPass(0),fMassInvAntiLambdaPass(0), fMassInvLambdaFail(0), fMassInvAntiLambdaFail(0),fYLambda(0),fPtLambda(0), fYAntiLambda(0),fPtAntiLambda(0), fCutsLambda(0), fCutsAntiLambda(0), fTruePtLambdaMC(0), fRecPtLambdaMC(0), fTruePtAntiLambdaMC(0),fRecPtAntiLambdaMC(0), fMassInvXimPass(0),fMassInvXipPass(0), fMassInvXimFail(0), fMassInvXipFail(0),fYXim(0),fPtXim(0), fYXip(0),fPtXip(0), fCutsXim(0), fCutsXip(0), recoParticleArrayXi(0), fTruePtXimMC(0), fRecPtXimMC(0), fTruePtXipMC(0), fRecPtXipMC(0), fDCAtoPrimVtx(0), fIfAliEventCuts(kFALSE), fFB(128), fPidMethod(kExclusivePID),  fEstEventMult(kRefMult),fIfXiAnalysis(kFALSE), fpidResponse(0), fAODpidUtil(0), fEventCuts(0)
...
void AliAnalysisEfficiencyPbPbExclusivePID::UserCreateOutputObjects()
{
...
bool IsPionNSigmaPbPb(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime)
{
...
bool IsPionNSigmaV0PbPb(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
bool IsPionNSigmaV0TPC5PbPb(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
bool IsProtonNSigmaV0TPC5PbPb(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
...
bool IsPionNSigma3PbPb(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime)
{
...
bool IsKaonNSigmaPbPb(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
    if (mom > 0.5) {
...
bool IsKaonNSigma3PbPb(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
  if (mom > 0.5) {
    //rejection of unwanted contamination
   // if(mom>1 && TOFtime<-400)
...
bool IsProtonNSigmaPbPb(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
...
bool IsProtonNSigmaV0PbPb(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
...
bool IsProtonNSigma3PbPb(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
  if (mom > 0.5) {
...
bool IsElectronPbPb(float nsigmaTPCe, float nsigmaTPCPi,float nsigmaTPCK, float nsigmaTPCP)
{
...
void AliAnalysisEfficiencyPbPbExclusivePID::UserExec(Option_t *)
{
...
*/
    ...
    if(IsElectronPbPb(nSigmaTPCe,nSigmaTPCPi,nSigmaTPCK,nSigmaTPCP))
      continue;
    fHistQA[10]->Fill(7);
    ...
    //******** With double counting *******************
      isPionNsigma = (IsPionNSigmaPbPb(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]));
      isKaonNsigma = (IsKaonNSigmaPbPb(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]));
      isProtonNsigma = (IsProtonNSigmaPbPb(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
	...
	isProtonNsigma = (IsProtonNSigmaPbPb(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      ...
      //******** Exclusive PID ********************
      isPionNsigma = (IsPionNSigmaPbPb(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]) && !IsKaonNSigmaPbPb(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigmaPbPb(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isKaonNsigma = (!IsPionNSigmaPbPb(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && IsKaonNSigmaPbPb(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigmaPbPb(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isProtonNsigma = (!IsPionNSigmaPbPb(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && !IsKaonNSigmaPbPb(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && IsProtonNSigmaPbPb(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
    else if(fPidMethod==kExclusivePIDDiffRejection){
      //******** Exclusive PID, different rejection  ********************
      isPionNsigma = (IsPionNSigma3PbPb(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]) && !IsKaonNSigma3PbPb(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma3PbPb(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isKaonNsigma = (!IsPionNSigma3PbPb(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && IsKaonNSigma3PbPb(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma3PbPb(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isProtonNsigma = (!IsPionNSigma3PbPb(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && !IsKaonNSigma3PbPb(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && IsProtonNSigma3PbPb(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
    if (isPionNsigma){
  ...
  for (Int_t i = 0; i < aodEvent->GetNumberOfV0s(); i++) 
    {
      ...
      if (aodv0->CosPointingAngle(fV1PbPb) < 0.99) continue;
      ...
      if(aodv0->DecayLength(fV1PbPb) > 60) continue;
      fCutsLambda->Fill(cutLam++);
      fCutsAntiLambda->Fill(cutALam++);
      ...
      else
	{
      ...
      isPionNsigmaPos = IsPionNSigmaV0TPC5PbPb(trackpos->Pt(),nSigmaTPCPiPos,nSigmaTOFPiPos);
      isProtonNsigmaPos = IsProtonNSigmaV0TPC5PbPb(trackpos->Pt(),nSigmaTPCPPos,nSigmaTOFPPos);
      isPionNsigmaNeg = IsPionNSigmaV0TPC5PbPb(trackneg->Pt(),nSigmaTPCPiNeg,nSigmaTOFPiNeg);
      isProtonNsigmaNeg = IsProtonNSigmaV0TPC5PbPb(trackneg->Pt(),nSigmaTPCPNeg,nSigmaTOFPNeg);
      ...
      if(isProtonNsigmaNeg && isPionNsigmaPos)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    ...
	    fAllVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt());
      ...
      if(isProtonNsigmaPos && isPionNsigmaNeg)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    ...
	    fAllVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt());
      ...
      if (MCtrkMother->IsPhysicalPrimary() && Lambda && pdgMother==3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fPrimVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt()); 
      ...
      else if (MCtrkMother->IsPhysicalPrimary() && AntiLambda && pdgMother==-3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fPrimVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt()); 
      ...
      else if(MCtrkMother->IsSecondaryFromWeakDecay() && Lambda && pdgMother==3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecWeakVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt()); 
      ...
      else if(MCtrkMother->IsSecondaryFromWeakDecay() && AntiLambda && pdgMother==-3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecWeakVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt()); 
	...
	fSecMatVsDCA[PARTTYPES*fcent+4][0]->Fill(aodv0->DcaV0ToPrimVertex(),aodv0->Pt());
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecMatVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt()); 
	...
	fSecMatVsDCA[PARTTYPES*fcent+4][1]->Fill(aodv0->DcaV0ToPrimVertex(),aodv0->Pt());
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){ 
	    fSecMatVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt()); 
      ...
      else if(Lambda)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fFakeVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt()); 
      ...
      else if(AntiLambda)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fFakeVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1PbPb),aodv0->Pt()); 
