...
#include "AliAnalysisTaskEffK0s.h"




ClassImp(AliAnalysisTaskEffK0s)
//ClassImp(AliAnalysisTaskParticleEff)

double fV1K0s[3];


void AliAnalysisTaskEffK0s::SetFB(int fb)
{
  fFB = fb;
}

void AliAnalysisTaskEffK0s::SetPidMethod(PidMethod method)
{
  fPidMethod = method;
}

int AliAnalysisTaskEffK0s::GetPidMethod()
{
  return (int)fPidMethod;
}

void AliAnalysisTaskEffK0s::SetPidMethod(int method)
{
  switch(method){
...
AliAnalysisTaskEffK0s::AliAnalysisTaskEffK0s(TString name, int pidMethod, int filterbit) :
  AliAnalysisTaskSE(name), centrality(0), fHistoList(0),  fMassInvK0sPass(0), fMassInvK0sFail(0), fMassInvK0s(0), fEtaK0s(0),fPtK0s(0), fCutsK0s(0), fTruePtK0sMC(0), fRecPtK0sMC(0), fDCAtoPrimVtx(0), fIfAliEventCuts(kFALSE), fFB(128), fPidMethod(kExclusivePIDDiffRejection),  fEstEventMult(kRefMult), fpidResponse(0), fAODpidUtil(0), fEventCuts(0), fTrackPileUpRemoval(kFALSE), fV0PileUpRemoval(kFALSE)
{
...
void AliAnalysisTaskEffK0s::UserCreateOutputObjects()
{
  ...
  fMassInvK0sFail = new TH1D("fMassfInvK0sFail","Mass Assuming K0s Hypothesis Fail", 200, 0.3, 0.7);
  fHistoList->Add(fMassInvK0sFail);

  fMassInvK0s = new TH1D("fMassfInvK0s","K0s invariant mass distribution", 200, 0.3, 0.7);
  fHistoList->Add(fMassInvK0s);

  fMassInvK0sPass = new TH1D("fMassfInvK0sPass","Mass Assuming K0s Hypothesis Pass", 200, 0.3, 0.7);
...
void AliAnalysisTaskEffK0s::UserExec(Option_t *)
{
  AliAODInputHandler *aodH = dynamic_cast<AliAODInputHandler *>(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler());
...
//***************************************************
 ...
 //**** getting MC array ******
  ...
  for (Int_t iTracks = 0; iTracks < aodEvent->GetNumberOfTracks(); iTracks++) // MC reconstructed loop
  {
    ...
    if (track->Pt() < 0.15 || track->Pt() > 10)  // Track pT cut
      continue;
    fHistQA[10]->Fill(5);
    ...
    if(IsElectronAM(nSigmaTPCe,nSigmaTPCPi,nSigmaTPCK,nSigmaTPCP))
      continue;
    fHistQA[10]->Fill(7);
  ...
  //  if (aodv0->GetNProngs() > 2) continue;
    ...
    if(aodv0->Pt() < 0.15 || aodv0->Pt() > 10) continue;
    fCutsK0s->Fill(cutK0s++);
    ...
    if(aodv0->DcaV0Daughters() > 0.8) continue;
    fCutsK0s->Fill(cutK0s++);


    if(aodv0->DcaPosToPrimVertex() < 0.1) continue;
    fCutsK0s->Fill(cutK0s++);


    if(aodv0->DcaNegToPrimVertex() < 0.1) continue;
    fCutsK0s->Fill(cutK0s++);
    ...
    if(isPionNsigmaPos && isPionNsigmaNeg){
      fCutsK0s->Fill(cutK0s++);  
     // if(trackpos->Pt() < 0.15 || trackpos->Pt() > 10) continue; //pion plus
	// fCutsK0s->Fill(cutK0s++);
    ...
    if (!MCtrkMother->IsPhysicalPrimary() && K0s && pdgMother==310) { //secondaries in K0ss
    	fReconstructedNotPrimaries[PARTTYPES*fcent+4][0]->Fill(aodv0->Eta(), aodv0->Pt());
    } 
    else if(MCtrkMother->IsPhysicalPrimary() && K0s && pdgMother==310) {
