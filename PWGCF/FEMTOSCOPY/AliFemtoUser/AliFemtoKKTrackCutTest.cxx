...
***************************************************************************
*
* $Id: AliFemtoKKTrackCutTest.cxx 60781 2013-02-08 16:24:10Z akisiel $ 
*
* 
***************************************************************************
*
...
#ifdef __ROOT__ 
ClassImp(AliFemtoKKTrackCutTest)
#endif
...
// 3       -4.608098e-02   8.336400e-03
  ...
  AliFemtoKKTrackCutTest::AliFemtoKKTrackCutTest() :
    fCharge(0),
    fLabel(0),
    fStatus(0),
    fPIDMethod(knSigma),
...
//------------------------------
bool AliFemtoKKTrackCutTest::Pass(const AliFemtoTrack* track)
{
  //cout<<"AliFemtoKKTrackCutTest::Pass"<<endl;
...
//------------------------------
AliFemtoString AliFemtoKKTrackCutTest::Report()
{
...
TList *AliFemtoKKTrackCutTest::ListSettings()
{
  // return a list of settings in a writable form
  TList *tListSetttings = new TList();
  char buf[200];
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.mass=%f", this->Mass());
  tListSetttings->AddLast(new TObjString(buf));

  snprintf(buf, 200, "AliFemtoKKTrackCutTest.charge=%i", fCharge);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobpion.minimum=%f", fPidProbPion[0]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobpion.maximum=%f", fPidProbPion[1]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobkaon.minimum=%f", fPidProbKaon[0]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobkaon.maximum=%f", fPidProbKaon[1]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobproton.minimum=%f", fPidProbProton[0]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobproton.maximum=%f", fPidProbProton[1]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobelectron.minimum=%f", fPidProbElectron[0]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobelectron.maximum=%f", fPidProbElectron[1]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobMuon.minimum=%f", fPidProbMuon[0]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pidprobMuon.maximum=%f", fPidProbMuon[1]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.minimumtpcclusters=%i", fminTPCclsF);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.minimumitsclusters=%i", fminTPCclsF);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pt.minimum=%f", fPt[0]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.pt.maximum=%f", fPt[1]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.rapidity.minimum=%f", fRapidity[0]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.rapidity.maximum=%f", fRapidity[1]);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.removekinks=%i", fRemoveKinks);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.maxitschindof=%f", fMaxITSchiNdof);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.maxtpcchindof=%f", fMaxTPCchiNdof);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.maxsigmatovertex=%f", fMaxSigmaToVertex);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.maximpactxy=%f", fMaxImpactXY);
  tListSetttings->AddLast(new TObjString(buf));
  snprintf(buf, 200, "AliFemtoKKTrackCutTest.maximpactz=%f", fMaxImpactZ);
  tListSetttings->AddLast(new TObjString(buf));
  if (fMostProbable) {
    if (fMostProbable == 2)
      snprintf(buf, 200, "AliFemtoKKTrackCutTest.mostprobable=%s", "Pion");
    if (fMostProbable == 3)
      snprintf(buf, 200, "AliFemtoKKTrackCutTest.mostprobable=%s", "Kaon");
    if (fMostProbable == 4)
      snprintf(buf, 200, "AliFemtoKKTrackCutTest.mostprobable=%s", "Proton");
    tListSetttings->AddLast(new TObjString(buf));
  }
  return tListSetttings;
}
void AliFemtoKKTrackCutTest::SetRemoveKinks(const bool& flag)
{
  fRemoveKinks = flag;
}
			    
void AliFemtoKKTrackCutTest::SetRemoveITSFake(const bool& flag)
{
  fRemoveITSFake = flag;
}
...
// 5       -1.125965e+00   2.821170e-01
// 6       2.009036e-01    5.438876e-02
float AliFemtoKKTrackCutTest::PidFractionElectron(float mom) const
{
...
// 1       -4.222208e-01   2.534402e-02
// 2       1.042004e-01    1.503945e-02
float AliFemtoKKTrackCutTest::PidFractionPion(float mom) const
{
...
// 2       -2.996790e-01   1.840964e-02
// 3       6.704652e-02    7.783990e-03
float AliFemtoKKTrackCutTest::PidFractionKaon(float mom) const
{
...
// 2       8.354116e-02    1.997948e-02
// 3       -4.608098e-02   8.336400e-03
float AliFemtoKKTrackCutTest::PidFractionProton(float mom) const
{
...
void AliFemtoKKTrackCutTest::SetMomRangeTOFpidIs(const float& minp, const float& maxp)
{
  fMinPforTOFpid = minp;
  fMaxPforTOFpid = maxp;
}

void AliFemtoKKTrackCutTest::SetMomRangeTPCpidIs(const float& minp, const float& maxp)
{
  fMinPforTPCpid = minp;
  fMaxPforTPCpid = maxp;
}

void AliFemtoKKTrackCutTest::SetMomRangeITSpidIs(const float& minp, const float& maxp)
{
  fMinPforITSpid = minp;
  fMaxPforITSpid = maxp;
}

bool AliFemtoKKTrackCutTest::IsPionTPCdEdx(float mom, float dEdx)
{
...
bool AliFemtoKKTrackCutTest::IsKaonTPCdEdx(float mom, float dEdx)
{
...
bool AliFemtoKKTrackCutTest::IsProtonTPCdEdx(float mom, float dEdx)
{
...
bool AliFemtoKKTrackCutTest::IsPionTOFTime(float mom, float ttof)
{
...
bool AliFemtoKKTrackCutTest::IsKaonTOFTime(float mom, float ttof)
{
...
bool AliFemtoKKTrackCutTest::IsProtonTOFTime(float mom, float ttof)
{
...
bool AliFemtoKKTrackCutTest::IsKaonTPCdEdxNSigma(float mom, float nsigmaK)
{
//  cout<<" AliFemtoKKTrackCutTest::IsKaonTPCdEdxNSigma "<<mom<<" "<<nsigmaK<<endl;
...
bool AliFemtoKKTrackCutTest::IsKaonTOFNSigma(float mom, float nsigmaK)
{
//  cout<<" AliFemtoKKTrackCutTest::IsKaonTPCdEdxNSigma "<<mom<<" "<<nsigmaK<<endl;
  //fan
...
bool AliFemtoKKTrackCutTest::IsKaonNSigma(float mom, float nsigmaTPCK, float nsigmaTOFK)
{
...
bool AliFemtoKKTrackCutTest::IsPionNSigma(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
  if(mom<0.5)
    {
  ...
  else if(mom>0.5 && mom<1.5 && TMath::Abs(nsigmaTOFPi)<2.0 && TMath::Abs(nsigmaTPCPi)<2.0) return true;
  return false;
}


bool AliFemtoKKTrackCutTest::IsProtonNSigma(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
    if (mom > 0.8) {
...
void AliFemtoKKTrackCutTest::SetPIDMethod(ReadPIDMethodType newMethod)
{
  fPIDMethod = newMethod;
}


void AliFemtoKKTrackCutTest::SetClusterRequirementITS(AliESDtrackCuts::Detector det, AliESDtrackCuts::ITSClusterRequirement req) 
{ 
  fCutClusterRequirementITS[det] = req; 
}

Bool_t AliFemtoKKTrackCutTest::CheckITSClusterRequirement(AliESDtrackCuts::ITSClusterRequirement req, Bool_t clusterL1, Bool_t clusterL2)
{
...
void AliFemtoKKTrackCutTest::SetNsigmaTPCle250(Double_t nsigma)
{
  fNsigmaTPCle250 = nsigma;
}

void AliFemtoKKTrackCutTest::SetNsigmaTPC250_400(Double_t nsigma)
{
  fNsigmaTPC250_400 = nsigma;
}

void AliFemtoKKTrackCutTest::SetNsigmaTPC400_450(Double_t nsigma)
{
  fNsigmaTPC400_450 = nsigma;
}

void AliFemtoKKTrackCutTest::SetNsigmaTPC450_500(Double_t nsigma)
{
  fNsigmaTPC450_500 = nsigma;
}

void AliFemtoKKTrackCutTest::SetNsigmaTPCge500(Double_t nsigma)
{
  fNsigmaTPCge500 = nsigma;
}

void AliFemtoKKTrackCutTest::SetNsigmaTOF500_800(Double_t nsigma)
{
  fNsigmaTOF500_800 = nsigma;
}


void AliFemtoKKTrackCutTest::SetNsigmaTOF800_1000(Double_t nsigma)
{
  fNsigmaTOF800_1000 = nsigma;
}

void AliFemtoKKTrackCutTest::SetNsigmaTOFge1000(Double_t nsigma)
{
