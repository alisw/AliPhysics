...
#ifndef AliFemtoKKTrackCutTest_H
#define AliFemtoKKTrackCutTest_H
...
class AliFemtoKKTrackCutTest : public AliFemtoTrackCut 
{
  public:
  ...
  virtual ~AliFemtoKKTrackCutTest();
...
#ifdef __ROOT__ 
  ClassDef(AliFemtoKKTrackCutTest, 1)
#endif
    };


inline void AliFemtoKKTrackCutTest::SetPt(const float& lo, const float& hi){fPt[0]=lo; fPt[1]=hi;}
inline void AliFemtoKKTrackCutTest::SetRapidity(const float& lo,const float& hi){fRapidity[0]=lo; fRapidity[1]=hi;}
inline void AliFemtoKKTrackCutTest::SetEta(const float& lo,const float& hi){fEta[0]=lo; fEta[1]=hi;}
inline void AliFemtoKKTrackCutTest::SetCharge(const int& ch){fCharge = ch;}
inline void AliFemtoKKTrackCutTest::SetPidProbElectron(const float& lo,const float& hi){fPidProbElectron[0]=lo; fPidProbElectron[1]=hi;}
inline void AliFemtoKKTrackCutTest::SetPidProbPion(const float& lo,const float& hi){fPidProbPion[0]=lo; fPidProbPion[1]=hi;}
inline void AliFemtoKKTrackCutTest::SetPidProbKaon(const float& lo,const float& hi){fPidProbKaon[0]=lo; fPidProbKaon[1]=hi;}
inline void AliFemtoKKTrackCutTest::SetPidProbProton(const float& lo,const float& hi){fPidProbProton[0]=lo; fPidProbProton[1]=hi;}
inline void AliFemtoKKTrackCutTest::SetPidProbMuon(const float& lo,const float& hi){fPidProbMuon[0]=lo; fPidProbMuon[1]=hi;}
inline void AliFemtoKKTrackCutTest::SetLabel(const bool& flag){fLabel=flag;}
inline void AliFemtoKKTrackCutTest::SetStatus(const long& status){fStatus=status;}
inline void AliFemtoKKTrackCutTest::SetminTPCclsF(const short& minTPCclsF){fminTPCclsF=minTPCclsF;}
inline void AliFemtoKKTrackCutTest::SetminTPCncls(const short& s){fminTPCncls=s;}
inline void AliFemtoKKTrackCutTest::SetminITScls(const int& minITScls){fminITScls=minITScls;}
inline void AliFemtoKKTrackCutTest::SetMostProbablePion() { fMostProbable = 2; }
inline void AliFemtoKKTrackCutTest::SetMostProbableKaon() { fMostProbable = 3; }
inline void AliFemtoKKTrackCutTest::SetMostProbableProton() { fMostProbable = 4; }
inline void AliFemtoKKTrackCutTest::SetNoMostProbable() { fMostProbable = 0; }
inline void AliFemtoKKTrackCutTest::SetMaxITSChiNdof(const float& maxchi) { fMaxITSchiNdof = maxchi; }
inline void AliFemtoKKTrackCutTest::SetMaxTPCChiNdof(const float& maxchi) { fMaxTPCchiNdof = maxchi; }
inline void AliFemtoKKTrackCutTest::SetMaxSigmaToVertex(const float& maxsig) { fMaxSigmaToVertex = maxsig; }
inline void AliFemtoKKTrackCutTest::SetMaxImpactXY(const float& maximpxy) { fMaxImpactXY = maximpxy; }
inline void AliFemtoKKTrackCutTest::SetMaxImpactXYPtDep(const float& maxoff, const float& maxnrm, const float& maxpow) { fMaxImpactXYPtOff = maxoff; fMaxImpactXYPtNrm = maxnrm; fMaxImpactXYPtPow = maxpow; }
inline void AliFemtoKKTrackCutTest::SetMaxImpactZ(const float& maximpz) { fMaxImpactZ = maximpz; }
