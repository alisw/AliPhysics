...
#include "AliAnalysisTaskEffK0ss_pp.h"




ClassImp(AliAnalysisTaskEffK0ss_pp)
//ClassImp(AliAnalysisTaskParticleEff)

double fV1K0spp[3];


void AliAnalysisTaskEffK0ss_pp::SetFB(int fb)
{
  fFB = fb;
}

void AliAnalysisTaskEffK0ss_pp::SetPidMethod(PidMethod method)
{
  fPidMethod = method;
}

int AliAnalysisTaskEffK0ss_pp::GetPidMethod()
{
  return (int)fPidMethod;
}

void AliAnalysisTaskEffK0ss_pp::SetPidMethod(int method)
{
  switch(method){
...
AliAnalysisTaskEffK0ss_pp::AliAnalysisTaskEffK0ss_pp() :
  AliAnalysisTaskSE("name"), centrality(0), fHistoList(0),  fMassInvK0sPass(0), fMassInvK0sFail(0), fMassInvK0s(0), fMassInvK0sAfterCuts(0), fMassInvK0sPt(0), fEtaK0s(0),fPtK0s(0), fCutsK0s(0), fTruePtK0sMC(0), fRecPtK0sMC(0), fDCAtoPrimVtx(0), fIfAliEventCuts(kFALSE), fFB(96), fPidMethod(kExclusivePIDDiffRejection),  fEstEventMult(kRefMult), fpidResponse(0), fAODpidUtil(0), fEventCuts(0), fTrackPileUpRemoval(kFALSE), fV0PileUpRemoval(kFALSE)
{
...
AliAnalysisTaskEffK0ss_pp::AliAnalysisTaskEffK0ss_pp(TString name, int pidMethod, int filterbit) :
  AliAnalysisTaskSE(name), centrality(0), fHistoList(0),  fMassInvK0sPass(0), fMassInvK0sFail(0), fMassInvK0s(0), fMassInvK0sAfterCuts(0), fMassInvK0sPt(0), fEtaK0s(0),fPtK0s(0), fCutsK0s(0), fTruePtK0sMC(0), fRecPtK0sMC(0), fDCAtoPrimVtx(0), fIfAliEventCuts(kFALSE), fFB(128), fPidMethod(kExclusivePIDDiffRejection),  fEstEventMult(kRefMult), fpidResponse(0), fAODpidUtil(0), fEventCuts(0), fTrackPileUpRemoval(kFALSE), fV0PileUpRemoval(kFALSE), fMassRejectCutK0(0.005)
{
...
void AliAnalysisTaskEffK0ss_pp::UserCreateOutputObjects()
{
  ...
  fHistQA2D[5] = new TH2F("RejectionCutsMass","", 100,-2,2,100,0.1,10);
  ...
  for ( Int_t i = 0; i < 11; i++){
    ...
    if(i<6) fHistoList->Add(fHistQA2D[i]);
    if(i<5){
     for(Int_t j = 0 ; j<PARTTYPES; j++){
...
bool IsPionNSigmaAMpp(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime){
  if (mom > 0.5){
...
bool IsPionNSigmaV0AMpp(float mom, float nsigmaTPCPi, float nsigmaTOFPi){
  if (TMath::Abs(nsigmaTPCPi) < 3.0) return true;
  return false;
}

bool IsPionNSigmaV0TPC5AMpp(float mom, float nsigmaTPCPi, float nsigmaTOFPi){
  if (TMath::Abs(nsigmaTPCPi) < 5.0) return true;
  return false;
}


bool IsProtonNSigmaV0TPC5AMpp(float mom, float nsigmaTPCP, float nsigmaTOFP){
  if (TMath::Abs(nsigmaTPCP) < 5.0) return true;
  return false;
}

bool IsPionNSigma3AMpp(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime){
  if (mom > 0.5) {
...
bool IsKaonNSigmaAMpp(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime){
  if (mom > 0.5) {
...
bool IsKaonNSigma3AMpp(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
  if (mom > 0.5) {
    //rejection of unwanted contamination
    // if(mom>1 && TOFtime<-400)
...
bool IsProtonNSigmaAMpp(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
  if (mom > 0.5) {
...
bool IsProtonNSigmaV0AMpp(float mom, float nsigmaTPCP, float nsigmaTOFP){
  if (mom < 0.8){
...
bool IsProtonNSigma3AMpp(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
  if (mom > 0.5) {   
    // if(mom>1.8 && TOFtime<-300)
    // return false;
...
bool IsElectronASMpp(float nsigmaTPCe, float nsigmaTPCPi,float nsigmaTPCK, float nsigmaTPCP)
{
...
bool IsElectronAMpp(float nsigmaTPCe, float nsigmaTPCPi,float nsigmaTPCK, float nsigmaTPCP)
{
...
void AliAnalysisTaskEffK0ss_pp::UserExec(Option_t *)
{
  AliAODInputHandler *aodH = dynamic_cast<AliAODInputHandler *>(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler());
...
*/
    ...
    if(IsElectronAMpp(nSigmaTPCe,nSigmaTPCPi,nSigmaTPCK,nSigmaTPCP))
      continue;
    fHistQA[10]->Fill(7);
    ...
    bool isProtonNsigma  = 0;
      ...
      isPionNsigma = (IsPionNSigmaAMpp(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]) && !IsKaonNSigma3AMpp(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma3AMpp(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isKaonNsigma = (!IsPionNSigma3AMpp(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && IsKaonNSigmaAMpp(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma3AMpp(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isProtonNsigma = (!IsPionNSigma3AMpp(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && !IsKaonNSigma3AMpp(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && IsProtonNSigmaAMpp(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
  ...
  for (Int_t i = 0; i < aodEvent->GetNumberOfV0s(); i++){
    ...
    double massK0 = aodv0->MassK0Short();
    double massLambda = aodv0->MassLambda();
    double massAntilambda = aodv0->MassAntiLambda();
    int cutK0s = 1;
    fCutsK0s->Fill(cutK0s++);
    
   
    Double_t massSellK0_lam = TMath::Abs(massLambda-1.115683);
    Double_t massSellK0_antilam = TMath::Abs(massAntilambda-1.115683);
    //pileup for LHC20e3a -> Injective Pileup over tracks 
    /*Bool_t isParticleFromOutOfBunchPileupCollision = kFALSE;
...
*/
    ...
    if(TMath::Abs(aodv0->DcaPosToPrimVertex()) < 0.06) continue;
    fCutsK0s->Fill(cutK0s++);


    if(TMath::Abs(aodv0->DcaNegToPrimVertex()) < 0.06) continue;
    fCutsK0s->Fill(cutK0s++);

    //if(TMath::Abs(aodv0->DcaV0ToPrimVertex()) > 0.3) continue;
    //fCutsK0s->Fill(cutK0s++);
   
    Double_t decayL = aodv0->DecayLength(fV1K0spp);
    ...
    //in pp 
	if(TMath::Abs(massSellK0_lam)<fMassRejectCutK0) continue;
	if (TMath::Abs(massSellK0_antilam)<fMassRejectCutK0) continue;
	
	fHistQA2D[5]->Fill(massK0, aodv0->Pt());
    ...
    isPionNsigmaPos = IsPionNSigmaV0TPC5AMpp(trackpos->Pt(),nSigmaTPCPiPos,0);
    isPionNsigmaNeg = IsPionNSigmaV0TPC5AMpp(trackneg->Pt(),nSigmaTPCPiNeg,0);
	...
	// fCutsK0s->Fill(cutK0s++);
     ...
     // if(aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
        ...
        fAllVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1K0spp),aodv0->Pt());
  ...
  // }
    ...
    if (MCtrkMother->IsPhysicalPrimary() && pdgMother==310){
	    ...
	    //if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	      fPrimVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1K0spp),aodv0->Pt()); 
	 ...
	 //   if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	      fSecWeakVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1K0spp),aodv0->Pt()); 
	 ...
	 // }
	  ...
	  //  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	      fSecMatVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1K0spp),aodv0->Pt()); 
