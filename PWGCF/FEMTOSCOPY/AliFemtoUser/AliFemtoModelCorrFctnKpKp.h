...
#ifndef ALIFEMTOMODELCORRFCTNKPKP_H
#define ALIFEMTOMODELCORRFCTNKPKP_H
...
class AliFemtoModelCorrFctnKpKp: public AliFemtoModelCorrFctn {

public:
  // default constructor:
  AliFemtoModelCorrFctnKpKp();
  // constructor with some parameters(histogram bins,low,high and so on):
  AliFemtoModelCorrFctnKpKp(const char *title,
			  Int_t aNbins,
			  Double_t aQinvLo,
			  Double_t aQinvHi);
  // copy constructor:
  AliFemtoModelCorrFctnKpKp(const AliFemtoModelCorrFctnKpKp& aCorrFctn);
  // destructor:
  virtual ~AliFemtoModelCorrFctnKpKp();

  AliFemtoModelCorrFctnKpKp& operator=(const AliFemtoModelCorrFctnKpKp& aCorrFctn);
...
#ifdef __ROOT__
  /// \cond CLASSIMP
  ClassDef(AliFemtoModelCorrFctnKpKp, 1);
  /// \endcond
#endif
};
