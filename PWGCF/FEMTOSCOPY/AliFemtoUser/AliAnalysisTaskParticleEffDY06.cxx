...
#include "AliAnalysisTaskParticleEffDY06.h"




ClassImp(AliAnalysisTaskParticleEffDY06)
...
void AliAnalysisTaskParticleEffDY06::SetFB(int fb)
{
  fFB = fb;
}

void AliAnalysisTaskParticleEffDY06::SetPidMethod(PidMethod method)
{
  fPidMethod = method;
}

int AliAnalysisTaskParticleEffDY06::GetPidMethod()
{
  return (int)fPidMethod;
}

void AliAnalysisTaskParticleEffDY06::SetPidMethod(int method)
{
  switch(method){
...
AliAnalysisTaskParticleEffDY06::AliAnalysisTaskParticleEffDY06(TString name, int pidMethod, int filterbit) :
...
AliAnalysisTaskParticleEffDY06::~AliAnalysisTaskParticleEffDY06()
{
...
void AliAnalysisTaskParticleEffDY06::UserCreateOutputObjects()
{
...
bool IsPionNSigma_06(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime)
{
...
bool IsPionNSigmaV0_06(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
bool IsPionNSigmaV0TPC5_06(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
bool IsProtonNSigmaV0TPC5_06(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
...
bool IsPionNSigma3_06(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime)
{
...
bool IsKaonNSigma_06(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
    if (mom > 0.5) {
...
bool IsKaonNSigma3_06(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
  if (mom > 0.5) {
    //rejection of unwanted contamination
   // if(mom>1 && TOFtime<-400)
...
bool IsProtonNSigma_06(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
...
bool IsProtonNSigmaV0_06(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
...
bool IsProtonNSigma3_06(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
  if (mom > 0.5) {
...
bool IsElectron6(float nsigmaTPCe, float nsigmaTPCPi,float nsigmaTPCK, float nsigmaTPCP)
{
...
void AliAnalysisTaskParticleEffDY06::UserExec(Option_t *)
{
...
*/
    ...
    if(IsElectron6(nSigmaTPCe,nSigmaTPCPi,nSigmaTPCK,nSigmaTPCP))
      continue;
    fHistQA[10]->Fill(7);
   ...
   if(fPidMethod==kNSigma){
    //******** With double counting *******************
      isPionNSigma = (IsPionNSigma_06(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]));
      isKaonNSigma = (IsKaonNSigma_06(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]));
      isProtonNSigma = (IsProtonNSigma_06(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
	...
	isProtonNSigma = (IsProtonNSigma_06(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      ...
      //******** Exclusive PID ********************
      isPionNSigma = (IsPionNSigma_06(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]) && !IsKaonNSigma_06(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma_06(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isKaonNSigma = (!IsPionNSigma_06(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && IsKaonNSigma_06(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma_06(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isProtonNSigma = (!IsPionNSigma_06(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && !IsKaonNSigma_06(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && IsProtonNSigma_06(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
    else if(fPidMethod==kExclusivePIDDiffRejection){
      //******** Exclusive PID, different rejection  ********************
      isPionNSigma = (IsPionNSigma_06(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]) && !IsKaonNSigma3_06(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma3_06(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
      isKaonNSigma = (!IsPionNSigma3_06(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && IsKaonNSigma_06(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma3_06(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
    if (isPionNSigma){
  ...
  for (Int_t i = 0; i < aodEvent->GetNumberOfV0s(); i++) 
    {
      ...
      if (aodv0->CosPointingAngle(fV1_6) < 0.99) continue;
      //if (aodv0->CosPointingAngle(fV1_6) < 0.95) continue;
      fCutsLambda->Fill(cutLam++);
      fCutsAntiLambda->Fill(cutALam++);
      ...
      if(aodv0->DecayLength(fV1_6) > 60) continue;
      fCutsLambda->Fill(cutLam++);
      fCutsAntiLambda->Fill(cutALam++);
      ...
      else
	{
      ...
      isPionNsigmaPos = IsPionNSigmaV0TPC5_06(trackpos->Pt(),nSigmaTPCPiPos,nSigmaTOFPiPos);
      isProtonNsigmaPos = IsProtonNSigmaV0TPC5_06(trackpos->Pt(),nSigmaTPCPPos,nSigmaTOFPPos);
      isPionNsigmaNeg = IsPionNSigmaV0TPC5_06(trackneg->Pt(),nSigmaTPCPiNeg,nSigmaTOFPiNeg);
      isProtonNsigmaNeg = IsProtonNSigmaV0TPC5_06(trackneg->Pt(),nSigmaTPCPNeg,nSigmaTOFPNeg);
      ...
      if(isProtonNsigmaNeg && isPionNsigmaPos)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    ...
	    fAllVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt());
      ...
      if(isProtonNsigmaPos && isPionNsigmaNeg)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    ...
	    fAllVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt());
      ...
      if (MCtrkMother->IsPhysicalPrimary() && Lambda && pdgMother==3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fPrimVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt()); 
      ...
      else if (MCtrkMother->IsPhysicalPrimary() && AntiLambda && pdgMother==-3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fPrimVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt()); 
      ...
      else if(MCtrkMother->IsSecondaryFromWeakDecay() && Lambda && pdgMother==3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecWeakVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt()); 
      ...
      else if(MCtrkMother->IsSecondaryFromWeakDecay() && AntiLambda && pdgMother==-3122)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecWeakVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt()); 
	...
	fSecMatVsDCA[PARTTYPES*fcent+4][0]->Fill(aodv0->DcaV0ToPrimVertex(),aodv0->Pt());
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fSecMatVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt()); 
	...
	fSecMatVsDCA[PARTTYPES*fcent+4][1]->Fill(aodv0->DcaV0ToPrimVertex(),aodv0->Pt());
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){ 
	    fSecMatVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt()); 
      ...
      else if(Lambda)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fFakeVsCosPointingAngle[PARTTYPES*fcent+4][0]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt()); 
      ...
      else if(AntiLambda)
	{
	  ...
	  if( aodv0->DcaV0ToPrimVertex() < fDCAtoPrimVtx){
	    fFakeVsCosPointingAngle[PARTTYPES*fcent+4][1]->Fill(aodv0->CosPointingAngle(fV1_6),aodv0->Pt()); 
