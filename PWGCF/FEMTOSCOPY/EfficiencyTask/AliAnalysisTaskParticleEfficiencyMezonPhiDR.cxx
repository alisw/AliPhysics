...
AliAnalysisTaskParticleEfficiencyMezonPhiDR::AliAnalysisTaskParticleEfficiencyMezonPhiDR(TString name, int pidMethod, int filterbit) :
...
AliAnalysisTaskParticleEfficiencyMezonPhiDR::~AliAnalysisTaskParticleEfficiencyMezonPhiDR()
{
...
void AliAnalysisTaskParticleEfficiencyMezonPhiDR::UserCreateOutputObjects()
{
  ...
  fGeneratedMCPhi = new TH2F("fGeneratedMCPhiEff", "Gen Meson Phi minv pT", 3000, 0, 10, 3000, 0.5, 2.0);
  
  fGeneratedMCPhiPrimaries = new TH2F("fGeneratedMCPhiEffPrim", "Gen Meson Phi minv pT", 3000, 0, 10, 3000, 0.5, 2.0);
  fMCPrimariesPhiThatAreReconstructed = new TH2F("fReconstructedMCPhiEffPrim", "Rec Meson Phi minv pT", 3000, 0, 10, 3000, 0.5, 2.0);
  
  fHistoList->Add(fGeneratedMCPhi);
  fHistoList->Add(fGeneratedMCPhiPrimaries);

  fMCPhiThatAreReconstructed = new TH2F("fReconstructedMCPhiEff", "Rec Meson Phi minv pT", 3000, 0, 10, 3000, 0.5, 2.0);
  
  fHistoList->Add(fMCPhiThatAreReconstructed);
  fHistoList->Add(fMCPrimariesPhiThatAreReconstructed);
  
  fEtaPtKp = new TH2F("EtaPtKp", "Eta Pt Kp", 300, -1.5, 1.5, 300, 0, 5); 
  
  fEtaPtKm = new TH2F("EtaPtKm", "Eta Pt Km", 300, -1.5, 1.5, 300, 0, 5); 
  
  fHistoList->Add(fEtaPtKp);
  fHistoList->Add(fEtaPtKm);
  ...
  for(Int_t j = 0; j < PARTTYPES; j++)  {
    ...
    fHistQAPID[0][j][0] = new TH2F(hname, htitle, 100, 0.0, 4.0, 250, 0.0, 500.0);
    htitle+="Minus"; hname+="Minus";
    fHistQAPID[0][j][1] = new TH2F(hname, htitle, 100, 0.0, 4.0, 250, 0.0, 500.0);
    hname  = "pidTOFTime";  hname+=parttypename;
    htitle = parttypename + " TOF Time vs. momentum";
    fHistQAPID[1][j][0] = new TH2F(hname, htitle, 100, 0.1, 4.0, 400, -4000.0, 4000.0);
    htitle+="Minus"; hname+="Minus";
    fHistQAPID[1][j][1] = new TH2F(hname, htitle, 100, 0.1, 4.0, 400, -4000.0, 4000.0);
    hname  = "pidTOFNSigma";  hname+=parttypename;
    htitle = parttypename + " TOF NSigma vs. momentum";
    fHistQAPID[2][j][0]= new TH2F(hname,htitle, 100, 0.0, 4.0, 100, -5.0, 5.0);
    htitle+="Minus"; hname+="Minus";
    fHistQAPID[2][j][1]= new TH2F(hname,htitle, 100, 0.0, 4.0, 100, -5.0, 5.0);
    hname  = "pidTPCNSigma";  hname+=parttypename;
    htitle = parttypename + " TPC NSigma vs. momentum";
    fHistQAPID[3][j][0] = new TH2F(hname,htitle, 100, 0.0, 4.0, 100, -5.0, 5.0);
    htitle+="Minus"; hname+="Minus";
    fHistQAPID[3][j][1] = new TH2F(hname,htitle, 100, 0.0, 4.0, 100, -5.0, 5.0);
    ...
    fHistQAPIDFail[0][j][0] = new TH2F(hname, htitle, 100, 0.0, 4.0, 250, 0.0, 500.0);
    htitle+="Minus"; hname+="Minus";
    fHistQAPIDFail[0][j][1] = new TH2F(hname, htitle, 100, 0.0, 4.0, 250, 0.0, 500.0);
    hname  = "pidTOFTimeFail";  hname+=parttypename;
    htitle = parttypename + " TOF Time vs. momentum Fail";
    fHistQAPIDFail[1][j][0] = new TH2F(hname, htitle, 100, 0.1, 4.0, 400, -4000.0, 4000.0);
    htitle+="Minus"; hname+="Minus";
    fHistQAPIDFail[1][j][1] = new TH2F(hname, htitle, 100, 0.1, 4.0, 400, -4000.0, 4000.0);
    hname  = "pidTOFNSigmaFail";  hname+=parttypename;
    htitle = parttypename + " TOF NSigma vs. momentum Fail";
    fHistQAPIDFail[2][j][0]= new TH2F(hname,htitle, 100, 0.0, 4.0, 100, -5.0, 5.0);
    htitle+="Minus"; hname+="Minus";
    fHistQAPIDFail[2][j][1]= new TH2F(hname,htitle, 100, 0.0, 4.0, 100, -5.0, 5.0);
    hname  = "pidTPCNSigmaFail";  hname+=parttypename;
    htitle = parttypename + " TPC NSigma vs. momentum Fail";
    fHistQAPIDFail[3][j][0] = new TH2F(hname,htitle, 100, 0.0, 4.0, 100, -5.0, 5.0);
    htitle+="Minus"; hname+="Minus";
    fHistQAPIDFail[3][j][1] = new TH2F(hname,htitle, 100, 0.0, 4.0, 100, -5.0, 5.0);
...
{ 
	...
	//NsigmaTPCle250(2.0);
	//NsigmaTPC250_400(2.0);
	//NsigmaTPC400_450(1.0);
	//NsigmaTPC450_500(3.0);
	//NsigmaTOF450_500(2.0);
	//NsigmaTOF450_500(true);
	//NsigmaTPCge500(3.0);
	//NsigmaTOF500_800(2.0);
	//NsigmaTOF800_1000(1.5);
	//NsigmaTOFge1000(1.0);
	bool fUseNsigmaTOF450_500 = true;
  if(mom<0.25)
    {
      if(TMath::Abs(nsigmaTPCK)< 2.0)//fNsigmaTPCle250
	{ 
	  return true;
	} 
      else 
  ...
  if(mom>=0.25 && mom<0.4)
    {
      if(TMath::Abs(nsigmaTPCK)< 2.0)//fNsigmaTPC250_400
	{ 
	  return true;
	} 
      else 
  ...
  if(mom>=0.4 && mom<0.45)
    {
      if(TMath::Abs(nsigmaTPCK)< 1.0)//fNsigmaTPC400_450
	{ 
	  return true;
	} 
      else 
 ...
 if(mom>=0.45 && mom<0.5 && !fUseNsigmaTOF450_500)
    {
      if(TMath::Abs(nsigmaTPCK)< 3.0)//fNsigmaTPC450_500
	{ 
	  return true;
	} 
      else 
...
/////////////  
 ...
 if(fUseNsigmaTOF450_500)
   {
     if(mom>=0.45 && mom<0.5)
       {
	 if(TMath::Abs(nsigmaTOFK)< 2.0 /*fNsigmaTOF450_500*/ && TMath::Abs(nsigmaTPCK)< 3.0)//fNsigmaTPC450_500) 
	   {
	     return true;
	   }
	 else
  ...
  if(mom>=0.5 && mom<0.8)
    {
      if(TMath::Abs(nsigmaTOFK)< 2.0 /*fNsigmaTOF500_800*/ && TMath::Abs(nsigmaTPCK)< 3.0)//fNsigmaTPCge500 
	{
  ...
  if(mom>=0.8 && mom<1.0)
    {
      if(TMath::Abs(nsigmaTOFK)< 1.5 /*fNsigmaTOF800_1000*/ && TMath::Abs(nsigmaTPCK)< 3.0)//fNsigmaTPCge500 
	{
 ...
 if(mom>=1.0)
    {
      if(TMath::Abs(nsigmaTOFK)< 1.0/*fNsigmaTOFge1000*/ && TMath::Abs(nsigmaTPCK)< 3.0)//fNsigmaTPCge500 
	{
...
void AliAnalysisTaskParticleEfficiencyMezonPhiDR::UserExec(Option_t *)
{
  AliAODInputHandler *aodH = dynamic_cast<AliAODInputHandler *>(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler());
...
//***************************************************
 ...
 //**** getting MC array ******
			...
			if(IsKaonNSigmaNCustom(track2->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3])){
				if(track->Charge() > 0){
					if(track2->Charge() < 0){
							...
							fMCPhiThatAreReconstructed->Fill(pt,minv);
							}
						}
  ...
  for (Int_t i = 0; i < aodEvent->GetNumberOfV0s(); i++)
  {
    ...
    if(aodv0->Pt() < 0.2 || aodv0->Pt() > 5) continue;
    fCutsLambda->Fill(cutLam++);
    fCutsAntiLambda->Fill(cutALam++);
	  ...
	  // fMCPrimariesThatAreReconstructedNoNsigma[fcent*PARTTYPES+4][charge]->Fill(MCtrk->Eta(), MCtrk->Pt());
    	...
    	if (recoParticleArray[2].Contains(MCtrk)){ //Kaons
    	  if(PDGcode==321){
    	    ...
    	    if(MCtrk->GetPdgCode() == 321){
				fEtaPtKp->Fill(MCtrk->Eta(), MCtrk->Pt());
			}
    	    
    	    if(MCtrk->GetPdgCode() == -321){
				fEtaPtKm->Fill(MCtrk->Eta(), MCtrk->Pt());
			}
    	   for(Int_t jpart = 0; jpart < arrayMC->GetEntries(); jpart++){
		if(ipart != jpart){
			AliAODMCParticle *MCtrk2 = (AliAODMCParticle*)arrayMC->At(jpart);
...
//void AliAnalysisTaskParticleEfficiencyMezonPhiDR::Terminate(Option_t *)
//{}


AliAnalysisTaskParticleEfficiencyMezonPhiDR & AliAnalysisTaskParticleEfficiencyMezonPhiDR::operator=(const AliAnalysisTaskParticleEfficiencyMezonPhiDR &copy)
{
...
AliAnalysisTaskParticleEfficiencyMezonPhiDR::AliAnalysisTaskParticleEfficiencyMezonPhiDR(const AliAnalysisTaskParticleEfficiencyMezonPhiDR &copy)
{
  fPidMethod = copy.fPidMethod;
  fFB = copy.fFB;
...
void AliAnalysisTaskParticleEfficiencyMezonPhiDR::AnalyseCascades(int fcent, AliAODEvent* aodEvent, TClonesArray  *arrayMC)
{
...
void AliAnalysisTaskParticleEfficiencyMezonPhiDR::SetAliEventCuts(Bool_t ec)
{
  fIfAliEventCuts = ec;
}

void AliAnalysisTaskParticleEfficiencyMezonPhiDR::SetFB(int fb)
{
  fFB = fb;
}

void AliAnalysisTaskParticleEfficiencyMezonPhiDR::SetPidMethod(PidMethod method)
{
  fPidMethod = method;
}

int AliAnalysisTaskParticleEfficiencyMezonPhiDR::GetPidMethod()
{
  return (int)fPidMethod;
}

void AliAnalysisTaskParticleEfficiencyMezonPhiDR::SetPidMethod(int method)
{
  switch(method){
  ...
  case 3: fPidMethod=kExclusivePIDDiffRejection;
    break;
  case 4: fPidMethod=kNSigmaCustom;
  }

}


void AliAnalysisTaskParticleEfficiencyMezonPhiDR::SetMultMethod(EventMult method)
{
  fEstEventMult = method;
}

void AliAnalysisTaskParticleEfficiencyMezonPhiDR::SetIfXiAnalysis(Bool_t xi)
{
  fIfXiAnalysis = xi;
}
