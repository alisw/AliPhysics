...
ClassImp(AliAnalysisTaskParticleEff_DyDphi)
//ClassImp(AliAnalysisTaskParticleEff_DyDphi)

double fV1[3];
AliAnalysisTaskParticleEff_DyDphiDyDphi::AliAnalysisTaskParticleEff_DyDphi(TString name, int pidMethod, int filterbit) :
...
AliAnalysisTaskParticleEff_DyDphi::~AliAnalysisTaskParticleEff_DyDphi()
{
...
void AliAnalysisTaskParticleEff_DyDphi::UserCreateOutputObjects()
{
  ...
  if(fIfXiAnalysis){
    ...
    fyXim = new TH1D("fyXim", "y distribution of Xi minus", 500, -1.0, 1.);
    fPtXim = new TH1D("fPtXim", "Pt distribution of Xi minus", 500, 0.0, 8.);
    fyXip = new TH1D("fyXip", "y distribution of Xi plus", 500, -1.0, 1.);
    fPtXip = new TH1D("fPtXip", "Pt distribution of Xi plus", 500, 0.0, 8.);
    fHistoList->Add(fyXim);
    fHistoList->Add(fPtXim);
    fHistoList->Add(fyXip);
...
void AliAnalysisTaskParticleEff_DyDphi::UserExec(Option_t *)
{
  AliAODInputHandler *aodH = dynamic_cast<AliAODInputHandler *>(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler());
...
//***************************************************
 ...
 //**** getting MC array ******
  ...
  for (Int_t i = 0; i < aodEvent->GetNumberOfV0s(); i++)
  {
    ...
    if(Lambda)
      {
	fReconstructedAfterCuts[PARTTYPES*fcent+4][0]->Fill(aodv0->y(), aodv0->Pt());
      }
    if(AntiLambda)
      {
	fReconstructedAfterCuts[PARTTYPES*fcent+4][1]->Fill(aodv0->y(), aodv0->Pt());
      }
    //********************************
  ...
  for (Int_t ipart = 0; ipart < arrayMC->GetEntries(); ipart++)
	...
	{
	  fGeneratedMCPrimaries[fcent*PARTTYPES+5][charge]->Fill(MCtrk->y(), MCtrk->Pt());
	...
	if(fIfXiAnalysis)
	  if (recoParticleArrayXi->Contains(MCtrk)){ //Xi
	    if(PDGcode==3312){
	      fMCPrimariesThatAreReconstructed[fcent*PARTTYPES+5][charge]->Fill(MCtrk->y(), MCtrk->Pt());
	      Double_t val[] = {MCtrk->y(), MCtrk->Pt(), MCtrk->Zv() ,MCtrk->Phi()};
...
//void AliAnalysisTaskParticleEff_DyDphi::Terminate(Option_t *)
//{}


AliAnalysisTaskParticleEff_DyDphi & AliAnalysisTaskParticleEff_DyDphi::operator=(const AliAnalysisTaskParticleEff_DyDphi &copy)
{
...
AliAnalysisTaskParticleEff_DyDphi::AliAnalysisTaskParticleEff_DyDphi(const AliAnalysisTaskParticleEff_DyDphi &copy)
{
  fPidMethod = copy.fPidMethod;
  fFB = copy.fFB;
...
void AliAnalysisTaskParticleEff_DyDphi::AnalyseCascades(int fcent, AliAODEvent* aodEvent, TClonesArray  *arrayMC)
{
  ...
  for(Int_t i = 0; i < aodEvent->GetNumberOfCascades(); i++){
    ...
    double yXi = 0.5*TMath::Log((TMath::Sqrt(aodCascade->Ptot2Xi())+aodCascade->MomXiZ())/(TMath::Sqrt(aodCascade->Ptot2Xi())-aodCascade->MomXiZ()+1.e-13));
    ...
    if(aodCascade->ChargeXi() == -1){
      ...
      if(TMath::Abs(yXi) > 0.8) continue; fCutsXim->Fill(cutXim++);
      ...
      if(TMath::Abs(BachelorTrack->y()) > 0.8) continue; fCutsXim->Fill(cutXim++);
      ...
      if(TMath::Abs(aodCascade->y()) > 0.8) continue; fCutsXim->Fill(cutXim++);
      ...
      if(TMath::Abs(daughterTrackPos->y()) > 0.8) continue; fCutsXim->Fill(cutXim++);
      if(TMath::Abs(daughterTrackNeg->y()) > 0.8) continue; fCutsXim->Fill(cutXim++);
    ...
    if(aodCascade->ChargeXi() == 1){
      ...
      if(TMath::Abs(yXi) > 0.8) continue; fCutsXip->Fill(cutXip++);
      ...
      if(TMath::Abs(BachelorTrack->y()) > 0.8) continue; fCutsXip->Fill(cutXip++);
      ...
      if(TMath::Abs(aodCascade->y()) > 0.8) continue; fCutsXip->Fill(cutXip++);
      ...
      if(TMath::Abs(daughterTrackPos->y()) > 0.8) continue; fCutsXip->Fill(cutXip++);
      if(TMath::Abs(daughterTrackNeg->y()) > 0.8) continue; fCutsXip->Fill(cutXip++);
   ...
   //********* PID - Ximinus, Xiplus ********                                                                                                                 \
	...
	{
	  int charge = 0;//-1
	  fReconstructedAfterCuts[PARTTYPES*fcent+5][charge]->Fill(yXi, TMath::Sqrt(aodCascade->Pt2Xi()));
    ...
    if (!MCtrkMother->IsPhysicalPrimary() && Ximinus && pdgMother==3312) { //secondaries in lambdas                                                              \
      ...
      fReconstructedNotPrimaries[PARTTYPES*fcent+5][0]->Fill(yXi, TMath::Sqrt(aodCascade->Pt2Xi()));
    }
    else if(MCtrkMother->IsPhysicalPrimary() && Ximinus && pdgMother==3312) {
      fReconstructedPrimaries[PARTTYPES*fcent+5][0]->Fill(yXi, TMath::Sqrt(aodCascade->Pt2Xi()));
    }
    else if (!MCtrkMother->IsPhysicalPrimary() && Xiplus && pdgMother==-3312) { //secondaries in lambdas                                                         \

      fReconstructedNotPrimaries[PARTTYPES*fcent+5][1]->Fill(yXi, TMath::Sqrt(aodCascade->Pt2Xi()));
    }
    else if(MCtrkMother->IsPhysicalPrimary() && Xiplus && pdgMother==-3312) {
      fReconstructedPrimaries[PARTTYPES*fcent+5][1]->Fill(yXi, TMath::Sqrt(aodCascade->Pt2Xi()));
    }
...
void AliAnalysisTaskParticleEff_DyDphi::SetAliEventCuts(Bool_t ec)
{
  fIfAliEventCuts = ec;
}

void AliAnalysisTaskParticleEff_DyDphi::SetFB(int fb)
{
  fFB = fb;
}

void AliAnalysisTaskParticleEff_DyDphi::SetPidMethod(PidMethod method)
{
  fPidMethod = method;
}

int AliAnalysisTaskParticleEff_DyDphi::GetPidMethod()
{
  return (int)fPidMethod;
}

void AliAnalysisTaskParticleEff_DyDphi::SetPidMethod(int method)
{
  switch(method){
...
void AliAnalysisTaskParticleEff_DyDphi::SetMultMethod(EventMult method)
{
  fEstEventMult = method;
}

void AliAnalysisTaskParticleEff_DyDphi::SetIfXiAnalysis(Bool_t xi)
{
  fIfXiAnalysis = xi;
}

void AliAnalysisTaskParticleEff_DyDphi::SetIfTrackPileUp(Bool_t ifTrackPlp)
{
  fTrackPileUpRemoval = ifTrackPlp;
}

void AliAnalysisTaskParticleEff_DyDphi::SetV0PileUpRemoval(Bool_t v0PileUpRemoval)
{
