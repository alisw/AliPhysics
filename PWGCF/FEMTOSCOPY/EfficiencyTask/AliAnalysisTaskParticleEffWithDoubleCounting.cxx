...
#include "AliAnalysisTaskParticleEffWithDoubleCounting.h"




ClassImp(AliAnalysisTaskParticleEffWithDoubleCounting)
//ClassImp(AliAnalysisTaskParticleEffWithDoubleCounting)
...
AliAnalysisTaskParticleEffWithDoubleCounting::AliAnalysisTaskParticleEffWithDoubleCounting(TString name, int pidMethod, int filterbit) :
...
AliAnalysisTaskParticleEffWithDoubleCounting::~AliAnalysisTaskParticleEffWithDoubleCounting()
{
...
void AliAnalysisTaskParticleEffWithDoubleCounting::UserCreateOutputObjects()
{
  ...
  for(Int_t j = 0; j < PARTTYPES; j++)  {
    ...
    for(Int_t i = 0; i < MULTBINS; i++)  {
      ...
      Double_t min[]={-1.8,0,-10,0};
      Double_t max[]={1.8,10,10,2*TMath::Pi()};
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsPionNSigma(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime)
{
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsPionNSigmaV0(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsPionNSigmaV0TPC5(float mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsProtonNSigmaV0TPC5(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsPionNSigma3(float mom, float nsigmaTPCPi, float nsigmaTOFPi, float TOFtime)
{
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsKaonNSigma(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
    if (mom > 0.5) {
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsKaonNSigma3(float mom, float nsigmaTPCK, float nsigmaTOFK, float TOFtime)
{
  if (mom > 0.5) {
    //rejection of unwanted contamination
    // if(mom>1 && TOFtime<-400)
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsProtonNSigma(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsProtonNSigmaV0(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsProtonNSigma3(float mom, float nsigmaTPCP, float nsigmaTOFP, float TOFtime)
{
  if (mom > 0.5) {
    // if(mom>1.8 && TOFtime<-300)
    //   return false;
...
bool AliAnalysisTaskParticleEffWithDoubleCounting::IsElectron(float nsigmaTPCE, float nsigmaTPCPi,float nsigmaTPCK, float nsigmaTPCP)
{
...
void AliAnalysisTaskParticleEffWithDoubleCounting::UserExec(Option_t *)
{
  AliAODInputHandler *aodH = dynamic_cast<AliAODInputHandler *>(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler());
 ...
 //**** getting MC array ******
  ...
  for (Int_t iTracks = 0; iTracks < aodEvent->GetNumberOfTracks(); iTracks++)
  {
    ...
    //******** With double counting *******************
      isPionNsigma = (IsPionNSigma3(track->Pt(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]));
      isKaonNsigma = (IsKaonNSigma3(track->Pt(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]));
      isProtonNsigma = (IsProtonNSigma3(track->Pt(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]));
    }
...
//void AliAnalysisTaskParticleEffWithDoubleCounting::Terminate(Option_t *)
//{}


AliAnalysisTaskParticleEffWithDoubleCounting & AliAnalysisTaskParticleEffWithDoubleCounting::operator=(const AliAnalysisTaskParticleEffWithDoubleCounting &copy)
{
...
AliAnalysisTaskParticleEffWithDoubleCounting::AliAnalysisTaskParticleEffWithDoubleCounting(const AliAnalysisTaskParticleEffWithDoubleCounting &copy)
{
  fPidMethod = copy.fPidMethod;
  fFB = copy.fFB;
...
void AliAnalysisTaskParticleEffWithDoubleCounting::AnalyseCascades(int fcent, AliAODEvent* aodEvent, TClonesArray  *arrayMC)
{
  ...
  for(Int_t i = 0; i < aodEvent->GetNumberOfCascades(); i++){
    ...
    if(aodCascade->ChargeXi() == -1){
      if(!aodCascade) continue;
      fCutsXim->Fill(cutXim++);
      if(TMath::Sqrt(aodCascade->Pt2Xi()) < 0.5) continue;
      fCutsXim->Fill(cutXim++);
      if(TMath::Abs(etaXi) > 0.8) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->DecayLengthXi(fV1[0],fV1[1],fV1[2]) > 100) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->DcaXiDaughters() > 1.6) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->CosPointingAngleXi(fV1[0],fV1[1],fV1[2]) < 0.97) continue;
      fCutsXim->Fill(cutXim++);
      if(radiusXi<0.8 || radiusXi>200) continue;
      fCutsXim->Fill(cutXim++);

      //*****Bachelor*********************
      if(BachelorTrack == NULL) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->DcaBachToPrimVertex() < 0.05) continue;
      fCutsXim->Fill(cutXim++);
      if(TMath::Abs(BachelorTrack->Eta()) > 0.8) continue;
      fCutsXim->Fill(cutXim++);
      if(BachelorTrack->GetTPCNcls() < 70) continue;
      fCutsXim->Fill(cutXim++);
      if(!(BachelorTrack->GetStatus() & (AliESDtrack::kTPCrefit))) continue;
      fCutsXim->Fill(cutXim++);

      //*****Lambda***********************
      if(aodCascade->CosPointingAngle(fV1) < 0.97) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->DcaV0ToPrimVertex() < 0.07) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->DcaV0Daughters() > 1.6) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->MassLambda() < (LambdaMass-0.005) || aodCascade->MassLambda() > (LambdaMass+0.005)) continue;
      fCutsXim->Fill(cutXim++);
      if(TMath::Abs(aodCascade->Eta()) > 0.8) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->DecayLength(fV1) > 100) continue;  
      fCutsXim->Fill(cutXim++);
      if(aodCascade->RadiusV0()<1.4 || aodCascade->RadiusV0()>200) continue;
      fCutsXim->Fill(cutXim++);

      //*****Lambda Daughters*************
      if(daughterTrackPos == NULL || daughterTrackNeg == NULL )continue;
      fCutsXim->Fill(cutXim++);
      if (daughterTrackNeg->Charge() == daughterTrackPos->Charge()) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->DcaPosToPrimVertex() < 0.04) continue;
      fCutsXim->Fill(cutXim++);
      if(aodCascade->DcaNegToPrimVertex() < 0.04) continue;
      fCutsXim->Fill(cutXim++);
      if(TMath::Abs(daughterTrackPos->Eta()) > 0.8) continue;
      fCutsXim->Fill(cutXim++);
      if(TMath::Abs(daughterTrackNeg->Eta()) > 0.8) continue;
      fCutsXim->Fill(cutXim++);
      if(daughterTrackPos->Pt()<0.3 || daughterTrackPos->Pt()>99) continue;
      fCutsXim->Fill(cutXim++);
      if(daughterTrackNeg->Pt()<0.3 || daughterTrackNeg->Pt()>99) continue;
      fCutsXim->Fill(cutXim++);
      if(daughterTrackPos->GetTPCNcls() < 70) continue;
      fCutsXim->Fill(cutXim++);
      if(daughterTrackNeg->GetTPCNcls() < 70) continue;
      fCutsXim->Fill(cutXim++);
    ...
    if(aodCascade->ChargeXi() == 1){
      ...
      if(aodCascade->DcaV0Daughters() > 1.6) continue;
      fCutsXip->Fill(cutXip++);
      if(aodCascade->MassAntiLambda() < (LambdaMass-0.005) || aodCascade->MassAntiLambda() > (LambdaMass+0.005)) continue;
      fCutsXip->Fill(cutXip++);
      if(TMath::Abs(aodCascade->Eta()) > 0.8) continue;
      fCutsXip->Fill(cutXip++);
...
void AliAnalysisTaskParticleEffWithDoubleCounting::SetAliEventCuts(Bool_t ec)
{
  fIfAliEventCuts = ec;
}

void AliAnalysisTaskParticleEffWithDoubleCounting::SetFB(int fb)
{
  fFB = fb;
}

void AliAnalysisTaskParticleEffWithDoubleCounting::SetPidMethod(PidMethod method)
{
  fPidMethod = method;
}

int AliAnalysisTaskParticleEffWithDoubleCounting::GetPidMethod()
{
  return (int)fPidMethod;
}

void AliAnalysisTaskParticleEffWithDoubleCounting::SetPidMethod(int method)
{
  switch(method){
...
void AliAnalysisTaskParticleEffWithDoubleCounting::SetMultMethod(EventMult method)
{
  fEstEventMult = method;
}

void AliAnalysisTaskParticleEffWithDoubleCounting::SetIfXiAnalysis(Bool_t xi)
{
  fIfXiAnalysis = xi;
}

void AliAnalysisTaskParticleEffWithDoubleCounting::SetIfTrackPileUp(Bool_t ifTrackPlp)
{
  fTrackPileUpRemoval = ifTrackPlp;
}

void AliAnalysisTaskParticleEffWithDoubleCounting::SetV0PileUpRemoval(Bool_t v0PileUpRemoval)
{
