...
void AliAnalysisTaskParticleEffpbpb5020::UserCreateOutputObjects()
{
  ...
  for(Int_t j = 0; j < PARTTYPES; j++)  {
    ...
    hna = "pidTOFNSigmaFail";  hna+=parttypename;
    htit = parttypename + " TOF NSigma vs. momentum Fail";
    ...
    hna = "pidTPCNSigmaFail";  hna+=parttypename;
    htit = parttypename + " TPC NSigma vs. momentum Fail";
    ...
    hna = "pidTPCTOFNSigmaFail";  hna+=parttypename;
    htit = parttypename + " TPC vs TOF NSigma Fail";
...
void AliAnalysisTaskParticleEffpbpb5020::UserExec(Option_t *)
{
  ...
  //  UInt_t filterBit = 1;//(1 << 0);
   ...
   //for FB 128 - tpc only tracks
    ...
    isPionNsigma = (IsPionNSigma(track->P(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2]) && !IsKaonNSigma(track->P(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma(track->P(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4]) && !IsDeuteronNSigma(track->P(), nSigmaTPCD,nSigmaTOFD));
    isKaonNsigma = (!IsPionNSigma(track->P(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && IsKaonNSigma(track->P(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && !IsProtonNSigma(track->P(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4])  && !IsDeuteronNSigma(track->P(), nSigmaTPCD,nSigmaTOFD));
    isProtonNsigma = (!IsPionNSigma(track->P(),nSigmaTPCPi, nSigmaTOFPi, tTofSig-pidTime[2])  && !IsKaonNSigma(track->P(),nSigmaTPCK, nSigmaTOFK, tTofSig-pidTime[3]) && IsProtonNSigma(track->P(),nSigmaTPCP, nSigmaTOFP, tTofSig-pidTime[4])  && !IsDeuteronNSigma(track->P(), nSigmaTPCD,nSigmaTOFD));
