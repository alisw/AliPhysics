AliAnalysisTaskKPFemto *AddTaskKPFemto ( Bool_t krunMCtruth  = kFALSE,
					 Int_t year = 2010,
					 //Bool_t krunMCtruth  = kTRUE,
					 Bool_t kusecontainer = kFALSE, 
					 //TString CollidingSystem = "PbPb",
					 TString CollidingSystem = "pp",
					 ...
					 Int_t  fistpart = 3, //2 pi 3 ka //4 p 
					 Int_t  secondpart = 4,
					 ...
					 Float_t maxptforprim = 1.5,     // 3.  for protons; 1.pions ;2. for Ka //sharp cuts 1.4 for K and 1.8 for p
					 Float_t minptforsec = 0.2,      //
					 Float_t maxptforsec = 3.0,  
  ...
  if (!mgr) {
    Error("AddTaskKPFemto", "No analysis manager found.");
    return 0;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskKPFemto", "This task requires an input event handler");
    return NULL;
  }
  ...
  combinedName.Form("TaskKPFemto%s%s", outlistsuffix, outlistsubwagon);
  AliAnalysisTaskKPFemto *task = new AliAnalysisTaskKPFemto(combinedName);
  ...
  if(!task) {
            Error("AddTaskKPFemto","AliAnalysisTaskKPFemto not created!");
            return;
  }
