...
AliAnalysisEfficiencyPbPbNodoublecount *AddTaskEfficiencyPbPbPID1(TString containerName="femtolist",int method=1, int filterbit=128)
{
  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskEfficiencyPbPbPID1", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskEfficiencyPbPbPID1", "This task requires an input event handler");
    return NULL;
  }
...
//   }
 ...
 // taskEfficiencyFB128->SetPidMethod(method);
  ...
  mgr->AddTask(taskEffPbPbNodoublecount);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEffPbPbNodoublecount, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEffPbPbNodoublecount, 1, cout_femto);
   ...
   return taskEffPbPbNodoublecount;
}
