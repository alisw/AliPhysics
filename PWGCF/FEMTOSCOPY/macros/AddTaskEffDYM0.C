...
AliAnalysisTaskParticleEffDYM0 *AddTaskEffDYM0(TString containerName="femtolist",int method=3, int filterbit=96)
{
  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskEffDYM0", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskEffDYM0", "This task requires an input event handler");
    return NULL;
  }
...
//   }
  ...
  AliAnalysisTaskParticleEffDYM0 *taskEffM0 = new AliAnalysisTaskParticleEffDYM0("EffTaskM0",method,filterbit);
  //taskEff->SetPidMethod(method);
  //taskEff->SetFB(filterbit);
  mgr->AddTask(taskEffM0);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEffM0, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEffM0, 1, cout_femto);
