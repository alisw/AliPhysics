...
AliAnalysisTaskParticleEffDYM2 *AddTaskEffDYM2(TString containerName="femtolist",int method=3, int filterbit=96)
{
  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskEffDYM2", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskEffDYM2", "This task requires an input event handler");
    return NULL;
  }
...
//   }
  ...
  AliAnalysisTaskParticleEffDYM2 *taskEffM2 = new AliAnalysisTaskParticleEffDYM2("EffTaskM2",method,filterbit);
  //taskEff->SetPidMethod(method);
  //taskEff->SetFB(filterbit);
  mgr->AddTask(taskEffM2);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEffM2, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEffM2, 1, cout_femto);
