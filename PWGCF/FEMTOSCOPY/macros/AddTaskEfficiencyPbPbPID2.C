...
AliAnalysisEfficiencyPbPbExclusivePID *AddTaskEfficiencyPbPbPID2(TString containerName="femtolist",int method=2, int filterbit=128)
{
  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskEfficiencyPbPbPID2", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskEfficiencyPbPbPID2", "This task requires an input event handler");
    return NULL;
  }
...
//   }
  ...
  AliAnalysisEfficiencyPbPbExclusivePID *taskEfficiencyPbPbExclsuivePID = new AliAnalysisEfficiencyPbPbExclusivePID("EffTaskPbPb_pid2",method,filterbit);
 // taskEfficiencyFB128->SetPidMethod(method);
  //taskEfficiencyFB128->SetFB(filterbit);
  mgr->AddTask(taskEfficiencyPbPbExclsuivePID);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEfficiencyPbPbExclsuivePID, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEfficiencyPbPbExclsuivePID, 1, cout_femto);
   ...
   return taskEfficiencyPbPbExclsuivePID;
}
