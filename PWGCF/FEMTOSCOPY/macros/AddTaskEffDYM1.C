...
AliAnalysisTaskParticleEffDYM1 *AddTaskEffDYM1(TString containerName="femtolist",int method=3, int filterbit=96)
{
  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskEffDYM1", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskEffDYM1", "This task requires an input event handler");
    return NULL;
  }
...
//   }
  ...
  AliAnalysisTaskParticleEffDYM1 *taskEffM1 = new AliAnalysisTaskParticleEffDYM1("EffTaskM1",method,filterbit);
  //taskEff->SetPidMethod(method);
  //taskEff->SetFB(filterbit);
  mgr->AddTask(taskEffM1);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEffM1, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEffM1, 1, cout_femto);
