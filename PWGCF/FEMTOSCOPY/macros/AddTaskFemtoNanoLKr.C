...
AliAnalysisTaskSE *AddTaskFemtoNanoLKr(bool isMC = true,		//2
                   ...
                   int CutKaon = 0, ///7 decide which cut use (Ramona s or Oton s)
									 const char *sTcut = "1", ///6 for the sphericity cuts. If it s 1, it does the sphericity cuts
  ...
  AliFemtoDreamTrackCuts *TrackPosKaonCuts = AliFemtoDreamTrackCuts::PrimKaonCuts(
  ...
  AliFemtoDreamTrackCuts *TrackNegKaonCuts =
      AliFemtoDreamTrackCuts::PrimKaonCuts(isMC, true, false, true);
...
const float KaonPtMax = 999;
  ...
  if (suffix != "0") {
    if (suffix == "1") {  ///depending on the value of "suffix", we have a different
		///random combination of kaons and llambdas
  ...
  AliAnalysisTaskNanoLKr* task = new AliAnalysisTaskNanoLKr(
		"AliAnalysisTaskNanoLKr",isMC);
///very important
