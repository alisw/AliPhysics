...
AliAnalysisEffTaskPbPbDRMultDY4090 *AddTaskEffMDR4090(TString containerName="femtolist",int method=3, int filterbit=128)
{
  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskEffMDR4090", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskEffMDR4090", "This task requires an input event handler");
    return NULL;
  }
...
//   }
  ...
  AliAnalysisEffTaskPbPbDRMultDY4090 *taskEffMDR4090 = new AliAnalysisEffTaskPbPbDRMultDY4090("EffTaskMDR4090",method,filterbit);
  taskEffMDR4090->SetPidMethod(method);
  taskEffMDR4090->SetFB(filterbit);
  mgr->AddTask(taskEffMDR4090);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEffMDR4090, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEffMDR4090, 1, cout_femto);
