...
AliAnalysisTaskEffK0ss_pp *AddTaskEffK0ss_pp(TString containerName="femtolist",int method=3, int filterbit=96)
{
...
//   }
  ...
  taskEffK0spp->SetPidMethod(method);
  taskEffK0spp->SetFB(filterbit);
  mgr->AddTask(taskEffK0spp);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEffK0spp, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEffK0spp, 1, cout_femto);
