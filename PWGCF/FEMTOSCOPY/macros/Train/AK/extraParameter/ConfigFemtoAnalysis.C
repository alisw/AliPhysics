...
//dodany kolejny parametr (21) - minpT
...
AliFemtoManager* ConfigFemtoAnalysis(const char* params) {
	...
	if(strlen(params)!=0)
	  {
	    ...
	    parameter[21] = strtok(NULL, ",");
	    cout<<"Parameter [21]: (min pT)"<<parameter[21]<<" "<<endl; //dodany parametr - dolny zakres pedu
	  }
	...
	double minPt = atof(parameter[21]); //nowy parametr
	...
	Reader->SetUseMultiplicity(AliFemtoEventReaderAOD::kReference); //zmiana z kCentrality
				...
				if (runch[ichg])
				{
					...
					*/
					...
					if (ichg == 0 || ichg == 1 ||ichg == 2) //protons
					  {
					    dtc1etaphitpc[aniter]->SetPt(minPt,maxPt); 
					...
					if (ichg == 0 || ichg == 1 ||ichg == 2) //protons
					  {
					    dtc2etaphitpc[aniter]->SetPt(minPt,maxPt); 
					...
					if (ichg == 12)//all
					{
						...
						dtc3etaphitpc[aniter]->SetPt(minPt,maxPt);
					}
