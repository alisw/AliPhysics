...
//int year=2015;
  ...
  if(year==2015)
  {
    ...
    if (pileup ==1) Reader2015->SetTrackPileUpRemoval(kTRUE);
    else Reader2015->SetTrackPileUpRemoval(kFALSE);
  ...
  */
  ...
  for (int imult = 0; imult < numOfMultBins; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  dtc2etaphitpc[aniter]->SetNsigmaRejection(2.0);

	  //DCA 

	  if(dca==1){
             dtc1etaphitpc[aniter]->SetMaxImpactXY(dcaMaxXY1);     //DCA xy - parametry zalezno?ci DCA od pt
             dtc1etaphitpc[aniter]->SetMaxImpactZ(dcaMaxZ1);    //DCA Z
             dtc2etaphitpc[aniter]->SetMaxImpactXY(dcaMaxXY2);     //DCA xy
             dtc2etaphitpc[aniter]->SetMaxImpactZ(dcaMaxZ2);    //DCA Z
          }

	  //clusters
          dtc1etaphitpc[aniter]->SetminTPCncls(nClusters);
          dtc2etaphitpc[aniter]->SetminTPCncls(nClusters);
	  ...
	  dtc2etaphitpc[aniter]->SetMostProbable(21); //Proton (Nsigma + Rejection) , TPC+TOF
	  ...
	  cutpassbetatcutmonitor[aniter] = new AliFemtoCutMonitorPairBetaT(Form("cutPass%stpcM%i", chrgs[ichg], imult), 100, 0.0, 1.0, KaonMass, ProtonMass);
	  cutfailbetatcutmonitor[aniter] = new AliFemtoCutMonitorPairBetaT(Form("cutFail%stpcM%i", chrgs[ichg], imult), 100, 0.0, 1.0, KaonMass, ProtonMass);
	  ...
	  if (runbetatdep) {
	    ...
	    for (int ikt=0; ikt<numOfkTbins; ikt++) {
	      ...
	      ktpcuts[ktm] = new AliFemtoBetaTPairCut(ktrng[ikt], ktrng[ikt + 1], KaonMass, ProtonMass);
