...
AliFemtoEventReaderAODMultSelection* GetReader2015(bool mcAnalysis)
{
 ...
 // Reader->SetUseMVPlpSelection(kTRUE);  
  ...
  Reader->Set15oPass2EventReject(3);
  Reader->SetPreCentralityCut(0,501);

  if(mcAnalysis) Reader->SetReadMC(kFALSE);
...
//_
AliFemtoManager* ConfigFemtoAnalysis(int runcentrality0, int runcentrality1, int runcentrality2, int runSHCorrFctn, int runNonIdCorrFctn, int paircutantigammaon, int paircutmergedfractionon, double distance, double fraction1, int runDPhiStarKStarMergedFraction, int runDPhiStarKStarAverageMergedPointsFraction, int runDPhiStarDEta, int turnOnMonitors, int turnOnBetaTMonitor, int runbetatdep, int runbetatylm, int runbetatnonid,int year) {
  ...
  const int numOfMultBins = 3;  
  const int numOfChTypes = 4;
  const int numOfkTbins = 2;

  int runmults[numOfMultBins] = {runcentrality0, runcentrality1, runcentrality2};
  int multbins[numOfMultBins + 1] = {0, 100, 300, 500};
  
  int runch[numOfChTypes] = {1, 1, 1, 1};
  const char *chrgs[numOfChTypes] = { "PpDp", "PmDm", "PpDm","PmDp"};
...
//int year=2015;
  ...
  */
  ...
  AliFemtoMJTrackCut            *dtc1etaphitpc[size];
  AliFemtoWRzTrackCut           *dtc2etaphitpc[size];
  ...
  for (int imult = 0; imult < numOfMultBins; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  anetaphitpc[aniter] = new AliFemtoVertexMultAnalysis(6, -9.0, 9.0, 4, multbins[imult], multbins[imult+1]);//10,-10,10,4
	  ...
	  mecetaphitpc[aniter]->SetVertZPos(-9,9);
	  ...
	  dtc1etaphitpc[aniter]->SetNsigmaRejection(2.0);

	  //Basic track cut for deuterons
	  dtc2etaphitpc[aniter] = new AliFemtoWRzTrackCut();
	  dtc2etaphitpc[aniter]->SetNsigmaTPCTOF(true);
	//  dtc2etaphitpc[aniter]->SetNsigmaTPConly(true);
	  dtc2etaphitpc[aniter]->SetdEdxcut(true);
	//  dtc2etaphitpc[aniter]->SetBasicSel(false);
	  dtc2etaphitpc[aniter]->SetNsigma(2);
	//  dtc2etaphitpc[aniter]->SetNsigmaRejection(3);  //0.7 - zwykły cut na mase, -1 strict low pt, 2 3 wercje sigm
	  dtc2etaphitpc[aniter]->SetMaxmom(2.2);
	...
	//  anetaphitpc[aniter]->SetReverseParticleVariables(true);
	  ...
	  if(turnOnMonitors == 1) {
	    cutPass1PIDetaphitpc[aniter] = new AliFemtoCutMonitorParticlePIDBeta(Form("cutPass1%stpcM%i", chrgs[ichg], imult),2);//0-pion,1-kaon,2-proton
	    cutFail1PIDetaphitpc[aniter] = new AliFemtoCutMonitorParticlePIDBeta(Form("cutFail1%stpcM%i", chrgs[ichg], imult),2);
	  ...
	  if(turnOnMonitors == 1) {
	    cutPass2PIDetaphitpc[aniter] = new AliFemtoCutMonitorParticlePIDBeta(Form("cutPass2%stpcM%i", chrgs[ichg], imult),3);//0-pion,1-kaon,2-proton
	    cutFail2PIDetaphitpc[aniter] = new AliFemtoCutMonitorParticlePIDBeta(Form("cutFail2%stpcM%i", chrgs[ichg], imult),3);
	  ...
	  if (paircutmergedfractionon == 1) 
	    sqpcetaphitpc[aniter] = new AliFemtoPairCutMergedFraction(distance, fraction1, 0.005, 0.8, 2.5);
	  else
	  ...
	  cutpassbetatcutmonitor[aniter] = new AliFemtoCutMonitorPairBetaT(Form("cutPass%stpcM%i", chrgs[ichg], imult), 100, 0.0, 1.0, ProtonMass, DeuteronMass);
	  cutfailbetatcutmonitor[aniter] = new AliFemtoCutMonitorPairBetaT(Form("cutFail%stpcM%i", chrgs[ichg], imult), 100, 0.0, 1.0, ProtonMass, DeuteronMass);
	  ...
	  if(runDPhiStarKStarMergedFraction == 1) {
	    dphistarkstarmftpc[aniter] = new AliFemtoCorrFctnDPhiStarKStarMergedFraction(Form("cdphistarkstarmergedfraction%stpcM%iD%lfF%lf", chrgs[ichg], imult, distance, fraction1), 0.8, 2.5, distance, fraction1, 0.005, 51, 0.0, 0.5, 127, -1.0, 1.0);
	    anetaphitpc[aniter]->AddCorrFctn(dphistarkstarmftpc[aniter]);
	  }
	  
	  if(runDPhiStarKStarAverageMergedPointsFraction == 1) {
	    dphistarkstarampftpc[aniter] = new AliFemtoCorrFctnDPhiStarKStarAverageMergedPointsFraction(Form("cdphistarkstaraveragemergedpointsfraction%stpcM%iD%lf", chrgs[ichg], imult, distance), 0.8, 2.5, distance, 0.005, 51, 0.0, 0.5, 127, -0.5, 0.5);
	  ...
	  if (runbetatdep) {
	    ...
	    for (int ikt=0; ikt<numOfkTbins; ikt++) {
	      ...
	      ktpcuts[ktm] = new AliFemtoBetaTPairCut(ktrng[ikt], ktrng[ikt + 1], ProtonMass, DeuteronMass);
	      
	      if (runbetatylm) {
		cylmkttpc[ktm] = new AliFemtoCorrFctnDirectYlm(Form("cylm%stpcM%iD%lfF%lfbetat%d", chrgs[ichg], imult, distance, fraction1, ikt),1,nbinssh, 0.0,shqmax, runshlcms);
