          ...
          for (int iepvzero = 0; iepvzero < numOfEPvzero; iepvzero++) {
            if (runepvzero[iepvzero]) {
              ...
              if (runtype == 0) {
               ...
               sqpcetaphitpc[aniter]->SetMinimumRadius(1.2); // biggest inefficiency for R=1.1 m (checked on small sample)
               sqpcetaphitpc[aniter]->SetPhiStarMin(kFALSE); // biggest inefficiency for R=1.1 m (checked on small sample)
