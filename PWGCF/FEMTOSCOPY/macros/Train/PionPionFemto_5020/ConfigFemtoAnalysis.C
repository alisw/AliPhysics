...
AliFemtoManager* ConfigFemtoAnalysis(int runcentrality0, int runcentrality1, int runcentrality2, int runcentrality3, int runcentrality4, int runcentrality5, int runcentrality6, double Vz, int Vz_bin, double eta, double dcaxy, double dcaz, int ncls, int runSHCorrFctn, int runNonIdCorrFctn, int paircutantigammaon, int paircutmergedfractionon, double distance, double fraction1, int runDPhiStarKStarMergedFraction, int runDPhiStarKStarAverageMergedPointsFraction, int runDPhiStarDEta, int turnOnMonitors, int turnOnBetaTMonitor, int runbetatdep, int runbetatylm, int runbetatnonid, int lmax, int no_mix_event, int ispileup, int trackpileup, double pTmin, double pTmax, int kt1, int kt2, int iskt, int isbeta, int PRF) {
//_
...
//if (iskt==1){	
  ...
  AliFemtoCorrFctnPtKstar       *ckstarpt[size*numOfkTbins];
  AliFemtoCorrFctn3DLCMSSym     *cq3dlcmskttpc[size*numOfkTbins];  
  //AliFemtoCorrFctnQLCMS         *cq3dlcmskttpc[size*numOfkTbins];  
  AliFemtoKtBinnedCorrFunc      *kt_binned_cfs[size*numOfkTbins];
  ...
  for (int imult = 0; imult < numOfMultBins; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  cutpassbetatcutmonitor[aniter] = new AliFemtoCutMonitorPairBetaT(Form("cutPass%stpcM%i", chrgs[ichg], imult), 100, 0.0, 1.0, PionMass, PionMass);
	  cutfailbetatcutmonitor[aniter] = new AliFemtoCutMonitorPairBetaT(Form("cutFail%stpcM%i", chrgs[ichg], imult), 100, 0.0, 1.0, PionMass, PionMass);
	  ...
	  /*	    ckstarpt[aniter] = new AliFemtoCorrFctnPtKstarBeta(Form("ckstarpT%stpcM%i", chrgs[ichg], imult));	    
	    ckstarpt[aniter]->SetBtPtKStart(kTRUE);
	    anetaphitpc[aniter]->AddCorrFctn(ckstarpt[aniter]);
	  */
	  ...
	  ckstarpt[aniter] = new AliFemtoCorrFctnPtKstar(Form("ckstarpT%stpcM%i", chrgs[ichg], imult));
	  anetaphitpc[aniter]->AddCorrFctn(ckstarpt[aniter]);


	  if(PRF==0){
       	  kt_binned_cfs[aniter] = new AliFemtoKtBinnedCorrFunc(Form("KT_Q3D_%s_M%i", chrgs[ichg], imult),  new AliFemtoCorrFctn3DLCMSSym("_q3d",60,0.3));
	  //kt_binned_cfs[aniter] = new AliFemtoKtBinnedCorrFunc(Form("KT_Q3D_%s_M%i", chrgs[ichg], imult),  new AliFemtoCorrFctnQLCMS("_q3d","",50,0.0,0.5));

	  for (int kt_idx=0; kt_idx<numOfkTbins; kt_idx+= 1) {
	    float low = ktrngkt[kt_idx], high = ktrngkt[kt_idx+1];
	    kt_binned_cfs[aniter]->AddKtRange(low, high);
	  }
	  anetaphitpc[aniter]->AddCorrFctn(kt_binned_cfs[aniter]);
	  }
	  ...
	  if (runbetatdep) {
	    ...
	    for (int ikt=0; ikt<numOfkTbins; ikt++) {
	     ...
	     if (isbeta==1){		    		    
	      ...
	      ktpcutsbeta[ktm] = new AliFemtoBetaTPairCut(ktrngbeta[ikt], ktrngbeta[ikt + 1], PionMass, PionMass);
	      
	     }

	      if (iskt==1){	
  	      ktpcutskt[ktm] = new AliFemtoPairkTPairCut(ktrngkt[ikt], ktrngkt[ikt + 1], PionMass, PionMass);
	      //ckstarpt[ktm] = new AliFemtoCorrFctnPtKstarBeta(Form("ckstarpT%stpcM%i", chrgs[ichg], imult));	    
	      //anetaphitpc[aniter]->AddCorrFctn(ckstarpt[ktm]);

	      //cq3dlcmskttpc[ktm] = new AliFemtoCorrFctn3DLCMSSym(Form("cq3d%stpcM%ikT%i", chrgs[ichg], imult, ikt),50,0.25);
	      //cq3dlcmskttpc[ktm] = new AliFemtoCorrFctnQLCMS(Form("pritam_cq3d%stpcM%ikT%i", chrgs[ichg], imult, ikt),"",50,0.0,0.5);
		cq3dlcmskttpc[ktm]->SetPairSelectionCut(ktpcutskt[ktm]);
