...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  int runpT[6] = {1, 1, 1, 1, 1};
  double pTbins[6] = {0.0, 1.32, 1.68, 2.056, 2.624, 5.0};  
  ...
  int runch[10] = {0, 0, 1, 0, 1, 1, 0, 0, 1, 0};
    ...
    if (runmults[imult]) {
	  ...
	  for (int ipT = 0; ipT < numOfpTBins; ipT++) {
		...
		if (runepvzero[iepvzero]) {
		  aniter = imult * numOfChTypes + ichg * numOfEPvzero + iepvzero + numOfpTBins;
		  ...
		  sqp1cetaphitpc[aniter]->SetMinAvgSeparation(0, 5); //proton-pion+
		  sqp1cetaphitpc[aniter]->SetMinAvgSeparation(1, 5); //proton-antiproton
		  sqp1cetaphitpc[aniter]->SetMinAvgSeparation(2, 0); //pion- - pion+
		  sqp1cetaphitpc[aniter]->SetMinAvgSeparation(3, 5); //antiproton - pion-
		  ...
		  if(ichg == 0) //V0LL
		    ...
		    {
		      sqp4cetaphitpc[aniter]->SetMinAvgSeparation(0, 5); //0 - track-pos, 1 - track-neg
		      sqp4cetaphitpc[aniter]->SetMinAvgSeparation(1, 5);
		      anetaphitpc[aniter]->SetV0SharedDaughterCut(performSharedDaughterCut);
		      anetaphitpc[aniter]->SetEventCut(mecetaphitpc[aniter]);
		      anetaphitpc[aniter]->SetFirstParticleCut(dtc1etaphitpc[aniter]);
		  ...
		  if(ichg==3 || ichg==4 || ichg==5 || ichg==6 || ichg==8) { //PL, APL, PAL, APAL, PAP
		    cnonidtpc[aniter] = new AliFemtoCorrFctnNonIdDR(Form("cnonid%stpcM%iPsi%ipTBin%i", chrgs[ichg], imult, iepvzero, ipT), nbinssh, 0.0,shqmax);
		    anetaphitpc[aniter]->AddCorrFctn(cnonidtpc[aniter]);
		  }
		  else {
		    cqinvtpc[aniter] = new AliFemtoQinvCorrFctn(Form("cqinv%stpcM%iPsi%ipTBin%i", chrgs[ichg], imult, iepvzero, ipT),nbinssh,0.0,shqmax);
