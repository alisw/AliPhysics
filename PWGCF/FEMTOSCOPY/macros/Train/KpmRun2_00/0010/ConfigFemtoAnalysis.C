 ...
 *********************************************************************
 *    K+K- in PbPb@5.02TeV Run2                                      *
 * Update: Konstantin.Mikhaylov@cern.ch                              *
 *         KpmRun2_00  0010  31-Jan-2019                             *
...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  //-------Single track cuts------------------------------------------------->
  //Set smaller DCA(0.3,0.3) due to FB8 (Hybrid?)
  ...
  int runmults[cMu] = {1, 0, 0, 0};//0-10%
  //int runmults[cMu] = {0, 1, 1, 1};//10-90%
  ...
  double shqmax;
  double shqmaxSH;
  int nbinssh = 400;//5 MeV/q-bin //200;// 10 MeV/q-bin //orig 100, 20 MeV per bin
  ...
  AliFemtoEventReaderAOD *Reader = new AliFemtoEventReaderAODMultSelection();
   Reader->SetFilterBit(8); //Global Hybrid Tracks (Bit 256)
  ...
  // *** Begin K+K- analysis ***
  int aniter = 0;
  bool verbose=false;//remove output
  ...
  for (int imult=0; imult<cMu/*4*/; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  anetaphitpc[aniter]->SetMinSizePartCollection(1);
          ...
          anetaphitpc[aniter]->SetVerboseMode(verbose);//remove output

	  mecetaphitpc[aniter] = new AliFemtoBasicEventCut();
	  mecetaphitpc[aniter]->SetEventMult(0.01,100000);//was 0,100000
...
//------------------- November 2013 -----------------------------------< 
	 ...
	 dtc2etaphitpc[aniter]->SetMaxImpactZ(DCAz);  
	  ...
	  dtc2etaphitpc[aniter]->AddCutMonitor(cutPass2PIDetaphitpc[aniter], cutFail2PIDetaphitpc[aniter]);//was mistype dtc1etaphitpc
...
//~~~	  sqpcetaphitpc[aniter] = new AliFemtoPairCutAntiGamma();
	  ...
	  //Next lines are in KchKch configuration file. I commented them:
