		...
		if (runmults[imult])
		{
			for (int ichg = 0; ichg < numOfChTypes; ichg++)
			{
				if (runch[ichg])
				{
					...
					dtc4etaphitpc[aniter]->SetNanoAODAnalysis(kFALSE);
					if(ichg>=20 && ichg<=26)
					...
					dtc5etaphitpc[aniter]->SetNanoAODAnalysis(kFALSE);
					if(ichg>=20 && ichg<=26)
  	        ...
  	        tXiCut[aniter]->SetNanoAODAnalysis(kFALSE);
  				...
  				//	tXiCut[aniter]->SetNanoAODAnalysis(kTRUE);  //yes or no?
					...
					//AXi -> ALam Pi+
  					...
  					tAXiCut[aniter]->SetNanoAODAnalysis(kFALSE); //yes or no?
