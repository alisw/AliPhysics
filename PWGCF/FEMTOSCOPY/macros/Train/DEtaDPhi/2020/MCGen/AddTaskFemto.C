...
AliAnalysisTaskFemto *AddTaskFemto(TString configMacroName, TString containerName="femtolist", TString configMacroParameters="",  Bool_t kVerbose = kFALSE, Bool_t kGridConfig = kFALSE, TString userName = "", TString configFunName = "ConfigFemtoAnalysis")
{
...
//     else
  ...
  if(kGridConfig)
    {
      taskfemto = new AliAnalysisTaskFemto("TaskFemto",configMacroName,configMacroParameters,kVerbose,kTRUE,userName, configFunName);
    }
  else
    {
      taskfemto = new AliAnalysisTaskFemto("TaskFemto","$ALICE_PHYSICS/"+configMacroName,configMacroParameters,kVerbose,kFALSE,userName, configFunName);
    }
