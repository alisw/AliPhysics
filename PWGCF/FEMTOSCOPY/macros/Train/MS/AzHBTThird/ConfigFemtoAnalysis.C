...
AliFemtoManager* ConfigFemtoAnalysis(int CentL=20, int CentH=30, int kTRange=4, int RPmix=20) {
   ...
   // int phirange2[11] = {1, 12, 24, 36, 48, 60, 72, 84, 96, 108, 119};// Moe
    int phirange2[11] = {-6, 6, 18, 30, 42, 54, 66, 78, 90, 102, 114};// Moe
    ...
    Reader->SetFilterBit(7);  // or Reader->fFilterMask(128); same thing Moe/ july 26/2016
    Reader->SetEPVZERO(kFALSE); //keep this line it fills the whole histogram of event plane
    // Reader->SetCentralityFlattening(kTRUE);
    Reader->SetCentralityPreSelection(0.001, 550);//was 0.001
    Reader->SetReadV0(0);
    Reader->SetReadMC(kFALSE);
    Reader->SetReadCascade(0);
    ...
    AliFemtoAnalysisAzimuthalPbPbThird		*ana[2][8][2];				//[charge][mult][cuts]
    ...
    //AliFemtoKTPairCutThird             *ktpaircut[2][10][5][4][9];		//[charge][mult][cuts][ktrange][phibins]
    //AliFemtoBPLCMS3DCorrFctn     *cq3dlcmskttpc[2][10][5][4][9];	//[charge][mult][cuts][ktrange][phibins]
    AliFemtoKTPairCutThird             *ktpaircut[2][8][2][4][10];		//[charge][mult][cuts][ktrange][phibins]
                ...
                if (runmults[imult]) {
                    for (int ichg=0; ichg<2; ichg++) {
                        if (runch[ichg]) {
                            ...
                            ana[ichg][imult][icuts] = new AliFemtoAnalysisAzimuthalPbPbThird(4, -8.0, 8.0, 5, multbins[imult], multbins[imult+1],RP);
                            ...
                            for (int ikt=0; ikt<4; ikt++){
                                if (runktdeprange[ikt]) {
                                    for (int iphi=0; iphi<10; iphi++){
                                        ...
                                        ktpaircut[ichg][imult][icuts][ikt][iphi] = new AliFemtoKTPairCutThird(ktrng[ikt],ktrng[ikt+1]);
                                        ktpaircut[ichg][imult][icuts][ikt][iphi]->SetPhiRange(phirange2[iphi],phirange2[iphi+1]);
                                        
                                        cq3dlcmskttpc[ichg][imult][icuts][ikt][iphi] = new AliFemtoBPLCMS3DCorrFctn(Form("%s_cq3d%imult%ikT%iRP%i_%s", nametest,ichg, imult, ikt, iphi,detadphistar[icuts]),48,-0.12,0.12);//was 30 bins
