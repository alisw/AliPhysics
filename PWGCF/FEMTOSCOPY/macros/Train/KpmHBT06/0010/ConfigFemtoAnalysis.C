 ...
 *********************************************************************
 *    K+K- in PbPb@2.76TeV                                           *
 * Update: Konstantin Mikhaylov(kmikhail@itpe.ru)                    *
 *                                                                   *
 *********************************************************************/
...
//new antigamma cos(alpha) and Minv cut:
...
> Dear Konstantin,
>
> as I remember correctly, you were using the DCA cut on the global tracks
> corrsponding to chosen TPC-only tracks (SetDCAglobalTrack(kTRUE), right?
> The update I made regards cutting on the DCA on the TPC-only track, so
> you need to change in your config to: SetDCAglobalTrack(kFALSE).*/
  ...
  //  AliFemtoPairCutAntiGamma      *sqpcetaphitpc[20];//K+-
  AliFemtoPairCutAntiGammaAlpha      *sqpcetaphitpc[20];//new antigamma cut
  ...
  for (int imult=0; imult<cMu/*4*/; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  //set verbose off (just for train)
...
//------------------- November 2013 -----------------------------------< 
	 ...
	 dtc1etaphitpc[aniter]->SetNsigmaTPC400_450(2.0);//cut on e+e- orig(2.0);
	 ...
	 dtc2etaphitpc[aniter]->SetNsigmaTPC400_450(2.0);//cut on e+e- orig(2.0);
	 ...
	 dtc2etaphitpc[aniter]->SetMaxImpactZ(DCAz);  
	  ...
	  //sqpcetaphitpc[aniter] = new AliFemtoPairCutAntiGamma();
          sqpcetaphitpc[aniter] = new AliFemtoPairCutAntiGammaAlpha();//new antigamma cut

	  sqpcetaphitpc[aniter]->SetRemoveSameLabel(kFALSE);
          //e+e-e+e-e+e-e+e-e+e-e+e-e+e-e+e- Set OLD cut on gumma conversion e+e-
	  //sqpcetaphitpc[aniter]->SetMaxEEMinv(0.001);
	  //sqpcetaphitpc[aniter]->SetMaxThetaDiff(0.09);
	  //sqpcetaphitpc[aniter]->SetTPCEntranceSepMinimum(0.00001);
	  //sqpcetaphitpc[aniter]->SetMaxEEMinv(0.0);
	  //sqpcetaphitpc[aniter]->SetMaxThetaDiff(0.0);
	  //sqpcetaphitpc[aniter]->SetTPCEntranceSepMinimum(0.0);

	  sqpcetaphitpc[aniter]->SetMaxEEMinv(0.025);          //new antigamma cut
	  sqpcetaphitpc[aniter]->SetMaxAlphaDiff(0.998);       //new antigamma cut
	  sqpcetaphitpc[aniter]->SetTPCEntranceSepMinimum(0.0);//new antigamma cut
