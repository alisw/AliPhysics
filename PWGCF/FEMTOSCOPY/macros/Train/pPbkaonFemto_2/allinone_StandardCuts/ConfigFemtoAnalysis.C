...
//AliFemtoManager* ConfigFemtoAnalysis(Int_t myInt=3,  TString str="ESD:VOA", Double_t dbl=0.0) {
AliFemtoManager* ConfigFemtoAnalysis(TString cent="V0A", TString kcut="KKC") {
 ...
 // Reader->SetReadTrackType(AliFemtoEventReaderESDChain::kGlobal);
  ...
  AliFemtoTrackCut *dtc1etaphitpc[20];
  AliFemtoTrackCut *dtc2etaphitpc[20];
  ...
  for (int imult=0; imult<3; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  if(kcut.Contains("ESD")){
	    anetaphitpc[aniter]->SetFirstParticleCut((AliFemtoESDTrackCut*)dtc1etaphitpc[aniter]);
	    anetaphitpc[aniter]->SetSecondParticleCut((AliFemtoESDTrackCut*)dtc1etaphitpc[aniter]);
	  } else if(kcut.Contains("KKC")){
	    anetaphitpc[aniter]->SetFirstParticleCut((AliFemtoKKTrackCut*)dtc1etaphitpc[aniter]);
	    anetaphitpc[aniter]->SetSecondParticleCut((AliFemtoKKTrackCut*)dtc1etaphitpc[aniter]);
	  } else{
	    Printf("Error: unknown cut selector");
	    return 0;
	  }
