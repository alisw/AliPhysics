...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  int runktdep = 1;
  //double ktrng[3] = {0.2, 0.5, 1.0};//!!orig
  //double ktrng[2] = {0.2, 1.0};
  ...
  //Reader->SetNoCentrality(kTRUE);//!!purity
  Reader->SetReadMC(kTRUE);//!!purity
  Reader->SetFilterBit(8);
  //Reader->SetReadV0(1);//!!orig
  Reader->SetReadV0(0);
  //Reader->SetEPVZERO(kTRUE);
  //Reader->SetCentralityFlattening(kTRUE);//!!orig
  ...
  AliFemtoCutMonitorParticlePtPDG *cutPass2PIDetaphitpc[20];
  AliFemtoCutMonitorParticlePtPDG *cutFail2PIDetaphitpc[20];
