...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  AliFemtoKKTrackCutFull           *dtc1etaphitpc[20];
  AliFemtoKKTrackCutFull           *dtc2etaphitpc[20];
  ...
  for (int imult=0; imult<3; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  //mecetaphitpc[aniter]->SetEPVZERO(-TMath::Pi()/2.,TMath::Pi()/2.);
...
//------------------- November 2013 -----------------------------------< 
	 ...
	 //dtc1etaphitpc[aniter]->SetStatus(AliESDtrack::kTPCrefit|AliESDtrack::kITSrefit);
	  ...
	  sqpcetaphitpc[aniter]->SetAverageSeparation(12.0);
