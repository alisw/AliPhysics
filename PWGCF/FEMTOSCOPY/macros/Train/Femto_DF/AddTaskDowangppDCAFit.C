...
AliAnalysisTaskDowangppDCAfit *AddTaskDowangppDCAFit(TString containerName="MyOutputContainer")
{
cout<<"AddTaskDowangppDCAFit here"<<endl;

  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskDowangppDCAFit", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskDowangppDCAFit", "This task requires an input event handler");
    return NULL;
  }
...
//   }
  ...
  AliAnalysisTaskDowangppDCAfit *taskppDCAfit = new AliAnalysisTaskDowangppDCAfit("dowangpp");
  mgr->AddTask(taskppDCAfit);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskppDCAfit, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskppDCAfit, 1, cout_femto);
