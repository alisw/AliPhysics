...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  int runch[4] = {1, 1, 1, 1};
  const char *chrgs[4] = { "PPPR", "PMPR", "PPAP","PMAP"};//PP-Pi+,PM-pi-,PR-Proton,AP-AntiProton
	  ...
	  if (runktdep) {
	    ...
	    for (int ikt=0; ikt<numOfkTbins; ikt++) {
	      ...
	      if (run3d) {
               ...
               cq3dprfkttpc[ktm] = new AliFemtoCorrFctn3DPRF(Form("cq3d%stpcM%ikT%i", chrgs[ichg], imult, ikt),200,0.5);
