...
//_
AliFemtoManager* ConfigFemtoAnalysis(int runcentrality0, int runcentrality1, int runcentrality2, int runcentrality3, int runcentrality4,int runcentrality5, int runcentrality6, int runcentrality7, int runcentrality8, int runcentrality9, int runSHCorrFctn, int runNonIdCorrFctn, int paircuton, int fieldsign) {
  ...
  for (int imult = 0; imult < numOfMultBins; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  if(fieldsign == 1) { // positive field
	    if (ichg == 0) {        // PIpKp
	      sqpcetaphitpc[aniter] = new AliFemtoPairCutRadialDistanceAsymmetric(0.8, 2.5, true, -0.01, 0.01, -0.3, 0.2);
	    }
	    else if (ichg == 1) {   // PImKm
	      sqpcetaphitpc[aniter] = new AliFemtoPairCutRadialDistanceAsymmetric(0.8, 2.5, true, -0.01, 0.01, -0.2, 0.3);
	    }
	    else if (ichg == 2) {   // PipKm
	      sqpcetaphitpc[aniter] = new AliFemtoPairCutRadialDistanceAsymmetric(0.8, 2.5, true, -0.01, 0.01, -0.8, -0.3);
	    }
	    else if (ichg == 3) {   // PImKp
	      sqpcetaphitpc[aniter] = new AliFemtoPairCutRadialDistanceAsymmetric(0.8, 2.5, true, -0.01, 0.01, 0.3, 0.8);
	    }
	  }
	  else { // negative field
	    if (ichg == 0) {        // PIpKp
