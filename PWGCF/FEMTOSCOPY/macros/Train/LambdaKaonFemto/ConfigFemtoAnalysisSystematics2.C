...
//This allows me to simultaneously change the min and max values for a given cut.
...
{
  std::cout << "[ConfigFemtoAnalysisSystematics2 (LambdaKaon)]\n";
  ...
  //
  CutVariations tCutVariationsMin;
  CutVariations tCutVariationsMax;

  // Read parameter string and update configurations
  //Initial call to BuildConfiguration
  BuildConfiguration(aParamString,tAnalysisConfig,tEventCutConfig,tPairCutConfig,tMacroConfig,tCutVariationsMin, tCutVariationsMax);
  ...
  double tNCutVals = tCutVariationsMin.valuesToVary.size();
  for(unsigned int iCutVal=0; iCutVal<tNCutVals; iCutVal++)
  {
    ...
    for(unsigned int iCent = 0; iCent+1 < tMacroConfig.centrality_ranges.size(); iCent += 2)
    {
      ...
      for(unsigned int iPair = 0; iPair < tMacroConfig.pair_codes.size(); iPair++)
      {
        ...
        BuildConfiguration(tNewParamString,tAnalysisConfig,tEventCutConfig,tPairCutConfig,tMacroConfig,tCutVariationsMin, tCutVariationsMax);
...
//  std::cout << "I-BuildConfiguration:" << TBase64::Encode(text) << " \n";
  ...
  const TString tAnalysisVarName = "aAnParams",//
                ...
                tMacroVarName    = "aMac",
                tCutVariationsMinName = "aCutVariationsMin",
                tCutVariationsMaxName = "aCutVariationsMax";
  ...
  while(tLineObj = tNextLine())
  {
    ...
    case '?':  //Centrality in Macro Params
    {
      TString tTempLine = tLine(1, tLine.Length()-1);
      TObjArray *tCutAndValues = tTempLine.Tokenize("=");
      const TString tCut = ((TObjString*)tCutAndValues->At(0))->String().Strip(TString::kBoth, ' ');
      const TString tCutValues = ((TObjString*)tCutAndValues->At(1))->String().Strip(TString::kBoth, ' ');

      if(tNCutVarsAdded == 0) tCmd = tCutVariationsMinName + ".cutName = '" + tCut(0,tCut.Length()) + "'";
      else tCmd = tCutVariationsMaxName + ".cutName = '" + tCut(0,tCut.Length()) + "'";
      tCmd.ReplaceAll("'", '"');

      TString tCmd2("");
      unsigned int tRangeEnd = tCutValues.Index("]");
      if(tRangeEnd == -1) tRangeEnd = tCutValues.Length();
      TString tCentralityRanges = tCutValues(1, tRangeEnd - 1);
      TObjArray *tRangeGroups = tCentralityRanges.Tokenize(",");
      TIter tNextRangeGroup(tRangeGroups);
      TObjString *tRangeGroup = NULL;
      ...
      while(tRangeGroup = (TObjString*)tNextRangeGroup())
      {
        ...
        while(tSubRange_it = (TObjString*)tNextSubRange())
        {
          ...
          if(tNCutVarsAdded == 0) tCmd2 = tCutVariationsMinName + ".valuesToVary.push_back(" + tNext + ");";
          else  tCmd2 = tCutVariationsMaxName + ".valuesToVary.push_back(" + tNext + ");";
          cout << "I-BuildConfiguration: `" << tCmd2 << "`\n";
          gROOT->ProcessLineFast(tCmd2);
        }
      }
      tNCutVarsAdded++;
    }
