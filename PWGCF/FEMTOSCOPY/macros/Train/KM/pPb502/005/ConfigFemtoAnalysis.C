...
// 005: PWGCF/FEMTOSCOPY/macros/Train/KM/pPb502/005/ConfigFemtoAnalysis.C
// Test for pi+pi- for Elena
// modified from PWGCF/FEMTOSCOPY/macros/Train/KM/pPb502/004/ConfigFemtoAnalysis.C
...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  int runktdep = 1;
  const int ktbins=3; //corresponds to ktbins-1 number of ranges
  double ktrng[ktbins] = {0.2, 0.3, 0.4};
  ...
  for (int imult=0; imult<3; imult++) {
	...
	if (runch[ichg]) {
	  ...
	  //--- K+- ---<
	   ...
	   dtc1etaphitpc[aniter]->SetPt(0.13,1.5);//pi+
	   dtc1etaphitpc[aniter]->SetEta(-0.8,0.8);//pi+
	   dtc2etaphitpc[aniter]->SetPt(0.13,1.5);//pi-
	   dtc2etaphitpc[aniter]->SetEta(-0.8,0.8);//pi-
...
//------------------- November 2013 -----------------------------------< 
	 ...
	 // sqpcetaphitpc[aniter]->SetPhiStarDifferenceMinimum(0.04);
	  ...
	  if (runktdep) {
	    int ktm;
	    for (int ikt=0; ikt<ktbins-1/*2*/; ikt++) {
	      ktm = aniter*2 + ikt;
