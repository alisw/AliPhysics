...
// March 2026 --> All wagon before 2025 failed
//              ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// pPb5.02(run2) -> K+K-
// 016_fb8_bec wagon created March 24, 2025
// ~~~~~~~~~~~
// PID is the same as in PbPb2.76->K+K- 016_fb wagon ,
// but with AliFemtoBasicEventCut instead of AliFemtoSphericityEventCut
// 0.2-0.5,0.5-1.5; 0-20,20-40,40-90%
// WITHOU the event shape !!!
// AliFemtoBasicEventCut should give more statistics than it was for AliFemtoSphericityEventCut (0<St<1)
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  AliFemtoCutMonitorEventVertex     *vmon[2][iz];//vertex monitor w/o sphericity ??? empty XY histograms???
  AliFemtoCutMonitorEventMult       *mul_[2][iz];//multiplicity
  AliFemtoKpm45TrackCut             *pid_[2][iz];//K+K- case
  AliFemtoPairCutRadialDistanceKKdist  *ttc[iz];
  AliFemtoQinvCorrFctn                 *cqi[iz*cKt];//1d CF(Qinv): all kT + cKt bins

  AliFemtoCutMonitorParticleYPt *mdca[2][2][iz];//[particle][pass-fail][iz]
  AliFemtoCutMonitorParticlePID *mpid[2][2][iz];//[particle][pass-fail][iz]
  ...
  for (int m=0; m<cMu; m++) {
    ...
    ix = m;//ichg*3+imult;
      ...
      //bcut[ix] = new AliFemtoSphericityEventCut();//spher: 
      bcut[ix] = new AliFemtoBasicEventCut();//w/o spher
      bcut[ix]->SetEventMult(0,10000);//w/o spher
      bcut[ix]->SetVertZPos(-10,10);//w/o spher
      //bcut[ix]->SetStMin(S_T_MIMA[0]);//spher: 
      //bcut[ix]->SetStMax(S_T_MIMA[1]);//spher: 
	...
	pid_[pa][ix] = new AliFemtoKpm45TrackCut();
	pid_[pa][ix]->SetCharge(zar[pa]);

	pid_[pa][ix]->SetPt(0.14,1.5);
	pid_[pa][ix]->SetEta(-0.8,0.8);
	pid_[pa][ix]->SetMass(m_K);
	pid_[pa][ix]->SetMostProbableKaon();
	pid_[pa][ix]->SetNsigmaTPCle250(2.0);    //K+K- standard PID -->
	pid_[pa][ix]->SetNsigmaTPC250_400(2.0);  //
	pid_[pa][ix]->SetNsigmaTPC400_450(1.0);  //to remove e-peak decrease from 3.0(default) to smaller value = 1.0(orig!!!!)
	pid_[pa][ix]->SetNsigmaTPC450_500(2.0);  //
	pid_[pa][ix]->SetNsigmaTPCge500(3.0);    //
	pid_[pa][ix]->SetNsigmaTOF500_800(2.0);  //
	pid_[pa][ix]->SetNsigmaTOF800_1000(1.5); //
	pid_[pa][ix]->SetNsigmaTOFge1000(1.0);   //K+K- standard PID --<
	
	pid_[pa][ix]->SetminTPCncls(80);         //K+K- track quality cuts -->
	pid_[pa][ix]->SetRemoveKinks(kTRUE);     //
	pid_[pa][ix]->SetMaxTPCChiNdof(4.0);     //
	//pid_[pa][ix]->SetMaxITSChiNdof(36);//	 //  
	pid_[pa][ix]->SetLabel(kFALSE);          //
	pid_[pa][ix]->SetMaxImpactXY(DCA[0]);    //
	pid_[pa][ix]->SetMaxImpactZ( DCA[1]);    //K+K- track quality cuts --<

	mdca[pa][0][ix] = new AliFemtoCutMonitorParticleYPt(Form("_Pass_%s_M%i",spart[pa],m),m_K);
	mdca[pa][1][ix] = new AliFemtoCutMonitorParticleYPt(Form("_Fail_%s_M%i",spart[pa],m),m_K);
	pid_[pa][ix]->AddCutMonitor(mdca[pa][0][ix],mdca[pa][1][ix]);

	mpid[pa][0][ix] = new AliFemtoCutMonitorParticlePID(Form("_Pass_%s_M%i", spart[pa],m),1);//1 - kaon time
	mpid[pa][1][ix]  = new AliFemtoCutMonitorParticlePID(Form("_Fail_%s_M%i",spart[pa],m),1);
	pid_[pa][ix]->AddCutMonitor(mpid[pa][0][ix],mpid[pa][1][ix]);
      ...
      mixi[ix]->SetFirstParticleCut(pid_[0][ix]);
      mixi[ix]->SetSecondParticleCut(pid_[1][ix]);
      mixi[ix]->SetPairCut(ttc[ix]);
	  
      //CF vs Qinv (without kT bins)
      cqi[ix] = new AliFemtoQinvCorrFctn(Form("C_KpKm_Mu_%i", m),nbinssh,0.0,shqmax);
      ...
      for(int k=0;k<cKt;k++){
	if(!cf_kT) continue;
	ik=ix*cKt+k+cMu;//+1;//"+1" to be different with cqi[ix] - does not matter! See /cern/kmikhail/Grid/Train/LocalRun2_KchKch_pPb.2025
	//AnalysisResults_016_fb8_bec_wo_+1.wodir.root and AnalysisResults_016_fb8_bec_wi_+1.wodir.root
	//Keep w/o "+1"
	cout<<"------------------> ik="<<ik<<" ix=m="<<ix<<endl;
	pcut[ik]=new AliFemtoKTPairCut(ktrng[k],ktrng[k+1]);
	cqi[ik] =new AliFemtoQinvCorrFctn(Form("C_KpKm_Mu_%i_kT_%i",m,k),nbinssh,0.0,shqmax);
