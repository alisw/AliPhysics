...
// Feb 2025 --> All wagon before 2025 failed
//              ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// pPb5.02(run2) -> K+K+ & K-K-
// 015_fb8_bec wagon created Feb 25, 2025
// ~~~~~~~~~~~               ~~~~~~~~~~~~
// 015_fb8_bec -> same as 015_fb8, but with AliFemtoBasicEventCut instead of AliFemtoSphericityEventCut
// PID is the same as in PbPb2.76->K+K- 015_fb wagon !!!
...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  for (int m=0; m<cMu; m++) {
    ...
    ix = m+zz*3;//ichg*3+imult;???
      ...
      mixi[ix]->SetVerboseMode(kFALSE);//SetVerboseMode(kTRUE/*kFALSE*/);
      ...
      //smon[0][ix] = new AliFemtoCutMonitorEventSphericity(Form("_Pass_M%i",m));//spher: 
      //smon[1][ix] = new AliFemtoCutMonitorEventSphericity(Form("_Fail_M%i",m));//spher: 
      //bcut[ix]->AddCutMonitor(smon[0][ix],smon[1][ix]);//spher: 

	
      //AliFemtoCutMonitorEventVertex class gives empty XY and wide Z vertex hists ???
      //015_fb_bec> .ls EvVer*_Pass_M0
      //EvVertXY_Pass* empty, but EvVertZ_Pass_M0 is a Z profile -10,+10
      //comment for first time (and may be need more study) --------->
