...
AliAnalysisTaskParticleEffDYM3 *AddTaskEffDYM3(TString containerName="femtolist",int method=3, int filterbit=96)
{
  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskEffDYM3", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskEffDYM3", "This task requires an input event handler");
    return NULL;
  }
...
//   }
  ...
  AliAnalysisTaskParticleEffDYM3 *taskEffM3 = new AliAnalysisTaskParticleEffDYM3("EffTaskM3",method,filterbit);
  //taskEff->SetPidMethod(method);
  //taskEff->SetFB(filterbit);
  mgr->AddTask(taskEffM3);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEffM3, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEffM3, 1, cout_femto);
