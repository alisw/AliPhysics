...
AliAnalysisTaskParticleEffDY06 *AddTaskEffDY06(TString containerName="femtolist",int method=3, int filterbit=96)
{
  // A. Get the pointer to the existing analysis manager via the static access method.
  //==============================================================================
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTaskEffDY06", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  if (!mgr->GetInputEventHandler()) {
    ::Error("AddTaskEffDY06", "This task requires an input event handler");
    return NULL;
  }
...
//   }
  ...
  AliAnalysisTaskParticleEffDY06 *taskEff06 = new AliAnalysisTaskParticleEffDY06("EffTask06",method,filterbit);
  taskEff06->SetPidMethod(method);
  taskEff06->SetFB(filterbit);
  mgr->AddTask(taskEff06);
  ...
  AliAnalysisDataContainer *cout_femto  = mgr->CreateContainer(containerName,  TList::Class(),
   ...
   mgr->ConnectInput(taskEff06, 0, mgr->GetCommonInputContainer());
   mgr->ConnectOutput(taskEff06, 1, cout_femto);
