///
/// \file AliFemtoBPLCMS3DCorrFctn.cxx
///
...
void AliFemtoBPLCMS3DCorrFctn::Finish()
{
  ...
  //   from OTHER CorrFctns which you read in (like when doing parallel processing)
    ...
    const Int_t nbins = fNumerator->GetNbinsX(),
             half_way = nbins / 2;
...
AliFemtoString AliFemtoBPLCMS3DCorrFctn::Report()
{
  ...
  report += TString::Format("Normalization region in Qinv was:\t%E\t%E\n", fQinvNormLo, fQinvNormHi);
  report += TString::Format("Number of pairs in Normalization region was:\n");
  report += TString::Format("In numerator:\t%lu\t In denominator:\t%lu\n", fNumRealsNorm, fNumMixedNorm);

  /*  if (fCorrection) {
        report += TString::Format("Coulomb correction used radius of\t%E\n", fCorrection->GetRadius());
      }
      else
      {
  ...
  if (fPairCut) {
    if (fUseRPSelection) {
      ...
      if (!ktc) {
        ...
        } else if (!(ktc->Pass(pair, arp->GetCurrentReactionPlane()))) {
          return;
        }
      }
    } else {
