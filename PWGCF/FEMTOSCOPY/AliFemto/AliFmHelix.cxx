...
void AliFmHelix::SetCurvature(double val)
{
  ...
  if (fCurvature <= numeric_limits<double>::epsilon()) {
...
double AliFmHelix::FudgePathLength(const AliFmThreeVector<double>& p) const
{
  ...
  if (fSingularity) {
    ...
    double x = dy*fCosPhase - dx*fSinPhase,
           y = 1.0/fCurvature + dx*fCosPhase+dy*fSinPhase;
    s = atan2(x, y) / (fH*fCurvature*fCosDipAngle);
  }
  return s;
}
...
double AliFmHelix::PathLength(const AliFmThreeVector<double>& p, bool scanPeriods) const
{
  ...
  if (fSingularity) {
    s = fCosDipAngle*(fCosPhase*dy-fSinPhase*dx) + fSinDipAngle*dz;
  }
  else { //
      ...
      for (int i=0; i<ktMaxIterations; i++) {
        ...
        s  -= (t11*t12*fH*fCosDipAngle - t19*t7*fH*fCosDipAngle -
              ...
              (t12*t12*fCosDipAngle*fCosDipAngle + t11*t7*t34 +
               t7*t7*fCosDipAngle*fCosDipAngle + t19*t12*t34+t41);
        if (fabs(sOld-s) < ktMaxPrecisionNeeded) {
          break;
        }
        sOld = s;
      }
  ...
  if (fSingularity) {
    ...
    for (i=0; i<ktMaxIterations; i++) {
      ...
      if ( fabs(fp)*deltas <= fabs(f) ) { //too big step
        const int sgn = (f < 0.0 ? -1 : 1) * (fp < 0.0 ? -1 : 1);
    ...
    if (i == ktMaxIterations) {
      return fgkNoSolution;
    }
  }
  return s;
}
