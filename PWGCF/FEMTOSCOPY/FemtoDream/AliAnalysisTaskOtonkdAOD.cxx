...
//AOD
 ...
 if (!Event) {
  ...
  Double_t PrimVtx[3];
//AOD
  fTRunNumber = Event->GetRunNumber();
  //fEvent->GetPrimaryVertex()->GetXYZ(PrimVtx);
  Event->GetPrimaryVertex()->GetXYZ(PrimVtx);
//NanoAOD
...
//    fTrack->SetTrack(track, fInputEvent);
    ...
    Bool_t IsDeuteron = kFALSE;
      ...
      //if(fTnDeuteron>0) fTree->Fill(); //for test
...
void AliAnalysisTaskOtonkdAOD::StoreGlobalTrackReference(AliVTrack *track) {
...
//________________________________________________________________________________
Bool_t AliAnalysisTaskOtonkdAOD::FillDeuteron(AliFemtoDreamTrack *TheTrack) {
 Bool_t Filled = kFALSE;
 TVector3 mom;
 mom = TheTrack->GetMomentum();
