...
bool AliFemtoDreamv0Cuts::isSelected(AliFemtoDreamv0 *v0) {
  ...
  if (!v0->IsSet()) {
    ...
    if (!fMinimalBooking) fHist->FillTrackCounter(0);
  }
  if (pass) {
...
bool AliFemtoDreamv0Cuts::DaughtersPassCuts(AliFemtoDreamv0 *v0) {
  ...
  if (!v0->GetHasDaughters()) {
    ...
    if (!fMinimalBooking) fHist->FillTrackCounter(1);
  }
  if (pass) {
    if (v0->GetCharge().at(1)<0&&v0->GetCharge().at(2)>0) {
      ...
      if (!fMinimalBooking) fHist->FillTrackCounter(13);
      ...
      if (passD1&&passD2) {
        if (!fMinimalBooking) fHist->FillTrackCounter(14);
      }
    } else {
      if (!fMinimalBooking) fHist->FillTrackCounter(15);
      ...
      if (passD1&&passD2) {
        if (!fMinimalBooking) fHist->FillTrackCounter(16);
      }
    }
    if (!(passD1&&passD2)) {
      pass=false;
    } else {
      if (!fMinimalBooking) fHist->FillTrackCounter(2);
    }
  }
  pass=passD1&&passD2;
  return pass;
...
bool AliFemtoDreamv0Cuts::MotherPassCuts(AliFemtoDreamv0 *v0) {
  ...
  if (fCutOnFlyStatus) {
    if (!(v0->GetOnlinev0()==fOnFlyStatus)) {
      ...
      if (!fMinimalBooking) fHist->FillTrackCounter(3);
    }
  }
  if (pass&&fCutCharge) {
    if (v0->GetCharge().at(0)!=fCharge) {
      ...
      if (!fMinimalBooking) fHist->FillTrackCounter(4);
    }
  }
  if (pass&&fCutPt) {
    if ((v0->GetPt()<fpTmin)||(v0->GetPt()>fpTmax)) {
      ...
      if (!fMinimalBooking) fHist->FillTrackCounter(5);
    }
  }
  if (pass&&fCutDecayVtxXYZ) {
    if ((v0->GetDCAv0Vtx(0)>fMaxDecayVtxXYZ)||
      ...
      if (!fMinimalBooking) fHist->FillTrackCounter(6);
    }
  }
  if (pass&&fCutTransRadius) {
    if ((v0->GetTransverseRadius()<fMinTransRadius)||
      ...
      if (!fMinimalBooking) fHist->FillTrackCounter(7);
    }
  }
  if (pass&&fCutMinDCADaugPrimVtx) {
    if ((v0->GetDCADaugPosVtx()<fMinDCADaugToPrimVtx)||
      ...
      if (!fMinimalBooking) fHist->FillTrackCounter(8);
    }
  }
  if (pass&&fCutMaxDCADaugToDecayVtx) {
    if (v0->GetDaugDCA()>fMaxDCADaugToDecayVtx) {
      ...
      if (!fMinimalBooking) fHist->FillTrackCounter(9);
    }
  }
  return pass;
}
...
bool AliFemtoDreamv0Cuts::CPAandMassCuts(AliFemtoDreamv0 *v0) {
  ...
  if (massPass) {
    if (!fMinimalBooking) fHist->FillTrackCounter(11);
  }
  if (massPass && cpaPass) {
    if (!fMinimalBooking) fHist->FillTrackCounter(12);
  }
