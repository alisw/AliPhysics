...
,fMCContCPAPtBins(0)
,fMCPrimCPAPtBinsMult()
,fMCMaterialCPAPtBinsMult()
,fMCSecondaryCPAPtBinsMult()
,fMCContCPAPtBinsMult()
,fPtResolution(0)
,fThetaResolution(0)
,fPhiResolution(0)
{
  ...
  if (CPADist) {
    ...
    for(int i=0; i<3; ++i) {
      fMCPrimCPAPtBinsMult[i]=new TH2F(primPtBinName[i].Data(),
    		  	  	  	  	  	       axisRange[i].Data(),8,0.3,4.3,1000,0.90,1.);
...
void AliFemtoDreamv0MCHist::FillMCCPAPtBins(
  ...
  if (org==AliFemtoDreamBasePart::kPhysPrimary) {
    ...
    FillMultiplicityHistos(multiplicity, pT, cpa,
    					   fMCPrimCPAPtBinsMult[0], fMCPrimCPAPtBinsMult[1], fMCPrimCPAPtBinsMult[2]);
  } else if(org == AliFemtoDreamBasePart::kWeak) {
    fMCSecondaryCPAPtBins->Fill(pT,cpa);
    FillMultiplicityHistos(multiplicity, pT, cpa,
    					   fMCSecondaryCPAPtBinsMult[0], fMCSecondaryCPAPtBinsMult[1], fMCSecondaryCPAPtBinsMult[2]);
  } else if(org == AliFemtoDreamBasePart::kMaterial) {
    fMCMaterialCPAPtBins->Fill(pT,cpa);
    FillMultiplicityHistos(multiplicity, pT, cpa,
    					   fMCMaterialCPAPtBinsMult[0], fMCMaterialCPAPtBinsMult[1], fMCMaterialCPAPtBinsMult[2]);
  } else if(org == AliFemtoDreamBasePart::kFake) {
    fMCContCPAPtBins->Fill(pT,cpa);
    FillMultiplicityHistos(multiplicity, pT, cpa,
    					   fMCContCPAPtBinsMult[0], fMCContCPAPtBinsMult[1], fMCContCPAPtBinsMult[2]);
  } else {
    AliFatal("Particle Origin not implemented");
  }
}

void AliFemtoDreamv0MCHist::FillMultiplicityHistos(int multiplicity, float pT, float cpa,
