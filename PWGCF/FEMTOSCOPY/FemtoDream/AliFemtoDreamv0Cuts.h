...
public:
  ...
  static AliFemtoDreamv0Cuts *LambdaSigma0Cuts(bool isMC, bool CPAPlots,
                                               bool SplitContrib);
  // Access function, use with utmost care!
  bool AccessDaughtersPassCuts(AliFemtoDreamv0 *v0)
  {
    return DaughtersPassCuts(v0);
  }
  bool AccessMotherPassCuts(AliFemtoDreamv0 *v0)
  {
    return MotherPassCuts(v0);
  }
  bool AccessRejectAsKaon(AliFemtoDreamv0 *v0)
  {
    return RejectAsKaon(v0);
  }
  bool AccessArmenterosSelection(AliFemtoDreamv0 *v0)
  {
    return ArmenterosSelection(v0);
  }
  // Setters for plots
  ...
  bool GetCutArmenteros() const { return fDoArmenterosCut; };
