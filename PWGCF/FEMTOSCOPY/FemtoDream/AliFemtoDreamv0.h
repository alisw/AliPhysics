...
class AliFemtoDreamv0 : public AliFemtoDreamBasePart {
 public:
  ...
  float Getv0Mass() const {return fv0Mass;};
  void Setv0Mass(float mass){fv0Mass=mass;};
  float GetDCAv0Vtx(int i)const {
    if(i>-1 && i<3){return fv0Vtx[i];}else{return 0.;};
  };
  float GetDaugDCA() const {return fdcav0Daug;};
  float GetDCAPrimVtx() const {return fdcaPrim;};
  float GetDCADaugPosVtx() const {return fdcaPrimPos;};
  float GetDCADaugNegVtx() const {return fdcaPrimNeg;};
  float GetDecayLength() const {return flenDecay;};
