    ...
    AliFemtoDreamBasePart &part)
{
  ...
  for (auto it : momenta)
  {
    ...
    for (int radius = 0; radius < 9; radius++)
    {
      tmpVec.push_back(
          phi0 - TMath::ASin(
                     0.1 * chg * fBField * 0.3 * TPCradii[radius] * 0.01 / (2. * pt)));
    }
    counter++;
...
// Caution!! Here we assume that part1 is always the V0
float AliFemtoDreamHigherPairMath::RelativePairMomentumV0(
    AliFemtoDreamBasePart *part1, AliFemtoDreamBasePart *part2,
    const int pdg2)
{
  TLorentzVector PartOne, PartTwo;
  PartOne.SetXYZM(part1->GetMomentum().X(), part1->GetMomentum().Y(),
                  part1->GetMomentum().Z(), part1->GetInvMass());
  PartTwo.SetXYZM(part2->GetMomentum().X(), part2->GetMomentum().Y(),
                  part2->GetMomentum().Z(),
                  TDatabasePDG::Instance()->GetParticle(pdg2)->Mass());
  return RelativePairMomentum(PartOne, PartTwo);
...
// Caution!! Assume that part1 is the V0
float AliFemtoDreamHigherPairMath::RelativePairmTV0(AliFemtoDreamBasePart *part1,
                                                    AliFemtoDreamBasePart *part2,
                                                    const int pdg2)
{
  TLorentzVector PartOne, PartTwo;
                                                   ...
                                                   bool SEorME, float relk)
{
  ...
  for (unsigned int iDaug1 = 0; iDaug1 < nDaug1; ++iDaug1)
  {
    ...
    for (unsigned int iDaug2 = 0; iDaug2 < nDaug2; ++iDaug2)
    {
      ...
      const int size =
          (PhiAtRad1.size() > phiAtRad2.size()) ? phiAtRad2.size() : PhiAtRad1.size();
      float dphiAvg = 0;
      ...
      {
        if ((dphiAvg / (float)size) * (dphiAvg / (float)size) / fDeltaPhiSqMax + deta * deta / fDeltaEtaSqMax < 1.)
        {
          pass = false;
        }
      }
