...
class AliFemtoDreamCascade : public AliFemtoDreamBasePart {
 public:
  ...
  float GetDecayLength() {return fLength;};
...
inline double AliFemtoDreamCascade::Massv0(int PDGPos,int PDGNeg) {
  ...
  return ::sqrt((EPos+ENeg)*(EPos+ENeg)-(fPosDaug->GetMomentum()+fNegDaug->GetMomentum()).Mag2());
}
...
inline double AliFemtoDreamCascade::CosPointingAngle(double *primVtx,double *secondVtx) {
  TVector3 lVectPrimVtxToXi(secondVtx[0] - primVtx[0],
                            secondVtx[1] - primVtx[1],
                            secondVtx[2] - primVtx[2]);
  return fP.Dot(lVectPrimVtxToXi)/TMath::Sqrt(fP.Mag2()*lVectPrimVtxToXi.Mag2());
}

inline float AliFemtoDreamCascade::DcaXiToPrimVertex(double *primVtx,double *secondVtx) {
  Double_t dx = (primVtx[1] - secondVtx[1] )*fP.Z() - (primVtx[2] - secondVtx[2] )*fP.Y();
  Double_t dy = (primVtx[2]- secondVtx[2] )*fP.X() - (primVtx[0]- secondVtx[0] )*fP.Z();
  Double_t dz = (primVtx[0]- secondVtx[0] )*fP.Y() - (primVtx[1]- secondVtx[1] )*fP.X();
