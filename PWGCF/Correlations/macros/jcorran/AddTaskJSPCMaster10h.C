...
AliAnalysisTask *AddTaskJSPCMaster10h(Bool_t doSPC = kTRUE, TString taskName = "JSPCMaster10h", double ptMin = 0.2, std::string Variations = "tpconly", Bool_t applyHMOcut = kTRUE, Bool_t saveCatalystQA = kFALSE, Bool_t saveHMOQA = kFALSE, Bool_t newWeightNaming = kTRUE)
{
...
// Configuration of the analysis task itself.
  ...
  for (Int_t i = 0; i < PassedVariations; i++){
    if (doSPC) {  
      ...
      myTask[i][0]->JSPCSetUseWeights(kTRUE);
