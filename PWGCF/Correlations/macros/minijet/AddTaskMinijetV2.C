AliAnalysisTaskMinijetV2* AddTaskMinijetV2(Int_t runNumber     =    -1, 
				       TString format      = "aod",
				       Bool_t  useMC       = false, 
				       Bool_t  mcOnly      = false,
				       ...
				       const char *foldername = "MiniJets")
{
  ...
  AliAnalysisTaskMinijetV2 *taskMB = 
    new AliAnalysisTaskMinijetV2("AliAnalysisTaskMinijetV2 Min bias");
  ...
  AliAnalysisTaskMinijetV2 *taskHM =0x0;
  if(useHighMult){
    taskHM  = new AliAnalysisTaskMinijetV2("AliAnalysisTaskMinijetV2 HighMult");
  ...
  if(runNumber>0){ 
    outputMB  =  mgr->CreateContainer(foldername,AliDirList::Class(),
				      AliAnalysisManager::kOutputContainer, 
				      Form("run%d.root",runNumber));
    if(useHighMult){
      outputHM =  mgr->CreateContainer("MiniJets_HighMult",AliDirList::Class(),
				       AliAnalysisManager::kOutputContainer, 
				       Form("run%d.root",runNumber));
    }
  }
