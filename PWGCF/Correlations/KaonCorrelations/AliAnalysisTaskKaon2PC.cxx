...
class Kaon2PC;    // your analysis class
...
Kaon2PC::Kaon2PC() : AliAnalysisTaskSE(), 
    fAOD(0), fOutputList(0), fPIDResponse(0), PVz(0), fLpTCut(0.2), fUpTCut(1.0), fEtaCut(0.8), fSigCut(2.0), 
    fDecayLv0Cut(8.05), fLpTv0Cut(0.2), fUpTv0Cut(1.0), fEtav0Cut(0.8), fDcaPosToPrimVtxv0Cut(0.1), 
    fDcaNegToPrimVtxv0Cut(0.1), fEtaPosv0Cut(0.8), fEtaNegv0Cut(0.8), fCosPACut(0.99), fSigPosv0Cut(2.0), 
    fSigNegv0Cut(2.0), fHistMK0(0), fHistMK0Cuts(0), fHistNV0(0), fHistKChPt(0), fHistK0Pt(0), fKChPhi(0), fK0Phi(0), fHistKpPhi(0), fHistKnPhi(0),
    fVtx(0), fClusters(0), fPID(0), fPIDKa(0), fPIDKaon(0), fPIDK(0), fPIDKeCut(0), fPIDKpiCut(0), 
...
void Kaon2PC::UserCreateOutputObjects()
{
    ...
    fHistKChPt->SetMarkerStyle(20);
    
    fHistK0Pt = new TH1F("fHistK0Pt", "p_{T} distribution of all Neutral Kaon Tracks", 100, 0, 2);
    fHistK0Pt->SetOption("HIST E p");

    fKChPhi = new TH1F("fKChPhi", "#phi distribution of all Charged Kaon Tracks", 100, 0, 2*Pi);
    fKChPhi->SetOption("HIST p");
    fKChPhi->SetMarkerStyle(20);

    fK0Phi = new TH1F("fK0Phi", "#phi distribution of all Neutral Kaon Tracks", 100, 0, 2*Pi);
    fK0Phi->SetOption("HIST E L");

    fHistKpPhi = new TH1F("fHistKpPhi", "#phi distribution of all K^{+} Tracks", 100, 0, 2*Pi);
    fHistKpPhi->SetOption("HIST E L");

    fHistKnPhi = new TH1F("fHistKnPhi", "#phi distribution of all K^{-} Tracks", 100, 0, 2*Pi);
    fHistKnPhi->SetOption("HIST E L");
    ...
    fPIDKa= new TH2F("fPIDKa","Kaons with |n#Sigma|<5",800,0.2,1.0,800,0.0,700.0);
    ...
    fHistInvCent = new TH2F("fHistInvCent", "", 100,0,100,100,0.4,0.6);
    ...
    fOutputList->Add(fKChPhi);
    fOutputList->Add(fK0Phi);
...
void Kaon2PC::UserExec(Option_t *)
{
    ...
    for(Int_t i(0); i < iTracks; i++) {
        ...
        if ((nSigmakaon < 1.0) ) { fPIDKeCut->Fill(track->Pt(),track->GetTPCsignal());  }
    ...
    fHistNV0->Fill(nv0s); 
    //cout << "number of v0 vertices per event are" << nv0s << endl;
    ...
    //cout<< "number of tracks in each event" << iTracks << endl;
    ...
    for(Int_t i=0; i < iTracks; i++) {                                       // loop over all these tracks
        ...
        fKChPhi->Fill(trackPhi);
        fHistKChPt->Fill(trackPt);
        fHistChPhi->Fill(trackPhi,CentV0M);
        fHistChEta->Fill(trackEta,CentV0M);
        fHistChRap->Fill(track->Y(),CentV0M);
        if (chargetrack > 0) {
            fHistKpPhi->Fill(trackPhi);
            fHistPosPhi->Fill(trackPhi,CentV0M);          //Yield vs Phi of +ve charged particles histo
            fHistPosEta->Fill(trackEta,CentV0M);          //Yield vs Eta of +ve charged particles histo
            fHistPosPhiEta->Fill(trackPhi,trackEta,CentV0M);
            fHistPosRap->Fill(track->Y(0.493),CentV0M);
            }
        if (chargetrack < 0) {
            fHistKnPhi->Fill(trackPhi);
            fHistNegPhi->Fill(trackPhi,CentV0M);          //Yield vs Phi of -ve charged particles histo
            fHistNegEta->Fill(trackEta,CentV0M);          //Yield vs Eta of -ve charged particles histo
