...
AliJJtAna::AliJJtAna(Bool_t execLocal) :
  TObject(),
  fFirstEvent(kTRUE),
  fRunNumber(-999),
  fcent(-999),
...
AliJJtAna::AliJJtAna(const AliJJtAna& obj) : 
  TObject(),
...
AliJJtAna& AliJJtAna::operator=(const AliJJtAna& obj){
  // equal sign operator
  JUNUSED(obj);
  return *this;
}
...
void AliJJtAna::Initialize() const{
    // init

}

void AliJJtAna::UserCreateOutputObjects(){
  // local init
...
void AliJJtAna::UserExec(){
	...
	// event loop
	fevt++;
        // fcent, fZvert, fRunNum are set in Task...
	if( fFirstEvent ) {
...
double AliJJtAna::DeltaPhi(double phi1, double phi2) {
	// dphi
