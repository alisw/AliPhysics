...
AliJPartLifetime::AliJPartLifetime(AliJCard *pcard)
:AliAnalysisTaskSE("PartLifetime")//:AliAnalysisTaskSE("AliJPartLifetime")
    , fOption()
    , fOutput(NULL)
    , fcard(pcard)
    //, fTrigger(NULL)
...

{
    ...
    for (uint i = 0; i < kAll; i++){
      ...
      for (uint hit = 0; hit < triggc; hit++)
      {
        ...
        //fV0Mass[i][j] = new TH1D(Form("mom_%s_%u",tname[i],j),tname[i],180*5,0.2,2); //0.2 to 1.5 GeV
        fV0Mass[i][hit] = new TH1D(Form("mom_%s_%u",tname[i],hit),tname[i],bc,pTt1,pTt2); //0.2 to 1.5 GeV
