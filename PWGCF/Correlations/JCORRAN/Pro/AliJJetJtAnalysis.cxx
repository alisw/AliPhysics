...
void AliJJetJtAnalysis::UserExec(){
	...
	for( int i=0;i<fJetListOfList.GetEntries();i++ ){
		...
		//cout << "Filling Jt histograms " << endl;
		this->FillJtHistogram(Jets,i);
	}

	if(fDoMC){
		//cout << "Number of reconstructed tracks: " << fTracks->GetEntries() << endl;
		//cout << "Number of generated tracks: " << fMCTracks->GetEntries() << endl;


		//fTrackPt.clear();
		for( int i=0;i<fMCJetListOfList.GetEntries();i++ ){
			TObjArray * Jets = (TObjArray*) fJetListOfList[i];
			TObjArray * MCJets = (TObjArray*) fMCJetListOfList[i];
			/*if(i ==0){
				//if( Jets->GetEntries()>0 || MCJets->GetEntries() > 0 ){
				if( Jets->GetEntries()>0 && MCJets->GetEntries() > 0 ){
					cout << "Number of reconstructed jets: " << Jets->GetEntries() << endl;
					for (int j = 0; j<Jets->GetEntries(); j++){
						AliJJet *jet = dynamic_cast<AliJJet*>( Jets->At(j) );
						if (!jet) continue;
						//if(jet->Pt() > 5){
							jet->ReSum();
							cout << "Jet " << j << " pt: " << jet->Pt() << " eta: " << jet->Eta() << " phi: " << jet->Phi() << endl;
							cout << "Constituents: " << jet->GetConstituents()->GetEntries() << " Leading particle pT: " << jet->LeadingParticlePt() << endl;
							//cout << "Constituents: " << jet->GetNumberOfConstituents()<< " Leading particle pT: " << jet->MaxTrackPt() << endl;
						//}
					}
					cout << "Number of particle level jets: " << MCJets->GetEntries() << endl;	
					for (int j = 0; j<MCJets->GetEntries(); j++){
						AliJJet *jet = dynamic_cast<AliJJet*>( MCJets->At(j) );
						if (!jet) continue;
						if(abs(jet->Eta()) < 0.26){
							jet->ReSum();
							cout << "Jet " << j << " pt: " << jet->Pt() << " eta: " << jet->Eta() << " phi: " << jet->Phi() << endl;
			...
			this->FillJtHistogramMC(MCJets,i);
			if(!Jets) {
				continue;
			}
