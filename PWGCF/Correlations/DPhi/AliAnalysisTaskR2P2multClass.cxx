...
#include "AliAnalysisTaskR2P2multClass.h"
...
ClassImp(AliAnalysisTaskR2P2multClass)

AliAnalysisTaskR2P2multClass::AliAnalysisTaskR2P2multClass()
: AliAnalysisTaskSE(),
  fNSigmaPID( 2 ),
  fNSigmaPID_veto( 2 ),
  ptUpperLimit( 2.0 ),
...
AliAnalysisTaskR2P2multClass::AliAnalysisTaskR2P2multClass(const TString & name)
...
void AliAnalysisTaskR2P2multClass::UserCreateOutputObjects()
{
  ...
  _nBins_vertexZ     = Int_t( 0.5 + ( _max_vertexZ - _min_vertexZ) / _width_vertexZ );
  
  _nBins_pt_1        = Int_t( 0.5 + ( _max_pt_1 -_min_pt_1 ) / _width_pt_1 );
  //cout<<"\n---------------------------\n"<<endl;
  //cout<<"_min_pt_1: "<< _min_pt_1 <<"\t" <<"_max_pt_1: "<<_max_pt_1<<"\t"<<_width_pt_1<<"\t"<< "_nBins_pt_1: "<<_nBins_pt_1<<endl;
  _nBins_eta_1       = Int_t( 0.5 + ( _max_eta_1-_min_eta_1 ) / _width_eta_1 );
  ...
  _nBins_pt_2        =  Int_t(0.5+ (_max_pt_2 -_min_pt_2 )/_width_pt_2);
  // cout<<"_min_pt_2: "<< _min_pt_2 <<"\t" <<"_max_pt_2: "<<_max_pt_2<<"\t"<<_width_pt_2<<"\t"<< "_nBins_pt_2: "<<_nBins_pt_2<<endl;
  _nBins_eta_2       = Int_t(0.5+ (_max_eta_2-_min_eta_2)/_width_eta_2);
  ...
  _id_1       = new Int_t[arraySize];
  _charge_1   = new Int_t[arraySize];
  _iEtaPhi_1  = new Int_t[arraySize];
  _iPt_1      = new Int_t[arraySize];
  _pt_1       = new Float_t[arraySize];
  _px_1       = new Float_t[arraySize];
  _py_1       = new Float_t[arraySize];
  _pz_1       = new Float_t[arraySize];
  _correction_1 = new Float_t[arraySize];
  _dedx_1     = new Float_t[arraySize];
  ...
  if (_requestedCharge_2!=_requestedCharge_1)
    {
      _sameFilter = 0;
      //particle 2
      _id_2       = new Int_t[arraySize];
      _charge_2   = new Int_t[arraySize];
      _iEtaPhi_2  = new Int_t[arraySize];
      _iPt_2      = new Int_t[arraySize];
      _pt_2       = new Float_t[arraySize];
      _px_2       = new Float_t[arraySize];
      _py_2       = new Float_t[arraySize];
      _pz_2       = new Float_t[arraySize];
      _correction_2 = new Float_t[arraySize];
      _dedx_2       = new Float_t[arraySize];
  ...
  if (_usePtEff)
    {
	  ...
	  for (iPt=0,iPt1=1; iPt<_nBins_pt_1; iPt++, iPt1++)
        ...
        {
	  AliError("AliAnalysisTaskR2P2multClass:: _hPtEff_1 is a null pointer.");
	  return;
        }
      if (!_sameFilter)
        {
	  if (_hPtEff_2)
	      ...
	      for (iPt=0,iPt1=1; iPt<_nBins_pt_2; iPt++, iPt1++)
		{
		  _correctionPtEff_2[iPt] = _hPtEff_2->GetBinContent(iPt1);
            ...
            {
	      AliError("AliAnalysisTaskR2P2multClass:: _hPtEff_2 is a null pointer.");
	      return;
            }
        }
    }
...
//-----------pt-efficiency ends------------
  ...
  if (_useWeights)
    {
      Int_t iZ, iEtaPhi, iPt;
      Int_t iZ1,iEtaPhi1,iPt1;
      Int_t a, b;
      if (_weight_1)
        {
	  _correctionWeight_1 = new Float_t[_nBins_vertexZ*_nBins_etaPhi_1*_nBins_pt_1];
	  a = _nBins_pt_1;
	  ...
	  for (iZ=0,iZ1=1; iZ<_nBins_vertexZ; iZ++, iZ1++)
        ...
        {
	  AliError("AliAnalysisTaskR2P2multClass:: _weight_1 is a null pointer.");
	  return;
        }
      if (!_sameFilter)
        {
	  if (_weight_2)
            {
	      _correctionWeight_2 = new Float_t[_nBins_vertexZ*_nBins_etaPhi_2*_nBins_pt_2];
	      a = _nBins_pt_2;
  ...
  cout<< "AliAnalysisTaskR2P2multClass::CreateOutputObjects() DONE " << endl;
  
}

void  AliAnalysisTaskR2P2multClass::createHistograms()
{
  //  cout << "\n\n\n\n\n\n AliAnalysisTaskR2P2multClass::createHistoHistograms()"<<endl;
  AliInfo("AliAnalysisTaskR2P2multClass::createHistoHistograms() Creating Event Histos");
  TString name;
  ...
  if ( _singlesOnly )
	...
	name = "ptMcTruth_phyPrimary_noSecFromWeakDecay_noSecFromMaterial_noResonance";            _ptMcTruth_phyPrimary_noSecFromWeakDecay_noSecFromMaterial_noResonance   = createHisto1F(name,name, 200, 0.0, 20., "#p_{T}","counts");
      ...
      // name = n1Name + part_1_Name + vsZ + vsEtaPhi + vsPt;        _n1_1_vsZVsEtaVsPhiVsPt = createHisto3F( name, name, _nBins_vertexZ, _min_vertexZ, _max_vertexZ, _nBins_etaPhi_1, 0., Double_t(_nBins_etaPhi_1), _nBins_pt_1, _min_pt_1, _max_pt_1, "zVertex", _title_etaPhi_1,  _title_pt_1);        

      // name = n1Name + part_1_Name +vsEtaPhi;      _wt_1_vsEtaVsPhi       = createHisto1F(name,name, _nBins_etaPhi_1, 0.,        Double_t(_nBins_etaPhi_1), _title_etaPhi_1, _title_wt );
      ...
      //  name = n1Name + part_2_Name + vsZ + vsEtaPhi + vsPt;        _n1_2_vsZVsEtaVsPhiVsPt = createHisto3F( name, name, _nBins_vertexZ, _min_vertexZ, _max_vertexZ, _nBins_etaPhi_2, 0., Double_t(_nBins_etaPhi_2), _nBins_pt_2, _min_pt_2, _max_pt_2, "zVertex", _title_etaPhi_2,  _title_pt_2);	

      //name = n1Name + part_2_Name +vsEtaPhi;      _wt_2_vsEtaVsPhi       = createHisto1F(name,name, _nBins_etaPhi_2, 0.,        Double_t(_nBins_etaPhi_2), _title_etaPhi_2, _title_wt );
  ...
  else
    {
      ...
      name = n2Name+pair_12_Name+vsEtaPhi;      _n2_12_vsEtaPhi       = createHisto1F(name,name, _nBins_etaPhi_12, 0.,        Double_t(_nBins_etaPhi_12), _title_etaPhi_12, _title_AvgN2_12);
      name = n2NwName+pair_12_Name+vsEtaPhi;      _n2Nw_12_vsEtaPhi       = createHisto1F(name,name, _nBins_etaPhi_12, 0.,        Double_t(_nBins_etaPhi_12), _title_etaPhi_12, _title_AvgN2_12);
      name = s2PtPtName+pair_12_Name + vsEtaPhi;_s2PtPt_12_vsEtaPhi   = createHisto1F(name,name, _nBins_etaPhi_12, 0.,        Double_t(_nBins_etaPhi_12), _title_etaPhi_12,  _title_AvgSumPtPt_12);
      name = s2PtNName+pair_12_Name + vsEtaPhi; _s2PtN_12_vsEtaPhi    = createHisto1F(name,name, _nBins_etaPhi_12, 0.,        Double_t(_nBins_etaPhi_12), _title_etaPhi_12,  _title_AvgSumPtN_12);
      name = s2NPtName+pair_12_Name + vsEtaPhi; _s2NPt_12_vsEtaPhi    = createHisto1F(name,name, _nBins_etaPhi_12, 0.,        Double_t(_nBins_etaPhi_12), _title_etaPhi_12,  _title_AvgNSumPt_12);
  ...
  cout << "AliAnalysisTaskR2P2multClass::createHistoHistograms() All Done"<<endl;
  AliInfo(" AliAnalysisTaskR2P2multClass::createHistoHistograms() All Done");
}
//-----------------------//

void  AliAnalysisTaskR2P2multClass::finalizeHistograms()
{
  //  cout << "\n\n\n\n\n\n AliAnalysisTaskR2P2multClass::finalizeHistograms() starting"<<endl;
  AliInfo("\n\n\n\n\n\n AliAnalysisTaskR2P2multClass::finalizeHistograms() starting");
  AliInfo(Form("CorrelationAnalyzers::finalizeHistograms()   _eventCount : %d",Int_t(_eventCount)));
  if (_singlesOnly)
    {
      if (_sameFilter)
        {
  ...
  cout << "AliAnalysisTaskR2P2multClass::finalizeHistograms()  Done "<<endl;
  AliInfo("AliAnalysisTaskR2P2multClass::finalizeHistograms()  Done ");
}
//--------------//


void  AliAnalysisTaskR2P2multClass::UserExec(Option_t */*option*/)
{
  Int_t    k1 = 0, k2 = 0;
  Int_t    k1Truth = 0, k2Truth = 0;
  Int_t    iPhi = 0, iEta = 0, iEtaPhi = 0, iPt = 0, charge = 0;
  Int_t    IDrec = 0;
  Float_t  q = 0.0, phi = 0.0, pt = 0.0, eta = 0.0, y = 0.0, y_direct = 0.0, mass = 0.0, corr = 0.0, corrPt = 0.0, px = 0.0, py = 0.0, pz = 0.0, dedx = 0.0, 
    p = 0.0, l = 0.0, timeTOF = 0.0, beta = 0.0, t0 = 0.0, msquare = 0.0; // Au-Au put p here to make _dedx_p and _beta_p plots. 
  Int_t    ij = 0;
  Int_t    id_1 = 0, q_1 = 0, iEtaPhi_1 = 0, iPt_1 = 0;
  Float_t  pt_1 = 0.0, px_1 = 0.0, py_1 = 0.0, pz_1 = 0.0, corr_1 = 0.0, dedx_1 = 0.0;
  Int_t    id_2 = 0, q_2 = 0, iEtaPhi_2 = 0, iPt_2 = 0;
  Float_t  pt_2 = 0.0, px_2 = 0.0, py_2 = 0.0, pz_2 = 0.0, corr_2 = 0.0, dedx_2 = 0.0;
  Float_t  ptpt = 0.0;
  Int_t    iVertex = 0, iVertexP1 = 0, iVertexP2 = 0;
  Int_t    iZEtaPhiPt = 0;
  Float_t  massElecSq = 1.94797849000000016e-02;  
  //Double_t b[2];
  //Double_t bCov[3];
  const  AliAODVertex*	vertex;
  Int_t    nClus = 0;
  bool   bitOK;
  const Float_t mpion   = 0.139570; // GeV/c2
  const Float_t mkaon   = 0.493677; // GeV/c2
  const Float_t massKaonSq = 0.2437169803; // GeV/c2
  const Float_t mproton = 0.938272; // GeV/c2
  Double_t c = TMath::C() * 1.E-9;// m/ns
  Double_t EP = 0;
    
  Float_t slope_vZ = 0.;
  Float_t vZ_bin_center = 0.;
  Int_t iVertexZ_plus1 = 0;
  Int_t iVertexZ_minus1 = 0;
  Int_t iVertexP1_vZplus1 = 0;
  Int_t iVertexP1_vZminus1 = 0;
  Int_t iVertexP2_vZplus1 = 0;
  Int_t iVertexP2_vZminus1 = 0;
  Int_t iZEtaPhiPt_vZplus1 = 0;
  Int_t iZEtaPhiPt_vZminus1 = 0;

  Float_t slope_y = 0.;
  Float_t y_bin_center = 0.;
  Int_t iEta_plus1 = 0;
  Int_t iEta_minus1 = 0;
  Int_t iEtaPhi_Etaplus1 = 0;
  Int_t iEtaPhi_Etaminus1 = 0;
  Int_t iZEtaPhiPt_Etaplus1 = 0;
  Int_t iZEtaPhiPt_Etaminus1 = 0;
  Double_t nsigmaElectron = 999.;

  Float_t v0Centr2  = -999.;  
  ...
  Float_t v0Centr  = -999.;
  Float_t v0ACentr = -999.;
  Float_t v0CCentr = -999.;
  Float_t trkCentr = -999.;
  Float_t spdCentr = -999.;
  Float_t spdTracklet = -999.;
  Float_t vertexX  = -999;
  Float_t vertexY  = -999;
  Float_t vertexZ  = -999;
  //Float_t vertexXY = -999;
  Float_t dcaZ     = -999;
  Float_t dcaXY    = -999;
  ...
  if( fAODEvent )
      ...
      if ( fSystemType == "pp18_V0_kMB_kFALSE")
	{
      ...
      iVertex = Int_t( ( vertexZ - _min_vertexZ ) / _width_vertexZ );
      iVertexP1 = iVertex*_nBins_etaPhiPt_1;
      iVertexP2 = iVertex*_nBins_etaPhiPt_2;
          if ( iVertex<0 || iVertex >= _nBins_vertexZ )
        {
	  AliError("AliAnalysisTaskR2P2multClass::Exec(Option_t *option) iVertex<0 || iVertex>=_nBins_vertexZ ");
	  ...
	  if ( fSystemType == "pp18_V0_kMB_kFALSE")
	      ...
	      if ( fAnalysisType == "RealData" || fAnalysisType == "MCAODreco" )
		{
		  ...
		  for (Int_t iTrack = 0; iTrack < _nTracks; iTrack++ )
		    {
		  ...
		  if ( _singlesOnly )
			...
			{
			  iPhi   = Int_t( phi/_width_phi_1);
			  
			  if (iPhi<0 || iPhi>=_nBins_phi_1 )
			    {
			      AliWarning("AliAnalysisTaskR2P2multClass::analyze() iPhi<0 || iPhi>=_nBins_phi_1");
			      return;
			    }
			  
			  iEta    = Int_t((eta-_min_eta_1)/_width_eta_1);
			  if (iEta<0 || iEta>=_nBins_eta_1)
			    {
			      AliWarning(Form("AliAnalysisTaskR2P2multClass::analyze(AliceEvent * event) Mismatched iEta: %d", iEta));
			      continue;
			    }
			  iPt     = Int_t((pt -_min_pt_1 )/_width_pt_1 );
			  if (iPt<0  || iPt >=_nBins_pt_1)
			    {
			      AliWarning(Form("AliAnalysisTaskR2P2multClass::analyze(AliceEvent * event) Mismatched iPt: %d",iPt));
			      continue;
			    }
			  ...
			  if ( iZEtaPhiPt < 0 || iZEtaPhiPt >= _nBins_zEtaPhiPt_1 )
			    {
			      AliWarning("AliAnalysisTaskR2P2multClass::analyze(AliceEvent * event) iZEtaPhiPt<0 || iZEtaPhiPt>=_nBins_zEtaPhiPt_1");
			      continue;
			    }                
			  ...
			  if ( _singlesOnly )
			      ...
			      if ( fAnalysisType == "MCAODreco" ) // This block of code used to get PdgCode for MC reco tracks
				{
			      ...
			      if (k1>=arraySize)
				{
				  AliError(Form("AliAnalysisTaskR2P2multClass::analyze(AliceEvent * event) k1 >=arraySize; arraySize: %d",arraySize));
				  return;
				}
			    }
			}//if ( _requestedCharge_1 == charge )
			...
			//&& dedx >=  _dedxMin && dedx < _dedxMax)                
			{
			  ...
			  iEta    = Int_t((eta-_min_eta_2)/_width_eta_2);
			  if (iEta<0 || iEta>=_nBins_eta_2)
			    {
			      AliWarning(Form("AliAnalysisTaskR2P2multClass::analyze(AliceEvent * event) Mismatched iEta: %d", iEta));
			      continue;
			    }
			  iPt     = Int_t((pt -_min_pt_2 )/_width_pt_2 );
			  ...
			  if (iZEtaPhiPt<0 || iZEtaPhiPt>=_nBins_zEtaPhiPt_2)
			    {
			      AliWarning("AliAnalysisTaskR2P2multClass::analyze(AliceEvent * event) iZEtaPhiPt<0 || iZEtaPhiPt>=_nBins_zEtaPhiPt_2");
			      continue;
			    }
		    ...
		    } //Track Loop ends here //for (Int_t iTrack=0; iTrack< _nTracks; iTrack++)
	  ...
	  for ( Int_t iTrack = 0; iTrack < _nTracks; iTrack++ )
	    {
	      ...
	      charge = Int_t( q/3. ); // particle charges are 3s in HIJING truth
	   ...
	   /*   //Exclude resonances
	      if( fExcludeResonancesInMC )
		{
		  ...
		  if( gMotherIndex != -1 ) {
		    ...
		    if( motherTrack ) {
		      ...
		      if (_singlesOnly) _ptMcTruth_phyPrimary_noSecFromWeakDecay_noSecFromMaterial_noResonance->Fill(pt);
		    }
		  }
		}
		...
		{		  
	      ...
	      /*
		const Float_t mpion = 0.1395701835; // GeV/c2
		const Float_t mkaon = 0.493667; // GeV/c2
		const Float_t mproton = 0.93827204621; // GeV/c2
		if ( particleSpecies == 0 )  mass = mpion;
		if ( particleSpecies == 1 )  mass = mkaon;
		if ( particleSpecies == 2 )  mass = mproton;
		...
		// electron: +/- 11
		// muon: +/- 13
		// If you get any other number, Double_t-check!
		Double_t mass = TParticlePDG::Mass( pdg );
	      */    
  ...
  else
    {
	  ...
	  if (_field>0)
		  ...
		  for (Int_t i2=i1+1; i2<k1; i2++)
			  ...
			  if ( fAnalysisType == "MCAODreco" || fAnalysisType == "RealData"){
			    ...
			    if( TMath::Abs(_TrackArray[i1]->Eta()-_TrackArray[i2]->Eta()) < (3.*4.*_max_eta_1/(2.*_nBins_eta_1-1.)) && TMath::Abs(_TrackArray[i1]->Phi()-_TrackArray[i2]->Phi()) < (3.*2.*TMath::Pi()/(Double_t)_nBins_phi_1) )
			  ...
			  if ( particleSpecies == 1 )  // invariant mass for kaon-kaon pairs
			    {
			      Float_t EngyKaon1Sq = massKaonSq + pt_1*pt_1 + pz_1*pz_1;
			      Float_t EngyKaon2Sq = massKaonSq + pt_2*pt_2 + pz_2*pz_2;
			      Float_t mInvKaonSq = 2*(massKaonSq + sqrt(EngyKaon1Sq*EngyKaon2Sq) - px_1*px_2 - py_1*py_2 - pz_1*pz_2 );
			      Float_t mInvKaon = sqrt(mInvKaonSq);
	  ...
	  for (Int_t i1=0; i1<k1; i1++)
		  ...
		  if (id_1!=id_2)  // exclude auto correlation
			...
			{
			  Float_t e1Sq = massElecSq + pt_1*pt_1 + pz_1*pz_1;
			  Float_t e2Sq = massElecSq + pt_2*pt_2 + pz_2*pz_2;
			  Float_t mInvSq = 2*(massElecSq + sqrt(e1Sq*e2Sq) - px_1*px_2 
					    - py_1*py_2 - pz_1*pz_2 );
			  Float_t mInv = sqrt(mInvSq);
  ...
  AliInfo("AliAnalysisTaskR2P2multClass::UserExec()   -----------------Event Done ");
  PostData(1,_outputHistoList);

  //  cout <<"AliAnalysisTaskR2P2multClass::UserExec()  Done "<<endl;
} // End of UserExec

void   AliAnalysisTaskR2P2multClass::FinishTaskOutput()
{
  AliInfo("AliAnalysisTaskR2P2multClass::FinishTaskOutput() Starting.");
  Printf("= 0 ====================================================================");
  finalizeHistograms();
  AliInfo("= 1 ====================================================================");
  PostData(1,_outputHistoList);
  AliInfo("= 2 ====================================================================");
  AliInfo("AliAnalysisTaskR2P2multClass::FinishTaskOutput() Done.");
}

void   AliAnalysisTaskR2P2multClass::Terminate(Option_t* /*option*/)
{
  AliInfo("AliAnalysisTaskR2P2multClass::Terminate() Starting/Done.");
}


//Tools
//===================================================================================================
void  AliAnalysisTaskR2P2multClass::fillHistoWithArray(TH1 * h, Float_t * array, Int_t size)
{
  Int_t i, i1;
  Float_t v1, ev1, v2, ev2, sum, esum;
...
void  AliAnalysisTaskR2P2multClass::fillHistoWithArray(TH2 * h, Float_t * array, Int_t size1, Int_t size2)
{
  Int_t i, i1;
  Int_t j, j1;
  Float_t v1, ev1, v2, ev2, sum, esum;
...
void  AliAnalysisTaskR2P2multClass::fillHistoWithArray(TH3 * h, Float_t * array, Int_t size1, Int_t size2, Int_t size3)
{
  Int_t i, i1;
  Int_t j, j1;
  Int_t k, k1;
  Float_t v1, ev1, v2, ev2, sum, esum;
  Int_t size23 = size2*size3;
  for (i=0, i1=1; i<size1; ++i,++i1)
    {
      for (j=0, j1=1; j<size2; ++j,++j1)
        {
...
void  AliAnalysisTaskR2P2multClass::fillHistoWithArray(TH1 * h, Double_t * array, Int_t size)
{
  Int_t i, i1;
  Double_t v1, ev1, v2, ev2, sum, esum;
  for (i=0, i1=1; i<size; ++i,++i1)
    {
      v1  = array[i]; ev1 = sqrt(v1);
      v2  = h->GetBinContent(i1);
...
void  AliAnalysisTaskR2P2multClass::fillHistoWithArray(TH2 * h, Double_t * array, Int_t size1, Int_t size2)
{
  Int_t i, i1;
  Int_t j, j1;
  Double_t v1, ev1, v2, ev2, sum, esum;
...
void  AliAnalysisTaskR2P2multClass::fillHistoWithArray(TH3 * h, Double_t * array, Int_t size1, Int_t size2, Int_t size3)
{
  Int_t i, i1;
  Int_t j, j1;
  Int_t k, k1;
...
Double_t *  AliAnalysisTaskR2P2multClass::getDoubleArray(Int_t size, Double_t v)
{
  /// Allocate an array of type Double_t with n values
  /// Initialize the array to the given value
  Double_t * array = new Double_t [size];
  for (Int_t i=0;i<size;++i) array[i]=v;
  return array;
}

//________________________________________________________________________
Float_t *  AliAnalysisTaskR2P2multClass::getFloatArray(Int_t size, Float_t v)
{
  /// Allocate an array of type Float_t with n values
  /// Initialize the array to the given value
  Float_t * array = new Float_t [size];
  for (Int_t i=0;i<size;++i) array[i]=v;
  return array;
}
...
//________________________________________________________________________
TH1D * AliAnalysisTaskR2P2multClass::createHisto1D(const TString &  name, const TString &  title, 
...
//________________________________________________________________________
TH2D * AliAnalysisTaskR2P2multClass::createHisto2D(const TString &  name, const TString &  title, 
					  Int_t nx, Double_t xMin, Double_t xMax, Int_t ny, Double_t yMin, Double_t yMax, 
...
//________________________________________________________________________
TH2D * AliAnalysisTaskR2P2multClass::createHisto2D(const TString &  name, const TString &  title, 
					  Int_t nx, Double_t* xbins, Int_t ny, Double_t yMin, Double_t yMax, 
					  const TString &  xTitle, const TString &  yTitle, const TString &  zTitle)
{
...
//// F /////
//________________________________________________________________________
TH1F * AliAnalysisTaskR2P2multClass::createHisto1F(const TString &  name, const TString &  title, 
					  Int_t n, Double_t xMin, Double_t xMax, 
					  const TString &  xTitle, const TString &  yTitle)
{
  //CreateHisto new 1D historgram
...
//________________________________________________________________________
TH2F * AliAnalysisTaskR2P2multClass::createHisto2F(const TString &  name, const TString &  title, 
					  Int_t nx, Double_t xMin, Double_t xMax, Int_t ny, Double_t yMin, Double_t yMax, 
					  const TString &  xTitle, const TString &  yTitle, const TString &  zTitle)
{
  AliInfo(Form("createHisto 2D histo %s  nx: %d  xMin: %f10.4 xMax: %f10.4  ny: %d   yMin: %f10.4 yMax: %f10.4",name.Data(),nx,xMin,xMax,ny,yMin,yMax));
...
//________________________________________________________________________
TH3F * AliAnalysisTaskR2P2multClass::createHisto3F(const TString &  name, const TString &  title, 
...
//________________________________________________________________________
TProfile * AliAnalysisTaskR2P2multClass::createProfile(const TString & name, const TString & description, 
					      Int_t nx,Double_t xMin,Double_t xMax,
					      const TString &  xTitle, const TString &  yTitle)
{
...
//________________________________________________________________________
void   AliAnalysisTaskR2P2multClass::addToList(TH1 *h)
{
  if (_outputHistoList)
    {
...
void AliAnalysisTaskR2P2multClass::CalculateNSigmas( AliVTrack * trk )   //defines data member fnsigmas
{  
...
//____________________________________________________________________________________________________
void AliAnalysisTaskR2P2multClass::CalculateTPCNSigmasElectron( AliVTrack * trk )   //defines data member fnsigmas
{  
  // Compute nsigma for each hypthesis
  AliVParticle * inEvHMain = dynamic_cast < AliVParticle * > ( trk );
  // --- TPC
...
void AliAnalysisTaskR2P2multClass::CheckTOF( AliVTrack * trk )     //check if the particle has TOF Matching
{ 
  //get the PIDResponse
...
Int_t AliAnalysisTaskR2P2multClass::TellParticleSpecies( AliVTrack * trk )  //function to determine the particle ID
{  
...
//________________________________________________________________________________________________________________
Int_t AliAnalysisTaskR2P2multClass::TellParticleSpecies_CircularCut( AliVTrack * trk )  //function to determine the particle ID
{  
  CalculateNSigmas( trk );
...
//________________________________________________________________________________________________________________
Int_t AliAnalysisTaskR2P2multClass::TellParticleSpecies_by_P( AliVTrack * trk )  //function to determine the particle ID
{  
  CalculateNSigmas( trk );
...
//________________________________________________________________________________________________________________
Int_t AliAnalysisTaskR2P2multClass::TellParticleSpecies_by_P_CircularCut( AliVTrack * trk )  //function to determine the particle ID
{  
  CalculateNSigmas( trk );
...
//________________________________________________________________________________________________________________
Double_t AliAnalysisTaskR2P2multClass::TOFBetaCalculation( AliVTrack * track ) const
{
  Double_t tofTime = 0;
...
//________________________________________________________________________________________________________________
Double_t AliAnalysisTaskR2P2multClass::massSquareCalculation( AliVTrack * track ) const
{
...
//________________________________________________________________________________________________________________
Float_t AliAnalysisTaskR2P2multClass::TPC_EventPlane(AliAODEvent *fAOD)
{
...
//________________________________________________________________________________________________________________
Bool_t AliAnalysisTaskR2P2multClass::Is2015PileUpEvent()
{
...
//________________________________________________________________________________________________________________
Bool_t AliAnalysisTaskR2P2multClass::StoreEventMultiplicities(AliVEvent *event)
{  
...
//________________________________________________________________________________________________________________
Double_t AliAnalysisTaskR2P2multClass::CalculateSharedFraction (const TBits *triggerClusterMap,const TBits *assocClusterMap,const TBits *triggerShareMap,const TBits *assocShareMap)
{
  Double_t nofhits=0;
