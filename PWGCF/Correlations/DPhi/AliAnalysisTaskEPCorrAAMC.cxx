 ...
 */
#include "AliAnalysisTaskEPCorrAAMC.h"
...
ClassImp(AliAnalysisTaskEPCorrAAMC)
ClassImp(AliCorrReducedTrackAAMC)

/*
//________________________________________________________________________
AliAnalysisTaskEPCorrAAMC::AliAnalysisTaskEPCorrAAMC() // All data members should be initialised here
	:AliAnalysisTaskSE(),
	fOutput(0),
//	fTrackCuts(0),
	fPoolMgr(0x0),
...
//________________________________________________________________________
AliAnalysisTaskEPCorrAAMC::AliAnalysisTaskEPCorrAAMC(const char *name) // All data members should be initialised here
...
//________________________________________________________________________
AliAnalysisTaskEPCorrAAMC::~AliAnalysisTaskEPCorrAAMC()
{
...
//________________________________________________________________________
void AliAnalysisTaskEPCorrAAMC::UserCreateOutputObjects()
{
	// Create histograms
...
*/
	...
	// call MC handler
//	AliMCEventHandler *mceventHandler = (AliMCEventHandler*)anamanager->GetMCtruthEventHandler();
	AliMCEventHandler *mceventHandler = dynamic_cast<AliMCEventHandler*>(anamanager->GetMCtruthEventHandler());
...
//________________________________________________________________________
void AliAnalysisTaskEPCorrAAMC::UserExec(Option_t *) 
{
	// Main loop
...
//	AliMCEvent *mcevent = mceventHandler->MCEvent();
...
*/
	...
	if(!(mcevent->GetPrimaryVertex())) return;
...
//	}
	...
	Float_t cent = -99;
//	cent = mcevent->GetCentrality()->GetCentralityPercentile("V0M"); // instead, take recon. cent
...
//*/
/*
	// determine cBin with impact parameter in MC
	float impactPara = -1;
	int cBin = -1;
	impactPara = headerH->ImpactParameter();
	cout << "cent. from recon. = " << cent << endl;
	cout << "impact parameter = " << impactPara << endl;
	if(headerH) {
		if(impactPara < 3.50) cBin = 0;
		else if(impactPara < 4.94) cBin = 1;
		else if(impactPara < 9.88) cBin = 2;
		else if(impactPara < 13.05) cBin = 3; // corresponds to 80%
...
//	AliEventplane *evp = mcevent->GetEventplane();
//	AliEventplane *evp = event->GetEventplane();

	// in MC get true centrality and true EP (i.e., reaction-plane with following way)
//	AliCollisionGeometry* headerH = new AliCollisionsGeometry();
//	fCollGeo = (AliCollisionGeometry*) mcevent->GenEventHeader();
	fCollGeo = dynamic_cast<AliCollisionGeometry*>(mcevent->GenEventHeader());

 	if (fCollGeo) {
		EPangle = fCollGeo->ReactionPlaneAngle();
		if(EPangle<0) EPangle = EPangle + TMath::Pi(); // V0 EP angle varies from -pi/2 to +pi/2, so need to shift

//		cout << "ReactionPlane angle = " << EPangle << endl;
...
/*
	if(evp) {
		EPangle = evp->GetEventplane("V0",mcevent, 2); // calculated by whold V0 for second harmonics
		EPangleVzeroA = evp->GetEventplane("V0A",mcevent, 2);
	 	EPangleVzeroC = evp->GetEventplane("V0C",mcevent, 2);
...
//	float zBinArray[] = {-8, -6, -4, -2, 0, 2, 4, 6, 8}; // 8 bins
	...
	float zvertex = -999;
	zvertex = mcevent->GetPrimaryVertex()->GetZ();
...
//	cout << "ntracks = " << ntracks << endl;
	...
	//	cout << "DEBUG5" << endl;
		AliVParticle *mctrack = (AliVParticle*)mcevent->GetTrack(i); // pointer to reconstructed to track          

//		cout << "track number i = " << i << endl;

		TParticle *mctruth = stack->Particle(i); // Truth

		if(!mctrack) { 
			AliError(Form("ERROR: Could not retrieve mctrack %d",i)); 
			continue; 
		}

		if(!mctruth) { 
			AliError(Form("ERROR: Could not retrieve mctruth %d",i)); 
			continue; 
		}
...
//        if(aodtrack->GetType() != AliAODTrack::kPrimary) continue;
		...
		if(!mctruth->IsPrimary()) continue;
//		if(!mctrack->IsPhysicalPrimary(i)) continue;
...
//        if(aodtrack->IsGlobalConstrained()==kTRUE) continue; // need to be False for hybrid track



		float trackpT = mctrack->Pt();
		float trackEta = mctrack->Eta(); 
		float trackPhi = mctrack->Phi();

//		float trackZv = mctrack->GetZ();
		float trackZv = mctrack->Charge();
...
/*
   ...
   _Hadron_MC_160428_local_LHC13f3a_AMPT_StrOn_ResOff_TEST.root:
	AliEventplane *evp = mcevent->GetEventplane();
...
*/
		...
		// 160501 to reduced the time for scan, only 8< pT,trig < 15 will be stored.
		// For AMPT MC, ntrack~100k so takes too long.
		if(trackpT<8.0) continue;
...
//		cout << "track number j = " << j << endl;


			AliVParticle* mctrack2 = (AliVParticle*)mcevent->GetTrack(j); // pointer to reconstructed to track          
			TParticle *mctruth2 = stack->Particle(j); // Truth



			if(!mctrack2) { 
				AliError(Form("ERROR: Could not retrieve mctrack %d",j)); 
				continue; 
			}
...
//        if(!(aodtrack2->IsHybridGlobalConstrainedGlobal())) continue;
//        if(aodtrack2->IsGlobalConstrained()==kTRUE) continue; // need to be False for hybrid track

			if(!mctruth2->IsPrimary()) continue;


			float trackpT2 = mctrack2->Pt();
			float trackEta2 = mctrack2->Eta(); 
			float trackPhi2 = mctrack2->Phi();

//			float trackZv2 = mctrack2->GetZ();
			float trackZv2 = mctrack2->Charge();

//			cout << "trackEta = " << trackEta << " trackEta2 = " << trackEta2 << endl;
...
*/
	...
	fMyprimRecoTracks = AcceptTracksReducedMC(mcevent, kTrackCut);
...
//________________________________________________________________________
void AliAnalysisTaskEPCorrAAMC::SetupForMixing()
{
...
//________________________________________________________________________
void AliAnalysisTaskEPCorrAAMC::FillMixedHistos(TObjArray* partNew, TObjArray* partMix, int cBin, int zBin, double EPangle, float bSignHistos, double weight)
{
...
//		if(!fTrackCuts->AcceptTrack( particle->At(targetindex) )) continue;
		...
		firstpT = ((AliCorrReducedTrackAAMC*)particle->At(targetindex))->Pt();
		firstEta = ((AliCorrReducedTrackAAMC*)particle->At(targetindex))->Eta();
		firstPhi = ((AliCorrReducedTrackAAMC*)particle->At(targetindex))->Phi();
//		firstZv = ((AliCorrReducedTrackAAMC*)particle->At(targetindex))->Zv(); // not implemented in Reduced track....
        firstZv = ((AliCorrReducedTrackAAMC*)particle->At(targetindex))->Charge(); 
...
//		float pTBinArray[] = { 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 6.0, 8.0, 15.0 }; // 10 bins
		...
		{ 
//			AliCorrReducedTrackAAMC* second = dynamic_cast<AliCorrReducedTrackAAMC*>(particleMixed->At(is));
...
*/
            secondpT = ((AliCorrReducedTrackAAMC*)particleMixed->At(is))->Pt();
            secondEta = ((AliCorrReducedTrackAAMC*)particleMixed->At(is))->Eta();
            secondPhi = ((AliCorrReducedTrackAAMC*)particleMixed->At(is))->Phi();
            secondZv = ((AliCorrReducedTrackAAMC*)particleMixed->At(is))->Charge();
            secondPID = ((AliCorrReducedTrackAAMC*)particleMixed->At(is))->GetMyPartID();
...
//________________________________________________________________________
void AliAnalysisTaskEPCorrAAMC::DoMixing(double cent, double zvertex, double EPangle, TObjArray* trigTracks, float bSignDoMixing)
{
	cout << "Event Mixing is started! ================" << endl;
...
//________________________________________________________________________
TObjArray* AliAnalysisTaskEPCorrAAMC::AcceptTracksReducedMC(AliMCEvent *mcevent, bool useCuts) {
	Int_t nTracks = mcevent->GetNumberOfTracks();
	...
	{
		AliVParticle *track = dynamic_cast<AliVParticle*>(mcevent->GetTrack(itrk));
...
//        if(track->IsGlobalConstrained()==kTRUE) continue; // need to be False for hybrid track

			TParticle *mctruth = mcevent->Stack()->Particle(itrk); // Truth
			if(!mctruth) { 
				AliError(Form("ERROR: Could not retrieve mctruth %d",itrk)); 
				continue; 
			}
			if(!mctruth->IsPrimary()) continue;
...
*/
		...
		tracksAccepted->Add(new AliCorrReducedTrackAAMC(myPartID,track->Eta(),track->Phi(),track->Pt(),track->Zv(),track->Charge()));
...
int AliAnalysisTaskEPCorrAAMC::GetParticleID(AliVParticle* track)
{
...
float AliAnalysisTaskEPCorrAAMC::CalculatedPhiStar(float dPhi, float dEta, float Zv, float Zv2, float pT, float pT2, float bSigntmp) {
...
//________________________________________________________________________
void AliAnalysisTaskEPCorrAAMC::Terminate(Option_t *) 
{
