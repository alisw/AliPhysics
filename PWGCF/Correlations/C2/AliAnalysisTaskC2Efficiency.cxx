...
void AliAnalysisTaskC2::UserCreateOutputObjects()
{
  ...
  const Double_t cent_bin_edges[] = {0, 1, 5, 50, 80, 101};
  const Int_t nCent    = sizeof(cent_bin_edges)/sizeof(cent_bin_edges[0]) - 1;
  const Int_t nZvtx    = 20;
  enum                            {kEta1,    kEta2,    kPhi1,    kPhi2,    kCent, kZvtx};
  const Int_t nbins[ndim_pairs] = {etaNbins, etaNbins, phiNbins, phiNbins, nCent, nZvtx};
  const Double_t xmin[ndim_pairs] = {-0.8, -0.8, 0, 0, 0, -10};
  const Double_t xmax[ndim_pairs] = {0.8, 0.8, 2*TMath::Pi(), 2*TMath::Pi(), 101, 10};
  
  this->fPairs = new THnS("pairs", "<N_{1}N_{2}>;#eta_{1};#eta_{2};#phi_{1};#phi_{2};cent;z_{vtx};",
			  ndim_pairs, nbins, xmin, xmax);
  this->fPairs->GetAxis(kCent)->Set(nCent, cent_bin_edges);
  // exclude_over_under_flow(this->fPairs);
  this->fOutputList->Add(fPairs);

  {
    const Int_t ndims_single1 = 4;
    Int_t nbins_single1[ndims_single1] = {nbins[kEta1], nbins[kPhi1], nbins[kCent], nbins[kZvtx]};
    Double_t xmin_single1[ndims_single1] = {xmin[kEta1], xmin[kPhi1], xmin[kCent], xmin[kZvtx]};
    Double_t xmax_single1[ndims_single1] = {xmax[kEta1], xmax[kPhi1], xmax[kCent], xmax[kZvtx]};
    this->fSingle1 = new THnF("single1", "<N_{1}>;#eta_{1};#phi_{1};cent;z_{vtx};",
			      ndims_single1, nbins_single1, xmin_single1, xmax_single1);
    this->fSingle1->GetAxis(2)->Set(nCent, cent_bin_edges);
    this->fOutputList->Add(this->fSingle1);
  }
  {
    const Int_t ndims_single2 = 4;
    Int_t nbins_single2[ndims_single2] = {nbins[kEta2], nbins[kPhi2], nbins[kCent], nbins[kZvtx]};
    Double_t xmin_single2[ndims_single2] = {xmin[kEta2], xmin[kPhi2], xmin[kCent], xmin[kZvtx]};
    Double_t xmax_single2[ndims_single2] = {xmax[kEta2], xmax[kPhi2], xmax[kCent], xmax[kZvtx]};
    this->fSingle2 = new THnF("single2", "<N_{2}>;#eta_{2};#phi_{2};cent;z_{vtx};",
				    ndims_single2, nbins_single2, xmin_single2, xmax_single2);
    this->fSingle2->GetAxis(2)->Set(nCent, cent_bin_edges);
    this->fOutputList->Add(this->fSingle2);
  }
  {
    // Event counter
    const Int_t ndims_evt_counter = 2;
    Int_t nbins_evt_counter[ndims_evt_counter] = {nbins[kCent], nbins[kZvtx]};
    Double_t xmin_evt_counter[ndims_evt_counter] = {xmin[kCent], xmin[kZvtx]};
    Double_t xmax_evt_counter[ndims_evt_counter] = {xmax[kCent], xmax[kZvtx]};
    this->fEventCounter = new THnF("eventConter", "Event counter;cent;z_{vtx};",
					 ndims_evt_counter, nbins_evt_counter, xmin_evt_counter, xmax_evt_counter);
    this->fEventCounter->GetAxis(0)->Set(nCent, cent_bin_edges);
  ...
  // Histogram to construct efficiency in eta, phi, pt, cent, z_vtx
  ...
  Int_t nbins_eff[ndims_eff] = {etaNbins, phiNbins, npTbins, nbins[kCent], nbins[kZvtx]};
  // Dummy values for pT; the axis is later changed to variable bin sizes!
  Double_t xmin_eff[ndims_eff] = {xmin[kEta2], xmin[kPhi2], 0.0, xmin[kCent], xmin[kZvtx]};
  Double_t xmax_eff[ndims_eff] = {xmax[kEta2], xmax[kPhi2], 100.0, xmax[kCent], xmax[kZvtx]};
  this->fSingleParticleForEfficiency = new THnF("singleParticleForEfficiency",
						"Single particle density for efficiency evaluation;"
						"#eta;#phi;p_{T};cent;z_{vtx};",
						ndims_eff, nbins_eff, xmin_eff, xmax_eff);
  this->fSingleParticleForEfficiency->GetAxis(2)->Set(npTbins, pt_bin_edges);
  this->fSingleParticleForEfficiency->GetAxis(3)->Set(nCent, cent_bin_edges);
...
void AliAnalysisTaskC2::UserExec(Option_t *)
{
  ...
  while (TObject* obj = nextTrack()){
    ...
    {
      Double_t stuffing[5] = {tmp_track.eta, tmp_track.phi, tmp_track.pt, centrality, zvtx};
