...
void AliAnalysisTaskPtCorr::UserCreateOutputObjects(){
  ...
  constexpr int nbinsV0M = 1000;
  constexpr double binV0Mlow = 0;
  constexpr double binV0Mhigh = 45000;
  fPtContV0Mmult = new AliPtPtContainer("ptcont_V0M_ch","ptcont_V0M_ch",nbinsV0M,binV0Mlow,binV0Mhigh,fPtMpar);
  ...
  fPtContET = new AliPtPtContainer("ptcont_ET_ch","ptcont_ET_ch",fNETBins,fETBins,fPtMpar);
  fPtContET->SetEventWeight(fEventWeight);
  fptList->Add(fPtContET);
  if(fNBootstrapProfiles) fPtContET->InitializeSubsamples(fNBootstrapProfiles);
  ...
  hCentvsV0M = new TH2D("hCentvsV0M","V0M amplitude vs Centrality; V0M amplitude; Centrality (%)",nbinsV0M,binV0Mlow,binV0Mhigh,nDefaultCentBins,defaultCentBins);
  fptList->Add(hCentvsV0M);
  hCentvsET = new TH2D("hCentvsET","ET vs Centrality; #Sigma E_{T}; Centrality (%)",fNETBins,fETBins,nDefaultCentBins,defaultCentBins);
  fptList->Add(hCentvsET);
  ...
  hETvsNch = new TH2D("hETvsNch","ET vs Nch; N_{ch}; #Sigma E_{T}",fNMultiBins,fMultiBins,fNETBins,fETBins);
  ...
  pMeanNchvsET = new TProfile("pmeanNchvsET","#LT N_{ch}#GT vs #Sigma E_{T}; #Sigma E_{T}; #LT N_{ch}#GT",fNETBins,fETBins);
...
void AliAnalysisTaskPtCorr::UserExec(Option_t*) {
  ...
  if(vzero) {
    ...
    multV0M += vzero->GetMultiplicity(ich);
  }
  ...
  if(fIsMC) {
    ...
    for(Int_t ipart = 0; ipart < nPrim; ipart++) {
      ...
      if((2.8 < lPart->Eta() && lPart->Eta() < 5.1) || (-3.7 < lPart->Eta() && lPart->Eta() < -1.7)) ++multV0MMC;
      ...
      etfull += TMath::Sqrt(TMath::Power(lPart->Pt(), 2.0) + TMath::Power(masspi, 2.0));
    };
    nTotNoTracks = fUseRecoNchForMC?nTotNoTracksReco:nTotNoTracksMC;
    multV0M = fUseRecoNchForMC?multV0M:multV0MMC;
    ...
    for(Int_t lTr=0;lTr<fAOD->GetNumberOfTracks();lTr++) {
      ...
      etfull += TMath::Sqrt(TMath::Power(lTrack->Pt(), 2.0) + TMath::Power(masspi, 2.0));
      if(fFillQA){
        double dcaxyz[2];
  ...
  fPtContET->CalculateCorrelations(wp);
  fPtContET->FillProfiles(etfull,l_Random);
  ...
  hCentvsV0M->Fill(multV0M,l_Cent);
  hCentvsET->Fill(etfull,l_Cent);
...
void AliAnalysisTaskPtCorr::ProcessOnTheFly() {
  ...
  for(Int_t i=0;i<nTracks;i++) {
    ...
    //Hardcoded cuts to inhereted from AcceptAODTrack
    if(lPart->Pt()<ptMin || lPart->Pt()>ptMax) continue;
    if((2.8 < lPart->Eta() && lPart->Eta() < 5.1) || (-3.7 < lPart->Eta() && lPart->Eta() < -1.7)) ++multV0M;
  ...
  pMeanNchvsV0M->Fill(multV0M,l_Multi);
  PostData(1,fptList);
  PostData(2,fQAList);
  return;
}
void AliAnalysisTaskPtCorr::ProcessGen(){
  ...
  for (Int_t ipart = 0; ipart < fMCEvent->GetNumberOfTracks(); ++ipart){
    ...
    if(fEtaMultAcceptance[0] < letaabs && letaabs < fEtaMultAcceptance[1]) {
      ...
      etfull += TMath::Sqrt(TMath::Power(lpt, 2.0) + TMath::Power(masspi, 2.0));
    }
  }
...
void AliAnalysisTaskPtCorr::SetupAxes() {
 const Int_t temp_NV0MBinsDefault=10;
  ...
  const int nbinsET = 1250;
  double* etbins = new double[nbinsET + 1];
  for (int i = 0; i <= nbinsET; ++i) etbins[i] = 0.0 + (2.0 * i);
  if(!fETAxis) { printf("Setting default ET bins\n"); SetETBins(nbinsET,etbins); }
  fNETBins = fETAxis->GetNbins();
  fETBins = GetBinsFromAxis(fETAxis);
  return;
}
...
void AliAnalysisTaskPtCorr::SetETBins(Int_t nbins, Double_t *etbins) {
  if(fETAxis) delete fETAxis;
  fETAxis = new TAxis(nbins,etbins);
}
