...
#include "TMath.h"
AliGFWWeightsFinerBin::AliGFWWeightsFinerBin():
  fDataFilled(kFALSE),
  fMCFilled(kFALSE),
  fW_data(0),
  fW_mcrec(0),
...
AliGFWWeightsFinerBin::AliGFWWeightsFinerBin(const AliGFWWeightsFinerBin& target):
  TNamed(target)
{
...
void AliGFWWeightsFinerBin::SetPtBins(Int_t Nbins, Double_t *bins) {
  if(fbinsPt) delete [] fbinsPt;
  fNbinsPt = Nbins;
  fbinsPt = new Double_t[fNbinsPt+1];
  for(Int_t i=0;i<=fNbinsPt;++i) fbinsPt[i] = bins[i];
};
void AliGFWWeightsFinerBin::Init(Bool_t AddData, Bool_t AddMC)
{
  // fDataFilled = kFALSE;
  // fMCFilled = kFALSE;
  ...
  if(AddMC) {
    ...
    fW_mcrec->SetName("AliGFWWeightsFinerBin_MCRec");
    fW_mcgen = new TObjArray();
    fW_mcgen->SetName("AliGFWWeightsFinerBin_MCGen");
...
void AliGFWWeightsFinerBin::Fill(Double_t phi, Double_t eta, Double_t vz, Double_t pt, Double_t cent, Int_t htype, Double_t weight) {
  TObjArray *tar=0;
  const char *pf="";
...
Double_t AliGFWWeightsFinerBin::GetWeight(Double_t phi, Double_t eta, Double_t vz, Double_t pt, Double_t cent, Int_t htype) {
  TObjArray *tar=0;
  const char *pf="";
...
Double_t AliGFWWeightsFinerBin::GetNUA(Double_t phi, Double_t eta, Double_t vz) {
...
Double_t AliGFWWeightsFinerBin::GetNUE(Double_t pt, Double_t eta, Double_t vz) {
...
Double_t AliGFWWeightsFinerBin::FindMax(TH3D *inh, Int_t &ix, Int_t &iy, Int_t &iz) {
...
void AliGFWWeightsFinerBin::MCToEfficiency() {
...
void AliGFWWeightsFinerBin::RebinNUA(Int_t nX, Int_t nY, Int_t nZ) {
...
void AliGFWWeightsFinerBin::CreateNUA(Bool_t IntegrateOverCentAndPt) {
...
TH1D *AliGFWWeightsFinerBin::GetdNdPhi() {
...
void AliGFWWeightsFinerBin::CreateNUE(Bool_t IntegrateOverCentrality) {
...
void AliGFWWeightsFinerBin::ReadAndMerge(TString filelinks, TString listName, Bool_t addData, Bool_t addRec, Bool_t addGen) {
  ...
  for(Int_t i=0;i<nFiles;i++) {
    ...
    AliGFWWeightsFinerBin *tw = (AliGFWWeightsFinerBin*)tl->FindObject(this->GetName());
    if(!tw) {
...
void AliGFWWeightsFinerBin::AddArray(TObjArray *targ, TObjArray *sour) {
  if(!sour) {
...
void AliGFWWeightsFinerBin::OverwriteNUA() {
  if(!fAccInt) CreateNUA();
...
Long64_t AliGFWWeightsFinerBin::Merge(TCollection *collist) {
  Long64_t nmerged=0;
  if(!fW_data) {
    fW_data = new TObjArray();
    fW_data->SetName("Weights_Data");
  ...
  AliGFWWeightsFinerBin *l_w = 0;
  TIter all_w(collist);
  while (l_w = ((AliGFWWeightsFinerBin*) all_w())) {
...
TH1D *AliGFWWeightsFinerBin::GetIntegratedEfficiencyHist() {
...
Bool_t AliGFWWeightsFinerBin::CalculateIntegratedEff() {
...
Double_t AliGFWWeightsFinerBin::GetIntegratedEfficiency(Double_t pt) {
  if(!fIntEff) if(!CalculateIntegratedEff()) return 0;
  return fIntEff->GetBinContent(fIntEff->FindBin(pt));
}
TH1D *AliGFWWeightsFinerBin::GetEfficiency(Double_t etamin, Double_t etamax, Double_t vzmin, Double_t vzmax) {
  TH3D *num = (TH3D*)fW_mcrec->At(0)->Clone("Numerator");
  for(Int_t i=1;i<fW_mcrec->GetEntries();i++) num->Add((TH3D*)fW_mcrec->At(i));
  TH3D *den = (TH3D*)fW_mcgen->At(0)->Clone("Denominator");
  for(Int_t i=1;i<fW_mcgen->GetEntries();i++) den->Add((TH3D*)fW_mcgen->At(i));
