...
const Int_t AliGFWMCuts::fNTrackFlags=16;
const Int_t AliGFWMCuts::fNEventFlags=10;
AliESDtrackCuts *AliGFWMCuts::fTCFB32=0;
AliESDtrackCuts *AliGFWMCuts::fTCFB64=0;
AliESDtrackCuts *AliGFWMCuts::fTCFB256=0;
AliESDtrackCuts *AliGFWMCuts::fTCFB512=0;
AliESDtrackCuts *AliGFWMCuts::fTCFB16=0;
AliGFWMCuts::AliGFWMCuts():
  fSystFlag(0),
  fFilterBit(96),
  fDCAxyCut(7.),
  fDCAzCut(2),
...
Int_t AliGFWMCuts::AcceptTrack(AliAODTrack *&l_Tr, Double_t *l_DCA, const Int_t &BitShift, const Bool_t &lDisableDCAxyCheck) {
...
Int_t AliGFWMCuts::AcceptTrack(AliESDtrack *&l_Tr, Double_t *l_DCA, const Int_t &BitShift, UInt_t &PrimFlags) {
...
Int_t AliGFWMCuts::AcceptParticle(AliVParticle *l_Pa, Int_t BitShift, Double_t ptLow, Double_t ptHigh) {
  if(TMath::Abs(l_Pa->Eta())>fEta) return 0;
  if(ptLow>0) if(l_Pa->Pt()<ptLow) return 0;
  if(ptHigh>0) if(l_Pa->Pt()>ptHigh) return 0;
  return 1<<BitShift;
};
Int_t AliGFWMCuts::AcceptVertex(AliAODEvent *l_Ev, Int_t BitShift) {
  Double_t lvtxz = TMath::Abs(l_Ev->GetPrimaryVertex()->GetZ());
  if(lvtxz>fVtxZ) return 0;
  return 1<<BitShift;
};
Int_t AliGFWMCuts::AcceptVertex(AliESDEvent *l_Ev, Int_t BitShift) {
  Double_t lvtxz = TMath::Abs(l_Ev->GetPrimaryVertex()->GetZ());
  if(lvtxz>fVtxZ) return 0;
  return 1<<BitShift;
};

void AliGFWMCuts::ResetCuts() {
  fSystFlag=0;
  fFilterBit=96;
  fDCAxyCut=7;
  fDCAzCut=2;
...
void AliGFWMCuts::SetupTrackCuts(Int_t sysflag) {
  switch(sysflag) {
  case 1:
    fFilterBit=768;
    fRequiresExtraWeight=kTRUE;
...
void AliGFWMCuts::SetupEventCuts(Int_t sysflag) {
  switch(sysflag) {
  case 1:
    fVtxZ = 5;
    fRequiresExtraWeight=kTRUE;
...
void AliGFWMCuts::SetupCuts(Int_t sysflag) {
  ResetCuts();
  fSystFlag=sysflag;
...
TString *AliGFWMCuts::GetTrackFlagDescriptor(Int_t sysflag) {
...
TString* AliGFWMCuts::GetEventFlagDescriptor(Int_t sysflag) {
  TString *retstr = new TString("");
  switch(sysflag) {
  case 1:
...
TString *AliGFWMCuts::GetFlagDescription(Int_t sysflag) {
...
Double_t AliGFWMCuts::GetChi2TPCConstrained(const AliESDtrack *l_Tr) {
