...
Bool_t AliAnalysisTaskUniFlowWithSphericity::IsEventSelectedSphericity()
{
  ...
  if(fAnalType == kMC) {
    AliMCEvent* mcevent = dynamic_cast<AliMCEvent*>(fEvent);
    ...
    Int_t nTracks(fEventAOD->GetNumberOfTracks());
    for(Int_t iTracks = 0; iTracks < nTracks; iTracks++) { 
      AliAODTrack* track = dynamic_cast<AliAODTrack*>(fEventAOD->GetTrack(iTracks));
      if(!track) continue;
      if (!track || TMath::Abs(track->Eta()) > .8 || track->GetTPCNcls() < 70 || !track->GetDetPid() || track->GetDetPid()->GetTPCsignal() < 10.0)
	continue;  // general quality cut
