...
void AliAnalysisTaskESEFlow::UserCreateOutputObjects()
{
    ...
    for (Int_t qi(0);qi<2;++qi){
        ...
        fQnxV0CCor[qi] = new TH2F(Form("fQ%ixvCentV0CCor",qi+2),"",100,0,100,100,-1500,1500);
        fQnxV0CCor[qi]->Sumw2();
        fQnyV0CCor[qi] = new TH2F(Form("fQ%iyvCentV0CCor",qi+2),"",100,0,100,100,-1500,1500);
        fQnyV0CCor[qi]->Sumw2();
        fQnxV0ACor[qi] = new TH2F(Form("fQ%ixvCentV0ACor",qi+2),"",100,0,100,100,-1500,1500);
        fQnxV0ACor[qi]->Sumw2();
        fQnyV0ACor[qi] = new TH2F(Form("fQ%iyvCentV0ACor",qi+2),"",100,0,100,100,-1500,1500);
        fQnyV0ACor[qi]->Sumw2();
    ...
    if(fSPAnalysis){
        for (Int_t iVN(0);iVN<1;++iVN){
            ...
            //TH1* vnSPESETPC = nullptr;

            //TH1* vnSPESEV0C = nullptr;
            
            //TH1* vnSPESEV0A = nullptr;
...
void AliAnalysisTaskESEFlow::CorrelationTask(const Float_t centrality, const Int_t iTracks, const AliAODEvent* fAOD, const float dVz, Int_t fSpCent)
{
    ...
    if((!fMakeqSelectionRun && fV0RunByRunCalibration)){
        ...
        if(fSPAnalysis){
            SPVienna(centrality, iTracks, fAOD, dVz);
        }
...
void AliAnalysisTaskESEFlow::SPVienna(const Float_t centrality, const Int_t iTracks, const AliAODEvent* fAOD, const float dVz)
{
    Int_t CenterCode = GetCentralityCode(centrality);
    ...
    for(Int_t iPt(1); iPt< iNumPtBins+1; ++iPt)
    {
        ...
        SPFillPOI(dPtL, dPtLow, dPtHigh, dVz, iTracks);
...
void AliAnalysisTaskESEFlow::SPFillPOI(const Double_t dPt, const Double_t dPtLow, const Double_t dPtHigh, const float dVz, Int_t iTracks)
{
    if(iTracks < 1 ) { return; }
    for(Int_t i(0); i < iTracks; ++i) 
    {
