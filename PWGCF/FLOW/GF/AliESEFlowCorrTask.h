...
#ifndef ESEFLOWCORRTASK_H
#define ESEFLOWCORRTASK_H

class AliESEFlowCorrTask : public TObject
{
    public:
        AliESEFlowCorrTask(); // default ctor
        AliESEFlowCorrTask( // actual ctor
            Bool_t doRFPs,
            Bool_t doPOIs,
            std::vector<Int_t> harms,
            std::vector<Double_t> gaps = std::vector<Double_t>());
        virtual ~AliESEFlowCorrTask() { fiHarm.clear(); fdGaps.clear(); }

        Bool_t      HasGap() const { return (Bool_t) fiNumGaps; }; // check if Gap
        void        PrintTask() const; // print AliESEFlowCorrTask properties
