 ...
 * This old version is used by Mingrui Zhao in Nonlinear Flow Analysis for PbPb15o pass1.
 */

#include "AliGFWNFCuts.h"
const Int_t AliGFWNFCuts::fNTrackFlags=9;
const Int_t AliGFWNFCuts::fNEventFlags=10;
AliESDtrackCuts *AliGFWNFCuts::fTCFB32=0;
AliESDtrackCuts *AliGFWNFCuts::fTCFB64=0;
AliESDtrackCuts *AliGFWNFCuts::fTCFB256=0;
AliESDtrackCuts *AliGFWNFCuts::fTCFB512=0;
AliGFWNFCuts::AliGFWNFCuts():
  fSystFlag(0),
  fFilterBit(96),
  fDCAxyCut(7),
  fDCAzCut(2),
...
Int_t AliGFWNFCuts::AcceptTrack(AliAODTrack* l_Tr, Double_t* l_DCA, const Int_t &BitShift, const Bool_t &lDisableDCAxyCheck) {
...
Int_t AliGFWNFCuts::AcceptTrack(AliESDtrack* l_Tr, Double_t* l_DCA, const Int_t &BitShift, UInt_t &PrimFlags) {
...
Int_t AliGFWNFCuts::AcceptParticle(AliVParticle *l_Pa, Int_t BitShift, Double_t ptLow, Double_t ptHigh) {
  if(TMath::Abs(l_Pa->Eta())>fEta) return 0;
  if(ptLow>0) if(l_Pa->Pt()<ptLow) return 0;
  if(ptHigh>0) if(l_Pa->Pt()>ptHigh) return 0;
  return 1<<BitShift;
};
Int_t AliGFWNFCuts::AcceptVertex(AliAODEvent *l_Ev, Int_t BitShift) {
  Double_t lvtxz = TMath::Abs(l_Ev->GetPrimaryVertex()->GetZ());
  if(lvtxz>fVtxZ) return 0;
  return 1<<BitShift;
};
Int_t AliGFWNFCuts::AcceptVertex(AliESDEvent *l_Ev, Int_t BitShift) {
  Double_t lvtxz = TMath::Abs(l_Ev->GetPrimaryVertex()->GetZ());
  if(lvtxz>fVtxZ) return 0;
  return 1<<BitShift;
};

void AliGFWNFCuts::ResetCuts() {
  fSystFlag=0;
  fFilterBit=96;
  fDCAxyCut=7;
  fDCAzCut=2;
...
void AliGFWNFCuts::SetupTrackCuts(Int_t sysflag) {
  switch(sysflag) {
  case 1:
    fFilterBit=768;
    fRequiresExtraWeight=kTRUE;
...
void AliGFWNFCuts::SetupEventCuts(Int_t sysflag) {
  switch(sysflag) {
  case 1:
    fVtxZ = 5;
    fRequiresExtraWeight=kTRUE;
...
void AliGFWNFCuts::SetupCuts(Int_t sysflag) {
  ResetCuts();
  fSystFlag=sysflag;
...
TString *AliGFWNFCuts::GetTrackFlagDescriptor(Int_t sysflag) {
...
TString* AliGFWNFCuts::GetEventFlagDescriptor(Int_t sysflag) {
  TString *retstr = new TString("");
  switch(sysflag) {
  case 1:
...
TString *AliGFWNFCuts::GetFlagDescription(Int_t sysflag) {
...
Double_t AliGFWNFCuts::GetChi2TPCConstrained(const AliESDtrack *l_Tr) {
