...
void AliForwardGenericFramework::saveEvent(TList* outputList, double cent, double zvertex,UInt_t r, Int_t ptn){
  ...
  for (Int_t n = 2; n <= 4; n++) {
    ...
    //cumuRef = static_cast<THnD*>(file->Get(fSettings.fileName + "/Analysis/Reference/cumuRef_v" + n_str + "_pt" + ptn_str));//
    cumuRef = static_cast<THnD*>(refList->At(n-2));//FindObject(Form("cumuRef_v%d_pt%d", n,ptn)));
    //cumuDiff = static_cast<THnD*>(file->Get(fSettings.fileName + "/Analysis/Differential/cumuDiff_v" + n_str + "_pt" + ptn_str));//(
