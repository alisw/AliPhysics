...
// modified from plotV2edepBevalacRhicSqrtS.C - from Art
{
  ...
  //------------------------------------------------------------------
  // ALICE measured v2{4} w systematic error
  // aR* : tracklets (pt>50 MeV), by Anthony Timmins (anthony.timmins@cern.ch)
  // jR* : extrapolated v2{4}{pt} in (0<pt<20 GeV), TPC-only tracks, by Jacopo Margutti (jacopo.margutti@cern.ch)
  
  float aR1 = 0.0623803;
  float aR1err = 0.000856477;
  float jR1 = 0.0622876;
  float jR1err = 0.00125821;
  
  float aR2 = 0.0654147;
  float aR2err = 0.0015822;
  float jR2 = 0.0653886;
  float jR2err = 0.00182434;
  
  float v24ALICE = (aR1/pow(aR1err,2.)+jR1/pow(jR1err,2.))/(1./pow(aR1err,2.)+1./pow(jR1err,2.));
  float v24eALICE = pow(1./(1./pow(aR1err,2.)+1./pow(jR1err,2.)),0.5);
  v24eALICE = pow(pow(v24eALICE,2.)+pow(aR1-jR1,2.),0.5);
  
  float v24ALICEr2 = (aR2/pow(aR2err,2.)+jR2/pow(jR2err,2.))/(1./pow(aR2err,2.)+1./pow(jR2err,2.));
  float v24eALICEr2 = pow(1./(1./pow(aR2err,2.)+1./pow(jR2err,2.)),0.5);
  v24eALICEr2 = pow(pow(v24eALICEr2,2.)+pow(aR2-jR2,2.),0.5);
  
  float xxhhn[2]={2760, 5020}; // sqrt(s)
  float yyhhn[2]={v24ALICE,v24ALICEr2};
  float exhhn[2]={0.,0.};
  float eyhh2n[2]={v24eALICE,v24eALICEr2};
  Double_t errratio = pow( pow(v24ALICEr2/v24ALICE,2.) * ( pow(v24eALICE/v24ALICE,2.) + pow(v24eALICEr2/v24ALICEr2,2.)),0.5);
  printf("%e %e, %e %e , ratio %e %e \n",v24ALICE,v24eALICE,v24ALICEr2,v24eALICEr2,v24ALICEr2/v24ALICE,errratio);
  
  alice = new TGraphAsymmErrors(2,xxhhn,yyhhn,exhhn,exhhn,eyhh2n,eyhh2n);
  alice->SetTitle("");
  alice->SetMarkerColor(cAlice);
  alice->SetMarkerStyle(20);
  alice->SetMarkerSize(1.5);
  alice->SetLineWidth(2);
  alice->SetLineColor(cAlice);
  alice->Draw("pz");
  
  //------------------------------------------------------------------
  TLegend *legend = new TLegend(.68,.25,.86,.71);
