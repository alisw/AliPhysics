...
class AliBlastwaveFit2D : public AliBlastwaveFit
{
 public:
  ...
  const Float_t GetParStart(Int_t ipar) {if(ipar>=0 && ipar <5) return fgStartValues[ipar]; else return 0.0;};
  const Float_t GetParStep(Int_t ipar) {if(ipar>=0 && ipar <5) return fgStepValues[ipar]; else return 0.0;};
  const Float_t GetParMin(Int_t ipar) {if(ipar>=0 && ipar <5) return fgMinValues[ipar]; else return 0.0;};
  const Float_t GetParMax(Int_t ipar) {if(ipar>=0 && ipar <5) return fgMaxValues[ipar]; else return 0.0;};
