...
AliAnalysisTaskFlowModesMC* AddTaskFlowModesMC(TString name = "FlowHarmonics",
                                             AliAnalysisTaskFlowModesMC::ColSystem collisionSystem = AliAnalysisTaskFlowModesMC::kPbPb,
                                             AliAnalysisTaskFlowModesMC::AnalType analysisType = AliAnalysisTaskFlowModesMC::kMC,
                                             Int_t PVtxZMax = 10,
                                             ...
                                             Double_t PIDprob = 0.9)
{
    ...
    AliAnalysisTaskFlowModesMC* task1 = new AliAnalysisTaskFlowModesMC(name.Data()); // now we create an instance of your task
    if(!task1) return 0x0;
    
    // Analysis
    task1->SetRunMode(AliAnalysisTaskFlowModesMC::kFull);//kFull
    task1->SetColisionSystem(collisionSystem);//AliAnalysisTaskFlowModesMC::kPbPb);// collisionSystem  kPP, kPbPb
    task1->SetNumEventsAnalyse(50);//In case of fRunMode == kTest it only analyses up to 50 events.
    task1->SetSampling(kTRUE);
    task1->SetAnalysisType(AliAnalysisTaskFlowModesMC::kMC);//analysisType
    ...
    //task1->SetUseNUEWeigthsFile("NUECorrectionMap_fb768_nSigmaComb2.root");
    ...
    // Events selection
    if(analysisType == AliAnalysisTaskFlowModesMC::kAOD) task1->SetTrigger(0); // case 0: kINT7, case 1: kHighMultV0, case 2: kHighMultSPD.
    if(analysisType == AliAnalysisTaskFlowModesMC::kAOD) task1->SetMultEstimator(MultEstimator);//CL1, CL0
    if(analysisType == AliAnalysisTaskFlowModesMC::kAOD) task1->SetCentralityRange(kTRUE); // kTRUE: run from 0-100% kFALSE: run from 50-100%
    if(analysisType == AliAnalysisTaskFlowModesMC::kMC){cout<<"Hi Hi"<<endl; task1->SetImpactParameterRange(0,14.5);}
