...
void AddTaskCMEAnalysis2018ReducedCentBin(Bool_t isPbPb = kTRUE,
			...
			Double_t gCentStart = 30,
			Int_t nCentBin = 3,
			Double_t nCentStepSize = 10,
  ...
  //=============================================//
  //Define centralities
  const int nCentralities = nCentBin;

  Double_t gCentrality[nCentralities];

  for(Int_t i = 0; i < nCentralities; i++)
     gCentrality[i] = gCentStart + (i)*nCentStepSize;
  ...
  for(Int_t iCentralityBin = 0; iCentralityBin < nCentralities - 1; iCentralityBin++) {
    ...
    cutsPOI_NN[iCentralityBin] = createFlowPOICutObject(gCentrality[iCentralityBin],gCentrality[iCentralityBin+1],qVector,gEtaGap,isVZERO,isPbPb,gAODfilterBit,whichData,gDCAvtxXY,gDCAvtxZ,-1,gMixNClustersTPC,gChi2PerClusterTPCMin,gChi2PerClusterTPCMax,kFALSE);
    cutsPOI_PN[iCentralityBin] = createFlowPOICutObject(gCentrality[iCentralityBin],gCentrality[iCentralityBin+1],qVector,gEtaGap,isVZERO,isPbPb,gAODfilterBit,whichData,gDCAvtxXY,gDCAvtxZ,0,gMixNClustersTPC,gChi2PerClusterTPCMin,gChi2PerClusterTPCMax,kFALSE);
    cutsPOI_PNwQA[iCentralityBin] = createFlowPOICutObject(gCentrality[iCentralityBin],gCentrality[iCentralityBin+1],qVector,gEtaGap,isVZERO,isPbPb,gAODfilterBit,whichData,gDCAvtxXY,gDCAvtxZ,0,gMixNClustersTPC,gChi2PerClusterTPCMin,gChi2PerClusterTPCMax,doQA);
 ...
 //----PN----
  ...
  for(Int_t iCentralityBin = 0; iCentralityBin < nCentralities - 1; iCentralityBin++) {
   ...
   //------------------- PN --------------
	...
	taskMHLS_PP[iCentralityBin]->SetCalculateVsM(kTRUE);
	if(isPbPb){
	  if (whichData != 2018) {
	...
	taskMHLS_PP2[iCentralityBin]->SetCalculateVsM(kTRUE);
	if(isPbPb){
	  if (whichData != 2018) {
	...
	taskMHLS_NN[iCentralityBin]->SetCalculateVsM(kTRUE);
	if(isPbPb){
	  if (whichData != 2018) {
	...
	taskMHLS_NN2[iCentralityBin]->SetCalculateVsM(kTRUE);
	if(isPbPb){
	  if (whichData != 2018) {
	...
	taskMHUS[iCentralityBin]->SetOppositeChargesPOI(kTRUE); //
 	...
 	taskMHUS[iCentralityBin]->SetCalculateVsM(kTRUE);
	if(isPbPb){
	  if (whichData != 2018) {
	...
	taskMHUS2[iCentralityBin]->SetOppositeChargesPOI(kTRUE); //
 	...
 	taskMHUS2[iCentralityBin]->SetCalculateVsM(kTRUE);
	if(isPbPb){
	  if (whichData != 2018) {
...
AliFlowEventCuts *createFlowEventCutObject(Int_t gCentralityMin = -1,
    ...
    if(isPbPb) {
      ...
      cutsEvent->SetCentralityPercentileRange(gCentralityMin,gCentralityMax,kFALSE); // kFALSE means it is not used
      ...
      if(whichData==2011){ 
	    ...
	    if(whichData==2018){ 
	      cutsEvent->SetCentralityPercentileRange(gCentralityMin,gCentralityMax,kTRUE);
          //cutsEvent->SetCheckPileup(bPileup); // dont' worry about it now. 
	    }
      }
...
//__@Shi cuts on pt(0.2-5) eta, MinTPCdEdX, FilterBit, Charge___________________//
AliFlowTrackCuts *createFlowRPCutObject(Int_t gCentralityMin = -1,
                                        Int_t gCentralityMax = -1,
                                        Bool_t isVZERO = kFALSE,
                                        Int_t gAODfilterBit = 768,
...
AliFlowTrackCuts *createFlowPOICutObject(Int_t gCentralityMin = -1,
    ...
    cutsPOI->SetMinNClustersTPC(gMixNClustersTPC);    
    cutsPOI->SetMinChi2PerClusterTPC(gChi2PerClusterTPCMin);
    cutsPOI->SetMaxChi2PerClusterTPC(gChi2PerClusterTPCMax);
