...
//  cutsEvent->SetNContributorsRange(2);
  ...
  if (chargeRP!=0) cutsRP->SetCharge(chargeRP);
  cutsRP->SetAODfilterBit(aodBit);  
  cutsRP->SetQA(doQA);
  ...
  if (chargePOI!=0) cutsPOI->SetCharge(chargePOI);
  ...
  cutsPOI->SetAODfilterBit(aodBit);
  cutsPOI->SetQA(doQA);
  cutsPOI->SetPriors((centrMin+centrMax)*0.5); // set priors and PID as a function of the centrality

  TString outputSlotName = uniqueStr;
  outputSlotName+=Form("_%.0f-",centrMin);
  ...
  if(SP) {
    ...
    //outputSP+= rptypestr;
    AliAnalysisDataContainer *coutputSP = mgr->CreateContainer(Form("cobjSP_%s",outputSlotName.Data()), 
  ...
  if(QC) {
    ...
    AliAnalysisDataContainer *coutputQC = mgr->CreateContainer(Form("cobjQC_%s",outputSlotName.Data()), 
  ...
  if(MH) {
    ...
    AliAnalysisDataContainer *coutputMH = mgr->CreateContainer(Form("cobjMH_%s",outputSlotName.Data()), 
