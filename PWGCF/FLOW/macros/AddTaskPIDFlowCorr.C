...
AliAnalysisTaskPIDFlowCorr* AddTaskPIDFlowCorr(Bool_t IsGrid, TString name = "PIDFlowCorr")
{
  if (IsGrid){
  gGrid->Connect("alien://");
  }
  ...
  AliAnalysisTaskPIDFlowCorr* task = new AliAnalysisTaskPIDFlowCorr(name.Data()); // now we create an instance of your task
  if(!task) return 0x0;
