AliAnalysisTaskCorrForFlowEta* AddCorrForFlowTaskEta(TString name = "name", TString efficiencyFile = "",TString calibrationFile = "",const char* suffix = "")
{
    ...
    AliAnalysisTaskCorrForFlowEta* task = new AliAnalysisTaskCorrForFlowEta(taskName.Data(), useEfficiency, useCalibration);
    if(!task) return 0x0;
    mgr->AddTask(task);
