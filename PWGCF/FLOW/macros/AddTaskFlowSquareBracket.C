...
AliAnalysisTaskFlowSquareBracket* AddTaskFlowSquareBracket(Bool_t IsGrid, TString name = "UniFlow")
{
  if (IsGrid){
  gGrid->Connect("alien://");
  }
  ...
  AliAnalysisTaskFlowSquareBracket* task = new AliAnalysisTaskFlowSquareBracket(name.Data()); // now we create an instance of your task
  if(!task) return 0x0;
