		...
		)
{
  ...
  const char *taskname = Form("NewTpcTofPidTaskPhiWt%d",100);
  ...
  if(analysislevel == "ESD")
	...
	TChain* chain = CreateESDChain("localrun.txt", 1);
      }
  ...
  gROOT->LoadMacro("AddTaskResonanceVsMultiplicity.C");
  AddTaskResonanceVsMultiplicity(0,90,10,0.1,1,2.5,36,70);
...
AliAnalysisGrid* CreateAlienHandler(const char *taskname, const char *gridmode, Int_t RunNo)
{
    ...
    plugin->SetROOTVersion("v5-34-30-alice10-79");
    plugin->SetAliROOTVersion("v5-09-57c-1");
