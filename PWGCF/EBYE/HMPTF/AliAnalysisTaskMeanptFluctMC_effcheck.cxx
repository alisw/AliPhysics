...
#include "AliAnalysisTaskMeanptFluctMC_effcheck.h"
...
void AliAnalysisTaskMeanptFluctMC_effcheck::UserCreateOutputObjects()
{
  ...
  fTreeEvent->Branch("fQ11", &fQ11, "fQ11/F");
  fTreeEvent->Branch("fQ01", &fQ01, "fQ01/F");
  fTreeEvent->Branch("fQ22", &fQ22, "fQ22/F");
  fTreeEvent->Branch("fQ12", &fQ12, "fQ12/F");
  fTreeEvent->Branch("fQ02", &fQ02, "fQ02/F");
  fTreeEvent->Branch("fQ33", &fQ33, "fQ33/F");
  fTreeEvent->Branch("fQ23", &fQ23, "fQ23/F");
  fTreeEvent->Branch("fQ13", &fQ13, "fQ13/F");
  fTreeEvent->Branch("fQ03", &fQ03, "fQ03/F");
...
//________________________________________________________________________
void AliAnalysisTaskMeanptFluctMC_effcheck::UserExec(Option_t *)
{
  ...
  if (lMCevent){
      ...
      for (Int_t iTrack = 0; iTrack < lMCevent->GetNumberOfTracks(); iTrack++)
	{
	  ...
	  hist2D_pt_gen_centrality->Fill(Track_pt,fCentImpBin);

	  //cout<<"+++++++++++++++++++++++++++++++++++ Min:"<<fPt_min_val<<"\tMax: "<<fPt_max_val<<"++++++++++++++++++++++++++++++++"<<endl;
	  x=ran.Uniform(0,1);
	  eff=fEff->Eval(Track_pt);
	 
	  if(x<(eff) && eff!=0.0)
	    {
	      w=1.0/eff;
	      hist2D_pt_rec_centrality->Fill(Track_pt,fCentImpBin);
	      ...
	      if(Track_pt>fPt_min_val && Track_pt<=fPt_max_val && TMath::Abs(Track_PID)==2212) //! proton
		{
		  ...
		  //eff correction terms
		  Q11+=(TMath::Power(Track_pt, 1.0)*TMath::Power(w, 1.0));
		  Q01+=(TMath::Power(w, 1.0));
		  Q22+=(TMath::Power(Track_pt, 2.0)*TMath::Power(w, 2.0));
		  Q12+=(TMath::Power(Track_pt, 1.0)*TMath::Power(w, 2.0));
		  Q02+=(TMath::Power(w, 2.0));
		  Q33+=(TMath::Power(Track_pt, 3.0)*TMath::Power(w, 3.0));
		  Q23+=(TMath::Power(Track_pt, 2.0)*TMath::Power(w, 3.0));
		  Q13+=(TMath::Power(Track_pt, 1.0)*TMath::Power(w, 3.0));
		  Q03+=(TMath::Power(w, 3.0));
      ...
      //eff. correction terms check only for proton
      fQ11=Q11;
      fQ01=Q01;
      fQ22=Q22;
      fQ12=Q12;
...
//________________________________________________________________________
void AliAnalysisTaskMeanptFluctMC_effcheck::Terminate(Option_t *)
{
