...
#include "AliAnalysisTaskResonanceVsMultiplicity2.h"
...
ClassImp(AliAnalysisTaskResonanceVsMultiplicity2)
...
AliAnalysisTaskResonanceVsMultiplicity2::~AliAnalysisTaskResonanceVsMultiplicity2()
{
    //------------------------------------------------
    // DESTRUCTOR
    //------------------------------------------------
...
void AliAnalysisTaskResonanceVsMultiplicity2::UserCreateOutputObjects()
{
...
//________________________________________________________________________
void AliAnalysisTaskResonanceVsMultiplicity2::UserExec(Option_t *)
{
  ...
  cout<<"****************************************WE ARE AFTER MULT********************************"<<" "<<lV0M<<endl;                         
  ...
  for (Int_t ipart = 0; ipart < lMCstack->GetNtrack(); ipart++)
	...
	continue;
      ...
      if(TMath::Abs(trk_VertexZ_gen)>10.0)continue;
      if(TMath::Abs(trk_charge_gen)!=1)continue;
      if(TMath::Abs(trk_eta_gen)>0.8)continue;

      hist2D_pt_gen_centrality->Fill(trk_pt_gen,lV0M);

      if(TMath::Abs(trk_eta_gen)<0.5)
	{
	  Nch_eta0pt5+=1.0;
	}
...
//________________________________________________________________________
void AliAnalysisTaskResonanceVsMultiplicity2::Terminate(Option_t *)
{
  
}
//----------------------------------------------------------------------------
Double_t AliAnalysisTaskResonanceVsMultiplicity2::MyRapidity(Double_t rE, Double_t rPz) const
{
...
//----------------------------------------------------------------------------
void AliAnalysisTaskResonanceVsMultiplicity2::SetPersonalESDtrackCuts(AliESDtrackCuts* trackcuts){
  if(fESDtrackCuts){ 
    delete fESDtrackCuts;
    fESDtrackCuts = 0x0;
  }
...
//----------------------------------------------------------------------------
Double_t AliAnalysisTaskResonanceVsMultiplicity2::GetTOFBeta(AliVTrack *vtrack)
{
...
Bool_t AliAnalysisTaskResonanceVsMultiplicity2::MatchTOF(AliVTrack *vtrack)
{
  if (!vtrack) {
