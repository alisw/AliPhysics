 ...
 **************************************************************************/
// Analysis task for Mean p_T fluctuations in MB pp collisions
...
#include "THnSparse.h"
#include "AliAnalysisMeanPtFluc.h"

class AliAnalysisMeanPtFluc;
...
ClassImp(AliAnalysisMeanPtFluc)

	//================================================
	//--------------Constructor-----------------------
	//================================================
	AliAnalysisMeanPtFluc::AliAnalysisMeanPtFluc()
	: AliAnalysisTaskSE(),
	  fAOD(0x0),
	  fOutputList(0),
	  fEventCount(0),
...
//-------------------------------------------------
AliAnalysisMeanPtFluc::AliAnalysisMeanPtFluc(const char *name)
...
void AliAnalysisMeanPtFluc::UserCreateOutputObjects()
{
	...
	const int dim = 7;
	int bins[dim] = {1200, 1200, 200, 200, 200, 200, 200};
	double mins[dim] = {0., 0., 0., 0., 0., 0., 0.};
	double maxs[dim] = {1200., 1200., 200., 200., 200., 200., 200.};
	...
	fTHnfnch->GetAxis(3)->SetTitle("N_{ch}^{nopt}");
	fTHnfnch->GetAxis(4)->SetTitle("N_{ch}^{pt6p0}");
	fTHnfnch->GetAxis(5)->SetTitle("N_{ch}^{pt2p0}");
	fTHnfnch->GetAxis(6)->SetTitle("N_{ch}^{pt}");
	fOutputList->Add(fTHnfnch);

	fpT = new TH1F("fpT", "Multiplicity", 500, 0, 10.);
	fOutputList->Add(fpT);
	...
	fHistPhi->StatOverflows(kTRUE);
	fHistPhi->Sumw2();
	fOutputList->Add(fHistPhi);

	htrk = new TH1F("htrk", "ncal", 200, 2, 202);
	htrk->StatOverflows(kTRUE);
	htrk->Sumw2();
	fOutputList->Add(htrk);

	hScale = new TH2D("hScale", "ncal", 30, 1, 31, 200, 2, 202);
	hScale->StatOverflows(kTRUE);
	hScale->Sumw2();
	fOutputList->Add(hScale);

	hTracks = new TH2D("hTracks", "ncal", 30, 1, 31, 200, 2, 202);
	hTracks->StatOverflows(kTRUE);
	hTracks->Sumw2();
	fOutputList->Add(hTracks);

	hTrackssq = new TH2D("hTrackssq", "ncal", 30, 1, 31, 200, 2, 202);
	hTrackssq->StatOverflows(kTRUE);
	hTrackssq->Sumw2();
	fOutputList->Add(hTrackssq);

	hTrackavgpt = new TH2D("hTrack_evt_pt", " ", 30, 1, 31, 200, 2, 202);
	hTrackavgpt->StatOverflows(kTRUE);
	hTrackavgpt->Sumw2();
	fOutputList->Add(hTrackavgpt);

	hTrackavgptsq = new TH2D("hTrack_evt_pt_sq", " ", 30, 1, 31, 200, 2, 202);
	hTrackavgptsq->StatOverflows(kTRUE);
	hTrackavgptsq->Sumw2();
	fOutputList->Add(hTrackavgptsq);

	hTrackpair = new TH2D("hTrack_npairs", "", 30, 1, 31, 200, 2, 202);
	hTrackpair->StatOverflows(kTRUE);
	hTrackpair->Sumw2();
	fOutputList->Add(hTrackpair);

	// hTracksumpt = new TH2D("hTrack_sumpt","term2",30, 1, 31, 200, 2, 202);
	// hTracksumpt->StatOverflows(kTRUE);	hTracksumpt->Sumw2();
	// fOutputList->Add(hTracksumpt);

	hTracksumptiptj = new TH2D("hTrack_sumptiptj", "term1", 30, 1, 31, 200, 2, 202);
	hTracksumptiptj->StatOverflows(kTRUE);
	hTracksumptiptj->Sumw2();
...
void AliAnalysisMeanPtFluc::UserExec(Option_t *)
{
	...
	if (fPrimaryVtx->GetNContributors() < 1)
		return;
	...
	if (zv < vzmin || zv > vzmax)
		return; // vrtxz from Addtask
	...
	int trk = 0, trk2 = 0, trk0 = 0, trk_pt = 0;
	...
	if (trk_pt > 2)
	{
		double meanpt = sum_trackpt / trk_pt;
		double meanptsq = sum_trackpt;
		double npair = trk * (trk - 1);

		//----filling histograms-----
		htrk->Fill(trk);
		hScale->Fill(sample, trk2);
		hTracks->Fill(sample, trk2, trk);
		hTrackssq->Fill(sample, trk2, trk_pt);
		hTrackavgpt->Fill(sample, trk2, meanpt);
		hTrackavgptsq->Fill(sample, trk2, meanptsq);
		hTrackpair->Fill(sample, trk2, npair);
		//++++++++++++++++++++++++++++++
		double term = sum_trackpt * sum_trackpt - sum_ptiptj;
		term = term / npair;
		hTracksumptiptj->Fill(sample, trk2, term);
	}
...
bool AliAnalysisMeanPtFluc::AcceptTrack(AliAODTrack *aodtrack) const
{
	if (!aodtrack)
		return kFALSE;
	...
	if (!aodtrack->TestFilterBit(ftrackBit))
		return kFALSE; // Track Bit
	if (aodtrack->Eta() < -0.8 || aodtrack->Eta() > 0.8)
		return kFALSE;
	if (aodtrack->GetTPCCrossedRows() < TPCrows)
		return kFALSE;
...
void AliAnalysisMeanPtFluc::Terminate(Option_t *)
{
	// terminate
	// called at the END of the analysis (when all events are processed)
	Info("AliAnalysisMeanPtFluc", " Task Successfully finished");
