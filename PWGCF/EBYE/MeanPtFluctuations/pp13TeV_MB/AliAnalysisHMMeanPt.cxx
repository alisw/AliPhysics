...
void AliAnalysisHMMeanPt::UserCreateOutputObjects()
{
	...
	//===========================
	const int ndim = 6;
	int nbins[ndim] = {200, 6, 64, 194, 180, 20};
	double nmins[ndim] = {0, -3., -0.8, 0.15, 0.0, -10};
	double nmaxs[ndim] = {200, 3., 0.8, 5., TMath::TwoPi(), 10};

	fTHnetaptphi = new THnSparseD("fTHnetaptphi","fTHnfetaptphi",ndim, nbins, nmins, nmaxs);
	fTHnetaptphi ->GetAxis(0)->SetTitle("N_{acc}");
	fTHnetaptphi ->GetAxis(1)->SetTitle("charge");	
	fTHnetaptphi ->GetAxis(2)->SetTitle("#eta");	
	fTHnetaptphi ->GetAxis(3)->SetTitle("p_{T}");
	fTHnetaptphi ->GetAxis(4)->SetTitle("#phi");
	fTHnetaptphi ->GetAxis(5)->SetTitle("v_{z}");	
	fOutputList->Add(fTHnetaptphi);
    ...
    fHistEta = new TH1F("hTrackEta", "fHistEta", 100, -1., 1.);
    ...
    fHistPt = new TH1F("hTrackPt", "fHistPt", 250, 0, 10);   
	...
	fHistMag = new TH1D("MAgnetic feild", " ",20,-10.,10.);
	...
	//--------
	fHistChi2perNDF = new TH1F("fHistChi2perNDF","Nclusters",50,0,10);
	fHistChi2perNDF->StatOverflows(kTRUE);
	fOutputList->Add(fHistChi2perNDF);

	fHistClustersTPC = new TH1F("fHistClustersTPC","Nclusters",500,0,500);
 ...
 /* 
    ...
    fTreept->Branch("fV0_total",&fV0_total,"fV0_total/F");
    fTreept->Branch("field",&field,"field/F");   
    fTreept->Branch("fRunNumber",&fRunNumber,"fRunNumber/I");           
    fTreept->Branch("Vz",&Vz,"Vz/I");    
...
//cout<<"starting analysis"<<endl;
    ...
    // vertex cut    
    if (TMath::Abs(zv) > fevtvrtxz) return;   	// vrtxz from Addtask     
...
//----------end of detector cuts---------        
    ...
    for(Int_t i(0); i < iTracks; i++) {                 
		...
		*/
		...
		double 	fillsparse[6] = {static_cast<double>(trk), static_cast<double>(track->Charge()), track->Eta(),  track->Pt(), track->Phi(), zv};
		fTHnetaptphi->Fill(fillsparse);
