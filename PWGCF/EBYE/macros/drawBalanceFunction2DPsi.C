...
//____________________________________________________________//
void drawProjectionsUser(TH2D *gHistBalanceFunction2D = 0x0,
		     Bool_t kProjectInEta = kFALSE,
		     Int_t binMin = 1,
		     Int_t binMax = 80,
		     Int_t gCentrality = 1,
		     ...
		     Bool_t bRootMoments = kFALSE,
		     Bool_t kUseZYAM = kFALSE,
		     Double_t reducedRangeForMoments = 100.,
		     Bool_t bFWHM = kFALSE) {
  //Macro that draws the balance functions PROJECTIONS 
  //for each centrality bin for the different pT of trigger and 
  //associated particles
  ...
  if(bRootMoments){
    ...
    // need to restrict to near side in case of dphi
    if(!kProjectInEta && reducedRangeForMoments > TMath::Pi()/2. ) gHistBalanceFunctionSubtracted->GetXaxis()->SetRangeUser(-TMath::Pi()/2,TMath::Pi()/2);

   if(reducedRangeForMoments < TMath::Pi()/2.){
...
//____________________________________________________________//
void mergeBFPsi2DUser(TString momDirectory = "./",
		  TString directory1 = "",
		  TString directory2 = "",
		  TString directory3 = "",
		  TString directory4 = "",
		  ...
		  Double_t ptAssociatedMax = -1.,
		  Double_t reducedRangeForMomentsEta = 1.6,
		  Double_t reducedRangeForMomentsPhi = TMath::Pi()/2.,
		  Bool_t bFWHM = kFALSE
) {
  //Macro that draws the BF distributions for each centrality bin
  //for reaction plane dependent analysis
