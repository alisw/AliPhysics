AliAnalysisMeanPtFluc *AddMeanPtFluc(
    int trackBit = 128,
    float vrtxz1 = -10.,
    float vrtxz2 = 10.,
    float p_low = 0.15,
    ...
    TString suffix = "")
{
    ...
    AliAnalysisMeanPtFluc *task = new AliAnalysisMeanPtFluc(TaskMeanPt);
    if (!task)
        return 0x0;
    ...
    // same for the output
    mgr->ConnectOutput(task, 1, mgr->CreateContainer(Form("fList_%s_FB%d_%s", period.Data(), trackBit, suffix.Data()), TList::Class(), AliAnalysisManager::kOutputContainer, fileName.Data()));
