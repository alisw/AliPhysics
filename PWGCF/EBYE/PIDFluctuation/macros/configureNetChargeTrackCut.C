AliESDtrackCuts *configureNetChargeTrackCut(Int_t cutMode, Double_t eta, Double_t ptl, Double_t pth) {
  ...
  Double_t etal = -1*eta;
  trackCuts->SetEtaRange(etal,eta);
  trackCuts->SetPtRange(ptl, pth);
  
  // Extra
  trackCuts->SetMinNClustersTPC(70);  
  trackCuts->SetClusterRequirementITS(AliESDtrackCuts::kSPD,AliESDtrackCuts::kAny); 
  trackCuts->SetRequireITSRefit(kTRUE);                                               // ITS 2010 
  trackCuts->SetDCAToVertex2D(kFALSE);                                                // VertexConstrained  2010
  trackCuts->SetRequireSigmaToVertex(kFALSE);                                         // VertexConstrained  2010
  trackCuts->SetMaxDCAToVertexZ(4);             
