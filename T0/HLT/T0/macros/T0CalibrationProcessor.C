 ...
 */
void T0CalibrationProcessor(const char *filename,
		  const char *cdbURI,
		  int minEvent=-1,
		  int maxEvent=-1)
{
...
//  AliHLTConfiguration rawpub("rawpub", "AliRawReaderPublisher", "", "-minid 768 -maxid 983 -datatype 'DDL_RAW '  T0");
//  AliHLTConfiguration rawpub("rawpub", "AliRawReaderPublisher", "", "-minid 3328 -maxid 3328 -datatype 'DDL_RAW '  T0");
 AliHLTConfiguration rawpub("RAW-Publisher", 
  ...
  AliHLTConfiguration tzerocalib("tzerocalib", "T0Calibration", "rawpub", "-pushback-period=2");

  // writer for data objects produced by the calibration component during
  // ProcessCalibration, 
  AliHLTConfiguration eventwriter("eventwriter", "ROOTFileWriter", "tzerocalib", "-datafile calibdata.root -concatenate-events -overwrite -write-all-events");

  // filter the output of the tzerocalib component and forward only the blocks
  // of type 'FXS_CAL ' designated for storage by the FXSSubscriber
  AliHLTConfiguration fxsfilter("fxsfilter", "BlockFilter", "tzerocalib", "-typeid 'FXS_CAL ' ");
 ...
 // rec.SetOption("HLT", "loglevel=0x7c ignore-hltout libAliHLTUtil.so libAliHLTT0.so chains=fxswriter,eventwriter");
 // set option for the HLT module in AliReconstruction
  ...
  rec.SetOption("HLT",
		...
		"loglevel=0x7c "
		"chains=T0-Calib"//"GLOBAL-ESD-Converter,RootWriter"
		);
...
void T0CalibrationProcessor(const char *filename,
		  ...
		  int maxEvent=-1)
{
  T0CalibrationProcessor(filename, "local://$ALICE_ROOT/OCDB", minEvent, maxEvent);
}

void T0CalibrationProcessor()
{
  cout << "sampleCalibrationProcessor: Run AliRoot reconstruction locally" << endl;
  cout << " Usage: aliroot -b -q -l \\" << endl;
  cout << "     T0CalibrationProcessor.C'(\"file\", \"cdb\", minEvent, maxEvent)'" << endl;
  cout << "" << endl;
  cout << " Examples:" << endl;
  cout << "     T0CalibrationProcessor.C'(\"alien:///alice/data/2009/.../....root\")' " << endl;
  cout << "     T0CalibrationProcessor.C'(\"raw.root\", \"local://$PWD\", minEvent, MaxEvent)'" << endl;
  cout << "" << endl;
  cout << " Defaults" << endl;
