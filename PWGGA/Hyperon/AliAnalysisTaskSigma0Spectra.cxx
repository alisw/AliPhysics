...
void AliAnalysisTaskSigma0Spectra::UserCreateOutputObjects()
{
    ...
    fOutputContainer->Add(new TH2F("hV0LamMvsPt","V0 Lam ALL Mass vs pt",200, mLammin,mLammax, 100,0.,ptmax)) ;
    fOutputContainer->Add(new TH2F("hV0PLamMvsPt","V0 Lam Part Mass vs pt",200, mLammin,mLammax, 100,0.,ptmax)) ;
    fOutputContainer->Add(new TH2F("hV0ALamMvsPt","V0 Lam APart Mass vs pt",200, mLammin,mLammax, 100,0.,ptmax)) ;
...
void AliAnalysisTaskSigma0Spectra::FillRealMixed(){
    ...
    if( nLam >= 1 ){
        for(Int_t icon=0; icon<nLam; icon++){
            ...
            if( ph1->M() > MassLam - 0.100 &&  ph1->M() < MassLam + 0.100 ) { // see rapidity distributions from data
                char key[55] ;
        ...
        for(Int_t icon=0; icon<nConv; icon++){
            ...
            for(Int_t icon2=0; icon2<nLam; icon2++){
                ...
                // printf("Sig0-CC  M %f PT %f  E %f nconv %d \n", sig0.M(), sig0.Pt(), sig0.E(), nConv );
		    ...
		    //                    FillHistogram("rec_Sig0", pt  ) ;
                    // if( abs(rap) < 1.4 )  FillHistogram("rec_Sig0eta1", pt) ;
                    ...
                    if( ( abs(  arpodr[1] ) > 0.5 && arpodr[0]< 0.14 ) )  FillHistogram("hCCReMvsPt1", mrec, ptrec ) ;
            ...
            for(Int_t ev=0; ev< prevConv->GetSize();ev++){
                TClonesArray * mixConv = static_cast<TClonesArray*>(prevConv->At(ev)) ;
                   ...
                   for(Int_t icon=0; icon<mixConv->GetEntriesFast() ; icon++){
                    ...
                    GetArPod( posr, negr, mothr, arpodr );
                    if( !( abs(  arpodr[1] ) > 0.6 && arpodr[0]< 0.12 ) ) continue ;
                    
                    Double_t rapmix = Rapidity(  ptmix,  sig0mix.Pz(),  mmix );
                    char key[55] ;
                    for (Int_t irap = 0; irap < 6; irap++) {
                        if( abs(rapmix) < fEtaCuts[irap] ) {
                            if( ModLam > 0 ) {
...
void AliAnalysisTaskSigma0Spectra::GetArPod( Double_t pos[3], Double_t neg[3], Double_t moth[3],  Double_t arpod[2] ){
    ...
    if ( abs( costhetaV0pos ) > 1 ||  abs( costhetaV0neg ) > 1 ) {
        arpod[0] = 0;
        arpod[1] = 0;
        return;
    }
