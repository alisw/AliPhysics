...
void PPRstyle();
Double_t PeakPosition(Double_t pt, Int_t centrality);
Double_t PeakWidth(Double_t pt, Int_t centrality);
...
void MakeMmixPi0(const Int_t centrality = 0,
		 const TString period     = "LHC18r",
		 const char* file       = "AnalysisResults_PHOSPbPbQA.root",
		 const char* cModule    = "",
...
/* $Id$ */
  ...
  const char* pid="All";
  const char* txtCent[] = {"0-20%","20-40%", "40-60%", "60-80%", "80-100%"};
  ...
  TH2F *h , *hAdd;
  TH2F *hm, *hmAdd;
  if (centrality >= 0 || centrality <= 5) {// centrality 0-20%, 20-100%
    inputKey = Form("hPi0%s%s_cen%d"  ,pid,cModule,centrality);
    h = (TH2F*)histESD->FindObject(inputKey) ;
    inputKey = Form("hMiPi0%s%s_cen%d",pid,cModule,centrality);
    hm= (TH2F*)histESD->FindObject(inputKey) ;
    if (h==0) {
  ...
  TCanvas *c4, *c2;
  TAxis * pta= (TAxis*)h->GetYaxis() ;
  TAxis * ma=  (TAxis*)h->GetXaxis() ;
...
//    delete hpcopy ;
  ...
  sprintf(name,"Pi0_ratio_cent%d%s.pdf" ,centrality,cModule) ;
  c1->Print(name) ;
  sprintf(name,"Pi0_signal_cent%d%s.pdf",centrality,cModule) ;
  c3->Print(name) ;
  ...
  TFile fout(Form("Pi0_FitResult_cen%d.root", centrality),"recreate");
