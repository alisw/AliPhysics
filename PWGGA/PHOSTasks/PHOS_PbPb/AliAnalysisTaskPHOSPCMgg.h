#ifndef AliAnalysisTaskPHOSPCMgg_cxx
#define AliAnalysisTaskPHOSPCMgg_cxx
...
class AliAnalysisTaskPHOSPCMgg : public AliAnalysisTaskSE {
public:
  
  enum CutList {kDefault, kDisp, kCPV, kBoth, kDistance1, kDistance2, kDistance3} ;  
  
  
  AliAnalysisTaskPHOSPCMgg(const char *name = "AliAnalysisTaskPHOSPCMgg");
  virtual ~AliAnalysisTaskPHOSPCMgg() {}
...
private:
  AliAnalysisTaskPHOSPCMgg(const AliAnalysisTaskPHOSPCMgg&); // not implemented
  AliAnalysisTaskPHOSPCMgg& operator=(const AliAnalysisTaskPHOSPCMgg&); // not implemented
  ...
  Bool_t PairCut(const AliCaloPhoton * ph1, const AliAODConversionPhoton * ph2, Int_t cut) const ; 
...
//  TClonesArray * fStack ;  
  ...
  TList *       fPCMEvents[1][10][11] ; //Containers for events with PHOS photons
  ...
  ClassDef(AliAnalysisTaskPHOSPCMgg, 1); // PHOS analysis task
};
