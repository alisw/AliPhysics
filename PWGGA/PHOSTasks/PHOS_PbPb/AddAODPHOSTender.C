AliPHOSTenderTask *AddAODPHOSTender(
    ...
    const char *tenderName = "PHOStender", const char *options = "",
    Int_t pass = 1, Bool_t isMC = kFALSE, const char *nonLinType = "",
    Double_t zsSimulation = 0, Bool_t useSoftBadMap = kFALSE,
    Bool_t useForcedRunNumber = kFALSE, Int_t ForcedRunNumber = 294925) {
  ...
  // // input must be AOD
  // TString inputDataType = mgr->GetInputEventHandler()->GetDataType(); // can
  // be "ESD" or "AOD" if( "AOD" != inputDataType )
  //   ::Error("AddAODPHOSTender", Form("AOD input data required, input data is
  //   of type: %s", inputDataType.Data()));
  ...
  if (useSoftBadMap)
    PHOSSupply->SetPrivateOADBBadMap(
        "$ALICE_PHYSICS/OADB/PHOS/PHOSBadMapsSoft.root");
  if (useForcedRunNumber)
    PHOSSupply->ForceUsingDummyRunNumber(ForcedRunNumber);
