...
ClassImp(AliAnalysisTaskEtaPhigg)
    ...
    : AliAnalysisTaskSE(name),
      // fStack(0x0),
      fOutputContainer(nullptr), fEvent(nullptr), fPHOSEvent(nullptr),
      fCPVEvent(nullptr), fV0AFlat(nullptr), fV0CFlat(nullptr), fRP(0.), fMF(0),
      fRunNumber(0), fCentrality(0.), fCenBin(0), fPHOSGeo(nullptr),
      fEventCounter(0), fBadMap(nullptr) {
  // Constructor
...
void AliAnalysisTaskEtaPhigg::UserCreateOutputObjects() {
  ...
  // Event selection
  fOutputContainer->Add(new TH2F("hSelEvents", "Event selection", 10, 0., 10.,
                                 nRuns, 0., float(nRuns)));
  ...
  // vertex distribution
  fOutputContainer->Add(new TH2F("hZvertex", "Z vertex position", 50, -25., 25.,
                                 nRuns, 0., float(nRuns)));

  // Centrality
  fOutputContainer->Add(new TH2F("hCentrality", "Event centrality", 100, 0.,
                                 100., nRuns, 0., float(nRuns)));
  fOutputContainer->Add(new TH2F("hCenPHOS", "Centrality vs PHOSclusters", 100,
                                 0., 100., 200, 0., 200.));
  fOutputContainer->Add(new TH2F("hCenPHOSm1",
                                 "Centrality vs PHOSclusters in mod1 ", 100, 0.,
                                 100., 100, 0., 100.));
  fOutputContainer->Add(new TH2F("hCenPHOSm2",
                                 "Centrality vs PHOSclusters in mod2 ", 100, 0.,
                                 100., 100, 0., 100.));
  fOutputContainer->Add(new TH2F("hCenPHOSm3",
                                 "Centrality vs PHOSclusters in mod3 ", 100, 0.,
                                 100., 100, 0., 100.));
  fOutputContainer->Add(new TH2F("hCenPHOSm4",
                                 "Centrality vs PHOSclusters in mod4 ", 100, 0.,
                                 100., 100, 0., 100.));
  fOutputContainer->Add(new TH2F("hCenPHOSCells", "Centrality vs PHOS cells",
                                 100, 0., 100., 100, 0., 1000.));
  fOutputContainer->Add(new TH1F(
      "hPHOSBadMod", "Number of bad event due to module", 10, 0., 10.));

  fOutputContainer->Add(new TH2F("hCenCPV", "Centrality vs CPVclusters", 100,
                                 0., 100., 2000, 0., 2000.));

  fOutputContainer->Add(new TH2F("hCenTrack", "Centrality vs tracks", 100, 0.,
                                 100., 100, 0., 15000.));
  fOutputContainer->Add(new TH2F("hCenTOF", "Centrality vs PHOS TOF", 100, 0.,
                                 100., 200, -400.e-9, 400.e-9));

  //   fOutputContainer->Add(new TH2F("hCenEMCAL","Centrality vs EMCAL photons",
  //   100,0.,100.,200,0.,1000.)) ;
  //
  //   fOutputContainer->Add(new TH2F("hPHOSvsEMCAL","PHOS vs EMCAL photons",
  //   100,0.,100.,200,0.,1000.)) ;
  ...
  // Bad Map
  fOutputContainer->Add(
      new TH2F("hCluLowM1", "Cell (X,Z), M1", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hCluLowM2", "Cell (X,Z), M2", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hCluLowM3", "Cell (X,Z), M3", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hCluLowM4", "Cell (X,Z), M4", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hDisp", "Cell (X,Z), M3", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hTrackVeto", "Cell (X,Z), M3", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(new TH2F("hDispTrackVeto", "Cell (X,Z), M3", 64, 0.5,
                                 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hCPVVeto", "Cell (X,Z), M3", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hDispCPVVeto", "Cell (X,Z), M3", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hBothVeto", "Cell (X,Z) ", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hDispBothVeto", "Cell (X,Z)", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hNotCPVVeto", "Cell (X,Z)", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hDispNotCPVVeto", "Cell (X,Z)", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hNotTrackVeto", "Cell (X,Z)", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(new TH2F("hDispNotTrackVeto", "Cell (X,Z)", 64, 0.5,
                                 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hNotBothVeto", "Cell (X,Z)", 64, 0.5, 64.5, 56, 0.5, 56.5));
  fOutputContainer->Add(
      new TH2F("hDispNotBothVeto", "Cell (X,Z)", 64, 0.5, 64.5, 56, 0.5, 56.5));

  fOutputContainer->Add(
      new TH2F("hTofM1", "TOF in M3", 100, 0., 20., 400, -4.e-6, 4.e-6));
  fOutputContainer->Add(
      new TH2F("hTofM2", "TOF in M3", 100, 0., 20., 400, -4.e-6, 4.e-6));
  fOutputContainer->Add(
      new TH2F("hTofM3", "TOF in M3", 100, 0., 20., 400, -4.e-6, 4.e-6));
  fOutputContainer->Add(
  ...
  for (Int_t cen = 0; cen < kCentBins; cen++) {
    for (Int_t iCut = 0; iCut < kCuts; iCut++) {
      ...
      fhReq[cen][iCut] =
          new TH2F(Form("hReq_%s_cen%d", fcut[iCut], cen), "Qinv distribution",
                   100, 0., 0.25, 40, 0., 2.);
      fOutputContainer->Add(fhReq[cen][iCut]);
      fhMiq[cen][iCut] =
          new TH2F(Form("hMiq_%s_cen%d", fcut[iCut], cen), "Qinv distribution",
                   100, 0., 0.25, 40, 0., 2.);
  ...
  for (Int_t cen = 0; cen < kCentBins; cen++) {
    for (Int_t ikT = 0; ikT < kKtbins; ikT++) {
      fhReOSL[cen][ikT] = new TH3F(Form("hReOSL_DzCr_Kt%3.1f-%3.1f_cen%d",
                                        fKtBins[ikT], fKtBins[ikT + 1], cen),
                                   "real Out-Side-Long", nQ, -qMax, qMax, nQ,
                                   -qMax, qMax, nQ, -qMax, qMax);
      fOutputContainer->Add(fhReOSL[cen][ikT]);
      fhMiOSL[cen][ikT] = new TH3F(Form("hMiOSL_DzCr_Kt%3.1f-%3.1f_cen%d",
                                        fKtBins[ikT], fKtBins[ikT + 1], cen),
                                   "real Out-Side-Long", nQ, -qMax, qMax, nQ,
                                   -qMax, qMax, nQ, -qMax, qMax);
      fOutputContainer->Add(fhMiOSL[cen][ikT]);
      fhReOSLCTS[cen][ikT] = new TH3F(Form("hReOSL_DzCTS_Kt%3.1f-%3.1f_cen%d",
                                           fKtBins[ikT], fKtBins[ikT + 1], cen),
                                      "real Out-Side-Long", nQ, -qMax, qMax, nQ,
                                      -qMax, qMax, nQ, -qMax, qMax);
      fOutputContainer->Add(fhReOSLCTS[cen][ikT]);
      fhMiOSLCTS[cen][ikT] = new TH3F(Form("hMiOSL_DzCTS_Kt%3.1f-%3.1f_cen%d",
                                           fKtBins[ikT], fKtBins[ikT + 1], cen),
                                      "real Out-Side-Long", nQ, -qMax, qMax, nQ,
                                      -qMax, qMax, nQ, -qMax, qMax);
...
//_____________________________________________________________________________
void AliAnalysisTaskEtaPhigg::FillHistogram(const char *key, double x,
                                            double y) const {
  // FillHistogram
  TObject *tmp = fOutputContainer->FindObject(key);
  if (!tmp) {
  ...
  AliError(
      Form("Calling FillHistogram with 2 parameters for histo <%s> of type %s",
           key, tmp->IsA()->GetName()));
}

//_____________________________________________________________________________
void AliAnalysisTaskEtaPhigg::FillHistogram(const char *key, double x, double y,
                                            double z) const {
...
//___________________________________________________________________________
bool AliAnalysisTaskEtaPhigg::PairCut(const AliCaloPhoton *ph1,
                                      const AliCaloPhoton *ph2,
                                      Int_t cut) const {
  ...
  //   badPair =
  //     ((ph1->DistToBad() == 49) || (ph1->DistToBad() == 50)) &&
  //     ((ph2->DistToBad() == 49) || (ph2->DistToBad() == 50));
  // }
  // if (ph1->Module() == 2 && ph2->Module() == 2) { // noisy regions 6,7,  18,
  // 34,37,38, 63
  //   badPair =
  //     (((ph1->DistToBad() == 6) || (ph1->DistToBad() == 7)) &&
  //     ((ph2->DistToBad() == 6) || (ph2->DistToBad() == 7))) ||
  //     (((ph1->DistToBad() == 34) || (ph1->DistToBad() == 37) ||
  //     (ph1->DistToBad() == 38)) &&
  //      ((ph2->DistToBad() == 34) || (ph2->DistToBad() == 37) ||
  //      (ph2->DistToBad() == 38)));
  // }
  // if (ph1->Module() == 3 && ph2->Module() == 3) { // noisy regions 6,
  // 18,21,22,  33,34,37,38,  49,50,63
  //   badPair =
  //     (((ph1->DistToBad() == 18) || (ph1->DistToBad() == 21) ||
  //     (ph1->DistToBad() == 22)) &&
  //      ((ph2->DistToBad() == 18) || (ph2->DistToBad() == 21) ||
  //      (ph2->DistToBad() == 22))) ||
  //     (((ph1->DistToBad() == 33) || (ph1->DistToBad() == 34) ||
  //     (ph1->DistToBad() == 37) || (ph1->DistToBad() == 38)) &&
  //      ((ph2->DistToBad() == 33) || (ph2->DistToBad() == 34) ||
  //      (ph2->DistToBad() == 37) ||
  //       (ph2->DistToBad() == 38))) ||
  //     (((ph1->DistToBad() == 49) || (ph1->DistToBad() == 50) ||
  //     (ph1->DistToBad() == 63)) &&
  ...
  if (cut < kCuts) { // Emin & Dz * CPV/CTS
    int iecut = cut / 16;
    float eMinCut = 0.1 + iecut * 0.05;
    int iPID = cut % 16;
    int iDz = iPID / 4;
    iPID = iPID % 4;
    if (ph1->E() < eMinCut || ph2->E() < eMinCut)
      return false; // Emin
    if (abs(ph1->Theta() - ph2->Theta()) < TMath::ATan2(6. + 1.5 * iDz, 460.))
      return false;
    if (iPID == 0)
      return track1CPV && ph1->IsDispOK() && track2CPV &&
             ph2->IsDispOK(); // Disp & CTS
    if (iPID == 1)
      return track1CPV2 && ph1->IsDispOK() && track2CPV2 &&
             ph2->IsDispOK(); // Disp & CTS2
    if (iPID == 2)
  ...
  //   case 4:
  //     return r > r1cut && track1CPV && ph1->IsDispOK() && track2CPV &&
  //     ph2->IsDispOK();
  //   case 5:
  //     return r > r1cut && ph1->Module() == 3 && ph2->Module() == 3;
  //   case 6:
  //     return r > r1cut && ph1->Module() == 3 && ph2->Module() == 3 && cpv1CPV
  //     && cpv2CPV;
  //   case 7:
  //     return r > r1cut && ph1->Module() == 3 && ph2->Module() == 3 && cpv1CPV
  //     && cpv2CPV && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 8:
  //     return r > r1cut && ph1->Module() == 3 && ph2->Module() == 3 &&
  //     cpv1CPV2 && cpv2CPV2;
  //   case 9:
  //     return r > r1cut && ph1->Module() == 3 && ph2->Module() == 3 &&
  //     cpv1CPV2 && cpv2CPV2 && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 10:
  //     return r > r1cut && ph1->Module() == 3 && ph2->Module() == 3 &&
  ...
  //     ph1->IsDispOK() && ph2->IsDispOK();
  //   case 17:
  //     return r > r2cut && !badPair && ph1->Module() == 3 && ph2->Module() ==
  //     3;
  //   case 18:
  //     return r > r2cut && !badPair && ph1->Module() == 3 && ph2->Module() ==
  //     3 && cpv1CPV && cpv2CPV;
  ...
  //   case 27:
  //     return r > r2cut && !badPair && ph1->E() > 0.2 && ph2->E() > 0.2 &&
  //     track1CPV && track2CPV && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 28:
  //     return r > r2cut && !badPair && ph1->E() > 0.2 && ph2->E() > 0.2 &&
  //     track1CPV2 && track2CPV2 && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 29:
  //     return r > r2cut && !badPair && ph1->E() > 0.2 && ph2->E() > 0.2 &&
  ...
  //   case 51:
  //     return r > r3cut && track1CPV && track2CPV && ph1->IsDispOK() &&
  //     ph2->IsDispOK();
  //   case 52:
  //     return r > r3cut && track1CPV2 && track2CPV2 && ph1->IsDispOK() &&
  //     ph2->IsDispOK();
  //   case 53:
  //     return r > r3cut && ph1->Module() == 3 && ph2->Module() == 3;
  //   case 54:
  //     return r > r3cut && ph1->Module() == 3 && ph2->Module() == 3 && cpv1CPV
  //     && cpv2CPV;
  //   case 55:
  //     return r > r3cut && ph1->Module() == 3 && ph2->Module() == 3 &&
  //     cpv1CPV2 && cpv2CPV2;
  ...
  //   case 60:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && ph1->IsDispOK()
  //     && ph2->IsDispOK();
  //   case 61:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && track1CPV &&
  //     track2CPV;
  //   case 62:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && track1CPV2 &&
  //     track2CPV2;
  //   case 63:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && track1CPV &&
  //     track2CPV && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 64:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && track1CPV2 &&
  //     track2CPV2 && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 65:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && ph1->Module()
  //     == 3 && ph2->Module() == 3;
  //   case 66:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && ph1->Module()
  //     == 3 && ph2->Module() == 3 && cpv1CPV &&
  //            cpv2CPV;
  //   case 67:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && ph1->Module()
  //     == 3 && ph2->Module() == 3 && cpv1CPV2 &&
  //            cpv2CPV2;
  //   case 68:
  //     return r > r3cut && ph1->E() > 0.2 && ph2->E() > 0.2 && ph1->Module()
  //     == 3 && ph2->Module() == 3 && cpv1CPV &&
  ...
  //   case 72:
  //     return r > r3cut && ph1->E() > 0.3 && ph2->E() > 0.3 && ph1->IsDispOK()
  //     && ph2->IsDispOK();
  //   case 73:
  //     return r > r3cut && ph1->E() > 0.3 && ph2->E() > 0.3 && track1CPV &&
  //     track2CPV;
  //   case 74:
  //     return r > r3cut && ph1->E() > 0.3 && ph2->E() > 0.3 && track1CPV2 &&
  //     track2CPV2;
  //   case 75:
  //     return r > r3cut && ph1->E() > 0.3 && ph2->E() > 0.3 && track1CPV &&
  //     track2CPV && ph1->IsDispOK() &&
  ...
  //            ph2->IsDispOK();
  //   case 77:
  //     return r > r3cut && ph1->E() > 0.3 && ph2->E() > 0.3 && ph1->Module()
  //     == 3 && ph2->Module() == 3;
  //   case 78:
  //     return r > r3cut && ph1->E() > 0.3 && ph2->E() > 0.3 && ph1->Module()
  //     == 3 && ph2->Module() == 3 && cpv1CPV &&
  ...
  //   case 87:
  //     return r > r4cut && track1CPV && track2CPV && ph1->IsDispOK() &&
  //     ph2->IsDispOK();
  //   case 88:
  //     return r > r4cut && track1CPV2 && track2CPV2 && ph1->IsDispOK() &&
  //     ph2->IsDispOK();
  //   case 89:
  //     return r > r4cut && ph1->Module() == 3 && ph2->Module() == 3;
  //   case 90:
  //     return r > r4cut && ph1->Module() == 3 && ph2->Module() == 3 && cpv1CPV
  //     && cpv2CPV;
  //   case 91:
  //     return r > r4cut && ph1->Module() == 3 && ph2->Module() == 3 &&
  //     cpv1CPV2 && cpv2CPV2;
  ...
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     ph1->E() > 0.2 && ph2->E() > 0.2 &&
  //            ph1->Module() == 3 && ph2->Module() == 3 && cpv1CPV2 &&
  //            cpv2CPV2;
  //   case 104:
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     ph1->E() > 0.2 && ph2->E() > 0.2 &&
  ...
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     ph1->E() > 0.2 && ph2->E() > 0.2 &&
  //            ph1->Module() == 3 && ph2->Module() == 3 && cpvAndTrack1 &&
  //            cpvAndTrack2;
  //   // Dz Emin 0.3
  //   case 107:
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  ...
  //   case 120:
  //     return r > r5cut && ph1->E() > 0.3 && ph2->E() > 0.3 && ph1->IsDispOK()
  //     && ph2->IsDispOK();
  //   case 121:
  //     return r > r5cut && ph1->E() > 0.3 && ph2->E() > 0.3 && track1CPV &&
  //     track2CPV;
  //   case 122:
  //     return r > r5cut && ph1->E() > 0.3 && ph2->E() > 0.3 && track1CPV2 &&
  //     track2CPV2;
  //   case 123:
  //     return r > r5cut && ph1->E() > 0.3 && ph2->E() > 0.3 && track1CPV &&
  //     track2CPV && ph1->IsDispOK() &&
  ...
  //            ph2->IsDispOK();
  //   case 125:
  //     return r > r5cut && ph1->E() > 0.3 && ph2->E() > 0.3 && ph1->Module()
  //     == 3 && ph2->Module() == 3;
  //   case 126:
  //     return r > r5cut && ph1->E() > 0.3 && ph2->E() > 0.3 && ph1->Module()
  //     == 3 && ph2->Module() == 3 && cpv1CPV &&
  ...
  //   case 167:
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     !badPair && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 168:
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     !badPair && track1CPV && track2CPV;
  //   case 169:
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     !badPair && track1CPV2 && track2CPV2;
  //   case 170:
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     !badPair && track1CPV && track2CPV &&
  //            ph1->IsDispOK() && ph2->IsDispOK();
  ...
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     !badPair && ph1->Module() == 3 &&
  //            ph2->Module() == 3 && cpv1CPV && cpv2CPV && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 176:
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     !badPair && ph1->Module() == 3 &&
  //            ph2->Module() == 3 && cpv1CPV2 && cpv2CPV2 && ph1->IsDispOK() &&
  //            ph2->IsDispOK();
  //   case 177:
  //     return abs(ph1->Theta() - ph2->Theta()) > TMath::ATan2(6., 460.) &&
  //     !badPair && ph1->Module() == 3 &&
...
void AliAnalysisTaskEtaPhigg::SetCutNames() {
  ...
  // First 160 cuts can be combined as
  // if(cut<160){ //Emin & Dz * CPV/CTS
  //   int iecut = cut/16;
  //   float eMinCut = 0.1 + iecut*0.05;
  //   int iPID = cut%16;
  //   int iDz = iPID/4;
  //   iPID = iPID%4;
  //   if(ph1->E()<eMinCut || ph2->E() < eMinCut) return false; //Emin
  //   if(abs(ph1->Theta() - ph2->Theta()) < TMath::ATan2(6. + 1.5*iDz, 460.))
  //   return false ; if(iPID==0) return track1CPV && ph1->IsDispOK() &&
  //   track2CPV && ph2->IsDispOK();  //Disp & CTS if(iPID==1) return track1CPV2
  //   && ph1->IsDispOK() && track2CPV2 && ph2->IsDispOK();  //Disp & CTS2
  //   if(iPID==2) return ph1->Module() == 3 && ph2->Module() == 3 && cpv1CPV &&
  //   ph1->IsDispOK() && cpv2CPV && ph2->IsDispOK();  //Disp & CTS if(iPID==2)
  //   return ph1->Module() == 3 && ph2->Module() == 3 && cpv1CPV2 &&
  //   ph1->IsDispOK() && cpv2CPV2 && ph2->IsDispOK();  //Disp & CTS
  // }

  char pid[4][10];
  snprintf(pid[0], 10, "DispCTS");
  snprintf(pid[1], 10, "DispCTS2");
  snprintf(pid[2], 10, "DispCPV");
  snprintf(pid[3], 10, "DispCPV2");
  ...
  for (int cut = 0; cut < kCuts; cut++) {
    ...
    int iDz = iPID / 4;
    iPID = iPID % 4;
    snprintf(fcut[cut], 20, "Dz%dE%d%s", iDz, iecut, pid[iPID]);
  }
...
//_________________________________________________________________________
int AliAnalysisTaskEtaPhigg::TestCPV(int mod, double e, double xPHOS,
                                     double zPHOS, double dxPHOS, double dzPHOS,
                                     int itr) {
  ...
  if (itr >= 0) {
    ...
    pt = tr->Pt();
    double sx = TMath::Min(5.2, 1.111 + 0.56 * TMath::Exp(-0.031 * pt * pt) +
                                    4.8 / TMath::Power(pt + 0.61, 3));
    double sz = TMath::Min(3.3, 1.12 + 0.35 * TMath::Exp(-0.032 * pt * pt) +
                                    0.75 / TMath::Power(pt + 0.24, 3));
    double meanX = 0, meanZ = 0.102;
    if (fMF < 0.) { // field --
      if (tr->Charge() > 0)
        meanX = TMath::Min(5.8, 0.42 + 0.70 * TMath::Exp(-0.015 * pt * pt) +
                                    35.8 / TMath::Power(pt + 1.41, 3));
      else
        meanX = -TMath::Min(5.8, 0.17 + 0.64 * TMath::Exp(-0.019 * pt * pt) +
                                     26.1 / TMath::Power(pt + 1.21, 3));
    } else { // Field ++
      if (tr->Charge() > 0)
        meanX = -TMath::Min(5.8, 0.58 + 0.68 * TMath::Exp(-0.027 * pt * pt) +
                                     28.0 / TMath::Power(pt + 1.28, 3));
      else
        meanX = TMath::Min(5.8, 0.11 + 0.67 * TMath::Exp(-0.015 * pt * pt) +
                                    29.9 / TMath::Power(pt + 1.29, 3));
    }
  ...
  if (mod == 3) {
    ...
    if (dxCPV * dxCPV + dzCPV * dzCPV < 25.) {
      ...
      dxMin = dxCPV - dxMin;
      double sigmaX =
          TMath::Max(1.5, -2.07124 / e + 6.69554 / TMath::Sqrt(e) - 1.70062);
      double sigmaZ = 1.24859122035152259;
      if (e > 0.4)
        sigmaZ =
            5.58984e-01 * TMath::Exp(-e * e / 2.20543 / 2.20543) + 7.07696e-01;
      double r1 =
    ...
    if (itr == icpvTr) {
      ...
      float sx = 1.5;
      float sz = 1.2;
      cpvAndTrack = (dxPHOS - dxCPV) * (dxPHOS - dxCPV) / (sx * sx) +
                        (dzPHOS - dzCPV) * (dzPHOS - dzCPV) / (sz * sz) >
                    9.;
    }
...
Int_t AliAnalysisTaskEtaPhigg::FindTrackMatching(int det, TVector3 &locPHOS,
                                                 int mod, float &dxMin,
                                                 float &dzMin) {
  // Find closest CPV
  if (det == 0) {
  ...
  if (!TGeoGlobalMagField::Instance()->GetField()) {
    ...
    AliMagF *field =
        new AliMagF("Maps", "Maps", magSign, magSign, AliMagF::k5kG);
  ...
  const Double_t r =
      TMath::Abs(locPHOS.Y()); // Distance to center of  PHOS/CPV module
  const Double_t kYmax =
      72. + 30.; // Size of the module (with some reserve) in phi direction
  const Double_t kZmax =
      64. + 30.; // Size of the module (with some reserve) in z direction
  const Double_t kAlpha0 =
      330. / 180. * TMath::Pi(); // First PHOS module angular direction
  const Double_t kAlpha = 20. / 180. * TMath::Pi(); // PHOS module angular size

  Double_t gposTrack[3];

  Double_t bz =
      ((AliMagF *)TGeoGlobalMagField::Instance()->GetField())->SolenoidField();
  ...
  //  Complementary hybrid tracks: filter bit 512 (1<<9) - all other bits are
  //  reset
  int filterbits[3] = {128, 256, 512};

  for (Int_t i = 0; i < nt; i++) {
    aodTrack = (AliAODTrack *)fEvent->GetTrack(i);
    if (!aodTrack->TestFilterBit(filterbits[0]) &&
        !aodTrack->TestFilterBit(filterbits[1]) &&
    ...
    if (TMath::Abs(y) < kYmax) {
      ...
      // outerParam.CorrectForMaterial(...); // Correct for the TOF material, if
      // needed
      ...
      if (aodTrack->Charge() > 0) { // Pos
        if (det == 1) {             // PHOS
          mean = 2.85224e-02 - exp(-(x * x - 4.57197 * x - 3.63253e+01) /
                                   (7.48345 + x * 8.14244));
        } else { // cpv
          mean = -2.93946e-01 - exp(-(x * x + 1.15256e+07 * x - 1.77233e+07) /
                                    (6.18025e+06 + x * 1.45154e+06));
        }
      } else { // Neg
        if (det == 1) {
          mean = 2.77034e+00 + exp(-(x * x + 2.83814e+07 * x - 1.03977e+08) /
                                   (2.80232e+07 + x * 3.80026e+06));
        } else { // cpv
          mean = 4.38793e-02 - exp(-(x * x + 2.34803e+01 * x - 4.88497e+01) /
                                   (1.24926e+01 + x * 1.23229e+01));
        }
      }
      ddx -= mean;
