...
public:
  ...
  enum PeriodVar
  {
    ...
    kLHC20f11c,       //!< anchored LHC16qt pass 2 - general purpose EPOS
    ...
    kLHC16rsP2GJ,     //!< anchored LHC16rs pass 2 - Gamma-jet MC in EMCal+DCAL acc
    kLHC16rsP2JJLow,  //!< anchored LHC16rs pass 2 - Jet-Jet MC in EMCal+DCAL acc gamma 3.5GeV
    kLHC16rsP2JJHigh, //!< anchored LHC16rs pass 2 - Jet-jet MC in EMCal+DCAL acc gamma 7.0GeV
    ...
    kLHC17HERJJ,    //! Herwig JJ MC anchored to LHC17
  ...
  void SetTriggerMimicking(Int_t value)
  {
    fMimicTrigger = value;
    if (value)
  ...
  {
    fRejectTriggerOverlap = value;
    if (value)
      AliInfo("enabled trigger overlap rejection");
  }
  ...
  void SetEtaShift(TString pPbOrPbp)
  {
    ...
    if (!pPbOrPbp.CompareTo("pPb"))
      etaShift = -0.465;
    else if (!pPbOrPbp.CompareTo("Pbp"))
      etaShift = 0.465;
    fEtaShift = etaShift;
  }
  ...
  {
    fMaxFacPtHardSingleParticle = value;
  ...
  void InitCutHistograms(TString name = "",
                         Bool_t preCut = kTRUE);
  void SetLightOutput(Int_t flag)
  {
    fDoLightOutput = flag;
    return;
  }
