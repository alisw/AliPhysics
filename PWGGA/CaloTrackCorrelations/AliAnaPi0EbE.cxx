...
TList *  AliAnaPi0EbE::GetCreateOutputObjects()
{
  ...
  if(fAnaType != kSSCalo && fSelectPairInIsoCone)
  {
    ...
    ("hMassPtIsoRCutPtMaxPair",Form("#it{M}: #it{p}_{T}^{max} vs #it{M}, for R = %1.1f, #it{p}_{T,1} < %2.2f",fR,fIsoCandMinPt),
     nptbins,ptmin,ptmax, nmassbins,massmin,massmax);
    ...
    ("hMassPtIsoRCutPtMinPair",Form("#it{M}: #it{p}_{T}^{min} vs #it{M}, for R = %1.1f, #it{p}_{T,1} < %2.2f",fR,fIsoCandMinPt),
     nptbins,ptmin,ptmax, nmassbins,massmin,massmax);
      ...
      if(fSelectPairInIsoCone && fMomentum1.Pt() > fIsoCandMinPt)
      {
        ...
        if(radius < fR)
        {
          ...
          if(photon1->Pt() >= photon2->Pt())
          {
            ...
            fhMassPtIsoRCutPtMinPair->Fill(photon2->Pt(), mass, GetEventWeight());
          }
          else
          {
            fhMassPtIsoRCutPtMaxPair->Fill(photon2->Pt(), mass, GetEventWeight());
