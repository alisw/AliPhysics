...
/// possibility to activate only certain analysis.
///
/// It can do:
                                     ...
                                     Bool_t  printSettings, Int_t  debug         )
{
  ...
  if ( simulation && kAnaGammaHadronCorrAnaSel.Contains("Smearing"))
  {
  ...
  // Pile-up
  if(kAnaGammaHadronCorrAnaSel.Contains("SPDPileUp"))
  {
    printf("ConfigureReader() - Switch on Pile-up event rejection by SPD\n");
                                      ...
                                      Bool_t  printSettings, Int_t   debug)
{
  ...
  // Branch AOD settings
  ana->SetOutputAODName(Form("PhotonTrigger_%s",kAnaGammaHadronCorrAnaSel.Data()));
                                      ...
                                      Bool_t  printSettings, Int_t debug           )
{
  ...
  // Branch AOD settings
  ana->SetOutputAODName(Form("%s%sTrigger_%s",particle.Data(), opt.Data(), kAnaGammaHadronCorrAnaSel.Data()));
  ...
  {
    ana->SetInputAODName(Form("PhotonTrigger_%s",kAnaGammaHadronCorrAnaSel.Data()));
  ...
  else  ///////////////////////////////////
  {
    ...
    if(col=="PbPb" || kAnaGammaHadronCorrAnaSel.Contains("150"))
    {
                                                    ...
                                                    Bool_t  printSettings, Int_t   debug                )
{
  ...
  if(particle.Contains("Photon"))
  {
    ...
    if(kAnaGammaHadronCorrAnaSel.Contains("Decay"))
    {
  ...
  ana->SetInputAODName(Form("%sTrigger_%s",particle.Data(),kAnaGammaHadronCorrAnaSel.Data()));
  ana->SetAODObjArrayName(Form("IC%sTrigger_%s_R%1.1f_ThMin%1.1f",particle.Data(),kAnaGammaHadronCorrAnaSel.Data(),cone,pth));
                                                                    ...
                                                                    Bool_t  printSettings, Int_t   debug                      )
{
  ...
  if(particle.Contains("Photon"))
  {
    if(kAnaGammaHadronCorrAnaSel.Contains("Decay"))
    {
      ...
      if(kAnaGammaHadronCorrAnaSel.Contains("Bkg")) ana->SwitchOnBackgroundBinsTaggedDecayPtInConeHistograms();
    }

    printf("**** SET M02 limits in correlation task *** \n");
    ana->SetM02Cut(0.10,shshMax);
    
    if(kAnaGammaHadronCorrAnaSel.Contains("Bkg")) ana->SwitchOnBackgroundBinsPtInConeHistograms();
  }
  ...
  else
  {
    ...
    ana->SetNCentrBin(12);
    ana->SetNRPBin(3);
    if(kAnaGammaHadronCorrAnaSel.Contains("60_90"))
    {
  ...
  ana->SetInputAODName(Form("%sTrigger_%s",particle.Data(),kAnaGammaHadronCorrAnaSel.Data()));
  ana->SetAODObjArrayName(Form("%sHadronCorrIso%dTrigger_%s",particle.Data(),bIsolated,kAnaGammaHadronCorrAnaSel.Data()));
  //ana->SetAODNamepTInConeHisto(Form("IC%s_%s_R%1.1f_ThMin%1.1f"           ,particle.Data(),kAnaGammaHadronCorrAnaSel.Data(),cone,pth));
...
AliAnaChargedParticles* ConfigureChargedAnalysis( Bool_t simulation, Bool_t printSettings, Int_t   debug )
{
  ...
  ana->SetOutputAODName(Form("HadronTrigger_%s",kAnaGammaHadronCorrAnaSel.Data()));
...
)
{
  ...
  //
  maker->SetReader   ( ConfigureReader   (col,simulation,clustersArray,calorimeter,nonLinOn,calibrate,year,
                                          trigger,rejectEMCTrig,minCen,maxCen,printSettings,debug) );  
  ...
  { 
    maker->AddAnalysis(ConfigureQAAnalysis(col,simulation,calorimeter,year,printSettings,debug) , n++);
  }
  ...
  //
  AliAnalysisTaskCaloTrackCorrelation * task = new AliAnalysisTaskCaloTrackCorrelation (Form("%s",kAnaGammaHadronCorrAnaSel.Data()));
  ...
  AliAnalysisDataContainer *cout_pc   = mgr->CreateContainer(kAnaGammaHadronCorrAnaSel, TList::Class(),
                                                             AliAnalysisManager::kOutputContainer,
                                                             Form("%s",outputfile.Data()));
  
  AliAnalysisDataContainer *cout_cuts = mgr->CreateContainer(Form("Param_%s",kAnaGammaHadronCorrAnaSel.Data()), TList::Class(),
