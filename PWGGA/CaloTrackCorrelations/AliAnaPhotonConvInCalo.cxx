...
TList *  AliAnaPhotonConvInCalo::GetCreateOutputObjects()
{
  ...
  if(IsDataMC())
  {
    ...
    ("hConvM02MCConversion","#it{p}_{T} of selected conversion pairs from string",100,0.,1.,100,0.,1.);
    ...
    //Float_t eConvPair   = fMomentum.E();
      ...
      if(pairM < fMassCut)
      {
        ...
        if(IsDataMC())
        {
          ...
          ancLabel  = GetMCAnalysisUtils()->CheckCommonAncestor(cluster1->GetLabel(), cluster2->GetLabel(),
           ...
           asymmetry        < fConvAsymCut       )
        {
          ...
          for(Int_t iebin = 0; iebin < 6; iebin++)
          {
            if( calo->Pt() > ebin[iebin] && calo->Pt() <= ebin[iebin+1] ) bin = iebin ;
          }
          
          if(bin > -1) fhEtaPhiPhotonConvPaired[bin] ->Fill(calo->Eta(), phi, GetEventWeight());
          
          bin = -1;
          for(Int_t iebin = 0; iebin < 6; iebin++)
          {
            if( calo2->Pt() > ebin[iebin] && calo2->Pt() <= ebin[iebin+1] ) bin = iebin ;
          }
          ...
          for(Int_t iebin = 0; iebin < 6; iebin++)
          {
            if( ptConvPair > ebin[iebin] && ptConvPair <= ebin[iebin+1] ) bin = iebin ;
          }
          
          if(bin > -1) fhEtaPhiPhotonConv[bin] ->Fill(etaConvPair, phiConvPair, GetEventWeight());
