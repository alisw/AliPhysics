 ...
 * Converted to Protons by A. Marin 2019
 * Version 1.0																*
 *																			*
...
////////////////////////////////////////////////
//--------------------------------------------- 
// Class reconstructing primary protons
//---------------------------------------------
////////////////////////////////////////////////
...
ClassImp(AliPrimaryProtonSelector)

//________________________________________________________________________
AliPrimaryProtonSelector::AliPrimaryProtonSelector(const char *name) : AliAnalysisTaskSE(name),
    fProtonCuts(0),
    fPosProtonsIndex(),
    fNegProtonsIndex(),
...
//________________________________________________________________________
AliPrimaryProtonSelector::~AliPrimaryProtonSelector()
{
...
//________________________________________________________________________
void AliPrimaryProtonSelector::Init()
{
    // Initialize function to be called once before analysis

    if(fProtonCuts == 0){
    //  fProtonCuts=AliConversionCuts::GetStandardCuts2010PbPb();
...
//________________________________________________________________________
void AliPrimaryProtonSelector::UserCreateOutputObjects()
{
    // Create User Output Objects
}

//________________________________________________________________________
void AliPrimaryProtonSelector::UserExec(Option_t *){
    // User Exec
	fEventIsSelected=ProcessEvent(fInputEvent,fMCEvent);
}

//________________________________________________________________________
Bool_t AliPrimaryProtonSelector::ProcessEvent(AliVEvent *inputEvent,AliMCEvent *mcEvent)
{
	//Reset the index

	fPosProtonsIndex.clear();
	fNegProtonsIndex.clear();
	...
	if(!fProtonCuts){AliError("No ConversionCuts");return kFALSE;}
...
///________________________________________________________________________
Bool_t AliPrimaryProtonSelector::ProcessESDs(){
	...
	if(fESDEvent){
		for(Int_t currentTrackIndex=0;currentTrackIndex<fESDEvent->GetNumberOfTracks();currentTrackIndex++){
			...
			if (  fProtonCuts->ProtonIsSelected( fCurrentTrack ) ) {
				if( fCurrentTrack->GetSign() > 0.0 ){
					fPosProtonsIndex.push_back(currentTrackIndex);
				} else {
					fNegProtonsIndex.push_back(currentTrackIndex);
				}
			}
		}
	}
...
///________________________________________________________________________
Bool_t AliPrimaryProtonSelector::ProcessAODs(){
    ...
    if(fAODEvent){
        for(Int_t currentTrackIndex=0;currentTrackIndex<fAODEvent->GetNumberOfTracks();currentTrackIndex++){
            ...
            if (  fProtonCuts->ProtonIsSelectedAOD( fCurrentTrack ) ) {
                if( sign > 0.0 ){
                    fPosProtonsIndex.push_back(currentTrackIndex);
                } else {
                    fNegProtonsIndex.push_back(currentTrackIndex);
...
//________________________________________________________________________
void AliPrimaryProtonSelector::Terminate(Option_t *)
{
