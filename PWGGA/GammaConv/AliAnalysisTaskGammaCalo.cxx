...
AliAnalysisTaskGammaCalo::AliAnalysisTaskGammaCalo(): AliAnalysisTaskSE(),
  ...
  fHistoTruePrimaryPi0RecPtMCPtVsMass(NULL),
  fHistoTruePrimaryEtaRecPtMCPtVsMass(NULL),
...
void AliAnalysisTaskGammaCalo::UserCreateOutputObjects(){
  ...
  for(int iCut = 0; iCut<fnCuts;iCut++){
    if(((AliCaloPhotonCuts*)fClusterCutArray->At(iCut))->GetDoSecondaryTrackMatching()){
  ...
  std::vector<double> arrPtBinningCourse;
  if( fDoPi0Only ){
    nBinsMinv                 = 150;
    maxMinv                  = 0.3;
  }
  ...
  double valPtBinCourse = 0.;
  while(true){
    arrPtBinningCourse.push_back(valPtBinCourse);
    if(valPtBinCourse - epsilon < 10) valPtBinCourse += 0.5;
    else if(valPtBinCourse - epsilon < 20) valPtBinCourse += 1;
    else if(valPtBinCourse - epsilon < 50) valPtBinCourse += 5;
    else if(valPtBinCourse - epsilon < 100) valPtBinCourse += 10;
    else break;
  }
  ...
  if(fDoMesonAnalysis){
    ...
    if(fDoECalibOutput || (fDoClusterQA > 0)){
  ...
  if(fIsMC> 0){
    ...
    if(fDoMesonAnalysis){
      ...
      if (fDoMesonQA > 0 && fDoMesonQA < 3 ){
        ...
        fHistoTruePrimaryPi0RecPtMCPtVsMass               = new TH3F*[fnCuts];
        fHistoTruePrimaryEtaRecPtMCPtVsMass               = new TH3F*[fnCuts];
        if( !fDoPi0Only ){
    ...
    for(Int_t iCut = 0; iCut<fnCuts;iCut++){
      ...
      if(fDoECalibOutput || (fDoClusterQA > 0)){
        fHistoTruePi0InvMassECalib[iCut] = new TH2F("True_Pi0_InvMass_E_Calib", "True_Pi0_InvMass_E_Calib", nBinsMinv, 0, maxMinv, nBinsPt, arrPtBinning);
        ...
        fHistoTrueEtaInvMassECalib[iCut] = new TH2F("True_Eta_InvMass_E_Calib", "True_Eta_InvMass_E_Calib", nBinsMinv, 0, maxMinv, nBinsPt, arrPtBinning);
      ...
      if (fIsMC > 1){
        ...
        if(fDoECalibOutput || (fDoClusterQA > 0)){
          fHistoTruePi0InvMassECalib[iCut]->Sumw2();
          fHistoTrueEtaInvMassECalib[iCut]->Sumw2();
        }
      }
      ...
      if(fDoMesonAnalysis){
        ...
        if (fDoMesonQA > 0 && fDoMesonQA < 3){
          ...
          std::vector<double> vecInvMassPi0 = {0.};
          double minvPi0=vecInvMassPi0.back();
          while(minvPi0 < 0.3){
            minvPi0+=0.02;
            vecInvMassPi0.push_back(minvPi0);
          }
          fHistoTruePrimaryPi0RecPtMCPtVsMass[iCut]       = new TH3F("ESD_TruePrimaryPi0_RecPt_MCPt_InvMass", "ESD_TruePrimaryPi0_RecPt_MCPt_InvMass", arrPtBinningCourse.size()-1, arrPtBinningCourse.data(), arrPtBinningCourse.size()-1, arrPtBinningCourse.data(), vecInvMassPi0.size()-1, vecInvMassPi0.data());
          fHistoTruePrimaryPi0RecPtMCPtVsMass[iCut]->SetXTitle("p_{T, rec.} (GeV/c)");
          fHistoTruePrimaryPi0RecPtMCPtVsMass[iCut]->SetYTitle("p_{T, MC} (GeV/c)");
          fHistoTruePrimaryPi0RecPtMCPtVsMass[iCut]->SetYTitle("m_{inv} (GeV/c^{2})");
          fTrueList[iCut]->Add(fHistoTruePrimaryPi0RecPtMCPtVsMass[iCut]);
          
          std::vector<double> vecInvMassEta = {0.4};
          ...
          fHistoTruePrimaryEtaRecPtMCPtVsMass[iCut]->SetYTitle("p_{T, MC} (GeV/c)");
          fHistoTruePrimaryEtaRecPtMCPtVsMass[iCut]->SetYTitle("m_{inv} (GeV/c^{2})");
          fTrueList[iCut]->Add(fHistoTruePrimaryEtaRecPtMCPtVsMass[iCut]);
          ...
          if (fIsMC > 1){
            ...
            fHistoTruePrimaryPi0RecPtMCPtVsMass[iCut]->Sumw2();
            fHistoTruePrimaryEtaRecPtMCPtVsMass[iCut]->Sumw2();
...
void AliAnalysisTaskGammaCalo::ProcessTrueClusterCandidates(AliAODConversionPhoton *TruePhotonCandidate, Double_t clusterM02)
{
  ...
  if (TruePhotonCandidate->IsLargestComponentPhoton() || (TruePhotonCandidate->IsLargestComponentElectron() && TruePhotonCandidate->IsConversion())) {
   ...
   if (!fDoLightOutput && TruePhotonCandidate->IsLargestComponentPhoton()) fHistoTrueClusPhotonGammaEResE[fiCut]->Fill(TruePhotonCandidate->E(), (TruePhotonCandidate->E()-Photon->E())/TruePhotonCandidate->E(), tempPhotonWeight);
  if (fDoClusterQA > 0){
    ...
    for(Int_t firstGammaIndex=0;firstGammaIndex<fClusterCandidates->GetEntries();firstGammaIndex++){
      ...
      for(Int_t secondGammaIndex=firstGammaIndex+1;secondGammaIndex<fClusterCandidates->GetEntries();secondGammaIndex++){
        ...
        if((((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->MesonIsSelected(pi0cand,kTRUE,((AliConvEventCuts*)fEventCutArray->At(fiCut))->GetEtaShift(),gamma0->GetLeadingCellID(),gamma1->GetLeadingCellID(), gamma0->GetIsCaloPhoton(), gamma1->GetIsCaloPhoton())) && ((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->MesonLeadTrackSelection(fInputEvent, pi0cand)){
          ...
          // fill new histograms
          if( (fDoECalibOutput || (fDoClusterQA > 0) ) && TMath::Abs(pi0cand->GetAlpha())<0.1){
            // we use pi0cand->E() and later when making the projection we use double the energy to get an everage cluster energy, since the clusters have roughly the same thanks to the alpha cut
...
void AliAnalysisTaskGammaCalo::ProcessTrueMesonCandidates(AliAODConversionMother *Pi0Candidate, AliAODConversionPhoton *TrueGammaCandidate0, AliAODConversionPhoton *TrueGammaCandidate1)
{
  ...
  if(isTruePi0 || isTrueEta){// True Pion or Eta
    if (isTruePi0){
      ...
      if( (fDoECalibOutput || (fDoClusterQA > 0) ) && TMath::Abs(Pi0Candidate->GetAlpha())< 0.1){
        fHistoTruePi0InvMassECalib[fiCut]->Fill(Pi0Candidate->M(),Pi0Candidate->E(),tempTruePi0CandWeight);
      }
    }
    if (isTrueEta && !fDoPi0Only){
      ...
      if((fDoECalibOutput || (fDoClusterQA > 0) ) && TMath::Abs(Pi0Candidate->GetAlpha())< 0.1){
        fHistoTrueEtaInvMassECalib[fiCut]->Fill(Pi0Candidate->M(),Pi0Candidate->E(),tempTruePi0CandWeight);
      }
    }
    if (fDoMesonQA > 0 && fDoMesonQA < 3){
    ...
    if(!isPrimary){ // Secondary Meson
      ...
      if (fDoMesonQA > 0 && fDoMesonQA < 3){
        if(isTruePi0){ // Only primary pi0 for resolution
          ...
          fHistoTruePrimaryPi0RecPtMCPtVsMass[fiCut]->Fill(Pi0Candidate->Pt(), ((AliMCParticle*)fMCEvent->GetTrack(gamma1MotherLabel))->Pt(), Pi0Candidate->M(), weighted* tempTruePi0CandWeight);
        }
        if (isTrueEta && !fDoPi0Only){ // Only primary eta for resolution
          ...
          fHistoTruePrimaryEtaRecPtMCPtVsMass[fiCut]->Fill(Pi0Candidate->Pt(), ((AliMCParticle*)fMCEvent->GetTrack(gamma1MotherLabel))->Pt(), Pi0Candidate->M(), weighted* tempTruePi0CandWeight);
        }
      }
    }
    ...
    if(!isPrimary){ // Secondary Meson
      ...
      if (fDoMesonQA > 0 && fDoMesonQA < 3){
        if(isTruePi0){ // Only primary pi0 for resolution
          ...
          fHistoTruePrimaryPi0RecPtMCPtVsMass[fiCut]->Fill(Pi0Candidate->Pt(), static_cast<AliAODMCParticle*>(fAODMCTrackArray->At(gamma1MotherLabel))->Pt(), Pi0Candidate->M(),weighted* tempTruePi0CandWeight);
        ...
        fHistoTruePrimaryEtaRecPtMCPtVsMass[fiCut]->Fill(Pi0Candidate->Pt(), static_cast<AliAODMCParticle*>(fAODMCTrackArray->At(gamma1MotherLabel))->Pt(), Pi0Candidate->M(),weighted* tempTruePi0CandWeight);
        }
      }
    }
  } else if(!isTruePi0 && !isTrueEta) { // Background
      ...
      for(Int_t iCurrent=0;iCurrent<fClusterCandidates->GetEntries();iCurrent++){
        ...
        for(UInt_t iPrevious=0;iPrevious<previousEventV0s->size();iPrevious++){
          ...
          if((((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))
            ...
            if((fDoECalibOutput || (fDoClusterQA > 0) ) && TMath::Abs(backgroundCandidate->GetAlpha())<0.1){
...
void AliAnalysisTaskGammaCalo::CalculateBackgroundSwapp(){
  ...
  if(((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->DoGammaSwappForBg()) {
    ...
    std::vector<std::array<Double_t, 2>> vSwappingInvMassEAlphaCut;
    ...
    if(fClusterCandidates->GetEntries() > 2 ){
            ...
            for(auto const& kCurrentClusterCandidates  : *fClusterCandidates){
              ...
              if(!(((AliCaloPhotonCuts*)fClusterCutArray->At(fiCut))->CheckDistanceToBadChannelSwapping(cellIDRotatedPhoton1, fInputEvent, ((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->GetDistanceToBorderForBg())) && lvRotationPhoton1.E() > ((AliCaloPhotonCuts*)fClusterCutArray->At(fiCut))->GetMinClusterEnergy())
              {
                if(((AliConversionMesonCuts*) fMesonCutArray->At(fiCut))->MesonIsSelected(backgroundCandidate1.get(),kFALSE,((AliConvEventCuts*)fEventCutArray->At(fiCut))->GetEtaShift(), cellIDRotatedPhoton1, ((AliAODConversionPhoton*) kCurrentClusterCandidates)->GetLeadingCellID())  && ((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->MesonLeadTrackSelection(fInputEvent, backgroundCandidate1.get()))
                {
                  ...
                  if(((fDoECalibOutput || (fDoClusterQA > 0) )) && TMath::Abs(backgroundCandidate1->GetAlpha())<0.1){
                    vSwappingInvMassEAlphaCut.push_back({backgroundCandidate1->M(),backgroundCandidate1->E()});
                  }
                }
              }
              if(!(((AliCaloPhotonCuts*)fClusterCutArray->At(fiCut))->CheckDistanceToBadChannelSwapping(cellIDRotatedPhoton2, fInputEvent, ((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->GetDistanceToBorderForBg())) && lvRotationPhoton2.E() > ((AliCaloPhotonCuts*)fClusterCutArray->At(fiCut))->GetMinClusterEnergy())
              {
                if(((AliConversionMesonCuts*) fMesonCutArray->At(fiCut))->MesonIsSelected(backgroundCandidate2.get(),kFALSE,((AliConvEventCuts*)fEventCutArray->At(fiCut))->GetEtaShift(), cellIDRotatedPhoton2, ((AliAODConversionPhoton*) kCurrentClusterCandidates)->GetLeadingCellID()) && ((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->MesonLeadTrackSelection(fInputEvent, backgroundCandidate2.get()))
                {
                  ...
                  if((fDoECalibOutput || (fDoClusterQA > 0) ) && TMath::Abs(backgroundCandidate2->GetAlpha())<0.1){
                    vSwappingInvMassEAlphaCut.push_back({backgroundCandidate2->M(),backgroundCandidate2->E()});
                  }
                }
              }
            }
      ...
      if(((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->DoWeightingInSwappBg() && vSwappingInvMassEAlphaCut.size() > 0){
        tempMultWeightSwapping = (0.5*(fClusterCandidates->GetEntries()*fClusterCandidates->GetEntries() - fClusterCandidates->GetEntries()))/(vSwappingInvMassEAlphaCut.size());
      }
      if(fDoECalibOutput || (fDoClusterQA > 0) ){
        for(Int_t i = 0; i < (Int_t)vSwappingInvMassEAlphaCut.size(); i++){
          fHistoMotherBackInvMassECalib[fiCut]->Fill(vSwappingInvMassEAlphaCut.at(i)[0], vSwappingInvMassEAlphaCut.at(i)[1],tempMultWeightSwapping*fWeightJetJetMC);
        }
      }
    }
  }
