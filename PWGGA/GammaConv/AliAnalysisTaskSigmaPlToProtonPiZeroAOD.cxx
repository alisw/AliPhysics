...
void AliAnalysisTaskSigmaPlToProtonPiZeroAOD::UserCreateOutputObjects()
{
	...
	const Int_t NBins[5] = {20, 30, 15, 30, 10};
	const Double_t lowerBinEdges[5] = {1.1 ,0., 0., 0. ,0.};
	const Double_t upperBinEdges[5] = {1.3, 30., 15., 0.3, TMath::Pi()};
  ...
  fHistProtonPt = new TH1F*[fnCuts];
    ...
    fHistPi0PtEta->Sumw2();
  	...
  	for(Int_t iCut = 0; iCut<fnCuts;iCut++){
    	...
    	fOutputList->Add(fAODList[iCut]);
	  	...
	  	fHistSigmaPlus[iCut] = new THnD("fHistSigmaPlus", "", 5, NBins, lowerBinEdges, upperBinEdges );
    ...
    fHistPodolanskiWCut[iCut]->Sumw2();
		...
		fFitPodolanski[iCut] = new TF1("fFitPodolanski","sqrt(0.3*0.3*(1-(((x-0.61)*(x-0.61))/(0.41*0.41))))",0.2,1.02);

		fAODList[iCut]->Add(fFitPodolanski[iCut]);
      ...
      fFitPi0MassMCHighPt[iCut] = new TF1("fFitPi0MassMCHighPt","[0]*1/x*log(x*x)+[1]*1/(x*x)+[2]",1.,25.);
			...
			fHistSigmaPlusMC[iCut] = new THnD("fHistSigmaPlusMC", "", 5, NBins, lowerBinEdges, upperBinEdges );
	...
	if(fIsMC > 0){
		...
		for(Long_t i = 1; i < fAODMCTrackArray->GetEntriesFast(); ++i) {
            ...
            if(fHistPi0PtEta) fHistPi0PtEta->Fill(pionTrueVec.Pt(), pionTrueVec.Eta());
						...
						if(fHistPodolanskiGenTrue) fHistPodolanskiGenTrue->Fill(GetPodAlpha(sigmaTrueVec, protonTrueVec, pionTrueVec),GetQT(sigmaTrueVec, protonTrueVec));
					}
			...
			if( photon.size() > 1){
				for(unsigned int iProton = 0; iProton < proton.size(); ++iProton){
					...
					for(unsigned int iPhoton1 = 0; iPhoton1 < photon.size(); ++iPhoton1) {
                     ...
                     if(fHistPodolanskiWCutTrue[iCut]) fHistPodolanskiWCutTrue[iCut]->Fill(GetPodAlpha(sigmaVektor, protonVektor, rekombinatedPi0),GetQT(sigmaVektor, protonVektor));
                    }
                    if(GetPodAlpha(sigmaVektor, protonVektor, rekombinatedPi0) > 1.02 || GetPodAlpha(sigmaVektor, protonVektor, rekombinatedPi0) < 0.2) continue;
										if(GetQT(sigmaVektor, rekombinatedPi0) > fFitPodolanski[iCut]->Eval(GetPodAlpha(sigmaVektor, protonVektor, rekombinatedPi0)) || GetQT(sigmaVektor, protonVektor) > fFitPodolanski[iCut]->Eval(GetPodAlpha(sigmaVektor, protonVektor, rekombinatedPi0))) continue;
                    ...
                    if(fHistPodolanskiWCut[iCut]) fHistPodolanskiWCut[iCut]->Fill(GetPodAlpha(sigmaVektor, protonVektor, rekombinatedPi0),GetQT(sigmaVektor, protonVektor));
                    Double_t farrSigma[5] = {sigmaVektor.M(), sigmaVektor.Pt(), pi0cand.Pt(), pi0cand.M(), rekombinatedPi0.Angle(protonVektor.Vect())};
