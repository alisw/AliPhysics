  ...
  Bool_t storeTriggerCondition = true)
{
  ...
  if (rmaxFacPtHardSetting->GetEntries() < 1) {
    cout << "ERROR: AddTask_ConvCaloTree during parsing of settingMaxFacPtHard String '" << settingMaxFacPtHard.Data() << "'" << endl;
    return;
  }
  ...
  if (periodNameV0Reader.CompareTo("") != 0)
    analysisEventCuts->SetPeriodEnum(periodNameV0Reader);
  ...
  if (TaskEMCCutnumber.CompareTo("") != 0 || TaskPHOSCutnumber.CompareTo("") != 0)
    fConvCaloTree->SetSaveClusters(kTRUE);
  if (TaskConversionCutnumber.CompareTo("") != 0)
    fConvCaloTree->SetSaveConversions(saveConversionCutInfo);
  fConvCaloTree->SetSaveTracks(doSaveSurroundingTracks);

  fConvCaloTree->SetEventCuts(analysisEventCuts, IsHeavyIon);
  if (TaskEMCCutnumber.CompareTo("") != 0)
    fConvCaloTree->SetClusterCutsEMC(analysisClusterCutsEMC);
  if (TaskPHOSCutnumber.CompareTo("") != 0)
    fConvCaloTree->SetClusterCutsPHOS(analysisClusterCutsPHOS);
  if (TaskConversionCutnumber.CompareTo("") != 0)
    fConvCaloTree->SetConversionCuts(analysisConversionCuts);
  ...
  if (isMC && doSaveMCInfo > 0)
    fConvCaloTree->SetSaveMCInformation(doSaveMCInfo);
  ...
  fConvCaloTree->SetJetReaderName(JetReaderName);
  if (fracEventDownscaling > 0.99)
