#ifndef ALIPRIMARYPROTONCUTS_H
#define ALIPRIMARYPROTONCUTS_H
...
class AliPrimaryProtonCuts : public AliAnalysisCuts {
	...
	AliPrimaryProtonCuts(const char *name="ProtonCuts", const char * title="Proton Cuts");
	virtual ~AliPrimaryProtonCuts();                            //virtual destructor
	...
	TString GetCutNumber();
		...
		// Cut Selection
    Bool_t ProtonIsSelectedMC(Int_t labelParticle,AliMCEvent *mcEvent);
    Bool_t ProtonIsSelectedAODMC(Int_t labelParticle, TClonesArray *AODMCTrackArray);
    Bool_t TrackIsSelected(AliESDtrack* lTrack);
    Bool_t TrackIsSelectedAOD(AliAODTrack* lTrack);
    Bool_t ProtonIsSelected(AliESDtrack* lTrack);
    Bool_t ProtonIsSelectedAOD(AliAODTrack* lTrack);
	static AliPrimaryProtonCuts * GetStandardCuts2010PbPb();
	static AliPrimaryProtonCuts * GetStandardCuts2010pp();
	...
	Bool_t SetTPCdEdxCutProtonLine(Int_t pidedxSigmaCut);
	Bool_t SetITSdEdxCutProtonLine(Int_t ededxSigmaCut);
	...
	Bool_t SetTOFProtonPIDCut(Int_t TOFelectronPID);
	...
	Double_t fPIDnSigmaAboveProtonLineITS; // sigma cut ITS
	Double_t fPIDnSigmaBelowProtonLineITS; // sigma cut ITS
	Double_t fPIDnSigmaAboveProtonLineTPC; // sigma cut TPC
	Double_t fPIDnSigmaBelowProtonLineTPC; // sigma cut TPC
	Double_t fPIDnSigmaAboveProtonLineTOF; // sigma cut TOF
	Double_t fPIDnSigmaBelowProtonLineTOF; // sigma cut TOF 
  ...
  TString fCutStringRead;
	...
	AliPrimaryProtonCuts(const AliPrimaryProtonCuts&); // not implemented
	AliPrimaryProtonCuts& operator=(const AliPrimaryProtonCuts&); // not implemented


    ClassDef(AliPrimaryProtonCuts,1)
};
