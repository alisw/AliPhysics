...
ClassImp(AliPrimaryProtonCuts)


const char* AliPrimaryProtonCuts::fgkCutNames[AliPrimaryProtonCuts::kNCuts] = {
	"kEtaCut",				// 0
	"kClsITSCut",			// 1
	"kClsTPCCut",			// 2
	"kDCAcut",				// 3
...
AliPrimaryProtonCuts::AliPrimaryProtonCuts(const char *name,const char *title) : AliAnalysisCuts(name,title),
	fHistograms(NULL),
	...
	fPIDnSigmaAboveProtonLineITS(100),
	fPIDnSigmaBelowProtonLineITS(-100),
	fPIDnSigmaAboveProtonLineTPC(100),
	fPIDnSigmaBelowProtonLineTPC(-100),
	fPIDnSigmaAboveProtonLineTOF(100), // RRnewTOF
	fPIDnSigmaBelowProtonLineTOF(-100), // RRnewTOF
...
//________________________________________________________________________
AliPrimaryProtonCuts::~AliPrimaryProtonCuts() {
		// Destructor
...
//________________________________________________________________________
void AliPrimaryProtonCuts::InitCutHistograms(TString name, Bool_t preCut,TString cutNumber){
	...
	if(fHistograms==NULL){
		...
		if(name=="")fHistograms->SetName(Form("ProtonCuts_%s",cutName.Data()));
		else fHistograms->SetName(Form("%s_%s",name.Data(),cutName.Data()));
	}


	fHistCutIndex=new TH1F(Form("IsProtonSelected %s",cutName.Data()),"IsProtonSelected",10,-0.5,9.5);
	fHistCutIndex->GetXaxis()->SetBinLabel(kProtonIn+1,"in");
	...
	fHistCutIndex->GetXaxis()->SetBinLabel(kProtonOut+1,"out");
	fHistograms->Add(fHistCutIndex);

	// dEdx Cuts
	fHistdEdxCuts=new TH1F(Form("ProtondEdxCuts %s",cutName.Data()),"dEdxCuts",5,-0.5,4.5);
	fHistdEdxCuts->GetXaxis()->SetBinLabel(1,"in");
	fHistdEdxCuts->GetXaxis()->SetBinLabel(2,"ITSproton");
	fHistdEdxCuts->GetXaxis()->SetBinLabel(3,"TPCproton");
	fHistdEdxCuts->GetXaxis()->SetBinLabel(4,"TOFproton");
    ...
    if(!fDoLightOutput){
      if(preCut){
        fHistITSdEdxbefore=new TH2F(Form("Proton_ITS_before %s",cutName.Data()),"ITS dEdx proton before" ,150,0.05,20,400,-10,10);
        fHistograms->Add(fHistITSdEdxbefore);
        axisBeforeITS = fHistITSdEdxbefore->GetXaxis();

        fHistTPCdEdxbefore=new TH2F(Form("Proton_dEdx_before %s",cutName.Data()),"dEdx proton before" ,150,0.05,20,400,-10,10);
        fHistograms->Add(fHistTPCdEdxbefore);
        axisBeforedEdx = fHistTPCdEdxbefore->GetXaxis();

        fHistTPCdEdxSignalbefore=new TH2F(Form("Proton_dEdxSignal_before %s",cutName.Data()),"dEdx proton signal before" ,150,0.05,20.0,800,0.0,200);
        fHistograms->Add(fHistTPCdEdxSignalbefore);
        axisBeforedEdxSignal = fHistTPCdEdxSignalbefore->GetXaxis();

        fHistTOFbefore=new TH2F(Form("Proton_TOF_before %s",cutName.Data()),"TOF proton before" ,150,0.05,20,400,-6,10);
        fHistograms->Add(fHistTOFbefore);
        axisBeforeTOF = fHistTOFbefore->GetXaxis();

        fHistTrackDCAxyPtbefore = new TH2F(Form("hTrackProton_DCAxy_Pt_before %s",cutName.Data()),"DCAxy Vs Pt of tracks before",800,-4.0,4.0,400,0.,10.);
        fHistograms->Add(fHistTrackDCAxyPtbefore);

        fHistTrackDCAzPtbefore  = new TH2F(Form("hTrackProton_DCAz_Pt_before %s",cutName.Data()), "DCAz  Vs Pt of tracks before",800,-4.0,4.0,400,0.,10.);
        fHistograms->Add(fHistTrackDCAzPtbefore);

        fHistTrackNFindClsPtTPCbefore = new TH2F(Form("hTrackProton_NFindCls_Pt_TPC_before %s",cutName.Data()),"Track: N Findable Cls TPC Vs Pt before",100,0,1,400,0.,10.);
        fHistograms->Add(fHistTrackNFindClsPtTPCbefore);
      }

      fHistITSdEdxafter=new TH2F(Form("Proton_ITS_after %s",cutName.Data()),"ITS dEdx proton after" ,150,0.05,20,400, -10,10);
      fHistograms->Add(fHistITSdEdxafter);

      fHistTPCdEdxafter=new TH2F(Form("Proton_dEdx_after %s",cutName.Data()),"dEdx proton after" ,150,0.05,20,400, -10,10);
      fHistograms->Add(fHistTPCdEdxafter);

      fHistTPCdEdxSignalafter=new TH2F(Form("Proton_dEdxSignal_after %s",cutName.Data()),"dEdx proton signal after" ,150,0.05,20.0,800,0.0,200);
      fHistograms->Add(fHistTPCdEdxSignalafter);

      fHistTOFafter=new TH2F(Form("Proton_TOF_after %s",cutName.Data()),"TOF proton after" ,150,0.05,20,400,-6,10);
      fHistograms->Add(fHistTOFafter);

      fHistTrackDCAxyPtafter  = new TH2F(Form("hTrackProton_DCAxy_Pt_after %s",cutName.Data()),"DCAxy Vs Pt of tracks after",800,-4.0,4.0,400,0.,10.);
      fHistograms->Add(fHistTrackDCAxyPtafter);

      fHistTrackDCAzPtafter  = new TH2F(Form("hTrackProton_DCAz_Pt_after %s",cutName.Data()), "DCAz Vs Pt of tracks  after",800,-4.0,4.0,400,0.,10.);
      fHistograms->Add(fHistTrackDCAzPtafter);

      fHistTrackNFindClsPtTPCafter = new TH2F(Form("hTrackProton_NFindCls_Pt_TPC_after %s",cutName.Data()),"Track: N Findable Cls TPC Vs Pt after",100,0,1,400,0.,10.);
...
//________________________________________________________________________
Bool_t AliPrimaryProtonCuts::InitPIDResponse(){
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::ProtonIsSelectedMC(Int_t labelParticle,AliMCEvent *mcEvent){
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::ProtonIsSelectedAODMC(Int_t labelParticle,TClonesArray *AODMCTrackArray){
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::ProtonIsSelected(AliESDtrack* lTrack){
	...
	if (fHistCutIndex) fHistCutIndex->Fill(kProtonIn);
	...
	//Proton passed the cuts
	if (fHistCutIndex) fHistCutIndex->Fill(kProtonOut);
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::ProtonIsSelectedAOD(AliAODTrack* lTrack){
    //Selection of Reconstructed electrons
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::TrackIsSelected(AliESDtrack* lTrack) {
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::TrackIsSelectedAOD(AliAODTrack* lTrack) {
  // Track Selection for Photon Reconstruction
  Double_t clsToF = GetNFindableClustersTPC(lTrack);
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::dEdxCuts(AliVTrack *fCurrentTrack){

	// Proton Identification Cuts for Photon reconstruction
	...
	if( fDodEdxSigmaITSCut == kTRUE ){
		if( fPIDResponse->NumberOfSigmasITS(fCurrentTrack,AliPID::kProton)<fPIDnSigmaBelowProtonLineITS ||
				fPIDResponse->NumberOfSigmasITS(fCurrentTrack,AliPID::kProton)> fPIDnSigmaAboveProtonLineITS ){	
			if(fHistdEdxCuts)fHistdEdxCuts->Fill(cutIndex);
			return kFALSE;
		}
	}
	...
	if(fDodEdxSigmaTPCCut == kTRUE){
		// TPC Proton Line
		if( fPIDResponse->NumberOfSigmasTPC(fCurrentTrack,AliPID::kProton)<fPIDnSigmaBelowProtonLineTPC ||
			fPIDResponse->NumberOfSigmasTPC(fCurrentTrack,AliPID::kProton)>fPIDnSigmaAboveProtonLineTPC){
			if(fHistdEdxCuts)fHistdEdxCuts->Fill(cutIndex);
			return kFALSE;
		}
		cutIndex++;
	...
	if( ( fCurrentTrack->GetStatus() & AliESDtrack::kTOFpid ) && ( !( fCurrentTrack->GetStatus() & AliESDtrack::kTOFmismatch) ) ){
		...
		if(fUseTOFpid){
			if( fPIDResponse->NumberOfSigmasTOF(fCurrentTrack, AliPID::kProton)>fPIDnSigmaAboveProtonLineTOF ||
				fPIDResponse->NumberOfSigmasTOF(fCurrentTrack, AliPID::kProton)<fPIDnSigmaBelowProtonLineTOF ){
				if(fHistdEdxCuts)fHistdEdxCuts->Fill(cutIndex);
				return kFALSE;
			}
		}
...
///________________________________________________________________________
AliVTrack *AliPrimaryProtonCuts::GetTrack(AliVEvent * event, Int_t label){
...
///________________________________________________________________________
Double_t AliPrimaryProtonCuts::GetNFindableClustersTPC(AliVTrack* lTrack){
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::UpdateCutString() {
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::InitializeCutsFromCutString(const TString analysisCutSelection ) {
  fCutStringRead = Form("%s",analysisCutSelection.Data());
  
	// Initialize Cuts from a given Cut string

	AliInfo(Form("Set ProtonCuts Number: %s",analysisCutSelection.Data()));
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetCut(cutIds cutID, const Int_t value) {
	...
	switch (cutID) {
		...
		case kPidedxSigmaITSCut:
			if( SetITSdEdxCutProtonLine(value)) { 
				fCuts[kPidedxSigmaITSCut] = value;
				UpdateCutString();
				return kTRUE;
			} else return kFALSE;
		case kPidedxSigmaTPCCut:
			if( SetTPCdEdxCutProtonLine(value)) { 
				fCuts[kPidedxSigmaTPCCut] = value;
				UpdateCutString();
				return kTRUE;
			} else return kFALSE;
		case kPiTOFSigmaPID:
			if( SetTOFProtonPIDCut(value)) {
...
///________________________________________________________________________
void AliPrimaryProtonCuts::PrintCuts() {
...
///________________________________________________________________________
void AliPrimaryProtonCuts::PrintCutsWithValues() {
   // Print out current Cut Selection with value
	printf("\nCharged Proton cutnumber \n");
	for(Int_t ic = 0; ic < kNCuts; ic++) {
		printf("%d",fCuts[ic]);
	}
	printf("\n\n");
...
// 	"kDCAcut",				// 3
	printf("\t min pT > %3.2f \n", fPtCut);
	printf("PID cuts \n");
	if (fDodEdxSigmaITSCut)printf("\t %3.2f < ITS n_sigma pi < %3.2f \n", fPIDnSigmaBelowProtonLineITS, fPIDnSigmaAboveProtonLineITS );
	if (fDodEdxSigmaTPCCut)printf("\t %3.2f < TPC n_sigma pi < %3.2f \n", fPIDnSigmaBelowProtonLineTPC, fPIDnSigmaAboveProtonLineTPC );
	if (fDoTOFsigmaCut)printf("\t %3.2f < TOF n_sigma pi < %3.2f \n", fPIDnSigmaBelowProtonLineTOF, fPIDnSigmaAboveProtonLineTOF );
	if (fDoMassCut) printf("two-proton mass cut < %3.2f \n", fMassCut);
	printf("\n\n");
}
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetITSdEdxCutProtonLine(Int_t ededxSigmaCut){ 
	switch(ededxSigmaCut){
		...
		case 1: // -10,10
			...
			fPIDnSigmaBelowProtonLineITS=-10;
			fPIDnSigmaAboveProtonLineITS=10;
			break;
		case 2: // -6,7
			fDodEdxSigmaITSCut = kTRUE;
			fPIDnSigmaBelowProtonLineITS=-6;
			fPIDnSigmaAboveProtonLineITS=7;
			break;
		case 3: // -5,5
			fDodEdxSigmaITSCut = kTRUE;
			fPIDnSigmaBelowProtonLineITS=-5;
			fPIDnSigmaAboveProtonLineITS=5;
			break;
		case 4: // -4,5
			fDodEdxSigmaITSCut = kTRUE;
			fPIDnSigmaBelowProtonLineITS=-4;
			fPIDnSigmaAboveProtonLineITS=5;
			break;
		case 5: // -3,5
			fDodEdxSigmaITSCut = kTRUE;
			fPIDnSigmaBelowProtonLineITS=-3;
			fPIDnSigmaAboveProtonLineITS=5;
			break;
		case 6: // -4,4
			fDodEdxSigmaITSCut = kTRUE;
			fPIDnSigmaBelowProtonLineITS=-4;
			fPIDnSigmaAboveProtonLineITS=4;
			break;
		case 7: // -2.5,4
			fDodEdxSigmaITSCut = kTRUE;
			fPIDnSigmaBelowProtonLineITS=-2.5;
			fPIDnSigmaAboveProtonLineITS=4;
			break;
		case 8: // -2,3.5
			fDodEdxSigmaITSCut = kTRUE;
			fPIDnSigmaBelowProtonLineITS=-2;
			fPIDnSigmaAboveProtonLineITS=3.5;
			break;
		default:
			cout<<"Warning: ITSdEdxCutProtonLine not defined"<<ededxSigmaCut<<endl;
			return kFALSE;
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetTPCdEdxCutProtonLine(Int_t ededxSigmaCut){
	switch(ededxSigmaCut){
		case 0: 
			fDodEdxSigmaTPCCut = kFALSE;
			fPIDnSigmaBelowProtonLineTPC=-10;
			fPIDnSigmaAboveProtonLineTPC=10;
			break;
		case 1: // -10,10
			fDodEdxSigmaTPCCut = kTRUE;
			fPIDnSigmaBelowProtonLineTPC=-10;
		...
		case 2: // -6,7
			fDodEdxSigmaTPCCut = kTRUE;
			fPIDnSigmaBelowProtonLineTPC=-6;
			fPIDnSigmaAboveProtonLineTPC=7;
			break;
		case 3: // -5,5
			fDodEdxSigmaTPCCut = kTRUE;
			fPIDnSigmaBelowProtonLineTPC=-5;
			fPIDnSigmaAboveProtonLineTPC=5;
			break;
		case 4: // -4,5
			fDodEdxSigmaTPCCut = kTRUE;
			fPIDnSigmaBelowProtonLineTPC=-4;
			fPIDnSigmaAboveProtonLineTPC=5;
			break;	
		case 5: // -4,4
			fDodEdxSigmaTPCCut = kTRUE;
			fPIDnSigmaBelowProtonLineTPC=-4;
			fPIDnSigmaAboveProtonLineTPC=4;
			break;
		case 6: // -3,4
			fDodEdxSigmaTPCCut = kTRUE;
			fPIDnSigmaBelowProtonLineTPC=-3;
			fPIDnSigmaAboveProtonLineTPC=4;
			break;
		case 7: // -3,3
			fDodEdxSigmaTPCCut = kTRUE;
			fPIDnSigmaBelowProtonLineTPC=-3;
			fPIDnSigmaAboveProtonLineTPC=3;
			break;
		case 8: // -2,3.
			fDodEdxSigmaTPCCut = kTRUE;
			fPIDnSigmaBelowProtonLineTPC=-2;
			fPIDnSigmaAboveProtonLineTPC=3.;
			break;
		default:
			cout<<"Warning: TPCdEdxCutProtonLine not defined"<<ededxSigmaCut<<endl;
			return kFALSE;
	}
	return kTRUE;
}
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetITSClusterCut(Int_t clsITSCut){
	if( !fEsdTrackCuts ) {
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetTPCClusterCut(Int_t clsTPCCut){  
	switch(clsTPCCut){
		case 0: // 0
			fMinClsTPC= 0.;
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetEtaCut(Int_t etaCut){ 
	// Set eta Cut
	switch(etaCut){
		case 0: 
			fEtaCut = 100.;
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetPtCut(Int_t ptCut){ 
	switch(ptCut){	  
		case 0: 
			fPtCut = 0.075;		
			break;
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetDCACut(Int_t dcaCut)
{ 
	// Set DCA Cut
	if( !fEsdTrackCuts ) {
		cout<<"Warning: AliESDtrackCut is not initialized "<<endl;
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetTOFProtonPIDCut(Int_t TOFelectronPID){
    // Set Cut
	switch(TOFelectronPID){ 
		case 0: // no cut
			fRequireTOF = kFALSE;
			fUseTOFpid = kFALSE;
			fPIDnSigmaBelowProtonLineTOF=-100;
			fPIDnSigmaAboveProtonLineTOF=100;
			break;
		case 1: // -7,7
			fRequireTOF = kFALSE;
			fUseTOFpid = kTRUE;
			fPIDnSigmaBelowProtonLineTOF=-7;
			fPIDnSigmaAboveProtonLineTOF=7;
			break;
		case 2: // -5,5
			fRequireTOF = kFALSE;
			fUseTOFpid = kTRUE;
			fPIDnSigmaBelowProtonLineTOF=-5;
			fPIDnSigmaAboveProtonLineTOF=5;
			break;
		case 3: // -3,5
			fRequireTOF = kFALSE;
			fUseTOFpid = kTRUE;
			fPIDnSigmaBelowProtonLineTOF=-3;
			fPIDnSigmaAboveProtonLineTOF=5;
			break;
		case 4: // -2,3
			fRequireTOF = kFALSE;
			fUseTOFpid = kTRUE;
			fPIDnSigmaBelowProtonLineTOF=-2;
			fPIDnSigmaAboveProtonLineTOF=3;
			break;
		case 5: // -3, 3 TOF mandatory
			...
			break;
		default:
			cout<<"Warning: TOFProtonCut not defined "<<TOFelectronPID<<endl;
		return kFALSE;
    } 
    return kTRUE;
}
...
///________________________________________________________________________
Bool_t AliPrimaryProtonCuts::SetMassCut(Int_t massCut){
    // Set Cut
	switch(massCut){ 
		case 0: // no cut
			fDoMassCut = kFALSE;
...
///________________________________________________________________________
TString AliPrimaryProtonCuts::GetCutNumber(){
...
///________________________________________________________________________
AliPrimaryProtonCuts* AliPrimaryProtonCuts::GetStandardCuts2010PbPb(){
    //Create and return standard 2010 PbPb cuts
    AliPrimaryProtonCuts *cuts=new AliPrimaryProtonCuts("StandardCuts2010PbPb","StandardCuts2010PbPb");
...
///________________________________________________________________________
AliPrimaryProtonCuts* AliPrimaryProtonCuts::GetStandardCuts2010pp(){
    //Create and return standard 2010 PbPb cuts
    AliPrimaryProtonCuts *cuts=new AliPrimaryProtonCuts("StandardCuts2010pp","StandardCuts2010pp");
