...
#include "AliESDpid.h"
#include "AliAnalysisTaskGammaConvCaloIso.h"
...
ClassImp(AliAnalysisTaskGammaConvCaloIso)

//________________________________________________________________________
AliAnalysisTaskGammaConvCaloIso::AliAnalysisTaskGammaConvCaloIso(): AliAnalysisTaskSE(),
  fV0Reader(NULL),
...
//________________________________________________________________________
AliAnalysisTaskGammaConvCaloIso::AliAnalysisTaskGammaConvCaloIso(const char *name):
...
AliAnalysisTaskGammaConvCaloIso::~AliAnalysisTaskGammaConvCaloIso()
{
  if(fGammaCandidates){
...
void AliAnalysisTaskGammaConvCaloIso::InitBack(){
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::UserCreateOutputObjects(){
  ...
  if(fDoMesonAnalysis){
    ...
    fHistoMotherInvMassIsoPt          = new TH2F*[fnCuts];
    fHistoMotherBackInvMassPt         = new TH2F*[fnCuts];
    fHistoMotherBackInvMassIsoPt      = new TH2F*[fnCuts];
    if(!fDoLightOutput){
  ...
  for(Int_t iCut = 0; iCut<fnCuts;iCut++){
    ...
    fHistoClusterCandidates[iCut]                         = new TH1F("ClusterCandidates","Cluster Photon Candidates",IsolationBins,startIsolation,endIsolation);
    fIsolation[iCut]->Add(fHistoClusterCandidates[iCut]);
    fHistoFailIsolationCut[iCut]                          = new TH1F("FailIsolationCut","Cluster Candidates, failing Isolation Cut",IsolationBins,startIsolation,endIsolation);
    ...
    if(fDoMesonAnalysis){
      ...
      fHistoMotherInvMassIsoPt[iCut]             = new TH2F("ESD_Mother_InvMass_IsoPt", "ESD_Mother_InvMass_IsoPt", 800, 0, 0.8, nBinsPt, arrPtBinning);
      fHistoMotherInvMassIsoPt[iCut]->SetXTitle("M_{inv} (GeV/c^{2})");
      fHistoMotherInvMassIsoPt[iCut]->SetYTitle("p_{T,Iso} (GeV/c)");
      fESDList[iCut]->Add(fHistoMotherInvMassIsoPt[iCut]);
      ...
      if (fIsMC > 1){
        ...
        fHistoMotherInvMassIsoPt[iCut]->Sumw2();
...
Bool_t AliAnalysisTaskGammaConvCaloIso::Notify()
{
  for(Int_t iCut = 0; iCut<fnCuts;iCut++){
...
//_____________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::UserExec(Option_t *)
{
  //
    ...
    if(fDoMesonAnalysis){ // Meson Analysis
      ...
      if(((AliConversionMesonCuts*)fMesonCutArray->At(iCut))->DoBGCalculation()){
        if(((AliConversionMesonCuts*)fMesonCutArray->At(iCut))->BackgroundHandlerType() == 0){
          ...
          //fBGClusHandlerRP[iCut]->AddEvent(fIsoCandidates,fInputEvent); // Store Event for mixed Events
        }
      }
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessClusters(){
  Int_t nclus = 0;
  ...
  if(!fCorrTaskSetting.CompareTo("")){
    ...
    if(!arrClustersProcess)
      AliFatal(Form("%sClustersBranch was not found in AliAnalysisTaskGammaConvCaloIso! Check the correction framework settings!",fCorrTaskSetting.Data()));
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessTrueClusterCandidates(AliAODConversionPhoton *TruePhotonCandidate, Float_t clusM02)
{
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessTrueClusterCandidatesAOD(AliAODConversionPhoton *TruePhotonCandidate, Float_t clusM02)
{
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessPhotonCandidates()
{
  Int_t nV0 = 0;
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessTruePhotonCandidatesAOD(AliAODConversionPhoton *TruePhotonCandidate)
{
  const AliVVertex* primVtxMC   = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX   = primVtxMC->GetX();
  Double_t mcProdVtxY   = primVtxMC->GetY();
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessTruePhotonCandidates(AliAODConversionPhoton *TruePhotonCandidate)
{
  const AliVVertex* primVtxMC   = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX   = primVtxMC->GetX();
  Double_t mcProdVtxY   = primVtxMC->GetY();
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessAODMCParticles()
{
  const AliVVertex* primVtxMC   = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX   = primVtxMC->GetX();
  Double_t mcProdVtxY   = primVtxMC->GetY();
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessMCParticles()
{
  const AliVVertex* primVtxMC   = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX   = primVtxMC->GetX();
  Double_t mcProdVtxY   = primVtxMC->GetY();
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::CalculatePi0Candidates(){
...
void AliAnalysisTaskGammaConvCaloIso::CalculatePi0CandidatesIsolation(){
    //Isolated Clusters
    if(fIsoCandidates->GetEntries()>0){
      for(Int_t firstGammaIndex=0;firstGammaIndex<fIsoCandidates->GetEntries();firstGammaIndex++){
        AliAODConversionPhoton *gamma0=dynamic_cast<AliAODConversionPhoton*>(fIsoCandidates->At(firstGammaIndex));
        if (gamma0==NULL) continue;
        ...
        for(Int_t secondGammaIndex=0;secondGammaIndex<fClusterCandidates->GetEntries();secondGammaIndex++){
         ...
         // cout << "fIsoCandidates: " << fIsoCandidates->GetEntries() << endl;
         // cout << "fClusterCandidates: " << fClusterCandidates->GetEntries() << endl;
         // cout << "gamma0: " << gamma0ref << endl;
         // cout << "gamma1: " << gamma1->fCaloClusterRef << endl;
          ...
          if((((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->MesonIsSelected(pi0cand,kTRUE,((AliConvEventCuts*)fEventCutArray->At(fiCut))->GetEtaShift()))){
            fHistoMotherInvMassIsoPt[fiCut]->Fill(pi0cand->M(),gamma0->Pt(), fWeightJetJetMC);
...
void AliAnalysisTaskGammaConvCaloIso::ProcessMCParticlesIsolation(AliVCluster *cluster,AliAODConversionPhoton *photoncandidate){
    ...
    // check if esd or aod
     ...
     if(esdev){
       ...
       if (cluster->GetNLabels()>0){
         ...
         AliVParticle* particleLead   = fMCEvent->GetTrack(mclabelsCluster[0]);
         //get particles PDG code
         pdgcode = particleLead->PdgCode();
       ...
       //        cout << motherDummy->GetMother() << endl;
             ...
             fHistoIsoMotherPDGtoPt[fiCut]->Fill(motherDummy->PdgCode(),photoncandidate->Pt(), fWeightJetJetMC);
           }
         }
       }
...
void AliAnalysisTaskGammaConvCaloIso::ProcessTrueMesonCandidates(AliAODConversionMother *Pi0Candidate, AliAODConversionPhoton *TrueGammaCandidate0, AliAODConversionPhoton *TrueGammaCandidate1, Bool_t matched)
{
  // obtain MC vertex
  const AliVVertex* primVtxMC   = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX   = primVtxMC->GetX();
        ...
        if(!isPrimary && gamma0MotherLabel>-1){ // Secondary Meson
          ...
          if (secMotherLabel >-1){
            if(fMCEvent->GetTrack(secMotherLabel)->PdgCode()==310 && isTruePi0 ){
              ...
              if (fDoMesonQA > 0 && fIsMC < 2)fHistoTrueK0sWithPi0DaughterMCPt[fiCut]->Fill(fMCEvent->GetTrack(secMotherLabel)->Pt(),fWeightJetJetMC);
            }
            if(fMCEvent->GetTrack(secMotherLabel)->PdgCode()==130 && isTruePi0 ){
              ...
              if (fDoMesonQA > 0 && fIsMC < 2)fHistoTrueK0lWithPi0DaughterMCPt[fiCut]->Fill(fMCEvent->GetTrack(secMotherLabel)->Pt(),fWeightJetJetMC);
            }
            if(fMCEvent->GetTrack(secMotherLabel)->PdgCode()==221 && isTruePi0){
              ...
              if (fDoMesonQA > 0 && fIsMC < 2)fHistoTrueEtaWithPi0DaughterMCPt[fiCut]->Fill(fMCEvent->GetTrack(secMotherLabel)->Pt(),fWeightJetJetMC);
            }
            if(fMCEvent->GetTrack(secMotherLabel)->PdgCode()==3122 && isTruePi0){
              ...
              if (fDoMesonQA > 0 && fIsMC < 2)fHistoTrueLambdaWithPi0DaughterMCPt[fiCut]->Fill(fMCEvent->GetTrack(secMotherLabel)->Pt(),fWeightJetJetMC);
            }
          }
          ...
          if (fDoMesonQA > 0 && fIsMC < 2){
            if(isTruePi0){ // Only primary pi0 for resolution
              fHistoTruePrimaryPi0MCPtResolPt[fiCut]->Fill(((AliMCParticle*)fMCEvent->GetTrack(gamma1MotherLabel))->Pt(),(Pi0Candidate->Pt()-((AliMCParticle*)fMCEvent->GetTrack(gamma1MotherLabel))->Pt())/((AliMCParticle*)fMCEvent->GetTrack(gamma1MotherLabel))->Pt(),weighted*fWeightJetJetMC);
            }
            if (isTrueEta){ // Only primary eta for resolution
              fHistoTruePrimaryEtaMCPtResolPt[fiCut]->Fill(((AliMCParticle*)fMCEvent->GetTrack(gamma1MotherLabel))->Pt(),(Pi0Candidate->Pt()-((AliMCParticle*)fMCEvent->GetTrack(gamma1MotherLabel))->Pt())/((AliMCParticle*)fMCEvent->GetTrack(gamma1MotherLabel))->Pt(),weighted*fWeightJetJetMC);
            }
          }
        }
      }
...
//______________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessTrueMesonCandidatesAOD(AliAODConversionMother *Pi0Candidate, AliAODConversionPhoton *TrueGammaCandidate0, AliAODConversionPhoton *TrueGammaCandidate1, Bool_t matched)
{
  const AliVVertex* primVtxMC   = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX   = primVtxMC->GetX();
  Double_t mcProdVtxY   = primVtxMC->GetY();
...
void AliAnalysisTaskGammaConvCaloIso::CalculateBackgroundIsolation(){

}

//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::CalculateBackground(){
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::CalculateBackgroundRP(){
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::RotateParticle(AliAODConversionPhoton *gamma){
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::RotateParticleAccordingToEP(AliAODConversionPhoton *gamma, Double_t previousEventEP, Double_t thisEventEP){
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::MoveParticleAccordingToVertex(AliAODConversionPhoton* particle,const AliGammaConversionAODBGHandler::GammaConversionVertex *vertex){
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::UpdateEventByEventData(){
  //see header file for documentation
  if(fGammaCandidates->GetEntries() >0 ){
    if(((AliConversionMesonCuts*)fMesonCutArray->At(fiCut))->UseTrackMultiplicity()){
      ...
      fBGClusHandler[fiCut]->AddEvent(fIsoCandidates,fInputEvent->GetPrimaryVertex()->GetX(),fInputEvent->GetPrimaryVertex()->GetY(),fInputEvent->GetPrimaryVertex()->GetZ(),fV0Reader->GetNumberOfPrimaryTracks(),fEventPlaneAngle);
      ...
      fBGClusHandler[fiCut]->AddEvent(fIsoCandidates,fInputEvent->GetPrimaryVertex()->GetX(),fInputEvent->GetPrimaryVertex()->GetY(),fInputEvent->GetPrimaryVertex()->GetZ(),fGammaCandidates->GetEntries(),fEventPlaneAngle);
    }
  }
}

//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::FillPhotonCombinatorialBackgroundHist(AliAODConversionPhoton *TruePhotonCandidate, Int_t pdgCode[])
{
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::RelabelAODPhotonCandidates(Bool_t mode){
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::SetLogBinningXTH2(TH2* histoRebin){
...
//________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::Terminate(const Option_t *)
{
...
//________________________________________________________________________
Int_t AliAnalysisTaskGammaConvCaloIso::GetSourceClassification(Int_t daughter, Int_t pdgCode){
...
Bool_t AliAnalysisTaskGammaConvCaloIso::CheckVectorOnly(vector<Int_t> &vec, Int_t tobechecked)
{
  if(tobechecked > -1)
  {
...
//_________________________________________________________________________________
Bool_t AliAnalysisTaskGammaConvCaloIso::CheckVectorForDoubleCount(vector<Int_t> &vec, Int_t tobechecked)
{
  if(tobechecked > -1)
  {
    vector<Int_t>::iterator it;
...
//_________________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessConversionPhotonsForMissingTags (){
...
//_________________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::ProcessConversionPhotonsForMissingTagsAOD (){
...
//_________________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::FillMultipleCountMap(map<Int_t,Int_t> &ma, Int_t tobechecked){
...
//_________________________________________________________________________________
void AliAnalysisTaskGammaConvCaloIso::FillMultipleCountHistoAndClear(map<Int_t,Int_t> &ma, TH1F* hist){
