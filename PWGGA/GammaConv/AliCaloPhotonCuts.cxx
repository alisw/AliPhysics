...
//----------------
  ...
  if(fUseDistTrackToCluster && !fDoLightOutput) {
    ...
    //----------------
    if(fExtendedMatchAndQA == 1 || fExtendedMatchAndQA > 2){
      ...
      fHistClusterdPhidPtNegTracksTrueMatched         = new TH2F(Form("dPhiVsPt_negTracks_TrueMatched %s",GetCutNumber().Data()),"dPhiVsPt_negTracks_TrueMatched",2*nPhiBins,2*PhiRange[0],2*PhiRange[1],300,0,30);
...
//cout << "+++++++++ Cluster: x, y, z, R" << clusterPosition[0] << ", " << clusterPosition[1] << ", " << clusterPosition[2] << ", " << clusterR << "+++++++++" << endl;
  ...
  for (Int_t i = 0;i < 2;i++){
    ...
    if (propagated){
      ...
      if(!fDoLightOutput && (fExtendedMatchAndQA == 1 || fExtendedMatchAndQA > 2)){
        if(inTrack->Charge() > 0) {
