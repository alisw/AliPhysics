...
//        fTriggerClassMask0.push_back(atol(row->GetField(5)));
//        fTriggerClassMask1.push_back(atol(row->GetField(6)));
        i++;
    }
    fNumberOfClusters=i;
}
...
void AliEveSaveViews::Save()
{
    ...
    { // NB: this skips the first children (first 3D View)
        ...
        // Save OpenGL view in file and read it back (missing method in Root returning TASImage)
...
/*
  ...
  */
        ...
        if(index==0){
            ...
            /*
            if(fTempImg->GetWidth()>width3DView)   // crop image horizontally
            {
                fTempImg->Crop((fTempImg->GetWidth()-width3DView)/2,0,width3DView,height3DView);
            }
            if(fTempImg->GetHeight()>height3DView) // crop image verically
            {
                fTempImg->Crop(0,(fTempImg->GetHeight()-height3DView)/2,width3DView,height3DView);
            }*/
            ...
            fCompositeImg->DrawRectangle(x,y, widthChildView, heightChildView, "#C0C0C0");
        }
