...
void colladaBufferExample()
{
    ...
    colladaBuffer->AddShape("sphere",sphere->MakeBuffer3D(),translation,rotation,scale);

    // Create a box and add it to Collada buffer
    TGeoBBox *box = new TGeoBBox(5.,10.,10.);
    
    colladaBuffer->AddShape("box",box->MakeBuffer3D(),translation,rotation,scale);

    // Create a tube and add it to Collada buffer
    TGeoTube *tube = new TGeoTube(2.,2.5,30);
    translation[1] = 30;
    
    colladaBuffer->AddShape("tube",tube->MakeBuffer3D(),translation,rotation,scale);
