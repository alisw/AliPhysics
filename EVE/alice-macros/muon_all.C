  ...
  */
  ...
  for (Int_t iSta = 1; iSta <= 7; iSta++) {
    ...
    for (Int_t iCha = 1; iCha <= 2; iCha++) {
      ...
      Reve::RenderElementList* ld = new Reve::RenderElementList(name);
	...
	sprintf(title,"Station %1d chamber %1d (trigger)",iSta,iCha);
      ...
      AliMpDEIterator itd;
      for ( itd.First(iChamber-1); ! itd.IsDone(); itd.Next() ) {
	
	Int_t detElemId = itd.CurrentDE();
  ...
  for (Int_t iSta = 1; iSta <= 5; iSta++) {
	...
	sprintf(title,"Station %1d chamber %1d (trigger)",iSta,iCha);
      ...
      AliMpDEIterator itc;
      for ( itc.First(iChamber-1); ! itc.IsDone(); itc.Next() ) {
