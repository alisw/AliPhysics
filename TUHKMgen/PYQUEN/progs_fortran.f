...
* ----------- INITIALIZATION OF RUNDOM GENERATOR
      MRPY(1)=iseed_fromC 
      write(*,*)'--initialization of high-pt part-- :  '
*      write(*,*)'evnt generator phase  ',iseed_fromC
 
* initialize HYINIT with the input parameters   
      write(*,*)' AW= ', AW,' energy= ', ene, ' ptmin= ',ptmin 
      write(*,*)' bminh= ',bminh,' bmaxh= ', bmaxh 
      write(*,*)' ifb= ', ifb, ' bfix= ', bfix 
      write(*,*)' ishad= ',ishad, ' nhsel=  ',nhsel 
      write(*,*)' ienglu= ',ienglu, ' ianglu= ',ianglu 
      write(*,*)' T0= ',T0,' tau0= ',tau0,' nf= ', nf  
...
* set input PYTHIA parameters from SERVICE common:
      ...
      MSTP(81)=1                  ! pp multiple scattering off 
      MSTJ(21)=0                    ! hadron decays off     
      ...
      PARP(82)=2.              !pt cutoff for multiparton interactions
      MSTJ(11)=3               !Choice of the fragmentation function
                       
* flags to select dijet production channels (active if msel=0)
      MSUB(11)=1   ! q+q->q+q
...
* flags to select prompt photon production channels (active if msel=0)
      ...
      write(*,*)'--initialization of PYTHIA parameters-- :  '

      write(*,*)'CKIN(3) MSEL MSTU(21) PARU(14)'
      write(*,*)CKIN(3), MSEL, MSTU(21), PARU(14)
      
      write(*,*)'MSTP(81) MSTJ(21) MSTP(2) MSTP(33)'
      write(*,*)MSTP(81), MSTJ(21), MSTP(2), MSTP(33)

      write(*,*)'MSTP(51) MSTP(82) PARP(67) PARP(82) MSTJ(11)'
      write(*,*)MSTP(51),MSTP(82),PARP(67),PARP(82),MSTJ(11)

      write(*,*)'MSUB(11) MSUB(12) MSUB(13) MSUB(28) MSUB(53) MSUB(68)'
      write(*,*)MSUB(11),MSUB(12),MSUB(13),MSUB(28),MSUB(53),MSUB(68)
    
      write(*,*)'MSUB(14) MSUB(18) MSUB(29) MSUB(114) MSUB(115)'
      write(*,*)MSUB(14),MSUB(18),MSUB(29),MSUB(114),MSUB(115)
                       
      call hyinit     !       (ene,AW,ifb,bmin,bmax,bfix1,nh) 
...
c* printout of HYDJET versioning 
c      write(6,*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c     >%%%%%%%%%%%%%%%%%%%%%%%%'
c      write(6,*) '%%                        This is HYDJET version 1.5  
c     >                      %%' 
c      write(6,*) '%%         Corresponding author: Igor Lokhtin (Igor.Lo
c     >khtin@cern.ch)        %%' 
c      write(6,*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c     >%%%%%%%%%%%%%%%%%%%%%%%%'
...
****************************** END HYINIT ***************************
      SUBROUTINE MYDELTA
      IMPLICIT DOUBLE PRECISION(A-H, O-Z)
      common/SERVICEEV/delta,KC,ipdg
      COMMON /PYDAT2/KCHG(500,4),PMAS(500,4),PARF(2000),VCKM(4,4)
      integer KC, PYCOMP
      real delta
* allows to transmit delta -limits for BW from PYTHIA to c++ part      
       KC=PYCOMP(ipdg)
       delta=PMAS(int(KC),3)
...
* fill array 'ppart' 
      ...
      do ih=1,nhj
        if(i.le.50000)then 
        ppart(1,ih)= khj(ih,1) ! pdg ?
        ppart(2,ih)= khj(ih,2) ! pdg  -mother        
...
c        write(*,*)'in hyhard pari(21)',pari(21)
    
        Q=pari(21) 
        x=r0*cos(psib1)         !fm
        y=r0*sin(psib1)  !fm        
c        write(*,*)'in hyhard ',x,y        
...
c       write(*,*)'in hyhard after gauss',tau
	z=tau*sinh(etaLj)
	t=tau*cosh(etaLj)	
c	write(*,*)'in hyhard z t',z,t	 
        endif

c       write(*,*)'x y z t', x,y,z,t,etaLj,Q
...
c-ml
        call pyedit(2)                      ! remove partons & leave hadrons 
...
c       write(*,*)'in hyhard 9', njet,nshad

      end if  
      njet=njet-nshad


c       write(*,*)'in hyhard OUT'
