...
void AliAnalysisBGMonitorQAHLT::DrawHist(Int_t* ftrigger, Int_t fSpdT, Int_t fSpdC1, Int_t fSpdC2, Int_t* BBFlagC, Int_t* BBFlagA){
    ...
    ((TH1F*)fList->FindObject(Form("hTotalTrkVsClsSPID_%s",triggername.Data())))->Fill(fSpdT, fSpdC1+fSpdC2); // No PF Selection
        for(Int_t ii=1; ii<33; ii++){
            ...
            if(SelGoodEvent) {
                if(ii == 2){
                    ((TH1F*)fList->FindObject(Form("hTotalTrkVsClsSPID_%s_PF2",triggername.Data())))->Fill(fSpdT, fSpdC1+fSpdC2); // PF = 2 Condition
                }
                if(ii == 10){
                    ((TH1F*)fList->FindObject(Form("hTotalTrkVsClsSPID_%s_PF10",triggername.Data())))->Fill(fSpdT, fSpdC1+fSpdC2); // PF = 10 Condition
                }
            }
        }
}