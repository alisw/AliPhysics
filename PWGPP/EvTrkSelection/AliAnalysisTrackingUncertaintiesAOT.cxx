...
void AliAnalysisTrackingUncertaintiesAOT::UserCreateOutputObjects()
{
    ...
    if(fMC) {
        const Int_t nvars = 9;
        Int_t nBins[nvars]   = {600,   62,   297,  297,   18,   nEtaBins,    3,   2,    5};
        Double_t xmin[nvars] = {-3., -3.2,  0.15, 0.15,   0.,   -fMaxEta, -0.5, -2., -0.5};
        Double_t xmax[nvars] = {3.,   3.2,   15.,  15., 6.28,    fMaxEta,  2.5,  2.,  4.5};
        TString axis[nvars]  = {"DCAxy","DCAz","track p_{T}","particle p_{T}","phi","eta","type (0=prim,1=sec,2=mat)","track label (1=lab>0,-1=lab<0)","species (0=e,1=#pi,2=k,3=p) - MC truth"};
        ...
        const Int_t nvars = 5;
        Int_t nBins[nvars]   = {600,   62,   297,    18,  nEtaBins};
        Double_t xmin[nvars] = {-3., -3.2,  0.15,    0.,  -fMaxEta};
        Double_t xmax[nvars] = {3.,   3.2,    15., 6.28,   fMaxEta};
        TString axis[nvars]  = {"DCAxy","DCAz","track p_{T}","phi","eta"};
                ...
                for(int iSpec=0; iSpec<5; iSpec++) {
                    if(fspecie&BIT(iSpec)) {
                        if(IsConsistentWithPid(iSpec, track)) {
                            Double_t vecHistTpcItsMatch[kNumberOfAxes] = {static_cast<Double_t>(isMatched), pT, eta, phi, (Double_t)iSpec, (Double_t)isph};
                            histTpcItsMatch->Fill(vecHistTpcItsMatch);
                            if(fMC){
                                Double_t vec4Sparse[9] = {dca[0],dca[1],pT,part->Pt(),phi,eta,partType,label,specie};
                ...
                if(fspecie&BIT(iSpec)) {
                    if(IsConsistentWithPid(iSpec, track)) {
                        ...
                        if(fMC){
                            ...
                            Double_t vec4Sparse[5] = {dca[0],dca[1],pT,phi,eta};
...
void AliAnalysisTrackingUncertaintiesAOT::InitializeTrackCutHistograms() {
    ...
    Int_t    binsTpcItsMatch[kNumberOfAxes] = {    2,   50,    20,            18,  6,      3};
    Double_t minTpcItsMatch[kNumberOfAxes]  = { -0.5,  0.1,    -1,             0, -0.5, -1.5};
    Double_t maxTpcItsMatch[kNumberOfAxes]  = {  1.5,   20,    +1, 2*TMath::Pi(),  5.5,  1.5};
    //
    TString axisNameTpcItsMatch[kNumberOfAxes]  = {"isMatched","pT","eta","phi","pid","primSec"};
    TString axisTitleTpcItsMatch[kNumberOfAxes] = {"isMatched","pT","eta","phi","pid","primSec"};
    //
