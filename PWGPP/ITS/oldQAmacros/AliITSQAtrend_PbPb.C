...
void MakePlot(Int_t run1=-1,Int_t run2=999999,TString ntupleFileName="TrendingITS_PbPb.root");
...
void AliITSQAtrend_PbPb(TString runListFile="lista_PbPb.txt",TString ntupleFileName="TrendingITS_PbPb.root");
...
ofstream myfile("outfile1_PbPb.txt",ios::app);

////////////////////////////////////////////////////////////////
//    THIS VERSION OPTIMIZED FOR PbPb
...
void MakePlot(Int_t run1,Int_t run2,TString ntupleFileName){
  ...
  ntsdd->SetBranchAddress("nEventsTriggeres",&nEventsTriggered);
 ...
 //-----------------------------------
  ...
  c2->cd(2);
  histomeanTime->Draw();
  histomeanTime->SetMinimum(3160);
  histomeanTime->SetMaximum(3260);
  ...
  TCanvas *cpt02=new TCanvas("cpt02","TPC-ITS matching efficiency",1200,600);
  cpt02->Divide(3,2);
  cpt02->cd(1);
...
//____________________________________________________________________________
void AliITSQAtrend_PbPb(TString runListFile,TString ntupleFileName){
...
#define MAX_LINE_LEN 255
  ...
  while ( in ) {
    ...
    else if(aux.Contains("LHC10h/")){
      lentrail = 31;
      lenfill=34;
    }    else if(aux.Contains("LHC12b/")){
      lentrail = 27;
      lenfill=30;
    }    else if(aux.Contains("LHC12d2_plus/")){
      lentrail = 29;
    }
    else if(aux.Contains("LHC12c/")  || aux.Contains("LHC12d/") ){
      lentrail = 27;
    }
    else if(aux.Contains("LHC12a17a_fix/")){
      lentrail = 30;
      lenfill=30;
    }
    else if(aux.Contains("LHC13b2_efix_p1/")){
      lentrail = 32;
      lenfill=30;
    }    
    else if(aux.Contains("LHC13b2_efix_p2/")){
      lentrail = 32;
      lenfill=30;
    }    
    else if(aux.Contains("LHC13b2_efix_p3/")){
      lentrail = 32;
      lenfill=30;
    }    
    else if(aux.Contains("LHC13b2_efix_p4/")){
      lentrail = 32;
      lenfill=30;
    }    
    else if(aux.Contains("LHC13e/")){
      lentrail = 27;
      lenfill=30;
    }
    else if(aux.Contains("LHC13g/")){
      lentrail = 27;
      lenfill=30;
    }
    else if(aux.Contains("LHC13f/")){
      lentrail = 27;
      lenfill=30;
    }
    else if(aux.Contains("LHC13d/")){
      lentrail = 27;
      lenfill=30;
    }
    else if(aux.Contains("LHC15o/")){
      lentrail = 27;
      lenfill=30;
    }
    ...
    else if(aux.Contains("LHC15g3/")){
      lentrail = 24;
      lenfill=30;
    } 

      
    else if(aux.Contains("hansena/")){
        lentrail = 31;
        lenfill=32;
    }
...
void FillMatchntuple(TFile* f,TNtuple* ntmatching, Int_t iRun, Float_t *xntMatching){
  ...
  cout<<"Tracking Matching ntuple"<<endl;
...
//  xntMatching[indexMatching++]=ioErrors[26];
  ...
  ntmatching->Fill(xntMatching);
//  cout<<"Tracking Matching ntuple -- end"<<endl;
...
void FillVTXntuple(TFile* f,TNtuple* ntvertex, Int_t iRun, Float_t *xntVertex){
   cout<<"Primary Vertex"<<endl;
    ...
    // }
     ...
     TH1F *xVtxSPD = (TH1F*)lt->FindObject("fhTRKVertexX");
    ...
    TH1F *yVtxSPD = (TH1F*)lt->FindObject("fhTRKVertexY");
    ...
    TH1F *zVtxSPD = (TH1F*)lt->FindObject("fhSPDVertexZonly");

    if(zVtxSPD->GetEntries()==0){
      printf("Run %d zVtxSPD EMPTY -> Return\n",iRun);
      myfile<<"zVtxSPDy not found"<<endl;

      return;
    }

    TF1 *fxTRK = new TF1("gausx", "gaus", -0.5, 0.5);
    xVtxTRK->Fit("gausx", "Q");

    TF1 *fyTRK = new TF1("gausy", "gaus", -0.2, 0.7);
    yVtxTRK->Fit("gausy","Q");
...
//     cout<<fyTRK->GetParError(2)<<endl;
    ...
    zVtxTRK->Fit("gausz","Q");
 TF1 *fxSPD = new TF1("gausxSPD", "gaus", -0.5, 0.5);
    xVtxSPD->Fit("gausxSPD", "Q");

    TF1 *fySPD = new TF1("gausySPD", "gaus", -0.2, 0.7);
    yVtxSPD->Fit("gausySPD","Q");
//     cout<<fyTRK->GetParameter(1)<<endl;
//     cout<<fyTRK->GetParError(1)<<endl;
//     cout<<fyTRK->GetParameter(2)<<endl;
//     cout<<fyTRK->GetParError(2)<<endl;
