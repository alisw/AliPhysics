...
void MakePlot(Int_t run1=-1,Int_t run2=999999,TString ntupleFileName="TrendingITS_pp.root");
...
void AliITSQAtrend_pp_pileup(TString runListFile="lista_pp2.txt",TString ntupleFileName="TrendingITS_pp.root");
...
ofstream myfile("outfile1_pp.txt",ios::app);
...
/*    TCanvas *cPileUp=new TCanvas("cPileUp","cPileUp",800,600);
    hpileupSPD->SetMinimum(0.0);
    hpileupSPD->SetMaximum(1.1);
    hpileupSPD->GetYaxis()->SetTitle("Fraction of events with SPD pileup");
    hpileupSPD->Draw();
...
*/
 //-----------------------------------
  ...
  c5->Update();
    ...
    TCanvas* c22=new TCanvas("c22","Number of Events",1200,800);
    histonEvents->Draw();
  ...
  TLatex* td1=new TLatex(0.2,0.85,"SDD minimum drift time (ref=495ns)");
  td1->SetNDC();
  td1->SetTextColor(1);
  td1->Draw();
  c2->cd(2);
  histomeanTime->Draw();
  histomeanTime->SetMinimum(3100);
  histomeanTime->SetMaximum(3250);
  ...
  tpc3->Draw();
    ...
    hEffTOF6Pt02->GetYaxis()->SetRangeUser(0,1);
    ...
    hEffTOF6Pt1->GetYaxis()->SetRangeUser(0,1);
    ...
    lptTOF1->Draw("same");
    TLatex* tof2=new TLatex(0.2,0.75,"TOFITS match eff Pt=1");
    tof2->SetNDC();
    tof2->SetTextColor(1);
    tof2->Draw();
    ...
    hEffTOF6Pt10->GetYaxis()->SetRangeUser(0,1);
    ...
    TLatex* tof3=new TLatex(0.2,0.75,"TOFITS match eff Pt=10");
    tof3->SetNDC();
    tof3->SetTextColor(1);
    tof3->Draw();
...
*/
 ...
 // merge the pdf files
  ...
  delete [] noRuns;
    ...
    // nameO+="_File.root";
    TFile *f=new TFile("nameO.root","RECREATE");
...
//____________________________________________________________________________
void AliITSQAtrend_pp_pileup(TString runListFile,TString ntupleFileName){
...
#define MAX_LINE_LEN 255
  ...
  for(Int_t jru=0;jru<nrun;jru++) {
    ...
    //    printf("%s\n",strings[jru]);
      ...
      myfile<<"Processing run "<<runNumb[jru]<<endl;
...
void FillVTXntuple(TFile* f,TNtuple* ntvertex, Int_t iRun, Float_t *xntVertex){
 ...
 TH1F *xVtxSPD = (TH1F*)lt->FindObject("fhSPDVertexX");
    ...
    if(zVtxSPDpil->GetEntries()==0){
        printf("Run %d zVtxSPDpil EMPTY -> Return\n",iRun);
