...
//void AliITSQAchecks_pp_pileup(TString option="local",             // local analysis version
//                              Int_t nRun=225322,
...
void AliITSQAchecks_pp_pileup(TString option="grid",                // grid analysis version
  ...
  //          the run number is taken from the path. Otherwise, in case of a8
...
//    aux=aux.Remove(9,aux.Length());  // OK per confronto con file di dati: 000244918
  ...
  if(option.Contains("local")){
    ...
    if(qaTrain.Contains("QA")){
      path=Form("/alice/data/%d/%s/%09d/ESDs/pass2/%s/",year,period.Data(),nRun,qaTrain.Data()); // non usato
    } else {
      path=Form("/alice/data/%d/%s/%09d/pass1/",year,period.Data(),nRun); // cambiare pass!!!
    }
