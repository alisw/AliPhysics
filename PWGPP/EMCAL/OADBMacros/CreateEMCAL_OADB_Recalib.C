...
/// created the OADB for misalignment matrices
///
/// In this macro, the histograms with Recalibraton Factors are loaded from pre-existing root files 
/// and some TObjarrays are filled with these histograms.
...
///
/// Let's read back the generated file
void test()
{
  ...
  TObjArray *recal = (TObjArray*) cont->GetObject(118503); //GetObject(int runnumber)
  recal->ls();
  
  TObjArray *recalpass = (TObjArray*) recal->FindObject("pass2");
...
/// Main method
///
void CreateEMCAL_OADB_Recalib()
{
  const char *fileName10s_d = "RecalDB/summer_december_2010/RecalibrationFactors.root";
  const char *fileName10d   = "RecalDB/december2010/RecalibrationFactors.root";
  const char *fileName10s   = "RecalDB/summer2010/RecalibrationFactors.root";
  const char *fileName11v1  = "RecalDB/2011_v1/RecalibrationFactors.root";
