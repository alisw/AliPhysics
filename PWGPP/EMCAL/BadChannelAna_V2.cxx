...
BadChannelAna_V2::BadChannelAna_V2():
TObject(),
	fCurrentRunNumber(-1),
	fPeriod(),
	fTrainNo(),
...
///
//________________________________________________________________________
BadChannelAna_V2::BadChannelAna_V2(TString period, TString train, TString trigger, Int_t runNumber,Int_t trial, TString workDir, TString listName):
	TObject(),
	fCurrentRunNumber(-1),
	fPeriod(),
	fTrainNo(),
...
///
//________________________________________________________________________
void BadChannelAna_V2::Run(Bool_t mergeOnly)
{
...
///
//________________________________________________________________________
TString BadChannelAna_V2::MergeRuns()
{
...
void BadChannelAna_V2::LoadExternalBadMap()
{
...
///
//_________________________________________________________________________
void BadChannelAna_V2::BCAnalysis()
{
	//. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
	//.. BAD CELLS
...
//
//________________________________________________________________________
void BadChannelAna_V2::AddMaskSM(Int_t iSM)
{
...
//________________________________________________________________________
void BadChannelAna_V2::AddPeriodAnalysis(Int_t criteria, Double_t nsigma, Double_t emin, Double_t emax)
{
	TArrayD periodArray;
	periodArray.Set(4);
...
void BadChannelAna_V2::PeriodAnalysis(Int_t criterion, Double_t nsigma, Double_t emin, Double_t emax)
{
	...
	if(criterion < 3)   histogram = BuildHitAndEnergyMean(criterion, emin, emax);
        if(criterion > 3)   histogram = BuildHitAndEnergyMean_scaled(criterion, emin, emax);
...
//		if(emin>=1.8)FlagAsBad(criterion, histogram, nsigma, -1);//..do not narrow the integration window
	...
	if(criterion==3) FlagAsBad(criterion, histogram, nsigma, -1);
}
...
///
//_________________________________________________________________________
TH1F* BadChannelAna_V2::BuildHitAndEnergyMean(Int_t crit, Double_t emin, Double_t emax)
{
...
///
//_________________________________________________________________________
TH1F* BadChannelAna_V2::BuildTimeMean(Int_t crit, Double_t tmin, Double_t tmax)
{
	...
	Double_t dHits = 0.;
        ...
        for(int iCell = 1; iCell <= fNoOfCells; iCell ++){
            ...
            for(int iTime = 1; iTime <= fCellTime->GetNbinsX(); iTime++){
                ...
                if(fCellTime->GetXaxis()->GetBinCenter(iTime) > -50 && fCellTime->GetXaxis()->GetBinCenter(iTime) < 50){
                    dIn += dHits;
                }
                    dAll += dHits;
            }
...
// ------------------------------------------------------------------------------------------
TH1F* BadChannelAna_V2::BuildHitAndEnergyMean_scaled(Int_t crit, Double_t emin, Double_t emax)
{
	...
	histogram->GetXaxis()->SetNdivisions(510);
        ...
        hHitDistrib_forScaling->Fit("fgaus_forScaling","M");
...
//         TCanvas *CanTestHits = new TCanvas("","",1200,1000);
//         CanTestHits->cd();
//         hHitDistrib_forScaling->Draw("");
//         fgaus_forScaling->Draw("same");
//         CanTestHits->SaveAs(Form("./EMCal_Checks/NewTest_%.02f_%.02f.png",emin,emax));
...
//             Int_t ColCounter[100] = {0};
//             Int_t RowCounter[250] = {0};
        
            TH2F *hmap = new TH2F("","",100,0.,100,250,0.,250.);
        
            for(int i = 1; i <= fNoOfCells; i++){
                if(fFlag[i -1 ] == 0 && fFlag_forScaling[i - 1] == 0){
                    fCaloUtils->GetModuleNumberCellIndexesAbsCaloMap(i - 1 ,0,cellCol, cellRow, trash, col, row);
                    dhits = 0.;
                    dNumOfHits = 0.;
                    for(int k = hEnergyScaled->GetXaxis()->FindBin(emin); k <= hEnergyScaled->GetXaxis()->FindBin(emax); k++){
                        if(crit==5){dhits += hEnergyScaled->GetBinContent(k, i);}
...
//             TCanvas *CanMap = new TCanvas("","",1200,1000);
//             CanMap->cd();
//             gPad->SetLogz();
//             hmap->Draw("colz");
//             CanMap->SaveAs(Form("./EMCal_Checks/Map_%.02f_%.02f_%i.png",emin,emax,iter));
...
//           CanRow = new TCanvas("","",1200,1000);
//           CanRow->cd();
// //         NiceCanvas(CanRow);
// //         NiceHisto(hEnergyRow, 43, 42, "EMCal cell row", "Hits/Cell", kBlack, 2.);
//           hEnergyRow->Draw("");
//           fFitRow->Draw("same");
//           CanRow->SaveAs(Form("./EMCal_Checks/EMC_Row_ %i.png", iter));
// 
// 
//           TCanvas *CanCol = new TCanvas("","",1200,1000);
//           CanCol->cd();
// //         NiceCanvas(CanCol);
// //         NiceHisto(hEnergyCol, 43, 42, "EMCal cell row", "Hits/Cell", kBlack, 2.);
//           hEnergyCol->Draw("");
//           fFitCol->Draw("same");
//           CanCol->SaveAs(Form("./EMCal_Checks/EMC_Col_ %i.png", iter));
      }
	...
	for (Int_t cell = fStartCell; cell < fNoOfCells; cell++)
	{
        ...
        delete hEnergyCol;
        delete hEnergyRow;	
// 	delete fCellAmp_Clone;
        
        
//         TCanvas *CanTest = new TCanvas("","",1200,1000);
//         CanTest->cd();
//         histogram->Draw("");
//         CanTest->SaveAs(Form("./EMCal_Checks/Test_%i_%.02f_%.02f.png", crit, emin,emax));
//         
//         delete CanTest;
...
///
//_________________________________________________________________________
void BadChannelAna_V2::FlagAsDead()
{
	Int_t sumOfExcl=0;
...
//_________________________________________________________________________
void BadChannelAna_V2::FlagAsBad(Int_t crit, TH1F* inhisto, Double_t nsigma, Double_t dnbins)
{  
	...
	TString histoName=inhisto->GetName();
        Double_t goodmax = 0. ;
	Double_t goodmin = 0. ;
	Double_t goodmax_wTRD = 0. ;
	Double_t goodmin_wTRD = 0. ;
        Double_t goodmax_woTRD = 0. ;
	Double_t goodmin_woTRD = 0. ;
...
// 	Double_t maxCenter = distrib->GetBinCenter(maxBin);
	...
	for(Int_t i = maxBin ; i<=dnbins ; i++)
	{
		...
		goodmax_wTRD = distrib->GetBinCenter(i);
	}
	for(Int_t i = maxBin ; i>2 ; i--)
	{
		if(distrib->GetBinContent(i)<2) break ;
		goodmin_wTRD = distrib->GetBinLowEdge(i);
	}
	//if(maxBin<2)goodmin = distrib->GetBinLowEdge(2);

	//..Define min/max range of the fit function
        Double_t   minFitRange=goodmin_wTRD;
        Double_t   maxFitRange=goodmax_wTRD;
...
// 	//cout<<"fit range:  "<<minFitRange<<" - "<<maxFitRange<<endl;
        ...
        TF1 *fFitGaus = new TF1("fFitGaus", "gaus",0.,distrib->GetBinCenter(distrib_woTRDStruc->GetNbinsX()));
	TF1 *fFitGauswTRD = new TF1("fFitGauswTRD", "gaus",0.,distrib_wTRDStruc->GetBinCenter(distrib_wTRDStruc->GetNbinsX()));
        TF1 *fFitGausWOTRD = new TF1("fFitGauswoTRD", "gaus",0.,distrib_woTRDStruc->GetBinCenter(distrib_woTRDStruc->GetNbinsX()));

        Double_t dvalSigma, dvalMean, dvalSigma_woTRD, dvalMean_woTRD, dvalSigma_wTRD, dvalMean_wTRD;
//         if( crit == 1 || crit == 2 || crit == 4 || crit == 5){
            FitDistrib(distrib, fFitGaus, &dvalMean, &dvalSigma);
            FitDistrib(distrib_wTRDStruc, fFitGauswTRD, &dvalMean_wTRD, &dvalSigma_wTRD);
            FitDistrib(distrib_woTRDStruc, fFitGausWOTRD, &dvalMean_woTRD, &dvalSigma_woTRD);
//         }
        


	//..for case 1 and 2 lower than 0 is an unphysical value
	if(crit<3 && dvalMean_wTRD <0.) dvalMean_wTRD=0.;
        if(crit<3 && dvalMean_woTRD <0.) dvalMean_woTRD=0.;
        
        goodmin = dvalMean - nsigma*dvalSigma ;
        goodmax = dvalMean +  nsigma*dvalSigma ;
	goodmin_wTRD = dvalMean_wTRD - nsigma*dvalSigma_wTRD ;
	goodmax_wTRD = dvalMean_wTRD + nsigma*dvalSigma_wTRD ;
        goodmin_woTRD = dvalMean_woTRD - nsigma*dvalSigma_woTRD ;
	goodmax_woTRD = dvalMean_woTRD + nsigma*dvalSigma_woTRD ;
	//..for case 1 and 2 lower than 0 is an unphysical value
	if(crit<3 && goodmin_wTRD <0.) goodmin_wTRD=0.;
        if(crit<3 && goodmin_woTRD <0.) goodmin_woTRD=0.;
	...
	if(inputBins==-1)         goodmin_wTRD=-1;
        fPrint = 1;
	if(fPrint==1)cout<<"    o Result of fit: "<<endl;
	if(fPrint==1)cout<<"    o  "<<endl;
	if(fPrint==1)cout<<"    o Mean: "<<dvalMean_wTRD <<" sigma: "<<dvalSigma_wTRD<<endl;
	if(fPrint==1)cout<<"    o good range : "<<goodmin <<" - "<<goodmax<<endl;
        
        
        
        ///..................................................................
        ///.......... Convert inhisto so that cells behind the TRD are scaled 
        ///.......... to the "normal" cells
        ///..................................................................
//         TH1F *ScaledHistTRD = (TH1F*) inhisto->Clone("ScaledHistTRD"); 
//         for(Int_t cell = fStartCell; cell < fNoOfCells; cell++){
//             fCaloUtils->GetModuleNumberCellIndexesAbsCaloMap(cell,0,cellColumn,cellRow,trash,cellColumnAbs,cellRowAbs);
//             if(IsCoveredByTRD(cellRowAbs,cellColumnAbs)==1){
//                 ScaledHistTRD->SetBinContent(cell + 1, ScaledHistTRD->GetBinContent( cell + 1) *  (dvalMean_woTRD/dvalMean_wTRD));
//             }
//             
//         }
...
// 	ScaledHistTRD->SetTitleOffset(0.6,"Y");
// 	ScaledHistTRD->GetXaxis()->SetRangeUser(0,fNoOfCells+1);
// 	ScaledHistTRD->GetYaxis()->SetTitleOffset(0.7);
//         inhisto->SetMarkerStyle(20);
//         inhisto->SetMarkerSize(1.);
// 	ScaledHistTRD->SetLineColor(kBlue+1);
// 	ScaledHistTRD->Draw("p");
        inhisto->SetMarkerColor(kRed);
        inhisto->Draw("");
	...
	distrib->GetYaxis()->SetTitleOffset(1.3);
        distrib->GetXaxis()->SetRangeUser(dvalMean - dvalSigma*10., dvalMean + dvalSigma*10. );
	distrib->Draw();
	...
	fFitGauswTRD->SetLineColor(kOrange-3);
	fFitGauswTRD->SetLineStyle(1);//7
	fFitGauswTRD->Draw("same");
        
        fFitGausWOTRD->SetLineColor(kMagenta);
        fFitGausWOTRD->SetLineStyle(1);
        fFitGausWOTRD->Draw("same");
        
        fFitGaus->SetLineColor(kBlue);
        fFitGaus->SetLineStyle(1);
        fFitGaus->Draw("same");
	...
	if(crit==1 || crit == 4)name=Form("%s/%s/AverageEperHit_%s.gif",fWorkdir.Data(), fAnalysisOutput.Data(), (const char*)histoName);
	if(crit==2 || crit == 5)name=Form("%s/%s/AverageHitperEvent_%s.gif",fWorkdir.Data(), fAnalysisOutput.Data(), (const char*)histoName);
	...
	for(Int_t cell = fStartCell; cell < fNoOfCells; cell++)
	{
		...
		//.. <= throws out zeros, might not be a dead cell but still have zero entries in a given energy range
                fCaloUtils->GetModuleNumberCellIndexesAbsCaloMap(cell,0,cellColumn,cellRow,trash,cellColumnAbs,cellRowAbs);
                    if((inhisto->GetBinContent(cell+1) <= goodmin && fFlag[cell]==0) || (inhisto->GetBinContent(cell+1) > goodmax && fFlag[cell]==0) )
                    {
...
/// This function is fitting the hit or the energy/hit distribution and determines the value of sigma
void BadChannelAna_V2::FitDistrib(TH1F* hdistrib, TF1* fFitGaus,  Double_t *dValMean, Double_t *dValSigma)
{ 
        Double_t maxDistr  = hdistrib->GetMaximum();
	Int_t    maxBin    = hdistrib->GetMaximumBin();
	Double_t maxCenter = hdistrib->GetBinCenter(maxBin);
        
	fFitGaus->SetParLimits(0,0,1.2*maxDistr); //..do not allow negative ampl values
	fFitGaus->SetParameter(1,maxCenter);  //..set the start value for the mean
        
        hdistrib->Fit(fFitGaus, "0LQEM");
        hdistrib->Fit(fFitGaus, "0LQEM");
        hdistrib->Fit(fFitGaus, "0LQEM");
        for(int i = 1; i < 5; i++){
            *dValMean    = fFitGaus->GetParameter(1);
            *dValSigma     = fFitGaus->GetParameter(2);
            hdistrib->Fit(fFitGaus, "0LQEM","",*dValMean - *dValSigma*3 , *dValMean + *dValSigma*3 );
        }
...
///
//________________________________________________________________________
void BadChannelAna_V2::SummarizeResultsByFlag()
{
	//. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
	//.. RESULTS
...
///
//________________________________________________________________________
void BadChannelAna_V2::SummarizeResults()
{
...
///
//________________________________________________________________________
void BadChannelAna_V2::SaveBadCellsToPDF(Int_t version, TString pdfName)
{
...
////
//_________________________________________________________________________
TH1D* BadChannelAna_V2::BuildMeanFromGood(Int_t warmIn)
{
	TH1D* hGoodAmp;
...
///
//_________________________________________________________________________
Bool_t BadChannelAna_V2::CheckDistribution(TH1* histogram, TH1* reference)
{
...
///
//_________________________________________________________________________
Bool_t BadChannelAna_V2::IsCoveredByTRD(Int_t row, Int_t collumn)
{
...
///
//_________________________________________________________________________
void BadChannelAna_V2::PlotFlaggedCells2D(Int_t flagBegin,Int_t flagEnd)
{
	//..build two dimensional histogram with values row vs. column
	TString histoName;
...
///
//_________________________________________________________________________
void BadChannelAna_V2::SaveHistoToFile()
{
	char name[100];
	for(Int_t cell=fStartCell;cell<fNoOfCells;cell++)
	{
