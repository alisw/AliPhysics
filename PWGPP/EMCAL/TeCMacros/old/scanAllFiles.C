...
void scanAllFiles(const char *listfile = "filelist.txt")
{
  ...
  while ( fin.good() ) {
   ...
   //   cout << " filename fn " << fn << " runno " << runno << endl;
	...
	if (nOk > 0) {
	  ...
	  if (debug>0) {
	    printf("iSM %d average min %3.2f max %3.2f (max+min)/2 %3.2f median %3.2f\n",
		   iSM, aveMin, aveMax, (aveMin + aveMax)/2., median);
	    }
	}

	tree->Fill();
        Printf("%d   %d   %f", runno, iSM, aveMin);
      } // SM loop
