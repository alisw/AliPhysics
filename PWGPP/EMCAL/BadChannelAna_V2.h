#ifndef BadChannelAna_V2_H
#define BadChannelAna_V2_H

/// \class BadChannelAna_V2
...
class BadChannelAna_V2 : public TObject {
	
public:
      BadChannelAna_V2() ;                // default ctor
	  virtual ~BadChannelAna_V2()  { ; }  // virtual dtor
	  BadChannelAna_V2(TString period, TString train, TString trigger, Int_t runNumber,Int_t trial, TString workDir, TString listName);
...
protected:
	  ...
	  TH1F* BuildHitAndEnergyMean(Int_t crit, Double_t emin = 0.1, Double_t emax=2.);
          TH1F* BuildHitAndEnergyMean_scaled(Int_t crit, Double_t emin = 0.1, Double_t emax=2.);
	  ...
	  void FlagAsBad(Int_t crit, TH1F* inhisto, Double_t nsigma = 4., Double_t dnbins = 200);
          void FitDistrib(TH1F* hdistrib, TF1* fFitGaus, Double_t *dValMean, Double_t *dValSigma);
...
private:
	  BadChannelAna_V2           (const BadChannelAna_V2&); // not implemented
	  BadChannelAna_V2 &operator=(const BadChannelAna_V2&); // not implemented
