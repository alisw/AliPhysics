...
void MakeFit(TString options) {
...
#if 1
  f.SetVar( 0, 4e-3, 0.0001, 0.001,  0.02);
  f.SetVar( 1, 4e-3, 0.0001, 0.001,  0.02);
  ...
  f.SetVar( 9, 4.0e-3, 0.0001, 0.001,  0.02);
  f.SetVar(10, 3.0e-3, 0.0001, 0.001,  0.02);
  ...
  TFile *fr = TFile::Open("root/5533/rates_5533.root");
  ...
  f.DoFit(TString::Format("root/%d/fit_%s.root", 5533, options.Data()),
          options);
}
  ...
  for (Int_t i=0; i<3; ++i) {
    for (Int_t j=0; j<3; ++j) {
      ...
      ofs << line.Data() << std::endl;
       c1->cd(1+3*i+j);
      TFile::Open(Form("pdf/5533/fit_%s%s.pdf_canvas.root", scans[i], var[j]));
