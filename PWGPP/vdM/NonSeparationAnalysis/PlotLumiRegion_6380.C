...
const char* scans[] = {
  ...
  "root/6380/lumiRegion_ScanLSC.root"
};
...
void PlotLumiRegion(Int_t bc=-1)
{
  ...
  for (Int_t i=0; i<5; ++i) {
    ...
    if (i == 4) {
      ...
      DrawFrame(pad->cd(1),  0.00, 0.14,  "<X>",  "", 0, xMin, xMax, label, xTitle); gX->Draw("P"); geX->Draw("PE");
      geX->Fit("pol1", "", "", 0.5, 6.5);
      gPad->Update();
      TPaveStats *ps = (TPaveStats*)geX->FindObject("stats");
      ps->SetX1NDC(0.35); ps->SetX2NDC(0.9);
      ps->SetY1NDC(0.2); ps->SetY2NDC(0.45);

      DrawFrame(pad->cd(2),  0.28, 0.42,  "<Y>",  "", 0, xMin, xMax, label, xTitle); gY->Draw("P"); geY->Draw("PE");
      geY->Fit("pol1", "", "", 7.5, 13.5);
      gPad->Update();
      TPaveStats *ps = (TPaveStats*)geY->FindObject("stats");
      ps->SetX1NDC(0.2); ps->SetX2NDC(0.7);
      ps->SetY1NDC(0.2); ps->SetY2NDC(0.45);

      DrawFrame(pad->cd(3),  0.3,   2.2,     "<Z>", "", 0, xMin, xMax, label, xTitle); gZ->Draw("P"); geZ->Draw("PE");
    } else {
      DrawFrame(pad->cd(1),  0.068, 0.082,  "<X>",  "", 0, xMin, xMax, label); gX->Draw("P"); geX->Draw("PE");
      DrawFrame(pad->cd(2),  0.350, 0.359,  "<Y>",  "", 0, xMin, xMax, label); gY->Draw("P"); geY->Draw("PE");
