...
{
  AliVdMMetaData d(AliVdMMetaData::GetFileName("4269/4269.xml"));
  ...
  const TString pn          = "pdf/4269/LumiRegionBkgd_4269.pdf";
  ...
  std::for_each(d.GetScansBegin(), d.GetScansEnd(), [&](const AliXMLEngine::Node& n) {
      PlotBkgd(c1, 4269, TE, n, vtxCuts);
      c1->SaveAs(pn);
      PlotBkgd(c1, 4269, TE, n, vtxCuts*bkgdCuts);
      c1->SaveAs(pn);
    });
  c1->SaveAs(pn+"]");
