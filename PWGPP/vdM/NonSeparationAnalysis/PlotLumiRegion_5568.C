...
const char* scans[] = {
  ...
  "root/5568/lumiRegion_ScanLSC.root",
};
...
void PlotLumiRegion()
{
  ...
  TString pn     = "pdf/5568/LumiRegion_5568.pdf";
  ...
  tt->SetTextSize(0.04);
  tt->SetTextAlign(22);
  TString label = "ALICE Pb-p #sqrt{#it{s}_{NN}}=8.2 TeV";
  for (Int_t i=0; i<5; ++i) {
    c1->Clear();
    c1->cd();
    ...
    if (i == 4) {
      ...
      DrawFrame(pad->cd(1),  0.074, 0.091,  "<X>",  "", 0, xMin, xMax, label, xTitle); gX->Draw("P"); geX->Draw("PE");
      geX->Fit("pol1", "", "", 0, 4);
      gPad->Update();
      ...
      DrawFrame(pad->cd(2),  0.349, 0.369,  "<Y>",  "", 0, xMin, xMax, label, xTitle); gY->Draw("P"); geY->Draw("PE");
      geY->Fit("pol1", "", "", 5, 9);
      gPad->Update();
      ...
      DrawFrame(pad->cd(3),  0.5,   3.0,     "<Z>", "", 0, xMin, xMax, label, xTitle); gZ->Draw("P"); geZ->Draw("PE");
    } else {
      DrawFrame(pad->cd(1),  0.078, 0.086,  "<X>",  "", 0, xMin, xMax, label, xTitle); gX->Draw("P"); geX->Draw("PE");
      DrawFrame(pad->cd(2),  0.352, 0.366,  "<Y>",  "", 0, xMin, xMax, label, xTitle); gY->Draw("P"); geY->Draw("PE");
      DrawFrame(pad->cd(3), -1.5,   5.5,     "<Z>", "", 0, xMin, xMax, label, xTitle); gZ->Draw("P"); geZ->Draw("PE");
    }
