...
void MakeFit(TString options) {
  ...
  TFile *fr = TFile::Open("root/6380/rates_6380.root");
  ...
  f.DoFit(TString::Format("root/%d/fit_%s.root", 6380, options.Data()),
          options);
}

void MakeNonseparationFit_6380() {
  ...
  for (Int_t i=0; i<3; ++i) {
    for (Int_t j=0; j<1; ++j) {
      MakePlots(Form("root/6380/fit_%s%s.root", scans[i], var[j]),
                "ALICE p-p #sqrt{#it{s}_{NN}}=5 TeV",
                0.75,
                !kTRUE);
      TString line = ExtractFromCanvas(Form("pdf/6380/fit_%s%s.pdf_canvas.root", scans[i], var[j]));
      std::ofstream ofs(Form("pdf/6380/fit_%s%s_XZ.pdf_canvas.txt", scans[i], var[j]));
      ofs << line.Data() << std::endl;
       c1->cd(1+3*i+j);
      TFile::Open(Form("pdf/6380/fit_%s%s.pdf_canvas.root", scans[i], var[j]));
