...
void AliAnalysisTaskCheckPileup::UserCreateOutputObjects()
{
  ...
  fTrackNtuple = new TNtuple("trackNtuple","Buffer of tracks vs. time",
			     "timeStamp:nTracksTPC:nTracksITSTPC:nTracklets");

  PostData(1, fOutputPrimV);
  PostData(2, fOutputSPDPil);
  PostData(3, fOutputMVPil);
  PostData(4, fCounterPerRun);
  PostData(5, fTrackNtuple);
...
void AliAnalysisTaskCheckPileup::UserExec(Option_t *) 
{
  ...
  fHistoTPCTracksVsTracklets->Fill(ntrklets,nTPCtracks);
  fHistoGloTracksVsTracklets->Fill(ntrklets,nITSTPCtracks);
  if(fFillNtuple){
    Float_t xnt[4];
    xnt[0]=esd->GetTimeStamp();
    xnt[1]=nTPCtracks;
    xnt[2]=nITSTPCtracks;
    xnt[3]=ntrklets;
    fTrackNtuple->Fill(xnt);
  }
  ...
  PostData(3, fOutputMVPil);
  PostData(4, fCounterPerRun);
  if(fFillNtuple) PostData(5, fTrackNtuple);
