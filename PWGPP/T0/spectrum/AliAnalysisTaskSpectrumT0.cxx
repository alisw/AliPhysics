...
void AliAnalysisTaskSpectrumT0::UserCreateOutputObjects()	{
  ...
  this->AddEventID("1");
  this->AddEventIDstat("1011010010");
  this->AddEventIDstat("10011010010");
  this->AddEventIDstat("1");
  ...
  */
  ...
  //Preparing output for stat hists
  TList *listStat = new TList();
  listStat->SetOwner(kTRUE);
  listStat->SetName("eventID_trend");
  mOutputList->Add(listStat);
  for(const auto &entry: mSetEventIDstat) {
    TH1F *hist = new TH1F("hTrendStat_"+GetNameEventID(entry),"hTrendStat_"+GetNameEventID(entry)+";run",mSetRunNums.size(),0,mSetRunNums.size());
    int binIndex=1;
    for(const auto &entryRun: mSetRunNums) {
      hist->GetXaxis()->SetBinLabel(binIndex,Form("%i",entryRun));
      binIndex++;
    }
    listStat->Add(hist);
    mMapEventID2Stat.insert({entry,hist});
  }
...
const std::vector<AliAnalysisTaskSpectrumT0::FunctorStruct> AliAnalysisTaskSpectrumT0::sVecFunctors ={
  ...
  {"PS","Physics selection",[](const EventStructT0& data) {
     return data.fIsPhysSel;
   }},

  {"PU","Pileup rejection",[](const EventStructT0& data) {
