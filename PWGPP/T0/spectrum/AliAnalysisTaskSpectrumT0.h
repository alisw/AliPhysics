...
class AliAnalysisTaskSpectrumT0 : public AliAnalysisTaskSE {
  public:
    ...
    typedef std::map<EventID_t,TH1F*,Comparer> MapEventID2Hist_t;
    typedef std::function<bool(const EventStructT0&)> Functor_t;
    ...
    TString GetNameEventID(const EventID_t& eventID) const {
    ...
    void FillHists(const AliESDEvent* esdData) {
      ...
      for(auto &entry: mMapEventID2Stat) {
        auto &eventID = entry.first;
    ...
    bool AddEventIDstat(ULong64_t eventID) {return AddEventIDstat(EventID_t{eventID});}
    bool AddEventIDstat(const std::string &eventID) {return AddEventIDstat(EventID_t{eventID});}
    bool AddEventIDstat(const EventID_t& eventID) {
      auto pairInserted = mSetEventIDstat.insert(eventID);
      return pairInserted.second;
    }
  ...
  private:
    ...
    SetEventID_t mSetEventIDstat; // Set of registered EventIDs for eventID stats
