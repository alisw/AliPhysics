...
ClassImp(AliEventPlaneVarManager)

const Char_t* AliEventPlaneVarManager::fTrackingFlagNames[AliEventPlaneVarManager::kNTrackingFlags] = {
...
const Char_t* AliEventPlaneVarManager::fOfflineTriggerNames[64] = {
...
////
////for(Int_t iflag=0; iflag<AliEventPlaneVarManager::kNTrackingFlags;iflag++){
////  AliEventPlaneVarManager::fTrackingFlagNames[iflag]   = flags[iflag];
////  };
////
////  
//const  TString AliEventPlaneVarManager::fOfflineTriggerNames[64] = {
//    "MB",              "INT7",              "MUON", "HighMult",    "EMC1", "CINT5",       "CMUS5/MUSPB",      "MUSH7/MUSHPB",
//    "MUL7/MuonLikePB", "MUU7/MuonUnlikePB", "EMC7", "MUS7",        "PHI1", "PHI7/PHOSPb", "EMCEJE",           "EMCEGA",
//    "Central",         "SemiCentral",       "DG5",  "ZED",         "SPI7", "CINT8",       "MuonSingleLowPt8", "MuonSingleHighPt8",  
//    "MuonLikeLowPt8",  "MuonUnlikeLowPt8",  "N/A",  "UserDefined", "N/A",  "N/A",         "FastOnly",         "N/A",
...
////  AliEventPlaneVarManager::fTrackingFlagNames[iflag]   = triggernames[iflag];
////  };
//  
//  //static const TString fTrackingFlagNames[AliEventPlaneVarManager::kNTrackingFlags] = {0x0};
//  //static const TString fOfflineTriggerNames[64] = {0x0};
  const Char_t* AliEventPlaneVarManager::fVariableNames[AliEventPlaneVarManager::kNVars][2] = {{""}};
  Bool_t AliEventPlaneVarManager::fUsedVars[AliEventPlaneVarManager::kNVars] = {kFALSE};
  Bool_t AliEventPlaneVarManager::fUseDefaultVariablesName = kFALSE;

//for(Int_t ivar=0; ivar<AliEventPlaneVarManager::kNVars; ivar++){AliEventPlaneVarManager::fVariableNames[ivar][0]="";AliEventPlaneVarManager::fVariableNames[ivar][1]=""; AliEventPlaneVarManager::fUsedVars[ivar]=kFALSE;}


//__________________________________________________________________
void AliEventPlaneVarManager::SetDefaultVarNames() {
  ...
  for(Int_t ich=0;ich<64;++ich) {
    ...
    fVariableNames[kVZEROflowV2TPC+ich][0] = Form("v_{2}^{VZERO-ch%d}{EP,TPC}", ich);
  }  
  ...
  for(Int_t iHarmonic=0;iHarmonic<6;++iHarmonic) {
    for(Int_t iSide=0;iSide<3;++iSide) {
      fVariableNames[kVZEROQvecX+iSide*6+iHarmonic][0] = Form("Q_{x,%d}^{VZERO-%s}",iHarmonic+1,vzeroSideNames[iSide].Data());
      fVariableNames[kVZEROQvecX+iSide*6+iHarmonic][1] = "";
      fVariableNames[kVZEROQvecY+iSide*6+iHarmonic][0] = Form("Q_{y,%d}^{VZERO-%s}",iHarmonic+1,vzeroSideNames[iSide].Data());
      fVariableNames[kVZEROQvecY+iSide*6+iHarmonic][1] = "";
      fVariableNames[kVZEROQvecMag+iSide*6+iHarmonic][0] = Form("|Q_{%d}^{VZERO-%s}|",iHarmonic+1,vzeroSideNames[iSide].Data());
      fVariableNames[kVZEROQvecMag+iSide*6+iHarmonic][1] = "";
      fVariableNames[kVZERORP+iSide*6+iHarmonic][0] = Form("#Psi_{%d}^{VZERO-%s}",iHarmonic+1,vzeroSideNames[iSide].Data());
      fVariableNames[kVZERORP+iSide*6+iHarmonic][1] = "rad.";
      fVariableNames[kTrackVZERORPdeltaPhi+iSide*6+iHarmonic][0] = Form("(#varphi-#Psi_{%d}^{VZERO-%s})/%d",
									iHarmonic+1,vzeroSideNames[iSide].Data(),iHarmonic+1);
      fVariableNames[kTrackVZERORPdeltaPhi+iSide*6+iHarmonic][1] = "rad.";
      fVariableNames[kTrackVZEROFlowVn+iSide*6+iHarmonic][0] = Form("v_{%d}{EP,VZERO-%s}",iHarmonic+1,vzeroSideNames[iSide].Data());
      fVariableNames[kTrackVZEROFlowVn+iSide*6+iHarmonic][1] = "";
    }
    fVariableNames[kVZERORPres+iHarmonic][0] = Form("#sqrt{%d(#Psi_{%d}^{VZERO-A}-#Psi_{%d}^{VZERO-C})}",
						    iHarmonic+1,iHarmonic+1,iHarmonic+1);
    fVariableNames[kVZERORPres+iHarmonic][1] = "rad.^{1/2}";
    fVariableNames[kVZEROXaXc+iHarmonic][0] = Form("Q_{x,%d}^{VZERO-A} #times Q_{x,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kVZEROXaXc+iHarmonic][1] = "";
    fVariableNames[kVZEROXaYa+iHarmonic][0] = Form("Q_{x,%d}^{VZERO-A} #times Q_{y,%d}^{VZERO-A}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kVZEROXaYa+iHarmonic][1] = "";
    fVariableNames[kVZEROXaYc+iHarmonic][0] = Form("Q_{x,%d}^{VZERO-A} #times Q_{y,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kVZEROXaYc+iHarmonic][1] = "";
    fVariableNames[kVZEROYaXc+iHarmonic][0] = Form("Q_{y,%d}^{VZERO-A} #times Q_{x,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kVZEROYaXc+iHarmonic][1] = "";
    fVariableNames[kVZEROYaYc+iHarmonic][0] = Form("Q_{y,%d}^{VZERO-A} #times Q_{y,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kVZEROYaYc+iHarmonic][1] = "";
    fVariableNames[kVZEROXcYc+iHarmonic][0] = Form("Q_{x,%d}^{VZERO-C} #times Q_{y,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kVZEROXcYc+iHarmonic][1] = "";
    fVariableNames[kVZEROdeltaRPac+iHarmonic][0]  = Form("#Psi_{%d}^{VZERO-A} - #Psi_{%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kVZEROdeltaRPac+iHarmonic][1]  = "rad.";
    fVariableNames[kVZEROdeltaRPa+iHarmonic][0]   = ""; fVariableNames[kVZEROdeltaRPa+iHarmonic][1] = "";
    fVariableNames[kVZEROdeltaRPa+6+iHarmonic][0] = ""; fVariableNames[kVZEROdeltaRPa+6+iHarmonic][1] = "";
    fVariableNames[kTPCQvecX+iHarmonic][0] = Form("Q_{x,%d}^{TPC}", iHarmonic+1);
    fVariableNames[kTPCQvecX+iHarmonic][1] = "";
    fVariableNames[kTPCQvecY+iHarmonic][0] = Form("Q_{y,%d}^{TPC}", iHarmonic+1);
    fVariableNames[kTPCQvecY+iHarmonic][1] = "";
    fVariableNames[kTPCRP+iHarmonic][0]    = Form("#Psi_{%d}^{TPC}", iHarmonic+1);
    fVariableNames[kTPCRP+iHarmonic][1]    = "rad.";
    fVariableNames[kTPCRPres+iHarmonic][0] = Form("#sqrt{%d(#Psi_{%d}^{TPC}-#Psi_{%d}^{VZERO-A})}", 
						  iHarmonic+1, iHarmonic+1, iHarmonic+1);
    fVariableNames[kTPCRPres+iHarmonic][1] = "rad^{1/2}";
    fVariableNames[kTPCRPres+6+iHarmonic][0] = Form("#sqrt{%d(#Psi_{%d}^{TPC}-#Psi_{%d}^{VZERO-C})}", 
						  iHarmonic+1, iHarmonic+1, iHarmonic+1);
    fVariableNames[kTPCRPres+6+iHarmonic][1] = "rad^{1/2}";
    fVariableNames[kRPXtpcXvzeroa+iHarmonic][0] = Form("Q_{x,%d}^{TPC} #times Q_{x,%d}^{VZERO-A}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPXtpcXvzeroa+iHarmonic][1] = "";
    fVariableNames[kRPXtpcXvzeroc+iHarmonic][0] = Form("Q_{x,%d}^{TPC} #times Q_{x,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPXtpcXvzeroc+iHarmonic][1] = "";
    fVariableNames[kRPYtpcYvzeroa+iHarmonic][0] = Form("Q_{y,%d}^{TPC} #times Q_{y,%d}^{VZERO-A}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPYtpcYvzeroa+iHarmonic][1] = "";
    fVariableNames[kRPYtpcYvzeroc+iHarmonic][0] = Form("Q_{y,%d}^{TPC} #times Q_{y,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPYtpcYvzeroc+iHarmonic][1] = "";
    fVariableNames[kRPXtpcYvzeroa+iHarmonic][0] = Form("Q_{x,%d}^{TPC} #times Q_{y,%d}^{VZERO-A}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPXtpcYvzeroa+iHarmonic][1] = "";
    fVariableNames[kRPXtpcYvzeroc+iHarmonic][0] = Form("Q_{x,%d}^{TPC} #times Q_{y,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPXtpcYvzeroc+iHarmonic][1] = "";
    fVariableNames[kRPYtpcXvzeroa+iHarmonic][0] = Form("Q_{y,%d}^{TPC} #times Q_{x,%d}^{VZERO-A}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPYtpcXvzeroa+iHarmonic][1] = "";
    fVariableNames[kRPYtpcXvzeroc+iHarmonic][0] = Form("Q_{y,%d}^{TPC} #times Q_{x,%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPYtpcXvzeroc+iHarmonic][1] = "";
    fVariableNames[kRPdeltaVZEROAtpc+iHarmonic][0] = Form("#Psi_{%d}^{TPC} - #Psi_{%d}^{VZERO-A}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPdeltaVZEROAtpc+iHarmonic][1] = "rad.";
    fVariableNames[kRPdeltaVZEROCtpc+iHarmonic][0] = Form("#Psi_{%d}^{TPC} - #Psi_{%d}^{VZERO-C}", iHarmonic+1, iHarmonic+1);
    fVariableNames[kRPdeltaVZEROCtpc+iHarmonic][1] = "rad.";
    fVariableNames[kTPCQvecXleft+iHarmonic][0] = Form("Q_{x,%d}^{TPC,left}", iHarmonic+1);
    fVariableNames[kTPCQvecXleft+iHarmonic][1] = "";
    fVariableNames[kTPCQvecYleft+iHarmonic][0] = Form("Q_{y,%d}^{TPC,left}", iHarmonic+1);
    fVariableNames[kTPCQvecYleft+iHarmonic][1] = "";
    fVariableNames[kTPCRPleft+iHarmonic][0] = Form("#Psi_{%d}^{TPC,left}", iHarmonic+1);
    fVariableNames[kTPCRPleft+iHarmonic][1] = "rad.";
    fVariableNames[kTPCQvecXright+iHarmonic][0] = Form("Q_{x,%d}^{TPC,right}", iHarmonic+1);
    fVariableNames[kTPCQvecXright+iHarmonic][1] = "";
    fVariableNames[kTPCQvecYright+iHarmonic][0] = Form("Q_{y,%d}^{TPC,right}", iHarmonic+1);
    fVariableNames[kTPCQvecYright+iHarmonic][1] = "";
    fVariableNames[kTPCRPright+iHarmonic][0] = Form("#Psi_{%d}^{TPC,right}", iHarmonic+1);
    fVariableNames[kTPCRPright+iHarmonic][1] = "rad.";
    fVariableNames[kTPCQvecXtotal+iHarmonic][0] = Form("Q_{x,%d}^{TPC,total}", iHarmonic+1);
    fVariableNames[kTPCQvecXtotal+iHarmonic][1] = "";
    fVariableNames[kTPCQvecYtotal+iHarmonic][0] = Form("Q_{y,%d}^{TPC,total}", iHarmonic+1);
    fVariableNames[kTPCQvecYtotal+iHarmonic][1] = "";
    fVariableNames[kTPCRPtotal+iHarmonic][0] = Form("#Psi_{%d}^{TPC,total}", iHarmonic+1);
    fVariableNames[kTPCRPtotal+iHarmonic][1] = "rad.";
    fVariableNames[kTPCsubResCos+iHarmonic][0] = Form("cos(%d(#Psi_{%d}^{TPC,left}-#Psi_{%d}^{TPC,right}))", 
						      iHarmonic+1, iHarmonic+1, iHarmonic+1);
    fVariableNames[kTPCsubResCos+iHarmonic][1] = "";

    fVariableNames[kDetADetBRPres+iHarmonic][0] = Form("<cos(%d(#Psi_{%d}^{a}-#Psi_{%d}^{b})>",iHarmonic+1,iHarmonic+1,iHarmonic+1); fVariableNames[kDetADetBRPsin+iHarmonic][1] = "";
    fVariableNames[kDetADetBRPsin+iHarmonic][0] = Form("<sin(%d(#Psi_{%d}^{a}-#Psi_{%d}^{b})>",iHarmonic+1,iHarmonic+1,iHarmonic+1); fVariableNames[kDetADetBRPsin+iHarmonic][1] = "";
    fVariableNames[kDetAQxDetBQx+iHarmonic][0] = Form("<X_{%d}^{a}*X_{%d}^{b}>",iHarmonic+1,iHarmonic+1); fVariableNames[kDetAQxDetBQx+iHarmonic][1] = "";
    fVariableNames[kDetAQxDetBQy+iHarmonic][0] = Form("<X_{%d}^{a}*Y_{%d}^{b}>",iHarmonic+1,iHarmonic+1); fVariableNames[kDetAQxDetBQy+iHarmonic][1] = "";
    fVariableNames[kDetAQyDetBQx+iHarmonic][0] = Form("<Y_{%d}^{a}*X_{%d}^{b}>",iHarmonic+1,iHarmonic+1); fVariableNames[kDetAQyDetBQx+iHarmonic][1] = "";
    fVariableNames[kDetAQyDetBQy+iHarmonic][0] = Form("<Y_{%d}^{a}*Y_{%d}^{b}>",iHarmonic+1,iHarmonic+1); fVariableNames[kDetAQyDetBQy+iHarmonic][1] = "";
    fVariableNames[kDetAQyDetBQy+iHarmonic][0] = Form("<Y_{%d}^{a}*Y_{%d}^{b}>",iHarmonic+1,iHarmonic+1); fVariableNames[kDetAQyDetBQy+iHarmonic][1] = "";

    fVariableNames[kDetADetBRPcosplus+iHarmonic][0] = Form("cos(%d(#Psi_{%d}^{a}+#Psi_{%d}^{b}))",iHarmonic+1,iHarmonic+1,iHarmonic+1); fVariableNames[kDetADetBRPcosplus+iHarmonic][1] = "";
    fVariableNames[kDetADetBRPsinplus+iHarmonic][0] = Form("sin(%d(#Psi_{%d}^{a}+#Psi_{%d}^{b}))",iHarmonic+1,iHarmonic+1,iHarmonic+1); fVariableNames[kDetADetBRPsinplus+iHarmonic][1] = "";
...
//__________________________________________________________________
AliEventPlaneVarManager::AliEventPlaneVarManager():
  TNamed("AliEventPlaneVarManager","AliEventPlaneVarManager")
{
  //
  // Default constructor
  //
...
//__________________________________________________________________
void AliEventPlaneVarManager::UnsetDefaultVarNames() {
  //
...
//__________________________________________________________________
void AliEventPlaneVarManager::UnsetUsedVars() {
  //
  // Unset default names for variables
  //
  for(Int_t ivar=0;ivar<kNVars;++ivar) { 
...
Double_t AliEventPlaneVarManager::DeltaPhi(Double_t phi1, Double_t phi2) {
  //
...
//void AliEventPlaneVarManager::GetThetaPhiCM(TRACK* leg1, TRACK* leg2,
...
//
////__________________________________________________________________
//void AliEventPlaneVarManager::PrintTrackFlags(TRACK* track) {
////  //
...
//__________________________________________________________________
void AliEventPlaneVarManager::PrintBits(ULong_t mask) {
  //
