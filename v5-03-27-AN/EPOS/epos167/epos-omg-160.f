...
c----------------------------------------------------------------------
     ...
     *,btildgp(idxD0:idxD1,kollmx),btildgpp(idxD0:idxD1,kollmx)
     ...
     &,cfaltar,cfbpap,cfbpam,cfbppap,cfbppam,gamv,eps
      ...
      call utpri('GfunParK',ish,ishini,5)
...
! calculate collision number according to Glauber -----------------------
      ...
      bglaub2=max(1.,sigine/10./pi)        !10= fm^2 -> mb
      bglaub=sqrt(bglaub2)
      nglevt=0   
      do ko=1,koll  
        r=bk(ko)
...
! Z_parton_projectile (zparpro) and Z_parton_target (zpartar)-----------
      ...
      if(iscreen.eq.1.or.isplit.eq.1)then
        ...
        bcut=0. !zbcut
ctp060829        bzero=bglaub 
        b2x=epscrp*epscrp*bglaub2  
        alpfom=alpfomi*epscrw*fscra(engy/egyscr)
        ztav=0.
        zpav=0.
        ...
        !....... targ partons seen by proj
          if(lproj(ip).ge.1)then
            absb=max(1.e-9,bk(k))
            b2=absb*absb
            zkp=epscrw*exp(-b2/2./b2x)*fscra(engy/egyscr)   
c           epsilongp(k,0)=max(-betDp(0,iclpro,icltar)-0.95+alppar,
c     &                       epscrs*min(epscrx,zkp*abs(betD(0,iclpro,icltar))))
c           epsilongp(k,1)=max(-betDp(1,iclpro,icltar)-0.95+alppar,
c     &                       epscrh*min(epscrx,zkp*abs(betD(1,iclpro,icltar))))
            if(lproj3(ip).ge.1)then
              zpartar(k)=zkp        
              do lt=1,lproj3(ip)
                kp=kproj3(ip,lt)
                if(kp.ne.k)then 
                  ...
                  b2=absb*absb
                  zkp=epscrw*exp(-b2/2./b2x)*fscro(engy/egyscr) 
                endif
                zpartar(k)=zpartar(k)+zkp
c                alpfom=max(alpfom,dble(zpartar(k)))
         ...
         !...........proj partons seen by targ
          if(ltarg(it).ge.1)then
            absb=max(1.e-9,bk(k))
            b2=absb*absb
            zkt=epscrw*exp(-b2/2./b2x)*fscra(engy/egyscr) 
c           epsilongt(k,0)=max(-betDp(0,iclpro,icltar)-0.95+alppar,
c     &                       epscrs*min(epscrx,zkt*abs(betD(0,iclpro,icltar))))
c           epsilongt(k,1)=max(-betDp(1,iclpro,icltar)-0.95+alppar,
c     &                       epscrh*min(epscrx,zkt*abs(betD(1,iclpro,icltar))))
            if(ltarg3(it).ge.1)then
              zparpro(k)=zkt   
              do lp=1,ltarg3(it)
                kt=ktarg3(it,lp)
                if(kt.ne.k)then
                  ...
                  b2=absb*absb
                  zkt=epscrw*exp(-b2/2./b2x)*fscro(engy/egyscr) 
                endif
                zparpro(k)=zparpro(k)+zkt        
...
c calculation of epsilongp epsilongt
     ...
     &                        min(epscrx,epscrs*x))
c     &                        epscrs*x)
c     &                       epscrs*x*abs(betDp(0,iclpro,icltar)))
c     &                       epscrs*min(epscrx,x*abs(betDp(0,iclpro,icltar))))
          epsilongp(k,1)=max(-betDp(1,iclpro,icltar)-0.95+alppar,
     &                 epscrh*min(epscrx,x*abs(betDp(1,iclpro,icltar))))
cc          epsilongp(k,0)=epscrs*min(epscrx,x*abs(betD(0,iclpro,icltar)))
cc          epsilongp(k,1)=epscrh*min(epscrx,x*abs(betD(1,iclpro,icltar)))
          gammaV(k)=1.d0
          else
         epsilongp(k,0)=0.
         epsilongp(k,1)=0.
     ...
     &                        min(epscrx,epscrs*x))
c     &                        epscrs*x)
c     &                       epscrs*x*abs(betDpp(0,iclpro,icltar)))
c     &                       epscrs*min(epscrx,x*abs(betDpp(0,iclpro,icltar))))
          epsilongt(k,1)=max(-betDpp(1,iclpro,icltar)-0.95+alppar,
     &                epscrh*min(epscrx,x*abs(betDpp(1,iclpro,icltar))))
cc          epsilongt(k,0)=epscrs*min(epscrx,x*abs(betD(0,iclpro,icltar)))
cc          epsilongt(k,1)=epscrh*min(epscrx,x*abs(betD(1,iclpro,icltar)))
...
!..............alpha beta betap for Gtilde (->PhiExpo).......................
      ...
      do k=1,koll
        ...
        if(b.lt.(nbkbin-1)*bkbin)then
          ...
          if(isetcs.gt.1.and.iclegy.lt.iclegy2)then
            egy0=egylow*egyfac**real(iclegy-1)
            xkappa1=xkappafit(iclegy,iclpro,icltar,ibk)
     *         +(bk(k)-bkbin*real(ibk-1))/bkbin
     ...
     *         -xkappafit(iclegy,iclpro,icltar,ibk))
        ...
        do i=idxDmin,imax
          ...
          gamb=gamD(i,iclpro,icltar)*b2+epsG
...
c          if(i.eq.0) atildg(i,k)=atildg(i,k)
     ...
     *                 *exp(-b2a/delD(i,iclpro,icltar)))
            btildgp(i,k)=dble(betDp(i,iclpro,icltar)-alppar)+1.d0
            btildgpp(i,k)=dble(betDpp(i,iclpro,icltar)-alppar)+1.d0
          endif
...
!...........................................................................
           ...
           if(ktot.le.nyeps)then
            w(15,ktot)=w(15,ktot)+zpav/koll
            w(16,ktot)=w(16,ktot)+ztav/koll
            w(17,ktot)=w(17,ktot)+1
           endif
           n=1+float(nglevt)/(0.1*maproj*matarg)*(nyeps-1)
           if(nglevt.ge.1.and.n.ge.1.and.n.le.nyeps)then
            w(18,n)=w(18,n)+zpav/koll
            w(19,n)=w(19,n)+ztav/koll
            w(20,n)=w(20,n)+1
           endif
...
!........alpha beta betap for Gpro...........................................
     ...
     *                 *exp(-b2a/delD(i,iclpro,icltar)))
            ...
            btildep(i,k)=dble(betDp(i,iclpro,icltar)-alppar)
            btildepp(i,k)=dble(betDpp(i,iclpro,icltar)-alppar)
          endif

        if(btildep(i,k)+1.d0.lt.-eps.or.btildepp(i,k)+1.d0.lt.-eps)
...
!...........................................................................
      ...
      do k=1,koll
        ...
        write(ifch,*)' gammaV,epsilonGP1/2,epsilonGP1/2'
     *      ,gammaV(k),epsilongp(k,0),epsilongp(k,1)
     *      ,epsilongt(k,0),epsilongt(k,1)
        write(ifch,*)'*******************************************'
      ...
      call utprix('GfunParK',ish,ishini,5)
              
      return
      end 

c----------------------------------------------------------------------
      subroutine GfunPar(m,i,b,spp,alp,bet,betp,epsp,epst,epss,gamvv)
c----------------------------------------------------------------------
...
c----------------------------------------------------------------------
      ...
      call utprj('GfunPar ',ish,ishini,6) 
   ...
   !???????????????????????????????????????????????????estimation
      ...
      if(sigine.eq.0.)then
        ...
        siginex=sigtotf-sigelaf
      else
       siginex=sigine
      endif
      bglaub2=max(1.,siginex/10./pi)
      bglaub=sqrt(bglaub2)
      if(ish.ge.6)write(ifch,*)'Gf in:',m,i,x,b,sigine,siginex,bglaub
     &                                 ,iclpro,icltar
      !!!!!print*,'++++++++++++++++++',x,siginex,bglaub
      b2=b*b
      ee=sqrt(spp)
      cfalpro=ucfpro
      cfaltar=ucftar
      ...
      if(iscreen.ne.0)then 
        bcut=0.!zbcut
ctp060829        bzero=bglaub   
        absb=max(1.e-9,abs(b)-bcut)
        b2a=absb*absb
        b2x=epscrp*epscrp*bglaub2
        zzp=(epscrw*exp(-b2a/2./b2x))*fscra(ee/egyscr)
        zzt=(epscrw*exp(-b2a/2./b2x))*fscra(ee/egyscr)
        x=zzp
        if(i.eq.0)then
          epsGp=max(-betDp(i,iclpro,icltar)-0.95+alppar
     &             ,min(epscrx,epscrs*x))
c     &             ,epscrs*x)
c     &             ,epscrs*x*abs(betDp(i,iclpro,icltar)))
c     &             ,epscrs*min(epscrx,x*abs(betDp(i,iclpro,icltar))))
c          epsG=epsG+epsGp
        elseif(i.eq.1)then
          epsGp=max(-betDp(i,iclpro,icltar)-0.95+alppar
     &             ,epscrh*min(epscrx,x*abs(betDp(i,iclpro,icltar))))
        else
          epsGp=0.
        endif
        gamV=1.
     ...
     &             ,min(epscrx,epscrs*x))
c     &             ,epscrs*x)
c     &             ,epscrs*x*abs(betDpp(i,iclpro,icltar)))
c     &             ,epscrs*min(epscrx,x*abs(betDpp(i,iclpro,icltar))))
c          epsG=epsG+epsGt
        elseif(i.eq.1)then
          epsGt=max(-betDpp(i,iclpro,icltar)-0.95+alppar
     &             ,epscrh*min(epscrx,x*abs(betDpp(i,iclpro,icltar))))
        else
          epsGt=0.
        endif
c        gamV=gamV
     ...
     *         +gamb-alppar
          ...
          b2a=absb*absb
          alp=alp
     *       *exp(betD(i,iclpro,icltar)*log(spp)
     *            -b2a/delD(i,iclpro,icltar))
          bet=betDp(i,iclpro,icltar)-alppar
          betp=betDpp(i,iclpro,icltar)-alppar
        endif
...
c        if(i.eq.0)alp=alp
     ...
     *     *exp(betD(i,iclpro,icltar)*log(spp)
     *          -b2a/delD(i,iclpro,icltar))
          bet=betDp(i,iclpro,icltar)-alppar+1.
          betp=betDpp(i,iclpro,icltar)-alppar+1.
        endif
        coefgdp=utgam2(1.d0+dble(alplea(iclpro))+bet)
        coefgdt=utgam2(1.d0+dble(alplea(icltar))+betp)
        alp=alp*utgam2(dble(bet))*utgam2(dble(betp))/coefgdp/coefgdt
      else
      ...
      if(ish.ge.6)write(ifch,*)'   GfunPar :',alp,bet,betp,epsp,epst
     &                                    ,epss,gamvv
     
      call utprjx('GfunPar ',ish,ishini,6) 
      end      
...
c----------------------------------------------------------------------
      ...
      fscro=epscrv
      x2=x*x
      if(x2.gt.1.)fscro=sqrt(log(x2)**2+epscrv**2) 
      end
...
c----------------------------------------------------------------------
      ...
      if(iomega.eq.2)imax=1
      do i=idxDmin,imax
        call GfunPar(1,i,b,spp,alp,bet,betp,epsp,epst,epss,gamv)
        Gf=Gf+alp*real(xp)**bet*real(xm)**betp
      enddo
      om1=Gf
...
c----------------------------------------------------------------------
      double precision function om1intb(b)   !---MC---
c----------------------------------------------------------------------
...
c----------------------------------------------------------------------
      ...
      spp=engy*engy
      call GfunPar(1,iqq,b,spp,alp,bet,betp,epsp,epst,epss,gamv)
      cint=gamv*alp
      if((bet+1.0).gt.eps)then
        cint=cint/(bet+1.0)
      else
...
c
c      i=iqq
c      call GfunPar(1,i,b,spp,alp,bet,betp,epsp,epst,epss,gamv)
...
c----------------------------------------------------------------------
      ...
      include 'epos.incems'
      include 'epos.incsem'
      double precision om1intb,om1intgc

      call utpri('intom1',ish,ishini,5)
      ...
      do k=1,koll 
        ...
        om1int(k)=om1intb(bk(k))
        om1intc(k)=om1intgc(bk(k))

      enddo

      endif
      call utprix('intom1',ish,ishini,5)
...
c----------------------------------------------------------------------
      double precision function om1xpk(xp,xpr1i,xmr1i,k)   !---test---
c----------------------------------------------------------------------
     ...
     &                 ,xpr1,xmr1,xpr1i,xmr1i
      ...
      do i=imin,imax
          ...
          if((deltam(i)+1.d0).gt.eps)then
            gamomx(i)=gamomx(i)/(deltam(i)+1.0)
          else
...
c----------------------------------------------------------------------
      ...
      parameter (eps=1.d-20)
      double precision xpremi,xmremi,lxmin
     ...
     &           -exp(x1)**(2.d0+deltap(i))/(2.d0+deltap(i)))
          ...
          lxmin=log(xminDf)
          f0=f0+gamomx(i)*(xprem*(x0-lxmin)-exp(x0)+xminDf)
          f1=f1+gamomx(i)*(xprem*(x1-lxmin)-exp(x1)+xminDf)
        endif
      enddo
      f00=f0
      cint=f1-f00
 ...
 111  continue
     ...
     &           -exp(x)**(2.d0+deltap(i))/(2.d0+deltap(i)))
          ...
          fx=fx+gamomx(i)*(xprem*(x-lxmin)-exp(x)+xminDf)
...
c----------------------------------------------------------------------
      ...
      double precision xpremi,xmremi
      parameter (eps=1.d-20)

      om1xmrk=0.d0
      if(xp.ge.xpremi)return
     ...
     &           -exp(x1)**(2.d0+deltam(i))/(2.d0+deltam(i)))
          ...
          lxmin=log(xminDf)
          f0=f0+gamomx(i)*(xmrem*(x0-lxmin)-exp(x0)+xminDf)
          f1=f1+gamomx(i)*(xmrem*(x1-lxmin)-exp(x1)+xminDf)
        endif
      enddo
      f00=f0
      cint=f1-f00
 ...
 111  continue
     ...
     &           -exp(x)**(2.d0+deltam(i))/(2.d0+deltam(i)))
          ...
          fx=fx+gamomx(i)*(xmrem*(x-lxmin)-exp(x)+xminDf)
...
c----------------------------------------------------------------------
      double precision function om1yrk(xh,k)   !---test---
c----------------------------------------------------------------------
...
c----------------------------------------------------------------------
      ...
      if(xm.ge.0.)then
       ...
       xh=xm*xp
       yp=0.5*log(xp/xm)
       ffom11=2*om51(dble(xh),dble(yp),b,iq1,iq2)
     *     *(1-xm)**alplea(icltar)*(1-xp)**alplea(iclpro)
       ...
       do m=1,2
        ...
        xh=xmm*xp
        yp=0.5*log(xp/xmm)
        f=2*om51(dble(xh),dble(yp),b,iq1,iq2)
...
c----------------------------------------------------------------------
      ...
      elseif(iq1.ge.0)then
        ...
        do i=iq1,iq2
          if(i.ne.5)then
            i1=min(i,1)
            call GfunPar(1,i1,b,sy,alp,bet,betp,epsp,epst,epss,gamv)
            om51=om51+om51p(sy*real(xh),xh,yp,b,i)
     *           *xp**dble(epsp)*xm**dble(epst)
     *           *dble(sy)**dble(epss)
          else
            call GfunPar(1,2,b,sy,alp,bet,betp,epsp,epst,epss,gamv)
            om51=om51+0.5d0*alp*real(xp)**bet*real(xm)**betp
          endif
       enddo
      else
...
c----------------------------------------------------------------------
      ...
      if(iqq.ne.5)then
        ...
        if(iscreen.ne.0)then
          ...
          epsG=exp(-gfactor*(zparpro(k)*zpartar(k)))*epsG
          epsGp=exp(-gfactor*(zparpro(k)*zpartar(k)))*epsGp
          epsGt=exp(-gfactor*(zparpro(k)*zpartar(k)))*epsGt
...
c----------------------------------------------------------------------
      ...
      if(iqq.eq.0)then
        ...
        do i=idxDmin,imax
          call GfunPar(1,i,b,sy,alpx,betx,betpx,epsp,epst,epss,gamv)
          betp=1.+betx
          betpp=1.+betpx
...
c-----------------------------------------------------------------------
      ...
      include 'epos.incems'
      include 'epos.incsem'
      doubleprecision xh,yp,om5Jk,w(0:7)
      ...
      if(iotst1.gt.0)then !????????????????????????????????????????????????
        ...
        w05=w(0)+w(5)
        if(whard.lt.ww)then
          w(0)=w(0)/w05*(ww-whard)
          w(5)=w(5)/w05*(ww-whard)
        else
          w(0)=0
          w(5)=0
        endif
...
c----------------------------------------------------------------------
      double precision function PhiExact(fj,xp,xm,s,b) !---test---
c----------------------------------------------------------------------
...
c----------------------------------------------------------------------
     ...
     *,yp,ym,xm,xp
     ...
     &   .and.xm.le.1.d0+eps)then
       ...
       Phitmp=0.d0
       do i=idxDmin,imax0
         call GfunPar(1,i,b,s,alpx,betx,betpx,epsp,epst,epss,gamv)
...
c----------------------------------------------------------------------
      double precision function PhiExpo(fj,xp,xm,s,b)   !---MC---
c----------------------------------------------------------------------
c    Exponential expression of the Phi function for pp collision 
c    for given b
c----------------------------------------------------------------------
      ...
      Gt1=0.d0
      do i=idxDmin,imax
        call GfunPar(2,i,b,s,alpx,betx,betpx,epsp,epst,epss,gamv)
        BeTip =dble(betx)
        BeTipp=dble(betpx)
        AlTi  =dble(alpx)
        Gt1=Gt1+AlTi*xp**BeTip*xm**BeTipp*dble(fj*xkappa**(2.5*fj-2.5))
      enddo
...
c----------------------------------------------------------------------
 ...
 100  if(xkf.gt.1.+eps)write(ifmt,*)
...
c      write(ifmt,*)'Z calculation for (s,b) :',s,b
      ...
      call GfomPar(b,s)
      Znorm=HPhiInt(s,b)
c      write(ifch,*)'int',Znorm,' phi',PhiExact(1.,1.d0,1.d0,s,b)
      Znorm=Znorm
     &       +PhiUnit(1.d0,1.d0)
c     &       +PhiExact(1.,1.d0,1.d0,s,b)
...
c      do i=idxDmin,imax
c        call GfunPar(1,i,b,s,alp,bet,betp,epsp,epst,epss,gamv)
...
c----------------------------------------------------------------------
      ...
      if(iomega.eq.2)imax=1
      do i=idxDmin,imax
        call GfunPar(1,i,b,s,alp,bet,betp,epsp,epst,epss,gamv)
        bep =dble(bet)
        bepp=dble(betp)
        al  =dble(alp*gamv)
...
c----------------------------------------------------------------------
      ...
      include 'epos.incsem'
      include 'epos.incems'
      double precision xh,Df,xp,xm,G2,w,xpm
...
c----------------------------------------------------------------------
      include 'epos.inc'
      include 'epos.incsem'
      double precision xh,Df,xp,xm,G2,w,xmm
      double precision PoInU!,Znorm
      common /ar3/  x1(7),a1(7)
...
c Calculation by numeric integration :
      ...
      do m=1,2
        do j=1,7
          ...
          xh=xp*xm
ctp060829          sy=s*real(xh)
          Df=0.d0
          do i=idxDmin,imax
            call GfunPar(1,i,b,s,alp,bet,betp,epsp,epst,epss,gamv)
            Df=Df+alp*real(xp)**bet*real(xm)**betp
          enddo
          call GfomPar(b,s)
          G2=Df*(1.d0+ztUni*xp**betfom)*(1.d0+zpUni*xm**betfom)
          w=w+dble(a1(j))*PoInU(xp,xm,s,b)*G2
...
c----------------------------------------------------------------------
      ...
      do i=1,7
        do m=1,2
          xh=1.d0-(.5d0+dble(x1(i)*(real(m)-1.5)))**(1.d0/allea)
...
c----------------------------------------------------------------------
      ...
      do i=1,7
        do m=1,2
          xh=xhm*(1.d0+dble(x1(i)*(2.*real(m)-3.)))
...
cc----------------------------------------------------------------------
c      double precision function PomIncXExact1(xh,b)   !---check---
cc----------------------------------------------------------------------
cc incluse Pomeron distribution  \int dy { 2G F_remn F_remn }
cc----------------------------------------------------------------------
c      include 'epos.inc'
c      include 'epos.incsem'
c      double precision xh,Df,xp,xm,w,ymax
c      common /ar3/  x1(7),a1(7)
c
c      imax=1
c      if(iomega.eq.2)imax=1
c      s=engy**2   
ccctp060829      sy=s*real(xh)
cc Calculation by numeric integration :
c      w=0.d0
c      ymax=-.5d0*log(xh)
c      do m=1,2
c        do j=1,7
c          xp=sqrt(xh)*exp(dble((2.*m-3.)*x1(j))*ymax)
c          xm=xh/xp
c          Df=0.d0
c          do i=idxDmin,imax
c            call GfunPar(1,i,b,s,alp,bet,betp,epsp,epst,epss,gamv)
c            Df=Df+alp*gamv*real(xp)**bet*real(xm)**betp
c     *      *(1.d0-xp)**dble(alplea(iclpro))
c     *      *(1.d0-xm)**dble(alplea(icltar))
c          enddo
c          w=w+dble(a1(j))*Df
c        enddo
c      enddo
c      w=w*ymax*xh**dble(-alppar)
c                
c
c      PomIncXExact1=w
c
...
c----------------------------------------------------------------------
      ...
      do i=1,7
        do m=1,2
          xp=1.d0-(1.d0-xpmin)*(.5d0+dble(x1(i)*(real(m)-1.5)))
     *                                         **(1.d0/dble(allea))
          xm=xh/xp
...
c     &           +PhiExact(1.,1.d0-xp,1.d0-xm,s,b)           
...
c----------------------------------------------------------------------
      include "epos.inc"
      include "epos.incsem"
      double precision Df,xp,xm!,xh

      Df=0.d0
      xh=xp*xm
ctp060829      sy=engy**2*real(xh)
      s=engy**2
      imax=1
      if(iomega.eq.2)imax=1
      do i=idxDmin,imax
        call GfunPar(1,i,b,s,alp,bet,betp,epsp,epst,epss,gamv)
        Df=Df+alp*gamv*real(xp)**bet*real(xm)**betp
      enddo
...
c      Phitab=PhiExpo(1.,sxp,sxm,sy,b)
c      Z=Gtab*Phitab
c      Zm=0.d0
c
c      do mtmp=2,m
...
c          pGtab=1.d0
c          do i=2,mtmp
c            rnau=rangen()*real(xprem-sxp)
c            xxpu(i)=dble(rnau)
c            sxp=sxp+xxpu(i)
c            rnau=rangen()*real(xmrem-sxm)
c            xxmu(i)=dble(rnau)
c            sxm=sxm+xxmu(i)
c          enddo
...
c              pGtab=pGtab*Gtab
c            enddo
c          Zm=Zm+pGtab*PhiExpo(1.,xprem-sxp,xmrem-sxm,sy,b)
c          endif
...
c          n=n+1
c          if(mod(n,1000000).eq.0)write(*,*)
c     &              "Calculation of PhiExact(",mtmp,")->",n
c          sxp=0.d0
c          sxm=0.d0
c          pGtab=1.d0
c          do i=1,mtmp
c            rnau=rangen()!*real(xp-sxp)
c            xxpu(i)=dble(rnau)
c            sxp=sxp+xxpu(i)
c            rnau=rangen()!*real(xm-sxm)
c            xxmu(i)=dble(rnau)
c            sxm=sxm+xxmu(i)
c          enddo
...
cc     *            +omNpuncut(sy*real(xh),xh,yp,b,1) !om1(xh,yp,b)
c              pGtab=pGtab*(-Gtab)
c            enddo
...
c----------------------------------------------------------------------
 ...
 10     continue
     ...
     &              "Calculation of Gammapp(",mtmp,")->",n
          ...
          pGtab=1.d0
          do i=1,mtmp
            rnau=rangen()!*real(xp-sxp)
            xxpu(i)=dble(rnau)
            sxp=sxp+xxpu(i)
            rnau=rangen()!*real(xm-sxm)
            xxmu(i)=dble(rnau)
            sxm=sxm+xxmu(i)
          enddo
          if(sxp.lt.xp.and.sxm.lt.xm)then
            do i=1,mtmp
              ...
              pGtab=pGtab*Gtab
            enddo
            Zm=Zm+pGtab*PhiExpo(1.,xp-sxp,xm-sxm,sy,b)
          endif
...
c                pGtab=pGtab*Gtab
c              enddo
c            Zm=Zm+pGtab*PhiExpo(1.,xp-sxp,xm-sxm,sy,b)
c          endif
c        if(n.lt.nmax)goto 10
c
...
c     &,exp(-zmmax(1)),exp(-zmmin(1))
     ...
     &     ,exp(-zpmin(k)),exp(-zpmax(k)),dzp(k),dzm(k),jp1
              ...
              if(sxp.gt.0.d0.and.sxm.gt.0.d0)then
                if(dzm(1).ne.0.d0)then
                  Gm1=Gm1+pGtab*
     &dble(a1(jm1))*PhiExpo(1.,sxp,sxm,sy,b)
     &*exp(-zm(1))
c     &dble(a1(jm1))*PhiExact(1.,sxp,sxm,sy,b)
c     &*exp(-zm(1))
                  else
                  Gp1=Gp1+pGtab*
     &dble(a1(jp1))*PhiExpo(1.,sxp,sxm,sy,b)
     &*exp(-zp(1))
c     &dble(a1(jp1))*PhiExact(1.,sxp,sxm,sy,b)
c     &*exp(-zp(1))
                endif
...
c     &,1.d0-xpmin(3)-exp(-zp(1)),xpmin(2),1.d0-xpmax(3)-exp(-zp(1))
     ...
     &     ,exp(-zpmin(k)),exp(-zpmax(k)),dzp(k),dzm(k),jp1,jp2
              ...
              if(sxp.gt.0.d0.and.sxm.gt.0.d0)then
                if(dzm(2).ne.0.d0)then
                  Gm2=Gm2+pGtab*
     &dble(a1(jm2))*PhiExpo(1.,sxp,sxm,sy,b)
     &*exp(-zm(2))
c     &dble(a1(jm2))*PhiExact(1.,sxp,sxm,sy,b,mk)
c     &*exp(-zm(2))
                  else
                  Gp2=Gp2+pGtab*
     &dble(a1(jp2))*PhiExpo(1.,sxp,sxm,sy,b)
     &*exp(-zp(2))
c     &dble(a1(jp2))*PhiExact(1.,sxp,sxm,sy,b,mk)
c     &*exp(-zp(2))
                endif
...
c     &,exp(-zmmax(3)),exp(-zmmin(3))
     ...
     &   ,exp(-zpmin(k)),exp(-zpmax(k)),dzp(k),dzm(k),jp1,jp2,jp3
              ...
              if(sxp.gt.0.d0.and.sxm.gt.0.d0)then
                if(dzm(3).ne.0.d0)then
                  Gm3=Gm3+pGtab
     &*dble(a1(jm3))*PhiExpo(1.,sxp,sxm,sy,b)
     &*exp(-zm(3))
                  else
                  Gp3=Gp3+pGtab
     &*dble(a1(jp3))*PhiExpo(1.,sxp,sxm,sy,b)
     &*exp(-zp(3))
                endif
              endif
            enddo
...
c                zm=zmmin+dzm*(1.d0+dble(2.*nm1-3.)*dgx1(jm1))
     ...
     &          write(*,*)'j1=',xp,xm,xmmin,xmmax,dzp,dzm,jp1
              ...
              if(sxp.gt.0.d0.and.sxm.gt.0.d0)then
                if(dzm.ne.0.d0)then
                  Gm1=Gm1+pGtab*
     &dble(a1(jm1))*PhiExpo(1.,sxp,sxm,sy,b)*xjacm
c     &dga1(jm1)*PhiExpo(1.,sxp,sxm,sy,b)*xjacm
c     &dble(a1(jm1))*PhiExact(1.,sxp,sxm,sy,b)*xjacm
                  else
                  Gp1=Gp1+pGtab*
     &dble(a1(jp1))*PhiExpo(1.,sxp,sxm,sy,b)*xjacp
c     &dga1(jp1)*PhiExpo(1.,sxp,sxm,sy,b)*xjacp
c     &dble(a1(jp1))*PhiExact(1.,sxp,sxm,sy,b)*xjacp
                endif
c          write(*,*)'m=1',mtmp,Gm1,Gp1,pGtab,sxp,sxm
              endif
