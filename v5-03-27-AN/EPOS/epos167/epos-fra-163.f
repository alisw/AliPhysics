...
c      common /cpptr/ pptr(4,mxptl),ystr(mxptl)
      ...
      common/czz/kky,krm,zzod,zzop,kdrm,zzoq,zzos,zzrm,zzipp,zzipt
     &,zzid,zzip,zzzzr,itp
...
c.......................................................................
 ...
 3    nob=-1
     ...
     *  ('-',k=1,25),'string decay',('-',k=1,25)
      ...
      kky=0    
      if(nk2-nk1.gt.1.or.iappl.eq.6)kky=1
      itp=ityptl(nk1)
      krm=0
      if(itp.ge.50.and.itp.le.59.or.itp.ge.40.and.itp.le.49)krm=1
      kdrm=0
      if(itp.eq.43.or.itp.eq.53)kdrm=1
c special behaviour of remnant of reactions with lost Pomerons
      if(itp.eq.47.or.itp.eq.57)then
        kdrm=1
        krm=0
      endif
      zzzz=1.
      ...
      zzip=1.
      if(isplit.eq.1)then
        zzzz=zpaptl(nk1)+zpaptl(nk2)
        zzzzp=zpaptl(nk1)
        zzzzt=zpaptl(nk2)
        zzid=min( 1+zidmax , 1+zidinc  *zzzz ) !<----- 
        zzip=min( 1+zopmax , 1+zopinc  *zzzz ) !<----- 
c        zzrm=min( zrmmax   , zrminc    *zzzz ) !<-----
        if(itp.ge.40.and.itp.le.49)then
          zzzzr=zzzzp
        elseif(itp.ge.50.and.itp.le.59)then
          zzzzr=zzzzt
        endif
        zzod=min( 1+zodmax , 1+zodinc  *zzzzr ) !<----- 
        zzos=min( 1+zosmax , 1+zosinc  *zzzzr ) !<----- 
        zzop=min( 1+zopmax , 1+zopinc  *zzzzr ) !<-----
        zzoq=min( 1+2.*zopmax , 1+ 2.*zopinc  *zzzzr ) !<-----
        zzipp=min( 1+zipmax , 1+zipinc  *zzzzp ) !<-----
        zzipt=min( 1+zipmax , 1+zipinc  *zzzzt ) !<-----
      endif
      do k=1,5
        p1(k)=0d0
      enddo
...
c     &    p1(5)=dble(pptl(5,iorrem))
      ...
      if(iappl.ne.6)qsqptl(nk1)=p1(5)*p1(5) !hadronic string energy
                                                        !(already defined for lepton)
...
c          pb=0.38
         ...
         pb=0.14369+ 854.965/x**3 - 111.1144/x**2 + 7.163/x 
c         !! {{14,0.4},{22,0.32},{34,0.28},{91.2,0.21}}
c         pb=0.143+ 900/x**3 - 77/x**2 + 4.5/x 
c         pb=0.165+ 1020/x**3 - 80/x**2 + 3.8/x 
c         pb=0.17+ 950/x**3 - 76/x**2 + 3.8/x 
c         pb=0.165+ 600/x**3 - 50/x**2 + 3.8/x 
c          pb=0.135+ 855./x**3 - 111./x**2 + 7./x
c          pb=0.05+ 1100./x**3 - 80./x**2 + 5./x
        endif
        pb=pb*abs(pbi)
      endif
c      write(*,*)'pb',kky,krm,kdrm,x,pb,zzrm,zzzz
...
c....................................................................
      ...
      do i=1,4
        ...
        ptb(i,1)=0.
      enddo
c calculate isospin to conserve isospin symmetry in popcorn
c p -> pi+ <=> n -> pi-
      if(abs(iflb(0)).gt.6)then
        iq01=mod(abs(iflb(0))/100,10)
        iq02=abs(iflb(0))/1000
        iso=(isospin(iq01)+isospin(iq02))*sign(1,-iflb(0))
      else
        iso=isospin(abs(iflb(0)))*sign(1,-iflb(0))
      endif
      if(abs(iflb(1)).gt.6)then
        iq11=mod(abs(iflb(1))/100,10)
        iq12=abs(iflb(1))/1000
        iso=iso+(isospin(iq11)+isospin(iq12))*sign(1,iflb(1))
      else
        iso=iso+isospin(abs(iflb(1)))*sign(1,iflb(1))
      endif
...
c to avoid leading strange baryon for diffractive string, rotate string

      zz=0.
      if(kdrm.eq.1.and.krm.ne.0.and.nbr.gt.0)then
        iq1=abs(iflb(1))
        iq2=abs(iflb(2))
        if(iq1.gt.3.or.iq2.gt.2)then
          am=0.
        elseif(iq1.eq.3)then
          am=qmass(iq1)
        elseif(iso.gt.0)then   !for u or d, use proper mass if isospin > 0 (proton)
          am=qmass(iq1)
        elseif(iso.lt.0)then                  !us d mass for u and u mass for u if isospin < 0
          am=qmass(3-iq1)
        else
          am=0.5*(qmass(1)+qmass(2))
        endif
        zz=zrminc*pf(nob,nob)**2.
        zz=zz*(1.+zrmmax*log(float(matarg+maproj-1)))
c        if(itp/10.eq.4)then
c          zz=zz*(1.+zrmmax*log(max(1.,float(matarg)/float(maproj))))
c        elseif(itp/10.eq.5)then
c          zz=zz*(1.+zrmmax*log(max(1.,float(maproj)/float(matarg))))
c        endif
        zz=am*zz
c        print *,iso,itp,pf(nob,nob),am,zz,iq1,iq2,exp(-min(100.,zz))
      endif
c      if(kdrm.eq.1)print *,'reminv',itp,am,pf(nob,nob),zz,iq1,iq2
c     &                       ,reminv+exp(-min(100.,zz))
      if(rangen().gt.reminv+exp(-min(100.,zz)))then
        leadstr=.true.
      else
        leadstr=.false.
      endif
     ...
     &       call utstop('gakfra (end): mxptl too small ...&') 
      do i=1,nbr+1
         ...
         if(i.lt.nbr+1)then     !particle = left side of breakpoints
           ...
           taubrr=pst(4,nob+7)+x*pst(4,nob+8)+y*pst(4,nob+9)
           x=xyb(1,i)
           y=xyb(2,i)
           aml2=co(1)+x*co(2)+y*co(3)+x*y*co(4)
           amr2=co(5)+x*co(6)+y*co(7)+x*y*co(8)
     ...
     & 'Negative mass, fragmentation not OK -> continue ...'
             n=i
             nptl=nptlini
             goto 9
           endif
...
c             pptr(j,nptl)=ptb(j,i) !should be zero
      ...
      if(leadstr)then
        iptmp=idptl(nptlini+2)
        idptl(nptlini+2)=idptl(nptlini+1)
        idptl(nptlini+1)=iptmp
        pptltmp=pptl(5,nptlini+2)
        pptl(5,nptlini+2)=pptl(5,nptlini+1)
        pptl(5,nptlini+1)=pptltmp
        pptl(4,nptlini+1)=pptl(5,nptlini+1)*pptl(5,nptlini+1)
        pptl(4,nptlini+2)=pptl(5,nptlini+2)*pptl(5,nptlini+2)
        do k=1,3
          pptl(4,nptlini+1)=pptl(4,nptlini+1)
     &                     +pptl(k,nptlini+1)*pptl(k,nptlini+1)
          pptl(4,nptlini+2)=pptl(4,nptlini+2)
     &                     +pptl(k,nptlini+2)*pptl(k,nptlini+2)
        enddo
        pptl(4,nptlini+1)=sqrt(pptl(4,nptlini+1))
        pptl(4,nptlini+2)=sqrt(pptl(4,nptlini+2))
      endif
      nptl=nptlini
      do i=1,nbr+1
         nptl=nptl+1
...
c.....breakpoint accepted......................
      ...
      if(kdrm.eq.1)then
        f=2.
        if(ib1.eq.0)f=1.
        if(abs(iflb(ib1)).gt.3)then
          iq1=mod(abs(iflb(ib1))/1000,10)
          iq2=mod(abs(iflb(ib1))/100,10)
          amlast=qmass(0)+f*(qmass(iq1)+qmass(iq2))
        else
          amlast=f*qmass(abs(iflb(ib1)))
        endif
        if(itp.ge.50)then
          ampt=amtarg
        elseif(itp.ge.40)then
          ampt=amproj
        endif
        xlastdiq=0.
        do i=0,ib1-1
          f=2.
          if(i.eq.0)f=1.
          if(abs(iflb(i)).gt.3)then
            iq1=mod(abs(iflb(i))/1000,10)
            iq2=mod(abs(iflb(i))/100,10)
            xlastdiq=xlastdiq+qmass(0)+f*(qmass(iq1)+qmass(iq2))
          else
            xlastdiq=xlastdiq+f*qmass(abs(iflb(i)))
          endif
        enddo
        if(pf(nob,nob).le.5.)amlast=1.e-10 !if very low energy, strangeness should not be totally killed
c        if(pf(nob,nob).le.5.)then
c          xlastdiq=-(ampt+2.*qmass(3)) !if very low energy, strangeness should not be totally killed
c          amlast=strcut**2
c        endif
       if(ish.ge.5)write(ifch,*) 'diff cut:',pf(nob,nob),xlastdiq,amlast
     &  ,float(ib1+1)-ampt-xlastdiq+diqcut*sqrt(pf(nob,nob))
     &  ,strcut/amlast/ampt*pf(nob,nob)
      endif
...
c ..........diquark...............................................
      ...
      if(kdrm.eq.1)pdiqu=pdiqu*(1.-exp(-min(100.,
     & strcut/amlast/qmass(0)/ampt
     & *max(0.,float(ib1+1)-ampt-xlastdiq-2.*qmass(0)+diqcut*
     &                      sqrt(pf(nob,nob)))*pf(nob,nob))))
c      if(kdrm.eq.1.and.sqrt(pf(nob,nob)).lt.diqcut)pdiqu=0.

      if(rangen().lt.pdiqu.and.iabs(iflb(ib1)).lt.6
     &  .and.iabs(iflb(ib2)).lt.6)then  
        jqu=2
      else 
        jqu=1
      endif
c ..........flavor...............................................
  77  continue 
      if(krm.eq.1)then
        if(kdrm.eq.0)then
          pud1= ( 1 - min(0.33,zzos*(1-2*pudr)) ) /2. 
c          pud2= pud1
        else
          pud1= pudd 
c          pud2= pud1
...
c     pud2= pud1 
      ...
      if(kdrm.eq.1)pud1=0.5*(1.-(1.-2.*pud1)
     &  *(1.-exp(-min(100.,strcut/amlast/qmass(3)/ampt
     &  *max(0.,float(ib1+1)-ampt-xlastdiq-2.*qmass(3)+diqcut
     &                      *sqrt(pf(nob,nob)))*pf(nob,nob)))))
c      if(kdrm.eq.1.and.sqrt(pf(nob,nob)).lt.strcut)pud1=0.5

      pud2=(1.-(1.-2.*pud1)*puds)*0.5   !from e+e- data
      
      if(iabs(iflb(ib1)).gt.1000)then  
        ifl1=int(rangen()**0.625/pud1)+1 
      else
        ifl1=int(rangen()**1.15/pud1)+1     !assymmetric u and d relevant because of assymmetry Kch / K0 in e+e-
      endif

c      amk0=2.*qmass(ifl1)                 !mass for mt distribution
      if(ifl1.eq.4)ifl1=3
      ifl2=0 
      if(jqu.eq.2)then  !diquark ------
        ifl2=int(rangen()**0.625/pud2)+1 
c        amk0=amk0+2.*qmass(ifl2)+qmass(0)  !mass for mt distribution with bounding energy for diquark (qmass(0))
        if(ifl2.eq.4)ifl2=3
      ...
      iflb(ib1+1)=ifl
      if(ish.ge.5)write(ifch,*) 'flavor,pud1/2:',ifl,pud1,pud2,pdiqu
c..............................pt.......................................
      if(krm.ne.1.and.kdrm.ne.1)then
        icub=ib1+1  
        !---------------------------------------------------------------------------
        !  ib1+1 is the current break point index
        !             (between 1 and nbr)
        !  ijb(1,ib1) and ijb(2,ib1) are band indices
        !         each index from 0 to nob-1 (nob= number of bands)
        !         0 is left, then come the gluons, then antiquark, then agin gluons
        !         like q - g - g - g - ~q - g - g - g
        !--------------------------------------------------------------------------
        call gaksco(icub-1,icub,icub+1
     &    ,ijb(1,icub),ijb(2,icub),x1,x2,y1,y2)
        x=xyb(1,icub)
        y=xyb(2,icub)
        aml2=co(1)+x*co(2)+y*co(3)+x*y*co(4)
        amr2=co(5)+x*co(6)+y*co(7)+x*y*co(8)
        aml=sign(sqrt(abs(aml2)),aml2)
        amr=sign(sqrt(abs(amr2)),amr2)
        !------segment left of current breakpoint icub -----
        pleft(5)=aml
        do j=1,4
          pleft(j)=pst(j,nob+1)-x*pst(j,nob+2)+y*pst(j,nob+3)
        enddo
        call utlob4(-1,psg(1),psg(2),psg(3),psg(4),psg(5)
     $     ,pleft(1),pleft(2),pleft(3),pleft(4))
        !------segment right of current breakpoint icub-----  
        pright(5)=amr
        do j=1,4
          pright(j)=pst(j,nob+4)+x*pst(j,nob+5)-y*pst(j,nob+6)
        enddo
        call utlob4(-1,psg(1),psg(2),psg(3),psg(4),psg(5)
     $     ,pright(1),pright(2),pright(3),pright(4))
        !-------------------------
        amt=pleft(5)**2+pleft(1)**2+pleft(2)**2
        if(amt.gt.0..and.pleft(4)+abs(pleft(3)).gt.0.d0)then
          amt=sqrt(amt)
          yleft=sign(1.,pleft(3))*alog((pleft(4)+abs(pleft(3)))/amt)
        else
          yleft=0.                  !
        endif
        amt=pright(5)**2+pright(1)**2+pright(2)**2
        if(amt.gt.0..and.pright(4)+abs(pright(3)).gt.0.d0)then
          amt=sqrt(amt)
          yright=sign(1.,pright(3))*alog((pright(4)+abs(pright(3)))/amt)
        else
          yright=0.                  !
        endif
        ybrk=(yleft+yright)/2.
        yhax=2                  !0.5*yhaha
        zzipx=1
        if(ybrk.gt.yhax)then
          zzipx=zzipx+(zzipp-1)
        elseif(ybrk.gt.-yhax)then  
          zzipx=zzipx+(zzipp-1)*(ybrk+yhax)/(2*yhax)
        endif
        if(ybrk.lt.-yhax)then
          zzipx=zzipx+(zzipt-1)
        elseif(ybrk.lt.yhax)then  
          zzipx=zzipx+(zzipt-1)*(yhax-ybrk)/(2*yhax)
        endif
      endif
      delptfra=0
      if(ifl1.eq.3.and.ifl2.eq.0)delptfra=delptfra+ptfrasr
c      if(ifl1.eq.3.or.ifl2.eq.3)delptfra=delptfra+ptfrasr
      fnsbp=1
      if(nsbp.gt.9)fnsbp=0

c for a better pt distribution, we generate mt=sqrt(pt2+m02) (with m0=amk0) instead of pt (do not give good results !)
      if(krm.eq.1)then
        if(kdrm.eq.0)then
c          ptadd=ranpt()*(ptfrair+delptfra)*(zzop-1)*fnsbp  !*jqu
c          ptadd=ranpt()*ptfrair*(zzop-1)*fnsbp  !*jqu
          ptadd=(ptfrair+delptfra)*(zzop-1)*fnsbp  !*jqu
          if(jqu.eq.1)then
c            pt=ranpt()*(ptfrair +delptfra )  + ptadd 
c            pt=ranpt()*ptfrair 
            pt=ranpt()*(ptfrair              + ptadd )
          else
            pt=ranpt()*(ptfrair   + ptadd          ) 
c            pt=ranpt()*ptfraqq
c            pt=ranpt()*(ptfraqq    + ptadd )
          endif
        else
c          ptadd=0.
           pt=ranpt()*ptfradr 
          if(jqu.eq.1)then
c            pt=ranpt()*(ptfradr +delptfra   ) 
            pt=ranpt()*ptfradr 
          else
            pt=ranpt()*ptfraqq  
c            pt=ranpt()*(ptfradr       ) 
      ...
      elseif(kky.eq.1)then
c        ptadd=ranptk()*(ptfrak+delptfra)*(zzipx-1)*fnsbp  !*jqu
        pttra=(ptfrak+delptfra)*(zzipx-1)*fnsbp    !/jqu
        if(jqu.eq.1)then
            !pt=ranptk()*(ptfrak+delptfra    +  pttra )
            !pt=ranptk()*(ptfrak   +  pttra)     
         pt=ranptk()*ptfrak  +  ranptk()*pttra     
        else
            !pt=ranptk()*(ptfrakd   +  pttra)    
         pt=ranptk()*ptfrakd  +  ranptk()*pttra    
        endif
      else
c        ptadd=ranpt()*(ptfra+delptfra)*(zzip-1)*fnsbp     !*jqu
c        pttra=ranpt()*ptfra*(zzip-1)*fnsbp     !*jqu
        pttra=(ptfra+delptfra)*(zzip-1)*fnsbp     !*jqu
        if(jqu.eq.1)then
c          pt=ranpt()*(ptfra+delptfra )   +  pttra 
c          pt=ranpt()*ptfra
...
c      pt=sqrt(pt*pt+2.*pt*amk0)+ptadd                  !sample mt-m0 instead of pt ... doesn't work at RHIC 
      beta=2.*pi*rangen()
 60   continue
      if(ish.ge.5)then
        write(ifch,*) 'pt:',pt
     ...
     &  ,ptb(1,ib1+1),ptb(2,ib1+1),ptb(3,ib1+1),ptb(4,ib1+1))
      ...
      if(az.eq.0..and.ay.eq.0.)az=1e-8
      if(ish.ge.8)write(ifch,*)'rot vector:',ax,ay,az,ae,am      

c.....call utrota(-1,ax,ay,az,ptb(1,ib1+1),ptb(2,ib1+1),ptb(3,ib1+1))
...
c----------------------------------------------------------------------
c .........exp(-x**2)
 12   x=sqrt(-(log(rangen()))/(3.1415927/4.)/xcut) !gauss
C 12   x=sqrt(-(log(rangen()))/(3.1415927/4.)/xcut) !gauss


c      if(xcut.gt.0.)then
c        if(rangen().lt.x/xcut)goto 12
c      endif
...
c      nmax=xpar2
      ...
      do i=1,nrbins
        ycut=xminim*(xmaxim/xminim)**((real(i)-0.5)/nrbins)
c        if(iologe.ne.1)ycut=xminim+(xmaxim-xminim)/nrbins*(nrbins)
      ...
      do i=1,nrbins
        ycut=xminim*(xmaxim/xminim)**((real(i)-0.5)/nrbins)
        write (ifhi,*) ycut,real(njets(n,ijadu,i))/nrevt
...
c        do i=i1,int(log(abs(real(id(j))))/log(10.))+1
