...
c (do not use it when gakstr() is called (some information lost)
          ...
          nptlbpo=nptl
          call jintpo  !parton-ladder-fusion
...
c----------------------------------------------------------------------
     ...
     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
      parameter(xgrid=8,mxgrid=10,kgrid=6,kegrid=3)
      parameter(ssgrid=0.5*xgrid*kgrid*kegrid)
      parameter(mssgrid=mxgrid*kgrid*kegrid)
      parameter(mxcl=1000,mxcli=200)
      integer idropgrid(mxgrid,mxgrid,mssgrid)
     &       ,jdropgrid(mxgrid,mxgrid,mssgrid)
     &       ,jclu(mssgrid),nsegp4(mxcl)
     &       ,irep(mxcl),mmji(mxcl,mxcli)
     &       ,jccl(mxcl,nflav,2),nseg(mxcl),mseg(mxcl),kclu(mxcl)
     &       ,naseg(0:mxcl),nfseg(mxcl),nsegmx(mxcl)
     &       ,ic(2),jc(nflav,2),ke(6),jcjj(nflav,2)
     &       ,nsegmt(mxptl)   !,nclk(mssgrid)
      double precision tpro,zpro,ttar,ztar,ttaus,detap,detat
     &                ,pptld(5,mxcl),p4tmp
      ...
      logical first
      double precision ptest(4),ttest,p52,p4mean(mxcl)  !,ppp(5),qqq(5),www(5)
      save ncntpo
      data ncntpo /0/
      ncntpo=ncntpo+1
      call utpri('jintpo',ish,ishini,4)

      if(ttaus.gt.kgrid)then
        write(ifmt,*)'ttaus,kgrid :',ttaus,kgrid
        stop'jintpo: ttaus too big.'
      endif
      tauico=ttaus
      fegrid=yhaha/5.36                 !rapidity range/rap range at RHIC
      sgrid=ttaus*ssgrid/kgrid *fegrid
      msgrid=max(1.4d0,ttaus)*mssgrid/kgrid *fegrid
      if(mod(msgrid,2).eq.0)msgrid=msgrid+1
      vocell= 2*xgrid/mxgrid * 2*xgrid/mxgrid * 2*sgrid/msgrid 
      delzet=2*sgrid/msgrid/ttaus
      delsgr=2*sgrid/msgrid
      nptla=nptl
      nsegsuj=max(3,nint(float(nsegsu)*fegrid))
      !print*,'dx,dz:',2*xgrid/mxgrid,2*sgrid/msgrid
      !. ,'  nx nz:',  mxgrid,msgrid, ttaus
...
c...count string segments in cell
      ...
      do n=1,nptla
        if(iaaptl(n).ne.0)then
          i=1+(xptl(n)+xgrid)/(2*xgrid/mxgrid)
          j=1+(yptl(n)+xgrid)/(2*xgrid/mxgrid)
          k=1+(sptl(n)+sgrid)/(2*sgrid/msgrid)
          if(  i.ge.1.and.i.le.mxgrid
     &    .and.j.ge.1.and.j.le.mxgrid
     &    .and.k.ge.1.and.k.le.msgrid)
...
c          i=1+(xptl(n)+xgrid)/(2*xgrid/mxgrid)
c          j=1+(yptl(n)+xgrid)/(2*xgrid/mxgrid)
c          k=1+(sptl(n)+sgrid)/(2*sgrid/msgrid)
c          if(  i.ge.1.and.i.le.mxgrid
c     &    .and.j.ge.1.and.j.le.mxgrid
c     &    .and.k.ge.1.and.k.le.msgrid)then
        ...
        do j=1,mxgrid
          ...
          do i=1,mxgrid
            if(idropgrid(i,j,k).ge.nsegce)then
              ...
              jdropgrid(i,j,k)=jj
              if(j.gt.1.and.jdropgrid(i,j-1,k).ne.0)then
...
c...calculate mean energy of segments going into in clusters
     ...
     &'calculate mean energy of segments going into in clusters'
      do 95 n=1,nptla
        ...
        if(    i.ge.1.and.i.le.mxgrid
     &          .and.j.ge.1.and.j.le.mxgrid
     &          .and.k.ge.1.and.k.le.msgrid)then
          jj=jdropgrid(i,j,k)
          if(jj.gt.0)then
...
c...add segments moving towards clusters
     ...
     &          .and.k.ge.1.and.k.le.msgrid)then
          jgr=jdropgrid(i,j,k)
          if(jgr.eq.0)then
           if(i.ge.mxgrid/2)then
            isi=-1
           else
            isi=1
           endif   
           if(j.ge.mxgrid/2)then
            jsi=-1
           else
            jsi=1
           endif   
            do ii=i,i+2*isi,isi
             do jj=j,j+2*jsi,jsi
               if(.not.(ii.eq.i.and.jj.eq.j))then
                if(ii.ge.1.and.ii.le.mxgrid
     .          .and.jj.ge.1.and.jj.le.mxgrid)then
   ...
   94   continue        
        if(ihit.eq.1)then
          istptl(n) = 3 
         nseg(jg)=nseg(jg)+1
         delx=2*xgrid/mxgrid*(ii-i)
         dely=2*xgrid/mxgrid*(jj-j)
         xptl(n)=xptl(n)+delx
         yptl(n)=yptl(n)+dely
        ! if(k.ge.10.and.k.le.13)
   !  .          print*,k, '  ',i,j,'  ',ii,jj,nseg(jg),'   ',delx,dely
        endif   
  93  continue
      endif
...
c...finish cluster storage to /cptl/
     ...
     *  ,ibptl(1,n),ibptl(2,n),ibptl(3,n),ibptl(4,n))
     ...
     &                   +pptld(5,mm)*pptld(5,mm))
      ...
      if(ish.ge.1.and.(abs(ttest).gt.1.d0.or.pptld(5,mm).gt.120d0))then
         call utmsg('jintpo&')
     ...
     *       -pptl(1,ni)**2-pptl(2,ni)**2 
     ...
     &                 ,nh,ntry
       ...
       mjjseg=mjjseg+mseg(mm)
        ...
        tivptl(2,n)=ainfin
        ityptl(n)=60
        vocri=pptl(5,n)/epscri(ioclude)
        vo=max(vocri,vocell)
        radptl(n)=(3.*vo/4./pi)**0.3333
        dezptl(n)=0.
       enddo
       do l=1,4
     ...
     *  ,ibptl(1,njj),ibptl(2,njj),ibptl(3,njj),ibptl(4,njj))
         !.................
         !inertia tensor:   
         !<y**2>   -<x*y>  with <x**2>=uptl  <y**2>=optl
         !-<x*y>   <x**2>                   <xy>=desptl
         !.................
      enddo
...
c...print
      ...
      if(ish.ge.5)then
        ...
        do j=mxgrid,1,-1
        write(ifch,'(10i4,3x,10i4)')(idropgrid(i,j,k),i=1,mxgrid)
     &                ,(jdropgrid(i,j,k),i=1,mxgrid)    
        enddo
        enddo
        write(ifch,'(a,a)')
      ...
      do jj=1,jjj
       do ii=1,max(1,nint(1.*nseg(jj)/nsegsuj))
        ...
        ip=nptlb+mm
          ...
          xx=ev2
              ecc=(yy-xx)/(yy+xx)
          ranecc=ranecc+ecc*pptl(5,ip)
          weiecc=weiecc+pptl(5,ip)
       enddo
      enddo
      if(weiecc.gt.0.)then
       ranecc=ranecc/weiecc
      endif

c...decay
             
      if(ish.ge.6)write(ifch,*)'decay'
...
c       ish=ishb
        if(iret.eq.1)then
          ...
          do n=nptlij+1,nptl
            ...
            ifrptl(1,n)=0
            ifrptl(2,n)=0
              !xorptl(1,n)  already defined in hnbaaa
              !xorptl(2,n)  already defined in hnbaaa
              !xorptl(3,n)  already defined in hnbaaa
              !xorptl(4,n)  already defined in hnbaaa
            tivptl(1,n)=xorptl(4,n)
            call idtau(idptl(n),pptl(4,n),pptl(5,n),taugm)
            r=rangen()
            tivptl(2,n)=tivptl(1,n)+taugm*(-alog(r))
              !ityptl(n)   already defined in hnbaaa
            radptl(n)=0.
            dezptl(n)=0.
            itsptl(n)=0
            rinptl(n)=kclu(jj)-msgrid/2
          enddo
        endif  
       enddo
      enddo
