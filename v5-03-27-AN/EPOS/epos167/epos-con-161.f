...
c     ---------------------------------------------------------------
      ...
      kproj3(1,1)=1
      ktarg3(1,1)=1
      iactpr(1)=1  !int(rangen()+1./chad(iclpro)) !active proj.
      iacttg(1)=1  !int(rangen()+1./chad(icltar)) !active targ.
      if(iactpr(1).eq.0.and.iacttg(1).eq.0)iret=-1
...
c      wactv=1./chad(2)              !probability to be "active"
      koll=0
      do i=1,maproj
      lproj(i)=0
      lproj3(i)=0
      iactpr(i)=1 !int(rangen()+wactv) !active proj. nucleons
      enddo
      do j=1,matarg
      ltarg(j)=0
      ltarg3(j)=0
      iacttg(j)=1 !int(rangen()+wactv) !active targ. nucleons
      enddo
      do 12 i=1,maproj
      if(iactpr(i).eq.0)goto 12
      do 11 j=1,matarg
      if(iacttg(j).eq.0)goto 11
...
12    continue
      ...
      if(((iactpr(1).eq.0.and.maproj.eq.1)
     &.or.(iacttg(1).eq.0.and.matarg.eq.1))
     &.and.iret.eq.0)iret=-1
