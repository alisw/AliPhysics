...
**************************************************************************/
void MakeACORDEQAReferenceLowMultiplicityData()
{
	...
	AliACORDECalibData *calibda = new AliACORDECalibData("QALowMultiplicity");
	...
	TObjString str("ACORDE QA Reference LowMultiplicity Data");      // object that will be stored

	AliCDBMetaData *md= new AliCDBMetaData(); // metaData describing the object

	AliCDBId id("ACORDE/QA/LowMultiplicity",0,9999999);

	md->SetResponsible("Mario Rodriguez");
	md->SetBeamPeriod(0);
	md->SetAliRootVersion("Current trunk version");
	md->SetComment("Version 1.0 of QA Reference LowMultiplicity Data for ACORDE");
	md->PrintMetaData();
