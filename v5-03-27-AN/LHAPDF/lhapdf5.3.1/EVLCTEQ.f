...
20    RETURN
      ...
      EndIf
c ++ remove unused quantities (jcp)
c ++  TMD = TIN + DT * NT / 2.
c ++  AMU = EXP(TMD)
c ++  TEM = 6./ (33.- 2.* NEFF) / CtLhALPI(AMU)
c ++  TLAM = TMD - TEM
      DO 9 IX = 1, NX
      UN(IX, 0)  = U0(IX)
    9 CONTINUE
   ...
   15 CONTINUE
	...
	stop
     ...
     >                  PNSP(0:MXX), PNSM(0:MXX)
      ...
      Else
         IF (LB) THEN
c +++ something is wrong, since QFF1 and RFF1 are not used.
c +++ but this code appears to only be used for extrapolation 
c +++ to small x, which is unreliable anyway, so ignore for now (jcp)
            QFF1 = TEM
            LB   =.FALSE.
         Else
            RFF1 = TEM * X / (1.-X)
      ...
      X = XX
      IF (X .GE. D1) THEN
        CtLhRGG1= 0		!error corrected? (jcp)
        RETURN
      ElseIF (X .GE. XMIN) THEN
        Z = CtLhZFRMX (X)
      ...
      EndIf
      IF (LA) THEN
         CtLhRGG1 = TEM / X / (1.-X)	!error corrected? (jcp)
         LA   =.FALSE.
      Else
         IF (LB) THEN
            CtLhRGG1 = TEM / X		!error corrected? (jcp)
            LB   =.FALSE.
         Else
            CtLhRGG1 = TEM / (1.-X)
         EndIf
      ...
      X = XX
      IF (X .GE. D1) THEN
        CtLhRFF2 = 0		!error corrected? (jcp)
        RETURN
      ElseIF (X .GE. XMIN) THEN
        Z = CtLhZFRMX (X)
      ...
      EndIf
      IF (LA) THEN
         CtLhRFF2 = TEM / X / (1.-X)	!error corrected? (jcp)
         LA   =.FALSE.
      Else
         IF (LB) THEN
            CtLhRFF2 = TEM / X		!error corrected? (jcp)
            LB   =.FALSE.
         Else
            CtLhRFF2 = TEM / (1.-X)
         EndIf
      ...
      X = XX
      IF (X .GE. D1) THEN
        CtLhRGG2 = 0		!error corrected? (jcp)
        RETURN
      ElseIF (X .GE. XMIN) THEN
        Z = CtLhZFRMX (X)
      ...
      EndIf
      IF (LA) THEN
         CtLhRGG2 = TEM / X / (1.-X)	!error corrected? (jcp)
         LA   =.FALSE.
      Else
         IF (LB) THEN
            CtLhRGG2 = TEM / X	!error corrected? (jcp)
            LB   =.FALSE.
         Else
            CtLhRGG2 = TEM / (1.-X)
         EndIf
