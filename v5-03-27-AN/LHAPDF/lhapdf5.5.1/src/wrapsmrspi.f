...
!                                                                       
! ::::::::::::  PION STRUCTURE FUNCTION :: 10% SEA :::::::::::::::::    
!                                                                       
      ...
      PARAMETER(NTENTH=21) 
      common /SMRSP/ F(7,NX,NQ,3) 
      DIMENSION G(7),XX(NX),N0(7),XX0(NX) 
!      DIMENSION F(7,NX,NQ),G(7),XX(NX),N0(7)                           
     ...
     &     .8D0,.85D0,.9D0,.95D0,.975D0,1.D0/                           
      ...
      DO 20 M=1,NQ 
         DO 25 I=1,7 
   25    F(I,N,M,jset)=F(I,N,M,jset)/(ONED-XX0(N))**N0(I) 
   20 CONTINUE 
      DO 30 J=1,NTENTH-1 
      if(jset.eq.1) XX(J)= LOG10(XX(J))+ONEDO 
      DO 30 I=2,6 
      DO 30 K=1,NQ 
   30 F(I,J,K,jset)= LOG(F(I,J,K,jset))                                 &
     &              *F(I,NTENTH,K,jset)/ LOG(F(I,NTENTH,K,jset))        
   50 FORMAT(7F10.5) 
      DO 40 I=1,7 
      DO 40 M=1,NQ 
   40 F(I,NX,M,jset)=ZEROD 
      enddo 
   10 CONTINUE 
!      IF(X.LT.XMIN) X=XMIN                                             
!      IF(X.GT.XMAX) X=XMAX                                             
...
!      IF(QSQ.GT.QSQMAX) QSQ=QSQMAX                                     
   ...
   65 CONTINUE 
      G(I)=G(I)*(ONED-X)**N0(I) 
   60 END DO 
! UPBAR DISTRIBUTION = D DISTRIBUTION                                   
      UPV=G(2) 
      DNV=G(2) 
! THIS SEA IS (UBAR+DBAR)/2                                             
      SEA=G(4) 
      STR=G(6) 
      CHM=G(5) 
      GLU=G(3) 
