...
/// \ingroup macros
/// \file MUONSurveyCh1.C
/// \brief Macro to process survey and photogrammetry data of chamber 1
/// 
...
void MUONSurveyCh1() {
  ...
  Int_t nDetElemsI = 2;
  //  Int_t nDetElemsO = 2;
  Int_t iDetElemToDetElemId[4] = {100,101,102,103};
  ...
  myChamberI->GetSurveyObj()->FillFromLocalFile("~/phgwrk/Reports/AliceSt1_TC1_1171d.txt");
  myChamberO = (AliMUONSurveyChamber*)myChamberArray->At(1);
  myChamberO->GetSurveyObj()->FillFromLocalFile("~/phgwrk/Reports/AliceSt1_TC1_1171d.txt");
  ...
  myChamberI->AddGButtonTargets(TString("T4_520"),3);
  myChamberI->AddStickerTargets(TString("T4_620"),6);
  myChamberI->AddLButtonTargets(lSO->GetData(),Form("%d_520",chId+1),3);
  myChamberI->AddGButtonTargets(TString("T4_510"),3);
  myChamberI->AddStickerTargets(TString("T4_610"),6);
  myChamberI->AddLButtonTargets(lSO->GetData(),Form("%d_510",chId+1),3);
  myChamberO->AddGButtonTargets(TString("T4_520"),3);
  myChamberO->AddStickerTargets(TString("T4_620"),6);
  myChamberO->AddLButtonTargets(lSO->GetData(),Form("%d_520",chId+1),3);
  myChamberO->AddGButtonTargets(TString("T4_510"),3);
  myChamberO->AddStickerTargets(TString("T4_610"),6);
  ...
  for (int iCh =0; iCh<=1; iCh++) {
    ...
    for (int iDetElemI=0; iDetElemI<nDetElemsI; iDetElemI++){
      ...
      if (myDetElem) {
	baseName = Form("T4_53%d",iDetElem+1);
...
//       lDiffRotDetElem0[iDetElem] = lRotDetElem[iDetElem];
  ...
  // Plot the found misalignements (Units here are mm and mrad!)
  ...
  if(bMonitor){    
    ...
    for (int iCh =0; iCh<=1; iCh++) {
      ...
      myChamber->FillCPSTHistograms(Form("T4_6%d",2-iCh),hCPSTc);
      myChamber->FillDESTHistograms(TString("T4_46"),hSSTc);
