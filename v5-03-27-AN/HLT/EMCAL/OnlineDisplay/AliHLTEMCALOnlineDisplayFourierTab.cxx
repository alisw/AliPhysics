// $Id: AliHLTEMCALOnlineDisplayFourierTab.cxx 35108 2009-09-30 01:58:37Z phille $
...
//#include "AliHLTEMCALSharedMemoryInterfacev2.h"
...
AliHLTEMCALOnlineDisplayFourierTab::AliHLTEMCALOnlineDisplayFourierTab()
{
...
AliHLTEMCALOnlineDisplayFourierTab::AliHLTEMCALOnlineDisplayFourierTab(AliHLTEMCALOnlineDisplay * const onlineDisplayPtr, TGTab  *tabPtr, 
								     const AliHLTHOMERReader * homerSyncPtr, const AliHLTHOMERReader * const homerPtrs[MAXHOSTS], int nHosts) :  AliHLTEMCALOnlineDisplayTab(), fEvtCnt(0)
{     
  // See header file for documentation
  // gStyle->SetOptLogy();
  ...
  //fFourierPtr = new AliHLTEMCALFourier();
  fFourierPtr = new AliHLTCaloFourier();
...
int
AliHLTEMCALOnlineDisplayFourierTab::GetNextEvent()
{
  // See header file for documentation
  //  ResetDisplay();
  DoGetNextEvent();
...
void 
AliHLTEMCALOnlineDisplayFourierTab::ReadBlockData(AliHLTHOMERReader * const homeReaderPtr)
{ 
  // See header file for documentation
  //  AliHLTEMCALValidCellDataStruct *currentChannel =0;
  cout << "AliHLTEMCALOnlineDisplayFourierTab::ReadBlockDat, Reading block data, therere are " <<  homeReaderPtr->GetBlockCnt() << " blocks " <<endl;
...
//       Int_t rcuX = 0;
//       Int_t rcuZ = 0;
//       AliHLTEMCALRcuCellEnergyDataStruct* cellEnergiesPtr = (AliHLTEMCALRcuCellEnergyDataStruct*)homeReaderPtr->GetBlockData( blk ); 
...
//       cout << "AliHLTEMCALOnlineDisplayFourierTab::ReadBlockData,  fModuleID =" <<moduleID << endl; 
...
//  FillHistograms(fFourierPtr->GetPSD(), fFourierPtr->GetDataSize());
  ...
  while ( blk != ~(unsigned long)0 ) 
    {  
      //      AliHLTEMCALRcuFFTDataStruct* fftDataPtr = (AliHLTEMCALRcuFFTDataStruct*)homeReaderPtr->GetBlockData( blk );
      AliHLTCaloRcuFFTDataStruct* fftDataPtr = (AliHLTCaloRcuFFTDataStruct*)homeReaderPtr->GetBlockData( blk );
...
void 
//AliHLTEMCALOnlineDisplayFourierTab::FillHistograms(const AliHLTEMCALRcuFFTDataStruct psd, const int size)
AliHLTEMCALOnlineDisplayFourierTab::FillHistograms(const AliHLTCaloRcuFFTDataStruct psd, const int size) 
{
  // See header file for documentation
  //  gStyle->SetOptLogy();
  //  gStyle->SetOptStat(false);
...
void
AliHLTEMCALOnlineDisplayFourierTab::InitDisplay(TGTab  *tabPtr)
{
  ...
  for(int gain=0; gain < NGAINS; gain++)
    {
      ...
      sprintf(label, "EMCAL Fourier transform %s", gainLabel);
  ...
  fgEventButtPtr = new  AliHLTEMCALGetEventButton(fSubF1, "get fourier", 'e');
}



void
AliHLTEMCALOnlineDisplayFourierTab::UpdateDisplay()
{
  // See header file for documentation
...
const  char* 
AliHLTEMCALOnlineDisplayFourierTab::Gain2Text(const int gain, const char delimeter)
{
  // See header file for documentation
  if(gain ==  LOWGAIN)
    {
