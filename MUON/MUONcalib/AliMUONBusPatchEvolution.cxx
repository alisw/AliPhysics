...
Bool_t AliMUONBusPatchEvolution::FillNumberOfPads()
{
	...
	for ( it = fNofPads.begin(); it != fNofPads.end(); ++it )
	{
		...
		if (!hde) {
			hde = new TH1F(Form("DE%04d", detElemId), "number of pads", 1, 0,
					1);
		...
		if (dcs.DCSAliasName(detElemId).Contains("Left")) {
			hchamberSide = fBPEVO.Histo(
					Form("/CHAMBER/NPADS/CHAMBER%dLEFT", chamberId));
			if (!hchamberSide) {
				hchamberSide = new TH1F(Form("CHAMBER%dLEFT", chamberId),
						"number of pads", 1, 0, 1);
				fBPEVO.Adopt("/CHAMBER/NPADS/", hchamberSide);
			}
			hstationSide = fBPEVO.Histo(
					Form("/STATION/NPADS/STATION%dLEFT", stationId));
			if (!hstationSide) {
				hstationSide = new TH1F(Form("STATION%dLEFT", stationId),
						"number of pads", 1, 0, 1);
			...
			if (dcs.DCSAliasName(detElemId).Contains("Right")) {
				hchamberSide = fBPEVO.Histo(
						Form("/CHAMBER/NPADS/CHAMBER%dRIGHT", chamberId));
				if (!hchamberSide) {
					hchamberSide = new TH1F(Form("CHAMBER%dRIGHT", chamberId),
							"number of pads", 1, 0, 1);
					fBPEVO.Adopt("/CHAMBER/NPADS/", hchamberSide);
				}
				hstationSide = fBPEVO.Histo(
						Form("/STATION/NPADS/STATION%dRIGHT", stationId));
				if (!hstationSide) {
					hstationSide = new TH1F(Form("STATION%dRIGHT", stationId),
							"number of pads", 1, 0, 1);
					fBPEVO.Adopt("/STATION/NPADS/", hstationSide);
				}
			}
		...
		if (!hchamber) {
			hchamber = new TH1F(Form("CHAMBER%d", chamberId), "number of pads",
					1, 0, 1);
		...
		if (!hstation) {
			hstation = new TH1F(Form("STATION%d", stationId), "number of pads",
					1, 0, 1);
...
int AliMUONBusPatchEvolution::GetTimeResolution(const TH1& h)
{
	...
	if (last != "bins" || m != "s") {
		std::cerr
		<< "Histogram title does not match the expected pattern (... xxx s bins) : "
		<< std::endl << h.GetTitle() << std::endl;
	} else {
		timeResolution =
				static_cast<TObjString*>(a->At(a->GetLast() - 2))->String().Atoi();
	}
	delete a;
		...
		std::map<int,double>& faultyBusPatchOccupancies)
{
	...
	{
		if ( hnevents->GetBinContent(i) > 1 ) lastbin = i;
	}
	...
	for ( firstbin = lastbin; firstbin > 0; --firstbin )
	{
		...
		if ( nevents > requiredEvents ) break;
	}
...
void AliMUONBusPatchEvolution::GroupByStation(int timeResolution)
{
	...
	for (Int_t ich = 1; ich < 10; ich += 2) {
		TH1* h = fBPEVO.Histo(
				Form("/CHAMBER/HITS/%ds/CHAMBER%d", timeResolution, ich));
		TH1* h1 = fBPEVO.Histo(
				Form("/CHAMBER/HITS/%ds/CHAMBER%d", timeResolution, ich + 1));
		...
		h = fBPEVO.Histo(
				Form("/CHAMBER/HITS/%ds/CHAMBER%dLEFT", timeResolution, ich));
		h1 = fBPEVO.Histo(
				Form("/CHAMBER/HITS/%ds/CHAMBER%dLEFT", timeResolution,
						ich + 1));
		...
		h = fBPEVO.Histo(
				Form("/CHAMBER/HITS/%ds/CHAMBER%dRIGHT", timeResolution, ich));
		h1 = fBPEVO.Histo(
				Form("/CHAMBER/HITS/%ds/CHAMBER%dRIGHT", timeResolution,
						ich + 1));
...
void AliMUONBusPatchEvolution::GroupByChamber(int timeResolution)
{
	...
	for (Int_t ich = 1; ich <= 10; ++ich) {
		...
		while (!it.IsDone()) {
			...
			TH1* h = fBPEVO.Histo(
					Form("/DE/HITS/%ds/DE%04d", timeResolution, detElemId));

			if (dcs.DCSAliasName(detElemId).Contains("Left")) {
				if (!hchamberLeft) {
					hchamberLeft = static_cast<TH1*>(h->Clone(
							Form("CHAMBER%dLEFT", ich)));
				} else {
					listLeft.Add(h);
				}
			} else {
				if (!hchamberRight) {
					hchamberRight = static_cast<TH1*>(h->Clone(
							Form("CHAMBER%dRIGHT", ich)));
				} else {
					listRight.Add(h);
				}
			}
		...
		TH1* hchamber = static_cast<TH1*>(hchamberLeft->Clone(
				Form("CHAMBER%d", ich)));
	...
	while (!it.IsDone()) {
		...
		if (de->GetStationType() != AliMp::kStationTrigger) {
			for (Int_t i = 0; i < de->GetNofBusPatches(); ++i) {
				...
				TH1* h = fBPEVO.Histo(
						Form("/BUSPATCH/HITS/%ds/BP%04d", timeResolution,
								busPatchId));
...
void
AliMUONBusPatchEvolution::ShrinkTimeAxis()
{
	...
	while ((h = static_cast<TH1F*>(next()))) {
		...
		if (h->GetEntries()) {
			lastbins[timeResolution] = TMath::Max(lastbins[timeResolution],
					h->FindLastBinAbove(0.0));

			if (firstbins.count(timeResolution)) {
				firstbins[timeResolution] = TMath::Min(
	...
	for (it = timeResolutions.begin(); it != timeResolutions.end(); ++it) {
		...
		for (std::vector<TH1F*>::size_type iv = 0; iv < v.size(); ++iv) {
			...
			TH1F* hnew = new TH1F(hold->GetName(), hold->GetTitle(),
					lastbin - firstbin + 1, hold->GetBinLowEdge(firstbin),
