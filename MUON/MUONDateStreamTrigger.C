...
// Macro to read rawdata Date File for trigger
// Ch. Finck, Subatech, June. 2006
//
// This macro is interface with AliRawReader and DATE for RAW
// The different stucture of the patload are readout and stored in TClonesArray
// with AliMUONRawStreamTrigger classe.
// The decoding of the payload is done by AliMUONPayLoadTrigger class
...
void MUONDateStreamTrigger(Int_t maxEvent = 1000, Int_t minDDL = 0, Int_t maxDDL = 1, TString fileName = "raw.root.date", Int_t eventType = 7 )
{
  ...
  // eventType = 0 for DATE file from Nantes test bench
   ...
   // default wise set to 2, 8, 16 respectively for AliRoot
   // else see below
   rawStream->SetMaxDDL(1);
   rawStream->SetMaxReg(1);
   rawStream->SetMaxLoc(16);
...
// 	 printf("Serial number %d\n",regHeader->GetSerialNb());
	 ...
	 for(Int_t iLocal = 0; iLocal < nLocal; iLocal++) {  
	   ...
	   //	   if (localStruct->GetTriggerY() == 0) { // no empty data
	     ...
	     Int_t loId      = (Int_t)localStruct->GetId();
	     Int_t loDec     = (Int_t)localStruct->GetDec();
