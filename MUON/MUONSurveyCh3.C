...
/// \brief Macro to process survey and photogrammetry data of chamber 3
/// 
...
void MUONSurveyCh3() {
  ...
  Int_t nDetElemsI = 2;
  //  Int_t nDetElemsO = 2;
  Int_t iDetElemToDetElemId[4] = {300,301,302,303};
  ...
  myChamberI->GetSurveyObj()->FillFromLocalFile("../Reports/AliceSt2_TC3_2571e.txt");
  myChamberO = (AliMUONSurveyChamber*)myChamberArray->At(1);
  myChamberO->GetSurveyObj()->FillFromLocalFile("../Reports/AliceSt2_TC3_2571e.txt");
  ...
  myChamberI->AddGButtonTargets(TString("T1_3520"),3);
  myChamberI->AddStickerTargets(TString("T1_3620"),6);
  myChamberI->AddLButtonTargets(lSO->GetData(),Form("%d_520",chId+1),3);
  myChamberI->AddGButtonTargets(TString("T1_3510"),3);
  myChamberI->AddStickerTargets(TString("T1_3610"),6);
  myChamberI->AddLButtonTargets(lSO->GetData(),Form("%d_510",chId+1),3);
  myChamberO->AddGButtonTargets(TString("T1_3520"),3);
  myChamberO->AddStickerTargets(TString("T1_3620"),6);
  myChamberO->AddLButtonTargets(lSO->GetData(),Form("%d_520",chId+1),3);
  myChamberO->AddGButtonTargets(TString("T1_3510"),3);
  myChamberO->AddStickerTargets(TString("T1_3610"),6);
  ...
  for (int iCh =0; iCh<=1; iCh++) {
    ...
    for (int iDetElemI=0; iDetElemI<nDetElemsI; iDetElemI++){
      ...
      if (myDetElem) {
	baseName = Form("T1_353%d",iDetElem+1);
	myDetElem->AddGButtonTargets(baseName,3);
	baseName = Form("T1_363%d",iDetElem+1);
	myDetElem->AddStickerTargets(baseName,6);
	baseName = Form("T2_363%d",iDetElem+1);
	myDetElem->AddStickerTargets(baseName,6);
...
//       lDiffRotDetElem0[iDetElem] = lRotDetElem[iDetElem];
  ...
  if(bMonitor){    
    ...
    for (int iCh =0; iCh<=1; iCh++) {
      ...
      myChamber->FillCPSTHistograms(Form("T1_36%d",2-iCh),hCPSTc);
      myChamber->FillDESTHistograms(TString("T1_36"),hSSTc,TString("T2_36"),hSSTa);
    ...
    pad->cd(2);
    hSSTa->SetMinimum(6680);
    hSSTa->SetMaximum(6700);
    hSSTa->Draw("lego2z");
    ...
    pad->cd(1);
    hCPSTc->SetMinimum(6790);
    hCPSTc->SetMaximum(6810);
    hCPSTc->Draw("lego2z");

    pad->cd(2);
    hSSTc->SetMinimum(6820);
    hSSTc->SetMaximum(6840);
    hSSTc->Draw("lego2z");
