                           ...
                           Bool_t readFromRP = 0)
{
  ...
  if (!diSim.IsValid()) {
    std::cerr << "Cannot access " << filenameSim << std::endl;
    return;
  }

  if (!diRec.IsValid()) {
    std::cerr << "Cannot access " << filenameRec << std::endl;
    return;
  }
  ...
  for (Int_t ievent = 0; ievent < nevents; ++ievent) {
      ...
      while ((mHit = static_cast<AliMUONHit*>(next()))) {
        ...
        for (Int_t j = 0; j < 4; j++) {
          ...
          if (hittrack == 1 || hittrack == 2) {
            if (Nch == kch && IdPart == -13 && NbHits[0][j] == 0)
              NbHits[0][j]++;
            if (Nch == kch && IdPart == 13 && NbHits[1][j] == 0)
              NbHits[1][j]++;
          }
        }
      }
    ...
    if (CoincMuPlus == 1 && CoincMuMinus == 1)
      coincmuon++;
    ...
    AliMUONVTriggerStore* triggerStore =
      diRec.TriggerStore(ievent, tree.Data());

    AliMUONGlobalTrigger* gloTrg = triggerStore->Global();

    if (gloTrg->PairUnlikeLpt() >= 1)
      muonlpt++;
    if (gloTrg->PairUnlikeHpt() >= 1)
      muonhpt++;
