...
const Int_t AliMUONAlignment::fgNDetElemHalfCh[AliMUONAlignment::fgNHalfCh] = { 2, 2, 2, 2, 2, 2, 2, 2, 9, 9, 9, 9, 13, 13, 13, 13, 13, 13, 13, 13 };

// list of detector elements for each half chamber
const Int_t AliMUONAlignment::fgDetElemHalfCh[AliMUONAlignment::fgNHalfCh][AliMUONAlignment::fgNDetHalfChMax] =
{
...
void AliMUONAlignment::Init( void )
{
  ...
  */
  ...
  {
    fTFile = new TFile( "AliMUONAlignment.root","RECREATE");
...
void AliMUONAlignment::Terminate( void )
{
  AliInfo( "Closing Evaluation TFile" );
  if( fTFile && fTTree )
  {
    fTFile->cd();
    fTTree->Write();
...
AliMillePedeRecord* AliMUONAlignment::ProcessTrack( AliMUONTrack* track, Bool_t doAlignment, Double_t weight )
{
  ...
  */
  ...
  for( Int_t iTrackParam = 0; iTrackParam < trackParamAtCluster->GetEntries(); ++iTrackParam )
  {

    // get track parameters
    AliMUONTrackParam* trackParam( (AliMUONTrackParam *) trackParamAtCluster->At(iTrackParam) );
...
//_____________________________________________________________________
void AliMUONAlignment::GroupHalfChamber( Int_t iCh, Int_t iHalf, UInt_t mask )
{
  ...
  if( iHalf < 0 || iHalf > 1 )
  { AliFatal( Form( "Invalid half chamber index %i", iHalf ) ); }
...
//_____________________________________________________________________
void AliMUONAlignment::GroupDetElems( const Int_t* detElemList, Int_t nDetElem, UInt_t mask )
{
...
//_____________________________________________________
LocalTrackParam AliMUONAlignment::RefitStraightTrack( AliMUONTrack* track, Double_t z0 ) const
{
  ...
  for( Int_t iTrackParam = 0; iTrackParam < trackParamAtCluster->GetEntries(); ++iTrackParam )
  {
    ...
    TMatrixD A(2, 4);
    A.Zero();
    A(0,0) = 1; A(0, 2) = (cluster->GetZ() - z0);
    A(1,1) = 1; A(1, 3) = (cluster->GetZ() - z0);
    ...
    TMatrixD G(2,2);
    G.Zero();
    G(0,0) = 1.0/Square(cluster->GetErrX());
    G(1,1) = 1.0/Square(cluster->GetErrY());
