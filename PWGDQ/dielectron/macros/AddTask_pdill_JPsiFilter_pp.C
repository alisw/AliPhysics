...
AliAnalysisTask *AddTask_pdill_JPsiFilter_pp(
    TString cfg="ConfigJPsi_nano_pp2015.C",
  ...
  TString configFile("");
  printf("%s \n",gSystem->pwd());
  if(cfg.IsNull()) cfg="ConfigJPsi_nano_pp2015.C";
  ...
  TString checkconfig="ConfigJPsi_nano_pp2015";
  if (!gROOT->GetListOfGlobalFunctions()->FindObject(cfg.Data()))
    gROOT->LoadMacro(configFile.Data());

  AliDielectron *jpsi=ConfigJPsi_nano_pp2015(0,hasMC,period);
