...
class AliDielectronHistosTaku : public TNamed {
public:
  AliDielectronHistosTaku();
  AliDielectronHistosTaku(const char* name, const char* title);
  virtual ~AliDielectronHistosTaku();
...
//   virtual TObject   *Remove(TObject *obj) { return 0; }
  ...
  void UserTree(const char *name, const char* title);
  void SetReserveVariableInTree(UInt_t valTypes=kNoAutoFill);
  void FillTree(Int_t nValues, const Double_t *values);
  const THashList* GetTreeList() const {return &fTreeList;}  
  void SetTreeList(THashList &list, Bool_t setOwner=kTRUE);
  void ResetTreeList(){fTreeList.Clear();}
  TTree * GetSingleTree() const {return fTree;}

  Double_t fgData[AliDielectronVarManager::kNMaxValues];

private:
  THashList fHistoList;             //-> list of histograms
  THashList fTreeList;             //-> list of histograms
  ...
  AliDielectronHistosTaku(const AliDielectronHistosTaku &hist);
  AliDielectronHistosTaku& operator = (const AliDielectronHistosTaku &hist);

  ClassDef(AliDielectronHistosTaku,2)
};
