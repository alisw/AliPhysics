...
#include "AliDielectronDebugTreeTaku.h"
...
ClassImp(AliDielectronTaku)

const char* AliDielectronTaku::fgkTrackClassNames[4] = {
  "ev1+",
  "ev1-",
  "ev2+",
  "ev2-"
...
const char* AliDielectronTaku::fgkPairClassNames[11] = {
  "ev1+_ev1+",
  "ev1+_ev1-",
  "ev1-_ev1-",
  "ev1+_ev2+",
...
//________________________________________________________________
AliDielectronTaku::AliDielectronTaku(const char* name, const char* title) :
  TNamed(name,title),
  fEventFilter("EventFilter"),
  fTrackFilter("TrackFilter"),
  fPairPreFilter("PairPreFilter"),
...
//________________________________________________________________
void AliDielectronTaku::Process(AliVEvent *ev1, AliVEvent *ev2)
{
  //
  // Process the events
  //
...
//________________________________________________________________
void AliDielectronTaku::FillHistogramsTracks(TObjArray **tracks)
{
  //
...
//________________________________________________________________
void AliDielectronTaku::FillHistogramsMC(const AliMCEvent *ev)
{
  //
...
//________________________________________________________________
void AliDielectronTaku::FillHistograms(const AliVEvent *ev)
{
  //
...
//________________________________________________________________
void AliDielectronTaku::FillHistogramsPair(AliDielectronPair *pair,Bool_t fromPreFilter/*=kFALSE*/)
{
  //
...
//________________________________________________________________
void AliDielectronTaku::FillTrackArrays(AliVEvent * const ev, Int_t eventNr)
{
  //
  ...
  for (Int_t itrack=0; itrack<ntracks; ++itrack){
    ...
    if (charge>0){
      fTracks[eventNr*2].Add(particle);
    }else if (charge<0){
      fTracks[eventNr*2+1].Add(particle);
    }
  }
}

//________________________________________________________________
void AliDielectronTaku::PairPreFilter(Int_t arr1, Int_t arr2, TObjArray &arrTracks1, TObjArray &arrTracks2)
{
  //cout<<"PairPreFilter"<<endl;
  ...
  for (Int_t itrack1=0; itrack1<ntrack1; ++itrack1){
    ...
    for (Int_t itrack2=0; itrack2<end; ++itrack2){
      ...
      //cout<<candidate.OpeningAngle()<<" "<<candidate.Y()<<" "<<cutMask<<" "<<selectedMask<<endl;
      //apply cut
    ...
    if ( accepted ) arrTracks1.AddAt(0x0,itrack1); //kTRUE
  }
...
//________________________________________________________________
void AliDielectronTaku::FillPairArrays(Int_t arr1, Int_t arr2)
{
  //
...
//________________________________________________________________
void AliDielectronTaku::FillPairArrayTR()
{
  //
  // select pairs and fill pair candidate arrays
  //
...
//________________________________________________________________
void AliDielectronTaku::FillDebugTree()
{
  //
  // Fill Histogram information for tracks and pairs
  //
...
//________________________________________________________________
void AliDielectronTaku::SaveDebugTree()
{
  //
...
void AliDielectronTaku::AddSignalMC(AliDielectronSignalMC* signal) {
  //
...
//________________________________________________________________
void AliDielectronTaku::FillTree(const AliVEvent *ev)
{
  //
  // Fill Histogram information for tracks and pairs
  //
  ...
  AliDielectronVarManager::Fill(ev, values);  
  //Fill track information, separately for the track array candidates
  for (Int_t i=0; i<2; ++i){ //same event in different charge 
    Int_t ntracks=fTracks[i].GetEntriesFast();
    for (Int_t itrack=0; itrack<ntracks; ++itrack){
      AliDielectronVarManager::Fill(fTracks[i].UncheckedAt(itrack), values);
      fTree->FillTree(AliDielectronVarManager::kNMaxValues, values);
      fTree->GetSingleTree()->Fill();
    }
  }
