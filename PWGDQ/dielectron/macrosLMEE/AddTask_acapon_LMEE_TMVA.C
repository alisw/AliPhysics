AliAnalysisTask *AddTask_acapon_LMEE_TMVA(Double_t userTMVACutValue = 0.05,
					   Int_t cutDefinition = 0,
					   TString userPathWeightFile="",
					   TString outputFileName = "AnalysisResult.root",
					   TString directoryBaseName = "acapon_LMEE_TMVA",
  ...
  if (!mgr) {
    Error("AddTask_acapon_LMEE_TMVA", "No analysis manager found.");
    return 0;
  }
  ...
  TString configFile("Config_acapon_LMEE_TMVA.C");
  ...
  if (mgr->GetInputEventHandler()->IsA()==AliAODInputHandler::Class()){
    ::Info("AddTask_acapon_LMEE_TMVA", "no dedicated AOD configuration");
  }
  else if (mgr->GetInputEventHandler()->IsA()==AliESDInputHandler::Class()){
    ::Info("AddTask_acapon_LMEE_TMVA","switching on ESD specific code");
    bESDANA=kTRUE;
  }
  
  //create task and add it to the manager
  AliAnalysisTaskMultiDielectron *task = new AliAnalysisTaskMultiDielectron("MultiDiEData_acapon_TMVA");
  ...
  AliDielectron *diel_low = Config_acapon_LMEE_TMVA(userTMVACutValue,cutDefinition,userPathWeightFile,bESDANA,bCutQA,kFALSE,useTPCCorr);
  if(diel_low){
