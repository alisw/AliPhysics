AliAnalysisTask *AddTask_rbailhac_RandomRejection(Bool_t getFromAlien=kFALSE,
                                                  Bool_t configsPreloaded=kFALSE,
                                                  TString cFileName = "Config_rbailhac_randomRejection.C", 
                                                 ...
                                                 )
{
  ...
  if (!mgr) {
    Error("AddTask_rbailhac_RandomRejection", "No analysis manager found.");
    return 0;
  }
  
  //Base Directory for GRID / LEGO Train
  TString configBasePath= "$ALICE_PHYSICS/PWGDQ/dielectron/macrosLMEE/";
  if(getFromAlien && (!configsPreloaded) &&(!gSystem->Exec(Form("alien_cp alien:///alice/cern.ch/user/r/rbailhac/PWGDQ/dielectron/macrosLMEE/%s .",cFileName.Data()))) ){
  ...
  for (Int_t i=0; i<nDie; ++i){ //nDie defined in config file
    //MB
    AliDielectron *diel_low = Config_rbailhac_randomRejection(i, kTRUE); //kTRUE -> "isRandomRejTask"
  ...
  AliAnalysisDataContainer *coutput1 =
	mgr->CreateContainer("rbailhac_RandomRejection_tree",
                       TTree::Class(),
                       AliAnalysisManager::kExchangeContainer,
                       outputFileName);
  
  AliAnalysisDataContainer *cOutputHist1 =
	mgr->CreateContainer("rbailhac_RandomRejection_out",
                       TList::Class(),
                       AliAnalysisManager::kOutputContainer,
                       outputFileName);
  
  AliAnalysisDataContainer *cOutputHist2 =
	mgr->CreateContainer("rbailhac_RandomRejection_CF",
                       TList::Class(),
                       AliAnalysisManager::kOutputContainer,
                       outputFileName);
  
  AliAnalysisDataContainer *cOutputHist3 =
	mgr->CreateContainer("rbailhac_RandomRejection_EventStat",
                       TH1D::Class(),
                       AliAnalysisManager::kOutputContainer,
                       outputFileName);
