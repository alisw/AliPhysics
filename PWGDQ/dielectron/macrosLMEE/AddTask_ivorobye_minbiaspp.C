AliAnalysisTask *AddTask_ivorobye_minbiaspp(Bool_t getFromAlien=kFALSE,
                                            ...
                                            )
{
    ...
    if (!mgr) {
        Error("AddTask_ivorobye_minbiaspp", "No analysis manager found.");
        return 0;
    }
    ...
    if (rejectPileup) task->SetRejectPileup(kTRUE); // rejectPileup == kTRUE by default for min.bias
    ...
    for (Int_t i=0; i<nDie; ++i){ //nDie defined in config file
        ...
        AliDielectron *diele = Config_ivorobye_pp(i, kTRUE); // second flag - min.bias analysis?
        if(!diele)continue;
