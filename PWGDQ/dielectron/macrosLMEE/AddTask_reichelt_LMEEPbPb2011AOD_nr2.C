AliAnalysisTask *AddTask_reichelt_LMEEPbPb2011AOD_nr2(Char_t* outputFileName="LMEEoutput.root", 
 Bool_t flag1=kFALSE, Bool_t cutlibPreloaded=kFALSE, Bool_t getFromAlien=kFALSE, 
 Int_t triggerNames=(AliVEvent::kMB+AliVEvent::kCentral+AliVEvent::kSemiCentral), Int_t collCands=AliVEvent::kAny) 
{
  ...
  if (!mgr) {
    Error("AddTaskLMEEPbPb2011AOD_nr2", "No analysis manager found.");
    return 0;
  }
  ...
  ::Info("AddTaskLMEEPbPb2011AOD_nr2",Form("configBasePath.Data(): %s\n",configBasePath.Data()));
  ...
  if (mgr->GetInputEventHandler()->IsA()==AliAODInputHandler::Class()){
    ::Info("AddTaskLMEEPbPb2011AOD_nr2","running on AODs.");
  }
  else if (mgr->GetInputEventHandler()->IsA()==AliESDInputHandler::Class()){
    ::Info("AddTaskLMEEPbPb2011AOD_nr2","switching on ESD specific code, make sure ESD cuts are used.");
    bESDANA=kTRUE;
  }
  ...
  AliAnalysisTaskMultiDielectron *task=new AliAnalysisTaskMultiDielectron("MultiDiEData_nr2");
  ...
  for (Int_t i=0; i<nDie; ++i){ //nDie defined in config file
    //MB
    AliDielectron *diel_low = Config_reichelt_LMEEPbPb2011_nr2(i, hasMC, bESDANA);
  ...
  AliAnalysisDataContainer *coutput1 =
	mgr->CreateContainer("reichelt_LMEEPbPb2011_tree_nr2",
                       TTree::Class(),
                       AliAnalysisManager::kExchangeContainer,
                       outputFileName);
  
  AliAnalysisDataContainer *cOutputHist1 =
	mgr->CreateContainer("reichelt_LMEEPbPb2011_out_nr2",
                       TList::Class(),
                       AliAnalysisManager::kOutputContainer,
                       outputFileName);
  
  AliAnalysisDataContainer *cOutputHist2 =
	mgr->CreateContainer("reichelt_LMEEPbPb2011_CF_nr2",
                       TList::Class(),
                       AliAnalysisManager::kOutputContainer,
                       outputFileName);
  
  AliAnalysisDataContainer *cOutputHist3 =
	mgr->CreateContainer("reichelt_EventStatPbPb2011_nr2",
                       TH1D::Class(),
                       AliAnalysisManager::kOutputContainer,
                       outputFileName);
