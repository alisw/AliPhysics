...
void SetTPCSigmaEleCorrection(AliDielectron *die, Int_t corrXdim, Int_t corrYdim, TString year, Bool_t hasSpline);
...
AliDielectronCutGroup *SetV0Cuts(Int_t cutDefinition);
//
//TString names=("pt200_TPCTOFcombITSshared");
//TString names=("pt200_TPCTOFcombITSshared;TPCTOFreq;TPCTOFreqV0");
TString names=("TPCTOFreq;TPCHadRej;TPCHadRejTOFif");
TObjArray *arrNames=names.Tokenize(";");
const Int_t nDie=arrNames->GetEntriesFast();

enum {kPhiV = 0, kPt3D, kEta3D, kPhi3D, kMee, kMeeLinear, kPtLinear, kMeeForDCAee, kPhiVRebinned, kP2D, kPtee3D, kMee3D, kPairDCA};
...
AliDielectron* Config_hmurakam_pp(Int_t cutDefinition, TString year = "", Bool_t hasSpline = kFALSE)
{
    //
    ...
    if (kTPCCorr) SetTPCSigmaEleCorrection(die, AliDielectronVarManager::kP, AliDielectronVarManager::kEta, year.Data(), hasSpline);
...
void SetTPCSigmaEleCorrection(AliDielectron *die, Int_t corrXdim, Int_t corrYdim, TString year, Bool_t hasSpline) {
    ...
    TH2F* histMean2D;
    TH2F* histWidth2D;
    TString rootFile = "alien://alice/cern.ch/user/h/hmurakam/PWGDQ/dielectron/calibLMEE/calMaps_TPC.root";
    TGrid::Connect("alien://");
    gSystem->Exec(Form("alien_cp %s .",rootFile.Data()));
    TString fileName= "calMaps_TPC.root";
    TFile* f = TFile::Open(fileName.Data());
    printf("Load %s",fileName.Data());
    TString meanName =Form("m%s",year.Data());
    TString widthName =Form("w%s",year.Data());
    if(year =="18" && !hasSpline){
      meanName =Form("m%sns",year.Data());
      widthName =Form("w%sns",year.Data());
      printf("2018 no spline\n");
    }
    f->GetObject(meanName.Data(),histMean2D);
    f->GetObject(widthName.Data(),histWidth2D);
    printf("%s and %s\n",meanName.Data(),widthName.Data());

    for (Int_t i = 0; i <= histMean2D->GetNbinsX()+1; i++){
      for (Int_t k = 0; k <= histMean2D->GetNbinsY()+1; k++){
	if ( (i == 0) || (k == 0) || (i > histMean2D->GetNbinsX()) || (k > histMean2D->GetNbinsY())) { // under/overflows
	  histMean2D->SetBinContent(i, k, 0.0 );
	  histWidth2D->SetBinContent(i, k, 1.0 );
	}
      }
    }
...
void SetTOFSigmaEleCorrection(AliDielectron *die, Int_t corrXdim, Int_t corrYdim) {
    ...
    TFile* f = new TFile("corr_TOF_17all.root");

    f->GetObject("TOFm_17all",histMean2DTOF);
    f->GetObject("TOFw_17all",histWidth2DTOF);
...
void InitHistograms(AliDielectron *die, Int_t cutDefinition)
{
    ...
    //--- DCA ---------------------------------------------
    histos->UserHistogram("Track","dXY","dXY;DCA_{xy} (cm);N of tracks", 400,-4.,4.,AliDielectronVarManager::kImpactParXY);
    ...
    histos->UserHistogram("Track","TPC_dEdx_P","TPC dEdx;#it{p} (GeV/#it{c});TPC signal (arb units)",
                          400,0.,20.,200,0.,200.,AliDielectronVarManager::kPIn,AliDielectronVarManager::kTPCsignal);
    histos->UserHistogram("Track","TPCnSigmaEle_P","TPC number of sigmas Electrons;#it{p} (GeV/#it{c});TPC number of sigmas Electrons",
                          400,0.,20.,600,-15.,15.,AliDielectronVarManager::kPIn,AliDielectronVarManager::kTPCnSigmaEle);
    histos->UserHistogram("Track","TPCnSigmaEleRaw_P","raw TPC number of sigmas Electrons;#it{p} (GeV/#it{c});raw TPC number of sigmas Electrons",
                          400,0.,20.,600,-15.,15.,AliDielectronVarManager::kPIn,AliDielectronVarManager::kTPCnSigmaEleRaw);
    histos->UserHistogram("Track","TPCnSigmaPio_P","TPC number of sigmas Pions;#it{p} (GeV/#it{c});TPC number of sigmas Pions",
                          400,0.,20.,600,-15.,15.,AliDielectronVarManager::kPIn,AliDielectronVarManager::kTPCnSigmaPio);
    histos->UserHistogram("Track","TPCnSigmaKao_P","TPC number of sigmas Kaons;#it{p} (GeV/#it{c});TPC number of sigmas Kaons",
                          400,0.,20.,600,-15.,15.,AliDielectronVarManager::kPIn,AliDielectronVarManager::kTPCnSigmaKao);
    histos->UserHistogram("Track","TPCnSigmaPro_P","TPC number of sigmas Protons;#it{p} (GeV/#it{c});TPC number of sigmas Protons",
                          400,0.,20.,600,-15.,15.,AliDielectronVarManager::kPIn,AliDielectronVarManager::kTPCnSigmaPro);
    ...
    histos->UserHistogram("Track","TOFbeta","TOF beta;#it{p} (GeV/#it{c});TOF #beta",
                          400,0.,20.,300,0.,3.,AliDielectronVarManager::kPIn,AliDielectronVarManager::kTOFbeta);
    histos->UserHistogram("Track","TOFbeta_Pt","TOF beta;#it{p}_{T} (GeV/#it{c});TOF #beta",
                          400,0.,20.,300,0.,3.,AliDielectronVarManager::kPt,AliDielectronVarManager::kTOFbeta);
    histos->UserHistogram("Track","TOFnSigmaEle_P","TOF number of sigmas Electrons;#it{p} (GeV/#it{c});TOF number of sigmas Electrons",
                          400,0.,20.,600,-15.,15.,AliDielectronVarManager::kPIn,AliDielectronVarManager::kTOFnSigmaEle);
    ...
    if (kPairing){
        ...
        histos->UserHistogram("Pair","InvMass","Inv.Mass;#it{m}_{ee} (GeV/#it{c}^{2});N of pairs", 500,0.,5.,AliDielectronVarManager::kM);
        histos->UserHistogram("Pair","InvMass_arb","Inv.Mass_arb;#it{m}_{ee} (GeV/#it{c}^{2});N of pairs", GetVector(kMee),AliDielectronVarManager::kM);
        ...
        // --- 2d ----------------------------------------------------------
	histos->UserHistogram("Pair","InvMass_PairPt","InvMass_PairPt;#it{m}_{ee} (GeV/#it{c}^{2});Pair Pt (GeV/#it{c}^{2})",500,0.,5.,300,0.,6.,
			      AliDielectronVarManager::kM, AliDielectronVarManager::kPt);
	histos->UserHistogram("Pair","InvMass_PairPt_arb","InvMass_PairPt_arb;#it{m}_{ee} (GeV/#it{c}^{2});Pair Pt (GeV/#it{c}^{2})",GetVector(kMee),GetVector(kPtLinear),
			      AliDielectronVarManager::kM, AliDielectronVarManager::kPt);
        histos->UserHistogram("Pair","InvMass_OpAngle","InvMass_OpAngle;#it{m}_{ee} (GeV/#it{c}^{2});Opening angle", 200, 0. , 4. , 180, 0. , TMath::Pi() ,
			      AliDielectronVarManager::kM, AliDielectronVarManager::kOpeningAngle);
        histos->UserHistogram("Pair","OpAngle_PhiV","OpAngle_PhiV;Opening angle;#phi_{V}",180, 0. , TMath::Pi(), 180 , 0. , TMath::Pi() ,
...
TVectorD *GetVector(Int_t var)
{
    ...
    case kMee:    return AliDielectronHelper::MakeArbitraryBinning("0., 0.02, 0.04, 0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18, 0.2, 0.22, 0.24, 0.26, 0.28, 0.32, 0.38, 0.54, 0.66, 0.72, 0.76, 0.78, 0.86, 0.98, 1.00, 1.02, 1.1, 1.2, 1.4, 1.64, 1.96, 2.34, 2.84, 2.95, 3.05, 3.1, 3.15, 3.3, 3.5, 3.75, 4.0, 5.0");
        case kMeeLinear:      return AliDielectronHelper::MakeLinBinning(500, 0., 5.);
        case kPtLinear:       return AliDielectronHelper::MakeLinBinning(500, 0., 6.);
...
void SetupCuts(AliDielectron *die, Int_t cutDefinition)
{
  ...
  //V0track cuts
  if(cutDefinition == 2) die->GetTrackFilter().AddCuts(SetV0Cuts(cutDefinition));
...
AliDielectronPID *SetPIDcuts(Int_t cutDefinition){
  ...
  }else if(cutDefinition == 1){//TPCHadRej
    pid->AddCut(AliDielectronPID::kTPC, AliPID::kElectron,   -3. ,    3.,   0.0, 1e30,  kFALSE,  AliDielectronPID::kRequire, AliDielectronVarManager::kPt);
    pid->AddCut(AliDielectronPID::kTPC, AliPID::kPion,     -100. ,    4.,   0.0, 1e30,  kTRUE,   AliDielectronPID::kRequire, AliDielectronVarManager::kPt);
    pid->AddCut(AliDielectronPID::kTPC, AliPID::kKaon,       -4. ,    4.,   0.0, 1e30,  kTRUE,   AliDielectronPID::kRequire, AliDielectronVarManager::kPt);
    pid->AddCut(AliDielectronPID::kTPC, AliPID::kProton,     -4. ,    4.,   0.0, 1e30,  kTRUE,   AliDielectronPID::kRequire, AliDielectronVarManager::kPt);  
  }else if(cutDefinition == 2){//TPCHadRejTOFif
    pid->AddCut(AliDielectronPID::kTPC, AliPID::kElectron,   -3. ,    3.,   0.0, 1e30,  kFALSE,  AliDielectronPID::kRequire, AliDielectronVarManager::kPt);
    pid->AddCut(AliDielectronPID::kTPC, AliPID::kPion,     -100. ,    4.,   0.0, 1e30,  kTRUE,   AliDielectronPID::kRequire, AliDielectronVarManager::kPt);
    pid->AddCut(AliDielectronPID::kTPC, AliPID::kKaon,       -4. ,    4.,   0.0, 1e30,  kTRUE,   AliDielectronPID::kRequire, AliDielectronVarManager::kPt);
    pid->AddCut(AliDielectronPID::kTPC, AliPID::kProton,     -4. ,    4.,   0.0, 1e30,  kTRUE,   AliDielectronPID::kRequire, AliDielectronVarManager::kPt);
    pid->AddCut(AliDielectronPID::kTOF, AliPID::kElectron,   -3. ,    3.,   0.4, 1e30,  kFALSE,  AliDielectronPID::kIfAvailable, AliDielectronVarManager::kP);
  }
  return pid;
}
