AliAnalysisTask *AddTask_miweber_LMEE_TMVA(Double_t userTMVACutValue = 0.0,
  ...
  if (!mgr) {
    Error("AddTask_miweber_LMEE_TMVA", "No analysis manager found.");
    return 0;
  }
  ...
  TString configFile("Config_miweber_LMEE_TMVA.C");
  ...
  if (mgr->GetInputEventHandler()->IsA()==AliAODInputHandler::Class()){
    ::Info("AddTask_miweber_LMEE_TMVA", "no dedicated AOD configuration");
  }
  else if (mgr->GetInputEventHandler()->IsA()==AliESDInputHandler::Class()){
    ::Info("AddTask_miweber_LMEE_TMVA","switching on ESD specific code");
    bESDANA=kTRUE;
  }
  
  //create task and add it to the manager
  AliAnalysisTaskMultiDielectron *task=new AliAnalysisTaskMultiDielectron("MultiDiEData_miweber_TMVA");
  ...
  AliDielectron *diel_low = Config_miweber_LMEE_TMVA(userTMVACutValue,cutDefinition,bESDANA,bCutQA,kFALSE,useTPCCorr);
  if(diel_low){
