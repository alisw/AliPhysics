AliAnalysisTask *AddTask_scheid_ElectronEfficiency_r5(Bool_t getFromAlien=kFALSE,
                                                     ...
                                                     TString resolutionfile = "pp_resolution_deltaXvsX.root"
                                                     )
{
  ...
  if (!mgr) {
    Error("AddTask_rbailhac_ElectronEfficiency", "No analysis manager found.");
    return 0;
  }
  ...
  AliAnalysisTaskElectronEfficiency *task = new AliAnalysisTaskElectronEfficiency("rbailhac_ElectronEfficiency");
  std::cout << "task created: " << task->GetName() << std::endl;

  if(CalcEfficiencyRec && !resolutionfile.IsNull() &&
     (!gSystem->Exec(Form("alien_cp alien:///alice/cern.ch/user/r/rbailhac/supportFiles/%s .",resolutionfile.Data()))) ){
  ...
  // pair efficiency
  if(doPairing){
    task->SetKineTrackCuts(SetupTrackCutsAndSettings(100)); // obsolte do not do anything
    task->SetPtCut(minptfiducial,maxptfiducial);
    task->SetEtaCut(minetafiducial,maxetafiducial);
  ...
  //
  AliAnalysisDataContainer *coutput1 = mgr->CreateContainer("rbailhac_ElectronEfficiency", TList::Class(),
                                                           AliAnalysisManager::kOutputContainer,outputFileName);
  AliAnalysisDataContainer *coutput2 = mgr->CreateContainer("rbailhac_supportHistos", TList::Class(),
                                                            AliAnalysisManager::kOutputContainer,outputFileName);
  AliAnalysisDataContainer *coutput3 = mgr->CreateContainer("rbailhac_EffTree", TTree::Class(),
                                                            AliAnalysisManager::kOutputContainer,outputFileName);
  AliAnalysisDataContainer *coutput4 = mgr->CreateContainer("rbailhac_stats", TH1D::Class(),
                                                            AliAnalysisManager::kOutputContainer,outputFileName);
