      ...
      fCutName("")
  {
    ...
    AliAnalysisCuts *SetupTrackCuts(){
      ...
      if(fCutName.Contains("PIDCalib",TString::kIgnoreCase)){
        ...
        ////index = 0 is always default value
        //const Double_t min_kNclsITS[3]        = {3.99, 2.99, 4.99};
        //const Double_t max_kITSchi2Cl[3]      = {5, 4, 6};
        //const Double_t min_kNFclsTPCr[3]      = {80, 70, 120};
        //const Double_t min_kNFclsTPCfCross[2] = {0.8, 0.9}:
        //const Double_t max_kTPCchi2Cl[2]      = {2.0, 2.5};
        ////in total, 108 variations.

        if(fCutName.Contains("track0",TString::kIgnoreCase)){//default
        ...
        else if(fCutName.Contains("track1",TString::kIgnoreCase)){//tightest
          varCuts->AddCut(AliDielectronVarManager::kNclsITS,  4.9,6.1);
          varCuts->AddCut(AliDielectronVarManager::kITSchi2Cl,0.0,5.0);
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCr,     120.0, 160.0);//crossed rows
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCfCross, 0.9,    2.);
          varCuts->AddCut(AliDielectronVarManager::kTPCchi2Cl,      0.0,    2.);
        }
        else if(fCutName.Contains("track2",TString::kIgnoreCase)){//loosest
          varCuts->AddCut(AliDielectronVarManager::kNclsITS,  2.9,6.1);
          varCuts->AddCut(AliDielectronVarManager::kITSchi2Cl,0.0,6.0);
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCr,     70.0, 160.0);//crossed rows
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCfCross, 0.8,    2.);
          varCuts->AddCut(AliDielectronVarManager::kTPCchi2Cl,      0.0,   2.5);
        }
        else if(fCutName.Contains("track3",TString::kIgnoreCase)){//only tight for TPC related values, default values to ITS
          varCuts->AddCut(AliDielectronVarManager::kNclsITS,  3.9,6.1);
          varCuts->AddCut(AliDielectronVarManager::kITSchi2Cl,0.0,5.0);
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCr,     120.0, 160.0);//crossed rows
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCfCross, 0.8,    2.);
        ...
        else if(fCutName.Contains("track4",TString::kIgnoreCase)){//only tight for ITS related values, default values to TPC
          varCuts->AddCut(AliDielectronVarManager::kNclsITS,  4.9,6.1);
          varCuts->AddCut(AliDielectronVarManager::kITSchi2Cl,0.0,4.0);
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCr,     80.0, 160.0);//crossed rows
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCfCross, 0.8,    2.);
          varCuts->AddCut(AliDielectronVarManager::kTPCchi2Cl,      0.0,   2.5);
        }
        else if(fCutName.Contains("track5",TString::kIgnoreCase)){//only loose for TPC related values, default values to ITS
          varCuts->AddCut(AliDielectronVarManager::kNclsITS,  3.9,6.1);
          varCuts->AddCut(AliDielectronVarManager::kITSchi2Cl,0.0,5.0);
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCr,     70.0, 160.0);//crossed rows
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCfCross, 0.8,    2.);
        ...
        else if(fCutName.Contains("track6",TString::kIgnoreCase)){//only loose for ITS related values, default values to TPC
          varCuts->AddCut(AliDielectronVarManager::kNclsITS,  2.9,6.1);
          varCuts->AddCut(AliDielectronVarManager::kITSchi2Cl,0.0,6.0);
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCr,     80.0, 160.0);//crossed rows
          varCuts->AddCut(AliDielectronVarManager::kNFclsTPCfCross, 0.8,    2.);
        ...
        if(fCutName.Contains("Nsc",TString::kIgnoreCase)){
          AliDielectronCutGroup *ITSscCG = new AliDielectronCutGroup("ITSscCutsGroup","ITSscCutsGroup",AliDielectronCutGroup::kCompOR);
    ...
    AliAnalysisCuts *SetupPIDCuts(){
      if(fCutName.Contains("pid0",TString::kIgnoreCase)){
        ...
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kPion    ,-100. ,3.5 ,0.0, 1e+10, kTRUE ,AliDielectronPID::kRequire    ,AliDielectronVarManager::kPt);
        cuts->AddCut(cutsPID);
        return cuts;
      }
      else if(fCutName.Contains("pid1",TString::kIgnoreCase)){//tight
        AliDielectronCutGroup* cuts = new AliDielectronCutGroup("cuts", "cuts", AliDielectronCutGroup::kCompAND);
        AliDielectronPID* cutsPID   = new AliDielectronPID("PID", "PID");
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kElectron,  -1.5 ,2.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire    ,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kITS,AliPID::kElectron,  -2.5 ,0.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire    ,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kTOF,AliPID::kElectron,  -2.5 ,2.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kIfAvailable,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kPion    ,-100. ,4.0 ,0.0, 1e+10, kTRUE ,AliDielectronPID::kRequire    ,AliDielectronVarManager::kPt);
        cuts->AddCut(cutsPID);
        return cuts;
      }
      else if(fCutName.Contains("pid2",TString::kIgnoreCase)){//loose
        AliDielectronCutGroup* cuts = new AliDielectronCutGroup("cuts", "cuts", AliDielectronCutGroup::kCompAND);
        AliDielectronPID* cutsPID   = new AliDielectronPID("PID", "PID");
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kElectron,  -3.5 ,3.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire    ,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kITS,AliPID::kElectron,  -3.5 ,1.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire    ,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kTOF,AliPID::kElectron,  -3.5 ,3.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kIfAvailable,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kPion    ,-100. ,3.0 ,0.0, 1e+10, kTRUE ,AliDielectronPID::kRequire    ,AliDielectronVarManager::kPt);
        cuts->AddCut(cutsPID);
        return cuts;
      }
      else if(fCutName.Contains("pid3",TString::kIgnoreCase)){//tight and TOFreq
        AliDielectronCutGroup* cuts = new AliDielectronCutGroup("cuts", "cuts", AliDielectronCutGroup::kCompAND);
        AliDielectronPID* cutsPID   = new AliDielectronPID("PID", "PID");
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kElectron,  -1.5 ,2.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kITS,AliPID::kElectron,  -2.5 ,0.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kTOF,AliPID::kElectron,  -2.5 ,2.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kPion    ,-100. ,4.0 ,0.0, 1e+10, kTRUE ,AliDielectronPID::kRequire  ,AliDielectronVarManager::kPt);
        cuts->AddCut(cutsPID);
        return cuts;
      }
      else if(fCutName.Contains("pid4",TString::kIgnoreCase)){//loose and TOFreq
        AliDielectronCutGroup* cuts = new AliDielectronCutGroup("cuts", "cuts", AliDielectronCutGroup::kCompAND);
        AliDielectronPID* cutsPID   = new AliDielectronPID("PID", "PID");
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kElectron,  -3.5 ,3.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kITS,AliPID::kElectron,  -3.5 ,1.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kTOF,AliPID::kElectron,  -3.5 ,3.5  ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire,AliDielectronVarManager::kPt);
        cutsPID->AddCut(AliDielectronPID::kTPC,AliPID::kPion    ,-100. ,3.0 ,0.0, 1e+10, kTRUE ,AliDielectronPID::kRequire  ,AliDielectronVarManager::kPt);
        cuts->AddCut(cutsPID);
        return cuts;
      }
      ...
      //else if(fCutName.Contains("pid1",TString::kIgnoreCase)){
    ...
    AliAnalysisCuts *SetupPIDCutsForTag(){
      ...
      cutsPIDtag->AddCut(AliDielectronPID::kITS,AliPID::kElectron,  -3. ,1. ,0.0, 1e+10, kFALSE,AliDielectronPID::kRequire,AliDielectronVarManager::kPt);
      return cutsPIDtag;
    }
