AliAnalysisTask *AddTask_reichelt_ElectronEfficiency_0010(Char_t* outputFileName="reichelt_ElectronEfficiency_0010.root", 
                                                     ...
                                                     )
{
  ...
  if (!mgr) {
    Error("AddTask_reichelt_ElectronEfficiency_0010", "No analysis manager found.");
    return 0;
  }
  ...
  if (getFromAlien //&&
      && (!gSystem->Exec("alien_cp alien:///alice/cern.ch/user/p/preichel/PWGDQ/dielectron/macrosLMEE/Config_reichelt_ElectronEfficiency_0010.C ."))
      && (!gSystem->Exec("alien_cp alien:///alice/cern.ch/user/p/preichel/PWGDQ/dielectron/macrosLMEE/LMEECutLib_reichelt.C ."))
      ) {
    configBasePath=Form("%s/",gSystem->pwd());
  }
  
  TString configFile("Config_reichelt_ElectronEfficiency_0010.C");
  ...
  if (mgr->GetInputEventHandler()->IsA()==AliAODInputHandler::Class()){
    ::Info("AddTaskElectronEfficiency_0010","running on AODs.");
  }
  else if (mgr->GetInputEventHandler()->IsA()==AliESDInputHandler::Class()){
    ::Info("AddTaskElectronEfficiency_0010","switching on ESD specific code, make sure ESD cuts are used.");
    bESDANA=kTRUE;
  }
