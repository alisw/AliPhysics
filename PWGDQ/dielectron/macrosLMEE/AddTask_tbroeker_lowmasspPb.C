...
//  taskMB->SetRejectPileup();
  ...
  if(RunHighMulttrigger){
    //HighMult
    AliDielectron *diel_lowHighMult = Config_lowmasspPb(i);
    if(!diel_lowHighMult)continue;
    diel_lowHighMult->SetNoPairing(kTRUE);
    taskHighMult->AddDielectron(diel_lowHighMult);
}
	  if(RunEMCtrigger){
    //EMC1 
    AliDielectron *diel_lowEMC1 = Config_lowmasspPb(i);
    if(!diel_lowEMC1)continue;
    diel_lowEMC1->SetNoPairing(kTRUE);
...
//  AliAnalysisDataContainer *cOutputHist2 =
//    mgr->CreateContainer("lowmass_tbroeker_EventStat",
//                         TList::Class(),
//                         AliAnalysisManager::kOutputContainer,
//                         "tbroeker_lowmass.root");
...
if(RunHighMulttrigger){
  ...
  AliAnalysisDataContainer *cOutputHistHighMult3 =
    mgr->CreateContainer("lowmass_tbroeker_EventStatHighMult",
                         TList::Class(),
                         AliAnalysisManager::kOutputContainer,
                         "lowmass_lowmassHighMult.root");
	}
  ...
  AliAnalysisDataContainer *cOutputHistEMC13 =
    mgr->CreateContainer("lowmass_tbroeker_EventStatEMC1",
                         TList::Class(),
                         AliAnalysisManager::kOutputContainer,
                         "lowmass_lowmassEMC1.root");
}
...
if(RunMBtrigger)       return taskMB;
if(RunHighMulttrigger) return taskHighMult;
if(RunEMCtrigger)      return taskEMC1;
