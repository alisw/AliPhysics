...
AliAnalysisTask* AddTask_raul_PbPb(TString cfg="ConfigJpsi_raul_PbPb",
  ...
  if (!mgr) {
    Error("AddTask_raul_PbPb", "No analysis manager found.");
    return 0;
  }
  ...
  AliAnalysisTaskMultiDielectron *task=new AliAnalysisTaskMultiDielectron("Jpsi_PbPb");
...
// set config file name
  ...
  if(cfg.IsNull()) cfg="ConfigJpsi_raul_PbPb";
  // the different paths
