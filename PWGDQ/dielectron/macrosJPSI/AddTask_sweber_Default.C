...
AliDielectron* ConfigDefault(Int_t cutDefinition)
{
  ...
  ConfDef::SetupEventCuts(die,cutDefinition);
...
void SetupTrackCuts(AliDielectron *die, Int_t cutDefinition)
{
  ...
  //
	...
	if (cutDefinition==ConfDef::kDefault|| cutDefinition==ConfDef::kLoose){
	...
	}else if( cutDefinition==ConfDef::kFirst  || cutDefinition==ConfDef::kStrict ){
		refit->SetRequireITSRefit(kTRUE);
		refit->SetRequireTPCRefit(kTRUE);
