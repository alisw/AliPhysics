...
#include "AliAnalysisTaskNewEtaReconstruction.h"
#include "AliVTrack.h"
...
AliAnalysisTaskNewEtaReconstruction::~AliAnalysisTaskNewEtaReconstruction(){
  delete fPtPion;
  delete fPtEta;
  delete fPtEtaPrime;
  delete fPtRho;
...
// ############################################################################
// ############################################################################
AliAnalysisTaskNewEtaReconstruction::AliAnalysisTaskNewEtaReconstruction(): AliAnalysisTaskSE(), fEventFilter(0x0),fdebug(false)
                                                                              ...
                                                                              , fHistGenPair(), fHistGenFourPair(), fHistGenSmearedPair(), fHistGenSmearedFourPair(), fHistRecPair(), fHistRecFourPair(), fHistGenPair_ULSandLS(), fHistGenFourPair_ULSandLS(), fHistGenSmearedPair_ULSandLS(), fHistGenSmearedFourPair_ULSandLS(), fWriteLegsFromPair(false), fHistRecPair_ULSandLS(), fHistRecFourPair_ULSandLS(), fTHnSparseGenSmearedLegsFromPair(), fTHnSparseRecLegsFromPair(), fPtMinLegsFromPair(-99.), fPtMaxLegsFromPair(-99.), fEtaMinLegsFromPair(-99.), fEtaMaxLegsFromPair(-99.), fPhiMinLegsFromPair(-99.), fPhiMaxLegsFromPair(-99.), fOpAngleMinLegsFromPair(-99.), fOpAngleMaxLegsFromPair(-99.), fPtNBinsLegsFromPair(-99), fEtaNBinsLegsFromPair(-99), fPhiNBinsLegsFromPair(-99), fOpAngleNBinsLegsFromPair(-99)
...
// ############################################################################
// ############################################################################
AliAnalysisTaskNewEtaReconstruction::AliAnalysisTaskNewEtaReconstruction(const char * name) : AliAnalysisTaskSE(name), fEventFilter(0x0), fdebug(false)
                                                                              , fResoFile(0x0), fResoFilename(""), fResoFilenameFromAlien(""), fArrResoPt(0x0), fArrResoEta(0x0), fArrResoPhi_Pos(0x0), fArrResoPhi_Neg(0x0)
                                                                              , fOutputList(0x0), fSingleElectronList(0x0), fPairList(0x0), fFourPairList(0x0), fResolutionList(0x0)
                                                                              , fPGen_DeltaP(0x0), fPGen_PrecOverPGen(0x0), fPtGen_DeltaPt(0x0), fPtGen_DeltaPtOverPtGen(0x0), fPtGen_PtRecOverPtGen(0x0), fPtGen_DeltaPt_wGenSmeared(0x0), fPtGen_DeltaPtOverPtGen_wGenSmeared(0x0), fPtGen_PtRecOverPtGen_wGenSmeared(0x0)
                                                                              , fPGen_DeltaEta(0x0), fPtGen_DeltaEta(0x0), fPGen_DeltaTheta(0x0), fPGen_DeltaPhi_Ele(0x0), fPGen_DeltaPhi_Pos(0x0), fPtGen_DeltaPhi_Ele(0x0)
...
// ############################################################################
// ############################################################################
void AliAnalysisTaskNewEtaReconstruction::Terminate(Option_t* option){
...
// ############################################################################
// ############################################################################
void AliAnalysisTaskNewEtaReconstruction::UserCreateOutputObjects(){
...
// ############################################################################
// ############################################################################
void AliAnalysisTaskNewEtaReconstruction::UserExec(Option_t* option){
  ...
  for (Int_t iTracks = 0; iTracks < fEvent->GetNumberOfTracks(); iTracks++){
    ...
    // ##########################################################
    // pushing primarys into primary_pos/neg_vector
      if      ((fDoPairing == true || fDoFourPairing == true) && part.fCharge <  0) fRecNegPart.push_back(part);
      else if ((fDoPairing == true || fDoFourPairing == true) && part.fCharge >  0) fRecPosPart.push_back(part);
...
//         } // End of ULS
    ...
    // Fill reconstructed pairs
                                                                                if(fdebug) std::cout << __LINE__ << " DEBUG_AnalysisTask: Start Reconstructed Two Pairing" << std::endl;
...
// ############################################################################
// ############################################################################
void    AliAnalysisTaskNewEtaReconstruction::FillTrackHistograms(AliVParticle* track, AliVParticle* mcTrack){
...
// ############################################################################
// ############################################################################
AliAnalysisTaskNewEtaReconstruction::Particle AliAnalysisTaskNewEtaReconstruction::CreateParticle(AliVParticle* mcPart1){
...
void AliAnalysisTaskNewEtaReconstruction::CheckIfFromMotherWithDielectronAsDaughter(Particle& part){
...
// ############################################################################
// ############################################################################
Bool_t AliAnalysisTaskNewEtaReconstruction::CheckIfOneIsTrue(std::vector<Bool_t>& vec){
...
// ############################################################################
// ############################################################################
void AliAnalysisTaskNewEtaReconstruction::SetBinsLinear(const std::string var, const double min, const double max, const unsigned int steps){
...
// ############################################################################
// ############################################################################
void AliAnalysisTaskNewEtaReconstruction::CheckSingleLegMCsignals(std::vector<Bool_t>& vec, const int tracklabel){
  for (unsigned int i = 0; i < fSingleLegMCSignal.size(); ++i){
...
// ############################################################################
// ############################################################################
void AliAnalysisTaskNewEtaReconstruction::CreateSupportHistos()
{
...
// ############################################################################
// ############################################################################
TLorentzVector AliAnalysisTaskNewEtaReconstruction::ApplyResolution(double pt, double eta, double phi, short ch) {
...
// ############################################################################
// ############################################################################
Double_t AliAnalysisTaskNewEtaReconstruction::GetSmearing(TObjArray *arr, Double_t x)
{
  TH1D *hisSlice(0x0);
...
bool AliAnalysisTaskNewEtaReconstruction::CheckGenerator(int trackID, std::vector<unsigned int> vecHashes){
...
double AliAnalysisTaskNewEtaReconstruction::GetWeight(Particle part1, Particle part2, double motherpt){
  int pdgMother = 0;
  double weight = 0;
  int bin = -1;
...
void AliAnalysisTaskNewEtaReconstruction::SetCentroidCorrFunction(Detector det, TObject *fun, UInt_t varx, UInt_t vary, UInt_t varz)
{
  if (fun == 0x0) {
...
//______________________________________________
void AliAnalysisTaskNewEtaReconstruction::SetWidthCorrFunction(Detector det, TObject *fun, UInt_t varx, UInt_t vary, UInt_t varz)
{
  if (fun == 0x0) {
    std::cout << "No correction function chosen" << std::endl;
    return;
