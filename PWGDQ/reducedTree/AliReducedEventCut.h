...
class AliReducedEventCut : public AliReducedVarCut {
 ...
 public:
  ...
  void AddEventTagFilter(ULong64_t filter, Bool_t exclude = kFALSE) {
    fEventFilter |= filter;
 ...
 protected: 
  ...
  Bool_t    fEventTagFilterExclude;   // if true apply the filter to the event tag as exclusion cut
