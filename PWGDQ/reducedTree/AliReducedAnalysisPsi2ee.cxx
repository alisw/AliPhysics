...
// Author: Jinjoo Seo, jseo@cern.ch
// Inspired from AliReducedAnalysisJpsi2ee
#include <AliReducedAnalysisPsi2ee.h>
...
ClassImp(AliReducedAnalysisPsi2ee);


//___________________________________________________________________________
AliReducedAnalysisPsi2ee::AliReducedAnalysisPsi2ee() :
  AliReducedAnalysisTaskSE(),
  fHistosManager(new AliHistogramManager("Histogram Manager", AliReducedVarManager::kNVars)),
  fMixingHandler(new AliMixingHandler("ccbar signal extraction","", AliMixingHandler::kMixResonanceLegs)),
  ...
  fOptionStoreccbarCandidates(kFALSE),
  fFillCaloClusterHistograms(kFALSE),
  fOptionStoreTree(kFALSE),
  fEventCuts(),
  fClusterCuts(),
  fTrackCuts(),
  fPreFilterTrackCuts(),
  ...
  fLeg1TreeOutput(0x0),
  fLeg2TreeOutput(0x0),
  fMCTruthTreeOutput(0x0),
  fTreeVariable(0x0),
  fPairTreeVariable(0x0),
  fLeg1TreeVariable(0x0),
  fLeg2TreeVariable(0x0),
  fMCTruthTreeVariable(0x0),
...
//___________________________________________________________________________
AliReducedAnalysisPsi2ee::AliReducedAnalysisPsi2ee(const Char_t* name, const Char_t* title) :
...
AliReducedAnalysisPsi2ee::~AliReducedAnalysisPsi2ee() 
{
  ...
  //
   ...
   fccbarCandidates.Clear("C");
   if(fPairTreeOutput) delete fPairTreeOutput;
   if(fLeg1TreeOutput) delete fLeg1TreeOutput;
   if(fLeg2TreeOutput) delete fLeg2TreeOutput;
   if(fMCTruthTreeOutput) delete fMCTruthTreeOutput;
   if(fTreeVariable) delete fTreeVariable;
   if(fPairTreeVariable) delete fPairTreeVariable;
   if(fLeg1TreeVariable) delete fLeg1TreeVariable;
   if(fLeg2TreeVariable) delete fLeg2TreeVariable;
   if(fMCTruthTreeVariable) delete fMCTruthTreeVariable;
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::AddTrackCut(AliReducedInfoCut* cut) {
   //
   // Add a new cut
   //
   fTrackCuts.Add(cut);
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::AddPairCut(AliReducedInfoCut* cut) {
   //
   // Add a new cut
   //
   fPairCuts.Add(cut);
...
//___________________________________________________________________________
Bool_t AliReducedAnalysisPsi2ee::IsEventSelected(AliReducedBaseEvent* event, Float_t* values/*=0x0*/) {
  //
  // apply event cuts
  //
...
//___________________________________________________________________________
Bool_t AliReducedAnalysisPsi2ee::IsClusterSelected(AliReducedCaloClusterInfo* cluster, Float_t* values/*=0x0*/) {
  //
  // apply cluster cuts
  //
...
//___________________________________________________________________________
Bool_t AliReducedAnalysisPsi2ee::IsTrackSelected(AliReducedBaseTrack* track, Float_t* values/*=0x0*/) {
  //
  // apply event cuts
  //
...
//___________________________________________________________________________
Bool_t AliReducedAnalysisPsi2ee::IsTrackPrefilterSelected(AliReducedBaseTrack* track, Float_t* values/*=0x0*/) {
   //
...
//___________________________________________________________________________
ULong_t AliReducedAnalysisPsi2ee::IsPairSelected(Float_t* values) {
  //
  // apply pair cuts
  //
...
//___________________________________________________________________________
Bool_t AliReducedAnalysisPsi2ee::IsPairPreFilterSelected(Float_t* values) {
   //
   // apply event cuts
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::Init() { 
  //
  // initialize stuff
  //
   AliReducedVarManager::SetDefaultVarNames();
   AliReducedVarManager::SetUseVariable(AliReducedVarManager::kPairOpeningAngle);
   AliReducedVarManager::SetUseVariable(AliReducedVarManager::kPairPointingAngle);
   AliReducedVarManager::SetUseVariable(AliReducedVarManager::kPairPointingAngle);
   AliReducedVarManager::SetUseVariable(AliReducedVarManager::kPairChisquare);
   ...
   AliReducedVarManager::SetUseVariable(AliReducedVarManager::kITSlayerHit);
   AliReducedVarManager::SetUseVariable(AliReducedVarManager::kITSncls);
   AliReducedVarManager::SetUseVariable(AliReducedVarManager::kTPCcrossedRowsOverFindableClusters);
   AliReducedVarManager::SetUseVariable(AliReducedVarManager::kPairLegPtMC);
   // make sure variables needed to create jpsi candidate objects are filled
   if(fOptionStoreccbarCandidates) {
      ...
      AliReducedVarManager::SetUseVariable(AliReducedVarManager::kPt_PsiPrime);
      AliReducedVarManager::SetUseVariable(AliReducedVarManager::kPhi_PsiPrime);
      AliReducedVarManager::SetUseVariable(AliReducedVarManager::kEta_PsiPrime);
      AliReducedVarManager::SetUseVariable(AliReducedVarManager::kMassPsiPrime);
  ...
  DefineTree();
  PostData(1, fPairTreeOutput);
  PostData(2, fLeg1TreeOutput);
  PostData(3, fLeg2TreeOutput);
  PostData(4, fMCTruthTreeOutput);
}


//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::Process() {
  //
  ...
  if(!(fEvent->IsA()==AliReducedEventInfo::Class())) {
     cout << "ERROR: AliReducedAnalysisPsi2ee::Process() needs AliReducedEventInfo events" << endl;
     return;
  }
  ...
  // select tracks
  if(fOptionLoopOverTracks) RunTrackSelection();
  ...
  if(fOptionLoopOverTracks && fOptionRunPrefilter) RunPrefilter();
  ...
  if(fOptionLoopOverTracks) FillTrackHistograms();
  ...
  if(fOptionRunMixing){
    ...
    fMixingHandler->FillEvent(&fPosTracks, &fNegTracks, fValues, AliReducedPairInfo::kPsi2SToEE); 
  }
  
  // Do the same event pairing
  if(fOptionRunPairing) RunSameEventPairing();
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillTrackHistograms(TString trackClass /*= "Track"*/) {
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillTrackHistograms(AliReducedBaseTrack* track, TString trackClass /*="Track"*/) {
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillPairHistograms(ULong_t trackMask, ULong_t pairMask, Int_t pairType, TString pairClass /*="PairSE"*/, UInt_t mcDecisions /* = 0*/) {
   //
      ...
      for(Int_t iTrackCut=0; iTrackCut<fTrackCuts.GetEntries(); ++iTrackCut) {
         for(Int_t iPairCut=0; iPairCut<fPairCuts.GetEntries(); ++iPairCut) {
            if((trackMask & (ULong_t(1)<<iTrackCut)) && (pairMask & (ULong_t(1)<<iPairCut))) {
               fHistosManager->FillHistClass(Form("%s%s_%s_%s", pairClass.Data(), typeStr[pairType].Data(), fTrackCuts.At(iTrackCut)->GetName(), fPairCuts.At(iPairCut)->GetName()), fValues);
               if(mcDecisions && pairType==1) {
                  for(Int_t iMC=0; iMC<=fLegCandidatesMCcuts.GetEntries(); ++iMC) {
                     if(mcDecisions & (UInt_t(1)<<iMC))
                        fHistosManager->FillHistClass(Form("%s%s_%s_%s_%s", pairClass.Data(), typeStr[pairType].Data(), fTrackCuts.At(iTrackCut)->GetName(), fPairCuts.At(iPairCut)->GetName(), fLegCandidatesMCcuts.At(iMC)->GetName()), fValues);
                  }
               }
            }
         }
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillClusterHistograms(TString clusterClass/*="CaloCluster"*/) {
  //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillClusterHistograms(AliReducedCaloClusterInfo* cluster, TString clusterClass/*="CaloCluster"*/) {
  //
  // fill cluster histograms
  //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::RunClusterSelection() {
  //
  // select cluster
  //
  fClusters.Clear("C");
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::RunTrackSelection() {
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::LoopOverTracks(Int_t arrayOption /*=1*/) {
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::RunSameEventPairing(TString pairClass /*="PairSE"*/) {
   //
   // Run the same event pairing
   //
   if(fOptionStoreccbarCandidates) fccbarCandidates.Clear("C");
   ...
   for(Int_t ip=0; ip<fPosTracks.GetEntries(); ++ip) {
      ...
      for(Int_t in=0; in<fNegTracks.GetEntries(); ++in) {
         ...
         AliReducedVarManager::FillPairInfo(pTrack, nTrack, AliReducedPairInfo::kJpsiToEE, fValues); // kJpsiToEE + kPsi2SToEE
         ...
         if(pairCutMask) {
            ...
            if(fOptionStoreccbarCandidates) {
               ...
               pair->CandidateId(AliReducedPairInfo::kJpsiToEE); //dummy
               pair->PairType(1);
               ...
               fccbarCandidates.Add(pair);
            }

            Int_t iTrackCut = 0;
            Int_t iPairCut = 0;
            if (fOptionStoreTree) {
               for(TObject* iter1 : fTrackCuts) {
                  for(TObject* iter2 : fPairCuts) {
                     ULong_t trkCut = (pTrack->GetFlags() & nTrack->GetFlags());
                     Bool_t pairCut = (fPairCuts.GetEntries()<=1? kTRUE: (pairCutMask & (ULong_t(1)<<iPairCut)));
                     if((trkCut & (ULong_t(1)<<iTrackCut)) && pairCut) {
                        FillPairTree(fValues, (fOptionRunOverMC ? CheckReconstructedLegMCTruth(pTrack, nTrack) : 0));
                        
                        AliReducedBaseTrack* pTrackInfo = (AliReducedBaseTrack*) pTrack;
                        AliReducedVarManager::FillTrackInfo(pTrackInfo, fValues);
                        FillLegTree(fValues, 1, (fOptionRunOverMC ? CheckReconstructedLegMCTruth(pTrack, nTrack) : 0));
                        AliReducedBaseTrack* nTrackInfo = (AliReducedBaseTrack*) nTrack;
                        AliReducedVarManager::FillTrackInfo(nTrackInfo, fValues);
                        FillLegTree(fValues, 2, (fOptionRunOverMC ? CheckReconstructedLegMCTruth(pTrack, nTrack) : 0));
                     }
                     iPairCut++;
                  }
                  iTrackCut++;
      ...
      if(fOptionRunLikeSignPairing) {
         for(Int_t ip2=ip+1; ip2<fPosTracks.GetEntries(); ++ip2) {
            ...
            AliReducedVarManager::FillPairInfo(pTrack, pTrack2, AliReducedPairInfo::kJpsiToEE, fValues); // kJpsiToEE + kPsi2SToEE
            ...
            if(pairCutMask) {
               ...
               if (fOptionStoreTree) {
                  for(TObject* iter1 : fTrackCuts) {
                     for(TObject* iter2 : fPairCuts) {
                        ULong_t trkCut = (pTrack->GetFlags() & pTrack2->GetFlags());
                        Bool_t pairCut = (fPairCuts.GetEntries()<=1? kTRUE: (pairCutMask & (ULong_t(1)<<iPairCut)));
                         if((trkCut & (ULong_t(1)<<iTrackCut)) && pairCut) {
                           FillPairTree(fValues);

                           AliReducedBaseTrack* pTrackInfo = (AliReducedBaseTrack*) pTrack;
                           AliReducedVarManager::FillTrackInfo(pTrackInfo, fValues);
                           FillLegTree(fValues, 1);
                           AliReducedBaseTrack* pTrack2Info = (AliReducedBaseTrack*) pTrack2;
                           AliReducedVarManager::FillTrackInfo(pTrack2Info, fValues);
                           FillLegTree(fValues, 2);
                        }
                        iPairCut++;
                     }
                     iTrackCut++;
         ...
         for(Int_t in2=in+1; in2<fNegTracks.GetEntries(); ++in2) {
            ...
            if(pairCutMask) {
               ...
               if (fOptionStoreTree) {
                  for(TObject* iter1 : fTrackCuts) {
                     for(TObject* iter2 : fPairCuts) {
                        ULong_t trkCut = (nTrack->GetFlags() & nTrack2->GetFlags());
                        Bool_t pairCut = (fPairCuts.GetEntries()<=1? kTRUE: (pairCutMask & (ULong_t(1)<<iPairCut)));
                        if((trkCut & (ULong_t(1)<<iTrackCut)) && pairCut) {
                           FillPairTree(fValues);
                           ...
                           AliReducedVarManager::FillTrackInfo(nTrackInfo, fValues);
                           FillLegTree(fValues, 1);
                           AliReducedBaseTrack* nTrack2Info = (AliReducedBaseTrack*) nTrack2;
                           AliReducedVarManager::FillTrackInfo(nTrack2Info, fValues);
                           FillLegTree(fValues, 2);
                        }
                        iPairCut++;
                     }
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::RunPrefilter() {
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::Finish() {
  //
  ...
  //
   if(fOptionRunMixing && !fOptionRunOverMC){
     ...
     fMixingHandler->RunLeftoverMixing(AliReducedPairInfo::kPsi2SToEE); //TODO: CHECK
   }
}


//___________________________________________________________________________
UInt_t AliReducedAnalysisPsi2ee::CheckReconstructedLegMCTruth(AliReducedBaseTrack* track) {
   //
...
//___________________________________________________________________________
UInt_t AliReducedAnalysisPsi2ee::CheckReconstructedLegMCTruth(AliReducedBaseTrack* ptrack, AliReducedBaseTrack* ntrack) {
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillMCTruthHistograms() {
  //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::LoopOverMCTracks(Int_t trackArray /*=1*/) {
   //
   ...
   if((fMCJpsiPtWeights && !fMCPsiPtWeights) || (!fMCJpsiPtWeights && fMCPsiPtWeights)) {
      TH1F*  fMCPtWeights = 0x0;
      Int_t reducedPairInfo = 0;
      
      if(fMCJpsiPtWeights) {
         reducedPairInfo = AliReducedPairInfo::kJpsiToEE;
         fMCPtWeights = fMCJpsiPtWeights;
      }
      if(fMCPsiPtWeights) {
         reducedPairInfo = AliReducedPairInfo::kPsi2SToEE;
         fMCPtWeights = fMCPsiPtWeights;
      }
      ...
      for(Int_t it=0; it<trackList->GetEntries(); ++it) {
         ...
         UInt_t motherDecisions = CheckMotherMCTruth(mother,kTRUE,reducedPairInfo);
         if(!motherDecisions) continue;
         
         Double_t pt = mother->Pt();
         if(pt>fMCPtWeights->GetXaxis()->GetXmax()) 
            pt = fMCPtWeights->GetXaxis()->GetXmax();
         Double_t weight = fMCPtWeights->GetBinContent(fMCPtWeights->FindBin(pt));
   ...
   std::vector <TList*> mcMotherCuts = {&fJpsiMotherMCcuts, &fPsiMotherMCcuts};
   std::vector <UInt_t> signalsWithoutMother = {0, 0}; // Jpsi, Psi
   for(int i = 0; i < mcMotherCuts.size(); i++){
       // Loop through the MC signals and check whether there are required MC signals without a defined mother 
      for(Int_t j=0; j<mcMotherCuts[i]->GetEntries(); ++j) {
         AliReducedInfoCut* cut = (AliReducedInfoCut*)mcMotherCuts[i]->At(j);
         if(!((AliReducedTrackCut*)cut)->GetMCFilterMap()) 
            signalsWithoutMother[i] |= (UInt_t(1)<<j);
      }
      ...
      for(Int_t it=0; it<trackList->GetEntries(); ++it) {
         ...
         // apply selections on the jpsi mother
         UInt_t motherDecisions = CheckMotherMCTruth(mother, kFALSE, (i==0? AliReducedPairInfo::kJpsiToEE : AliReducedPairInfo::kPsi2SToEE));
         if(!motherDecisions) continue;
         ...
         FindJpsiTruthLegs(mother, daughter1Label, daughter2Label); //ccbar->e+e-   
         ...
         for(Int_t iCut = 0; iCut<mcMotherCuts[i]->GetEntries(); ++iCut) {
            if(!(motherDecisions & (UInt_t(1)<<iCut)))  continue;
            fHistosManager->FillHistClass(Form("%s_PureMCTruth_BeforeSelection", mcMotherCuts[i]->At(iCut)->GetName()), fValues);         
         }
         ...
         UInt_t daughter1Decisions = CheckDaughterMCTruth(daughter1, (i==0? AliReducedPairInfo::kJpsiToEE : AliReducedPairInfo::kPsi2SToEE));
         if(!daughter1Decisions) continue;
         UInt_t daughtersDecisions = daughter1Decisions & CheckDaughterMCTruth(daughter2, (i==0? AliReducedPairInfo::kJpsiToEE : AliReducedPairInfo::kPsi2SToEE));
         if(!daughtersDecisions) continue;
         
         for(Int_t iCut = 0; iCut<mcMotherCuts[i]->GetEntries(); ++iCut) {
            if(!(motherDecisions & (UInt_t(1)<<iCut)))  continue;
            if(!(daughtersDecisions & (UInt_t(1)<<iCut)))  continue;
            FillMCTruthTree(fValues, iCut);
            fHistosManager->FillHistClass(Form("%s_PureMCTruth_AfterSelection", mcMotherCuts[i]->At(iCut)->GetName()), fValues);         
         }
      }  // end loop over tracks
      ...
      if(signalsWithoutMother[i]) {
         ...
         for(Int_t it=0; it<trackList->GetEntries(); ++it) {
            ...
            UInt_t daughterDecisions = CheckDaughterMCTruth(tempTrack, (i==0? AliReducedPairInfo::kJpsiToEE : AliReducedPairInfo::kPsi2SToEE));
            daughterDecisions &= signalsWithoutMother[i];
...
//___________________________________________________________________________
UInt_t AliReducedAnalysisPsi2ee::CheckMotherMCTruth(AliReducedTrackInfo* mother, Bool_t checkReweight, Int_t type /*= AliReducedPairInfo::kJpsiToEE*/) {
   //
   // Check the mother pure MC truth against all defined selections and return a bit map with all decisions
   //
   UInt_t decisionMap = 0;
   TList* mcMotherCuts;
   if(type == AliReducedPairInfo::kJpsiToEE) mcMotherCuts = &fJpsiMotherMCcuts;
   else if(type == AliReducedPairInfo::kPsi2SToEE) mcMotherCuts = &fPsiMotherMCcuts;
   else return decisionMap;

   if(mcMotherCuts->GetEntries()==0) return 0;
   for(Int_t i=0; i<mcMotherCuts->GetEntries(); ++i) {
   AliReducedInfoCut* cut = (AliReducedInfoCut*)mcMotherCuts->At(i);
...
//___________________________________________________________________________
UInt_t AliReducedAnalysisPsi2ee::CheckDaughterMCTruth(AliReducedTrackInfo* daughter, Int_t type /*= AliReducedPairInfo::kJpsiToEE*/) {
   //
   // Check the daughter pure MC truth against all defined selections and return a bit map with all decisions
   //
   UInt_t decisionMap = 0;
   TList* fElectronMCcuts;
   if(type == AliReducedPairInfo::kJpsiToEE) fElectronMCcuts = &fJpsiElectronMCcuts;
   else if(type == AliReducedPairInfo::kPsi2SToEE) fElectronMCcuts = &fPsiElectronMCcuts;
   else return decisionMap;

   if(fElectronMCcuts->GetEntries()==0) return 0;
   for(Int_t i=0; i<fElectronMCcuts->GetEntries(); ++i) {
   AliReducedInfoCut* cut = (AliReducedInfoCut*)fElectronMCcuts->At(i);
...
//___________________________________________________________________________
AliReducedTrackInfo* AliReducedAnalysisPsi2ee::FindMCtruthTrackByLabel(Int_t label) {
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FindJpsiTruthLegs(AliReducedTrackInfo* mother, Int_t& leg1Label, Int_t& leg2Label) {
   //
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::DefineTree()
{
	fPairTreeOutput = new TTree("fPairTreeOutput","fPairTreeOutput");
	std::vector<TString> fPairTreeVariableName =
	{
		"PairPt", "PairMass", "PairEta", "PairRapidity", "PairPhi", "PairOpeningAngle", "PairPointingAngle", "PairCosPointingAngle",
      "PairType", "PairTypeSPD", "PairChisquare", "PairLxy", "PairPseudoProperDecayTime",
      "MCPt", "MCMass", "MCEta", "MCRapidity", "MCPhi", "MCPseudoProperDecayTime", "Tag"
  };
   fPairTreeVariable = new Float_t [fPairTreeVariableName.size()];
   for (Int_t ivar=0; ivar<(Float_t)fPairTreeVariableName.size(); ivar++) 
		fPairTreeOutput->Branch(fPairTreeVariableName[ivar].Data(), &fPairTreeVariable[ivar], Form("%s/F",fPairTreeVariableName[ivar].Data()));

   fMCTruthTreeOutput = new TTree("fMCTruthTreeOutput","fMCTruthTreeOutput");
	std::vector<TString> fMCTruthTreeVariableName =
	{
		"Pt", "Mass", "Eta", "Rapidity", "Phi", "Leg1Pt", "Leg2Pt", "Pdg", "Tag"
	};
	fMCTruthTreeVariable = new Float_t [fMCTruthTreeVariableName.size()];
	for (Int_t ivar=0; ivar<(Float_t)fMCTruthTreeVariableName.size(); ivar++)
      fMCTruthTreeOutput->Branch(fMCTruthTreeVariableName[ivar].Data(), &fMCTruthTreeVariable[ivar], Form("%s/F",fMCTruthTreeVariableName[ivar].Data()));

   fLeg1TreeOutput = new TTree("fLeg1TreeOutput","fLeg1TreeOutput");
   fLeg2TreeOutput = new TTree("fLeg2TreeOutput","fLeg2TreeOutput");
	std::vector<TString> fLegTreeVariableName =
	{
      "LegPt", "LegMass", "LegEta", "LegRapidity", "LegPhi", "LegCharge", "LegPin", "LegDCAxy", "LegDCAz",
      "LegTOFnSigma", "LegTPCnSigma", "LegITS", "LegTPCCrossedrows", "LegTPCFindable", "LegTPCcrossedRowsOverFindableClusters",
      "LegITSnCls", "LegITSchi2", "LegTPCnCls", "LegTPCchi2",  "LegTRDntracklets", "LegTRDntrackletsPID",
      "MCPt", "MCMass", "MCEta", "MCRapidity", "MCPhi", "MCPseudoProperDecayTime", "Tag"
   };
   fLeg1TreeVariable = new Float_t [fLegTreeVariableName.size()];
   fLeg2TreeVariable = new Float_t [fLegTreeVariableName.size()];
   fTreeVariable = new Float_t [fLegTreeVariableName.size()];
   for (Int_t ivar=0; ivar<(Float_t)fLegTreeVariableName.size(); ivar++) {
		fLeg1TreeOutput->Branch(fLegTreeVariableName[ivar].Data(), &fLeg1TreeVariable[ivar], Form("%s/F",fLegTreeVariableName[ivar].Data()));
      fLeg2TreeOutput->Branch(fLegTreeVariableName[ivar].Data(), &fLeg2TreeVariable[ivar], Form("%s/F",fLegTreeVariableName[ivar].Data()));
   }
...
//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillMCTruthTree(Float_t* values, Int_t type){
     for (Int_t i=0; i<9; i++) fMCTruthTreeVariable[i] = -9999.;

     fMCTruthTreeVariable[0] = values[AliReducedVarManager::kPtMC];
     fMCTruthTreeVariable[1] = values[AliReducedVarManager::kMassMC];
     fMCTruthTreeVariable[2] = values[AliReducedVarManager::kEtaMC];
     fMCTruthTreeVariable[3] = values[AliReducedVarManager::kRapMC];
     fMCTruthTreeVariable[4] = values[AliReducedVarManager::kPhiMC];
     fMCTruthTreeVariable[5] = values[AliReducedVarManager::kPairLegPtMC+0];
     fMCTruthTreeVariable[6] = values[AliReducedVarManager::kPairLegPtMC+1];
     fMCTruthTreeVariable[7] = values[AliReducedVarManager::kPdgMC];
     fMCTruthTreeVariable[8] = type;

     fMCTruthTreeOutput->Fill();

     return;
}

//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillPairTree(Float_t* values, UInt_t mcDecisions /*= 0*/){
   for (Int_t i=0; i<20; i++) fPairTreeVariable[i] = -9999.;
   
   // Pair info
   fPairTreeVariable[0] = values[AliReducedVarManager::kPt];
   fPairTreeVariable[1] = values[AliReducedVarManager::kMass];
   fPairTreeVariable[2] = values[AliReducedVarManager::kEta];
   fPairTreeVariable[3] = values[AliReducedVarManager::kRap];
   fPairTreeVariable[4] = values[AliReducedVarManager::kPhi];
   fPairTreeVariable[5] = values[AliReducedVarManager::kPairOpeningAngle];
   fPairTreeVariable[6] = values[AliReducedVarManager::kPairPointingAngle];
   fPairTreeVariable[7] = values[AliReducedVarManager::kPairCosPointingAngle];
   fPairTreeVariable[8] = values[AliReducedVarManager::kPairType];
   fPairTreeVariable[9] = values[AliReducedVarManager::kPairTypeSPD];
   fPairTreeVariable[10] = values[AliReducedVarManager::kPairChisquare];
   fPairTreeVariable[11] = values[AliReducedVarManager::kPairLxy];
   fPairTreeVariable[12] = values[AliReducedVarManager::kPseudoProperDecayTime];
   
   // MC rec info
   if(fOptionRunOverMC){
      for(Int_t iMC=0; iMC<=fLegCandidatesMCcuts.GetEntries(); ++iMC) {
         if(mcDecisions & (UInt_t(1)<<iMC)) {
            TString motherTag(fLegCandidatesMCcuts.At(iMC)->GetName());
            
            fPairTreeVariable[13] = values[AliReducedVarManager::kPtMC];
            fPairTreeVariable[14] = values[AliReducedVarManager::kMassMC];
            fPairTreeVariable[15] = values[AliReducedVarManager::kEtaMC];
            fPairTreeVariable[16] = values[AliReducedVarManager::kRapMC];
            fPairTreeVariable[17] = values[AliReducedVarManager::kPhiMC];
            fPairTreeVariable[18] = values[AliReducedVarManager::kPseudoProperDecayTimeMC];

            for(int i = 0; i < motherTags.size(); i++)
               if(motherTags[i].Data() == motherTag) fPairTreeVariable[19] = i;
         }
      }
   }
   
   fPairTreeOutput->Fill();
     
   return;
}

//___________________________________________________________________________
void AliReducedAnalysisPsi2ee::FillLegTree(Float_t* values, Int_t trkTag /*= 1*/, UInt_t mcDecisions /*= 0*/){
   for (Int_t i=0; i<28; i++) fTreeVariable[i] = -9999.;
   
   // Leg1 info
   fTreeVariable[0] = values[AliReducedVarManager::kPt];
   fTreeVariable[1] = values[AliReducedVarManager::kMass];
   fTreeVariable[2] = values[AliReducedVarManager::kEta];
   fTreeVariable[3] = values[AliReducedVarManager::kRap];
   fTreeVariable[4] = values[AliReducedVarManager::kPhi];
   fTreeVariable[5] = values[AliReducedVarManager::kCharge];
   fTreeVariable[6] = values[AliReducedVarManager::kPin];
   fTreeVariable[7] = values[AliReducedVarManager::kDcaXY];
   fTreeVariable[8] = values[AliReducedVarManager::kDcaZ];
   fTreeVariable[9] = values[AliReducedVarManager::kTOFnSig];
   fTreeVariable[10] = values[AliReducedVarManager::kTPCnSig];
   fTreeVariable[11] = values[AliReducedVarManager::kITSlayerHit];
   fTreeVariable[12] = values[AliReducedVarManager::kTPCcrossedRows];
   fTreeVariable[13] = values[AliReducedVarManager::kTPCnclsF];
   fTreeVariable[14] = values[AliReducedVarManager::kTPCcrossedRowsOverFindableClusters];
   fTreeVariable[15] = values[AliReducedVarManager::kITSncls];
   fTreeVariable[16] = values[AliReducedVarManager::kITSchi2];
   fTreeVariable[17] = values[AliReducedVarManager::kTPCncls];
   fTreeVariable[18] = values[AliReducedVarManager::kTPCchi2];
   fTreeVariable[19] = values[AliReducedVarManager::kTRDntracklets];
   fTreeVariable[20] = values[AliReducedVarManager::kTRDntrackletsPID];
   ...
   if(fOptionRunOverMC){
      for(Int_t iMC=0; iMC<=fLegCandidatesMCcuts.GetEntries(); ++iMC) {
         if(mcDecisions & (UInt_t(1)<<iMC)) {
            ...
            fTreeVariable[21] = values[AliReducedVarManager::kPtMC];
            fTreeVariable[22] = values[AliReducedVarManager::kMassMC];
            fTreeVariable[23] = values[AliReducedVarManager::kEtaMC];
            fTreeVariable[24] = values[AliReducedVarManager::kRapMC];
            fTreeVariable[25] = values[AliReducedVarManager::kPhiMC];
            fTreeVariable[26] = values[AliReducedVarManager::kPseudoProperDecayTimeMC];

            for(int i = 0; i < motherTags.size(); i++)
               if(motherTags[i].Data() == motherTag) fTreeVariable[27] = i;
         }
      }
   }

   if(trkTag == 1) {
      std::copy_n(fTreeVariable, 28, fLeg1TreeVariable);
      fLeg1TreeOutput->Fill();

      return;
   }
   if(trkTag == 2) {
      std::copy_n(fTreeVariable, 28, fLeg2TreeVariable);
      fLeg2TreeOutput->Fill();
