...
AliAnalysisTask *AddTask_ffionda_dstDCAcorrections(Int_t reducedEventType=-1, Bool_t writeTree=kTRUE, TString prod="LHC10h", Bool_t hasMCaod = kTRUE){
  ...
  Bool_t hasMC = isAOD ? hasMCaod : (AliAnalysisManager::GetAnalysisManager()->GetMCtruthEventHandler()!=0x0);
