...
AliAnalysisTask* AddTask_iarsene_jpsi2ee_pPb(Bool_t isAliRoot=kTRUE, Int_t runMode=1, TString prod="LHC10h"){    
   //
  ...
  printf("INFO on AddTask_iarsene_jpsi2ee_pPb(): (isAliRoot, runMode) :: (%d,%d) \n", isAliRoot, runMode);
  ...
  if(isAliRoot){
     ...
     if (!mgr) {
        Error("AddTask_iarsene_jpsi2ee_pPb", "No analysis manager found.");
        return 0;
     }
     
     AliAnalysisDataContainer* cReducedEvent = NULL;
     if(runMode==AliAnalysisTaskReducedEventProcessor::kUseOnTheFlyReducedEvents) {
       printf("INFO on AddTask_iarsene_jpsi2ee_pPb(): use on the fly events ");
       cReducedEvent = (AliAnalysisDataContainer*)mgr->GetContainers()->FindObject("ReducedEventDQ");
       if(!cReducedEvent) {
         printf("ERROR: In AddTask_iarsene_jpsi2ee_pPb(), couldn't find exchange container with ReducedEvent! ");
