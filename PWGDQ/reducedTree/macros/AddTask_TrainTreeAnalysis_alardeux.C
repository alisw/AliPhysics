...
AliAnalysisTask* AddTask_TrainTreeAnalysis_alardeux(Bool_t isGrid=kFALSE, Bool_t hasMC=kFALSE, TString prod="LHC10h", Int_t reducedEventType=-1, Bool_t writeTree=kTRUE, TString tasks="dst", TString pathForMacros="$ALICE_PHYSICS/PWGDQ/reducedTree/macros") {
   ...
   if(makeTrees) {
      ...
      gROOT->ProcessLine(Form("AddTask_%s(%d,%d,%d,\"%s\")", tasksArray->At(0)->GetName(), reducedEventType, (writeTree ? 1 : 0), hasMC, prod.Data()));
   ...
   for(Int_t i=(makeTrees ? 1 : 0); i<tasksArray->GetEntries(); ++i) {
      ...
      gROOT->ProcessLine(Form("AddTask_%s(kTRUE,%d,%d,\"%s\")", tasksArray->At(i)->GetName(), runMode, hasMC, prod.Data()));
