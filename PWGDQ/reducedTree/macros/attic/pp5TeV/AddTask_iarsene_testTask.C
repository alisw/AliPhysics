...
void Setup(AliReducedAnalysisTest* processor, TString prod /*="LHC10h"*/) {
  ...
  //
   ...
   processor->SetFillEventHistograms(kTRUE);
   processor->SetFillTriggerHistograms(kTRUE);
   processor->SetFillTrackHistograms(kTRUE);
   processor->SetFillTrackV0Histograms(kTRUE);
   processor->SetFillTrackMCTruthHistograms(kFALSE);
   processor->SetFillPairHistograms(kTRUE);
  ...
  processor->AddTrackFilterBitNames("basicCut;prefilterCut;assocHadr;assocHadrNoITSrefit;assocPion;assocKaon;assocProton");
...
void DefineHistograms(AliReducedAnalysisTest* testTask, AliHistogramManager* man, TString prod /*="LHC10h"*/) {
  ...
  if(testTask->GetFillEventHistograms()) {
      ...
      histClasses += "Event_AfterCuts;EvtTags;EventMC_SPDtrkBins_AfterCuts;";     //ok
  }
  ...
  for(Int_t iclass=0; iclass<arr->GetEntries(); ++iclass) {
    ...
    if(classStr.Contains("Event")) {
      ...
      man->AddHistogram(classStr.Data(),"NVtxContributors_vtxZ","",kFALSE,200,0.,200.,AliReducedVarManager::kNVtxContributors,200,-25.,+25.,AliReducedVarManager::kVtxZ);      
      man->AddHistogram(classStr.Data(),"NVtxContributors_vtxZ","",kFALSE,200,0.,200.,AliReducedVarManager::kNVtxContributors,200,-0.1.,+0.3,AliReducedVarManager::kVtxX);
      man->AddHistogram(classStr.Data(),"NVtxContributors_vtxZ","",kFALSE,200,0.,200.,AliReducedVarManager::kNVtxContributors,200,-0.1.,+0.4,AliReducedVarManager::kVtxY);
