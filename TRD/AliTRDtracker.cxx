...
Int_t AliTRDtracker::Clusters2Tracks(AliESD* event)
{
  ...
  for (Int_t i=0; i<n; i++) {
    ...
    //AliTRDtrack *pt = new AliTRDtrack(*seed2); // !!!
    AliTRDtrack &t=*pt; 
...
Int_t AliTRDtracker::PropagateBack(AliESD* event) 
{
  ...
  for (Int_t i=0; i<n; i++) {
	...
	continue;
      ...
      Double_t y; track->GetYAt(xtof,GetBz(),y);
      if (y > ymax) {
...
//    cout<<found<<'\r';     
    ...
    if(PropagateToX(t,xTPC,fgkMaxStep)) {
      ...
      //AliTRDtrack *pt2 = new AliTRDtrack(*seed2); // !!!
      delete seed2;
...
Int_t AliTRDtracker::FollowProlongation(AliTRDtrack& t)
{
  ...
  for (Int_t iplane = lastplane; iplane>=0; iplane--){
    ...
    t.GetXYZ(xyz0);   //starting global position
    // end global position
    ...
    for (Int_t itime=0 ;itime<GetTimeBinsPerPlane();itime++) {
      ...
      t.fNExpected++;
      if (t.GetX()>345) t.fNExpectedLast++;
