...
void AliTRDrawStreamOld::ResetCounters()
{
  ...
  //
  fBufferRead = kFALSE; // read buffer if it is false (important to read buffer)
...
{                 
  ...
  for (Int_t istack=0; istack<5; istack++){
     ...
     for (Int_t ilink=0; ilink<12; ilink++){
        ...
        ResetPerHC();
        for (Int_t imcm=0; imcm<TRDMAXMCM; imcm++){
           fMCM = &fHC->fMCMs[imcm];
           ResetPerMCM();
           for (Int_t iadc=0; iadc<TRDMAXADC; iadc++){
      ...
      if (fSM.fStackActive[fStackNumber] == kTRUE) {
        ...
        while (fStackLinkNumber < 12) {
          if (fStack->fLinksActive[fStackLinkNumber] == kTRUE && fStack->fLinksMonitor[fStackLinkNumber] == 0) {
            ...
            if (fHC->fCorrupted == 0 && (fHC->fH0Corrupted == 0 && fHC->fH1Corrupted == 0) | !fgSkipData) { // if HC data corrupted(in any case), we don't read data at all from this HC 
              while (fhcMCMcounter < fHC->fMCMmax) {
                ...
                } // while fmcmADCcounter
                fhcMCMcounter++;
                fmcmADCcounter = 0; // next MCM should go through all active ADCs - start from 0
              } // while fhcMCMcounter
            } // if HC OK
          } // if link active
          fStackLinkNumber++;
          fhcMCMcounter = 0; // next stack link (HC) should go through all active MCMs - start from 0
        } // while fStackLinkNumber
      } // if stack active
      fStackNumber++;
      fStackLinkNumber = 0; // next stack should go through all links - start from 0
    } // while fStackNumber
  } // if fBufferRead
...
Int_t AliTRDrawStreamOld::NextChamber(AliTRDdigitsManager *const digitsManager, UInt_t **trackletContainer, UShort_t **/*errorCodeContainer*/) 
{
  ...
  if (trackletContainer) { 
    for (Int_t i = 0; i < 2; i++) 
      for (Int_t j = 0; j < MAXTRACKLETSPERHC; j++) 
  ...
  for (Int_t istack = 0; istack < 5; istack++) {
     ...
     for (Int_t ilink = 0; ilink < 12; ilink++) {
        ...
        if (fpPos >= fpEnd) {
          ...
          if (fWarnError) AliError("HC data missing.");      
          fSM.fClean = kFALSE;
          break;
        }
...
Bool_t AliTRDrawStreamOld::SeekNextMCMheader()
{
  ...
  while ( *fpPos != ENDOFRAWDATAMARKER && fpPos < fpEnd ) {
    if (MCM_HEADER_MASK_ERR(*fpPos) == 0 && MCM_HEADER_MASK_ERR(*(fpPos+1)) == 0) {      
      if (fgDebugFlag) AliDebug(11,Form("^^^ Found : Pos 0x%p : Val 0x%08x", fpPos, *fpPos));
      return kTRUE;
    }
    if ( *fpPos == END_OF_TRACKLET_MARKERNEW) {
      fMCM->fCorrupted += 16;
...
Bool_t AliTRDrawStreamOld::DecodeTracklets()
{
  ...
  for (Int_t i = 0; i < MAXTRACKLETSPERHC; i++) 
     fHC->fTrackletWords[i] = 0; 

  if (fgDebugFlag)  AliDebug(10, Form("Decode tracklets at 0x%p : 0x%08x", fpPos, *fpPos));

  while ( *fpPos != END_OF_TRACKLET_MARKEROLD && *fpPos != END_OF_TRACKLET_MARKERNEW && fpPos < fpEnd ) {
    if (fgDebugFlag)  AliDebug(10, Form("Tracklet found at 0x%p : 0x%08x", fpPos, *fpPos));
  ...
  while ( ( *fpPos == END_OF_TRACKLET_MARKEROLD || *fpPos == END_OF_TRACKLET_MARKERNEW ) && fpPos < fpEnd ) {
    if (fgDebugFlag)  AliDebug(10, Form("EoTracklets found at 0x%p : 0x%08x", fpPos, *fpPos));
...
{  
  ...
  if (IsHCheaderOK() == kFALSE) {
    fHC->fH0Corrupted += 2;
    if (fgDebugFlag) AliDebug(11,Form("H0 Header Insane. Word 0x%p", *fHC->fPos));
      return kFALSE;
  }
      ...
      for (Int_t iadc = 0; iadc < fMCM->fADCmax; iadc++) {
         ...
         if (fHC->fRawVMajor < 64) { // normal(real) ADC data
           if (fHC->fTimeBins < 31) {
             ...
             if (DecodeADCExtended() == kFALSE) { // decoding TRAP data with ZS and more than 32 samples
               if (fMCM->fCorrupted < 4) fMCM->fCorrupted += 4; // benchmark mcm data corruption as 4
               if (fHC->fCorrupted < 4) fHC->fCorrupted += 4;   // benchmark hc data corruption as 4
               if (fADC->fIsShared && fADC->fCorrupted == 32) { // check if we are out of the det when the pad is shared
                 fADC->fExtendedCOL = -1;
           ...
           if (fWarnError) AliError("These are test pattern data. You need other reader"); // will be served in other class
         }
      } // for iadc 
    } // if fMCM->fADCmax
    else {
...
Bool_t AliTRDrawStreamOld::DecodeADC()
{
  ...
  for (Int_t i = 0; i < GetNumberOfTimeBins(); i++)
  //for (Int_t i = 0; i < TRDMAXTBINS; i++)
    fADC->fSignals[i] = 0;
...
Bool_t AliTRDrawStreamOld::DecodeHCwordH0(const UInt_t *word, struct AliTRDrawHC *hc) const
{
  ...
  if (hc->fH0Corrupted > 0) {
    if (fgDebugFlag) AliDebug(11,Form("H0 Header Mask Error. Word 0x%p", *fHC->fPos));
    return kFALSE;
  }
...
Bool_t AliTRDrawStreamOld::DecodeHCwordH1(const UInt_t *word, struct AliTRDrawHC *hc) const
{
  ...
  if (hc->fH1Corrupted > 0) {
    if (fgDebugFlag) AliDebug(11,Form("H1 Header Mask Error. Word 0x%p", *fHC->fPos));
    return kFALSE;
  }
...
const char *AliTRDrawStreamOld::DumpHCinfoH0(const struct AliTRDrawHC *hc)
{
  ...
  if (!hc)
    ...
    return Form("[ HC[0] at 0x%p ] : 0x%08x Info is : RawV %d SM %d Stack %d Layer %d Side %d DCSboard %d",
...
const char *AliTRDrawStreamOld::DumpHCinfoH1(const struct AliTRDrawHC *hc)
{
  ...
  if (!hc)
    ...
    return Form("[ HC[1] at 0x%p ] : 0x%08x Info is : TBins %d BCcount %d PreTrigCount %d PreTrigPhase %d",
