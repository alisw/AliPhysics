...
void AliTRDCommonParam::Copy(TObject &p) const
{
  ...
  // Do not copy timestructure and diffusion corefficients, 
  // just invalidate buffered vdrift.
  // Next time they are requested, they get recalculated
  target->fDiffusionT        =  0.0;
  target->fDiffusionL        =  0.0;
  target->fDiffLastVdrift    = -1.0;
  if (target->fTimeStruct1) {
    delete [] target->fTimeStruct1;
    target->fTimeStruct1 = 0x0;
  }
  if (target->fTimeStruct2) {
    delete [] target->fTimeStruct2;
    target->fTimeStruct2 = 0x0;
  }  
  target->fTimeLastVdrift    = -1.0;
  target->fVDlo              =  0.0;
  target->fVDhi              =  0.0;
...
//_____________________________________________________________________________
Float_t AliTRDCommonParam::GetOmegaTau(Float_t vdrift)
{
  //
  ...
  Double_t bz       = 0.1 * fld->SolenoidField();   // kGauss -> Tesla
  Double_t fieldAbs = TMath::Abs(bz);
  Double_t fieldSgn = (bz > 0.0) ? 1.0 : -1.0;
...
//_____________________________________________________________________________
Bool_t AliTRDCommonParam::GetDiffCoeff(Float_t &dl, Float_t &dt, Float_t vdrift)
{
  //
  ...
  if      (IsXenon()) {
    ...
    Double_t field = 0.1 * fld->SolenoidField();   // kGauss -> Tesla
...
//_____________________________________________________________________________
Double_t AliTRDCommonParam::TimeStruct(Float_t vdrift, Double_t dist, Double_t z)
{
  //
...
//_____________________________________________________________________________
void AliTRDCommonParam::SampleTimeStruct(Float_t vdrift)
{
  //
  ...
  // Nothing to do
  if (TMath::Abs(fTimeLastVdrift - vdrift) < 1.e-3) {
    return;
  }
  ...
  if (!fTimeStruct1) fTimeStruct1 = new Float_t[ktimebin*kZbin];
  if (!fTimeStruct2) fTimeStruct2 = new Float_t[ktimebin*kZbin];
  memset(fTimeStruct1, 0, ktimebin*kZbin*sizeof(Float_t));
  memset(fTimeStruct2, 0, ktimebin*kZbin*sizeof(Float_t));
