...
{
  fNRefFigures = 0;
  TH1 *histo = dynamic_cast<TH1 *>(fContainer->At(fNRefFigures++));
  ...
  histo = dynamic_cast<TH1 *>(fContainer->At(fNRefFigures++));
  histo->SetMarkerStyle(22);
  histo->SetMarkerColor(kBlue);
  histo->GetXaxis()->SetTitle("p [GeV/c]");
  histo->GetXaxis()->SetMoreLogLabels();
...
void AliTRDtrackingEfficiencyCombined::GetRefFigure(Int_t ifig){
  TH1 * histo = dynamic_cast<TH1 *>(fContainer->At(ifig < 2 ? ifig : 0));
  histo->Draw("e1");
}
