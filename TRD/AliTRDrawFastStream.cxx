...
Bool_t AliTRDrawFastStream::DecodeHCwordH1(const UInt_t *word, struct AliTRDrawHC *hc) const
{
  ...
  if (hc->fH1Corrupted > 0) {
    if (fgDebugFlag) AliDebug(11,Form("H1 Header Mask Error. Word 0x%p", *(fHC->fPos+1) ));
    return kFALSE;
  }
...
Bool_t AliTRDrawFastStream::SetGlobalNTimebins()
{
  ...
  while (SetNTimebins()==kFALSE){
    if (fgDebugFlag) AliDebug(11,Form("Failed to get number of time bin information from the %dth HC",nHCs));
    nHCs++;
  }
