...
//class AliTRDtrapAlu {
 ...
 void Init(const Int_t& precom=10, const Int_t& postcom=2, const Int_t& lRestriction = -1, const Int_t& uRestriction = -1);
...
AliTRDtrapAlu& operator+(AliTRDtrapAlu& binary); //binary is not const, because in a+(b*c) binary is reference to the object, to which Mem() is also a reference and this object is changed

AliTRDtrapAlu& operator-(AliTRDtrapAlu& binary);
