...
ClassImp(AliV0ReaderV1_CF)

    //________________________________________________________________________
    AliV0ReaderV1_CF::AliV0ReaderV1_CF(const char *name) : AliAnalysisTaskSE(name),
...
//________________________________________________________________________
AliV0ReaderV1_CF::~AliV0ReaderV1_CF()
{
 ...
 */
AliConversionPhotonBase *AliV0ReaderV1_CF::operator[](int index) const
{
...
/*
//________________________________________________________________________
AliV0ReaderV1_CF::AliV0ReaderV1_CF(AliV0ReaderV1_CF &original) : AliAnalysisTaskSE(original),
fConversionCuts(NULL),
fConversionGammas(NULL),
...
AliV0ReaderV1_CF &AliV0ReaderV1_CF::operator=(const AliV0ReaderV1_CF &ref){
//
// Assignment operator
...
//________________________________________________________________________
void AliV0ReaderV1_CF::Init()
{
  // Initialize function to be called once before analysis
  if (fConversionCuts == NULL)
  {
    if (fConversionCuts == NULL)
      AliError("No Conversion Cut Selection initialized");
  }
  if (fEventCuts == NULL)
  {
    if (fEventCuts == NULL)
      AliError("No Event Cut Selection initialized");
  }
...
//________________________________________________________________________
void AliV0ReaderV1_CF::UserCreateOutputObjects()
{
  // Create AODs
...
//________________________________________________________________________
Bool_t AliV0ReaderV1_CF::Notify()
{
  ...
  if (!fEventCuts->GetDoEtaShift())
    return kTRUE; // No Eta Shift requested, continue
...
//________________________________________________________________________
Int_t AliV0ReaderV1_CF::GetSumSDDSSDClusters(AliVEvent *event)
{
...
//________________________________________________________________________
void AliV0ReaderV1_CF::UserExec(Option_t *option)
{
  fSDDSSDClusters = -1;

  if (!fConversionCuts->GetPIDResponse())
    fConversionCuts->InitPIDResponse();
...
//________________________________________________________________________
Bool_t AliV0ReaderV1_CF::ProcessEvent(AliVEvent *inputEvent, AliMCEvent *mcEvent)
{
  if (!fConversionCuts->GetPIDResponse())
    fConversionCuts->InitPIDResponse();
  ...
  if (!fEventCuts)
  {
    AliError("No EventCuts");
    return kFALSE;
  }
  if (!fConversionCuts)
  {
    AliError("No ConversionCuts");
    return kFALSE;
  }
  ...
  if (fCalcSphericity)
    CalculateSphericity();
  if (fCalcSphericity && fMCEvent)
    CalculateSphericityMCTrue(fInputEvent);
  if (fCalcSector)
    CalculatePtMaxSector();
...
///________________________________________________________________________
void AliV0ReaderV1_CF::FillAODOutput()
{
...
///________________________________________________________________________
const AliExternalTrackParam *AliV0ReaderV1_CF::GetExternalTrackParam(AliESDv0 *fCurrentV0, Int_t &tracklabel, Int_t charge)
{
  ...
  else
  {
    ...
    if (fCurrentV0)
    {
      if (!fCurrentV0->GetParamN() || !fCurrentV0->GetParamP())
        return 0x0;
      if (!fConversionCuts->GetTrack(fInputEvent, fCurrentV0->GetNindex()) || !fConversionCuts->GetTrack(fInputEvent, fCurrentV0->GetPindex()))
        return 0x0;
...
///________________________________________________________________________
Bool_t AliV0ReaderV1_CF::ProcessESDV0s()
{
    ...
    for (Int_t currentV0Index = 0; currentV0Index < fESDEvent->GetNumberOfV0s(); currentV0Index++)
    {
      ...
      if (fCurrentMotherKFCandidate)
      {
        ...
        if (kUseAODConversionPhoton)
        {
          ...
          if (kAddv0sInESDFilter)
          {
            fPCMv0BitField->SetBitNumber(currentV0Index, kTRUE);
          }
        }
        else
        {
...
///________________________________________________________________________
AliKFConversionPhoton *AliV0ReaderV1_CF::ReconstructV0(AliESDv0 *fCurrentV0, Int_t currentV0Index)
{
  ...
  if (fMCEvent && fProduceV0findingEffi)
    FillRecMCHistosForV0FinderEffiESD(fCurrentV0);
  ...
  if (fMCEvent)
  {
    ...
    if (labeln > -1)
      fNegativeMCParticle = (AliMCParticle *)fMCEvent->GetTrack(labeln);
    AliMCParticle *fPositiveMCParticle = 0x0;
    if (labelp > -1)
      fPositiveMCParticle = (AliMCParticle *)fMCEvent->GetTrack(labelp);
  ...
  if (fProduceImpactParamHistograms)
    FillImpactParamHistograms(posTrack, negTrack, fCurrentV0, fCurrentMotherKF);
...
///________________________________________________________________________
Double_t AliV0ReaderV1_CF::GetPsiPair(const AliESDv0 *v0, const AliExternalTrackParam *positiveparam, const AliExternalTrackParam *negativeparam, const Double_t convpos[3]) const
{
  //
  ...
  if (fImprovedPsiPair == 1 || fImprovedPsiPair == 0)
  {
    if (nt.PropagateTo(radiussum, magField) == 0)
      return psiPair; // propagate tracks to the outside -> Better Purity and Efficiency
...
///________________________________________________________________________
Bool_t AliV0ReaderV1_CF::GetHelixCenter(const AliExternalTrackParam *track, Double_t center[2])
{
...
///________________________________________________________________________
Bool_t AliV0ReaderV1_CF::GetConversionPoint(const AliExternalTrackParam *pparam, const AliExternalTrackParam *nparam, Double_t convpos[3], Double_t dca[2])
{
  ...
  if (!p.PropagateTo(vertexPosRot.X(), fInputEvent->GetMagneticField()))
    return kFALSE;
  if (!n.PropagateTo(vertexNegRot.X(), fInputEvent->GetMagneticField()))
    return kFALSE;

  // Check whether propagation was sucessful

  if (TMath::Abs(vertexPos.Mod() - TMath::Sqrt(p.GetX() * p.GetX() + p.GetY() * p.GetY())) > 0.01)
    return kFALSE;
  if (TMath::Abs(vertexNeg.Mod() - TMath::Sqrt(n.GetX() * n.GetX() + n.GetY() * n.GetY())) > 0.01)
    return kFALSE;
...
//________________________________________________________________________
Bool_t AliV0ReaderV1_CF::GetAODConversionGammas()
{
...
//________________________________________________________________________
void AliV0ReaderV1_CF::FindDeltaAODBranchName()
{
...
//________________________________________________________________________
Bool_t AliV0ReaderV1_CF::RelabelAODPhotonCandidates(AliAODConversionPhoton *PhotonCandidate)
{

  if (fPreviousV0ReaderPerformsAODRelabeling == 2)
    return kTRUE;
  else if (fPreviousV0ReaderPerformsAODRelabeling == 0)
  {
    ...
    {
      if ((obj->At(i))->IsA() == AliV0ReaderV1_CF::Class())
      {
        AliV0ReaderV1_CF *tempReader = (AliV0ReaderV1_CF *)obj->At(i);
    ...
    if (prevV0ReaderRunningButNotRelabeling)
      AliFatal(Form("There are V0Readers before '%s', but none of them is relabeling!", this->GetName()));
  ...
  if (fPreviousV0ReaderPerformsAODRelabeling != 1)
    AliFatal(Form("In %s: fPreviousV0ReaderPerformsAODRelabeling = '%i' - while it should be impossible it is something different than '1'!", this->GetName(), fPreviousV0ReaderPerformsAODRelabeling));
...
//************************************************************************
//________________________________________________________________________
void AliV0ReaderV1_CF::CountTracks()
{
...
///________________________________________________________________________
void AliV0ReaderV1_CF::CountTPCoutTracks()
{
  fNumberOfTPCoutTracks = 0;
...
///________________________________________________________________________
void AliV0ReaderV1_CF::CalculateSphericity()
{
  TMatrixD Si(2, 2);
  TMatrixD S(2, 2);
  TMatrixD St(2, 2);
  ...
  if (fInputEvent->IsA() == AliESDEvent::Class())
  {
    ...
    for (Int_t iTracks = 0; iTracks < fInputEvent->GetNumberOfTracks(); iTracks++)
    {
      ...
      S += (1 / curTrack->Pt()) * Si;
      P += curTrack->Pt();
      if (curTrack->Pt() > fHighestPt)
        fHighestPt = curTrack->Pt();
    }
    ...
    else
    {
      ...
      if (MirroredMainSphericityAxis > 2 * TMath::Pi())
        MirroredMainSphericityAxis -= 2 * TMath::Pi();
...
///________________________________________________________________________
void AliV0ReaderV1_CF::CalculateSphericityMCTrue(AliVEvent *inputEvent)
{
  TMatrixD Si(2, 2);
  TMatrixD S(2, 2);
  TMatrixD St(2, 2);
...
///________________________________________________________________________
void AliV0ReaderV1_CF::CalculatePtMaxSector()
{
  ...
  if (fInputEvent->IsA() == AliESDEvent::Class())
  {
    ...
    if (tempPt != 0 && PtMaxTrack)
    {
      ...
      fPtMaxSector = 0;
      if ((PtMaxPhi > (TMath::Pi() / 4.)) && (PtMaxPhi < (3 * TMath::Pi() / 4.)))
        fPtMaxSector = 1;
      if ((PtMaxPhi > (3 * TMath::Pi() / 4.)) && (PtMaxPhi < (5 * TMath::Pi() / 4.)))
        fPtMaxSector = 2;
      if ((PtMaxPhi > (5 * TMath::Pi() / 4.)) && (PtMaxPhi < (7 * TMath::Pi() / 4.)))
        fPtMaxSector = 3;
      if ((PtMaxPhi > (7 * TMath::Pi() / 4.)) && (PtMaxPhi < (TMath::Pi() / 4.)))
        fPtMaxSector = 4;
      if (PtMaxTrack->Eta() > 0)
        fPtMaxSector += 4;
    }
...
///________________________________________________________________________
Bool_t AliV0ReaderV1_CF::ParticleIsConvertedPhoton(AliMCEvent *mcEvent, AliMCParticle *particle, Double_t etaMax, Double_t rMax, Double_t zMax)
{
  ...
  if (particle->PdgCode() == 22)
  {
    ...
    if (TMath::Abs(particle->Eta()) > etaMax)
      return kFALSE;
    ...
    { // means we do not have two daughters from pair production
      return kFALSE;
    }
...
void AliV0ReaderV1_CF::CreatePureMCHistosForV0FinderEffiESD()
{
...
///_______________________________________________________________________
void AliV0ReaderV1_CF::FillRecMCHistosForV0FinderEffiESD(AliESDv0 *currentV0)
{
  ...
  if (labeln > -1)
    negPart = (AliMCParticle *)fMCEvent->GetTrack(labeln);
  AliMCParticle *posPart = 0x0;
  if (labelp > -1)
    posPart = (AliMCParticle *)fMCEvent->GetTrack(labelp);

  if (negPart == NULL || posPart == NULL)
    return;
...
void AliV0ReaderV1_CF::FillImpactParamHistograms(AliVTrack *pTrack, AliVTrack *nTrack, AliESDv0 *fCurrentV0, AliKFConversionPhoton *fCurrentMotherKF)
{
  ...
  if (convR > 80)
  { // conversion within TPC <-> TPC-only tracks
...
//_________________________________________________________________________________
Bool_t AliV0ReaderV1_CF::CheckVectorOnly(vector<Int_t> &vec, Int_t tobechecked)
{
  if (tobechecked > -1)
  {
...
//_________________________________________________________________________________
Bool_t AliV0ReaderV1_CF::CheckVectorForDoubleCount(vector<Int_t> &vec, Int_t tobechecked)
{
  if (tobechecked > -1)
  {
    vector<Int_t>::iterator it;
...
//________________________________________________________________________
void AliV0ReaderV1_CF::Terminate(Option_t *)
{
}

AliV0ReaderV1_CF::iterator::iterator(const AliV0ReaderV1_CF *reader, AliV0ReaderV1_CF::iterator::Direction_t dir, int position) : std::iterator<std::bidirectional_iterator_tag, AliConversionPhotonBase>(),
                                                                                                                                  fkData(reader),
...
AliV0ReaderV1_CF::iterator::iterator(const AliV0ReaderV1_CF::iterator &other) : std::iterator<std::bidirectional_iterator_tag, AliConversionPhotonBase>(other),
                                                                                fkData(other.fkData),
...
AliV0ReaderV1_CF::iterator &AliV0ReaderV1_CF::iterator::operator=(const AliV0ReaderV1_CF::iterator &other)
{
  if (this != &other)
  {
    fkData = other.fkData;
...
bool AliV0ReaderV1_CF::iterator::operator!=(AliV0ReaderV1_CF::iterator &other) const
{
  if (fkData != other.fkData)
    return true;
  return fCurrentIndex != other.fCurrentIndex;
}

AliV0ReaderV1_CF::iterator AliV0ReaderV1_CF::iterator::operator++(int index)
{
  iterator tmp(*this);
  operator++();
  return tmp;
...
AliV0ReaderV1_CF::iterator &AliV0ReaderV1_CF::iterator::operator++()
{
  if (fDirection == AliV0ReaderV1_CF::iterator::kForwardDirection)
  {
    fCurrentIndex++;
  }
  else
...
AliV0ReaderV1_CF::iterator AliV0ReaderV1_CF::iterator::operator--(int)
{
  iterator tmp(*this);
  operator--();
  return tmp;
...
AliConversionPhotonBase *AliV0ReaderV1_CF::iterator::operator*()
{
