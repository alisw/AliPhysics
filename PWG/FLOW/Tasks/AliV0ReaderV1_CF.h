#ifndef AliV0ReaderV1_CF_H
#define AliV0ReaderV1_CF_H
...
// needs to be disabled for AliV0ReaderV1_CF
...
class AliV0ReaderV1_CF : public AliAnalysisTaskSE
{
...
public:
  ...
  public:
    ...
    iterator(const AliV0ReaderV1_CF *reader, Direction_t dir, int position);
  ...
  private:
    const AliV0ReaderV1_CF *fkData; ///< V0 reader used to iterate over
    int fCurrentIndex;              ///< Index of the current element
    Direction_t fDirection;         ///< Iterator in forward direction
  };

  AliV0ReaderV1_CF(const char *name = "V0ReaderV1");
  virtual ~AliV0ReaderV1_CF(); // virtual destructor
  ...
  {
    fUseOwnXYZCalculation = flag;
    return;
  }
  void SetUseConstructGamma(Bool_t flag)
  {
    fUseConstructGamma = flag;
    return;
  }
  ...
  void SetProduceV0FindingEfficiency(Bool_t b)
  {
    ...
    if (b)
      AliInfo("Enabled V0finding Efficiency");
    return;
  }
  ...
  void SetProduceImpactParamHistograms(Bool_t b)
  {
    ...
    if (b)
      AliInfo("Producing additional impact parameter histograms");
    return;
  }
...
private:
  AliV0ReaderV1_CF(AliV0ReaderV1_CF &original);
  AliV0ReaderV1_CF &operator=(const AliV0ReaderV1_CF &ref);

  ClassDef(AliV0ReaderV1_CF, 25)
};
...
inline void AliV0ReaderV1_CF::SetConversionCuts(const TString cut)
{
...
inline void AliV0ReaderV1_CF::SetEventCuts(const TString cut)
{
  if (fEventCuts != NULL)
  {
    delete fEventCuts;
