...
#include "AliESDpid.h"
#include "AliAnalysisTaskGammaCaloV2.h"
...
ClassImp(AliAnalysisTaskGammaCaloV2)
...
AliAnalysisTaskGammaCaloV2::~AliAnalysisTaskGammaCaloV2()
{
...
void AliAnalysisTaskGammaCaloV2::UserCreateOutputObjects()
{
  ...
  if (fDoMesonAnalysis)
  { //
    if (((AliConvEventCuts *)fEventCutArray->At(0))->GetUseSphericityTrue())
      fDoTrueSphericity = kTRUE;
  }
  ...
  if (((AliConvEventCuts *)fEventCutArray->At(0))->GetUseJetFinderForOutliers())
  {
    ...
    if (!fOutlierJetReader)
    {
      AliFatal("Error: No AliAnalysisTaskJetOutlierRemoval");
    } // GetV0Reader
    else
    {
      printf("Found AliAnalysisTaskJetOutlierRemoval used for outlier removal!\n");
    }
  }
      ...
      ((AliConvEventCuts *)fV0Reader->GetEventCuts())->GetEnergyEnum() == AliConvEventCuts::kpPb8TeV)
  {
    ...
    {
      if (i < 100)
        arrPtBinning[i] = 0.10 * i;
      else if (i < 140)
        arrPtBinning[i] = 10. + 0.25 * (i - 100);
      else if (i < 180)
        arrPtBinning[i] = 20. + 1.00 * (i - 140);
      else
    ...
    {
      if (i < 100)
        arrQAPtBinning[i] = 0.10 * i;
      else if (i < 140)
        arrQAPtBinning[i] = 10. + 0.25 * (i - 100);
      else if (i < 180)
        arrQAPtBinning[i] = 20. + 1.00 * (i - 140);
      else
        arrQAPtBinning[i] = maxQAPt;
    }
    nBinsClusterPt = 310;
    minClusterPt = 0;
    maxClusterPt = 100;
    ...
    {
      if (i < 1)
        arrClusPtBinning[i] = 0.3 * i;
      else if (i < 198)
        arrClusPtBinning[i] = 0.3 + 0.1 * (i - 1);
      else if (i < 238)
        arrClusPtBinning[i] = 20. + 0.25 * (i - 198);
      else if (i < 278)
        arrClusPtBinning[i] = 30. + 0.5 * (i - 238);
      else if (i < 298)
        arrClusPtBinning[i] = 50. + 1.0 * (i - 278);
      else if (i < 310)
        arrClusPtBinning[i] = 70. + 2.5 * (i - 298);
      else
        arrClusPtBinning[i] = maxClusterPt;
    }
  }
           ...
           ((AliConvEventCuts *)fV0Reader->GetEventCuts())->GetEnergyEnum() == AliConvEventCuts::k13TeVLowB)
  {
    if (fDoPi0Only)
    {
      ...
      {
        if (i < 1)
          arrPtBinning[i] = 0.3 * i;
        else if (i < 198)
          arrPtBinning[i] = 0.3 + 0.1 * (i - 1);
        else if (i < 218)
          arrPtBinning[i] = 20. + 0.25 * (i - 198);
        else
          arrPtBinning[i] = maxPt;
      }
      nBinsQAPt = 145;
      ...
      {
        if (i < 100)
          arrQAPtBinning[i] = 0.1 * i;
        else if (i < 140)
          arrQAPtBinning[i] = 10. + 0.25 * (i - 100);
        else if (i < 145)
          arrQAPtBinning[i] = 20. + 1.0 * (i - 140);
        else
          arrQAPtBinning[i] = maxQAPt;
      }
      nBinsClusterPt = 157;
      ...
      for (Int_t i = 0; i < nBinsClusterPt + 1; i++)
      {
        if (i < 1)
          ...
          arrClusPtBinning[i] = 10. + 0.25 * (i - 98);
        else if (i < 148)
          arrClusPtBinning[i] = 20. + 1.0 * (i - 138);
        else if (i < 152)
          arrClusPtBinning[i] = 30. + 5.0 * (i - 148);
        else if (i < 157)
          arrClusPtBinning[i] = 50. + 10. * (i - 152);
        else
          arrClusPtBinning[i] = maxClusterPt;
      }
    }
    else
    {
      ...
      for (Int_t i = 0; i < nBinsPt + 1; i++)
      {
        if (i < 1)
          ...
          arrPtBinning[i] = 30. + 0.5 * (i - 238);
        else if (i < 298)
          arrPtBinning[i] = 50. + 1.0 * (i - 278);
        else if (i < 310)
          arrPtBinning[i] = 70. + 2.5 * (i - 298);
        else
          arrPtBinning[i] = maxPt;
      }
      nBinsQAPt = 240;
      ...
      for (Int_t i = 0; i < nBinsQAPt + 1; i++)
      {
        if (i < 100)
          ...
          arrQAPtBinning[i] = 20. + 0.5 * (i - 140);
        else if (i < 240)
          arrQAPtBinning[i] = 40. + 1.0 * (i - 180);
        else
          arrQAPtBinning[i] = maxQAPt;
      }
      nBinsClusterPt = 310;
  ...
  else if (((AliConvEventCuts *)fV0Reader->GetEventCuts())->GetEnergyEnum() == AliConvEventCuts::kXeXe5440GeV)
  {
    ...
    for (Int_t i = 0; i < nBinsQAPt + 1; i++)
    {
      if (i < 60)
        ...
        arrQAPtBinning[i] = 6. + 0.25 * (i - 60);
      else if (i < 92)
        arrQAPtBinning[i] = 12. + 1.0 * (i - 84);
      else
        arrQAPtBinning[i] = maxQAPt;
    }
    nBinsClusterPt = 148;
    ...
    for (Int_t i = 0; i < nBinsClusterPt + 1; i++)
    {
      if (i < 1)
        ...
        arrClusPtBinning[i] = 10. + 0.2 * (i - 98);
      else if (i < 148)
        arrClusPtBinning[i] = 15. + 1.0 * (i - 123);
      else
        arrClusPtBinning[i] = maxClusterPt;
    }
  }
  ...
  else
  {
    ...
    {
      if (i < 60)
        arrQAPtBinning[i] = 0.05 * i;
      else if (i < 130)
        arrQAPtBinning[i] = 3. + 0.1 * (i - 60);
      else if (i < 170)
        arrQAPtBinning[i] = 10. + 0.25 * (i - 130);
      else if (i < 175)
        arrQAPtBinning[i] = 20. + 1.0 * (i - 170);
      else
        arrQAPtBinning[i] = maxQAPt;
    }
  }
  ...
  for (Int_t iCut = 0; iCut < fnCuts; iCut++)
  {
    ...
    if (fIsMC > 1)
    {
      ...
      if (!fDoLightOutput && fDoClusterQA > 0)
        fHistoClusGammaPtM02[iCut]->Sumw2();
    }

    if (fDoMesonAnalysis)
    {
      double minPhi = 0;
      double maxPhi = TMath::TwoPi();
      int nPhiBins = 6;
      double arrPhiBin[nPhiBins + 1];
      for (Int_t i = 0; i < nPhiBins + 1; i++)
      {
        arrPhiBin[i] = ((maxPhi - minPhi) / nPhiBins) * i;
      }
      ...
      if (fProduceTreeEOverP)
      {
        ...
        if (fIsMC > 0)
          tClusterEOverP[iCut]->Branch("ClusClassification", &fClusterClassification, "fClusterClassification/I");
  ...
  if (fIsMC > 0)
  {
    ...
    {
      if (fDoClusterQA > 0)
        fHistoTrueClusGammaPtM02 = new TH2F *[fnCuts];
    ...
    for (Int_t iCut = 0; iCut < fnCuts; iCut++)
    {
      ...
      if (fDoMesonAnalysis)
      {
        ...
        fHistoMCPi0PtNotTriggered[iCut]->SetXTitle("p_{T} (GeV/c)");
        ...
        fHistoMCPi0PtNoVertex[iCut]->SetXTitle("p_{T} (GeV/c)");
        ...
        fHistoMCPi0InAccPtNotTriggered[iCut]->SetXTitle("p_{T} (GeV/c)");
        ...
        if (!fDoPi0Only)
        {
          ...
          fHistoMCEtaPtNotTriggered[iCut]->SetXTitle("p_{T} (GeV/c)");
          ...
          fHistoMCEtaPtNoVertex[iCut]->SetXTitle("p_{T} (GeV/c)");
          ...
          fHistoMCEtaInAccPtNotTriggered[iCut]->SetXTitle("p_{T} (GeV/c)");
      ...
      if (fIsMC > 1)
      {
        ...
        if (!fDoLightOutput)
        {
          ...
          if (fDoClusterQA > 0)
            fHistoTrueClusGammaPtM02[iCut]->Sumw2();
      ...
      if (fDoMesonAnalysis)
      {
        ...
        if (fDoMesonQA > 0 && fDoMesonQA < 3)
        {
          ...
          fHistoTrueBckGCInvMassPt[iCut]->SetYTitle("p_{T} (GeV/c)");
          ...
          fHistoTrueBckCCInvMassPt[iCut]->SetYTitle("p_{T} (GeV/c)");
          ...
          fHistoTrueBckPartConvInvMassPt[iCut]->SetYTitle("p_{T} (GeV/c)");
          ...
          fHistoTrueBckPartGammaInvMassPt[iCut]->SetYTitle("p_{T} (GeV/c)");
          ...
          fHistoTrueBckRestInvMassPt[iCut]->SetYTitle("p_{T} (GeV/c)");
  ...
  for (Int_t iCut = 0; iCut < fnCuts; iCut++)
  {
    if (!((AliConvEventCuts *)fEventCutArray->At(iCut)))
      continue;
    if (((AliConvEventCuts *)fEventCutArray->At(iCut))->GetCutHistograms())
    {
      fCutFolder[iCut]->Add(((AliConvEventCuts *)fEventCutArray->At(iCut))->GetCutHistograms());
    }
    if (!((AliCaloPhotonCuts *)fClusterCutArray->At(iCut)))
      continue;
    ...
    if (fDoMesonAnalysis)
    {
      if (!((AliConversionMesonCuts *)fMesonCutArray->At(iCut)))
        continue;
...
Bool_t AliAnalysisTaskGammaCaloV2::Notify()
{
  for (Int_t iCut = 0; iCut < fnCuts; iCut++)
  {
...
//_____________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::UserExec(Option_t *)
{
  //
  ...
  { // Event Not Accepted due to MC event missing or wrong trigger for V0ReaderV1 or because it is incomplete
    ...
    for (Int_t iCut = 0; iCut < fnCuts; iCut++)
    {
      if (!TriggerFiles_Filled)
      {
        ...
        {                                                                                                                                                                 // Only PHOS
          if ((fIsMC == 0)                                                                                                                                                // Only Data
              && fCaloTriggerMimicHelper[fiCut] && (fDoClusterQA > 1)                                                                                                     // If QA Flag is set
              && ((((AliConvEventCuts *)fEventCutArray->At(iCut))->IsSpecialTrigger() == 1) || (((AliConvEventCuts *)fEventCutArray->At(iCut))->IsSpecialTrigger() == 0)) // Only MB
          )
          {
            if (fCaloTriggerMimicHelper[fiCut]->GetEventChosenByTrigger())
            {
              ...
              {
                if (tTriggerFiles_wL0)
                  tTriggerFiles_wL0->Fill();
              }
              else
              {
                if (tTriggerFiles_woL0)
                  tTriggerFiles_woL0->Fill();
              }
  ...
  if (fIsHeavyIon == 1)
  {
    ...
    double fEventPlaneAngleV0C = EventPlane->GetEventplane("V0C", fInputEvent, 2);
    double fEventPlaneAngleV0A = EventPlane->GetEventplane("V0A", fInputEvent, 2);
  }
  else
  ...
  for (Int_t iCut = 0; iCut < fnCuts; iCut++)
  {
    ...
    if (eventNotAccepted != 0)
    {
      ...
      {
        if (eventNotAccepted == 3)
        { // wrong trigger selected. However, we still want to count the MC particles fot these events! If MC particles should be rejected in addition, use IsMCTriggerSelected function
          if (fInputEvent->IsA() == AliESDEvent::Class())
    ...
    { // Event Not Accepted
      ...
      if (fIsMC > 0)
      {
        if (eventQuality == 3 || eventQuality == 5)
        { // 3 = trigger, 5 = contr. to vertex
          if (fInputEvent->IsA() == AliESDEvent::Class())
    ...
    if (fIsMC > 0)
    {
      ...
      if (((AliConvEventCuts *)fEventCutArray->At(iCut))->GetSignalRejection() != 0)
      {
        if (fInputEvent->IsA() == AliESDEvent::Class())
        {
          ((AliConvEventCuts *)fEventCutArray->At(iCut))->GetNotRejectedParticles(((AliConvEventCuts *)fEventCutArray->At(iCut))->GetSignalRejection(), ((AliConvEventCuts *)fEventCutArray->At(iCut))->GetAcceptedHeader(), fMCEvent);
        }
        else if (fInputEvent->IsA() == AliAODEvent::Class())
        {
          ((AliConvEventCuts *)fEventCutArray->At(iCut))->GetNotRejectedParticles(((AliConvEventCuts *)fEventCutArray->At(iCut))->GetSignalRejection(), ((AliConvEventCuts *)fEventCutArray->At(iCut))->GetAcceptedHeader(), fInputEvent);
        }
        if (!fDoLightOutput)
        {
    ...
    //  cout << "fiCut==========" << fiCut << endl;
    //  cout << "iCut============" << iCut << endl;
    //  cout << "centSPD1======" << centSPD1 << endl;
    ...
    if (((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetDoSecondaryTrackMatching())
      ProcessConversionCandidates(); // process conversion candidates for secondary track matching
    ...
    if (!fDoLightOutput)
      fHistoNGoodESDTracksVsNGammaCandidates[iCut]->Fill(fV0Reader->GetNumberOfPrimaryTracks(), fClusterCandidates->GetEntries(), fWeightJetJetMC);
    if (fDoMesonAnalysis)
    {                           // Meson Analysis
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::ProcessClusters()
{
  ...
  else
  {
    ...
    if (!arrClustersProcess)
      AliFatal(Form("%sClustersBranch was not found in AliAnalysisTaskGammaCaloV2! Check the correction framework settings!", fCorrTaskSetting.Data()));
  ...
  vector<clusterLabelV2> fTrueClusterLabels;
  ...
  for (Long_t i = 0; i < nclus; i++)
  {
    ...
    {
      if (fProduceTreeEOverP && ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->ClusterIsSelectedBeforeTrackMatch())
        mapIsClusterAcceptedWithoutTrackMatch[i] = 1;
      if (((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetIsAcceptedForBasicCounting())
    ...
    if (fIsMC > 0 && ((AliConvEventCuts *)fEventCutArray->At(fiCut))->GetSignalRejection() > 0)
    {
      ...
      {
        Int_t *mclabelsCluster = clus->GetLabels();
        if (fLocalDebugFlag > 1)
          cout << "testing if other labels in cluster belong to different header, need to test " << (Int_t)clus->GetNLabels() - 1 << " additional labels" << endl;
        for (Int_t l = 1; l < (Int_t)clus->GetNLabels(); l++)
        {
          if (((AliConvEventCuts *)fEventCutArray->At(fiCut))->IsParticleFromBGEvent(mclabelsCluster[l], fMCEvent, fInputEvent, fLocalDebugFlag) == 0)
            fIsOverlappingWithOtherHeader = kTRUE;
        }
        if (fLocalDebugFlag > 1 && fIsOverlappingWithOtherHeader)
          cout << "found overlapping header: " << endl;
      }
    }

    fHistoClusAllHeadersGammaPt[fiCut]->Fill(PhotonCandidate->Pt(), tempPhotonWeight);
    if (!fIsFromDesiredHeader)
      fHistoClusRejectedHeadersGammaPt[fiCut]->Fill(PhotonCandidate->Pt(), tempPhotonWeight);
  ...
  if (((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetIsConversionRecovery())
  {
    ...
    if (fLocalDebugFlag > 1 && rejected)
      cout << "found clusters to be rejected" << endl;
  }
      ...
      if (((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetClusterType() == 2)
      {
        if (fCaloTriggerMimicHelper[fiCut])
        {
          if ((fCaloTriggerMimicHelper[fiCut]->IsClusterIDBadMapTrigger(vectorCurrentClusters.at(iter)->GetCaloClusterRef())) > 0)
          {
            ...
            {
              if (((((AliConvEventCuts *)fEventCutArray->At(fiCut))->IsSpecialTrigger() == 6) && (fCaloTriggerMimicHelper[fiCut]->IsClusterIDTriggered(vectorCurrentClusters.at(iter)->GetCaloClusterRef()))) || (((AliConvEventCuts *)fEventCutArray->At(fiCut))->IsSpecialTrigger() == 0) || (((AliConvEventCuts *)fEventCutArray->At(fiCut))->IsSpecialTrigger() == 1))
              {
  ...
  for (Int_t iter = 0; iter < (Int_t)vectorCurrentClusters.size(); iter++)
  {
    ...
    if (!((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->CheckVectorForIndexAndAdd(vectorRejectCluster, iter, kFALSE))
    {
      ...
      if (((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetClusterType() == 2)
      {
        ...
        {
          if ((vectorCurrentClusters.at(iter)->E()) > 0)
          { // Analysis bad map protection; Energy of bad clusters set to 0
            if ((fCaloTriggerMimicHelper[fiCut]->IsClusterIDBadMapTrigger(vectorCurrentClusters.at(iter)->GetCaloClusterRef())) > 0)
            { // Good cluster by bad trigger map decision
            ...
            if (fIsMC == 0)
            {
              ...
              if (fCaloTriggerMimicHelper[fiCut])
              {
                ...
                if ((((AliConvEventCuts *)fEventCutArray->At(fiCut))->IsSpecialTrigger() == 1) || (((AliConvEventCuts *)fEventCutArray->At(fiCut))->IsSpecialTrigger() == 0))
                {
                  ...
                  if ((fCaloTriggerMimicHelper[fiCut]->GetEventChosenByTrigger()) && (fCaloTriggerMimicHelper[fiCut]->IsClusterIDBadMapTrigger(vectorCurrentClusters.at(iter)->GetCaloClusterRef())))
                  {
                    if (eventHasL0Flag_ClusE)
                    {
    ...
    {
      if (fLocalDebugFlag > 1)
        cout << "removed cluster: " << iter << endl;
    }
  }
  if (fLocalDebugFlag > 1 && rejected)
    cout << "reduced from: " << vectorCurrentClusters.size() << " to " << fClusterCandidates->GetEntries() << endl;
  ...
  if (fProduceCellIDPlots)
  {
    for (Long_t i = 0; i < nclus; i++)
    {
      ...
      if (maxClusterID == i && maxClusterID > -1)
  ...
  if (fProduceTreeEOverP)
  {
    ...
    for (Long_t i = 0; i < nclus; i++)
    {
      if (mapIsClusterAcceptedWithoutTrackMatch[i] != 1)
        continue;
      ...
      for (Int_t j = 0; j < nclus; j++)
      {
        if (i == j)
          continue;
        if (mapIsClusterAcceptedWithoutTrackMatch[j] != 1)
          continue;
      ...
      if (fIsMC > 0)
        fClusterClassification = ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->ClassifyClusterForTMEffi(clus, fInputEvent, fMCEvent, isESD);
  ...
  if (fProduceCellIDPlots || fProduceTreeEOverP)
    mapIsClusterAccepted.clear();

  if (fLocalDebugFlag == 2)
    EventDebugMethod();

  // Do more merged studies
  if (fDoMesonQA >= 10 && fTrueClusterLabels.size() > 1)
  { // we need two labels for this to be possible

    clusterLabelV2 thisLabel = fTrueClusterLabels.at(0);
    ...
    {
      clusterLabelV2 otherLabel = fTrueClusterLabels.at(b);
      ...
      if (otherLabel.mesonID != thisLabel.mesonID)
        continue;
    ...
    if ((pdg == 111) && isMerged)
      fHistoMCPi0GenFoundInOneCluster[fiCut]->Fill(pi0Pt, pi0Eta, fWeightJetJetMC);
    if ((pdg == 111) && isSep)
      fHistoMCPi0GenFoundInTwoCluster[fiCut]->Fill(pi0Pt, pi0Eta, fWeightJetJetMC);
    if ((pdg == 221) && isMerged)
      fHistoMCEtaGenFoundInOneCluster[fiCut]->Fill(pi0Pt, pi0Eta, fWeightJetJetMC);
    if ((pdg == 221) && isSep)
      fHistoMCEtaGenFoundInTwoCluster[fiCut]->Fill(pi0Pt, pi0Eta, fWeightJetJetMC);
  }
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::ProcessConversionCandidates()
{
  ...
  for (Int_t i = 0; i < fReaderGammas->GetEntriesFast(); i++)
  {
    ...
    if (!PhotonCandidate)
      continue;
    if (!fConversionCuts->PhotonIsSelected(PhotonCandidate, fInputEvent))
      continue;
    if (!fConversionCuts->InPlaneOutOfPlaneCut(PhotonCandidate->GetPhotonPhi(), fEventPlaneAngle))
      continue;
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::ProcessTrueClusterCandidates(AliAODConversionPhoton *TruePhotonCandidate, Double_t clusterM02)
{
  ...
  if (!fDoLightOutput && fDoClusterQA > 0)
    fHistoTrueNLabelsInClus[fiCut]->Fill(TruePhotonCandidate->GetNCaloPhotonMCLabels(), tempPhotonWeight);

  if (TruePhotonCandidate->GetCaloPhotonMCLabel(0) < 0)
    return;
  if (TruePhotonCandidate->GetNCaloPhotonMCLabels() > 0)
    Photon = (AliMCParticle *)fMCEvent->GetTrack(TruePhotonCandidate->GetCaloPhotonMCLabel(0));
  else
    return;
  ...
  if (fIsMC > 0 && ((AliConvEventCuts *)fEventCutArray->At(fiCut))->GetSignalRejection() == 4)
  {
    if (((AliConvEventCuts *)fEventCutArray->At(fiCut))->IsParticleFromBGEvent(TruePhotonCandidate->GetCaloPhotonMCLabel(0), fMCEvent, fInputEvent) == 2)
      tempPhotonWeight = 1;
  }
  ...
  else if (fDoClusterQA > 0)
    fHistoTrueClusEMNonLeadingPt[fiCut]->Fill(TruePhotonCandidate->Pt());
  if (fDoClusterQA > 0)
  {
    ...
    if (!TruePhotonCandidate->IsLargestComponentPhoton())
    {
      ...
      if (!fDoLightOutput)
        FillPhotonBackgroundM02Hist(TruePhotonCandidate, clusterM02, pdgCodeParticle);
    }
    if (!(TruePhotonCandidate->IsLargestComponentPhoton() || (TruePhotonCandidate->IsLargestComponentElectron() && TruePhotonCandidate->IsConversion())))
    {
      FillPhotonPlusConversionBackgroundHist(TruePhotonCandidate, pdgCodeParticle);
      if (!fDoLightOutput)
        FillPhotonPlusConversionBackgroundM02Hist(TruePhotonCandidate, clusterM02, pdgCodeParticle);
    }
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::ProcessTrueClusterCandidatesAOD(AliAODConversionPhoton *TruePhotonCandidate, Double_t clusterM02)
{
  const AliVVertex *primVtxMC = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX = primVtxMC->GetX();
  Double_t mcProdVtxY = primVtxMC->GetY();
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::ProcessAODMCParticles(Int_t isCurrentEventSelected)
{
  const AliVVertex *primVtxMC = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX = primVtxMC->GetX();
  Double_t mcProdVtxY = primVtxMC->GetY();
  ...
  if (!((AliConvEventCuts *)fEventCutArray->At(fiCut))->IsMCTriggerSelected(fInputEvent, fMCEvent))
  {
    if (isCurrentEventSelected == 0)
      fHistoMCEventsTrigg[fiCut]->Fill(2., fWeightJetJetMC); // event is rejected and triggered
    else
      fHistoMCEventsTrigg[fiCut]->Fill(3., fWeightJetJetMC); // event is rejected but not triggered
    return;
  }
  if (isCurrentEventSelected == 0)
    fHistoMCEventsTrigg[fiCut]->Fill(0., fWeightJetJetMC); // event is accepted and triggered
  else
    fHistoMCEventsTrigg[fiCut]->Fill(1., fWeightJetJetMC); // event is accepted but not triggered
  ...
  for (Long_t i = 0; i < fAODMCTrackArray->GetEntriesFast(); i++)
  {
    ...
    if (isPrimary)
    {
      ...
      if (((AliConvEventCuts *)fEventCutArray->At(fiCut))->GetSignalRejection() != 0)
      {
        ...
        if (isMCFromMBHeader == 2 && ((AliConvEventCuts *)fEventCutArray->At(fiCut))->GetSignalRejection() == 4)
      ...
      if (fDoMesonAnalysis)
      {
        ...
        if (((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))
            ...
            ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->MesonLeadTrackSelectionAODMC(fInputEvent, particle))
        {
          ...
          {
            if (isCurrentEventSelected == 1)
              fHistoMCPi0PtNotTriggered[fiCut]->Fill(particle->Pt(), weighted * tempParticleWeight); // All MC Pi0 in not triggered collisions
            else if (isCurrentEventSelected == 2)
              fHistoMCPi0PtNoVertex[fiCut]->Fill(particle->Pt(), weighted * tempParticleWeight); // All MC Pi0 in not triggered collisions
            ...
            if (fIsMC > 1)
              fHistoMCPi0WOEvtWeightPt[fiCut]->Fill(particle->Pt());
            if (fDoMesonQA > 0 && fDoMesonQA < 3)
            {
              ...
              if (fIsMC == 2)
                fHistoMCPi0PtJetPt[fiCut]->Fill(particle->Pt(), ((AliConvEventCuts *)fEventCutArray->At(fiCut))->GetMaxPtJet(), tempParticleWeight);
            }
          }
          else if (particle->GetPdgCode() == 221 && !fDoPi0Only)
          {
            if (isCurrentEventSelected == 1)
              fHistoMCEtaPtNotTriggered[fiCut]->Fill(particle->Pt(), weighted * tempParticleWeight); // All MC Eta in not triggered collisions
            else if (isCurrentEventSelected == 2)
              fHistoMCEtaPtNoVertex[fiCut]->Fill(particle->Pt(), weighted * tempParticleWeight); // All MC Eta in not triggered collisions
            ...
            if (fIsMC > 1)
              fHistoMCEtaWOEvtWeightPt[fiCut]->Fill(particle->Pt());
            if (fDoMesonQA > 0 && fDoMesonQA < 3)
            {
              ...
              if (fIsMC == 2)
                fHistoMCEtaPtJetPt[fiCut]->Fill(particle->Pt(), ((AliConvEventCuts *)fEventCutArray->At(fiCut))->GetMaxPtJet(), tempParticleWeight);
            }
          }
              ...
              ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->ClusterIsSelectedAODMC(daughter1, fAODMCTrackArray))
          {
            ...
            if (particle->GetPdgCode() == 111)
            {
              ...
              if (isCurrentEventSelected == 1)
                fHistoMCPi0InAccPtNotTriggered[fiCut]->Fill(particle->Pt(), weighted * tempParticleWeight); // MC Pi0 with gamma in acc for events which are not triggered
              if (fIsMC > 1)
                fHistoMCPi0WOEvtWeightInAccPt[fiCut]->Fill(particle->Pt()); // MC Pi0 with gamma in acc
            }
            else if (particle->GetPdgCode() == 221 && !fDoPi0Only)
            {
              fHistoMCEtaInAccPt[fiCut]->Fill(particle->Pt(), weighted * tempParticleWeight); // MC Eta with gamma in acc
              if (isCurrentEventSelected == 1)
                fHistoMCEtaInAccPtNotTriggered[fiCut]->Fill(particle->Pt(), weighted * tempParticleWeight); // MC Eta with gamma in acc for not triggered events
              if (fIsMC > 1)
                fHistoMCEtaWOEvtWeightInAccPt[fiCut]->Fill(particle->Pt()); // MC Eta with gamma in acc
            }
          }
        }
      }
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::ProcessMCParticles(Int_t isCurrentEventSelected)
{
  const AliVVertex *primVtxMC = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX = primVtxMC->GetX();
  Double_t mcProdVtxY = primVtxMC->GetY();
      ...
      if (fDoMesonQA >= 10)
      {
        ...
        { // only look at pi0
          ...
          if (nDaughters == 2)
          {
            ...
            if (nPhotons == 2)
              fHistoMCPi0GenVsNClus[fiCut]->Fill(particle->Pt(), nclus, fWeightJetJetMC);
      ...
      if (fDoMesonAnalysis)
      {
        ...
        if (fHistoMCPrimaryPtvsSource[fiCut])
        {
          if ((mesonY > ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GetRapidityCutValueMin()) && (mesonY < ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GetRapidityCutValueMax()))
          {
            ...
            else if (TMath::Abs(particle->PdgCode()) == 310)
            { // K0s
              fHistoMCPrimaryPtvsSource[fiCut]->Fill(particle->Pt(), 4., tempParticleWeight);
            }
            else if (TMath::Abs(particle->PdgCode()) == 130)
            { // K0l
              fHistoMCPrimaryPtvsSource[fiCut]->Fill(particle->Pt(), 5., tempParticleWeight);
            }
            else if (TMath::Abs(particle->PdgCode()) == 3122)
            { // Lambda/ AntiLambda
              fHistoMCPrimaryPtvsSource[fiCut]->Fill(particle->Pt(), 6., tempParticleWeight);
            }
          }
        ...
        if (((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))
            ...
            ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->MesonLeadTrackSelectionMC(fInputEvent, particle))
        {
    ...
    else
    {
      ...
      if (fDoMesonAnalysis)
      {
        if (((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->MesonIsSelectedMC(particle, fMCEvent, ((AliConvEventCuts *)fEventCutArray->At(fiCut))->GetEtaShift()) && ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->MesonLeadTrackSelectionMC(fInputEvent, particle))
        {
          ...
          Int_t pdgCode = -1;
          if (particle->GetMother() > -1)
            pdgCode = ((AliMCParticle *)fMCEvent->GetTrack(particle->GetMother()))->PdgCode();
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::CalculatePi0Candidates()
{
  ...
  if (fClusterCandidates->GetEntries() > 0)
  {
    ...
    for (Int_t firstGammaIndex = 0; firstGammaIndex < fClusterCandidates->GetEntries(); firstGammaIndex++)
    {
      ...
      if (fDoInOutTimingCluster)
      {
        ...
        if (tof < fMinTimingCluster || tof > fMaxTimingCluster)
          continue;
      }
      for (Int_t secondGammaIndex = firstGammaIndex + 1; secondGammaIndex < fClusterCandidates->GetEntries(); secondGammaIndex++)
      {
        ...
        {
          if (fLocalDebugFlag == 1)
            DebugMethodPrint1(pi0cand, gamma0, gamma1);
          ...
          if (pi0cand->Eta() > 0)
          {
            fHistoMotherInvMassPtPhi[fiCut]->Fill(pi0cand->M(), pi0cand->Pt(), pi0cand->Phi() - fPsi2V0C, tempPi0CandWeight);
          }
          else
          {
            fHistoMotherInvMassPtPhi[fiCut]->Fill(pi0cand->M(), pi0cand->Pt(), pi0cand->Phi() - fPsi2V0A, tempPi0CandWeight);
          }
          ...
          if ((((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetClusterType() == 2 && fDoMesonQA == 5 && fIsMC == 0 && ((pi0cand->M() > 0.05 && pi0cand->M() < 0.17) || (pi0cand->Pt() > 15. && pi0cand->M() > 0.45 && pi0cand->M() < 0.65))) || ((((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetClusterType() == 4 || ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetClusterType() == 1 || ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetClusterType() == 3) && fDoMesonQA == 5 && fIsMC == 0 && (pi0cand->M() > 0.0 && pi0cand->M() < 0.3)))
          {
...
void AliAnalysisTaskGammaCaloV2::ProcessTrueMesonCandidates(AliAODConversionMother *Pi0Candidate, AliAODConversionPhoton *TrueGammaCandidate0, AliAODConversionPhoton *TrueGammaCandidate1)
{
  // Process True Mesons
  const AliVVertex *primVtxMC = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX = primVtxMC->GetX();
  ...
  if (gamma0MCLabel != -1)
  { // Gamma is Combinatorial; MC Particles don't belong to the same Mother
    gammaMC0 = (AliMCParticle *)fMCEvent->GetTrack(gamma0MCLabel);
    if (TrueGammaCandidate0->IsLargestComponentPhoton() || TrueGammaCandidate0->IsLargestComponentElectron())
    { // largest component is electro magnetic
      // get mother of interest (pi0 or eta)
      tmpGammaMotherlabel = gammaMC0->GetMother();
      if (TrueGammaCandidate0->IsLargestComponentPhoton())
      { // for photons its the direct mother
        gamma0MotherLabel = gammaMC0->GetMother();
      }
      else if (TrueGammaCandidate0->IsLargestComponentElectron())
      { // for electrons its either the direct mother or for conversions the grandmother
    ...
    { // largest component is electro magnetic
      ...
      { // for electrons its either the direct mother or for conversions the grandmother
        if (TrueGammaCandidate1->IsConversion())
        {
          ...
          fRconv = gammaMC1->Particle()->R();
          if (convertedPhotonLabel1 > -1)
            gamma1MotherLabel = fMCEvent->GetTrack(convertedPhotonLabel1)->GetMother();
        }
        else
        {
          gamma1MotherLabel = gammaMC1->GetMother();
  ...
  { // True Pion or Eta
    if (isTruePi0)
    {
      ...
      if (previouslyNotFoundTrueMesons && !fDoLightOutput && fHistoTruePi0InvMassPtAdditional[fiCut])
        fHistoTruePi0InvMassPtAdditional[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), tempTruePi0CandWeight);
    ...
    if (isTrueEta && !fDoPi0Only)
    {
      ...
      if (previouslyNotFoundTrueMesons && !fDoLightOutput && fHistoTrueEtaInvMassPtAdditional[fiCut])
        fHistoTrueEtaInvMassPtAdditional[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), tempTruePi0CandWeight);
    }
    if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
    {
      ...
      {
        if (isTruePi0)
          fHistoTruePi0CaloPhotonInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
        if (isTrueEta && !fDoPi0Only)
          fHistoTrueEtaCaloPhotonInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
      ...
      {
        if (isTruePi0)
          fHistoTruePi0CaloElectronInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
        if (isTrueEta && !fDoPi0Only)
          fHistoTrueEtaCaloElectronInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
      ...
      {
        if (isTruePi0)
          fHistoTruePi0CaloMixedPhotonConvPhotonInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
        if (isTrueEta && !fDoPi0Only)
          fHistoTrueEtaCaloMixedPhotonConvPhotonInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
      ...
      {
        if (isTruePi0)
          fHistoTruePi0NonMergedElectronPhotonInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
      ...
      {
        if (isTruePi0)
          fHistoTruePi0NonMergedElectronMergedPhotonInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
      ...
      {
        if (isTruePi0)
          fHistoTruePi0CaloMergedClusterInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
        if (isTrueEta && !fDoPi0Only)
          fHistoTruePi0CaloMergedClusterInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
      ...
      {
        if (isTruePi0)
          fHistoTruePi0CaloMergedClusterPartConvInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
        if (isTrueEta && !fDoPi0Only)
          fHistoTrueEtaCaloMergedClusterPartConvInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
    }
    ...
    if (fDoMesonQA == 2 && fIsMC < 2)
    {
      ...
      {
        if (isTruePi0)
          fHistoTruePi0Category1[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
        if (isTrueEta && !fDoPi0Only)
          fHistoTrueEtaCategory1[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
      ...
      if ((TrueGammaCandidate0->IsLargestComponentPhoton() && !TrueGammaCandidate0->IsMerged()) && (TrueGammaCandidate1->IsLargestComponentElectron() && TrueGammaCandidate1->IsConversion()))
      {
        if (isTruePi0)
        {
          if (TrueGammaCandidate1->IsMergedPartConv())
            fHistoTruePi0Category2[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
          if (!TrueGammaCandidate1->IsMergedPartConv())
          {
        ...
        {
          if (TrueGammaCandidate1->IsMergedPartConv())
            fHistoTrueEtaCategory2[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
          if (!TrueGammaCandidate1->IsMergedPartConv())
            fHistoTrueEtaCategory3[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
        }
      }
      ...
      if ((TrueGammaCandidate0->IsLargestComponentElectron() && TrueGammaCandidate0->IsConversion()) && (TrueGammaCandidate1->IsLargestComponentElectron() && TrueGammaCandidate1->IsConversion()))
      {
        if (isTruePi0)
        {
          ...
          if (isSameConvertedGamma && !TrueGammaCandidate0->IsMergedPartConv() && !TrueGammaCandidate1->IsMergedPartConv())
            fHistoTruePi0Category4_6[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
          if (!isSameConvertedGamma)
          {
            if (!TrueGammaCandidate0->IsMergedPartConv() && !TrueGammaCandidate1->IsMergedPartConv())
            { // category 5: both electrons from different converted photons, electrons not merged
              fHistoTruePi0Category5[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
            }
            else if (TrueGammaCandidate0->IsMergedPartConv() && TrueGammaCandidate1->IsMergedPartConv())
            { // category 8: both electrons from different converted photons, both electrons merged
              fHistoTruePi0Category8[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
            }
            else
            { // category 7: both electrons from different converted photons, 1 electrons not merged, 1 electron merged
        ...
        if (isTrueEta && !fDoPi0Only)
        {
          // category 4: both electrons are from same conversion
          if (isSameConvertedGamma && !TrueGammaCandidate0->IsMergedPartConv() && !TrueGammaCandidate1->IsMergedPartConv())
            fHistoTrueEtaCategory4_6[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
          if (!isSameConvertedGamma)
          {
            if (!TrueGammaCandidate0->IsMergedPartConv() && !TrueGammaCandidate1->IsMergedPartConv())
            { // category 5: both electrons from different converted photons, electrons not merged
    ...
    { // Secondary Meson
      ...
      if (gamma0MotherLabel > -1)
        secMotherLabel = ((AliMCParticle *)fMCEvent->GetTrack(gamma0MotherLabel))->GetMother();
      ...
      if (secMotherLabel > -1)
      {
        if (fMCEvent->GetTrack(secMotherLabel)->PdgCode() == 310 && isTruePi0)
        {
          ...
          if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
            fHistoTrueK0sWithPi0DaughterMCPt[fiCut]->Fill(fMCEvent->GetTrack(secMotherLabel)->Pt(), tempTruePi0CandWeight);
          if (fDoMesonQA == 3)
          {
            iFlag = 3;
            if (!isSameConvertedGamma)
        ...
        else if (fMCEvent->GetTrack(secMotherLabel)->PdgCode() == 130 && isTruePi0)
        {
          ...
          if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
            fHistoTrueK0lWithPi0DaughterMCPt[fiCut]->Fill(fMCEvent->GetTrack(secMotherLabel)->Pt(), tempTruePi0CandWeight);
        }
        else if (fMCEvent->GetTrack(secMotherLabel)->PdgCode() == 221 && isTruePi0)
        {
          ...
          if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
            fHistoTrueEtaWithPi0DaughterMCPt[fiCut]->Fill(fMCEvent->GetTrack(secMotherLabel)->Pt(), tempTruePi0CandWeight);
          if (fDoMesonQA == 3)
          {
            iFlag = 4;
            if (!isSameConvertedGamma)
        ...
        else if (fMCEvent->GetTrack(secMotherLabel)->PdgCode() == 3122 && isTruePi0)
        {
          ...
          if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
            fHistoTrueLambdaWithPi0DaughterMCPt[fiCut]->Fill(fMCEvent->GetTrack(secMotherLabel)->Pt(), tempTruePi0CandWeight);
          if (fDoMesonQA == 3)
          {
            iFlag = 5;
            if (!isSameConvertedGamma)
    ...
    { // Only primary pi0 for efficiency calculation
      ...
      if (isTruePi0)
      {
        ...
        if (CheckVectorForDoubleCount(fVectorDoubleCountTruePi0s, gamma0MotherLabel))
          fHistoDoubleCountTruePi0InvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), weighted * tempTruePi0CandWeight);
      }
      else if (isTrueEta && !fDoPi0Only)
      {
        if (fDoMesonQA == 3)
        ...
        if (CheckVectorForDoubleCount(fVectorDoubleCountTrueEtas, gamma0MotherLabel))
          fHistoDoubleCountTrueEtaInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), weighted * tempTruePi0CandWeight);
      }

      if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
      {
        if (isTruePi0)
        { // Only primary pi0 for resolution
        ...
        if (isTrueEta && !fDoPi0Only)
        { // Only primary eta for resolution
  ...
  { // Background
    if (fDoMesonQA > 0 && fDoMesonQA < 3)
    {
      ...
      { // Both Tracks are Photons and have a mother but not Pi0 or Eta
        if (TrueGammaCandidate0->IsLargestComponentPhoton() && TrueGammaCandidate1->IsLargestComponentPhoton())
        {
        ...
        if (((((AliMCParticle *)fMCEvent->GetTrack(gamma0MotherLabel))->PdgCode() == 111 || ((AliMCParticle *)fMCEvent->GetTrack(gamma0MotherLabel))->PdgCode() == 221) && (TrueGammaCandidate0->IsMerged() || TrueGammaCandidate0->IsMergedPartConv())) ||
            ((((AliMCParticle *)fMCEvent->GetTrack(gamma1MotherLabel))->PdgCode() == 111 || ((AliMCParticle *)fMCEvent->GetTrack(gamma1MotherLabel))->PdgCode() == 221) && (TrueGammaCandidate1->IsMerged() || TrueGammaCandidate1->IsMergedPartConv())))
        {
...
//______________________________________________________________________
void AliAnalysisTaskGammaCaloV2::ProcessTrueMesonCandidatesAOD(AliAODConversionMother *Pi0Candidate, AliAODConversionPhoton *TrueGammaCandidate0, AliAODConversionPhoton *TrueGammaCandidate1)
{
  const AliVVertex *primVtxMC = fMCEvent->GetPrimaryVertex();
  Double_t mcProdVtxX = primVtxMC->GetX();
  Double_t mcProdVtxY = primVtxMC->GetY();
    ...
    if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
    {
      ...
      if (TrueGammaCandidate0->IsMerged() || TrueGammaCandidate0->IsMergedPartConv() || TrueGammaCandidate0->IsDalitzMerged() || TrueGammaCandidate1->IsMerged() || TrueGammaCandidate1->IsMergedPartConv() || TrueGammaCandidate1->IsDalitzMerged())
      {
        if (isTruePi0)
          fHistoTruePi0CaloMergedClusterInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
        if (isTrueEta && !fDoPi0Only)
          fHistoTrueEtaCaloMergedClusterInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
      }
      // at least one of the photon is merged and part conv
      if (TrueGammaCandidate1->IsMergedPartConv() || TrueGammaCandidate0->IsMergedPartConv())
      {
    ...
    if (fDoMesonQA == 2 && fIsMC < 2)
    {
      ...
      if ((TrueGammaCandidate0->IsLargestComponentElectron() && TrueGammaCandidate0->IsConversion()) && (TrueGammaCandidate1->IsLargestComponentElectron() && TrueGammaCandidate1->IsConversion()))
      {
        if (isTruePi0)
        {
          ...
          if (!isSameConvertedGamma)
          {
            if (!TrueGammaCandidate0->IsMergedPartConv() && !TrueGammaCandidate1->IsMerged())
            { // category 5: both electrons from different converted photons, electrons not merged
              fHistoTruePi0Category5[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
            }
            else if (TrueGammaCandidate0->IsMergedPartConv() && TrueGammaCandidate1->IsMerged())
            { // category 8: both electrons from different converted photons, both electrons merged
              fHistoTruePi0Category8[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt());
            }
            else
    ...
    { // Secondary Meson
      ...
      if (secMotherLabel > -1)
      {
        if (static_cast<AliAODMCParticle *>(fAODMCTrackArray->At(secMotherLabel))->GetPdgCode() == 310 && isTruePi0)
        {
          fHistoTrueSecondaryPi0FromK0sInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), weightedSec * tempTruePi0CandWeight);
          if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
            fHistoTrueK0sWithPi0DaughterMCPt[fiCut]->Fill(static_cast<AliAODMCParticle *>(fAODMCTrackArray->At(secMotherLabel))->Pt(), tempTruePi0CandWeight);
        }
        if (static_cast<AliAODMCParticle *>(fAODMCTrackArray->At(secMotherLabel))->GetPdgCode() == 130 && isTruePi0)
        {
          fHistoTrueSecondaryPi0FromK0lInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), weightedSec * tempTruePi0CandWeight);
          if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
            fHistoTrueK0lWithPi0DaughterMCPt[fiCut]->Fill(static_cast<AliAODMCParticle *>(fAODMCTrackArray->At(secMotherLabel))->Pt(), tempTruePi0CandWeight);
        }
        if (static_cast<AliAODMCParticle *>(fAODMCTrackArray->At(secMotherLabel))->GetPdgCode() == 221 && isTruePi0)
        {
          fHistoTrueSecondaryPi0FromEtaInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), weightedSec * tempTruePi0CandWeight);
          if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
            fHistoTrueEtaWithPi0DaughterMCPt[fiCut]->Fill(static_cast<AliAODMCParticle *>(fAODMCTrackArray->At(secMotherLabel))->Pt(), tempTruePi0CandWeight);
        }
        if (static_cast<AliAODMCParticle *>(fAODMCTrackArray->At(secMotherLabel))->GetPdgCode() == 3122 && isTruePi0)
        {
          fHistoTrueSecondaryPi0FromLambdaInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), weightedSec * tempTruePi0CandWeight);
          if (fDoMesonQA > 0 && fDoMesonQA < 3 && fIsMC < 2)
            fHistoTrueLambdaWithPi0DaughterMCPt[fiCut]->Fill(static_cast<AliAODMCParticle *>(fAODMCTrackArray->At(secMotherLabel))->Pt(), tempTruePi0CandWeight);
        }
      }
    }
    else
      ...
      { // Both Tracks are Photons and have a mother but not Pi0 or Eta
        ...
        if (gamma0MotherLabel < fAODMCTrackArray->GetEntries() && gamma1MotherLabel < fAODMCTrackArray->GetEntries())
        {
          ...
          if (trackgamma0 != NULL && trackgamma1 != NULL)
          {
            ...
            if (((((AliAODMCParticle *)fAODMCTrackArray->At(gamma0MotherLabel))->GetPdgCode() == 111 || ((AliAODMCParticle *)fAODMCTrackArray->At(gamma0MotherLabel))->GetPdgCode() == 221) && (TrueGammaCandidate0->IsMerged() || TrueGammaCandidate0->IsMergedPartConv())) ||
                ((((AliAODMCParticle *)fAODMCTrackArray->At(gamma1MotherLabel))->GetPdgCode() == 111 || ((AliAODMCParticle *)fAODMCTrackArray->At(gamma1MotherLabel))->GetPdgCode() == 221) && (TrueGammaCandidate1->IsMerged() || TrueGammaCandidate1->IsMergedPartConv())))
            {
              fHistoTrueBckFullMesonContainedInOneClusterInvMassPt[fiCut]->Fill(Pi0Candidate->M(), Pi0Candidate->Pt(), tempTruePi0CandWeight);
            }
            else if ((TrueGammaCandidate0->E() / Pi0Candidate->E() > 0.7) || (TrueGammaCandidate1->E() / Pi0Candidate->E() > 0.7))
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::ProcessAODSphericityParticles()
{
  if (!fAODMCTrackArray)
    fAODMCTrackArray = dynamic_cast<TClonesArray *>(fInputEvent->FindListObject(AliAODMCParticle::StdBranchName()));
  if (fAODMCTrackArray == NULL)
  ...
  for (Long_t i = 0; i < fAODMCTrackArray->GetEntriesFast(); i++)
  {
    ...
    else if (particle->GetPdgCode() == 421)
    { // D0 meson
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::CalculateBackground()
{
    ...
    for (Int_t nEventsInBG = 0; nEventsInBG < fBGHandler[fiCut]->GetNBGEvents(); nEventsInBG++)
    {
      ...
      for (Int_t iCurrent = 0; iCurrent < fClusterCandidates->GetEntries(); iCurrent++)
      {
        ...
        for (UInt_t iPrevious = 0; iPrevious < previousEventV0s->size(); iPrevious++)
        {
          ...
          AliAODConversionMother *backgroundCandidate = new AliAODConversionMother(&currentEventGoodV0, &previousGoodV0); // TODO background
          ...
          if ((((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))
               ...
               currentEventGoodV0.GetLeadingCellID(), previousGoodV0.GetLeadingCellID(), currentEventGoodV0.GetIsCaloPhoton(), previousGoodV0.GetIsCaloPhoton()) &&
              ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->MesonLeadTrackSelection(fInputEvent, backgroundCandidate))
          {
            ...
            {
              fHistoMotherBackInvMassPtdPhi[fiCut]->Fill(backgroundCandidate->M(), backgroundCandidate->Pt(), backgroundCandidate->Phi() - fPsi2V0C, tempBGCandidateWeight);
            }
            else
            {
              fHistoMotherBackInvMassPtdPhi[fiCut]->Fill(backgroundCandidate->M(), backgroundCandidate->Pt(), backgroundCandidate->Phi() - fPsi2V0A, tempBGCandidateWeight);
            }
            if (fDoTHnSparse)
            {
  ...
  else if (((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->UsePtmaxMethod())
  {
    ...
    Double_t currentPtMax = 0;
    Double_t previousPtMax = 0;
    Double_t currentAvePt = 0;
    Double_t previousAvePt = 0;
    Double_t currentAveEta = 0;
    Double_t previousAveEta = 0;
    Double_t currentAvePhi = 0;
    Double_t previousAvePhi = 0;
    ...
    for (Int_t nEventsInBG = 0; nEventsInBG < fBGHandler[fiCut]->GetNBGEvents(); nEventsInBG++)
    {
      ...
      if (previousEventV0s)
      {
        ...
        for (Int_t iCurrent = 0; iCurrent < fClusterCandidates->GetEntries(); iCurrent++)
        {
          ...
          if (currentV0->GetPhotonPt() > currentPtMax)
          {
            currentPtMax = currentV0->GetPhotonPt();
          }
        }
        ...
        for (UInt_t iPrevious = 0; iPrevious < previousEventV0s->size(); iPrevious++)
        {
          ...
          if (previousV0->GetPhotonPt() > previousPtMax)
          {
            previousPtMax = previousV0->GetPhotonPt();
          }
        }
        ...
        if (currentPtMax > 0. && previousPtMax > 0.)
        {
          ...
          if (TMath::Abs(previousAveEta - currentAveEta) < 0.4 && TMath::Abs(previousAvePhi - currentAvePhi) < 0.6 && (previousAvePt / currentAvePt) < 4. && (previousAvePt / currentAvePt) > 0.25)
            acceptedPtMax = kTRUE;
        }
        if (acceptedPtMax)
        {
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::CalculateBackgroundSwapp()
{
  ...
  if (((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->DoGammaSwappForBg())
  {
    ...
    std::vector<std::array<Double_t, 4>> vSwappingInvMassPT;
    ...
    if (fClusterCandidates->GetEntries() > 2)
    {
        ...
        for (Int_t iCurrent2 = iCurrent1 + 1; iCurrent2 < fClusterCandidates->GetEntries(); iCurrent2++)
        {
          ...
          for (int iSwapp = 0; iSwapp < ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GetNumberOfSwappsForBg(); ++iSwapp)
          {
            ...
            if ((((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GammaSwappMethodBg() == 0 || ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GammaSwappMethodBg() == 1))
            {
              ...
              else if (((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GammaSwappMethodBg() == 1)
              { // rotate by random angle between
            ...
            else if (((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GammaSwappMethodBg() >= 10)
            { // generate new decay with TGenPhaseSpace
            ...
            {
              if (currentEventGoodV0Temp1 == ((AliAODConversionPhoton *)kCurrentClusterCandidates) || currentEventGoodV0Temp2 == ((AliAODConversionPhoton *)kCurrentClusterCandidates))
              {
                continue;
              }
              ...
              if (!(((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->CheckDistanceToBadChannelSwapping(cellIDRotatedPhoton1, fInputEvent, ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GetDistanceToBorderForBg())) && lvRotationPhoton1.E() > ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetMinClusterEnergy())
              {
                ...
                {
                  vSwappingInvMassPT.push_back({backgroundCandidate1->M(), backgroundCandidate1->Pt(), backgroundCandidate1->Phi(), backgroundCandidate1->Eta()});
              ...
              if (!(((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->CheckDistanceToBadChannelSwapping(cellIDRotatedPhoton2, fInputEvent, ((AliConversionMesonCuts *)fMesonCutArray->At(fiCut))->GetDistanceToBorderForBg())) && lvRotationPhoton2.E() > ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetMinClusterEnergy())
              {
                ...
                {
                  vSwappingInvMassPT.push_back({backgroundCandidate2->M(), backgroundCandidate2->Pt(), backgroundCandidate1->Phi(), backgroundCandidate1->Eta()});
      ...
      for (Int_t i = 0; i < (Int_t)vSwappingInvMassPT.size(); i++)
      {
        ...
        {
          fHistoMotherBackInvMassPtdPhi[fiCut]->Fill(vSwappingInvMassPT.at(i)[0], vSwappingInvMassPT.at(i)[1], vSwappingInvMassPT.at(i)[2] - fPsi2V0C, tempMultWeightSwapping * fWeightJetJetMC);
        }
        else
        {
          fHistoMotherBackInvMassPtdPhi[fiCut]->Fill(vSwappingInvMassPT.at(i)[0], vSwappingInvMassPT.at(i)[1], vSwappingInvMassPT.at(i)[2] - fPsi2V0A, tempMultWeightSwapping * fWeightJetJetMC);
        }
      }
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::RotateParticle(AliAODConversionPhoton *gamma)
{
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::UpdateEventByEventData()
{
  ...
  if (fClusterCandidates->GetEntries() > 1)
  {
    ...
    { // means we use #V0s for multiplicity
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::FillPhotonBackgroundHist(AliAODConversionPhoton *TruePhotonCandidate, Int_t pdgCode)
{
  ...
  if (!fDoLightOutput)
  {
    if (fIsFromDesiredHeader)
    {
      ...
      if (TMath::Abs(pdgCode) == 11)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 0., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 211)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 1., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 2212)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 2., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 321)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 3., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 2112)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 4., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 310)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 5., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 3122)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 6., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 13)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 7., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 130)
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 8., tempParticleWeight);
      else
        fHistoClusPhotonBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 9., tempParticleWeight);
    }
  }
}

//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::FillPhotonPlusConversionBackgroundHist(AliAODConversionPhoton *TruePhotonCandidate, Int_t pdgCode)
{
  // Bck = 0 e+-, 1 pi+-, 2 p+-, 3 K+-, 4 n, 5 K0s, 6 Lambda, 7 mu+-, 8 K0l, 9 rest
  Double_t tempParticleWeight = fWeightJetJetMC;
  if (!fDoLightOutput && fHistoClusPhotonPlusConvBGPt[fiCut])
  {
    if (fIsFromDesiredHeader)
    {
      ...
      if (TMath::Abs(pdgCode) == 11)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 0., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 211)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 1., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 2212)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 2., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 321)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 3., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 2112)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 4., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 310)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 5., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 3122)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 6., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 13)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 7., tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 130)
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 8., tempParticleWeight);
      else
        fHistoClusPhotonPlusConvBGPt[fiCut]->Fill(TruePhotonCandidate->Pt(), 9., tempParticleWeight);
    }
  }
}

//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::FillPhotonBackgroundM02Hist(AliAODConversionPhoton *TruePhotonCandidate, Double_t clusterM02, Int_t pdgCode)
{
  ...
  if (!fDoLightOutput && fDoClusterQA > 1)
  {
    if (fIsFromDesiredHeader)
    {
      ...
      if (TMath::Abs(pdgCode) == 11)
        fHistoClustPhotonElectronBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 211)
        fHistoClustPhotonPionBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 2212)
        fHistoClustPhotonNeutronBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 321)
        fHistoClustPhotonKaonBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 130)
        fHistoClustPhotonK0lBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else
        fHistoClustPhotonRestBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
    }
  }
}

//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::FillPhotonPlusConversionBackgroundM02Hist(AliAODConversionPhoton *TruePhotonCandidate, Double_t clusterM02, Int_t pdgCode)
{
  // Bck = e+-, pi+-, n, K+-, K0l, rest
  Double_t tempParticleWeight = fWeightJetJetMC;
  if (!fDoLightOutput && fDoClusterQA > 1)
  {
    if (fIsFromDesiredHeader)
    {
      ...
      if (TMath::Abs(pdgCode) == 11)
        fHistoClustPhotonPlusConvElectronBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 211)
        fHistoClustPhotonPlusConvPionBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 2212)
        fHistoClustPhotonPlusConvNeutronBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 321)
        fHistoClustPhotonPlusConvKaonBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else if (TMath::Abs(pdgCode) == 130)
        fHistoClustPhotonPlusConvK0lBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
      else
        fHistoClustPhotonPlusConvRestBGPtM02[fiCut]->Fill(TruePhotonCandidate->Pt(), clusterM02, tempParticleWeight);
    }
  }
}

//_________________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::SetLogBinningXTH2(TH2 *histoRebin)
{
...
//_________________________________________________________________________________
Bool_t AliAnalysisTaskGammaCaloV2::CheckVectorForDoubleCount(vector<Int_t> &vec, Int_t tobechecked)
{
  if (tobechecked > -1)
  {
...
//________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::Terminate(const Option_t *)
{
...
//________________________________________________________________________
Int_t AliAnalysisTaskGammaCaloV2::GetSourceClassification(Int_t daughter, Int_t pdgCode)
{

  if (daughter == 111)
  {
    if (TMath::Abs(pdgCode) == 310)
      return 1; // k0s
    else if (TMath::Abs(pdgCode) == 3122)
      return 2; // Lambda
    else if (TMath::Abs(pdgCode) == 130)
      ...
      return 7; // kaon
    else if (TMath::Abs(pdgCode) == 113 || TMath::Abs(pdgCode) == 213)
      return 8; // rho 0,+,-
    else if (TMath::Abs(pdgCode) == 3222 || TMath::Abs(pdgCode) == 3212 || TMath::Abs(pdgCode) == 3112)
      return 9; // Sigma
    else if (TMath::Abs(pdgCode) == 2224 || TMath::Abs(pdgCode) == 2214 || TMath::Abs(pdgCode) == 2114 || TMath::Abs(pdgCode) == 1114)
      return 10; // Delta
    else if (TMath::Abs(pdgCode) == 313 || TMath::Abs(pdgCode) == 323)
      return 11; // K*
    else
      return 15;
  }
...
//_________________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::FillMultipleCountMap(map<Int_t, Int_t> &ma, Int_t tobechecked)
{
  if (ma.find(tobechecked) != ma.end())
    ma[tobechecked] += 1;
  else
    ma[tobechecked] = 2;
  return;
...
//_________________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::FillMultipleCountHistoAndClear(map<Int_t, Int_t> &ma, TH1F *hist)
{
...
//_________________________________________________________________________________
Int_t AliAnalysisTaskGammaCaloV2::WhichDDL(Int_t module, Int_t cellx)
{
...
//_________________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::DebugMethodPrint1(AliAODConversionMother *pi0cand, AliAODConversionPhoton *gamma0, AliAODConversionPhoton *gamma1)
{
  if (pi0cand->GetOpeningAngle() < 0.02)
    DebugMethod(pi0cand, gamma0, gamma1);
  return;
}

//_________________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::DebugMethod(AliAODConversionMother *pi0cand, AliAODConversionPhoton *gamma0, AliAODConversionPhoton *gamma1)
{
  if (fLocalDebugFlag != 1)
    return;
  fstream fOutputLocalDebug;
  fOutputLocalDebug.open("debugOutput.txt", ios::out | ios::app);
  if (!fOutputLocalDebug.is_open())
    return;
  ...
  for (Int_t iCell = 0; iCell < nCellCluster; iCell++)
  {
    ...
    ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetGeomEMCAL()->GetCellIndex(fInputEvent->GetCaloCluster(gamma0->GetCaloClusterRef())->GetCellAbsId(iCell), nSupMod, nModule, nIphi, nIeta);
    Int_t nphi, neta;
    ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetGeomEMCAL()->GetCellPhiEtaIndexInSModule(nSupMod, nModule, nIphi, nIeta, nphi, neta);
  ...
  {
    Int_t nSupMod, nModule, nIphi, nIeta;
    ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetGeomEMCAL()->GetCellIndex(fInputEvent->GetCaloCluster(gamma1->GetCaloClusterRef())->GetCellAbsId(iCell), nSupMod, nModule, nIphi, nIeta);
    Int_t nphi, neta;
    ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetGeomEMCAL()->GetCellPhiEtaIndexInSModule(nSupMod, nModule, nIphi, nIeta, nphi, neta);
...
//_________________________________________________________________________________
void AliAnalysisTaskGammaCaloV2::EventDebugMethod()
{
  if (fLocalDebugFlag != 2)
    return;
  fstream fOutputLocalDebug;
  ...
  for (Long_t i = 0; i < nclus; i++)
  {
    ...
    {
      Int_t nSupMod, nModule, nIphi, nIeta;
      ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetGeomEMCAL()->GetCellIndex(clus->GetCellAbsId(iCell), nSupMod, nModule, nIphi, nIeta);
      Int_t nphi, neta;
      ((AliCaloPhotonCuts *)fClusterCutArray->At(fiCut))->GetGeomEMCAL()->GetCellPhiEtaIndexInSModule(nSupMod, nModule, nIphi, nIeta, nphi, neta);
...
Int_t AliAnalysisTaskGammaCaloV2::CheckClustersForMCContribution(Int_t mclabel, Bool_t leading)
{
  ...
  for (Long_t i = 0; i < nclus; i++)
  {
    ...
    if (clus->GetNLabels() > 0)
    {
      ...
      {
        if ((mclabelsCluster[0] == mclabel))
          clusID = i;
      }
    }

    if (clusID != -1)
    { // do a check if its a conversion electron
...
Bool_t AliAnalysisTaskGammaCaloV2::CheckSpecificClusterForMCContribution(Int_t mclabel, Int_t cluslabel)
{
  TClonesArray *arrClustersProcess = NULL;
  fNCurrentClusterBasic = 0;
  if (fCorrTaskSetting.CompareTo(""))
...
Int_t AliAnalysisTaskGammaCaloV2::CountPhotonsInCluster(Int_t cluslabel)
{
  TClonesArray *arrClustersProcess = NULL;
  fNCurrentClusterBasic = 0;
  Int_t nphotons = 0;
...
void AliAnalysisTaskGammaCaloV2::DoClusterMergingStudies(AliVCluster *clus, vector<clusterLabelV2> &labelvect)
{
  // vertex
  Double_t vertex[3] = {0};
  InputEvent()->GetPrimaryVertex()->GetXYZ(vertex);
  ...
  if (clus->GetNLabels() > 0)
  {
    for (Int_t k = 0; k < (Int_t)clus->GetNLabels(); k++)
    {
      ...
      if (pi0Pos == -1)
        continue; // label does not belong to pi0
      ...
      if (clusParticleMother->GetNDaughters() != 2)
        continue;
      ...
      if ((daughter1->PdgCode() != 22) || (daughter2->PdgCode() != 22))
        continue;
      TVector3 vec1, vec2;
      ...
      if (TMath::Abs(clusParticle->PdgCode()) == 11)
      {
        ...
        if (pdgMoth != 22)
          continue; // we only consider labels of electrons if they come from gamma
...
void AliAnalysisTaskGammaCaloV2::DoClusterMergingStudiesAOD(AliVCluster *clus, vector<clusterLabelV2> &labelvect)
{
  Int_t *mclabelsClus = clus->GetLabels();
  if (clus->GetNLabels() > 0)
  {
    for (Int_t k = 0; k < (Int_t)clus->GetNLabels(); k++)
    {
      ...
      if ((daughter1->GetPdgCode() != 22) || (daughter2->GetPdgCode() != 22))
        continue;
      TVector3 vec1, vec2;
      vec1.SetXYZ(daughter1->Px(), daughter1->Py(), daughter1->Pz());
      vec2.SetXYZ(daughter2->Px(), daughter2->Py(), daughter2->Pz());
...
double AliAnalysisTaskGammaCaloV2::GetEventPlane(double qx, double qy, double harmonic)
{
