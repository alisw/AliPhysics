...
#ifndef AliAnalysisTaskFlowModesMC_H
#define AliAnalysisTaskFlowModesMC_H
...
class AliAnalysisTaskFlowModesMC : public AliAnalysisTaskSE
{
public:
    enum    RunMode {kTest, kFillWeights, kFull}; // task running mode (NOT GRID MODE)
    enum    AnalType {kAOD, kESD, kMC, kMCAOD}; // tag for analysis type
    ...
    AliAnalysisTaskFlowModesMC(); // constructor
    AliAnalysisTaskFlowModesMC(const char *name); // named (primary) constructor
    virtual                 ~AliAnalysisTaskFlowModesMC(); // destructor
    ...
    void                    SetImpactParameterRange(Double_t min=0., Double_t max=14.) { fImpactParameterMin=min;fImpactParameterMax=max;fCutImpactParameter=kTRUE; }
...
private:
    ...
    // properties
    AliAODEvent*            fEventAOD; //! AOD event container
    AliMCEvent*             fEventMC; //! MC event container
    ...
    Double_t                fIndexImpactParameter; // impact parameter of the collision in MC studies
    ...
    Bool_t                  fCutImpactParameter; // Cut on the impact parameter
    ...
    Double_t          fImpactParameterMin; // [0] min range for impact parameter
    Double_t          fImpactParameterMax; // [50] max range for impact parameter
    ...
    const static Int_t      fNumMixedHarmonics = 4; //4 number of mixed harmonics: 4{psi2}, 6{psi3} and 5{psi2,3}
    ...
    Double_t                GetImpactParameter(); // returns the impact parameter or the number of selected tracks
    ...
    void                    FillQARefs(const Short_t iQAindex, const AliMCParticle* particle = 0x0); // filling QA plots for RFPs selection

    void                    FillQACharged(const Short_t iQAindex, const AliAODTrack* track = 0x0); // filling QA plots for charged track selection
    void                    FillQACharged(const Short_t iQAindex, const AliMCParticle* particle = 0x0); // filling QA plots for charged track selection
    ...
    void                    FillPIDQA(const Short_t iQAindex, const AliMCParticle* particle = 0x0, const PartSpecies species = kUnknown); // filling pi,K,p QA histograms
    ...
    AliAnalysisTaskFlowModesMC(const AliAnalysisTaskFlowModesMC&); // not implemented
    AliAnalysisTaskFlowModesMC& operator=(const AliAnalysisTaskFlowModesMC&); // not implemented
    
    ClassDef(AliAnalysisTaskFlowModesMC, 4);
};
