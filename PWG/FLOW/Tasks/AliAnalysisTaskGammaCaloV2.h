...
struct clusterLabelV2
{
  clusterLabelV2() : mesonID(0), clusID(0), daughterID(0), daughterPDG(0),
...
class AliAnalysisTaskGammaCaloV2 : public AliAnalysisTaskSE
{
public:
  AliAnalysisTaskGammaCaloV2();
  AliAnalysisTaskGammaCaloV2(const char *name);
  virtual ~AliAnalysisTaskGammaCaloV2();
  ...
  void SetCaloTriggerHelperName(TString name)
  {
    fCaloTriggerHelperName = name;
    return;
  }
  ...
  {
    fDoInOutTimingCluster = kTRUE;
    fMinTimingCluster = min;
    fMaxTimingCluster = max;
    return;
  }
  ...
  void DoClusterMergingStudies(AliVCluster *clus, vector<clusterLabelV2> &labelvect);
  void DoClusterMergingStudiesAOD(AliVCluster *clus, vector<clusterLabelV2> &labelvect);
...
private:
  ...
  AliAnalysisTaskGammaCaloV2 &operator=(const AliAnalysisTaskGammaCaloV2 &); // Prevent assignment

  ClassDef(AliAnalysisTaskGammaCaloV2, 94);
};
