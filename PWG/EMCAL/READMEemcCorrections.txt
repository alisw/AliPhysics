...
The new EMCal corrections framework centralizes all of the various EMCal corrections to ensure that everyone
is using the same corrections and getting the same results. In addition, it provides a simple and unified interface.
Instead of error-prone task of setting arguments through ``AddTask`` macros, configuration of each component is
performed using a simple configuration file.
...
There are a few special (and rare) exceptions - contact the developers for further infomration if you think this
applies to your case.

This matters in particular because we always write corrections in place - it is generally not possible to write out new collections.

## Switching to the EMCal Corrections Framework

For those who are swtiching to this framework, a special page has been prepared to explain the process.
...
For the instrauctions, please see \subpage READMEemcCorrectionsChange.

# %Setup the Correction Task 

There are two steps:
- [Confiure the Correction Add Task](\ref configureEMCalCorrectionRunMacro)
...
more information on its use can be through the examples available [here](https://en.wikipedia.org/wiki/YAML). **All**
...
The general approach is very similar to the sterring in ``AliAnalysisTaskEmcal``. Roughly, your task should derive from ``AliEmcalCorrectionComponent``
and implement:
...
Until this section is expanded further, for more information, contact the developers.
