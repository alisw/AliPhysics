/**
 * Add Macro for the EMCAL preparation chain for the
 * new jet framework
 * @param perstr data / MC period
 * @param clusterizer clusterizing method
 * @param physsel Physics selection settings
 * @param pass Reconstruction pass
 * @return Clusterizer task
 */
AliAnalysisTaskSE *AddTaskEmcalPreparationNew(
    const char *perstr  = "LHC11h",
    UInt_t clusterizer  = AliEMCALRecParam::kClusterizerv2,
    UInt_t physsel = AliVEvent::kAnyINT,
  ...
  AliEmcalClusterMaker *clusMaker = AddTaskEmcalClusterMaker(nonLinFunct,remExoticClus,0,"usedefault",0.,kTRUE);
#endif
  ...
  std::cout << "Calling Add macro using command string " << clustertrackmatcheraddstring << std::endl;
  AliEmcalClusTrackMatcherTask *pMatcherTask = reinterpret_cast<AliEmcalClusTrackMatcherTask *>(gROOT->ProcessLine(clustertrackmatcheraddstring.c_str()));
#else
...
#endif
  pMatcherTask->SelectCollisionCandidates(physsel);
  ...
  std::cout << "Calling Add macro using command string " << hadcorraddstring << std::endl;
  AliHadCorrTask *pHadCorrTask = reinterpret_cast<AliHadCorrTask *>(gROOT->ProcessLine(hadcorraddstring.c_str()));
#else
  gROOT->LoadMacro("$ALICE_PHYSICS/PWG/EMCAL/macros/AddTaskHadCorr.C");
  AliHadCorrTask *pHadCorrTask = AddTaskHadCorr("usedefault", "usedefault", "", kHadCorrF, 0.15, 0.030, 0.015, 0, kTRUE, kFALSE);
#endif
  pHadCorrTask->SelectCollisionCandidates(physsel);
