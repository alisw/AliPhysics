...
private:
    ...
    std::vector<float>
    fBinsMultCent{};        /*!< centrality or multiplicity binning */
    std::vector<float> fBinsPt; /*!< pT binning */
    TH3F*
    fHistMCGenPrimTrackParticle;  /*!< Histogram for MC particle information
                                  */
    TH3F*
    fHistDataFractions; /*!< Histogram for particle abundances from
                         published data */
    TH3F*
    fHistMCFractions; /*!< Histogram for particle abundances from MC */
    TH3F*
    fHistMCWeights;     /*!< Histogram for weight factors to re-weight MC
                         abundances to data ones. */
...
default; /*!<  copy assignment */
    ...
    float GetMCSpectraWeight(
                             ...
                             float eventMultiplicityOrCentrality); /*!< main function to use. Will
    ...
    void FillMCSpectra(
                       AliMCEvent* mcEvent); /*!< function to fill internal mc spectra
                                              for calculation of weight factors*/
    ...
    // Getter
    const std::vector<float>& GetBinsPt() { return fBinsPt; }
