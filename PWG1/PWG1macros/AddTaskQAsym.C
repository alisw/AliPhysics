AliAnalysisTaskSE* AddTaskQAsym()
{
  ...
  if (!mgr->GetInputEventHandler()) {
   ...
   AliAnalysisTaskQASym *task = new AliAnalysisTaskQASym("AliAnalysisTaskQASym");
   ...
   task->SetCuts(esdTrackCutsL);

   mgr->AddTask(task);
  
   AliAnalysisDataContainer *cout  = mgr->CreateContainer("QAsymHists",TList::Class(),
							AliAnalysisManager::kOutputContainer,"QAsym.root");
