...
vector<serverListStruct> AliStorageDatabase::GetList(struct listRequestStruct list)
{
    ...
    cout<<"QUERY:"<<Form("SELECT * FROM %s WHERE run_number >= %d AND run_number <= %d AND event_number >= %d AND event_number <= %d AND multiplicity >= %d AND multiplicity <= %d AND (permanent = %d OR permanent = %d) AND (system = '%s' OR system = '%s') AND trigger_mask = %ld AND trigger_mask_next = %ld ORDER BY run_number,event_number;",
                         fTable.c_str(),
                         list.runNumber[0],
                         list.runNumber[1],
                         list.eventNumber[0],
    ...
    {
        result =  fServer->Query(Form("SELECT * FROM %s WHERE run_number >= %d AND run_number <= %d AND event_number >= %d AND event_number <= %d AND multiplicity >= %d AND multiplicity <= %d AND (permanent = %d OR permanent = %d) AND (system = '%s' OR system = '%s') AND trigger_mask = %ld AND trigger_mask_next = %ld ORDER BY run_number,event_number;",
                                      fTable.c_str(),
                                      list.runNumber[0],
                                      list.runNumber[1],
                                      list.eventNumber[0],
