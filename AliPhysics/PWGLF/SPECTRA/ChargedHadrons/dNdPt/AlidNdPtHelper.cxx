...
Bool_t AlidNdPtHelper::IsPrimaryParticle(AliStack* const stack, Int_t idx, ParticleMode particleMode)
{
    ...
    if(particleMode==kMCLambda) {
        if(prim && pdg==3122) return kTRUE;
        else return kFALSE;
    }

    if(particleMode==kMCK0)
    {
        if(prim && pdg==311) return kTRUE;
        else return kFALSE;
    }
    ...
    if (particleMode==kMCSigmaPlus) {
        if(prim && pdg==3222) return kTRUE;
        else return kFALSE;
    }

    if (particleMode==kMCSigmaMinus) {
        if(prim && pdg==3112) return kTRUE;
        else return kFALSE;
    }

    if(particleMode==kMCXiMinus)
    {
        if(prim && pdg==3312) return kTRUE;
        else return kFALSE;
    }

    if(particleMode==kMCElectron)
    {
        if(prim && pdg==11) return kTRUE;
        else return kFALSE;
    }

    if(particleMode==kMCMuon)
    {
        if(prim && pdg==13) return kTRUE;
        else return kFALSE;
    }

    if(particleMode==kMCOmegaMinus)
    {
        if(prim && pdg==3334) return kTRUE;
        else return kFALSE;
    }

    if(particleMode==kMCRest) {
        if(prim && pdg!=kPiPlus && pdg!=kKPlus && pdg!=kProton && pdg!=3222 && pdg!=3112) return kTRUE;
        else return kFALSE;
    }

    if(particleMode==kMCRemainingRest) {
        if(prim && pdg!=kPiPlus && pdg!=kKPlus && pdg!=kProton && pdg!=3222 && pdg!=3112 && pdg!=3312 && pdg!=11 && pdg!=3334 && pdg!=13) return kTRUE;
        else return kFALSE;
    }

    if(particleMode==kInclWoSimga)/// all primary inclusive charged particles without charged sigmas
    {
        if(prim && pdg!=3222 && pdg!=3112) return kTRUE;
            else return kFALSE;
    }
