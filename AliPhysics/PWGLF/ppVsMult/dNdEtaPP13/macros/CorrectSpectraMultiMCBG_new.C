...
const char* figDir = "figures";//"figMult";
const char* resDir = "output";//"resMult";
TString  useBgType    = "";
...
//Int_t    useScaleType = kSclWghMean;       // which type of tails normalization to use

Bool_t   useZbinWAv  = kFALSE;//kFALSE;
//Bool_t   useZbinWAv  = kTRUE;
...
void    CorrectSpectraMultiMCBG_new(const char* flNameData, const char* flNameMC, const char *nameL = "clist", const char* unique="", Bool_t forceMCMB = kFALSE , int maxBins=20);
...
void CorrectSpectraMultiMCBG_new(const char* flNameData, const char* flNameMC, const char *nameL, const char* uniqueNm, Bool_t forceMCMB ,int maxBin)
{

    printf("--->>> CorrectSpectraMultiMCBG_new(\"%s\", \"%s\", \"%s\", \"%s\", %d, %d)\n", flNameData, flNameMC, nameL, uniqueNm, forceMCMB, maxBin);
...
//    gROOT->LoadMacro("SaveCanvas.C+");
    ...
    double myMergeFactor = hstat->GetBinContent(kOneUnit+4);
    ...
    //
    kdPhiSgCut  = hstat->GetBinContent(kDPiSCut+4)/myMergeFactor;
    kWDistSgCut = hstat->GetBinContent(kNStdCut+4)/myMergeFactor;
    ...
    sprintf(outStr,"CutEta%.1f_%.1f_Zv%.1f_%.1f_bg%s_Shape_%s_mcLB%d_cutSig%.1f_cutBg%.1f",
            hstat->GetBinContent(kEtaMin+4)/myMergeFactor,
            hstat->GetBinContent(kEtaMax+4)/myMergeFactor,
            hstat->GetBinContent(kZVMin+4)/myMergeFactor,
            hstat->GetBinContent(kZVMax+4)/myMergeFactor,
            useBgType.Data(),
...
Bool_t PrepareHistos(int bin, TList* lst, Bool_t isMC)
{
  ...
  printf("--->>> PrepareHistos(%d, %p, %d)\n", bin, lst, isMC);
    ...
    if (isMC) {
        ...
        //
        double myMergeFactor = hstat->GetBinContent(kOneUnit+4);
        double zmn = hstat->GetBinContent(kZVMin+4)/myMergeFactor;
        double zmx = hstat->GetBinContent(kZVMax+4)/myMergeFactor;
...
void PlotResults(Bool_t forceMCMB)
{
    ...
    for (int i=0;i<nCentBins;i++) {
        ...
        sprintf(outTitle,"%s, %d<C_%s<%d, %.1f<#eta<%.1f, %.1f<Z_{V}<%.1f,  Bg.:%s, UseMCLB=%d, CutVar:%s, |sig|<%.2f, %.2f<|bg.nrm|<%.2f",
                uniqueName.Data(),
                (int)binArr[i],
                hstat->GetXaxis()->GetBinLabel(kCentVar+4),
                (int)binArr[i+1],
                hstat->GetBinContent(kEtaMin+4)/myMergeFactor,
                hstat->GetBinContent(kEtaMax+4)/myMergeFactor,
                hstat->GetBinContent(kZVMin+4)/myMergeFactor,
...
Int_t CheckStat(const TList* lst, const char* dtType)
{
    ...
    TString dts = dtType;
    if (dts=="Data") return int( hstat->GetBinContent(kEvProcData+4) );
    if (dts=="Mix")  return int( hstat->GetBinContent(kEvProcMix+4) );
    if (dts=="Inj")  return int( hstat->GetBinContent(kEvProcInj+4) );
    if (dts=="Rot")  return int( hstat->GetBinContent(kEvProcRot+4) );
