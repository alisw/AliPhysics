	     ...
	     Bool_t   sat=false)
{
  ...
  TString sumDir("ForwardELossSums"); 
  TString resDir("ForwardELossResults");
  if (fileName.Contains("trending")) 
    sumDir = resDir = "forwardQAResults";
  ...
  if (fmdEx.Init(fname, sumDir, "fmd_corrections.root")) 
    fmdEx.Extract(AliFMDCorrELossFit::Class(),
		  Form("%s/fmdEnergyFitter",resDir.Data()));
