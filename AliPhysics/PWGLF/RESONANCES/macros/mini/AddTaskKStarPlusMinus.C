...
AliRsnMiniAnalysisTask *AddTaskKStarPlusMinus
(
 Bool_t      isMC,
 Int_t       collSyst,
 Float_t     cutV = 10.0,
...
{  
  ...
  ::Info("AddTaskKStarPlusMinus", Form("Min rapidity = %6.3f, Max rapidity = %6.3f",  minYlab, maxYlab) );
  ...
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
   ...
   TString taskName = Form("KStarPlusMinus%s%s_%.1f_%d_%.1f_%.1f_%.2f_%.4f_%.2f_%.2f_%.1f", 
  ...
  ::Info("AddTaskKStarPlusMinus", Form("Event mixing configuration: \n events to mix = %i \n max diff. vtxZ = cm %5.3f \n max diff multi = %5.3f \n max diff EP angle = %5.3f deg", nmix, maxDiffVzMix, maxDiffMultMix, ( !collSyst ? 0.0 : maxDiffAngleMixDeg)));
   ...
   if (collSyst==kPPb) { 
     ...
     ::Info("AddTaskKStarPlusMinus", Form(":::::::::::::::::: Vertex cut as pA 2013: %s", (useVtxCut2013pA?"ON":"OFF")));   
     if (useMVPileUpSelection){
       ...
       ::Info("AddTaskKStarPlusMinus", Form("Multiple-vtx Pile-up rejection settings: MinPlpContribMV = %i, MinPlpContribSPD = %i", 
				       MinPlpContribMV, MinPlpContribSPD));
     } else {
       cutEventUtils->SetMinPlpContribSPD(MinPlpContribSPD);
       ::Info("AddTaskKStarPlusMinus", Form("SPD Pile-up rejection settings: MinPlpContribSPD = %i", MinPlpContribSPD));
     }
     ::Info("AddTaskKStarPlusMinus", Form(":::::::::::::::::: Pile-up rejection mode: %s", (rejectPileUp?"ON":"OFF")));   
     ::Info("AddTaskKStarPlusMinus", Form("::::::::::::: Remove first event in chunk: %s", (rmFirstEvtChunk?"ON":"OFF")));   
   }
   ...
   //
   // -- CONFIG ANALYSIS --------------------------------------------------------------------------
   gROOT->LoadMacro("$ALICE_PHYSICS/PWGLF/RESONANCES/macros/mini/ConfigKStarPlusMinus.C");
   if (isMC) {
   ...
   if (!ConfigKStarPlusMinus(task, collSyst, isMC, piPIDCut, pPIDCut, aodFilterBit, trackDCAcut, massTol, lambdaDCA, lambdaCosPoinAn, lambdaDaughDCA, NTPCcluster, "", cutsPair)) return 0x0;
