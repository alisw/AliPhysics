#ifndef ALICALOPHOTONC_H
#define ALICALOPHOTONC_H
...
class AliCaloPhotonC :public TLorentzVector{
  
 public:
  
  AliCaloPhotonC() ;
  AliCaloPhotonC(Double_t px,Double_t py,Double_t pz,Double_t E) ; 
  ~AliCaloPhotonC(){} 
...
//Id of pi0 from which this photon is decayed (to check if 2 photons originate from the same pi0 or not)
   ...
   Int_t ComparePi0Ids( AliCaloPhotonC *phot) { if(AliCaloPhotonC::fPi0Id!=0 && (*phot).fPi0Id !=0 && AliCaloPhotonC::fPi0Id == (*phot).fPi0Id) return 1; else return 0; }
   ...
   void SetCPVBit2(Bool_t cpv){fCpv2 = cpv; }
