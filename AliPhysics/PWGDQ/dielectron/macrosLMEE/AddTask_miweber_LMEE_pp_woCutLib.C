AliAnalysisTask *AddTask_miweber_LMEE_pp_woCutLib(Int_t cutDefinition = 1, TString outputFileName = "AnalysisResult.root", TString directoryBaseName = "miweber_LMEE_pp", Bool_t isNano = kFALSE, Bool_t bCutQA = kTRUE){
  ...
  if (!mgr) {
    Error("AddTask_miweber_LMEE_pp_woCutLib", "No analysis manager found.");
    return 0;
  }
  ...
  TString configFile("Config_miweber_LMEE_pp_woCutLib.C");
  ...
  if (mgr->GetInputEventHandler()->IsA()==AliAODInputHandler::Class()){
    ::Info("AddTask_miweber_LMEE_pp_woCutLib", "no dedicated AOD configuration");
  }
  else if (mgr->GetInputEventHandler()->IsA()==AliESDInputHandler::Class()){
    ::Info("AddTask_miweber_LMEE_pp_woCutLib","switching on ESD specific code");
    bESDANA=kTRUE;
  }
