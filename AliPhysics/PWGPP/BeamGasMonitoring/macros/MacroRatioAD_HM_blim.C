...
GetRatio(Int_t split=-1){
    ...
    TList *list = (TList*)dir->Get("cOutputH_HM");
    
    
    TH2F *hTotalTrkVsClsSPID = (TH2F*)list->FindObject("hTotalTrkVsClsSPID_HM");    hTotalTrkVsClsSPID->GetXaxis()->SetTitle("Tracklet");
    hTotalTrkVsClsSPID->GetYaxis()->SetTitle("Cluster (fspdC1)");
    
    TH2F *hTotalV0 = (TH2F*)list->FindObject("hTotalV0_HM");
    hTotalV0->GetXaxis()->SetTitle("V0A-V0C");
    hTotalV0->GetYaxis()->SetTitle("V0A+V0C");
    
    TH2F *hTotalAD = (TH2F*)list->FindObject("hTotalAD_HM");
    ...
    for(int k=0; k<3; k++){
        ...
        hNumEffPurityBC[i][j][k] =(TH1F*)list->FindObject(Form("hNumEffPurityBC_HM%d_V0%01d_Flag%d",i,j,k));
        hDenomEffBC[i][j][k]=(TH1F*)list->FindObject(Form("hDenomEffBC_HM%d_V0%01d_Flag%d",i,j,k));
        hDenomPurityBC[i][j][k]=(TH1F*)list->FindObject(Form("hDenomPurityBC_HM%d_V0%01d_Flag%d",i,j,k));
        hDenomRejecEffBC[i][j][k]=(TH1F*)list->FindObject(Form("hDenomRejecEffBC_HM%d_V0%01d_Flag%d",i,j,k));
        hNumRejecEffBC[i][j][k]=(TH1F*)list->FindObject(Form("hNumRejecEffBC_HM%d_V0%01d_Flag%d",i,j,k));
        hSPDNumBC[i][j][k]=(TH1F*)list->FindObject(Form("hSPDNumBC_HM%d_V0%01d_Flag%d",i,j,k));
        hSPDDenomBC[i][j][k]=(TH1F*)list->FindObject(Form("hSPDDenomBC_HM%d_V0%01d_Flag%d",i,j,k));
        
        hNumTrkVsClsSPID[i][j][k] = (TH2F*)list->FindObject(Form("hNumTrkVsClsSPID_HM%d_V0%d_Flag%d",i,j,k));
        hDenomTrkVsClsSPID[i][j][k]=(TH2F*)list->FindObject(Form("hDenomTrkVsClsSPID_HM%d_V0%d_Flag%d",i,j,k));
        hNumV0[i][j][k] = (TH2F*)list->FindObject(Form("hNumV0_HM%d_V0%d_Flag%d",i,j,k));
        hDenomV0[i][j][k] = (TH2F*)list->FindObject(Form("hDenomV0_HM%d_V0%d_Flag%d",i,j,k));
        
        hADNumEffPurityBC[i][j][k] =(TH1F*)list->FindObject(Form("hADNumEffPurityBC_HM%d_AD%01d_Flag%d",i,j,k));
        hADDenomPurityBC[i][j][k] =(TH1F*)list->FindObject(Form("hADDenomPurityBC_HM%d_AD%01d_Flag%d",i,j,k));
        hADNumRejecEffBC[i][j][k] =(TH1F*)list->FindObject(Form("hADNumRejecEffBC_HM%d_AD%01d_Flag%d",i,j,k));
        hADNumTrkVsClsSPD[i][j][k] = (TH2F*)list->FindObject(Form("hADNumTrkVsClsSPD_HM%d_AD%d_Flag%d",i,j,k));
        hNumAD[i][j][k] = (TH2F*)list->FindObject(Form("hNumAD_HM%d_AD%d_Flag%d",i,j,k));
        hDenomAD[i][j][k] = (TH2F*)list->FindObject(Form("hDenomAD_HM%d_AD%d_Flag%d",i,j,k));
            }
        }
