...
AliAnalysisTaskADPilot::AliAnalysisTaskADPilot() 
  : AliAnalysisTaskSE(),fAnalysisUtils(0), fTriggerAnalysis(0), fListHist(0),
...
AliAnalysisTaskADPilot::AliAnalysisTaskADPilot(const char *name) 
  : AliAnalysisTaskSE(name),fAnalysisUtils(new AliAnalysisUtils), fTriggerAnalysis(new AliTriggerAnalysis), fListHist(0),
    fHistTotalChargePerEventADA(0),
    fHistTotalChargePerEventADC(0),
...
void AliAnalysisTaskADPilot::UserCreateOutputObjects()
{
 ...
 const Int_t kNRawTimeBins = 3062;
  ...
  fHistADMOn = CreateHist1D("fHistADMOn","Total trigger charge in AD per event",kNChargeSideBins,kChargeSideMin,kChargeSideMax,"ADC counts","Entries");
  fListHist->Add(fHistADMOn);
  
  fHistADMOff = CreateHist1D("fHistADMOff","Total integrated charge in AD per event",kNChargeSideBins,kChargeSideMin,kChargeSideMax,"ADC counts","Entries");
  fListHist->Add(fHistADMOff);
...
void AliAnalysisTaskADPilot::UserExec(Option_t *) 
{
  ...
  for(Int_t i=0; i<16; i++){ 
  	totTrigChargeAD += esdAD->GetAdcTrigger(i);
  ...
  Bool_t acceptDefault = isADA & isADC;
  ...
  if(acceptDefault){
  	fHistADMOn->Fill(totTrigChargeAD);
	fHistADMOff->Fill(totChargeADA+totChargeADC);
  }
