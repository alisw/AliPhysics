...
#include "AlianalysisTaskDptPID.h"
...
ClassImp(AlianalysisTaskDptPID)

AlianalysisTaskDptPID::AlianalysisTaskDptPID()
: AliAnalysisTaskSE(),
fAODEvent(0), 
...
AlianalysisTaskDptPID::AlianalysisTaskDptPID(const TString & name)
...
void AlianalysisTaskDptPID::UserCreateOutputObjects()
{
  OpenFile(0);
  ...
  if (_useWeights)
    ...
    if (_weight_1)
      ...
      {
      AliError("AlianalysisTaskDptPID:: _weight_1 is a null pointer.");
      return;
      }
    if (!_sameFilter) 
      {
  ...
  //cout<< "AlianalysisTaskDptPID::CreateOutputObjects() DONE " << endl;
  
}

void  AlianalysisTaskDptPID::createHistograms()
{
  AliInfo(" AlianalysisTaskDptPID::createHistoHistograms() Creating Event Histos");
  TString name;
  ...
  AliInfo(" AlianalysisTaskDptPID::createHistoHistograms() All Done"); 
}
//-----------------------//

void  AlianalysisTaskDptPID::finalizeHistograms()
{
  AliInfo("AlianalysisTaskDptPID::finalizeHistograms() starting");
  ...
  AliInfo("AlianalysisTaskDptPID::finalizeHistograms()  Done ");
}
//--------------//


void  AlianalysisTaskDptPID::UserExec(Option_t */*option*/)
{
  ...
  if(fAODEvent)
	...
	{
	  AliError("AlianalysisTaskDptPID::Exec(Option_t *option) iVertex<0 || iVertex>=_nBins_vertexZ ");
	  return;
	}
      ...
      for (int iTrack=0; iTrack< _nTracks; iTrack++)
	{
	  ...
	  if (_requestedCharge_1 == charge && dedx >= _dedxMin && dedx < _dedxMax)
		...
		{
		  AliWarning("AlianalysisTaskDptPID::analyze() iPhi<0 || iPhi>=_nBins_phi_1");
		  return;
		}
		...
		{
		  AliWarning(Form("AlianalysisTaskDptPID::analyze(AliceEvent * event) Mismatched iEta: %d", iEta));
		  continue;
		}
	      iPt     = int((pt -_min_pt_1 )/_width_pt_1 ); 
	      if (iPt<0  || iPt >=_nBins_pt_1)
		{
		  AliWarning(Form("AlianalysisTaskDptPID::analyze(AliceEvent * event) Mismatched iPt: %d",iPt));
		  continue;
		}
		...
		{
		  AliWarning("AlianalysisTaskDptPID::analyze(AliceEvent * event) iZEtaPhiPt<0 || iZEtaPhiPt>=_nBins_zEtaPhiPt_1");
		  continue;
		}
	      ...
	      else
		{
		  ...
		  if (k1>=arraySize)
		    {
		      AliError(Form("AlianalysisTaskDptPID::analyze(AliceEvent * event) k1 >=arraySize; arraySize: %d",arraySize));
		      return;
		    }
		}
	    }
	  ...
	  if (!_sameFilter && _requestedCharge_2 == charge && dedx >=  _dedxMin && dedx < _dedxMax)
		...
		{
		  AliWarning("AlianalysisTaskDptPID::analyze(AliceEvent * event) iZEtaPhiPt<0 || iZEtaPhiPt>=_nBins_zEtaPhiPt_2");
		  continue;
		}
  ...
  AliInfo("AlianalysisTaskDptPID::UserExec()   -----------------Event Done ");
  PostData(0,_outputHistoList);
  
}

void   AlianalysisTaskDptPID::FinishTaskOutput()
{
  AliInfo("AlianalysisTaskDptPID::FinishTaskOutput() Starting.");
  Printf("= 0 ====================================================================");
  finalizeHistograms();
  AliInfo("= 1 ====================================================================");
  PostData(0,_outputHistoList);
  AliInfo("= 2 ====================================================================");
  AliInfo("AlianalysisTaskDptPID::FinishTaskOutput() Done.");
}

void   AlianalysisTaskDptPID::Terminate(Option_t* /*option*/)
{
  AliInfo("AlianalysisTaskDptPID::Terminate() Starting/Done.");
}


//Tools
//===================================================================================================
void  AlianalysisTaskDptPID::fillHistoWithArray(TH1 * h, float * array, int size)
{
  int i, i1;
...
void  AlianalysisTaskDptPID::fillHistoWithArray(TH2 * h, float * array, int size1, int size2)
{
  int i, i1;
  int j, j1;
  float v1, ev1, v2, ev2, sum, esum;
...
void  AlianalysisTaskDptPID::fillHistoWithArray(TH3 * h, float * array, int size1, int size2, int size3)
{
  int i, i1;
  int j, j1;
  int k, k1;
...
void  AlianalysisTaskDptPID::fillHistoWithArray(TH1 * h, double * array, int size)
{
  int i, i1;
...
void  AlianalysisTaskDptPID::fillHistoWithArray(TH2 * h, double * array, int size1, int size2)
{
  int i, i1;
  int j, j1;
  double v1, ev1, v2, ev2, sum, esum;
...
void  AlianalysisTaskDptPID::fillHistoWithArray(TH3 * h, double * array, int size1, int size2, int size3)
{
  int i, i1;
  int j, j1;
  int k, k1;
...
double *  AlianalysisTaskDptPID::getDoubleArray(int size, double v)
{
...
//________________________________________________________________________
float *  AlianalysisTaskDptPID::getFloatArray(int size, float v)
{
...
//________________________________________________________________________
TH1D * AlianalysisTaskDptPID::createHisto1D(const TString &  name, const TString &  title, 
...
//________________________________________________________________________
TH2D * AlianalysisTaskDptPID::createHisto2D(const TString &  name, const TString &  title, 
...
//// F /////
//________________________________________________________________________
TH1F * AlianalysisTaskDptPID::createHisto1F(const TString &  name, const TString &  title, 
                                                        int n, double xMin, double xMax, 
                                                        const TString &  xTitle, const TString &  yTitle)
{
  //CreateHisto new 1D historgram
...
//________________________________________________________________________
TH2F * AlianalysisTaskDptPID::createHisto2F(const TString &  name, const TString &  title, 
                                                        int nx, double xMin, double xMax, int ny, double yMin, double yMax, 
                                                        const TString &  xTitle, const TString &  yTitle, const TString &  zTitle)
{
  AliInfo(Form("createHisto 2D histo %s  nx: %d  xMin: %f xMax: %f  ny: %d   yMin: %f yMax: %f",name.Data(),nx,xMin,xMax,ny,yMin,yMax));
...
//________________________________________________________________________
TH3F * AlianalysisTaskDptPID::createHisto3F(const TString &  name, const TString &  title, 
...
//________________________________________________________________________
TProfile * AlianalysisTaskDptPID::createProfile(const TString & name, const TString & description, 
                                                            int nx,double xMin,double xMax,
                                                            const TString &  xTitle, const TString &  yTitle)
{
...
void   AlianalysisTaskDptPID::addToList(TH1 *h)
{
  if (_outputHistoList)
    {
