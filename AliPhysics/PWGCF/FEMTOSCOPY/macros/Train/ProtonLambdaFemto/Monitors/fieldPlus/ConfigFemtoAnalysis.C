...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  int multbins[11] = {0.00001, 900, 200, 300, 400, 500, 600, 700, 800, 900, 900};
  int runch[10] = {0, 0, 1, 0, 0, 0, 0, 0, 1, 1};
  ...
  for (int imult=0; imult<numOfMultBins; imult++) {
	...
	if (runch[ichg]) {
		...
		dtc3etaphitpc[aniter]->AddCutMonitor(cutPass1PIDetaphitpc[aniter], cutFail1PIDetaphitpc[aniter]);
	      ...
	      cutPass2PIDetaphitpc[aniter] = new AliFemtoCutMonitorParticlePID(Form("cutPass2%stpcM%i", chrgs[ichg], imult),2);
