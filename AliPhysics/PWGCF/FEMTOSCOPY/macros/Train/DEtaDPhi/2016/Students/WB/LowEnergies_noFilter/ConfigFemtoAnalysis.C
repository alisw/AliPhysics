...
//parameters:
  ...
  TH2D *fYPtWeights = new TH2D("Weigths", "Weights", 100, 0, 6, 100, 0, 4);
      ...
      if (runmults[imult]) // dla kazdego wlaczonego multiplicity
      {
         for (int ichg = 0; ichg < numOfChTypes; ichg++)
         {
            if (runch[ichg]) //dla kazdych wlaczonych typow czastek
            {
               ...
               dtc1etaphitpc[aniter]->SetEta(-100, 100);// psedopospiesznosc 0 - czastki wyprodukowane prostopadle do osi detektora
               dtc2etaphitpc[aniter]->SetEta(-100, 100);// limit 0.9; powyzej czatka nie zostaje zarejestrowana
               dtc3etaphitpc[aniter]->SetEta(-100, 100);
               

				if (ichg == 0 ||ichg == 1 ||ichg == 2)//protons 0-2
				{
					dtc1etaphitpc[aniter]->SetPt(0,100); // w zaleznosci od czastki rozne pedy
					dtc2etaphitpc[aniter]->SetPt(0,100);
					
					dtc1etaphitpc[aniter]->SetRapidity(-100, 100);
					dtc2etaphitpc[aniter]->SetRapidity(-100, 100);
					...
					if(ichg==0 || ichg==1){ 
						cutPass3YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPtWithWeights(Form("cutPass%stpcM%i_Weights", chrgs[ichg], imult),ProtonMass, fYPtWeights, 0);
						cutFail3YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPtWithWeights(Form("cutFail%stpcM%i_Weights", chrgs[ichg], imult),ProtonMass, fYPtWeights, 0);
				...
				{
					dtc1etaphitpc[aniter]->SetPt(0,100);
					dtc2etaphitpc[aniter]->SetPt(0,100);
					...
					if(ichg==3 || ichg==4){
						cutPass3YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPtWithWeights(Form("cutPass%stpcM%i_Weights", chrgs[ichg], imult),KaonMass, fYPtWeights, 0);
						cutFail3YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPtWithWeights(Form("cutFail%stpcM%i_Weights", chrgs[ichg], imult),KaonMass, fYPtWeights, 0);
						dtc1etaphitpc[aniter]->AddCutMonitor(cutPass3YPtetaphitpc[aniter], cutFail3YPtetaphitpc[aniter]);
				...
				if (ichg == 6 ||ichg == 7 ||ichg == 8)//pions 6-8
				{
					...
					if(ichg==6 || ichg==7){
						cutPass3YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPtWithWeights(Form("cutPass%stpcM%i_Weights", chrgs[ichg], imult),PionMass, fYPtWeights, 0);
						cutFail3YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPtWithWeights(Form("cutFail%stpcM%i_Weights", chrgs[ichg], imult),PionMass, fYPtWeights, 0);
						dtc1etaphitpc[aniter]->AddCutMonitor(cutPass3YPtetaphitpc[aniter], cutFail3YPtetaphitpc[aniter]);
					}
				...
				if (ichg == 9 || ichg == 10 || ichg == 11){ //lambdy 9-11
					...
					if(ichg==9 || ichg==10){
						cutPass3YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPtWithWeights(Form("cutPass%stpcM%i_Weights", chrgs[ichg], imult),LambdaMass, fYPtWeights, 0);
						cutFail3YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPtWithWeights(Form("cutFail%stpcM%i_Weights", chrgs[ichg], imult),LambdaMass, fYPtWeights, 0);
						dtc1etaphitpc[aniter]->AddCutMonitor(cutPass3YPtetaphitpc[aniter], cutFail3YPtetaphitpc[aniter]);
                ...
                if (ichg == 12)//all
                {
					dtc3etaphitpc[aniter]->SetPt(0,100);//min 0.12 TPC jeszcze wyciaga
					dtc3etaphitpc[aniter]->SetRapidity(-100,100);
                }
