...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  int multbins[11] = {0.01, 138, 216, 318, 1000, 950, 500, 600, 700, 800, 900};
  ...
  //Reader->SetCentralityPreSelection(0.01, 990);

  AliFemtoEventReaderKinematicsChain* Reader=new AliFemtoEventReaderKinematicsChain();
  Reader->SetUseMultiplicity(AliFemtoEventReaderKinematicsChain::kVZERO);
 ...
 // Reader->SetReadTrackType(AliFemtoEventReaderESDChain::kGlobal);
  ...
  AliFemtoMCTrackCut             *dtc1etaphitpc[320];
    ...
    if (runmults[imult]) {
      for (int ichg=0; ichg<2; ichg++) {
        if (runch[ichg]) {
         ...
         /* cutPass1YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPt(Form("cutPass1%stpcM%i", chrgs[ichg]\
                                                                                , imult),PionMass);
          cutFail1YPtetaphitpc[aniter] = new AliFemtoCutMonitorParticleYPt(Form("cutFail1%stpcM%i", chrgs[ichg]\
                                                                                , imult),PionMass);
          dtc1etaphitpc[aniter]->AddCutMonitor(cutPass1YPtetaphitpc[aniter], cutFail1YPtetaphitpc[aniter]);
          */
