...
#include "AliAnalysisTaskKPFemto.h"
...
AliAnalysisTaskKPFemto::~AliAnalysisTaskKPFemto() {
  //
  // Destructor
  //
  if (fOutputContainer){
...
void AliAnalysisTaskKPFemto::UserCreateOutputObjects() {
  ...
  if(fkDoSphericity == kFALSE){
    fEventColl = new AliAnalysisKPEventCollection **[fzVertexBins]; 
  
    for (unsigned short i=0; i<fzVertexBins; i++) {
      fEventColl[i] = new AliAnalysisKPEventCollection *[fnCentBins];
      for (unsigned short j=0; j<fnCentBins; j++) {
	fEventColl[i][j] = new AliAnalysisKPEventCollection(fnEventsToMix+1, fMaxFirstMult, fMaxSecondMult);
      }
    }
  }

  else{
    fEventCollwSp = new AliAnalysisKPEventCollection ***[fzVertexBins]; 
  
    for (unsigned short i=0; i<fzVertexBins; i++) {
      fEventCollwSp[i] = new AliAnalysisKPEventCollection **[fnCentBins];
      for (unsigned short j=0; j<fnCentBins; j++) {
	fEventCollwSp[i][j] = new AliAnalysisKPEventCollection*[fnCentBins];
	for(UChar_t k=0;k<fnSphericityBins;k++){
	  fEventCollwSp[i][j][k] = new AliAnalysisKPEventCollection(fnEventsToMix+1, fMaxFirstMult, fMaxSecondMult);
	}
      }
    }
  }
...
void AliAnalysisTaskKPFemto::UserExec(Option_t *) {
  ...
  */
  ...
  for (Int_t ip = 0; ip < ntracks; ip++){
    ...
    rapidityFirst  = 0.5*TMath::Log( (track->E(fPDGfirst) + track->Pz()) / (track->E(fPDGfirst) - track->Pz() +1.e-13) );
    ...
    /* //-->Questo e il VETO applicato al 2015
    // //New 30/03 --> Add pion veto + Electron veto + cut pt tra 300-400 MeV  VETI
    if(!IsElectron(pullsTPC[0],pullsTPC[1],pullsTPC[2],pullsTPC[3]))continue;
    if((fFirstpart == 3 && fSecondpart == 4) || ((fFirstpart == 4 && fSecondpart == 3))){
    if (IsPionNSigma(globaltrack->P(),  pullsTPC[1],  pullsTOF[1]))continue;
    }
    */

    

    // if(TMath::Abs(fPIDResponse->NumberOfSigmasTPC(globaltrack,AliPID::kElectron))<3 ){
    //   // if(globaltrack->GetTPCmomentum() < 0.45 && !isTOFPIDok)
    //   // 	continue;
    //   // else if(isTOFPIDok && (TMath::Abs(fPIDResponse->NumberOfSigmasTOF(globaltrack,AliPID::kElectron))<3))
    //   continue;
    // }
	...
	if(fReadMCTruth == kTRUE){
	  ...
	  //   fEvt->fReconstructedFirst[fCount].fPt           = tparticle->Pt();
	  //   fEvt->fReconstructedFirst[fCount].fEta          = tparticle->Eta();
	  //   fEvt->fReconstructedFirst[fCount].fPhi          = tparticle->Phi();
	}
	...
	fEvt->fReconstructedFirst[fCount].nSigmaFirstTPC[0]  = track->Px();
	fEvt->fReconstructedFirst[fCount].nSigmaFirstTPC[1]  = track->Py();
	
	// for (int pIndex = 0; pIndex <5; pIndex++) {
	//   fEvt->fReconstructedFirst[fCount].nSigmaFirstTPC[pIndex]  = pullsTPC[pIndex];
	//   fEvt->fReconstructedFirst[fCount].nSigmaFirstTOF[pIndex]  = pullsTOF[pIndex];
	// }

	
	// for (int pIndex = 0; pIndex <3; pIndex++) {
	//   fEvt->fReconstructedFirst[fCount].fMomentumTruth[pIndex]  = pMomentumTruth[pIndex];
	// }
	...
	fHistSharedFrTPCclFirst->Fill(sharedFractionTPCcl);
      ...
      else if (track->Pt()>= fMomemtumLimitForTOFPIDfirst && track->Pt() < fMaxPtForPrim && isTOFPIDok) {
	
	//	tTOF -= expectedTimes[(AliPID::EParticleType)fFirstpart];  
	...
	fHistFirstTOFmisvspt->Fill(track->Pt(),probMis);
	fHistFirstTOFmisvsp->Fill(track->P(),probMis);
	fHistFirstTOFnsigmavspt->Fill(track->Pt(),nsigmaTOFf);
	fHistFirstTOFnsigmavsp->Fill(track->P(),nsigmaTPCf);
	fHistFirstTOFsignalvsp->Fill(track->P(),tTOF - expectedTimes[(AliPID::EParticleType)fFirstpart]);  
	fHistFirstTOFsignalvspt->Fill(track->Pt(),tTOF - expectedTimes[(AliPID::EParticleType)fFirstpart]);
	
	fHistFirstTOFTPCsignalvspt->Fill(track->Pt(),TMath::Sqrt(nsigmaTOFf*nsigmaTOFf+nsigmaTPCf*nsigmaTPCf));
	...
	if (TMath::Abs(nsigmaTOFf)< fnSigmaTPCTOFPIDfirstParticle && tTOF > 0. /*&& length>350*/) {
	  ...
	  // for (int pIndex = 0; pIndex <3; pIndex++) {
	  //   fEvt->fReconstructedFirst[fCount].fMomentumTruth[pIndex]  = ntumTrutl[pIndex];
	  // }
	  ...
	  //	  cout<<fCount<<" This proton in the stack with pt TOF "<<fEvt->fReconstructedFirst[fCount].isaP<<endl;
	  
	  fCount++;
	  //---------------------- End Save first particle
	  
	  if (track->TestBit(AliAODTrack::kIsDCA)) {fHistIPtoPVxyzTPCFirst->Fill(dz[0],dz[1]); } else { /*cout<<"Bit is not DCA!!!!!!!!!!!"<<endl;*/ }
	  fHistIPtoPVxyzGlobalFirst->Fill(dzg[0],dzg[1]);
	  ...
	  //Plot AFTER the PID cuts
	  fHistFirstTOFTPCsignalvsptAfter->Fill(track->Pt(),TMath::Sqrt(nsigmaTOFf*nsigmaTOFf+nsigmaTPCf*nsigmaTPCf));
	  fHistyptFirst->Fill(track->Pt(),rapidityFirst,lcentrality);
	  fHistphietaFirst->Fill(track->Phi(),track->Eta());
...
void AliAnalysisTaskKPFemto::DoPairsh1h2 ( const Float_t lcentrality, int fieldsign, const Double_t fSphericityvalue) {
  ...
  for (int i=0; i<fEvt->fNumberCandidateFirst; i++) {
	...
	*/
	...
	else if(isMCvector && ((fFirstpart == 3 && fSecondpart == 4) || (fFirstpart == 4 && fSecondpart == 3))){
	    
	  if(mcGrandMotherLabelP1 == mcGrandMotherLabelP2){sameGrandMother = kTRUE;}
...
double AliAnalysisTaskKPFemto::CalculateKstar(double momentum1[3], double momentum2[3], double mass1, double mass2) { // Jai S
...
//-----------------------------------------------------------------------------------------------
double AliAnalysisTaskKPFemto::CalculateMass(double momentum1[3], double momentum2[3], double mass1, double mass2) { // Jai S
...
//-----------------------------------------------------------------------------------------------
double AliAnalysisTaskKPFemto::CalculateDphiSatR12m(Short_t chg1, Short_t chg2, Int_t magSign, Double_t ptv1, Double_t ptv2, Double_t phi1, Double_t phi2) { // AliFemto framework AliFemtoUser/AliFemtoPairCutRadialDistance.cxx + Dhevan not consistent?
  /*
  double rad = 1.2;
...
//-----------------------------------------------------------------------------------------------
double AliAnalysisTaskKPFemto::CalculateDPhiStar(Short_t chg1, Short_t chg2, Int_t magSign, Double_t ptv1, Double_t ptv2, Double_t phi1, Double_t phi2,Double_t rad) { //AliFemtoUser/AliFemtoPairCutDetaDphi.h
...
Double_t AliAnalysisTaskKPFemto::CalculateDeltaEta( Double_t eta1, Double_t eta2 )  {   //AliFemtoUser/AliFemtoPairCutDetaDphi.h
  const double deta = eta2 - eta1;
  return deta;
}
//_______________________________________________________________
Double_t AliAnalysisTaskKPFemto::CalculateDeltaTheta( Double_t theta1, Double_t theta2 )  {  
  const double dtheta = theta2 - theta1;
  return dtheta;
}

//-----------------------------------------------------------------------------------------------
double AliAnalysisTaskKPFemto::CalculateDphiSatR12m(Double_t pos1SftR125[3], Double_t pos2SftR125[3]) { // Hans B
...
void AliAnalysisTaskKPFemto::SetSftPosR125(AliVTrack *track, const Float_t bfield, Double_t priVtx[3], Double_t posSftR125[3] ) {  // Hans B
...
//----------------------------------------------------------------------------------------------
Double_t AliAnalysisTaskKPFemto::ThetaS( Double_t posSftR125[3] ) const { // Hans B
...
//_______________________________________________________________
Double_t AliAnalysisTaskKPFemto::EtaS( Double_t posSftR125[3] ) const {  // Hans B
...
Double_t AliAnalysisTaskKPFemto::CalculateSphericityofEvent(AliAODEvent *aodEvent)
{ //from Oliver
...
bool AliAnalysisTaskKPFemto::IsElectron(float nsigmaTPCE, float nsigmaTPCPi,float nsigmaTPCK, float nsigmaTPCP)
{
...
bool AliAnalysisTaskKPFemto::IsPionNSigma(double mom, float nsigmaTPCPi, float nsigmaTOFPi)
{
...
//----------------------------------------------------------
bool AliAnalysisTaskKPFemto::IsKaonNSigma(float mom, float nsigmaTPCK, float nsigmaTOFK)
{
  if (fNsigmaTPCTOF) {
    if (mom > 0.5) {
...
bool AliAnalysisTaskKPFemto::IsProtonNSigma(float mom, float nsigmaTPCP, float nsigmaTOFP)
{
  if (fNsigmaTPCTOF) {
    if (mom > 0.5) {
//        if (TMath::Hypot( nsigmaTOFP, nsigmaTPCP )/TMath::Sqrt(2) < 3.0)
...
//-----------------------------------------------------------------------------------------------
void AliAnalysisTaskKPFemto::Terminate(const Option_t *) {
