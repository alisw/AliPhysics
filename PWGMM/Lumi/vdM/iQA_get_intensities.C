...
void get_single_intensities(Int_t opt, Int_t bc, Bool_t save)
...
{ 
  ...
  frame1->SetTitle(Form("beam1 (red), beam2 (blue); timerel; beam intensity for bc=%d and Fill=%d",bc,g_vdm_Fill));
  if (g_vdm_Fill==4937)
      frame1->GetYaxis()->SetRangeUser(70*1e9,80*1e9); //2016
  if (g_vdm_Fill==6012)
      frame1->GetYaxis()->SetRangeUser(80*1e9,95*1e9); //2017
  gr1->Draw("p,same");
  gr2->Draw("p,same");
  ...
  if (save) {
      TString plotName = "";
      if (opt == 0) plotName = Form("../Fill-%d/Plots/FBCT_beamIntensity_bc%d.pdf",g_vdm_Fill,bc);
      if (opt == 1) plotName = Form("../Fill-%d/Plots/BPTX_beamIntensity_bc%d.pdf",g_vdm_Fill,bc);
...
void iQA_get_intensities(Int_t Fill, Int_t opt, Int_t bc, Bool_t save = kFALSE)
// opt = 0 => fBCT; opt = 1 => BPTX

{
  ...
  get_single_intensities(opt, bc, save);
