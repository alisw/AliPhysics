...
// opt = 0 => fBCT; opt = 1 => BPTX
{
	...
	//Int_t idx2 = ((Int_t)(BucketC[bc]/10.0)); //kimc: isn't index should be provided instead of bc?
	Int_t idx1 = -1;
	Int_t idx2 = -1;
	for (int a=0; a<nIBC; a++)
	{
		//cout <<a <<" " <<bunches[a] <<" " <<BucketA[a] <<" " <<BucketC[a] <<endl;
		if (bunches[a] == bc)
		{
			idx1 = (Int_t)(BucketA[a]/10.0);
			idx2 = (Int_t)(BucketC[a]/10.0);
			break;
		}
	}

	// storage space
	const Int_t ngr =  g_vdm_Tree->GetEntries();
	...
	Double_t imax =  50.e9; //0;
	Double_t imin = 100.e9; //1e20;
	...
	gr1->SetMarkerStyle(24); gr1->SetMarkerColor(kRed);  gr1->SetMarkerSize(0.5);
	gr2->SetMarkerStyle(24); gr2->SetMarkerColor(kBlue); gr2->SetMarkerSize(0.5);
	...
	TH1F* frame1 = gPad->DrawFrame(-1, imin*0.95, ngr+1, imax*1.05);
	...
	cx->Print(Form("c1c_QAintensity_Fill%i_opt%i_bc%i.png", g_vdm_Fill, opt, bc));
