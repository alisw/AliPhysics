...
void Create_one_intensity_corrected_rate_file(
		Int_t scan_type, Int_t scan, const char *rate_name, const char *intensity_corr_name, bool systChk)
// scan_type: 1 => x-scan; 2 => y-scan
{
	...
	if (scan_type == 1)
	{
		...
		sprintf(file_name_rate,"../Fill-%d/PileupCorrRate_%s_x_Scan_%d%s.root", g_vdm_Fill,rate_name,scan,sfx);
		sprintf(file_name_new_rate,"../Fill-%d/IntensityCorr%sRate_%s_x_Scan_%d%s.root",
				g_vdm_Fill,intensity_corr_name,rate_name,scan,sfx);
	}
	if (scan_type == 2) {
		sprintf(file_name_corr,"../Fill-%d/%sCorr_y_Scan_%d.root", g_vdm_Fill,intensity_corr_name,scan);
		sprintf(file_name_rate,"../Fill-%d/PileupCorrRate_%s_y_Scan_%d%s.root", g_vdm_Fill,rate_name,scan,sfx);
		sprintf(file_name_new_rate,"../Fill-%d/IntensityCorr%sRate_%s_y_Scan_%d%s.root",
				g_vdm_Fill,intensity_corr_name,rate_name,scan,sfx);
	}
...
void Create_intensity_corrected_rate_file(
		Int_t Fill, const char *rate_name, const char *intensity_corr_name, bool systChk = false
		)
{
	...
	{
		Create_one_intensity_corrected_rate_file(1, i, rate_name, intensity_corr_name, systChk); //x-scans
		Create_one_intensity_corrected_rate_file(2, i, rate_name, intensity_corr_name, systChk); //y-scans
	}
