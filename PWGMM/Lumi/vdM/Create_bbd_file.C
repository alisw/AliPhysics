...
void Init_parameters(Int_t Fill)
{
	...
	else if (Fill == 4937 || Fill == 6012 || Fill == 6864) //!
	{
...
void Create_BBD_separation_file(
		Int_t Fill,
		Int_t scan,
		...
		)
{
	...
	for(Int_t i=0;i<n_sepx;i++) bbd_file << "5) Separation sepx, sepy [mm,mm]:" << sepx[i] << ",0" << endl;
	bbd_file << "5) Separation sepx, sepy [mm,mm] :0,0" << endl;
	bbd_file << "5) Separation sepx, sepy [mm,mm] :0,0" << endl;
	for(Int_t i=0;i<n_sepy;i++) bbd_file << "5) Separation sepx, sepy [mm,mm]:0," << sepy[i]  << endl;
