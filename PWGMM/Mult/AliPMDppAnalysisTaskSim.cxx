...
#include "AliPMDppAnalysisTaskSim.h"
...
ClassImp(AliPMDppAnalysisTaskSim)

//.......................................................//

AliPMDppAnalysisTaskSim::AliPMDppAnalysisTaskSim(const char *name)
...
void AliPMDppAnalysisTaskSim::UserCreateOutputObjects() {
  ...
  fHistTotEvent = new TH1F("fHistTotEvent","fHistTotEvent",7,0,7);
  ...
  fHistTotEvent->GetXaxis()->SetBinLabel(7, "AfterGenPileUpCut");
...
//________________________________________________________________________
void AliPMDppAnalysisTaskSim::UserExec(Option_t *) {
  // Main loop
  ...
  if(vertex){
	...
	TList* lh = mcEvent->GetCocktailList();
        Int_t nh = lh->GetEntries();
        for (Int_t i = nh-1; i >= 0; i--) {
	  AliGenEventHeader* gh = (AliGenEventHeader*)lh->At(i);
	  genname = gh->GetName();
	}
        //cout << "........Generator name =..............." << genname.Data() << endl;
	if(fGenPileUp){
	  Bool_t isPileupInGeneratedEvent = kFALSE;
	  isPileupInGeneratedEvent = AliAnalysisUtils::IsPileupInGeneratedEvent(mcEvent,genname.Data());
	  if(isPileupInGeneratedEvent) return;
	}
	fHistTotEvent->Fill(6.5);
	for(Int_t mcTrack = 0; mcTrack < mcEvent->GetNumberOfTracks(); mcTrack++) {
	  if(AliAnalysisUtils::IsParticleFromOutOfBunchPileupCollision(mcTrack, mcEvent)) continue;
	  ...
	  if(ch!=0) fHistEtaChTrue->Fill(eta);
	  if(mpart == 22) {
...
//________________________________________________________________________
void AliPMDppAnalysisTaskSim::Terminate(Option_t *) {
