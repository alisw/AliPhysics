...
void  AliAnaPi0EbE::MakeInvMassInCalorimeterAndCTS() 
{
	...
	for(Int_t iphoton = 0; iphoton < GetInputAODBranch()->GetEntriesFast(); iphoton++){
		...
		//Play with the MC stack if available
		fInputAODGammaConv = (TClonesArray *) GetReader()->GetAOD()->FindListObject(fInputAODGammaConvName);
		if(!fInputAODGammaConv) 
			AliFatal(Form("MakeInvMassInCaloAndCTS: No input gamma conversions in AOD branch with name < %s >",fInputAODGammaConvName.Data()));	
