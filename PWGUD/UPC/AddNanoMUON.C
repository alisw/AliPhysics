AliAnalysisTaskNanoMUON* AddNanoMUON(Int_t opt = 0)
{
  ...
  if (!mgr) {
    ...
    // same for the output
    mgr->ConnectOutput(task,1,mgr->CreateContainer("fNanoAnaTree", TTree::Class(), AliAnalysisManager::kOutputContainer,fileName.Data()));
    mgr->ConnectOutput(task,2,mgr->CreateContainer("fOutputList", TList::Class(), AliAnalysisManager::kOutputContainer, fileName.Data()));
    mgr->ConnectOutput(task,3,mgr->CreateContainer("fNanoMCTree", TTree::Class(), AliAnalysisManager::kOutputContainer,fileName.Data()));
