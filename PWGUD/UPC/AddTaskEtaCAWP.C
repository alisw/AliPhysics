AliAnalysisTaskUpcEtaCAWP *AddTaskEtaCAWP(Bool_t runTree = kFALSE, Bool_t runHist = kTRUE, Bool_t runSyst = kFALSE, Int_t tracking = 0){

	//--- get the current analysis manager ---//
	AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
	if (!mgr) {
		Error("AddTask_UpcEtaCAWP", "No analysis manager found.");
		return 0;
	}
	...
	if (!mgr->GetInputEventHandler()) {
		Error("AddTask_UpcEtaCAWP", "This task requires an input event handler");
		return 0;
	}
