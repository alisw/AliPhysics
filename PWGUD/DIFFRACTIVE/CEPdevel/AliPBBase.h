...
public:
	enum{
		...
		// gap-condition bits used in AliAnalysisTaskPB::fGapRun
		kBitBaseLine = (1<<0),
		...
		kBitHitMapFMD = (1<<10),
		kBitVtxStudies = (1<<11),
		kBitEEStudy = (1<<12),          // Empty Event Study
		kBitPIDStudy = (1<<13),
		kBitMCProcess = (1<<14),
		kBitFMDsum = (1<<15),
		kBitSoftTracks = (1<<16),
		...
		kBitFulltree = (1<<28),
		kBitADStudies = (1<<29),
		kBitConfigurationVersion = (1<<30) // always set, last bit
	...
	static THnSparseD* GetThnMother(TString name = "PB_Mother");
