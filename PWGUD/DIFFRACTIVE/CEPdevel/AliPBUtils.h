...
public:
	...
	static Bool_t V0AND(const AliESDEvent *ESDEvent) {
		...
		// PB, 16.03.2016: return (GetV0(ESDEvent) == 6);
		// (1<<1) && (1<<2) -> >4
	...
	static Bool_t V0OR(const AliESDEvent *ESDEvent) {
		...
		// PB, 16.03.2016: return (GetV0(ESDEvent) > 0);
		// (1<<1) || (1<<2) -> >1
		return (GetV0(ESDEvent) > 1);
	}
	...
	static Int_t GetGapConfig(const AliESDEvent *ESDEvent,
      ...
      TH1 *mulADA, TH1 *mulADC, TH1 *chargeADA, TH1 *chargeADC, 
...
private:
	...
	static Int_t GetAD(const AliESDEvent *ESDEvent, TH1 *mulADA, TH1 *mulADC,
                      TH1 *chargeADA, TH1 *chargeADC);
