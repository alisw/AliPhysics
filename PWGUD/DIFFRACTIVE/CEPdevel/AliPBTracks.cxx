...
//------------------------------------------------------------------------------
AliPBTracks::AliPBTracks(const AliPBTracks& tracks)
...
//------------------------------------------------------------------------------
AliPBTracks& AliPBTracks::operator=(const AliPBTracks& tracks)
{
	//
	// assignment operator
	//
                                      ...
                                      Bool_t doSoft /* = kTRUE */)
{
  ...
  // is done, the results can be used via the getters
	...
	// count different types of tracks after application of cuts
  ApplyCuts();
...
//------------------------------------------------------------------------------
AliVTrack* AliPBTracks::GetTrack(UInt_t index) const
{
	//
...
//------------------------------------------------------------------------------
Double_t AliPBTracks::GetInvariantMass(Bool_t includeSoftTracks /*=kTRUE*/)
{
	//
...
//------------------------------------------------------------------------------
void AliPBTracks::CutTrack(AliESDEvent *ESDEvent, Int_t mode /* = 0 */)
{
	//
	//  CutTrack to be combined with the AOD function // TODO
  //  mode:
  //    0 : cuts for normal tracks (ITS + TPC)       --> fTracks, fNch
  //    1 : cuts for soft tracks (ITS only - kITSsa) --> fSoftTracks, fNsoft
  //    2 : cuts for ITSpureSA tracks used in order to get rid of them for
  //        noise studies                            --> fNITSpureSA
  //
  // selected tracks are contained in fTracks
...
//------------------------------------------------------------------------------
void AliPBTracks::CutTrack(AliAODEvent *AODEvent, Int_t mode /* = 0 */)
{
	//
	// CutTrack for AODs
	//
...
//------------------------------------------------------------------------------
void AliPBTracks::GetRemainingTracklets()
{
