...
// good-run list of LHC10b period : 33
...
// good-run list of LHC10c period : 31
...
// good-run list of LHC10d period : 55
...
// good-run list of LHC10e period : 113
...
// good-run list of LHC15f period : 64
...
void runCD(
    ...
    , const char *option = "LHC15f" // LHC10b or c or d or e or LHC15f
    ...
    )
{
    ...
    plugin->SetAliPhysicsVersion("vAN-20160112-1");
    ...
    plugin->SetJDLName(Form("%s.jdl",Form("%s%s",taskname,option)));
    ...
    AliAnalysisTaskCDPWA *preselectiontaskfor13tev;
    gROOT->LoadMacro("$ALICE_PHYSICS/PWGUD/DIFFRACTIVE/legotrain/AddTaskCDTree.C");
    AliAnalysisTaskCDTree *preselectiontaskfor7tev;

    if (foption.Contains("LHC15f")){
        preselectiontaskfor13tev = AddTaskCDPWA();
        mgr->AddTask(preselectiontaskfor13tev);
    } else {
        gROOT->LoadMacro("$ALICE_PHYSICS/OADB/macros/AddTaskPhysicsSelection.C");
        AliPhysicsSelectionTask *physSelTask = AddTaskPhysicsSelection(0);
        if(!physSelTask) { Printf("no physSelTask"); return; }
        preselectiontaskfor7tev = AddTaskCDTree();
        mgr->AddTask(preselectiontaskfor7tev);
    }
