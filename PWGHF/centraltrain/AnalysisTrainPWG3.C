...
//==================   TRAIN NAME   ============================================
TString     train_name         = "PWG3"; // local folder name
TString     train_tag          = "_Pb-Pb_";        // Train special tag appended to 
                                            // visible name. ("data", "sim", "pp", "highmult", ...)
               // Name in train page (DON'T CHANGE)
TString     visible_name       = Form("PWG3%s$2_$3", train_tag.Data()); //# FIXED #
               // Add train composition and other comments
TString     job_comment        = "PWG3 tasks on AODs (di)muon/vertexing/dielectrons";
...
TString     grid_workdir       = "/alice/cern.ch/user/a/alidaq/PWG3/PWG3_$2";
...
TString     alien_outdir       = "$1/PWG3OUT$2";
...
TString     mergeDirName       = "PWG3OUT$2";
...
void AnalysisTrainPWG3(const char *analysis_mode="local", 
...
// are taken from there but may be altered by CheckModuleFlags.
   ...
   if (iPWG3d2h)     printf("=  PWG3 D -> hadrons tasks                                      =\n");
...
// ********** PWG3 wagons ******************************************************
   ...
   // PWG3 electrons
   if (iPWG3hfe) {
      gROOT->LoadMacro("$ALICE_ROOT/PWG3/hfe/macros/AddTaskHFE.C");
   ...
   if (iPWG3d2h) {
      ...
      AddD2HTrain(readMC, 1,1,1,0,0,1,0,0,0,0);
   }   
...
// Set the run mode (can be "full", "test", "offline", "submit" or "terminate")
   ...
   if (iPWG3d2h) plugin->SetFriendChainName("AliAOD.VertexingHF.root");
...
// Optionally set a name for the generated validation script
   plugin->SetValidationScript("PWG3validation.sh");
