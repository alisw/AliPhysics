...
ClassImp(AliAnalysisHFjetTagHFE)
    ...
    : AliAnalysisTaskEmcalJet("AliAnalysisHFjetTagHFE", kTRUE),
      ...
      fOADBFileName("PWGHF/vertexingHF/QnCalibrations/calibV0TPCRun2Vtx10P118qPass3.root"),
...
void AliAnalysisHFjetTagHFE::UserCreateOutputObjects() {
    ...
    // for Event Plane.
    fEPcorV0AV0C = new TH2F("fEPcorV0AV0C", "EPcorV0AV0C", 320, -1.6, 1.6, 320, -1.6, 1.6);
    fOutput->Add(fEPcorV0AV0C);

    fEPcorV0ATPC = new TH2F("fEPcorV0ATPC", "EPcorV0ATPC", 320, -1.6, 1.6, 320, -1.6, 1.6);
    fOutput->Add(fEPcorV0ATPC);

    fEPcorV0CTPC = new TH2F("fEPcorV0CTPC", "EPcorV0CTPC", 320, -1.6, 1.6, 320, -1.6, 1.6);
    fOutput->Add(fEPcorV0CTPC);

    fsubV0AV0Ccos2 = new TH2F("fsubV0AV0Ccos2", "fsubV0AV0Ccos2 vs cetrality", 40, 0, 80, 200, -1, 1);
    fOutput->Add(fsubV0AV0Ccos2);

    fsubV0ATPCcos2 = new TH2F("fsubV0ATPCcos2", "fsubV0ATPCcos2 vs cetrality", 40, 0, 80, 200, -1, 1);
    fOutput->Add(fsubV0ATPCcos2);
    ...
    */
    ...
    fHistUE_true = new TH1F("fHistUE_true", "UE from EPOS in true jet; p_{T}(GeV/c)", 100, 0.0, 10.0);
    fOutput->Add(fHistUE_true);

    fHistUE_reco = new TH1F("fHistUEreco", "UE from EPOS in reco jet; p_{T}(GeV/c)", 100, 0.0, 10.0);
    fOutput->Add(fHistUE_reco);
    ...
    fOutput->Add(fHistphoPi0MC);

    fHistphoEtaMC = new TH1D("fHistphoEtaMC", "MC e^{photo} from eta without emb", 100, 0, 50);
    fOutput->Add(fHistphoEtaMC);

    Int_t nBinEop[3] = {301, 500, 200};
    Double_t mim_Eop[3] = {-0.5, 0.0, 0.0};
    Double_t max_Eop[3] = {300.5, 100.0, 4.0};

    fNtrklEopHFE = new THnSparseD("NtrklcorrEopHFE", "Ntracklet vs Eop; N^{corr}_{trkl};p_{T};E/p", 3, nBinEop, mim_Eop, max_Eop);
    fNtrklEopHFE->Sumw2();
    fOutput->Add(fNtrklEopHFE);
    ...
    for (int iDet = 0; iDet < 6; iDet++) {
        ...
        fEvPlaneVsCentr[iDet] = new TH2F(Form("fEvPlaneVsCentr_%s vs cent", detConfName[iDet].Data()), Form("hEvPlaneVsCentr_%s;centrality(%%);#psi_{%d}", detConfName[iDet].Data(), fHarmonic), ncentbins, fcentMim, fcentMax, 200, -TMath::Pi() / 2.0, TMath::Pi() / 2.0);

        fOutput->Add(fEvPlaneVsCentr[iDet]);
    }
    ...
    for (int iHist = 0; iHist < 3; iHist++) {
        ...
        fOutput->Add(fEPResolVsCentr[iHist]);
    }
...
Bool_t AliAnalysisHFjetTagHFE::Run() {
    ...
    if (TMath::Abs(Zvertex) < 10.0 && fcentID)  // event cuts
    {
        ...
        // get the unnormalised Qn-vectors --> normalization can be done the task
        ...
        // Event Plane vs Centrality(lPercentile)
        fEvPlaneVsCentr[0]->Fill(evCentr, PsinFullTPC);
        fEvPlaneVsCentr[1]->Fill(evCentr, PsinPosTPC);
        fEvPlaneVsCentr[2]->Fill(evCentr, PsinNegTPC);
        fEvPlaneVsCentr[3]->Fill(evCentr, PsinFullV0);
        fEvPlaneVsCentr[4]->Fill(evCentr, PsinV0A);
        fEvPlaneVsCentr[5]->Fill(evCentr, PsinV0C);

        // Correration of Event Plane.
        fEPcorV0AV0C->Fill(PsinV0A, PsinV0C);
        ...
        subV0AV0C = PsinV0A - PsinV0C;
        fsubV0AV0Ccos2->Fill(evCentr, cos(2 * subV0AV0C));
        subV0ATPC = PsinV0A - PsinFullTPC;
        fsubV0ATPCcos2->Fill(evCentr, cos(2 * subV0ATPC));
                    ...
                    if (fabs(jetEta) < fJetEtaCut && jet->Pt() > 1.0)  // pt cut for CMS bg cal.
                    {
                        ...
                        if (iTagHFjet && isElectron)  // TPC+EMCal
                        {
                            ...
                            if (!fFlagULS) {
                                ...
                                HFjetRap2[0] = corrPt;
                                HFjetRap2[1] = Eta_eJet;
                                HFjetRap2[2] = eta;
                                HFjetRap2[3] = Eta_eJet - eta;
                                HFjetRap2[4] = dphi_jet_e;
                            ...
                            if (iMCHF && pTeJetTrue < pthard) {
                                ...
                                double HFjetVals2[7];
                                HFjetVals2[0] = track->Pt();
                                HFjetVals2[1] = 0.0;
                                HFjetVals2[2] = reducedJetPt0;
                                HFjetVals2[3] = pTeJet;
                                HFjetVals2[4] = pTeJetTrue;
                                HFjetVals2[5] = 0.0;
                                HFjetVals2[6] = 0.0;
                                ...
                                double HFjetVals3[7];
                                HFjetVals3[0] = track->Pt();
                                HFjetVals3[1] = 0.0;
                                HFjetVals3[2] = reducedJetPt1;
                                HFjetVals3[3] = pTeJet;
                                HFjetVals3[4] = pTeJetTrue;
                                HFjetVals3[5] = 0.0;
                                HFjetVals3[6] = 0.0;
                                ...
                                HFjetVals4[1] = 0.0;
                                HFjetVals4[2] = corrPt_UE;
                                HFjetVals4[3] = pTeJet;
                                HFjetVals4[4] = pTeJetTrue;
                                HFjetVals4[5] = 0.0;
                                HFjetVals4[6] = 0.0;
                                ...
                                HFjetRap1[0] = pTeJetTrue;
                                HFjetRap1[1] = jetEta;
                                HFjetRap1[2] = eta;
                                HFjetRap1[3] = jetEta - eta;
                                HFjetRap1[4] = dphi_jet_e;
                    ...
                    while (jetPart) {
                        ...
                        if (fabs(jetEta) < fJetEtaCut && jetPart->Pt() < pthard) {
                            ...
                            if (iTagHFjet) {
                                ...
                                HFjetRap0[0] = jetPart->Pt();
                                HFjetRap0[1] = jetEta;
                                HFjetRap0[2] = etaMC;
                                HFjetRap0[3] = jetEta - etaMC;
                                HFjetRap0[4] = dphi_jet_e;
