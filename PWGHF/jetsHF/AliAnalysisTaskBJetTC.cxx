...
Bool_t AliAnalysisTaskBJetTC::Run()
{
    ...
    if (fApplyV0RejectionAll)
    {
        ...
        for (Int_t i = 0; i < fMCArray->GetEntriesFast(); i++)
        {
            ...
            if (id == 310)
            {
                fh1dKshortPtMC->Fill(pAOD->Pt(), fPythiaEventWeight);
            }
            if (id == 3122)
            {
                fh1dLamdaPtMC->Fill(pAOD->Pt(), fPythiaEventWeight);
            }
            if (id == -3122)
            {
                fh1dAnLamdaPtMC->Fill(pAOD->Pt(), fPythiaEventWeight);
            }
            ...
            while ((jetMC = fJetContainerMC->GetNextAcceptJet()))
            {
                ...
                if (IsParticleInCone(pAOD, jetMC, 0.4)) // If good jet in event, find out whether V0 is in that jet
                {
                    if (id == 310)
                    {
                        fh2dKshortPtVsJetPtMC->Fill(pAOD->Pt(), fJetPt, fPythiaEventWeight);
                    }
                    if (id == 3122)
                    {
                        fh2dLamdaPtVsJetPtMC->Fill(pAOD->Pt(), fJetPt, fPythiaEventWeight);
                    }
                    if (id == -3122)
                    {
                        fh2dAnLamdaPtVsJetPtMC->Fill(pAOD->Pt(), fJetPt, fPythiaEventWeight);
                    }
                    break;
                }
            }
    ...
    if (fEnableV0GammaRejection)
    {
        ...
        for (Int_t i = 0; i < fReaderGammas->GetEntriesFast(); i++)
        {
            ...
            TVector3 vV0;
            if (PhotonCandidate)
                vV0.SetXYZ(PhotonCandidate->GetPx(), PhotonCandidate->GetPy(), PhotonCandidate->GetPz());

            if (IsV0InJet(vV0, 5.))
            {
                fh1dPhotonPt->Fill(PhotonCandidate->GetPhotonPt(), fPythiaEventWeight);
            }
        }
    ...
    if (fCalcDCATruth)
    {
        for (int itrack = 0; itrack < nTracksInEvent; ++itrack)
        {
            ...
            if (!CalculateTrackImpactParameter(trackAOD, dca, cov))
                continue;
            ...
            if (fIsPythia)
            {
                ...
                if (!CalculateTrackImpactParameterTruth(trackAOD, dcaMC, covMC))
                    continue;
    ...
    if (JetContName.Contains("PicoTracks"))
        fUsePicoTracks = kTRUE;
    else
        fUsePicoTracks = kFALSE;
    ...
    if (fIsPythia)
    {
        ...
        if (!MatchJetsGeometricDefault())
            cout << "Error running jet matching!" << endl;
        fJetContainerMC->ResetCurrentID();
    ...
    if (fDoPtRelAnalysis && fResolutionFunction[0] && fDoSelectionPtRel)
    {
        ...
        while ((selJet = fJetContainerData->GetNextAcceptJet()))
        {
            ...
            {
                if (fUsePicoTracks)
                    trackAOD = (AliAODTrack *)((AliPicoTrack *)selJet->Track(itrack))->GetTrack();
                else
                    trackAOD = (AliAODTrack *)((fJetContainerData->GetParticleContainer())->GetParticle(selJet->TrackAt(itrack)));
    ...
    while ((jetrec = fJetContainerData->GetNextJet()))
    {
        ...
        {
            if (fJetFlavor == 0)
                fh1dJetRecPtUnidentified->Fill(fJetPt, fPythiaEventWeight);
            else if (fJetFlavor == 1)
                fh1dJetRecPtudsg->Fill(fJetPt, fPythiaEventWeight);
            else if (fJetFlavor == 2)
                fh1dJetRecPtc->Fill(fJetPt, fPythiaEventWeight);
            else if (fJetFlavor == 3)
                fh1dJetRecPtb->Fill(fJetPt, fPythiaEventWeight);
        }
        ...
        if (fIsPythia)
        {
            if (jetrec->MatchedJet())
            {
                ...
                if (fJetFlavor == 0)
                {
                    fh1dJetRecPtUnidentifiedAccepted->Fill(fJetPt, fPythiaEventWeight);
                }
                else if (fJetFlavor == 1)
                {
                    fh1dJetRecPtudsgAccepted->Fill(fJetPt, fPythiaEventWeight);
                    fh2dJetGenPtVsJetRecPtudsg->Fill(fJetPt, genpt, fPythiaEventWeight);
                }
                else if (fJetFlavor == 2)
                {
                    fh1dJetRecPtcAccepted->Fill(fJetPt, fPythiaEventWeight);
                    fh2dJetGenPtVsJetRecPtc->Fill(fJetPt, genpt, fPythiaEventWeight);
                }
                else if (fJetFlavor == 3)
                {
                    fh1dJetRecPtbAccepted->Fill(fJetPt, fPythiaEventWeight);
                    fh2dJetGenPtVsJetRecPtb->Fill(fJetPt, genpt, fPythiaEventWeight);
                }
            }
        }
        ...
        for (Int_t itrack = 0; itrack < ntracks; ++itrack)
        {
            ...
            if (fUsePicoTracks)
                trackAOD = (AliAODTrack *)((AliPicoTrack *)jetrec->Track(itrack))->GetTrack();
            else
                trackAOD = (AliAODTrack *)((fJetContainerData->GetParticleContainer())->GetParticle(jetrec->TrackAt(itrack)));
            ...
            if (fDoPtRelAnalysis && PtRelSample)
            {
                ...
                if (IsElectronHF(trackAOD))
                {
                    ...
                    if (hasIP)
                        fhistLepIPVsJetPt->Fill(fJetPt, GetValImpactParameter(kXY, LepIP, COV), fPythiaEventWeight);
                    EleID[ElecNum] = itrack;
                    ElePtRel[ElecNum] = PtRel;
                    if (hasIP)
                        EleIP[ElecNum] = GetValImpactParameter(kXY, LepIP, COV);
                    else
                        EleIP[ElecNum] = -999;
                    if (fIsPythia)
                    {
                        if (fJetFlavor == 0)
                        {
                            fhistPtRelVsJetPtUnidentified->Fill(fJetPt, PtRel, fPythiaEventWeight);
                        }
                        else if (fJetFlavor == 1)
                        {
                            fhistPtRelVsJetPtudsg->Fill(fJetPt, PtRel, fPythiaEventWeight);
                        }
                        else if (fJetFlavor == 2)
                        {
                            fhistPtRelVsJetPtc->Fill(fJetPt, PtRel, fPythiaEventWeight);
                        }
                        else if (fJetFlavor == 3)
                        {
                            fhistPtRelVsJetPtb->Fill(fJetPt, PtRel, fPythiaEventWeight);
                        }
                        if (hasIP)
                        {
                            if (fJetFlavor == 0)
                            {
                                fhistLepIPVsJetPtUnidentified->Fill(fJetPt, GetValImpactParameter(kXY, LepIP, COV), fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 1)
                            {
                                fhistLepIPVsJetPtudsg->Fill(fJetPt, GetValImpactParameter(kXY, LepIP, COV), fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 2)
                            {
                                fhistLepIPVsJetPtc->Fill(fJetPt, GetValImpactParameter(kXY, LepIP, COV), fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 3)
                            {
                                fhistLepIPVsJetPtb->Fill(fJetPt, GetValImpactParameter(kXY, LepIP, COV), fPythiaEventWeight);
                            }
                        }
                    }
                    ElecNum++;
            ...
            if (!IsTrackAcceptedBJetCuts(trackAOD, fJetFlavor))
                continue;
            ...
            if (abs(dca[0]) > fTCMaxIPxy)
                continue;
            FillCandidateJet(10, fJetFlavor);
            if (abs(dca[1]) > fTCMaxIPz)
                continue;
            FillCandidateJet(11, fJetFlavor);
            if (lineardecaylenth > fTCMaxDecayLength)
                continue;
            FillCandidateJet(12, fJetFlavor);
            if (dcatrackjet > fTCMaxDCATrackJet)
                continue;
            FillCandidateJet(13, fJetFlavor);
            ...
            if (cursImParXY > fThresholdIP)
                FillCandidateJet(14, fJetFlavor);
        ...
        if (fDoTrackCountingAnalysis)
        {
            ...
            if (DefaultDiscriminator.size() > 0)
            {
                ...
                if (fDoPtRelAnalysis && PtRelSample && DefaultDiscriminator.at(0) > fThresholdIP)
                {
                    for (int e = 0; e < ntracks; e++)
                    {
                        ...
                        if (EleIP[e] != -999)
                            fhistLepIPVsJetPtTaggedFirst->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                        if (fIsPythia)
                        {
                            if (fJetFlavor == 0)
                            {
                                ...
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedUnidentifiedFirst->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 1)
                            {
                                fhistPtRelVsJetPtTaggedudsgFirst->Fill(fJetPt, ElePtRel[e], fPythiaEventWeight);
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedudsgFirst->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 2)
                            {
                                fhistPtRelVsJetPtTaggedcFirst->Fill(fJetPt, ElePtRel[e], fPythiaEventWeight);
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedcFirst->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 3)
                            {
                                fhistPtRelVsJetPtTaggedbFirst->Fill(fJetPt, ElePtRel[e], fPythiaEventWeight);
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedbFirst->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                        }
                    }
                }
            ...
            if (DefaultDiscriminator.size() > 1)
            {
                ...
                {
                    TaggedSecond = kTRUE;
                    if (!firstJetFound && fEmbeddPerpendicular)
                    {
                        firstJetFound = true;
                ...
                if (fDoPtRelAnalysis && PtRelSample && DefaultDiscriminator.at(1) >= fThresholdIP)
                {
                    for (int e = 0; e < ntracks; e++)
                    {
                        ...
                        if (EleIP[e] != -999)
                            fhistLepIPVsJetPtTaggedSecond->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                        if (fIsPythia)
                        {
                            if (fJetFlavor == 0)
                            {
                                ...
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedUnidentifiedSecond->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 1)
                            {
                                fhistPtRelVsJetPtTaggedudsgSecond->Fill(fJetPt, ElePtRel[e], fPythiaEventWeight);
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedudsgSecond->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 2)
                            {
                                fhistPtRelVsJetPtTaggedcSecond->Fill(fJetPt, ElePtRel[e], fPythiaEventWeight);
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedcSecond->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 3)
                            {
            ...
            if (DefaultDiscriminator.size() > 2)
            {
                ...
                if (fDoPtRelAnalysis && PtRelSample && DefaultDiscriminator.at(2) >= fThresholdIP)
                {
                    for (int e = 0; e < ntracks; e++)
                    {
                        ...
                        if (EleIP[e] != -999)
                            fhistLepIPVsJetPtTaggedThird->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                        if (fIsPythia)
                        {
                            if (fJetFlavor == 0)
                            {
                                ...
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedUnidentifiedThird->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 1)
                            {
                                fhistPtRelVsJetPtTaggedudsgThird->Fill(fJetPt, ElePtRel[e], fPythiaEventWeight);
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedudsgThird->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 2)
                            {
                                fhistPtRelVsJetPtTaggedcThird->Fill(fJetPt, ElePtRel[e], fPythiaEventWeight);
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedcThird->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                            else if (fJetFlavor == 3)
                            {
                                fhistPtRelVsJetPtTaggedbThird->Fill(fJetPt, ElePtRel[e], fPythiaEventWeight);
                                if (EleIP[e] != -999)
                                    fhistLepIPVsJetPtTaggedbThird->Fill(fJetPt, EleIP[e], fPythiaEventWeight);
                            }
                        }
                    }
                }
            ...
            if (fIsPythia && fDoTaggedDRM)
            {
                ...
                if (jetrec->MatchedJet())
                {
                    ...
                    {
                        if (DefaultDiscriminator.at(0) >= fThresholdIP)
                            fh2dJetGenPtVsJetRecPtFirst->Fill(fJetPt, genpt, fPythiaEventWeight);
                    }

                    if (DefaultDiscriminator.size() > 1)
                    {
                        if (DefaultDiscriminator.at(1) >= fThresholdIP)
                            fh2dJetGenPtVsJetRecPtSecond->Fill(fJetPt, genpt, fPythiaEventWeight);
                    }

                    if (DefaultDiscriminator.size() > 2)
                    {
                        if (DefaultDiscriminator.at(2) >= fThresholdIP)
                            fh2dJetGenPtVsJetRecPtThird->Fill(fJetPt, genpt, fPythiaEventWeight);
                    }
                }
            }
        ...
        if (fDoSVAnalysis)
        {
            ...
            if (nVtx3Prong > 0)
            {
                ...
                {
                    if (fJetFlavor == 0)
                        fHistDispersion3ProngUnidentified->Fill(fJetPt, fDispersion);
                    if (fJetFlavor == 1)
                        fHistDispersion3Pronglf->Fill(fJetPt, fDispersion);
                    if (fJetFlavor == 2)
                        fHistDispersion3Prongc->Fill(fJetPt, fDispersion);
                    if (fJetFlavor == 3)
                ...
                if (fDispersion < 0.03)
                {
                    ...
                    {
                        if (fJetFlavor == 0)
                            fHistSV3ProngUnidentified->Fill(fJetPt, fLxySign, fInvariantMass);
                        if (fJetFlavor == 1)
                            fHistSV3Pronglf->Fill(fJetPt, fLxySign, fInvariantMass);
                        if (fJetFlavor == 2)
                            fHistSV3Prongc->Fill(fJetPt, fLxySign, fInvariantMass);
                        if (fJetFlavor == 3)
                            fHistSV3Prongb->Fill(fJetPt, fLxySign, fInvariantMass);
                    }
            ...
            if (nVtx2Prong > 0)
            {
                ...
                {
                    if (fJetFlavor == 0)
                        fHistDispersion2ProngUnidentified->Fill(fJetPt, fDispersion);
                    if (fJetFlavor == 1)
                        fHistDispersion2Pronglf->Fill(fJetPt, fDispersion);
                    if (fJetFlavor == 2)
                        fHistDispersion2Prongc->Fill(fJetPt, fDispersion);
                    if (fJetFlavor == 3)
                ...
                if (fDispersion < 0.03)
                {
                    ...
                    {
                        if (fJetFlavor == 0)
                            fHistSV2ProngUnidentified->Fill(fJetPt, fLxySign, fInvariantMass);
                        if (fJetFlavor == 1)
                            fHistSV2Pronglf->Fill(fJetPt, fLxySign, fInvariantMass);
                        if (fJetFlavor == 2)
                            fHistSV2Prongc->Fill(fJetPt, fLxySign, fInvariantMass);
                        if (fJetFlavor == 3)
                            fHistSV2Prongb->Fill(fJetPt, fLxySign, fInvariantMass);
                    }
                    if (fDoJetProbabilityAnalysis && fResolutionFunction[0] && fValJetProb >= 0 && fLxySign >= 5.)
                    {
    ...
    if (TaggedFirst)
    {
        ...
        if (fDoDeltaPtWithSignal)
            f2histRhoVsDeltaPtWithSignalFirst->Fill(randomConePtWithSignal, fJetContainerData->GetRhoVal(), fPythiaEventWeight);
    }
    if (TaggedSecond)
    {
        f2histRhoVsDeltaPtSecond->Fill(randomConePt, fJetContainerData->GetRhoVal(), fPythiaEventWeight);
        if (fDoDeltaPtWithSignal)
            f2histRhoVsDeltaPtWithSignalSecond->Fill(randomConePtWithSignal, fJetContainerData->GetRhoVal(), fPythiaEventWeight);
    }
    if (TaggedThird)
    {
        f2histRhoVsDeltaPtThird->Fill(randomConePt, fJetContainerData->GetRhoVal(), fPythiaEventWeight);
        if (fDoDeltaPtWithSignal)
            f2histRhoVsDeltaPtWithSignalThird->Fill(randomConePtWithSignal, fJetContainerData->GetRhoVal(), fPythiaEventWeight);
    }
...
Double_t AliAnalysisTaskBJetTC::CalculateJetProb(AliEmcalJet *jet, Int_t jetFlavor)
{
    ...
    for (Int_t itrack = 0; itrack < ntracks; ++itrack)
    {
        ...
        if (fUsePicoTracks)
            trackV = (AliAODTrack *)((AliPicoTrack *)jet->Track(itrack))->GetTrack();
        else
            trackV = (AliAODTrack *)((fJetContainerData->GetParticleContainer())->GetParticle(jet->TrackAt(itrack)));
...
Bool_t AliAnalysisTaskBJetTC::IsEventSelected()
{
    ...
    if (!(IsSelected(WhyRejected, RejectionBits)))
    {
        ...
        if (RejectionBits & (1 << kPhysicsSelection))
            fh1dEventRejectionRDHFCuts->SetBinContent(3, fh1dEventRejectionRDHFCuts->GetBinContent(3) + 1);
        else if (RejectionBits & (1 << kNoVertex))
            fh1dEventRejectionRDHFCuts->SetBinContent(6, fh1dEventRejectionRDHFCuts->GetBinContent(6) + 1);
        else if (RejectionBits & (1 << kNoVertexTracks))
            fh1dEventRejectionRDHFCuts->SetBinContent(11, fh1dEventRejectionRDHFCuts->GetBinContent(11) + 1);
        else if (RejectionBits & (1 << kNoContributors))
            fh1dEventRejectionRDHFCuts->SetBinContent(12, fh1dEventRejectionRDHFCuts->GetBinContent(12) + 1);
        else if (RejectionBits & (1 << kTooFewVtxContrib))
            fh1dEventRejectionRDHFCuts->SetBinContent(7, fh1dEventRejectionRDHFCuts->GetBinContent(7) + 1);
        else if (RejectionBits & (1 << kVertexZContrib))
            fh1dEventRejectionRDHFCuts->SetBinContent(15, fh1dEventRejectionRDHFCuts->GetBinContent(15) + 1);
        else if (RejectionBits & (1 << kVertexZResolution))
            fh1dEventRejectionRDHFCuts->SetBinContent(14, fh1dEventRejectionRDHFCuts->GetBinContent(14) + 1);
        else if (RejectionBits & (1 << kDeltaVertexZ))
            fh1dEventRejectionRDHFCuts->SetBinContent(13, fh1dEventRejectionRDHFCuts->GetBinContent(13) + 1);
        else if (RejectionBits & (1 << kZVtxOutFid))
            fh1dEventRejectionRDHFCuts->SetBinContent(5, fh1dEventRejectionRDHFCuts->GetBinContent(5) + 1);
        else if (RejectionBits & (1 << kOutsideCentrality))
            fh1dEventRejectionRDHFCuts->SetBinContent(4, fh1dEventRejectionRDHFCuts->GetBinContent(4) + 1);
        else if (RejectionBits & (1 << kNotSelTrigger))
            fh1dEventRejectionRDHFCuts->SetBinContent(8, fh1dEventRejectionRDHFCuts->GetBinContent(8) + 1);
        else if (RejectionBits & (1 << kSPDClusterCut))
            fh1dEventRejectionRDHFCuts->SetBinContent(9, fh1dEventRejectionRDHFCuts->GetBinContent(9) + 1);
        else if (RejectionBits & (1 << kMVPileup))
            fh1dEventRejectionRDHFCuts->SetBinContent(10, fh1dEventRejectionRDHFCuts->GetBinContent(10) + 1);
        else if (RejectionBits & (1 << kSelPtHardBin))
            fh1dEventRejectionRDHFCuts->SetBinContent(16, fh1dEventRejectionRDHFCuts->GetBinContent(16) + 1);
...
Bool_t AliAnalysisTaskBJetTC::IsSelected(Int_t &WhyRejected, ULong_t &RejectionBits)
{
    ...
    if (fIsPythia)
    {
        ...
        if (fPtHardAndTrackPtFactor > 0.)
        {
            ...
            {
                for (auto mctrack : mcpartcont->all())
                { // Not cuts applied ; use accept for cuts
...
void AliAnalysisTaskBJetTC::UserCreateOutputObjects()
{
    ...
    if (fEnableV0GammaRejection)
    {
        ...
        if (fV0Reader && fV0Reader->GetProduceImpactParamHistograms())
            fOutput->Add(fV0Reader->GetImpactParamHistograms());
    }
    ...
    if (fApplyV0Rec)
    {
        ...
        printf("-------------------------------------------------------\n");
        if (fbTPCRefit)
            printf("TPC refit for daughter tracks\n");
        if (fbRejectKinks)
            printf("reject kink-like production vertices of daughter tracks\n");
        if (fbFindableClusters)
            printf("require positive number of findable clusters\n");
        if (fdCutNCrossedRowsTPCMin > 0.)
            printf("min number of crossed TPC rows: %g\n", fdCutNCrossedRowsTPCMin);
        if (fdCutCrossedRowsOverFindMin > 0.)
            printf("min ratio crossed rows / findable clusters: %g\n", fdCutCrossedRowsOverFindMin);
        if (fdCutCrossedRowsOverFindMax > 0.)
            printf("max ratio crossed rows / findable clusters: %g\n", fdCutCrossedRowsOverFindMax);
        if (fdCutPtDaughterMin > 0.)
            printf("min pt of daughter tracks [GeV/c]: %g\n", fdCutPtDaughterMin);
        if (fdCutDCAToPrimVtxMin > 0.)
            printf("min DCA of daughters to the prim vtx [cm]: %g\n", fdCutDCAToPrimVtxMin);
        if (fdCutDCADaughtersMax > 0.)
            printf("max DCA between daughters [sigma of TPC tracking]: %g\n", fdCutDCADaughtersMax);
        if (fdCutEtaDaughterMax > 0.)
            printf("max |eta| of daughter tracks: %g\n", fdCutEtaDaughterMax);
        if (fdCutNSigmadEdxMax > 0. && (!fbIsPbPb || (fbIsPbPb && fdPtProtonPIDMax > 0.)))
            printf("max |Delta(dE/dx)| in the TPC [sigma dE/dx]: %g\n", fdCutNSigmadEdxMax);
        if (fdCutNSigmadEdxMax > 0. && fbIsPbPb && fdPtProtonPIDMax > 0.)
            printf("max pt of proton for applying PID cut [GeV/c]: %g\n", fdPtProtonPIDMax);
        printf("V0 reconstruction method: %s\n", fbOnFly ? "on-the-fly" : "offline");
        if (fdCutCPAKMin > 0.)
            printf("min CPA, K0S: %g\n", fdCutCPAKMin);
        if (fdCutCPALMin > 0.)
            printf("min CPA, (A)Lambda: %g\n", fdCutCPALMin);
        if (fdCutRadiusDecayMin > 0. && fdCutRadiusDecayMax > 0.)
            printf("R of the decay vertex [cm]: %g-%g\n", fdCutRadiusDecayMin, fdCutRadiusDecayMax);
        if (fdCutEtaV0Max > 0.)
            printf("max |eta| of V0: %g\n", fdCutEtaV0Max);
        if (fdCutRapV0Max > 0.)
            printf("max |y| of V0: %g\n", fdCutRapV0Max);
        if (fdCutNTauKMax > 0.)
            printf("max proper lifetime, K0S [tau]: %g\n", fdCutNTauKMax);
        if (fdCutNTauLMax > 0.)
            printf("max proper lifetime, (A)Lambda [tau]: %g\n", fdCutNTauLMax);
        if (fbCutArmPod)
            printf("Armenteros-Podolanski cut for K0S\n");
        if (fbCutCross)
            printf("cross-contamination cut\n");
        printf("-------------------------------------------------------\n");
    ...
    if (fEnableV0GammaRejection)
        fh1dPhotonPt = new TH1D("fh1dPhotonPt", "Photon Mass Vs Pt;p_{T,photon} (GeV/c)", 200, 0, 20);
...
Bool_t AliAnalysisTaskBJetTC::CalculateTrackImpactParameter(AliAODTrack *track, double *impar, double *cov)
{
    ...
    if (fUseNewIPsCalculation)
    {
        vtxAODNew = fAODIn->GetPrimaryVertex();
    }
    else
    {
    ...
    if (etp.PropagateToDCA(vtxAODNew, fAODIn->GetMagneticField(), 3., impar, cov))
    {
        if (fCorrectRes)
        {
            ...
            cov[0] = CorrectResolution(cov[0], psc, fAODIn->GetPrimaryVertex()->GetNContributors(), track->GetITSNcls());
        }
...
Bool_t AliAnalysisTaskBJetTC::CalculateJetSignedTrackImpactParameter(AliAODTrack *track, AliEmcalJet *jet, double *impar, double *cov, double &sign, double &dcajetrack, double &lineardecaylength)
{
    ...
    if (fUseNewIPsCalculation)
    {
        AliAODVertex *vtxAOD = fAODIn->GetPrimaryVertex();
        if (!etp.PropagateToDCA(vtxAOD, fAODIn->GetMagneticField(), 100., impar, cov))
            return kFALSE;

        vtxAOD->GetXYZ(vtxPos);
    }
    else
    {
        Int_t skipped[1] = {-1};
    ...
    Double_t ipvector3[3] = {posdcatrack[0] - vtxPos[0], posdcatrack[1] - vtxPos[1], posdcatrack[2] - vtxPos[2]};
...
Double_t AliAnalysisTaskBJetTC::CorrectResolution(Double_t resolution, Double_t pScat, Int_t nVtxContrib, Int_t nITS)
{
    Double_t AlphaPullVsPscat = 1.;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            std::cout << "Correction function Vs pScat don't exists, Returning the original value of resolution" << std::endl;
            return resolution;
        }
    }
    ...
    for (int i = 3; i < 5; i++)
    {
        ...
        {
            std::cout << "Correction function doesn't exists for ITS hits > 4, Returning the original value of resolution" << std::endl;
            return resolution;
        }
    }

    AlphaPullVsPscat = fCorrectionFactorsPscat[nITS - 2]->Eval(pScat);

    Double_t CorrResVsPscat = resolution * TMath::Power(AlphaPullVsPscat, 2);

    Double_t AlphaPullVsNvtxContrib = 1.;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            std::cout << "Correction function Vs primary vertex contributors don't exists, Returning the original value of resolution" << std::endl;
            return resolution;
        }
    }
    ...
    AlphaPullVsNvtxContrib = fCorrectionFactorsNvtxContrib[nITS - 2]->Eval(nVtxContrib);

    Double_t CorrResVsPscatNvtxContrib = CorrResVsPscat * TMath::Power(AlphaPullVsNvtxContrib, 2);

    //cout<<"Old Value of Res: "<<resolution<<" And the new value is: "<<CorrResVsPscatNvtxContrib<<
    //" pSat= "<<pScat<<" Alpha pScat: "<<AlphaPullVsPscat<<" nVtxContrib= "<<nVtxContrib<<" Alpha NvtxContrib: "<<AlphaPullVsNvtxContrib<<endl;

    return CorrResVsPscatNvtxContrib;
}
...
Bool_t AliAnalysisTaskBJetTC::IsTrackAccepted(AliAODTrack *track)
{
    ...
    if (SPDSSDHits < fTCMinHitsITS)
        return kFALSE; //2 in case of FAST and CENT_woSDD and 3 if there was an SDD
...
Bool_t AliAnalysisTaskBJetTC::SelectV0CandidateVIT()
{
    ...
    for (Int_t iV0 = 0; iV0 < iNV0s; iV0++)
    {
        ...
        if (bPrintCuts)
            printf("Rec: Applying cut: Reconstruction method: %s\n", (fbOnFly ? "on-the-fly" : "offline"));
        ...
        // Tracks TPC OK
        if (bPrintCuts)
            printf("Rec: Applying cut: Correct charge of daughters\n");
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: TPC refit\n");
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Type of production vertex of daughter: No kinks\n");
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Positive number of findable clusters\n");
            if (dFindableNeg <= 0.)
                continue;
            if (dFindablePos <= 0.)
                continue;
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Number of TPC rows >= %g\n", fdCutNCrossedRowsTPCMin);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: rows/findable >= %g\n", fdCutCrossedRowsOverFindMin);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Daughter pt >= %g\n", fdCutPtDaughterMin);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Daughter DCA to prim vtx >= %g\n", fdCutDCAToPrimVtxMin);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: DCA between daughters <= %g\n", fdCutDCADaughtersMax);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: CPA >= %g (K)\n", fdCutCPAKMin);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: CPA >= %g (L, AL)\n", fdCutCPALMin);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Decay radius >= %g, <= %g\n", fdCutRadiusDecayMin, fdCutRadiusDecayMax);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Daughter |eta| < %g\n", fdCutEtaDaughterMax);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: V0 |eta| < %g\n", fdCutEtaV0Max);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: V0 |y| < %g\n", fdCutRapV0Max);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Proper lifetime < %g (K)\n", fdCutNTauKMax);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Proper lifetime < %g (L, AL)\n", fdCutNTauLMax);
        ...
        if (fdCutNSigmadEdxMax > 0.)
        {
            ...
            {
                if (bPrintCuts)
                    printf("Rec: Applying cut: Delta dE/dx (proton below %g GeV/c) < %g\n", fdPtProtonPIDMax, fdCutNSigmadEdxMax);
            ...
            {
                if (bPrintCuts)
                    printf("Rec: Applying cut: Delta dE/dx (both daughters): < %g\n", fdCutNSigmadEdxMax);
        ...
        {
            if (bPrintCuts)
                printf("Rec: Applying cut: Armenteros-Podolanski (K0S) pT > %g * |alpha|\n", 0.2);
    ...
    */
        ...
        if (bIsCandidateK0s)
        {
            fh2dKshortMassVsPt->Fill(dPtV0, dMassV0K0s, fPythiaEventWeight);
        }
        if (bIsCandidateLambda)
        {
            fh2dLamdaMassVsPt->Fill(dPtV0, dMassV0Lambda, fPythiaEventWeight);
        }
        if (bIsCandidateALambda)
        {
            fh2dAnLamdaMassVsPt->Fill(dPtV0, dMassV0ALambda, fPythiaEventWeight);
        }
        ...
        if (fIsPythia)
        {
            ...
            {
                if (bPrintCuts)
                    printf("Rec->Gen: Applying cut: V0 |eta|: < %g\n", fdCutEtaV0Max);
            ...
            {
                if (bPrintCuts)
                    printf("Rec->Gen: Applying cut: V0 |y|: < %g\n", fdCutRapV0Max);
...
Bool_t AliAnalysisTaskBJetTC::IsV0InJet(TVector3 vV0, Double_t dJetPtMin)
{
    ...
    fJetContainerData->ResetCurrentID();
    AliEmcalJet *pJet = fJetContainerData->GetNextAcceptJet();
    while (pJet)
    {
...
Bool_t AliAnalysisTaskBJetTC::IsElectronHF(AliAODTrack *track)
{
    ...
    if (!track->PropagateToDCA(fPrimaryVertex, fAODIn->GetMagneticField(), 20., d0z0, cov))
        return kFALSE;
    ...
    if (track->GetTPCNcls() < 80)
        return kFALSE;
    if (track->GetITSNcls() < 2)
        return kFALSE; // AOD track level

    if (!(track->HasPointOnITSLayer(0) || track->HasPointOnITSLayer(1)))
        return kFALSE; // kAny
    if ((!(track->GetStatus() & AliESDtrack::kITSrefit) || (!(track->GetStatus() & AliESDtrack::kTPCrefit))))
        return kFALSE;
    ...
    if (cTypeVtxProd == AliAODVertex::kKink)
        return kFALSE; // kink daughter rejection
    ...
    if (fIsPythia)
    {
        ...
        if (abs(ParticleMC->GetPdgCode()) == 11)
            pid_ele = 1.0;
    }
    ...
    if (fTPCnSigma < -0.5 || fTPCnSigma > 3.)
        return kFALSE; //++++++++
    ...
    if (pt > 6)
    {
        ...
        if (clustMatch && clustMatch->IsEMCAL())
        {
            ...
            if (TMath::Abs(fPhiDiff) > 0.05 || TMath::Abs(fEtaDiff) > 0.05)
                return kFALSE;
...
//________________________________________________________________________
Double_t AliAnalysisTaskBJetTC::GetDeltaPtPerpEmbedding(Double_t signalEta, Double_t signalPhi)
{
    ...
    for (Int_t i = 0; i < partcont->GetNAcceptedParticles(); i++)
    {
        ...
        AliAODTrack *trk = static_cast<AliAODTrack *>(partcont->GetParticle(i));
