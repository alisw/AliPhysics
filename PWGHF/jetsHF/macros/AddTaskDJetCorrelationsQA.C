...
)
{
  ...
  AliAnalysisTaskDJetCorrelationsQA *taskCorr = new AliAnalysisTaskDJetCorrelationsQA(taskCorrName.Data(), analysiscuts, cand);
  ...
  if(jetCont) {
     ...
     jetCont->SetPercAreaCut(static_cast<Float_t>(percjetareacut));
  ...
  if(!jetArrBkgname.IsNull())
  {
     // AliParticleContainer *trackContBkg  = taskCorr->AddParticleContainer(trackArrBkgname); //is this change OK?
      ...
      if(CorrMethod == AliAnalysisTaskDJetCorrelationsQA::kResponseMatrix)
      {
      ...
      if(jetContBkg) {
          ...
          jetContBkg->SetPercAreaCut(static_cast<Float_t>(percjetareacut));
