// AddTaskDmesonsFilterCJ.C

AliAnalysisTaskDmesonsFilterCJ *AddTaskDmesonsFilterCJ(AliAnalysisTaskDmesonsFilterCJ::ECandidateType cand = AliAnalysisTaskDmesonsFilterCJ::kDstartoKpipi,
                                                           ...
                                                           TString suffix = "")
{
  ...
  if (!mgr) {
    ::Error("AddTaskDmesonsFilterCJ", "No analysis manager to connect to.");
    return NULL;
  } 
  ...
  if (!filecuts || (filecuts && !filecuts->IsOpen())) {
    ::Warning("AddTaskDmesonsFilterCJ", "Input file not found: use std cuts");
    useStdC = kTRUE;
  }

  if (!theMCon && !reco) {
    ::Warning("AddTaskDmesonsFilterCJ", "'reco' must be kTRUE if 'theMCon' is kFALSE!");
    reco = kTRUE;
  }
  ...
  AliAnalysisTaskDmesonsFilterCJ* task = (AliAnalysisTaskDmesonsFilterCJ*)mgr->GetTask(taskFiltername.Data());
  if (task) {
    ::Info("AddTaskDmesonsFilterCJ", Form("Task %s already exist, continue",taskFiltername.Data()));
    return task;
  }
  else {
    ::Info("AddTaskDmesonsFilterCJ", "Creating the task");

    // create the task
    task = new AliAnalysisTaskDmesonsFilterCJ(taskFiltername.Data(), analysiscuts, cand);
    task->SetMC(theMCon);
  ...
  ::Info("AddTaskDmesonsFilterCJ", "Input and Output connected to the manager");
  return task;
}
