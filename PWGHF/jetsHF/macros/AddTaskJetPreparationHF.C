...
)
{
  ...
  if (dType == "ESD") {
    ...
    aodfilter = AddTaskEmcalAodTrackFilter(inputTracks,"tracks",period); //period here is to set specific bits for hybrid tracks. In case of HF tasks, we set below the specific bits SetAODfilterBits(16,512);
    if (doAODTrackProp) {
