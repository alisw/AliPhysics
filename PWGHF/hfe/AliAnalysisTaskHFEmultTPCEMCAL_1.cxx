...
#include "AliAnalysisTaskHFEmultTPCEMCAL_1.h"
...
ClassImp(AliAnalysisTaskHFEmultTPCEMCAL_1)

//________________________________________________________________________//
AliAnalysisTaskHFEmultTPCEMCAL_1::AliAnalysisTaskHFEmultTPCEMCAL_1(): 
AliAnalysisTaskSE(),
...
fnSigmaVsP_TPC(0), fnSigmaVsP_TPC_old(0),fnSigmaVsPt_TPC(0), fnSigmaVsPt_TPC_old(0)
	{
...
AliAnalysisTaskHFEmultTPCEMCAL_1::AliAnalysisTaskHFEmultTPCEMCAL_1(const char *name) 
...
void AliAnalysisTaskHFEmultTPCEMCAL_1::Init()
{
  // Initialization	
  if(fDebug > 1) printf("AliAnalysisTaskHFEmultTPCEMCAL_1::Init() \n");
  return;
}
...
//________________________________________________________________________
void AliAnalysisTaskHFEmultTPCEMCAL_1::UserCreateOutputObjects()
{
  // Create histograms
  // Called once
	//Initialize PID
  ...
  fOutputList->Add(fnSigmaVsP_TPC);
   ...
   fnSigmaVsP_TPC_old= new TH2F("fnSigmaVsP_TPC_old", "fnSigmaVsP_TPC_old distribution",300,0.,15.,750,-15.,15.);
  fnSigmaVsP_TPC_old->Sumw2(); 
  fOutputList->Add(fnSigmaVsP_TPC_old);
 ...
 //--------------------------TPC dedx nsig vs pt ----------------------------------------------
  ...
  fnSigmaVsPt_TPC_old= new TH2F("fnSigmaVsPt_TPC_old", "fnSigmaVsPt_TPC_old distribution",300,0.,15.,750,-15.,15.);
  fnSigmaVsPt_TPC_old->Sumw2(); 
  fOutputList->Add(fnSigmaVsPt_TPC_old); 
...
void AliAnalysisTaskHFEmultTPCEMCAL_1::UserExec(Option_t *) 
{
...
//////==================================================================================================/////
  ...
  for ( Int_t index = 0; index < Nclust ; index++ ) 
  {
				...
				if (clu->IsEMCAL())
				{
				  ...
				  //if(!fIsMC)if(clu->GetIsExotic()) continue; //remove exotic clusters
  ...
  Int_t runNumber = fAOD->GetRunNumber();
	...
	for (Int_t iTracks = 0; iTracks < ntracks; iTracks++) 
	{
   		...
   		if(tracktypeTrig!=1) continue;  //if(tracktype==0) continue; if(tracktype==1)  	
			...
			Double_t pt=0., p=0., eta=-999, dEdx_TPC=-999., Signal_TOF=-999., TOFbeta=-999., fTPCnSigma=-999.0,  fTPCnSigma_old=-999.0, fTOFnSigma=-999.0;    
		...
		//if(fIsMC &&(track->GetLabel())>=0)
			...
			fnSigmaVsP_TPC_old->Fill(p,fTPCnSigma_old);
			fnSigmaVsPt_TPC_old->Fill(pt,fTPCnSigma_old);
    	...
    	if(clustMatch && clustMatch->IsEMCAL())
    	{
    	 	...
    	 	//  if(!fIsMC)  if(clustMatch->GetIsExotic()) continue;
...
void AliAnalysisTaskHFEmultTPCEMCAL_1::GetTrkClsEtaPhiDiff(AliVTrack *t, AliVCluster *v, Double_t &phidiff, Double_t &etadiff)
{
...
Bool_t AliAnalysisTaskHFEmultTPCEMCAL_1::PassEIDCuts(AliAODTrack *track, AliVCluster *clust)
{
...
//--------------------END of UserExec----------------------------------
Int_t AliAnalysisTaskHFEmultTPCEMCAL_1::ClassifyTrack(AliAODTrack* track,const AliVVertex* pVtx)
{  
...
void AliAnalysisTaskHFEmultTPCEMCAL_1::SelectPhotonicElectronR(Int_t itrack, AliAODTrack *track, Int_t motherindex, Int_t pdg1, Int_t source , Double_t SPDntr1,Double_t ptmotherwg)
{
...
void AliAnalysisTaskHFEmultTPCEMCAL_1::SelectPhotonicElectron(Int_t itrack, AliVTrack *track, Bool_t &fFlagPhotonicElec, Int_t iMC)
{
    ///////////////////////////////////////////
...
/*
Double_t AliAnalysisTaskHFEmultTPCEMCAL_1::WeightMCncCorr(Int_t multSPDtr)
{
...
*/
//====================================================================================================================================
Int_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetElecSourceType(AliAODMCParticle *electron,Double_t &ptm)
{
    //
...
Int_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetPi0EtaType(AliAODMCParticle *pi0eta)
{
...
Int_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetHFE(AliAODMCParticle *electron, TClonesArray *mcArray)
{
...
Int_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetNcharged(){
...
Bool_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetNMCPartProduced()
{
...
void AliAnalysisTaskHFEmultTPCEMCAL_1::GetPi0EtaWeight(THnSparse *SparseWeight)
{
...
//_________________________________________
Bool_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetNonHFEEffiDenom(AliVTrack *track)
//Bool_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetNonHFEEffiDenom(AliAODTrack *track)
{
...
//=======================================================================
Bool_t  AliAnalysisTaskHFEmultTPCEMCAL_1::IsNonHFE(AliAODMCParticle *MCPart, Bool_t &fFromMB, Int_t &type, Int_t &iMCmom, Int_t &MomPDG, Double_t &MomPt)
{
...
Bool_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetNonHFEEffiRecoTag(AliVTrack *track)
{
    //Tagging method
...
//_________________________________________
Bool_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetNonHFEEffiULSLS(AliVTrack *track, AliVTrack *Assotrack, Bool_t fFlagLS, Bool_t fFlagULS, Double_t mass)
{
    //ULS-LS method
...
//===============================================================================
TProfile* AliAnalysisTaskHFEmultTPCEMCAL_1::GetEstimatorHistogram(const AliAODEvent* fAOD)
{
...
*/
Double_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetCorrectedNtracklets(TProfile* estimatorAvg, Double_t uncorrectedNacc, Double_t vtxZ, Double_t refMult) {
  //
...
Double_t AliAnalysisTaskHFEmultTPCEMCAL_1::GetTPCCalibration(Int_t runNo, Double_t TPCnsigma0)
{
...
/*
//===================================================================================
Double_t AliAnalysisTaskHFEmultTPCEMCAL_1::Beta(AliAODTrack *track)
{
...
*/
//===================================================================================
void AliAnalysisTaskHFEmultTPCEMCAL_1::Terminate(Option_t *) 
{
