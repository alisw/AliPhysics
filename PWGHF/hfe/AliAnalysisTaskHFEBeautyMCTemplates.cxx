...
#include "AliAnalysisTaskHFEBeautyMCTemplates.h"
...
ClassImp(AliAnalysisTaskHFEBeautyMCTemplates)
...
//________________________________________________________________________
void AliAnalysisTaskHFEBeautyMCTemplates::UserCreateOutputObjects()
{
  // Set up HFE objects:
...
Int_t AliAnalysisTaskHFEBeautyMCTemplates::FindSource(AliMCParticle * mcple, AliMCEvent* fMCEvent, Double_t &motherPt, Double_t &GroundStateMotherPt)
{
  motherPt = 0.;
...
Int_t AliAnalysisTaskHFEBeautyMCTemplates::MotherPDG(AliMCParticle * mcple, AliMCEvent* fMCEvent)
{
...
void AliAnalysisTaskHFEBeautyMCTemplates::PrintHierarchy(AliMCParticle * mcple, AliMCEvent* fMCEvent)
{
  AliMCParticle * mother;
  Int_t pdgCode = 0;
  mother=mcple;
...
Bool_t AliAnalysisTaskHFEBeautyMCTemplates::PassesTrackCuts(AliAODTrack *track)
{
...
Bool_t AliAnalysisTaskHFEBeautyMCTemplates::PassesMinimalTrackCuts(AliAODTrack *track)
{
    if(TMath::Abs(track->Eta())>0.8) return kFALSE;
    ULong_t status = track->GetStatus();
    // Basic tracking
...
Bool_t AliAnalysisTaskHFEBeautyMCTemplates::PassesITSTrackCuts(AliAODTrack *track)
{
    if(TMath::Abs(track->Eta())>0.8) return kFALSE;
    ULong_t status = track->GetStatus();
    // Basic tracking
...
void AliAnalysisTaskHFEBeautyMCTemplates::UserExec(Option_t *)
{
  //
...
//________________________________________________________________________
void AliAnalysisTaskHFEBeautyMCTemplates::Process(AliAODEvent *const aodEvent)
{
  // Main loop
  // Called for each event
    ...
    for(int i=0;i<nTracks;i++)
    {
      ...
      if(TMath::Abs(pdgCode) == 11)
      {
        ...
        if(Source == 0)
        {
          ...
          CorrCharm3050 = 1410.16*(TMath::Exp(-1.16759*MotherPt)+0.00498538*TMath::Exp(-0.357565*MotherPt))/(TMath::Exp(-MotherPt*0.11023)+234.578*TMath::Exp(-MotherPt*0.439153))
        ...
        if(Source == 1)
        {
          ...
          CorrBeautyHalfRAA = (1.+(0.5/(1. + TMath::Exp((MotherPt-7.)*0.7)) + 0.5 + (MotherPt-15.)/300.))/2.;
          ...
          CorrBeautyRAA = 0.5/(1. + TMath::Exp((MotherPt-7.)*0.7)) + 0.5 + (MotherPt-15.)/300.;
...
//________________________________________________________________________
void AliAnalysisTaskHFEBeautyMCTemplates::Terminate(const Option_t *)
{
//   treeoutput->Write();
