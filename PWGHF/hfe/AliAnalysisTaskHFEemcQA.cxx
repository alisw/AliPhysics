 ...
 */
    ...
    fCompTrackPt = new TH1F("fCompTrackPt","p_{T} distribution of tracks after cuts for comparison;p_{T} (GeV/c);counts",250,0,25);
    fOutputList->Add(fCompTrackPt);
    
    fCompTrackPtinEMCreg = new TH1F("fCompTrackPtinEMCreg","p_{T} distribution of tracks after cuts in EMC region for comparison;p_{T} (GeV/c);counts",250,0,25);
    fOutputList->Add(fCompTrackPtinEMCreg);
    
    fCompTrackPtMatch = new TH1F("fCompTrackPtMatch","p_{T} distribution of tracks matched to EMC surface for comparison;p_{T} (GeV/c);counts",250,0,25);
    fOutputList->Add(fCompTrackPtMatch);
    
    fCompTrackPtMatchwithEMC = new TH1F("fCompTrackPtMatchwithEMC","p_{T} distribution of tracks matched to EMC cluster for comparison;p_{T} (GeV/c);counts",250,0,25);
    fOutputList->Add(fCompTrackPtMatchwithEMC);
    
    if(fFlagSparse){
    Int_t bins[9]=      {8, 280, 160, 40, 200, 200, 3, 20,  10}; // trigger;pT;nSigma;eop;m20;m02;sqrtm02m20;eID;nSigma_Pi;cent
...
Bool_t AliAnalysisTaskHFEemcQA::ComparePropWithTPCAna(const AliVVertex *pVtx, AliAODTrack* atrack)
{
    ...
    //Number of clusters in TPC for PID not applied
