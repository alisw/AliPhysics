...
ClassImp(AliAnalysisTask_QA_EMCALElectrons)

//________________________________________________________________________//
AliAnalysisTask_QA_EMCALElectrons::AliAnalysisTask_QA_EMCALElectrons(): 
AliAnalysisTaskSE(),
...
AliAnalysisTask_QA_EMCALElectrons::AliAnalysisTask_QA_EMCALElectrons(const char *name) 
...
void AliAnalysisTask_QA_EMCALElectrons::Init()
{
  // Initialization	
  if(fDebug > 1) printf("AliAnalysisTask_QA_EMCALElectrons::Init() \n");
  return;
}
...
//________________________________________________________________________
void AliAnalysisTask_QA_EMCALElectrons::UserCreateOutputObjects()
{
  // Create histograms
  // Called once
	//Initialize PID
 ...
 */ 
  ...
  Int_t bins[7]	=       {380,  180,  100,  100,    7,   7, 100};
  Double_t xmin[7]	=	{  2,  -10,    0,    0, -0.5, -0.5,  0};
  Double_t xmax[7]	=	{ 40,    8,    2,    2,  6.5,  6.5,  2};

  fSparseElectron 	= new THnSparseD ("Electron","Electron;pT;TPCnsig;EbyP;m02;clusPID;trackPID;EbyPNonLnrCorr",7,bins,xmin,xmax);
  fSparseElectron->GetAxis(0)->SetName("pT");
  fSparseElectron->GetAxis(1)->SetName("TPCnsigma");
  fSparseElectron->GetAxis(2)->SetName("E/p");
  fSparseElectron->GetAxis(3)->SetName("m02");
  fSparseElectron->GetAxis(4)->SetName("ClusPID");
  fSparseElectron->GetAxis(5)->SetName("TrackPID");
   fSparseElectron->GetAxis(6)->SetName("E/p after Non Linearity Correction");
  fSparseElectron->Sumw2();
  fOutputList->Add(fSparseElectron);
...
void AliAnalysisTask_QA_EMCALElectrons::UserExec(Option_t *) 
{
  ...
  Int_t runNumber = fAOD->GetRunNumber();
	...
	for (Int_t iTracks = 0; iTracks < ntracks; iTracks++) 
	{
      ...
      //Double_t clustTime = clustMatch->GetTOF()*1e+9; // ns;
		  ...
		  Double_t  clustMatchE = -999.0, clustMatchENonLnr = -999.0, Eoptrk = -999.0 , EoptrkNonLnr = -999.0 ,M02trkmatch = -999.0, M20trkmatch = -999.0;
  
		  clustMatchE = clustMatch->E();
		  clustMatchENonLnr = clustMatch->GetNonLinCorrEnergy();
		  if(p>0)Eoptrk = clustMatchE/p;
		  if(p>0)EoptrkNonLnr = clustMatchENonLnr/p;
		...
		if(fIsMC && track->GetLabel()>=0) {
			...
			if(fIsMC && clustMatch->GetLabel()>=0) {
   		  
   		   fMCparticle=(AliAODMCParticle*)fMCArray->At(clustMatch->GetLabel());
		    Int_t pdg = TMath::Abs(fMCparticle->GetPdgCode());
       ...
       if(pt < 2) continue;   
		  ...
		  fvalueElectron[4] = clsPid; //
          fvalueElectron[5] = trkPid; //
		  fvalueElectron[6] = EoptrkNonLnr;//
...
void AliAnalysisTask_QA_EMCALElectrons::GetTrkClsEtaPhiDiff(AliVTrack *t, AliVCluster *v, Double_t &phidiff, Double_t &etadiff)
{
...
Bool_t AliAnalysisTask_QA_EMCALElectrons::PassEIDCuts(AliAODTrack *track, AliVCluster *clust)
{
...
//--------------------END of UserExec----------------------------------
Int_t AliAnalysisTask_QA_EMCALElectrons::ClassifyTrack(AliAODTrack* track,const AliVVertex* pVtx)
{  
...
void AliAnalysisTask_QA_EMCALElectrons::SelectPhotonicElectronR(Int_t itrack, AliAODTrack *track, Int_t motherindex, Int_t pdg1, Int_t source , Double_t SPDntr1,Double_t ptmotherwg)
{
...
void AliAnalysisTask_QA_EMCALElectrons::SelectPhotonicElectron(Int_t itrack, AliVTrack *track, Bool_t &fFlagPhotonicElec, Int_t iMC)
{
    ///////////////////////////////////////////
...
/*
Double_t AliAnalysisTask_QA_EMCALElectrons::WeightMCncCorr(Int_t multSPDtr)
{
...
*/
//====================================================================================================================================
Int_t AliAnalysisTask_QA_EMCALElectrons::GetElecSourceType(AliAODMCParticle *electron,Double_t &ptm)
{
    //
...
Int_t AliAnalysisTask_QA_EMCALElectrons::GetPi0EtaType(AliAODMCParticle *pi0eta)
{
...
Int_t AliAnalysisTask_QA_EMCALElectrons::GetHFE(AliAODMCParticle *electron, TClonesArray *mcArray)
{
...
Int_t AliAnalysisTask_QA_EMCALElectrons::GetNcharged(){
...
Bool_t AliAnalysisTask_QA_EMCALElectrons::GetNMCPartProduced()
{
...
void AliAnalysisTask_QA_EMCALElectrons::GetPi0EtaWeight(THnSparse *SparseWeight)
{
...
//_________________________________________
Bool_t AliAnalysisTask_QA_EMCALElectrons::GetNonHFEEffiDenom(AliVTrack *track)
//Bool_t AliAnalysisTask_QA_EMCALElectrons::GetNonHFEEffiDenom(AliAODTrack *track)
{
...
//=======================================================================
Bool_t  AliAnalysisTask_QA_EMCALElectrons::IsNonHFE(AliAODMCParticle *MCPart, Bool_t &fFromMB, Int_t &type, Int_t &iMCmom, Int_t &MomPDG, Double_t &MomPt)
{
...
Bool_t AliAnalysisTask_QA_EMCALElectrons::GetNonHFEEffiRecoTag(AliVTrack *track)
{
    //Tagging method
...
//_________________________________________
Bool_t AliAnalysisTask_QA_EMCALElectrons::GetNonHFEEffiULSLS(AliVTrack *track, AliVTrack *Assotrack, Bool_t fFlagLS, Bool_t fFlagULS, Double_t mass)
{
    //ULS-LS method
...
TProfile* AliAnalysisTask_QA_EMCALElectrons::GetEstimatorHistogram(const AliAODEvent* fAOD)
{
...
Double_t AliAnalysisTask_QA_EMCALElectrons::GetCorrectedNtracklets(TProfile* estimatorAvg, Double_t uncorrectedNacc, Double_t vtxZ, Double_t refMult) {
  //
...
/*
//===================================================================================
Double_t AliAnalysisTask_QA_EMCALElectrons::Beta(AliAODTrack *track)
{
...
*/
//===================================================================================
void AliAnalysisTask_QA_EMCALElectrons::Terminate(Option_t *) 
{
