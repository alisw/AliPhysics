...
// and hadrons correlations Vs mult
//
// Authors:
// Fabio Colamaria, fabio.colamaria@ba.infn.it (correlations)
// Samrangy Sadhu, samrangy.sadhu@cern.ch (mult dependence)
/////////////////////////////////////////////////////////////
...
#include "AliAnalysisTaskSED0CorrelationsVsMult.h"
...
ClassImp(AliAnalysisTaskSED0CorrelationsVsMult)


//________________________________________________________________________
AliAnalysisTaskSED0CorrelationsVsMult::AliAnalysisTaskSED0CorrelationsVsMult():
AliAnalysisTaskSE(),
  fNPtBinsCorr(0), 
  fBinLimsCorr(),
  fPtThreshLow(),
...
//________________________________________________________________________
AliAnalysisTaskSED0CorrelationsVsMult::AliAnalysisTaskSED0CorrelationsVsMult(const char *name,AliRDHFCutsD0toKpi* cutsD0, AliHFAssociatedTrackCuts* cutsTrk):
...
//________________________________________________________________________
AliAnalysisTaskSED0CorrelationsVsMult::AliAnalysisTaskSED0CorrelationsVsMult(const AliAnalysisTaskSED0CorrelationsVsMult &source):
  ...
  //added part ==================//
  fMultiplicityEstimator(source.fMultiplicityEstimator),
  fRefMult(source.fRefMult),
  fDoVZER0ParamVertexCorr(source.fDoVZER0ParamVertexCorr),
  fMultEstimatorAvg(source.fMultEstimatorAvg),
  fMultipOrCent(source.fMultipOrCent)
{
  // Copy constructor
}
...
AliAnalysisTaskSED0CorrelationsVsMult& AliAnalysisTaskSED0CorrelationsVsMult::operator=(const AliAnalysisTaskSED0CorrelationsVsMult& orig)
{
// Assignment
  ...
  // added part ==================== //
  fMultiplicityEstimator=orig.fMultiplicityEstimator;
  fRefMult=orig.fRefMult;
  fDoVZER0ParamVertexCorr=orig.fDoVZER0ParamVertexCorr;
  fMultEstimatorAvg=orig.fMultEstimatorAvg;
  fMultipOrCent=orig.fMultipOrCent;
  
  return *this; //returns pointer of the class
}

//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::Init()
{
  // Initialization
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::UserCreateOutputObjects()
{
  ...
  TString nameMass=" ",nameSgn=" ", nameBkg=" ", nameRfl=" ",nameMassWg=" ",nameSgnWg=" ", nameBkgWg=" ", nameRflWg=" ",nameMass2D=" ",nameMassWg2D=" ";
...
//for origin c case (or for data)
  for(Int_t i=0;i<fCutsD0->GetNPtBins();i++){
    ...
    nameMassWg+=i;
    nameMass2D="histMass2D_";
    nameMass2D+=i;
    nameMassWg2D="histMass2D_WeigD0Eff_";
    nameMassWg2D+=i;
    ...
    // ++++++++++ added two histos ++++++++++++++++++++
    TH2F* tmpMt2D = new TH2F(nameMass2D.Data(),"D^{0} invariant mass c; M [GeV]; Entries",150,1.5648,2.1648,300,0,300);
    tmpMt2D->Sumw2();
    fOutputMass->Add(tmpMt2D);
    //mass weighted by 1/D0eff
    TH2F* tmpMt2Dwg = new TH2F(nameMassWg2D.Data(),"D^{0} invariant mass c - weight 1/D0eff; M [GeV]; Entries",150,1.5648,2.1648,300,0,300);
    tmpMt2Dwg->Sumw2();
    fOutputMass->Add(tmpMt2Dwg);
...
//for origin b case (no Bkg and Mass histos, here for weights you should use c+b efficiencies, while on data (on MC they're useless))
  ...
  //Added histograms for mult correction
  
   TH1F* huncorrected_mult = new TH1F("huncorrected_mult","uncorrected multiplicity",200,0,200);
  TH2F* zvtxVsMult = new TH2F("zvtxVsMult","zVtx Vs uncorrected multiplicity",100,-10,10,200,0,200);
  TH1F* hcorrected_mult = new TH1F("hcorrected_mult","corrected multiplicity",200,0,200);
  TH2F* zvtxVsMultCorr = new TH2F("zvtxVsMultCorr","zVtx Vs corrected multiplicity",100,-10,10,200,0,200);
  fOutputStudy->Add(huncorrected_mult);
  fOutputStudy->Add(zvtxVsMult);
  fOutputStudy->Add(hcorrected_mult);
  fOutputStudy->Add(zvtxVsMultCorr);
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::UserExec(Option_t */*option*/)
{
  ...
  if(!inputArray || !aod) {
    printf("AliAnalysisTaskSED0CorrelationsVsMult::UserExec: input branch not found!\n");
    return;
  }
  ...
  if(fReadMC) {
    ...
    if(!mcArray) {
      printf("AliAnalysisTaskSED0CorrelationsVsMult::UserExec: MC particles branch not found!\n");
      return;
    }
    ...
    if(!mcHeader) {
      printf("AliAnalysisTaskSED0CorrelationsVsMult::UserExec: MC header branch not found!\n");
      return;
    }
  }
  ...
  ((TH1F*)fOutputStudy->FindObject("huncorrected_mult"))->Fill(countTreta1);
  ...
  //AliAODVertex *vtxmult = (AliAODVertex*)aod->GetPrimaryVertex();
...
if(isVtxOk && isDataDrivenZvtxCorr){
  cout << "count mult---> " << countTreta1 <<endl;
    countTreta1corr=static_cast<Int_t>(AliVertexingHFUtils::GetCorrectedNtracklets(fMultEstimatorAvg,countTreta1,vtx1->GetZ(),fRefMult));
    cout << "count mult estimated---> " << countTreta1corr <<endl;
      }

 fMultipOrCent=countTreta1corr;
 ((TH1F*)fOutputStudy->FindObject("hcorrected_mult"))->Fill(countTreta1corr);
 ((TH2F*)fOutputStudy->FindObject("zvtxVsMult"))->Fill(vtx1->GetZ(),countTreta1);
 
 ((TH2F*)fOutputStudy->FindObject("zvtxVsMultCorr"))->Fill(vtx1->GetZ(),countTreta1corr);
  //=========================end of added part===================================================

  //Pool definition
  //Double_t MultipOrCent = fCorrelatorTr->GetCentrality();
  if(!fMergePools) fPoolNum = fCutsTracks->GetPoolBin(fMultipOrCent, zVtxPosition);
  ...
  if(fReadMC && fUseNtrklWeight) {
    ...
    printf("Using Ntrkl weights, tracklets=%d, Weight=%f\n",nTracklets,fWeight);
  }
...
void AliAnalysisTaskSED0CorrelationsVsMult::FillMassHists(AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliRDHFCutsD0toKpi* cuts, TList *listout, AliAODEvent *aod) {
  //
  ...
  if ((fIsSelectedCandidate==1 || fIsSelectedCandidate==3) && fFillOnlyD0D0bar<2) { //D0
	...
	if(fCutsD0->GetUseCentrality()) centFill = fCutsD0->GetCentrality(aod);
      ...
      // //====================added===========================//
      fillthis="histMass2D_";
      fillthis+=ptbin;
      ((TH2F*)(listout->FindObject(fillthis)))->Fill(invmassD0,fMultipOrCent);
       fillthis="histMass2D_WeigD0Eff_";
       fillthis+=ptbin;
       // Double_t effD0 = fCutsTracks->GetTrigWeight(part->Pt(),MultipOrCent);
       ((TH2F*)(listout->FindObject(fillthis)))->Fill(invmassD0,fMultipOrCent,1./effD0);
  ...
  if (fIsSelectedCandidate>1 && (fFillOnlyD0D0bar==0 || fFillOnlyD0D0bar==2)) { //D0bar
	...
	if(fCutsD0->GetUseCentrality()) centFill = fCutsD0->GetCentrality(aod);
      ...
      fillthis="histMass2D_";
      fillthis+=ptbin;
      ((TH2F*)(listout->FindObject(fillthis)))->Fill(invmassD0bar, fMultipOrCent);
      fillthis="histMass2D_WeigD0Eff_";
      fillthis+=ptbin;
      //  Double_t effD0 = fCutsTracks->GetTrigWeight(part->Pt(),MultipOrCent);
      ((TH2F*)(listout->FindObject(fillthis)))->Fill(invmassD0bar,fMultipOrCent,1./effD0);
    }
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::Terminate(Option_t */*option*/)
{
  // Terminate analysis
  //
...
Int_t AliAnalysisTaskSED0CorrelationsVsMult::CheckD0Origin(TClonesArray* arrayMC, AliAODMCParticle *mcPartCandidate) const {		
  //
  // checking whether the mother of the particles come from a charm or a bottom quark
  //
  printf("AliAnalysisTaskSED0CorrelationsVsMult::CheckD0Origin() \n");
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::CreateCorrelationsObjs() {
//
  ...
  Int_t nBinsPhi[6] = {32,150,(int)(2*fPtAssocLimit+1),3,16,50}; 
  Double_t binMinPhi[6] = {-TMath::Pi()/2.,1.5848,0.,0.,-1.6,0.};  //is the minimum for all the bins
  Double_t binMaxPhi[6] = {3.*TMath::Pi()/2.,2.1848,fPtAssocLimit+0.5,3.,1.6,300.};  //is the maximum for all the bins

  //Vars: DeltaPhi, InvMass, DeltaEta
  Int_t nBinsMix[6] = {32,150,16,(int)(2*fPtAssocLimit+1),2,50};
  Double_t binMinMix[6] = {-TMath::Pi()/2.,1.5848,-1.6,0.,-0.5,0.};  //is the minimum for all the bins
  Double_t binMaxMix[6] = {3.*TMath::Pi()/2.,2.1848,1.6,fPtAssocLimit+0.5,1.5,300.};  //is the maximum for all the bins
  ...
  for(Int_t i=0;i<fNPtBinsCorr;i++) {
    ...
    if(!fMixing) {
     if(!fFillTrees) {
      for(Int_t k=0; k<nPoolForHistos; k++) {    	    
        ...
        THnSparseF *hPhiK = new THnSparseF(namePlot.Data(), "Azimuthal correlation; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
        hPhiK->Sumw2();
        ...
        THnSparseF *hPhiH = new THnSparseF(namePlot.Data(), "Azimuthal correlation; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
        hPhiH->Sumw2();
        ...
        THnSparseF *hPhiC = new THnSparseF(namePlot.Data(), "Azimuthal correlation; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
        hPhiC->Sumw2();
        ...
        if (fReadMC) {
          ...
          THnSparseF *hPhiK_c = new THnSparseF(namePlot.Data(), "Azimuthal correlation - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiK_c->Sumw2();
          ...
          THnSparseF *hPhiH_c = new THnSparseF(namePlot.Data(), "Azimuthal correlation - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiH_c->Sumw2();
          ...
          THnSparseF *hPhiC_c = new THnSparseF(namePlot.Data(), "Azimuthal correlation - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiC_c->Sumw2();
          ...
          THnSparseF *hPhiK_b = new THnSparseF(namePlot.Data(), "Azimuthal correlation - b origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiK_b->Sumw2();
          ...
          THnSparseF *hPhiH_b = new THnSparseF(namePlot.Data(), "Azimuthal correlation - b origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiH_b->Sumw2();
          ...
          THnSparseF *hPhiC_b = new THnSparseF(namePlot.Data(), "Azimuthal correlation - b origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiC_b->Sumw2();
          ...
          THnSparseF *hPhiK_HF_c = new THnSparseF(namePlot.Data(), "Azimuthal correlation HF - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiK_HF_c->Sumw2();
          ...
          THnSparseF *hPhiC_HF_c = new THnSparseF(namePlot.Data(), "Azimuthal correlation HF - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiC_HF_c->Sumw2();
          ...
          THnSparseF *hPhiH_HF_b = new THnSparseF(namePlot.Data(), "Azimuthal correlation HF - b origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiH_HF_b->Sumw2();
          ...
          THnSparseF *hPhiC_HF_b = new THnSparseF(namePlot.Data(), "Azimuthal correlation HF - b origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiC_HF_b->Sumw2();
          ...
          THnSparseF *hPhiK_Non = new THnSparseF(namePlot.Data(), "Azimuthal correlation - Non HF; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiK_Non->Sumw2();
          ...
          THnSparseF *hPhiH_Non = new THnSparseF(namePlot.Data(), "Azimuthal correlation - Non HF; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiH_Non->Sumw2();
          ...
          THnSparseF *hPhiC_Non = new THnSparseF(namePlot.Data(), "Azimuthal correlation - Non HF; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsPhi,binMinPhi,binMaxPhi);
          hPhiC_Non->Sumw2();
      ...
      if(!fSpeed) {
        ...
        THnSparseF *hCorrLead = new THnSparseF(namePlot.Data(), "Leading particle correlations; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
        hCorrLead->Sumw2();
        ...
        if (fReadMC) {
          ...
          THnSparseF *hCorrLead_c = new THnSparseF(namePlot.Data(), "Leading particle correlations - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
          hCorrLead_c->Sumw2();
          ...
          THnSparseF *hCorrLead_b = new THnSparseF(namePlot.Data(), "Leading particle correlations - b origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
          hCorrLead_b->Sumw2();
          ...
          THnSparseF *hCorrLead_HF_c = new THnSparseF(namePlot.Data(), "Leading particle correlations HF - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
          ...
          THnSparseF *hCorrLead_HF_b = new THnSparseF(namePlot.Data(), "Leading particle correlations HF - b origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
          ...
          THnSparseF *hCorrLead_Non = new THnSparseF(namePlot.Data(), "Leading particle correlations - Non HF; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
          hCorrLead_Non->Sumw2();
        ...
        THnSparseF *hCorrWeig = new THnSparseF(namePlot.Data(), "Charged particle correlations (pT weighted); #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
        ...
        if (fReadMC) {
          ...
          THnSparseF *hCorrWeig_c = new THnSparseF(namePlot.Data(), "Charged particle correlations (pT weighted) - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
          fOutputCorr->Add(hCorrWeig_c);
  
          namePlot="hPhi_Weig_From_b_Bin";
          namePlot+=i;
  
          THnSparseF *hCorrWeig_b = new THnSparseF(namePlot.Data(), "Charged particle correlations (pT weighted) - b origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
          fOutputCorr->Add(hCorrWeig_b);
  
          namePlot="hPhi_Weig_HF_From_c_Bin";
          namePlot+=i;
  
          THnSparseF *hCorrWeig_HF_c = new THnSparseF(namePlot.Data(), "Charged particle correlations (pT weighted) HF - c origin; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
          ...
          THnSparseF *hCorrWeig_Non = new THnSparseF(namePlot.Data(), "Charged particle correlations (pT weighted) - Non HF; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
    ...
    if(fMixing && !fFillTrees) {
      for(Int_t k=0; k<nPoolForHistos; k++) {     	    
        ...
        THnSparseF *hPhiK_EvMix = new THnSparseF(namePlot.Data(), "Az. corr. EvMix; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
        hPhiK_EvMix->Sumw2();
        ...
        THnSparseF *hPhiH_EvMix = new THnSparseF(namePlot.Data(), "Az. corr. EvMix; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
        hPhiH_EvMix->Sumw2();
        ...
        THnSparseF *hPhiC_EvMix = new THnSparseF(namePlot.Data(), "Az. corr. EvMix; #Delta#phi; Inv. Mass (GeV/c^{2}); p_{t} (GeV/c)",6,nBinsMix,binMinMix,binMaxMix);
        hPhiC_EvMix->Sumw2();
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::CalculateCorrelations(AliAODRecoDecayHF2Prong* d, Int_t labD0, TClonesArray* mcArray) {
//
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::CalculateCorrelationsMCKine(AliAODMCParticle* d, TClonesArray* mcArray) {
//
// Method for correlations D0-hadrons study
//
  Int_t N_Charg = 0, N_KCharg = 0, N_Kaons = 0;
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::FillSparsePlots(TClonesArray* mcArray, Double_t mInv[], Int_t origD0, Int_t PdgD0, AliReducedParticle* track, Int_t ptbin, Int_t type, Int_t softpiME, Double_t wg) {
  //
  ...
  if(fMixing == kSE) {
    ...
    Double_t fillSpPhiD0[6] = {deltaphi,mD0,ptTrack,d0Track,deltaeta,fMultipOrCent};
    Double_t fillSpPhiD0bar[6] = {deltaphi,mD0bar,ptTrack,d0Track,deltaeta,fMultipOrCent};
    Double_t fillSpWeigD0[6] = {deltaphi,mD0,deltaeta,ptTrack,fMultipOrCent};
    Double_t fillSpWeigD0bar[6] = {deltaphi,mD0bar,deltaeta,ptTrack,fMultipOrCent};
  ...
  if(fMixing == kME) {
    ...
    //variables for filling histos
    Double_t fillSpPhiD0[6] = {deltaphi,mD0,deltaeta,0.4,0,fMultipOrCent}; //dummy for ME threshold! unless explicitly set by flag...
    Double_t fillSpPhiD0bar[6] = {deltaphi,mD0bar,deltaeta,0.4,0,fMultipOrCent};
    if(fMEAxisThresh) {
...
//_________________________________________________________________________________________________
Int_t AliAnalysisTaskSED0CorrelationsVsMult::CheckTrackOrigin(TClonesArray* arrayMC, AliAODMCParticle *mcPartCandidate) const {		
  //
  ...
  // 8) b hadronization
  //
  if(fDebug>2) printf("AliAnalysisTaskSED0CorrelationsVsMult::CheckTrkOrigin() \n");
...
//________________________________________________________________________
Bool_t AliAnalysisTaskSED0CorrelationsVsMult::IsDDaughter(AliAODMCParticle* d, AliAODMCParticle* track, TClonesArray* mcArray) const {
...
//________________________________________________________________________
Int_t AliAnalysisTaskSED0CorrelationsVsMult::PtBinCorr(Double_t pt) const {
  //
...
//---------------------------------------------------------------------------
Bool_t AliAnalysisTaskSED0CorrelationsVsMult::SelectV0(AliAODv0* v0, AliAODVertex *vtx, Int_t opt, Int_t idArrayV0[][2]) const
{
  //
...
//---------------------------------------------------------------------------
Bool_t AliAnalysisTaskSED0CorrelationsVsMult::IsSoftPion_MCKine(AliAODMCParticle* d, AliAODMCParticle* track, TClonesArray* arrayMC) const
{
  //
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::FillTreeD0(AliAODRecoDecayHF2Prong* d, AliAODEvent* aod) {
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::FillTreeTracks(AliAODEvent* aod) {
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::FillTreeD0ForCutOptim(AliAODRecoDecayHF2Prong* d, AliAODEvent* aod) {
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::ResetBranchD() {
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::ResetBranchTracks() {
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::ResetBranchDForCutOptim() {
...
//________________________________________________________________________
Bool_t AliAnalysisTaskSED0CorrelationsVsMult::AcceptTrackForMEOffline(Double_t pt) {
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::FillPurityPlots(TClonesArray* mcArray, AliReducedParticle* track, Int_t ptbin, Double_t deltaphi) {
...
Double_t AliAnalysisTaskSED0CorrelationsVsMult::GetNtrklWeight(Int_t ntrkl){
  //
...
//________________________________________________________________________
void AliAnalysisTaskSED0CorrelationsVsMult::PrintBinsAndLimits() {
