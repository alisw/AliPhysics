...
void RunFeedown_pp_Dstar(Int_t collsyst, Bool_t subtrMCclos, Bool_t oldnames, Int_t centbin, Int_t purityOpt){
...
void GetEnvelopeForEachV2(Int_t collsyst, Bool_t subtrMCclos, Bool_t oldnames, Int_t centbin, Int_t purityOpt){
    ...
    for(Int_t iDpt = dPtMin; iDpt <dPtmax; iDpt++){
        for(Int_t ihadpt = 0; ihadpt <dhadPtmax; ihadpt++){
            ...
            if(collsyst==3) {
              if(iDpt == 0) {Dptbin[0] = 1; Dptbin[1] = 1;}
              if(iDpt == 1) {Dptbin[0] = 2; Dptbin[1] = 3;}
              if(iDpt == 2) {Dptbin[0] = 4; Dptbin[1] = 6;}
              if(iDpt == 3) {Dptbin[0] = 7; Dptbin[1] = 9;}
              if(iDpt == 4) {Dptbin[0] = 10; Dptbin[1] = 10;}
            }            
