...
TString collsyst[ncollsyst]={"pp, #sqrt{s} = 5.02 TeV","pp, #sqrt{s} = 7 TeV","pp, #sqrt{s} = 13 TeV"};
...
Double_t minYaxis[nbinAssocpt]={-0.6,-0.4,-0.29}; // or -0.6 for all  
Double_t maxYaxis[nbinAssocpt]={3.4,1.4,1.6};// or 4.1 for the first , 2.9 for the middle and 1.9 for the last
Double_t mesonptcenter[nbinDpt]={4.,6.5,12.};// needed for getting the baseline
Color_t colourSystem[ncollsyst]={kBlue,kRed,kGreen+2}; 
Color_t colourSystemBaseUnc[ncollsyst]={kBlue,kRed,kGreen+2};//was {kBlack,kRed-7};
Int_t fillColourBaselineStyle=1001;
Int_t markerstyle[3]={20,21,22};// pp, p-Pb
Double_t innerPadHeight;// not touch, set internally
Double_t innerPadWidth;// not touch, set internally
Int_t rebin =1;
Double_t **ptbinsD;
Int_t ndesiredbins=3;
TString inputdatadirectory[ncollsyst] = {"./pp_5TeV","./pp_7TeV","./pp_13TeV"};
TString baselinedirectory[ncollsyst]={"./pp_5TeV/FitResults/Trends_pp","./pp_7TeV/FitResults/Trends_pp","./pp_13TeV/FitResults/Trends_pp"};
...
void SetBaselineDirectory(TString dirbase1,TString dirbase2,TString dirbase3){
  baselinedirectory[0]=dirbase1;
  baselinedirectory[1]=dirbase2;
  baselinedirectory[2]=dirbase3;
}

void SetInputDataDirectory(TString inputdir1,TString inputdir2,TString inputdir3){
  inputdatadirectory[0]=inputdir1;
  inputdatadirectory[1]=inputdir2;
  inputdatadirectory[2]=inputdir3;
}
...
/*
  if(collsyst==0){
    if(TMath::Abs(hUncCorrMin->GetBinContent(1)-hUncCorrMax->GetBinContent(1))<0.001)tUncertainty=new TLatex(0.55,0.49,Form("%.0f#% scale uncertainty (pp, 5 TeV)",hUncCorrMin->GetBinContent(1)*100.));
    else tUncertainty=new TLatex(0.55,0.49,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc. (pp, 5 TeV)","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==1){
    if(TMath::Abs(hUncCorrMin->GetBinContent(1)-hUncCorrMax->GetBinContent(1))<0.001)tUncertainty=new TLatex(0.55,0.45,Form("#bf{%.0f#% scale uncertainty (pp, 7 TeV)}",hUncCorrMin->GetBinContent(1)*100.));
    else tUncertainty=new TLatex(0.55,0.45,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc. (pp, 7 TeV)","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==2){
    if(TMath::Abs(hUncCorrMin->GetBinContent(1)-hUncCorrMax->GetBinContent(1))<0.001)tUncertainty=new TLatex(0.55,0.45,Form("#bf{%.0f#% scale uncertainty (pp, 13 TeV)}",hUncCorrMin->GetBinContent(1)*100.));
    else tUncertainty=new TLatex(0.55,0.45,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc. (pp, 13 TeV)","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }  
*/
  if(collsyst==0){
    if(TMath::Abs(hUncCorrMin->GetBinContent(1)-hUncCorrMax->GetBinContent(1))<0.001)tUncertainty=new TLatex(0.49,0.49,Form("%.0f#% scale uncertainty (pp, 5 TeV)",hUncCorrMin->GetBinContent(1)*100.));
    else tUncertainty=new TLatex(0.35,0.49,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} ","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==1){
    if(TMath::Abs(hUncCorrMin->GetBinContent(1)-hUncCorrMax->GetBinContent(1))<0.001)tUncertainty=new TLatex(0.53,0.49,Form("#bf{%.0f#% scale uncertainty (pp, 7 TeV)}",hUncCorrMin->GetBinContent(1)*100.));
    else tUncertainty=new TLatex(0.43,0.49,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} ","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==2){
    if(TMath::Abs(hUncCorrMin->GetBinContent(1)-hUncCorrMax->GetBinContent(1))<0.001)tUncertainty=new TLatex(0.57,0.49,Form("#bf{%.0f#% scale uncertainty (pp, 13 TeV)}",hUncCorrMin->GetBinContent(1)*100.));
    else tUncertainty=new TLatex(0.53,0.49,Form("{}^{#plus%.0f%s}_{#minus%.0f%s}","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));

  }  

  tUncertainty->SetNDC();
  tUncertainty->SetTextSize(0.027);
  tUncertainty->SetTextFont(62);
  tUncertainty->SetTextColor(colourSystem[collsyst]);
...
void SetScaleUncertaintyPositionAndSize(TLatex *tl1,TLatex *tl2,TLatex *tl3, TLatex *scale, Int_t jDpt, Int_t iassoc){
  tl1->SetNDC();
  tl2->SetNDC();
  tl3->SetNDC();
  scale->SetNDC();
  ...
  tl2->SetTextFont(43);
  tl3->SetTextFont(43);
  scale->SetTextFont(43);

  //  tlpp->SetX(0.13/gPad->GetWNDC()+gPad->GetLeftMargin()); //draft 1
  //   tlpp->SetY(0.19/gPad->GetHNDC()+gPad->GetBottomMargin());
 /* tl1->SetX(0.07/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl1->SetY(0.8);

  //  tlpPb->SetX(0.13/gPad->GetWNDC()+gPad->GetLeftMargin()); // draft 1
  //  tlpPb->SetY(0.156/gPad->GetHNDC()+gPad->GetBottomMargin());
  tl2->SetX(0.07/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl2->SetY(tl1->GetY()-0.1);

  tl3->SetX(0.07/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl3->SetY(tl1->GetY()-0.2);

  scale->SetX(0.07/gPad->GetWNDC()+gPad->GetLeftMargin());
  scale->SetY(tl1->GetY()-0.2);*/

  tl1->SetTextSize(29*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  tl2->SetTextSize(29*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  tl3->SetTextSize(29*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  scale->SetTextSize(29*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  tl1->SetTextAlign(12);
  tl2->SetTextAlign(12);
  tl3->SetTextAlign(12);
  scale->SetTextAlign(12);

  if(jDpt>0) {
    tl1->SetX(tl1->GetX()-0.18);
    tl2->SetX(tl2->GetX()-0.18);
    tl3->SetX(tl3->GetX()-0.18);
    scale->SetX(scale->GetX()-0.18);
  }

  if(iassoc==1) {
    tl1->SetY(tl1->GetY()+0.2);
    tl2->SetY(tl2->GetY()+0.2);
    tl3->SetY(tl3->GetY()+0.2);
    scale->SetY(scale->GetY()+0.2);
  }

  if(iassoc==2) {
    tl1->SetY(tl1->GetY()+0.3);
    tl2->SetY(tl2->GetY()+0.3);
    tl3->SetY(tl3->GetY()+0.3);
    scale->SetY(scale->GetY()+0.3);
  }  
...
TLegend *GetLegendDataPoints(TH1D *h1,TH1D *h2,TH1D *h3,Int_t identifier){
    
  TLegend * legend = new TLegend(0.03/gPad->GetWNDC()+gPad->GetLeftMargin(),0.18/gPad->GetHNDC()+gPad->GetBottomMargin(),0.2/gPad->GetWNDC()+gPad->GetLeftMargin(),0.245/gPad->GetHNDC()+gPad->GetBottomMargin());
  ...
  //previous settings: (0.011/gPad->GetWNDC()+gPad->GetLeftMargin(),0.16/gPad->GetHNDC()+gPad->GetBottomMargin(),0.2/gPad->GetWNDC()+gPad->GetLeftMargin(),0.22/gPad->GetHNDC()+gPad->GetBottomMargin());
    ...
    legend->AddEntry(h2,"pp, #sqrt{s} = 7 TeV, EPJ C (2017) 7, 245","lep");
    legend->AddEntry(h3,"pp, #sqrt{s} = 13 TeV","lep");
...
TLegend *GetLegendDataPointsFake(TH1D *hpp,TH1D *hpPb,Int_t identifier){
  ...
  TLegend * legend = new TLegend(0.03/gPad->GetWNDC()+gPad->GetLeftMargin(),0.185/gPad->GetHNDC()+gPad->GetBottomMargin(),0.2/gPad->GetWNDC()+gPad->GetLeftMargin(),0.25/gPad->GetHNDC()+gPad->GetBottomMargin());
    //new TLegend(0.011/gPad->GetWNDC()+gPad->GetLeftMargin(),0.14/gPad->GetHNDC()+gPad->GetBottomMargin(),0.2/gPad->GetWNDC()+gPad->GetLeftMargin(),0.2/gPad->GetHNDC()+gPad->GetBottomMargin());
  //previous settings: (0.011/gPad->GetWNDC()+gPad->GetLeftMargin(),0.16/gPad->GetHNDC()+gPad->GetBottomMargin(),0.2/gPad->GetWNDC()+gPad->GetLeftMargin(),0.22/gPad->GetHNDC()+gPad->GetBottomMargin());
    legend->SetFillColor(0);
    legend->SetFillStyle(0);
...
TLegend *GetLegendBaselines(TGraphAsymmErrors *g1,TGraphAsymmErrors *g2,TGraphAsymmErrors *g3,Int_t identifier,Int_t positionOption=1){
  TLegend * legend;
  if(positionOption==0){
    ...
    legend= new TLegend(0.025/gPad->GetWNDC()+gPad->GetLeftMargin(),0.185/gPad->GetHNDC()+gPad->GetBottomMargin(),0.155/gPad->GetWNDC()+gPad->GetLeftMargin(),0.25/gPad->GetHNDC()+gPad->GetBottomMargin());
  }
  else return 0x0;
  legend->SetFillColor(0);
  legend->SetFillStyle(0);
  ...
  legend->AddEntry(g1,"baseline-subtraction unc. (pp, 5 TeV)","f");
  legend->AddEntry(g2,"baseline-subtraction unc. (pp, 7 TeV)","f");
  legend->AddEntry(g3,"baseline-subtraction unc. (pp, 13 TeV)","f");
...
TPaveText *GetALICEpavetext(Int_t identifier){
  TPaveText *alice = new TPaveText(0.0004/gPad->GetWNDC()+gPad->GetLeftMargin(),0.255/gPad->GetHNDC()+gPad->GetBottomMargin(),0.37/gPad->GetWNDC()+gPad->GetLeftMargin(),0.275/gPad->GetHNDC()+gPad->GetBottomMargin(),"NDC");
...
TPaveText *GetAveragepavetext(Int_t identifier){
  TPaveText* pvAverage = new TPaveText(0.004/gPad->GetWNDC()+gPad->GetLeftMargin(),0.205/gPad->GetHNDC()+gPad->GetBottomMargin(),0.3/gPad->GetWNDC()+gPad->GetLeftMargin(),0.275/gPad->GetHNDC()+gPad->GetBottomMargin(),"NDC");
...
void DoComparison_Distributions(){
  ...
  for(Int_t iassoc=0;iassoc<nbinAssocpt;iassoc++){
    for(Int_t jDpt=firstDpt;jDpt<nbinDpt;jDpt++){
      ...
      endline=subtractedhisto[1][iassoc][jDpt]->GetBinLowEdge(subtractedhisto[1][iassoc][jDpt]->GetNbinsX())+wi+0.35;// 0.15
...
*/
	...
	h->GetXaxis()->SetLabelSize(0);
      ...
      TLatex *scalelab = new TLatex(0.62,0.486,"scale uncertainty");
      SetScaleUncertaintyPositionAndSize(ltscale[0][iassoc][jDpt],ltscale[1][iassoc][jDpt],ltscale[2][iassoc][jDpt],scalelab,jDpt,iassoc);
   /*   
...
*/
      ...
      scalelab->Draw();//commented
      Printf("Getting data legend");
      TLegend *legend=GetLegendDataPoints(histo[0][iassoc][firstDpt],histo[1][iassoc][firstDpt],histo[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1);
      ...
      TLegend *legend2;
      if(style==0)legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,0);
      else if(style==1)legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,3); // default option was 1 in older settings; 2 for draft 1 version;
      else legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,1);
      ...
      if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==1){
        ...
        pvKineInfo->SetY2(0.225/gPad->GetHNDC()+gPad->GetBottomMargin());
        ...
        pvKineInfo->SetY1(0.175/gPad->GetHNDC()+gPad->GetBottomMargin());
        pvKineInfo->SetY2(0.20/gPad->GetHNDC()+gPad->GetBottomMargin());
        pvKineInfo->SetX1(0.01/gPad->GetWNDC()+gPad->GetLeftMargin());
	...
	pvKineInfo->Draw("same");
      ...
      pvKineInfo->SetY1(pvKineInfo->GetY1()+0.045);
      pvKineInfo->SetY2(pvKineInfo->GetY2()+0.045);
  ...
  TString outdir = Form("%s/ScriptOutput/ReflectedPlots/StdRebin/AllPlots/Averages/ComparisonToOtherpp",basicdir.Data());
  ...
  cFinalPaperStyle->SaveAs(Form("%s/plotComparison_%sAverage_pp_vsEnergies_UniqueCanvas_Style%d.root",outdir.Data(),avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("%s/plotComparison_%sAverage_pp_vsEnergies_UniqueCanvas_Style%d.png",outdir.Data(),avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("%s/plotComparison_%sAverage_pp_vsEnergies_UniqueCanvas_Style%d.pdf",outdir.Data(),avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("%s/plotComparison_%sAverage_pp_vsEnergies_UniqueCanvas_Style%d.eps",outdir.Data(),avType.Data(),style));
