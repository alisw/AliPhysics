...
declare templateDirPP="/home/colamaria/Scrivania/Codici_Ausiliari_Dh/Dhadron_Final_Output_pp13TeV_001/Inputs/Templates_pp"
declare -a templateDirSystemSuffix=( "none" "none" "none" "none" "none" ) #### THIS IS KEPT JUST FOR BACKWARD COMPATIBILITY WITH OLD TEMPLATES! NO NEED TO TOUCH IT UNLESS YOU WANT TO USE OLD TEMPLATES
declare -a templateDir=( "$templateDirPP" "$templateDirPP" "$templateDirPP" "$templateDirPP" "$templateDirPP")

###these for loading beta values from MC, in case of fixed beta approach!
declare betaTemplDirPP="/home/colamaria/Scrivania/Codici_Ausiliari_Dh/Dhadron_Final_Output_pp13TeV_001/Inputs/Templates_pp/FitResults/Trends_pp/PYTHIA8"
declare -a betaTemplDir=( "$betaTemplDirPP" "$betaTemplDirPP" "$betaTemplDirPP" "$betaTemplDirPP" "$betaTemplDirPP")

### the following is needed for hte comparison to MC (as well as MC fitting)
declare -a Nmccase=( 6 6 6 8 0 ) #warning! If you change this, the number of fitted MC models will change (and also their order!) I suggest not changing it but only the array below...
declare -a mccasePP=( 0 0 0 0 0 0 0 0 ) # according to CompareFitResults array: Perugia0, Perugia2010, Perugia2011, PYTHIA8, HERWIG, POHWEG+Perugia2011, POWHEG+Perugia2011 with EPS09, EPOS 3
...
declare puritytemplateDirpp="/home/colamaria/Scrivania/Codici_Ausiliari_Dh/Dhadron_Final_Output_pp13TeV_001/Inputs/PurityTemplates_pp"
declare -a puritytemplateDir=( "$puritytemplateDirpp" "$puritytemplateDirpp" "$puritytemplateDirpp" "$puritytemplateDirpp" "$puritytemplateDirpp")

########## THE FOLLOWING DIRECTORIES SHOULD CONTAIN THE RESULTS BEFORE FD SUBTRACTION #####
declare dirppDzeroNotFDsubt="/home/colamaria/Scrivania/Codici_Ausiliari_Dh/Dhadron_Final_Output_pp13TeV_001/Inputs/Dzero_pp"
declare -a dirDzeroNotFDsubt=( "$dirppDzeroNotFDsubt" "$dirppDzeroNotFDsubt" "$dirppDzeroNotFDsubt" "$dirppDzeroNotFDsubt" "$dirppDzeroNotFDsubt")
declare -a fpromptfileDzero=( "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb_001.root")
declare -a filerootDzero=( "NONE" "NONE" "NONE" "NONE" "AzimCorrDistr_Dzero_Canvas_PtIntBins" )

declare dirppDstarNotFDsubt="/home/colamaria/Scrivania/Codici_Ausiliari_Dh/Dhadron_Final_Output_pp13TeV_001/Inputs/Dstar_pp"
declare -a dirDstarNotFDsubt=( "$dirppDstarNotFDsubt" "$dirppDstarNotFDsubt" "$dirppDstarNotFDsubt" "$dirppDstarNotFDsubt" "$dirppDstarNotFDsubt")
declare -a fpromptfileDstar=( "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb_001.root")
declare -a filerootDstar=( "NONE" "NONE" "NONE" "NONE" "AzimCorrDistr_Dstar_Canvas_PtIntBins" )

declare dirppDplusNotFDsubt="/home/colamaria/Scrivania/Codici_Ausiliari_Dh/Dhadron_Final_Output_pp13TeV_001/Inputs/Dplus_pp"
declare -a dirDplusNotFDsubt=( "$dirppDplusNotFDsubt" "$dirppDplusNotFDsubt" "$dirppDplusNotFDsubt" "$dirppDplusNotFDsubt" "$dirppDplusNotFDsubt")
declare -a fpromptfileDplus=( "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb.root" "HFPtSpectrum_Nb_001.root")
declare -a filerootDplus=( "NONE" "NONE" "NONE" "NONE" "AzimCorrDistr_Dplus_Canvas_PtIntBins" )
...
declare averageOpt=1  #0 = weighted, 1=arithmetic (BE AWARE THAT SETTINGS FOR FINAL STYLE PLOTS MIGHT NOT BE OK FOR ARITHMETIC AVERAGE AND THEY MIGHT USE PATHS EXPECTED FOR WEIGHTING AV)
declare averageUncOnlyOneMeson=1  #0 = standard procedure; 1 = use if have only 1 meson and the others are just placeholders! (e.g. pp13TeV vs cent, D0 only)
declare -a includev2=( 0 0 1 0 1 ) 
...
declare doCompareWithMCPP2017=0 #For pp 2017 @ 5 TeV prels
declare doFitResultComparisonPPtoMC2017=0 #For pp 2017 @ 5 TeV prels
declare doFitResultComparisonPPtoMC2017awayside=0 #For pp 2017 @ 5 TeV prels
#pp 13 TeV vs mult
...
declare -i firstcollsyst=4   #default 0
declare -i lastcollsyst=4   #default 1 
declare -i firstmeson=0     #default 0
declare -i lastmeson=2      #default 2

declare -i firstassocbin=0  #default 0
declare -i lastassocbin=4   #default 2
declare -i firsttrigbin=0   #default 0, 0=3-5, 1=5-8, 2=8-16, 3=16-24 (as in the past)
declare -i lasttrigbin=3    #default 2

declare -i centralitybin=1  #default 0, use always 0 for cent-integrated cases! then, 1 is 0-0.1%, 2 is 0.1-10%, 3 is 10-30%, 4 is 30%-100%
...
declare -a ptassoc=("0.3to1.0" "0.3to99.0" "1.0to2.0" "2.0to3.0" "3.0to99.0")
declare -a collsystdir=("pp" "pPb" "pPb2016" "pp2017" "pp13TeV")
...
if [ ${doAverage} = 1 ]; then
	...
	while [ ${itrigbin} -le ${lasttrigbin} ]; do
		...
		$HFCJlocalCodeDir/DoAverages.sh ${collsyst} ${itrigbin} ${iassocbin} ${averageOpt} ${reflect} 5 ${baseDir}/AllPlots/ ${baseDir}/AllPlots/ ${baseDir}/AllPlots/ 1 ${centralitybin} ${averageUncOnlyOneMeson}
		iassocbin=${iassocbin}+1
		done
	    itrigbin=${itrigbin}+1
	    done	    
