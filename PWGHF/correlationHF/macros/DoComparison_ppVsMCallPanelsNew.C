...
TString strsyst="pp";
TString sets[nSets]={"pp","Perugia0","Perugia2010","Perugia2011","POWHEG","PYTHIA8","EPOS3"};
...
Color_t modelColors[nmodels]={kMagenta+1,kGreen+2,kBlue,kRed+2,kCyan,kOrange,kViolet};
Int_t modelMarkerStyle[nmodels]={kOpenSquare,kOpenCircle,kOpenDiamond,3,28,26,22};
...
TPaveText *GetPaveKineInfo(Int_t iPtMeson,Int_t iPtAssoc,Int_t identifier){
  ...
  // cout<<" ********* PAD info -> left m="<< gPad->GetLeftMargin()<<"  WNDC="<<gPad->GetWNDC()<<"  HNDC="<<gPad->GetHNDC()<<endl;
   Int_t binrange=3*iPtAssoc+iPtMeson;
   ...
   if(binrange==3 || binrange==4 || binrange==5)pvKineInfo = new TPaveText(0.004/gPad->GetWNDC()+gPad->GetLeftMargin(),0.245/gPad->GetHNDC()+gPad->GetBottomMargin(),0.15/gPad->GetWNDC()+gPad->GetLeftMargin(),0.27/gPad->GetHNDC()+gPad->GetBottomMargin(),"NDC");
  
   if(binrange>=6)pvKineInfo = new TPaveText(0.02/gPad->GetWNDC()+gPad->GetLeftMargin(),0.245/gPad->GetHNDC()+gPad->GetBottomMargin(),0.19/gPad->GetWNDC()+gPad->GetLeftMargin(),0.27/gPad->GetHNDC()+gPad->GetBottomMargin(),"NDC");
   ...
   pvKineInfo->SetTextSize(19.77*innerPadHeight/referencePadHeight*resizeTextFactor);// settings for font 42: 0.07/(gPad->GetHNDC())*scaleHeight
