...
TString collsyst[ncollsyst]={"pp, #sqrt{s} = 5.02 TeV"};
TString pthadron[nbinAssocpt]={"1.0to99.0","1.0to2.0","2.0to3.0"};
...
Double_t minYaxis[nbinAssocpt]={-0.6,-0.3,-0.2}; // or -0.6 for all  
Double_t maxYaxis[nbinAssocpt]={3.4,1.4,1.1};// or 4.1 for the first , 2.9 for the middle and 1.9 for the last
Double_t mesonptcenter[nbinDpt]={4.,6.5,12.,20.};// needed for getting the baseline
Color_t colourSystem[ncollsyst]={kBlack}; 
Color_t colourSystemBaseUnc[ncollsyst]={kGray+2};//was {kBlack,kRed-7};
Int_t fillColourBaselineStyle=1001;
Int_t markerstyle[1]={20};// pp, p-Pb
Double_t innerPadHeight;// not touch, set internally
Double_t innerPadWidth;// not touch, set internally
Int_t rebin =1;
Double_t **ptbinsD;
Int_t ndesiredbins=3;
TString inputdatadirectory[ncollsyst] = {"./ScriptOutput/ReflectedPlots/StdRebin/AllPlots/Averages"};
TString baselinedirectory[ncollsyst]={"./ScriptOutput/ReflectedPlots/StdRebin/AllPlots/Averages/FitResults/Trends_pp"};
...
void SetBaselineDirectory(TString dirbase){
  baselinedirectory[0]=dirbase;
}

void SetInputDataDirectory(TString inputdir){
  inputdatadirectory[0]=inputdir;
}
...
TH1D * GetHistoAndSyst(TString path, Int_t collsyst,TString hname, TString hnamesyst,TGraphAsymmErrors *&gr2, TLatex *&tUncertainty){
  ...
  if(collsyst==0){
    ...
    else tUncertainty=new TLatex(0.55,0.49,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc.","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
...
void SetScaleUncertaintyPositionAndSize(TLatex *tl1){
  tl1->SetTextFont(43);
  tl1->SetX(0.077/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl1->SetY(0.22/gPad->GetHNDC()+gPad->GetBottomMargin());
  tl1->SetTextSize(27*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  tl1->SetTextAlign(12);
  return;
}

TLegend *GetLegendDataPoints(TH1D *h1,Int_t identifier){
    
  TLegend * legend = new TLegend(0.035/gPad->GetWNDC()+gPad->GetLeftMargin(),0.165/gPad->GetHNDC()+gPad->GetBottomMargin(),0.155/gPad->GetWNDC()+gPad->GetLeftMargin(),0.195/gPad->GetHNDC()+gPad->GetBottomMargin());
...
TLegend *GetLegendDataPointsFake(TH1D *hpp,Int_t identifier){
...
TLegend *GetLegendBaselines(TGraphAsymmErrors *g1,Int_t identifier,Int_t positionOption=1){
  TLegend * legend;
  if(positionOption==0){
    ...
    legend= new TLegend(0.035/gPad->GetWNDC()+gPad->GetLeftMargin(),0.135/gPad->GetHNDC()+gPad->GetBottomMargin(),0.155/gPad->GetWNDC()+gPad->GetLeftMargin(),0.165/gPad->GetHNDC()+gPad->GetBottomMargin());
  }
  else return 0x0;
  legend->SetFillColor(0);
  legend->SetFillStyle(0);
  ...
  legend->AddEntry(g1,"baseline-subtraction uncertainty","f");
...
void DrawSinglePanel_CorrelationDistributions_2(){
  ...
  for(Int_t iassoc=0;iassoc<nbinAssocpt;iassoc++){
    for(Int_t jDpt=firstDpt;jDpt<nbinDpt;jDpt++){
      ...
      endline=subtractedhisto[0][iassoc][jDpt]->GetBinLowEdge(subtractedhisto[0][iassoc][jDpt]->GetNbinsX())+wi+0.15;// 0.15
...
*/
  ...
  h->GetXaxis()->SetLabelSize(0);
      ...
      SetScaleUncertaintyPositionAndSize(ltscale[0][iassoc][jDpt]);
      ...
      TLegend *legend=GetLegendDataPoints(histo[0][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1);
      ...
      TLegend *legend2;
      if(style==0)legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,0);
      else if(style==1)legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,3); // default option was 1 in older settings; 2 for draft 1 version;
      else legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,1);
  ...
  Printf("Getting Pave Kine infos (style %d)",style);
  if(style==0){
  ...
  TString outdir = Form("%s/ScriptOutput/ReflectedPlots/StdRebin/AllPlots/Averages/MultiPanelAverages",basicdir.Data());
  ...
  cFinalPaperStyle->SaveAs(Form("%s/DrawSinglePanel_CorrelationDistributions_%sAverage_2.root",outdir.Data(),avType.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/DrawSinglePanel_CorrelationDistributions_%sAverage_2.png",outdir.Data(),avType.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/DrawSinglePanel_CorrelationDistributions_%sAverage_2.pdf",outdir.Data(),avType.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/DrawSinglePanel_CorrelationDistributions_%sAverage_2.eps",outdir.Data(),avType.Data()));
