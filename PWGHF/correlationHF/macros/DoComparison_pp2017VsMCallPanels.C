...
TString strsyst="pp";
TString sets[nSets]={"pp","Perugia0","Perugia2010","Perugia2011","PYTHIA8","HERWIG","POWHEG"};
TString setsBoost[nSets]={"pp","Perugia0","Perugia2010","Perugia2011","PYTHIA8","HERWIG","POWHEG"};
...
Color_t modelColors[nmodels]={kRed+2,kCyan,kGreen+2,kMagenta+1,kOrange+1,kBlue,kViolet};
...
Bool_t isReflectedData=kTRUE;
void SetSkip3to5pp(Bool_t skip){
  skip3to5=skip;
}
...
void DoComparison_pp2017VsMCallPanels(){
	...
	for(Int_t jmes=0;jmes<nbinDpt;jmes++){
	  ...
	  if(iset==0){
	    filenames[iset][kassoc][jmes]=Form("%s/%sAverage%sDzeroDstarDplus%s_assoc%s.root",inputdatadirectory.Data(),avType.Data(),sets[iset].Data(),strmesonpt[jmes].Data(),pthadron[kassoc].Data());//pp data
          ...
          else if(iset>=1 && iset<=5){ //add "wBoost"
...
TPaveText *GetAveragepavetext(Int_t identifier){
  ...
  pvAverage->SetTextSize(23*innerPadHeight/referencePadHeight*resizeTextFactor);
...
TPaveText *  pvKineInfo = new TPaveText(0.005/gPad->GetWNDC()+gPad->GetLeftMargin(),0.075/gPad->GetHNDC()+gPad->GetBottomMargin(),0.19/gPad->GetWNDC()+gPad->GetLeftMargin(),0.21/gPad->GetHNDC()+gPad->GetBottomMargin(),"NDC");
  ...
  pvKineInfo->SetTextFont(43);
  pvKineInfo->SetTextSize(22*innerPadHeight/referencePadHeight*resizeTextFactor);// settings for font 42: 0.07/(gPad->GetHNDC())*scaleHeight
...
TLegend *GetLegendData(TH1D *hpp,Int_t identifier){
  ...
  // TLegend * legend = new TLegend(0.011/gPad->GetWNDC()+gPad->GetLeftMargin(),0.13/gPad->GetHNDC()+gPad->GetBottomMargin(),0.2/gPad->GetWNDC()+gPad->GetLeftMargin(),0.18/gPad->GetHNDC()+gPad->GetBottomMargin());
   TLegend * legend = new TLegend(0.34,0.48,0.6,0.57,NULL,"brNDC");
...
//   else legend= new TLegend(0.1,0.37,0.7,0.68,NULL,"brNDC");
   ...
   //  TLegend * legend = new TLegend(0.25,0.44,0.79,0.84,NULL,"brNDC");
    ...
    if(hMC5)legend->AddEntry(hMC5,"HERWIG","l");
    if(hMC6)legend->AddEntry(hMC6,"POWHEG+PYTHIA6","l");
...
void DoComparison_pp2017VsMCSinglePanel(){
