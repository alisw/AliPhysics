...
TString strPtAss[3]={"1.0to99.0","1.0to2.0","2.0to3.0"};
TString strPtAssCanvas[3]={"#it{p}_{T}^{assoc} > 1 GeV/#it{c}","1 < #it{p}_{T}^{assoc} < 2 GeV/#it{c}","2 < #it{p}_{T}^{assoc} < 3 GeV/#it{c}"};
...
TH1D *GetAndPreparePP(Int_t binass,Int_t quantity,Int_t numsyst,TGraphAsymmErrors *&gr){
...
TH1D *GetAndPreparePPb(Int_t binass,Int_t quantity,Int_t numsyst,TGraphAsymmErrors *&gr,TGraphAsymmErrors *&grV2){
  ...
  if(numsyst==1 && plotv2unc==kTRUE) {
      ...
      for(Int_t iPoint=0;iPoint<5;iPoint++) grV2->SetPointError(iPoint,0.7*grV2->GetErrorXlow(iPoint),0.7*grV2->GetErrorXhigh(iPoint),grV2->GetErrorYlow(iPoint),grV2->GetErrorYhigh(iPoint));
  }
...
TCanvas* Compare(Int_t binass,Int_t quantity,TPad *pd,Int_t textlegendOptions){
  ...
  TH1D *hPPb_1=GetAndPreparePP(binass,quantity,0,grPPb_1);
  
  printf("Preparing p-Pb...(%d)\n",quantity);
  TGraphAsymmErrors *grPPb_2;
  TGraphAsymmErrors *grV2;
  TH1D *hPPb_2=GetAndPreparePPb(binass,quantity,1,grPPb_2,grV2);
...
/****************************************/
//REMOVE AD HOC POINTS WITH LARGE ERRORS!! -> nothing shall be removed in higher pT ranges!
/****************************************/
/*  printf("binass = %d, quantity = %d\n",binass,quantity);
  if((binass==1 && quantity==3) || (binass==1 && quantity==4)) { 
...
void CompareFitResults_UniqueCanvas(){
  ...
  cFinalPaperStyle->Modified();
  cFinalPaperStyle->Update();
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_ppVspPb_5TeV_2.root",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_ppVspPb_5TeV_2.eps",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_ppVspPb_5TeV_2.png",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_ppVspPb_5TeV_2.pdf",outdir.Data()));
...
void CompareFitResults_UniqueCanvas_AwaySide(){
  ...
  cFinalPaperStyle->Modified();
  cFinalPaperStyle->Update();
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_ppVspPb_5TeV_AwaySide_2.root",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_ppVspPb_5TeV_AwaySide_2.eps",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_ppVspPb_5TeV_AwaySide_2.png",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_ppVspPb_5TeV_AwaySide_2.pdf",outdir.Data()));
