...
Double_t markersize[3]={1.*resizeTextFactor,1.*resizeTextFactor,1.7*resizeTextFactor};
Int_t style=1;
// FOR ELENA: HERE THERE ARE JUST STRINGS AND NAMES/LEGENDS
TString collsyst[ncollsyst]={"020","2060","60100"};
...
TString strDeltaEta="-0.96 < #it{y}^{D}_{cms} < 0.04, |#Delta#eta| < 1";
Double_t minYaxis[nbinAssocpt]={-0.6,-0.6,-0.29}; // or -0.6 for all  
Double_t maxYaxis[nbinAssocpt]={4.0,1.4,1.6};// or 4.1 for the first , 2.9 for the middle and 1.9 for the last
...
Int_t markerstyle[3]={20,21,33};// pp, p-Pb
Double_t innerPadHeight;// not touch, set internally
Double_t innerPadWidth;// not touch, set internally
Int_t rebin =1;
Double_t **ptbinsD;
Int_t ndesiredbins=3;
TString inputdatadirectory[ncollsyst] = {"./020","./2060","./60100"};
TString baselinedirectory[ncollsyst]={"./020","./2060","./60100"};
...
void LoadFileNamesppVspPb(){
  ...
  for(Int_t icoll=0;icoll<ncollsyst;icoll++){
	...
	filenames[icoll][kassoc][jmes]=Form("%s/%sAveragepPbDzeroDstarDplus%s_assoc%s.root",inputdatadirectory[icoll].Data(),avType.Data(),strmesonpt[jmes].Data(),pthadron[kassoc].Data());
      }
        pedestalfilenames[icoll][kassoc]=Form("%s/Trends_pPb/CanvasBaselineVariationTrendPedestal_pthad%s.root",baselinedirectory[icoll].Data(),pthadron[kassoc].Data());
    }
  }
}
...
TH1D * GetHistoAndSyst(TString path, Int_t collsyst,TString hname, TString hnamesyst,TGraphAsymmErrors *&gr2, TLatex *&tUncertainty){
  ...
  if(collsyst==0){
    ...
    else tUncertainty=new TLatex(0.52,0.49,Form("#color[4]{{}^{#plus%.0f%s}_{#minus%.0f%s}}","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==1){
    ...
    else tUncertainty=new TLatex(0.55,0.49,Form("#color[2]{{}^{#plus%.0f%s}_{#minus%.0f%s}}","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==2){
    ...
    else tUncertainty=new TLatex(0.58,0.49,Form("#color[418]{{}^{#plus%.0f%s}_{#minus%.0f%s}} scale uncertainty","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==3){
    ...
    else tUncertainty=new TLatex(0.55,0.46,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc. (0-100%)","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }  
...
void SetScaleUncertaintyPositionAndSize(TLatex *tl1,TLatex *tl2,TLatex *tl3){
  tl1->SetTextFont(43);
  tl2->SetTextFont(43);
  tl3->SetTextFont(43);
  /* old settings
  ...
  */
  ...
  //   tlpp->SetY(0.19/gPad->GetHNDC()+gPad->GetBottomMargin());
  tl1->SetX(0.078/gPad->GetWNDC()+gPad->GetLeftMargin());
  ...
  tl2->SetY(0.22/gPad->GetHNDC()+gPad->GetBottomMargin());

  tl3->SetX(0.088/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl3->SetY(0.22/gPad->GetHNDC()+gPad->GetBottomMargin());
...
TLegend *GetLegendDataPoints(TH1D *h1,TH1D *h2,TH1D *h3,Int_t identifier){
  ...
  //previous settings: (0.011/gPad->GetWNDC()+gPad->GetLeftMargin(),0.16/gPad->GetHNDC()+gPad->GetBottomMargin(),0.2/gPad->GetWNDC()+gPad->GetLeftMargin(),0.22/gPad->GetHNDC()+gPad->GetBottomMargin());
    ...
    legend->SetTextSize(32*innerPadHeight/referencePadHeight*resizeTextFactor);
			// old settings with font 42, still good (0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
    legend->AddEntry(h1,"0-20% ZNA","lep");
    legend->AddEntry(h2,"20-60% ZNA","lep");
    legend->AddEntry(h3,"60-100% ZNA","lep");
...
TLegend *GetLegendBaselinesA(TGraphAsymmErrors *g1,TGraphAsymmErrors *g2,TGraphAsymmErrors *g3,Int_t identifier,Int_t positionOption=1){
  TLegend * legend;
  if(positionOption==0){
    ...
    legend= new TLegend(0.005/gPad->GetWNDC()+gPad->GetLeftMargin(),0.19/gPad->GetHNDC()+gPad->GetBottomMargin(),0.125/gPad->GetWNDC()+gPad->GetLeftMargin(),0.235/gPad->GetHNDC()+gPad->GetBottomMargin());
  }
  else return 0x0;
  legend->SetFillColor(0);
  legend->SetFillStyle(0);
...
TLegend *GetLegendBaselinesB(TGraphAsymmErrors *g1,TGraphAsymmErrors *g2,TGraphAsymmErrors *g3,Int_t identifier,Int_t positionOption=1){
  TLegend * legend;
  if(positionOption==0){
    legend=new TLegend(0.011/gPad->GetWNDC()+gPad->GetLeftMargin(),0.21/gPad->GetHNDC()+gPad->GetBottomMargin(),0.10/gPad->GetWNDC()+gPad->GetLeftMargin(),0.275/gPad->GetHNDC()+gPad->GetBottomMargin());
  }
    ...
    legend= new TLegend(0.035/gPad->GetWNDC()+gPad->GetLeftMargin(),0.19/gPad->GetHNDC()+gPad->GetBottomMargin(),0.155/gPad->GetWNDC()+gPad->GetLeftMargin(),0.235/gPad->GetHNDC()+gPad->GetBottomMargin());
  }
  else return 0x0;
  legend->SetFillColor(0);
  legend->SetFillStyle(0);
...
TLegend *GetLegendBaselinesC(TGraphAsymmErrors *g1,TGraphAsymmErrors *g2,TGraphAsymmErrors *g3,Int_t identifier,Int_t positionOption=1){
  TLegend * legend;
  if(positionOption==0){
    legend=new TLegend(0.011/gPad->GetWNDC()+gPad->GetLeftMargin(),0.21/gPad->GetHNDC()+gPad->GetBottomMargin(),0.10/gPad->GetWNDC()+gPad->GetLeftMargin(),0.275/gPad->GetHNDC()+gPad->GetBottomMargin());
  }
    ...
    legend= new TLegend(0.065/gPad->GetWNDC()+gPad->GetLeftMargin(),0.19/gPad->GetHNDC()+gPad->GetBottomMargin(),0.185/gPad->GetWNDC()+gPad->GetLeftMargin(),0.235/gPad->GetHNDC()+gPad->GetBottomMargin());
  }
  else return 0x0;
  legend->SetFillColor(0);
  legend->SetFillStyle(0);
  ...
  legend->SetTextAlign(12);
  legend->SetTextSize(23*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42, 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  legend->AddEntry(g3," baseline-subtraction uncertainty","f");
  // legend->AddEntry(box2[4],"v2 subtr p-Pb","f");
  legend->SetName(Form("LegendBaselineUncPPandpPb_%d",identifier));
  return legend;
}
...
TPaveText *GetALICEpavetext(Int_t identifier){
  TPaveText *alice = new TPaveText(4.33/gPad->GetWNDC()+gPad->GetLeftMargin(),0.225/gPad->GetHNDC()+gPad->GetBottomMargin(),0.37/gPad->GetWNDC()+gPad->GetLeftMargin(),0.275/gPad->GetHNDC()+gPad->GetBottomMargin(),"NDC");
  ...
  alice->SetTextSize(35*innerPadHeight/referencePadHeight*resizeTextFactor);//old settings with font 42 : 0.08/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
...
TPaveText *GetAveragepavetext(Int_t identifier){
  TPaveText* pvAverage = new TPaveText(0.0107/gPad->GetWNDC()+gPad->GetLeftMargin(),0.322/gPad->GetHNDC()+gPad->GetBottomMargin(),0.3/gPad->GetWNDC()+gPad->GetLeftMargin(),0.275/gPad->GetHNDC()+gPad->GetBottomMargin(),"NDC");
  ...
  pvAverage->SetTextSize(35*innerPadHeight/referencePadHeight*resizeTextFactor);//old settings with font 42: 0.08/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
...
TPaveText *GetSystemColl(Int_t identifier){
    TPaveText* pvSystColl = new TPaveText(0.01/gPad->GetWNDC()+gPad->GetLeftMargin(),0.315/gPad->GetHNDC()+gPad->GetBottomMargin(),0.3/gPad->GetWNDC()+gPad->GetLeftMargin(),0.275/gPad->GetHNDC()+gPad->GetBottomMargin(),"NDC");
   
    SetPaveStyle(pvSystColl);
    pvSystColl->SetTextFont(43);
    pvSystColl->SetTextSize(35*innerPadHeight/referencePadHeight*resizeTextFactor);//old settings with font 42: 0.08/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
    pvSystColl->AddText("p-Pb, #sqrt{#it{s}_{NN}} = 5.02 TeV");
    pvSystColl->SetName(Form("paveSystColl_%d",identifier));
    return pvSystColl;
}
...
TPaveText *GetPaveKineInfo(TString strPtMeson,TString strPtAssoc,Int_t identifier,Int_t mode=10/* mode: alignment * 10 + 0,1,2,3 (two rows, two rows + include DeltaEta, single row, single row + included delta eta, with alignment=0 -> align left, 1->align right*/){
  ...
  pvKineInfo->SetTextSize(27.*innerPadHeight/referencePadHeight*resizeTextFactor);// settings for font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  ...
  if(!strPtAssoc.IsNull()){
    ...
    pvKineInfo->SetTextSize(35.*innerPadHeight/referencePadHeight*resizeTextFactor);    
  }
...
void DoComparison_pPbVsCentrality_Distributions(){
  ...
  for(Int_t icoll=0;icoll<ncollsyst;icoll++){
	...
	histo[icoll][kassoc][jmes]->SetMarkerSize(markersize[icoll]); 
...
*/
      ...
      h->GetYaxis()->SetTitleSize(33.5*innerPadHeight/referencePadHeight*resizeTextFactor);
      h->GetXaxis()->SetTitleSize(38*innerPadHeight/referencePadHeight*resizeTextFactor);
	...
	h->GetXaxis()->SetLabelSize(0);
      ...
      SetColorGraphs(2,grbase[2][iassoc][jDpt],colourSystemBaseUnc[2]);// redundant...
      ...
      SetScaleUncertaintyPositionAndSize(ltscale[0][iassoc][jDpt],ltscale[1][iassoc][jDpt],ltscale[2][iassoc][jDpt]);
      
      if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==1){
	ltscale[0][iassoc][jDpt]->SetY(0.13/gPad->GetHNDC()+gPad->GetBottomMargin());// older settings 0.135/gPad->GetHNDC()+gPad->GetBottomMargin());
	ltscale[1][iassoc][jDpt]->SetY(0.13/gPad->GetHNDC()+gPad->GetBottomMargin());// older settings 0.105/gPad->GetHNDC()+gPad->GetBottomMargin());
        ltscale[2][iassoc][jDpt]->SetY(0.13/gPad->GetHNDC()+gPad->GetBottomMargin());// older settings 0.135/gPad->GetHNDC()+gPad->GetBottomMargin());
        ltscale[0][iassoc][jDpt]->SetX(0.055/gPad->GetWNDC()+gPad->GetLeftMargin());
	ltscale[1][iassoc][jDpt]->SetX(0.08/gPad->GetWNDC()+gPad->GetLeftMargin());
        ltscale[2][iassoc][jDpt]->SetX(0.105/gPad->GetWNDC()+gPad->GetLeftMargin());
      }
	...
	ltscale[1][iassoc][jDpt]->SetY(0.15/gPad->GetHNDC()+gPad->GetBottomMargin());
        ltscale[2][iassoc][jDpt]->SetY(0.15/gPad->GetHNDC()+gPad->GetBottomMargin());
        ltscale[0][iassoc][jDpt]->SetX(0.055/gPad->GetWNDC()+gPad->GetLeftMargin());
        ltscale[1][iassoc][jDpt]->SetX(0.08/gPad->GetWNDC()+gPad->GetLeftMargin());
        ltscale[2][iassoc][jDpt]->SetX(0.105/gPad->GetWNDC()+gPad->GetLeftMargin());
      }
        ...
        if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==4){
            ltscale[0][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[1][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[2][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[0][iassoc][jDpt]->SetX(0.055/gPad->GetWNDC()+gPad->GetLeftMargin());
            ltscale[1][iassoc][jDpt]->SetX(0.08/gPad->GetWNDC()+gPad->GetLeftMargin());
            ltscale[2][iassoc][jDpt]->SetX(0.105/gPad->GetWNDC()+gPad->GetLeftMargin());
        }
        ...
        if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==5){
            ltscale[0][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[1][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[2][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[0][iassoc][jDpt]->SetX(0.055/gPad->GetWNDC()+gPad->GetLeftMargin());
        ...
        if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==6){
            ltscale[0][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[1][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[2][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[0][iassoc][jDpt]->SetX(0.055/gPad->GetWNDC()+gPad->GetLeftMargin());
        ...
        if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==7){
            ltscale[0][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[1][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[2][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[0][iassoc][jDpt]->SetX(0.055/gPad->GetWNDC()+gPad->GetLeftMargin());
        ...
        if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==8){
            ltscale[0][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[1][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[2][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[0][iassoc][jDpt]->SetX(0.055/gPad->GetWNDC()+gPad->GetLeftMargin());
        ...
        if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==9){
            ltscale[0][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[1][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[2][iassoc][jDpt]->SetY(0.205/gPad->GetHNDC()+gPad->GetBottomMargin());
            ltscale[0][iassoc][jDpt]->SetX(0.055/gPad->GetWNDC()+gPad->GetLeftMargin());
      ...
      TLegend *legend2a;
      if(style==0)legend2a=GetLegendBaselinesA(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,0);
      else if(style==1)legend2a=GetLegendBaselinesA(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,3); // default option was 1 in older settings; 2 for draft 1 version;
      else legend2a=GetLegendBaselinesA(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,1);
      TLegend *legend2b;
      if(style==0)legend2b=GetLegendBaselinesB(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,0);
      else if(style==1)legend2b=GetLegendBaselinesB(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,3); // default option was 1 in older settings; 2 for draft 1 version;
      else legend2b=GetLegendBaselinesB(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,1);
      TLegend *legend2c;
      if(style==0)legend2c=GetLegendBaselinesC(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,0);
      else if(style==1)legend2c=GetLegendBaselinesC(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,3); // default option was 1 in older settings; 2 for draft 1 version;
      else legend2c=GetLegendBaselinesC(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,1);

//style is 1


      if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==2){legend->Draw(); /*legendSuperimp->Draw();*/}
      if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==3){legend2a->Draw();legend2b->Draw();legend2c->Draw();}// was in pad 3 before
      Printf("Getting average pave text");
      TPaveText *pvAverage=GetAveragepavetext((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1);
        pvAverage->SetX1(0.26);
        pvAverage->SetY1(0.65);
        
       Printf("Getting collision system");
        TPaveText *pvSystColl=GetSystemColl((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1);
        pvSystColl->SetX1(0.26);
        pvSystColl->SetY1(0.49);
	...
	pvKineInfo->SetY1(0.165/gPad->GetHNDC()+gPad->GetBottomMargin());
	pvKineInfo->SetY2(0.19/gPad->GetHNDC()+gPad->GetBottomMargin());
    //pvKineInfo->SetY3(0.175/gPad->GetHNDC()+gPad->GetBottomMargin());
	pvKineInfo->SetX1(0.008/gPad->GetWNDC()+gPad->GetLeftMargin());
	pvKineInfo->SetX2(0.23/gPad->GetWNDC()+gPad->GetLeftMargin());
    //pvKineInfo->SetX3(0.23/gPad->GetWNDC()+gPad->GetLeftMargin());
	pvKineInfo->SetTextAlign(12);	
	...
	pvKineInfo->SetY1(0.135/gPad->GetHNDC()+gPad->GetBottomMargin());
	pvKineInfo->SetY2(0.17/gPad->GetHNDC()+gPad->GetBottomMargin());
	pvKineInfo->SetX1(0.008/gPad->GetWNDC()+gPad->GetLeftMargin());
	pvKineInfo->SetX2(0.23/gPad->GetWNDC()+gPad->GetLeftMargin());
	pvKineInfo->SetTextAlign(12);
	pvKineInfo->Draw("same");
	...
	pvKineInfo->Draw("same");
      ...
      if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==1)  {
        pvKineInfo->SetX1(0.26);
        pvKineInfo->SetY1(0.75);
	      pvKineInfo->SetX2(0.85);
      } else if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==4){
        pvKineInfo->SetX1(0.4);
        pvKineInfo->SetX2(0.85);        
      } else if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==7){
        pvKineInfo->SetX1(0.4);
        pvKineInfo->SetX2(0.85);        
      } else if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==5){
        pvKineInfo->SetX1(0.24);
        pvKineInfo->SetX2(0.85);        
      }
      ...
      if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==1) pvSystColl->Draw("same");
  ...
  cFinalPaperStyle->SetFillColor(kWhite);

  cFinalPaperStyle->SaveAs(Form("plotComparison_%sAverage_pp_pPb_UniqueCanvas_Style%d_Paper.root",avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("plotComparison_%sAverage_pp_pPb_UniqueCanvas_Style%d_Paper.png",avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("plotComparison_%sAverage_pp_pPb_UniqueCanvas_Style%d_Paper.pdf",avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("plotComparison_%sAverage_pp_pPb_UniqueCanvas_Style%d_Paper.eps",avType.Data(),style));
