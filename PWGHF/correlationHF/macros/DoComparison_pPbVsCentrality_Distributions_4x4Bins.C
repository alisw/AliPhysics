//Int_t nhistos=6;
const Int_t ncollsyst=4;
const Int_t nbinDpt=4;
const Int_t nbinAssocpt=4;
Int_t firstDpt=0;
...
TString collsyst[ncollsyst]={"001","0110","1030","30100"};
TString pthadron[nbinAssocpt]={"0.3to99.0","0.3to1.0","1.0to2.0","2.0to3.0"};
TString strmesonpt[nbinDpt]={"3to5","5to8","8to16","16to24"};
TString strPtAssocText[nbinAssocpt]={"#it{p}_{T}^{assoc} > 0.3 GeV/#it{c}","0.3 < #it{p}_{T}^{assoc} < 1 GeV/#it{c}","1 < #it{p}_{T}^{assoc} < 2 GeV/#it{c}","2 < #it{p}_{T}^{assoc} < 3 GeV/#it{c}"};
...
Double_t minYaxis[nbinAssocpt]={-0.6,-0.6,-0.29,-0.2}; // or -0.6 for all  
Double_t maxYaxis[nbinAssocpt]={5.2,1.8,1.9,1.0};// or 4.1 for the first , 2.9 for the middle and 1.9 for the last
Double_t mesonptcenter[nbinDpt]={4.,6.5,12.,20.};// needed for getting the baseline
Color_t colourSystem[ncollsyst]={kBlue,kRed,kGreen+2,kBlack}; 
Color_t colourSystemBaseUnc[ncollsyst]={kBlue,kRed,kGreen+2,kBlack};//was {kBlack,kRed-7};
Int_t fillColourBaselineStyle=1001;
Int_t markerstyle[4]={20,21,22,23};// pp, p-Pb
Double_t innerPadHeight;// not touch, set internally
Double_t innerPadWidth;// not touch, set internally
Int_t rebin =1;
Double_t **ptbinsD;
Int_t ndesiredbins=3;
TString inputdatadirectory[ncollsyst] = {"./001","./0110","./1030","./30100"};
TString baselinedirectory[ncollsyst]={"./001","./0110","./1030","./30100"};
TString avType="Arithmetic";
...
void LoadFileNamesppVspPb(){
  ...
  for(Int_t icoll=0;icoll<ncollsyst;icoll++){
	...
	filenames[icoll][kassoc][jmes]=Form("%s/%sAverageppDzeroDstarDplus%s_assoc%s.root",inputdatadirectory[icoll].Data(),avType.Data(),strmesonpt[jmes].Data(),pthadron[kassoc].Data());
      }
        pedestalfilenames[icoll][kassoc]=Form("%s/Trends_pp/CanvasBaselineVariationTrendPedestal_pthad%s.root",baselinedirectory[icoll].Data(),pthadron[kassoc].Data());
    }
  }
}
...
TH1D * GetHistoAndSyst(TString path, Int_t collsyst,TString hname, TString hnamesyst,TGraphAsymmErrors *&gr2, TLatex *&tUncertainty){
  ...
  if(collsyst==0){
    ...
    else tUncertainty=new TLatex(0.55,0.49,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc. (0-20%)","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==1){
    ...
    else tUncertainty=new TLatex(0.55,0.45,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc. (20-60%)","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==2){
    if(TMath::Abs(hUncCorrMin->GetBinContent(1)-hUncCorrMax->GetBinContent(1))<0.001)tUncertainty=new TLatex(0.55,0.41,Form("#bf{%.0f#% scale uncertainty p-Pb}",hUncCorrMin->GetBinContent(1)*100.));
    else tUncertainty=new TLatex(0.55,0.41,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc. (60-100%)","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }
  if(collsyst==3){
    if(TMath::Abs(hUncCorrMin->GetBinContent(1)-hUncCorrMax->GetBinContent(1))<0.001)tUncertainty=new TLatex(0.55,0.37,Form("#bf{%.0f#% scale uncertainty p-Pb}",hUncCorrMin->GetBinContent(1)*100.));
    else tUncertainty=new TLatex(0.55,0.37,Form("{}^{#plus%.0f%s}_{#minus%.0f%s} scale unc. (0-100%)","%","%",TMath::Abs(hUncCorrMax->GetBinContent(1))*100.,TMath::Abs(hUncCorrMin->GetBinContent(1)*100.)));
  }  
...
void SetScaleUncertaintyPositionAndSize(TLatex *tl1,TLatex *tl2,TLatex *tl3,TLatex *tl4){
  tl1->SetTextFont(43);
  tl2->SetTextFont(43);
  tl3->SetTextFont(43);
  tl4->SetTextFont(43);
  ...
  */
  ...
  //   tlpp->SetY(0.19/gPad->GetHNDC()+gPad->GetBottomMargin());
  tl1->SetX(0.083/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl1->SetY(0.22/gPad->GetHNDC()+gPad->GetBottomMargin());

  //  tlpPb->SetX(0.13/gPad->GetWNDC()+gPad->GetLeftMargin()); // draft 1
  //  tlpPb->SetY(0.156/gPad->GetHNDC()+gPad->GetBottomMargin());
  tl2->SetX(0.083/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl2->SetY(0.19.5/gPad->GetHNDC()+gPad->GetBottomMargin());

  tl3->SetX(0.083/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl3->SetY(0.17/gPad->GetHNDC()+gPad->GetBottomMargin());

  tl4->SetX(0.083/gPad->GetWNDC()+gPad->GetLeftMargin());
  tl4->SetY(0.14.5/gPad->GetHNDC()+gPad->GetBottomMargin());

  tl1->SetTextSize(24*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  tl2->SetTextSize(24*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  tl3->SetTextSize(24*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  tl4->SetTextSize(24*innerPadHeight/referencePadHeight*resizeTextFactor);// old settings with font 42: 0.07/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  tl1->SetTextAlign(12);
  tl2->SetTextAlign(12);
  tl3->SetTextAlign(12);
  tl4->SetTextAlign(12);
...
TLegend *GetLegendDataPoints(TH1D *h1,TH1D *h2,TH1D *h3,TH1D *h4,Int_t identifier){
  ...
  //previous settings: (0.011/gPad->GetWNDC()+gPad->GetLeftMargin(),0.16/gPad->GetHNDC()+gPad->GetBottomMargin(),0.2/gPad->GetWNDC()+gPad->GetLeftMargin(),0.22/gPad->GetHNDC()+gPad->GetBottomMargin());
    ...
    legend->AddEntry(h1,"pp 0-0.1% V0M","lep");
    legend->AddEntry(h2,"pp, 0.1-10% V0M","lep");
    legend->AddEntry(h3,"pp, 10-30% V0M","lep");
    legend->AddEntry(h4,"pp, 30-100% V0M","lep");
...
TLegend *GetLegendBaselines(TGraphAsymmErrors *g1,TGraphAsymmErrors *g2,TGraphAsymmErrors *g3,TGraphAsymmErrors *g4,Int_t identifier,Int_t positionOption=1){
  TLegend * legend;
  if(positionOption==0){
  ...
  legend->AddEntry(g1,"baseline-subtraction unc. (0-0.1%)","f");
  legend->AddEntry(g2,"baseline-subtraction unc. (0.1-10%)","f");
  legend->AddEntry(g3,"baseline-subtraction unc. (10-30%)","f");
  legend->AddEntry(g4,"baseline-subtraction unc. (30-100%)","f");
...
void DoComparison_pPbVsCentrality_Distributions(){
  ...
  if(skip3to5){
    ...
    cFinalPaperStyle=new TCanvas("cFinalPaperStyle","cFinalPaperStyle",ratioForCanvasWidth*canvasheight,canvasheight);
    SetPadStyle(cFinalPaperStyle);
    cFinalPaperStyle->Divide(4,4,0,0);
    Set4x4PadPositions(cFinalPaperStyle);
    cFinalPaperStyle->Modified();
    cFinalPaperStyle->Update();
  }
  ...
  for(Int_t iassoc=0;iassoc<nbinAssocpt;iassoc++){
    for(Int_t jDpt=firstDpt;jDpt<nbinDpt;jDpt++){
      ...
      endline=subtractedhisto[1][iassoc][jDpt]->GetBinLowEdge(subtractedhisto[1][iassoc][jDpt]->GetNbinsX())+wi+0.45;// 0.15
...
*/
      ...
      SetColorGraphs(2,grbase[1][iassoc][jDpt],colourSystemBaseUnc[1]);// redundant...
      SetColorGraphs(3,grbase[1][iassoc][jDpt],colourSystemBaseUnc[1]);// redundant...
      grbase[0][iassoc][jDpt]->SetFillStyle(fillColourBaselineStyle);
      grbase[1][iassoc][jDpt]->SetFillStyle(fillColourBaselineStyle);
      grbase[2][iassoc][jDpt]->SetFillStyle(fillColourBaselineStyle);
      grbase[3][iassoc][jDpt]->SetFillStyle(fillColourBaselineStyle);
      grbase[3][iassoc][jDpt]->Draw("E2");
      grbase[2][iassoc][jDpt]->Draw("E2"); 
      grbase[1][iassoc][jDpt]->Draw("E2");
      grbase[0][iassoc][jDpt]->Draw("E2"); 

      line->Draw(); 
      subtractedhisto[1][iassoc][jDpt]->Draw("same");
      subtractedhisto[2][iassoc][jDpt]->Draw("same");
      subtractedhisto[3][iassoc][jDpt]->Draw("same");
      suberr[1][iassoc][jDpt]->Draw("E2");
      suberr[2][iassoc][jDpt]->Draw("E2");
      suberr[3][iassoc][jDpt]->Draw("E2");
      Printf("Setting scale unc text pos and size");
      SetScaleUncertaintyPositionAndSize(ltscale[0][iassoc][jDpt],ltscale[1][iassoc][jDpt],ltscale[2][iassoc][jDpt],ltscale[3][iassoc][jDpt]);
      ...
      if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==1){
        ...
        ltscale[2][iassoc][jDpt]->SetY(0.09/gPad->GetHNDC()+gPad->GetBottomMargin());// older settings 0.135/gPad->GetHNDC()+gPad->GetBottomMargin());
        ltscale[3][iassoc][jDpt]->SetY(0.06/gPad->GetHNDC()+gPad->GetBottomMargin());// older settings 0.105/gPad->GetHNDC()+gPad->GetBottomMargin());
        ...
        ltscale[2][iassoc][jDpt]->SetX(0.11/gPad->GetWNDC()+gPad->GetLeftMargin());
        ltscale[3][iassoc][jDpt]->SetX(0.11/gPad->GetWNDC()+gPad->GetLeftMargin());
      }
      // not in older settings:
      if((nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1==2){
      	ltscale[0][iassoc][jDpt]->SetY(0.18/gPad->GetHNDC()+gPad->GetBottomMargin());
      	ltscale[1][iassoc][jDpt]->SetY(0.15/gPad->GetHNDC()+gPad->GetBottomMargin());	
        ltscale[0][iassoc][jDpt]->SetY(0.12/gPad->GetHNDC()+gPad->GetBottomMargin());
        ltscale[1][iassoc][jDpt]->SetY(0.09/gPad->GetHNDC()+gPad->GetBottomMargin());   
      }
      ...
      ltscale[2][iassoc][jDpt]->Draw();//commented
      ltscale[3][iassoc][jDpt]->Draw();//commented
      ltscale[0][iassoc][jDpt]->Draw();//commented
      Printf("Getting data legend");
      TLegend *legend=GetLegendDataPoints(histo[0][iassoc][firstDpt],histo[1][iassoc][firstDpt],histo[2][iassoc][firstDpt],histo[3][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1);
      ...
      TLegend *legend2;
      if(style==0)legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],grbase[3][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,0);
      else if(style==1)legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],grbase[3][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,3); // default option was 1 in older settings; 2 for draft 1 version;
      else legend2=GetLegendBaselines(grbase[0][iassoc][firstDpt],grbase[1][iassoc][firstDpt],grbase[2][iassoc][firstDpt],grbase[3][iassoc][firstDpt],(nbinDpt-firstDpt)*iassoc+jDpt-firstDpt+1,1);
  ...
  cFinalPaperStyle->SaveAs(Form("plotComparison_%sAverage_pp_pPb_UniqueCanvas_Style%d_4x4Bins.root",avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("plotComparison_%sAverage_pp_pPb_UniqueCanvas_Style%d_4x4Bins.png",avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("plotComparison_%sAverage_pp_pPb_UniqueCanvas_Style%d_4x4Bins.pdf",avType.Data(),style));
  cFinalPaperStyle->SaveAs(Form("plotComparison_%sAverage_pp_pPb_UniqueCanvas_Style%d_4x4Bins.eps",avType.Data(),style));
...
void Set4x4PadPositions(TCanvas* c){
  ...
  TPad * pd13 =(TPad*) c->GetPad(13);
  TPad * pd14 =(TPad*) c->GetPad(14);
  TPad * pd15 =(TPad*) c->GetPad(15);
  TPad * pd16 =(TPad*) c->GetPad(16);  
  ...
  innerPadHeight=(1-marginTop-marginBottom)/4.;// this is the height w/o margin, not the real pad height, which differs between inner pads and pads at the "boarders"!!
  Printf("innerPadHeight: %f",innerPadHeight);
  Printf("innerPadWidth: %f",innerPadWidth);
  Double_t marginLeftForXAxis=0.02;
  Double_t marginBottomForYAxis=0.0;
 ...
 // Least-bottom row
    ...
    pd13->GetPadPar(xl,yl,xu,yu);
    Printf("PAD 7 Original values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    pd13->SetPad(0.,0.,innerPadWidth+marginLeft,innerPadHeight+marginBottom);
    pd13->GetPadPar(xl,yl,xu,yu);
    Printf("New values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
 
    pd13->SetLeftMargin(marginLeft/(marginLeft+innerPadWidth));
    pd13->SetRightMargin(0);
    pd13->SetBottomMargin(marginBottom/(marginBottom+innerPadHeight));
    pd13->SetTopMargin(0.);
    pd13->Modified();
    pd13->SetFillStyle(0);

    pd14->GetPadPar(xl,yl,xu,yu);
    Printf("PAD 10 Original values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    pd14->SetPad(innerPadWidth+marginLeft-marginLeftForXAxis,0,2*innerPadWidth+marginLeft,innerPadHeight+marginBottom);
    pd14->GetPadPar(xl,yl,xu,yu);
    Printf("New values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
 
    pd14->SetLeftMargin(marginLeftForXAxis/(innerPadWidth+marginLeftForXAxis));
    //pd8->SetLeftMargin(0.004/(1.-innerPadWidth-marginLeft+0.004));
    pd14->SetRightMargin(0.);
    pd14->SetBottomMargin(marginBottom/(marginBottom+innerPadHeight));
    pd14->SetTopMargin(0);
    pd14->SetFillStyle(0);
    ...
    pd15->GetPadPar(xl,yl,xu,yu);
    Printf("PAD 11 Original values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
  
    pd15->SetPad(2*innerPadWidth+marginLeft-marginLeftForXAxis,0,3*innerPadWidth+marginLeft,2*innerPadHeight+marginBottom);
    pd15->GetPadPar(xl,yl,xu,yu);
    Printf("New values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    pd15->SetLeftMargin(marginLeftForXAxis/(innerPadWidth+marginLeftForXAxis));
    pd15->SetRightMargin(0);
    pd15->SetBottomMargin(marginBottom/(marginBottom+innerPadHeight));
    pd15->SetTopMargin(0.);
    pd15->SetFillStyle(0);
    ...
    pd16->GetPadPar(xl,yl,xu,yu);
    Printf("PAD 12 Original values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    pd16->SetPad(3*innerPadWidth+marginLeft-marginLeftForXAxis,0,1.,innerPadHeight+marginBottom);
    pd16->GetPadPar(xl,yl,xu,yu);
    Printf("New values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
 
    pd16->SetLeftMargin(marginLeftForXAxis/(innerPadWidth+marginLeftForXAxis+marginRight));
    pd16->SetRightMargin(marginRight/(innerPadWidth+marginRight));
    pd16->SetBottomMargin(marginBottom/(marginBottom+innerPadHeight));
    pd16->SetTopMargin(0.);
    pd16->SetFillStyle(0);
 ...
 // Bottom row
    ...
    pd9->GetPadPar(xl,yl,xu,yu);
    Printf("PAD 7 Original values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    pd9->SetPad(0.,innerPadHeight+marginBottom-marginBottomForYAxis,innerPadWidth+marginLeft,2.*innerPadHeight+marginBottom);
    pd9->GetPadPar(xl,yl,xu,yu);
    Printf("New values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
 
    pd9->SetLeftMargin(marginLeft/(marginLeft+innerPadWidth));
    pd9->SetRightMargin(0);
    pd9->SetBottomMargin(marginBottomForYAxis/(innerPadHeight+marginBottomForYAxis));
    pd9->SetTopMargin(0.);
    pd9->Modified();
    pd9->SetFillStyle(0);
    ...
    Printf("PAD 10 Original values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    pd10->SetPad(innerPadWidth+marginLeft-marginLeftForXAxis,innerPadHeight+marginBottom-marginBottomForYAxis,2*innerPadWidth+marginLeft,2.*innerPadHeight+marginBottom);
    pd10->GetPadPar(xl,yl,xu,yu);
    Printf("New values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    ...
    pd10->SetBottomMargin(marginBottomForYAxis/(innerPadHeight+marginBottomForYAxis));
    pd10->SetTopMargin(0);
    pd10->SetFillStyle(0);
    ...
    pd11->SetPad(2*innerPadWidth+marginLeft-marginLeftForXAxis,innerPadHeight+marginBottom-marginBottomForYAxis,3*innerPadWidth+marginLeft,2.*innerPadHeight+marginBottom);
    pd11->GetPadPar(xl,yl,xu,yu);
    Printf("New values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    pd11->SetLeftMargin(marginLeftForXAxis/(innerPadWidth+marginLeftForXAxis));
    pd11->SetRightMargin(0);
    pd11->SetBottomMargin(marginBottomForYAxis/(innerPadHeight+marginBottomForYAxis));
    pd11->SetTopMargin(0.);
    pd11->SetFillStyle(0);
    ...
    Printf("PAD 12 Original values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    pd12->SetPad(3*innerPadWidth+marginLeft-marginLeftForXAxis,innerPadHeight+marginBottom-marginBottomForYAxis,1.,2.*innerPadHeight+marginBottom);
    pd12->GetPadPar(xl,yl,xu,yu);
    Printf("New values: xl %f  xu %f  yl  %f  yu %f, gPad->GetwNDC=%f, gPad->GetHNDC=%f",xl,xu,yl,yu,gPad->GetWNDC(),gPad->GetHNDC());
    ...
    pd12->SetBottomMargin(marginBottomForYAxis/(innerPadHeight+marginBottomForYAxis));
    pd12->SetTopMargin(0.);
    pd12->SetFillStyle(0);
    ...
    pd5->GetPadPar(xl,yl,xu,yu);
    Printf("Original values: xl %f  xu %f  yl  %f  yu %f",xl,xu,yl,yu);
    pd5->SetPad(0.,2*innerPadHeight+marginBottom-marginBottomForYAxis,innerPadWidth+marginLeft,3.*innerPadHeight+marginBottom); 
    pd5->SetLeftMargin(marginLeft/(marginLeft+innerPadWidth));
    pd5->SetRightMargin(0.);
    pd5->SetBottomMargin(marginBottomForYAxis/(innerPadHeight+marginBottomForYAxis));
    pd5->SetTopMargin(0.);
    ...
    pd6->GetPadPar(xl,yl,xu,yu);
    Printf("Original values: xl %f  xu %f  yl  %f  yu %f",xl,xu,yl,yu);
    pd6->SetPad(innerPadWidth+marginLeft-marginLeftForXAxis,2*innerPadHeight+marginBottom-marginBottomForYAxis,2*innerPadWidth+marginLeft,3.*innerPadHeight+marginBottom);
    ...
    pd7->GetPadPar(xl,yl,xu,yu);
    Printf("Original values: xl %f  xu %f  yl  %f  yu %f",xl,xu,yl,yu);
    pd7->SetPad(2*innerPadWidth+marginLeft-marginLeftForXAxis,2*innerPadHeight+marginBottom-marginBottomForYAxis,3*innerPadWidth+marginLeft,3.*innerPadHeight+marginBottom);
    ...
    pd8->GetPadPar(xl,yl,xu,yu);
    Printf("Original values: xl %f  xu %f  yl  %f  yu %f",xl,xu,yl,yu);
    pd8->SetPad(3*innerPadWidth+marginLeft-marginLeftForXAxis,2*innerPadHeight+marginBottom-marginBottomForYAxis,1.,3.*innerPadHeight+marginBottom);
  ...
  // Top Row
    pd1->GetPadPar(xl,yl,xu,yu);
    Printf("Original values: xl %f  xu %f  yl  %f  yu %f",xl,xu,yl,yu);
    pd1->SetPad(0,3.*innerPadHeight+marginBottom-marginBottomForYAxis,innerPadWidth+marginLeft,1.);
    pd1->SetLeftMargin(marginLeft/(marginLeft+innerPadWidth));
    pd1->SetRightMargin(0.);
    pd1->SetBottomMargin(marginBottomForYAxis/(innerPadHeight+marginBottomForYAxis+marginTop));
    pd1->SetTopMargin(marginTop/(innerPadHeight+marginTop));
    ...
    pd2->GetPadPar(xl,yl,xu,yu);
    Printf("Original values: xl %f  xu %f  yl  %f  yu %f",xl,xu,yl,yu);
    pd2->SetPad(innerPadWidth+marginLeft-marginLeftForXAxis,3.*innerPadHeight+marginBottom-marginBottomForYAxis,2*innerPadWidth+marginLeft,1);
    pd2->SetLeftMargin(marginLeftForXAxis/(innerPadWidth+marginLeftForXAxis));
    pd2->SetFillStyle(0);
    pd2->SetRightMargin(0);
    pd2->SetBottomMargin(marginBottomForYAxis/(innerPadHeight+marginBottomForYAxis+marginTop));
    ...
    pd3->GetPadPar(xl,yl,xu,yu);
    Printf("Original values: xl %f  xu %f  yl  %f  yu %f",xl,xu,yl,yu);
    pd3->SetPad(2*innerPadWidth+marginLeft-marginLeftForXAxis,3.*innerPadHeight+marginBottom-marginBottomForYAxis,3*innerPadWidth+marginLeft,1);
    ...
    pd4->GetPadPar(xl,yl,xu,yu);
    Printf("Original values: xl %f  xu %f  yl  %f  yu %f",xl,xu,yl,yu);
    pd4->SetPad(3*innerPadWidth+marginLeft-marginLeftForXAxis,3.*innerPadHeight+marginBottom-marginBottomForYAxis,1,1);
