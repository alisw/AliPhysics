...
void DoPlotSingle1Canvas(TString TrackpTthr ="0.3to1.0",TString system = "pPb"){
  ...
  for(Int_t k=0; k<nhistos; k++){
    ...
    if(k==0 || k==3 || k==6){  
      ...
      if(kTemp==1)TLatex *tlTitle00=new TLatex(0.18,0.75,"#bf{5 < #it{p}_{T}^{D^{0}} < 8 GeV/c,   #it{p}_{T}^{assoc} = 0.3 - 1.0  GeV/c}");
      if(kTemp==2)TLatex *tlTitle00=new TLatex(0.18,0.75,"#bf{8 < #it{p}_{T}^{D^{0}} < 16 GeV/c,   #it{p}_{T}^{assoc} = 0.3 - 1.0  GeV/c}");
	...
	if(strName.Contains("TLatex")){
	  ...
	  if(str.Contains("assoc")){
	    ...
	    if(kTemp==1)strTitle5.ReplaceAll("#bf{5 < #it{p}_{T}^{D^{0}} < 8 GeV/c,  #it{p}_{T}^{assoc} = 0.3 - 1.0  GeV/c}","");
	    if(kTemp==2)strTitle5.ReplaceAll("#bf{8 < #it{p}_{T}^{D^{0}} < 16 GeV/c,  #it{p}_{T}^{assoc} = 0.3 - 1.0  GeV/c}","");
