...
TString strSystem[3]={"pp, 5 TeV","pp, 7 TeV","pp, 13 TeV"};
Color_t colSystem[3]={kBlue,kRed,kGreen+2};
Int_t markerStyle[3]={20,21,22};
Bool_t useLegendForData=kTRUE;
Bool_t plotv2unc=kFALSE;
TString strFitResultPPb[3]={"./pp_5TeV/FitResults/Trends_pp","./pp_7TeV/FitResults/Trends_pp","./pp_13TeV/FitResults/Trends_pp"}; //  "/Users/administrator/ALICE/CHARM/HFCJ/DCorrelations_Test/2015June7finalPlots/ReflectedPlots/StdRebin/AllPlots/Averages/FitResults";
...
Double_t minptData=3.5,maxptData=14.; //you have to be tighter than real limits...
Int_t ncolumns=3;
Int_t nrows=3;
...
void SetDirectoryFitResult(TString str1,TString str2,TString str3){
  strFitResultPPb[0]=str1;
  strFitResultPPb[1]=str2;
  strFitResultPPb[2]=str3;
}
...
TLatex *GetDRapForSystem(Int_t collSystem,Int_t identifier,Int_t includeDEta=0){
  ...
  if(style==-1){
    ...
    tlrap->SetTextSize(26*innerPadHeight/referencePadHeight*resizeTextFactor);//0.06/(gPad->GetHNDC())*scaleHeightPads*resizeTextFactor);
  }  
...
TCanvas* Compare(Int_t binass,Int_t quantity,TPad *pd=0x0,Int_t textlegendOptions=0){
  ...
  printf("Preparing pp@5...\n");
  TGraphAsymmErrors *grPPb_1;
  TH1D *hPPb_1=GetAndPreparePPb(binass,quantity,0,grPPb_1);

  printf("Preparing pp@7...\n");
  TGraphAsymmErrors *grPPb_2;
  TH1D *hPPb_2=GetAndPreparePPb(binass,quantity,1,grPPb_2);

  printf("Preparing pp@13...\n");
  TGraphAsymmErrors *grPPb_3;
  TH1D *hPPb_3=GetAndPreparePPb(binass,quantity,2,grPPb_3);  
/*
  for(int i=1; i<=hPPb_1->GetNbinsX(); i++) {
  	printf("syst 0 - Bin %d: val %f #pm %f\n",i,hPPb_1->GetBinContent(i),hPPb_1->GetBinError(i));
  } 
  for(int i=1; i<=hPPb_2->GetNbinsX(); i++) {
  	printf("syst 1 - Bin %d: val %f #pm %f\n",i,hPPb_2->GetBinContent(i),hPPb_2->GetBinError(i));
  }   
*/
  TCanvas *cout=0x0;
  if(!pd){
...
//     hPP->GetXaxis()->SetLabelSize(0.04);
    ...
    hDraw->GetXaxis()->SetRangeUser(0,17);
...
//Conversion of pPb TH1F to TGraph to displace the points along x axis (Fabio)
  TGraphAsymmErrors *gr_points_PPb1, *gr_pointCount_PPb1, *gr_points_PPb2, *gr_pointCount_PPb2, *gr_points_PPb3, *gr_pointCount_PPb3;
    ...
    ConvertTH1ToTGraphAsymmError2016(hPPb_3,gr_points_PPb3,shift*(1),1);
...
void CompareFitResults_UniqueCanvas(){
  ...
  cFinalPaperStyle->Modified();
  cFinalPaperStyle->Update();
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_DiffppEnergies.root",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_DiffppEnergies.eps",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_DiffppEnergies.png",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_DiffppEnergies.pdf",outdir.Data()));
...
void CompareFitResults_UniqueCanvas_AwaySide(){
  ...
  cFinalPaperStyle->Modified();
  cFinalPaperStyle->Update();
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_DiffppEnergies_AwaySide.root",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_DiffppEnergies_AwaySide.eps",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_DiffppEnergies_AwaySide.png",outdir.Data()));
  cFinalPaperStyle->SaveAs(Form("%s/CompareFitResults_DiffppEnergies_AwaySide.pdf",outdir.Data()));
