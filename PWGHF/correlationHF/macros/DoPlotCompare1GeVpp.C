...
void DoPlotSingle1Canvas(TString TrackpTthr ="dot5",TString system = "pp"){
  ...
  for(Int_t k=0; k<nhistos; k++){
    ...
    if(k==0 || k==3 || k==6){
      ...
      if(kTemp==1)TLatex *tlTitle00=new TLatex(0.18,0.75,"#bf{3 < #it{p}_{T}^{D^{}} < 5 GeV/c,  #it{p}_{T}^{assoc}>1  GeV/#it{c}}");
      if(kTemp==2)TLatex *tlTitle00=new TLatex(0.18,0.75,"#bf{5 < #it{p}_{T}^{D^{}} < 8 GeV/c,  #it{p}_{T}^{assoc}>1 GeV/#it{c}}");
      if(kTemp==3)TLatex *tlTitle00=new TLatex(0.18,0.75,"#bf{8 < #it{p}_{T}^{D^{}} < 16 GeV/c,  #it{p}_{T}^{assoc}>1  GeV/#it{c}}");
	...
	if(strName.Contains("TLatex")){
	  ...
	  if(str.Contains("assoc")){
	    ...
	    if(kTemp==1)strTitle5.ReplaceAll("#bf{3 < #it{p}_{T}^{D^{0}} < 5 GeV/c,  #it{p}_{T}^{assoc} > 1.0  GeV/c}","");
	    if(kTemp==2)strTitle5.ReplaceAll("#bf{5 < #it{p}_{T}^{D^{0}} < 8 GeV/c,  #it{p}_{T}^{assoc} > 1.0  GeV/c}","");
	    if(kTemp==3)strTitle5.ReplaceAll("#bf{8 < #it{p}_{T}^{D^{0}} < 16 GeV/c,  #it{p}_{T}^{assoc} > 1.0  GeV/c}","");
