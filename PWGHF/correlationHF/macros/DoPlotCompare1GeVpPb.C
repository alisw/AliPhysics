/* 
 ...
 Associated pT : > 1.0 GeV/c (old pT bin)
...
void DoPlotCompare1GeVpPb(){// THIS IS NOT A GENERAL MACRO YET --> USE IT ONLY FOR p-Pb, assoc>1 case
  DoPlotSingle1Canvas("1.0to99.0","pPb");
}

// Main function
void DoPlotSingle1Canvas(TString TrackpTthr ="1.0to99.0",TString system = "pPb"){
  ...
  for(Int_t k=0; k<nhistos; k++){
    ...
    for(Int_t jl=0;jl<entries;jl++){
      ...
      //1 Histogram final correlations points 
    ...
    if(k==0 || k==3 || k==6){
      ...
      hcur->GetYaxis()->SetRangeUser(0,8);
      hcur->GetYaxis()->SetTitle("#frac{1}{#it{N}_{D^{}}}#frac{d#it{N}^{assoc}}{d#Delta#varphi} (rad^{-1})");
      hcur->Draw("ep");
      
      //dot3 to 1 = 0.15, 0.21, 027
      TLatex *tlTitleDplus=new TLatex(0.48,0.48,"#bf{D^{+}#cbar{}^{+19%}_{-18%} scale uncertainty}");
      tlTitleDplus->SetNDC();
      tlTitleDplus->SetTextColor(kGreen+3);
      tlTitleDplus->SetTextSize(0.033);
      tlTitleDplus->Draw();
      
      TLatex *tlTitleDStar=new TLatex(0.48,0.54,"#bf{D^{*+}#cbar{}^{+14%}_{-13%} scale uncertainty}");
      tlTitleDStar->SetNDC();
      tlTitleDStar->SetTextColor(kAzure-2);
      tlTitleDStar->SetTextSize(0.033);
      tlTitleDStar->Draw();
      
      TLatex *tlTitleDZero=new TLatex(0.48,0.60,"#bf{D^{0}#cbar{}^{+14%}_{-13%} scale uncertainty}");
      tlTitleDZero->SetNDC();
      ...
      TLatex *tlTitle0=new TLatex(0.18,0.80,"#bf{p-Pb, #sqrt{#it{s}_{NN}} = 5.02 TeV, L_{int}= 50 #mub^{-1}}");
      tlTitle0->SetNDC();
      ...
      if(kTemp==1)TLatex *tlTitle00=new TLatex(0.18,0.75,"#bf{5 < #it{p}_{T}^{D^{}} < 8 GeV/c,   #it{p}_{T}^{assoc} > 1.0  GeV/c}");
      if(kTemp==2)TLatex *tlTitle00=new TLatex(0.18,0.75,"#bf{8 < #it{p}_{T}^{D^{}} < 16 GeV/c,   #it{p}_{T}^{assoc} > 1.0  GeV/c}");
	...
	TString str=obj->GetTitle();
	  ...
	  if(strName.Contains("TLatex")){
	    TLatex *tl=(TLatex*)obj;
	    //cout << "Latex Title " << str.Data() << endl;
	    ...
	    if(str.Contains("p-Pb")){
	      TString strTitle3=tl->GetTitle();
	      Printf("\n%s <-- Default txt(3)",strTitle3.Data());
	      strTitle3.ReplaceAll("p-Pb, #sqrt{#it{s}_{NN}} = 5.02 TeV, L_{int} = 50 #mub^{-1}","");
	    ...
	    if(str.Contains("assoc")){
	      ...
	      if(kTemp==1)strTitle5.ReplaceAll("#bf{5 < #it{p}_{T}^{D^{0}} < 8 GeV/c,  #it{p}_{T}^{assoc} > 1.0  GeV/c}","");
	      if(kTemp==2)strTitle5.ReplaceAll("#bf{8 < #it{p}_{T}^{D^{0}} < 16 GeV/c,  #it{p}_{T}^{assoc} > 1.0  GeV/c}","");
	    ...
	    if(str.Contains("scale uncertainty")){
	      ...
	      strTitle4.ReplaceAll("{}^{+14%}_{-13%} scale uncertainty","D^{0}, D^{+}, D^{*+} Comparison");
	      tl->SetTitle(strTitle4.Data());
	      tl->SetX(tl->GetX()+0.05); // +0.10 for dot3to1
		...
		tl->SetTextSize(0.037);
	      ...
	      cout << "\nComparison Box txt --> " << strTitle41.Data() << endl;
	    }
