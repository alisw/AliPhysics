...
//________________________________________________________________________
AliAnalysisTaskSED0MassCutVarFDSub::AliAnalysisTaskSED0MassCutVarFDSub(const char *name,AliRDHFCutsD0toKpi* cuts):
  ...
  fhsparsecutvar(0x0)
{
  ...
  DefineOutput(10,THnSparseF::Class());
...
void AliAnalysisTaskSED0MassCutVarFDSub::UserCreateOutputObjects()
{
  ...
  if(!fobjSpr){
    ...
    else fobjSpr->SetFillMC(kFALSE);
    fhsparsecutvar=fobjSpr->GetSparseData();
  }
...
void AliAnalysisTaskSED0MassCutVarFDSub::UserExec(Option_t */*option*/)
{
  ...
  if(!inputArray || !aod) {
    printf("AliAnalysisTaskSED0MassCutVarFDSub::UserExec: input branch not found!\n");
    return;
  }
  ...
  if(fReadMC) {
    ...
    if(!mcArray) {
      printf("AliAnalysisTaskSED0MassCutVarFDSub::UserExec: MC particles branch not found!\n");
      return;
    }
    ...
    if(!mcHeader) {
      printf("AliAnalysisTaskSED0MassCutVarFDSub::UserExec: MC header branch not found!\n");
      return;
    }
  }
  ...
  if (fIsRejectSDDClusters) {
    ...
    for(Int_t itrack=0; itrack<ntracks; itrack++) { // loop on tacks
      //    ... get the track
      AliVTrack * track = aod->GetTrack(itrack);
...
void AliAnalysisTaskSED0MassCutVarFDSub::FillMassHists(AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliAODMCHeader *mcHeader, AliRDHFCutsD0toKpi* cuts, TList *listout){
  ...
  if(fobjSpr)fobjSpr->FillSparses(part,fIsSelectedCandidate, pt,invmassD0, invmassD0bar);
