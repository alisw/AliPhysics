...
AliAnalysisTaskSED0Mass::AliAnalysisTaskSED0Mass() : AliAnalysisTaskSE(),
                                                     fOutputMass(0),
                                                     fOutputMassPt(0),
                                                     fOutputMassY(0),
                                                     fDistr(0),
                                                     ...
                                                     fHistChi2nClsptD_after(0x0)
{
  ...
  for (Int_t ih = 0; ih < 5; ih++)
    fHistMassPtImpParTC[ih] = 0x0;
}

//________________________________________________________________________
AliAnalysisTaskSED0Mass::AliAnalysisTaskSED0Mass(const char *name, AliRDHFCutsD0toKpi *cuts) : AliAnalysisTaskSE(name),
                                                                                               fOutputMass(0),
                                                                                               fOutputMassPt(0),
                                                                                               fOutputMassY(0),
                                                                                               fDistr(0),
...
void AliAnalysisTaskSED0Mass::Init()
{
  ...
  if (fDebug > 1)
    printf("AnalysisTaskSED0Mass::Init() \n");
...
void AliAnalysisTaskSED0Mass::UserCreateOutputObjects()
{
  ...
  //
  if (fDebug > 1)
    printf("AnalysisTaskSED0Mass::UserCreateOutputObjects() \n");
  ...
  Int_t nbins2dPt = 60;
  Float_t binInPt = 0., binFinPt = 30.;
  Int_t nbins2dY = 60;
  Float_t binInY = -1.5, binFinY = 1.5;
  ...
  for (Int_t i = 0; i < fCuts->GetNPtBins(); i++)
  {
    ...
    if (fSys == 0)
    { //histograms filled only in pp to save time in PbPb
      if (fFillVarHists)
      {
  ...
  if (fFillSparses)
  {
    ...
    for (Int_t iax = 0; iax < 9; iax++)
      fhStudyImpParSingleTrackCand->GetAxis(iax)->SetTitle(axTit[iax].Data());
  ...
  if (fReadMC)
  {
    ...
    if (fFillSparses)
    {
      ...
      for (Int_t iax = 0; iax < 8; iax++)
        fhStudyImpParSingleTrackSign->GetAxis(iax)->SetTitle(axTitMC[iax].Data());
      ...
      for (Int_t iax = 0; iax < 8; iax++)
        fhStudyImpParSingleTrackFd->GetAxis(iax)->SetTitle(axTitMC[iax].Data());
  ...
  if (fReadMC)
    ...
    fNentries->GetXaxis()->SetBinLabel(3, "Dstar<-D0");
  ...
  if (fSys == 1)
    fNentries->GetXaxis()->SetBinLabel(16, "Nev in centr");
  ...
  if (fEnableCentralityCorrCuts)
    fNentries->GetXaxis()->SetBinLabel(25, "n. rejected for bad cent corr");
  ...
  if (fWriteProtosgnVar)
  {
    ...
    if (fReadMC)
      fCandidateVariableNames[16] = "ispromptc";
    for (Int_t ivar = 0; ivar < nVar; ivar++)
    {
      fVariablesTree->Branch(fCandidateVariableNames[ivar].Data(), &fCandidateVariables[ivar], Form("%s/d", fCandidateVariableNames[ivar].Data()));
    }
...

{
  ...
  for (Int_t itrack = 0; itrack < ntracksEv; itrack++)
  { // loop on tacks
    //    ... get the track
  ...
  if (fhMultVZEROTPCoutTrackCorrNoCut)
    fhMultVZEROTPCoutTrackCorrNoCut->Fill(nTPCout, mTotV0);
  ...
  if (fhMultVZEROTPCclustersCorrNoCut)
    fhMultVZEROTPCclustersCorrNoCut->Fill(nTPCcls, mTotV0);
  Float_t mV0TPCclsCut = -2000. + (0.013 * nTPCcls) + (1.25e-9 * nTPCcls * nTPCcls);
  if (fEnablePileupRejVZEROTPCcls)
  { // this pile-up rejection is specific for 2018 Pb-Pb analysis
    if (fRejectOutOfBunchPileUp && mTotV0 < mV0TPCclsCut)
      return;
    else if (!fRejectOutOfBunchPileUp && mTotV0 > mV0TPCclsCut)
      return; //keep only out-of-bunch pile-up events
  }
  if (fhMultVZEROTPCclustersCorr)
    fhMultVZEROTPCclustersCorr->Fill(nTPCcls, mTotV0);
  ...
  if (!fCuts->IsEventSelected(aod))
  {
    if (fCuts->GetWhyRejection() == 1) // rejected for pileup
      fNentries->Fill(13);
    if (fSys == 1 && (fCuts->GetWhyRejection() == 2 || fCuts->GetWhyRejection() == 3))
      fNentries->Fill(15);
    if (fCuts->GetWhyRejection() == 7)
      fNentries->Fill(17);
    return;
  }
  ...
  if (fhMultVZEROTPCoutTrackCorr)
    fhMultVZEROTPCoutTrackCorr->Fill(nTPCout, mTotV0);
  ...
  if (fSys == 1 && (fWriteProtosgnVar || fWriteVariableTree))
  {
    if (!fUseRejectionMethod)
      AliFatal("FATAL: Fill candidates variables in Pb-Pb w/o Rejection Method --> ACTIVATE IT!");
    if (fUseRejectionMethod)
      AliWarning("WARNING: Fill candidates variables in Pb-Pb with Rejection Method --> Check a proper rejection factor is used!");
  }
  ...
  for (Int_t iD0toKpi = 0; iD0toKpi < nInD0toKpi; iD0toKpi++)
  {
    ...
    if (d->GetIsFilled() == 0)
      fNentries->Fill(19); //tmp check
    if (d->GetIsFilled() == 1)
      fNentries->Fill(20); //tmp check
    ...
    if (fCuts->IsInFiducialAcceptance(d->Pt(), d->Y(421)))
    {
      ...
      if (fSys == 0)
      {
        if (fCuts->IsSelected(d, AliRDHFCuts::kTracks, aod))
          fNentries->Fill(6);
      }
      ...
      if (fPIDCheck)
      {
        ...
        if (!fReadMC || (fReadMC && fUsePid4Distr))
          isSelectedPIDfill = fCuts->IsSelectedPID(d); //0 rejected,1 D0,2 Dobar, 3 both
        if (isSelectedPIDfill == 0)
          fNentries->Fill(7);
        if (isSelectedPIDfill == 1)
          fNentries->Fill(8);
        if (isSelectedPIDfill == 2)
          fNentries->Fill(9);
        if (isSelectedPIDfill == 3)
          fNentries->Fill(10);
      }
...
void AliAnalysisTaskSED0Mass::DrawDetSignal(AliAODRecoDecayHF2Prong *part, TList *ListDetSignal)
{
  ...
  Int_t isSelectedPIDfill = 3;
  if (!fReadMC || (fReadMC && fUsePid4Distr))
    isSelectedPIDfill = fCuts->IsSelectedPID(part); //0 rejected,1 D0,2 Dobar, 3 both
  ...
  if (isSelectedPIDfill != 0)
  { //fill "After PID" with everything that's not rejected
...
void AliAnalysisTaskSED0Mass::FillVarHists(AliAODEvent *aod, AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliRDHFCutsD0toKpi *cuts, TList *listout)
{
  ...
  if (!fPIDCheck)
  { //if fPIDCheck, this is already filled elsewhere
    if (isSelectedPID == 0)
      fNentries->Fill(7);
    if (isSelectedPID == 1)
      fNentries->Fill(8);
  ...
  if (fCutOnDistr && !fIsSelectedCandidate)
    return;
  ...
  if (!fUsePid4Distr)
    isSelectedPID = 0;
  if ((lab >= 0 && fReadMC) || (!fReadMC && isSelectedPID))
  { //signal (from MC or PID)
    ...
    if (fSys == 0)
    {
      ...
      if ((TMath::Abs(pdgProng[0]) == 211 && TMath::Abs(pdgProng[1]) == 321) || (isSelectedPID == 1 || isSelectedPID == 3))
      {
        ((TH1F *)listout->FindObject(fillthispi))->Fill(prong0->Pt());
        ((TH1F *)listout->FindObject(fillthisK))->Fill(prong1->Pt());
      }

      if ((TMath::Abs(pdgProng[0]) == 321 && TMath::Abs(pdgProng[1]) == 211) || (isSelectedPID == 2 || isSelectedPID == 3))
      {
    ...
    if ((fReadMC && ((AliAODMCParticle *)arrMC->At(lab))->GetPdgCode() == 421) || (!fReadMC && (isSelectedPID == 1 || isSelectedPID == 3)))
    { //D0
      ((TH1F *)listout->FindObject(fillthis))->Fill(minvD0);
      if (fFillPtHist && fReadMC)
        ((TH2F *)fOutputMassPt->FindObject(fillthispt))->Fill(minvD0, pt);
    ...
    { //D0bar
      if (fReadMC || (!fReadMC && isSelectedPID > 1))
      {
        ...
        if (fFillPtHist && fReadMC)
          ((TH2F *)fOutputMassPt->FindObject(fillthispt))->Fill(minvD0bar, pt);
    ...
    if (TMath::Abs(minvD0 - mPDG) < invmasscut || TMath::Abs(minvD0bar - mPDG) < invmasscut)
    {
      ...
      if (fSys == 0)
      {
        ...
        if ((fReadMC && ((AliAODMCParticle *)arrMC->At(lab))->GetPdgCode() == 421))
          ((TH1F *)listout->FindObject(fillthis))->Fill(cosThetaStarD0);
        else
        {
          if (fReadMC || isSelectedPID > 1)
            ((TH1F *)listout->FindObject(fillthis))->Fill(cosThetaStarD0bar);
      ...
      if ((fReadMC && ((AliAODMCParticle *)arrMC->At(lab))->GetPdgCode() == 421))
      {
        for (Int_t it = 0; it < 2; it++)
        {
          ...
          {
            if (((AliAODTrack *)fDaughterTracks.UncheckedAt(it))->HasPointOnITSLayer(il))
              nPointsITS++;
          }
          ...
          fillthis += ptbin;
          if (!(((AliAODTrack *)fDaughterTracks.UncheckedAt(it))->HasPointOnITSLayer(0)) && !(((AliAODTrack *)fDaughterTracks.UncheckedAt(it))->HasPointOnITSLayer(1)))
          { //no SPD points
  ...
  { //Background or LS
    ...
    if (TMath::Abs(minvD0 - mPDG) < invmasscut || TMath::Abs(minvD0bar - mPDG) < invmasscut)
    {
      if (fSys == 0)
      {
        ptProng[0] = ((AliAODTrack *)fDaughterTracks.UncheckedAt(0))->Pt();
        ptProng[1] = ((AliAODTrack *)fDaughterTracks.UncheckedAt(0))->Pt();
        cosThetaStarD0 = part->CosThetaStarD0();
        cosThetaStarD0bar = part->CosThetaStarD0bar();
      }
      ...
      if (fReadMC)
      {
        ...
        for (Int_t iprong = 0; iprong < 2; iprong++)
        {
          ...
          if (lab >= 0)
          {
            ...
            if (mcprong)
            {
              ...
              if (labmom >= 0)
              {
                ...
                if (mcmother)
                  pdgMother[iprong] = mcmother->GetPdgCode();
              }
            }
          }
          ...
          if (fSys == 0)
          {
            ...
            if (TMath::Abs(pdgMother[iprong]) == 310 || TMath::Abs(pdgMother[iprong]) == 130 || TMath::Abs(pdgMother[iprong]) == 321)
            { //K^0_S, K^0_L, K^+-
              if (ptProng[iprong] <= 1)
                factor[iprong] = 1. / .7;
              else
            ...
            if (TMath::Abs(pdgMother[iprong]) == 3122)
            { //Lambda
...
void AliAnalysisTaskSED0Mass::FillMassHists(AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliAODMCHeader *mcHeader, AliRDHFCutsD0toKpi *cuts, TList *listout)
{
  ...
  if (fReadMC)
    labD0 = part->MatchToMC(421, arrMC, 2, pdgDgD0toKpi); //return MC particle label if the array corresponds to a D0, -1 if not (cf. AliAODRecoDecay.cxx)
  ...
  if (fCuts->GetCombPID() && (fCuts->GetBayesianStrategy() == AliRDHFCutsD0toKpi::kBayesWeight || fCuts->GetBayesianStrategy() == AliRDHFCutsD0toKpi::kBayesWeightNoFilter))
  {
    ...
    if (weigD0 < 0. || weigD0 > 1.0)
    {
      weigD0 = 0.;
    }
    if (weigD0bar < 0. || weigD0bar > 1.0)
    {
      weigD0bar = 0.;
    } //Prevents filling with weight > 1, or < 0
  }
  ...
  if (fFillForPIDcase)
  {
    if (fIsSelectedCandidate == 1 && fFillOnlyD0D0bar == 1)
    { //D0

      arrayForSparse[0] = invmassD0;
      arrayForSparse[2] = impparXY;
      ...
      if (fReadMC)
      {
        if (labD0 >= 0)
        {
          ...
          if (AliVertexingHFUtils::CheckOrigin(arrMC, partD0, fUseQuarkTagInKine) == 5)
            isPrimary = kFALSE;
          if (!isPrimary)
            trueImpParXY = GetTrueImpactParameter(mcHeader, arrMC, partD0) * 10000.;
          arrayForSparseTrue[0] = invmassD0;
          arrayForSparseTrue[2] = trueImpParXY;
          ...
          { //D0
            ...
            {
              if (isPrimary)
                fHistMassPtImpParTC[1]->Fill(arrayForSparse, weigD0);
              else
              {
        ...
        { //background
          ...
          if (fFillImpParHist)
            fHistMassPtImpParTC[4]->Fill(arrayForSparse, weigD0);
    ...
    if (fIsSelectedCandidate == 2 && fFillOnlyD0D0bar == 2)
    { //D0bar

      arrayForSparse[0] = invmassD0bar;
      arrayForSparse[2] = impparXY;
      ...
      if (fReadMC)
      {
        if (labD0 >= 0)
        {
          ...
          if (!isPrimary)
            trueImpParXY = GetTrueImpactParameter(mcHeader, arrMC, partD0) * 10000.;
          arrayForSparseTrue[0] = invmassD0bar;
          arrayForSparseTrue[2] = trueImpParXY;
          ...
          { //D0bar
            ...
            if (fFillImpParHist)
            {
              ...
              if (isPrimary)
                fHistMassPtImpParTC[1]->Fill(arrayForSparse, weigD0bar);
              else
              {
        ...
        { //background or LS
          ...
          if (fFillImpParHist)
            fHistMassPtImpParTC[4]->Fill(arrayForSparse, weigD0bar);
          if (fFillYHist)
          {
            fillthismassy = "histBkgY_";
            fillthismassy += ptbin;
      ...
      else
      {
        ...
        if (fFillImpParHist)
          fHistMassPtImpParTC[0]->Fill(arrayForSparse, weigD0bar);
        if (fFillYHist)
        {
          fillthismassy = "histMassY_";
          fillthismassy += ptbin;
    ...
    if (fIsSelectedCandidate == 3 && fFillOnlyD0D0bar == 0)
    {
      //D0
...
void AliAnalysisTaskSED0Mass::FillCandVariables(AliAODEvent *aodev, AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliAODMCHeader *mcHeader, AliRDHFCutsD0toKpi *cuts)
{
  ...
  if (fWriteProtosgnVar)
  { //write candidate variables for proto-significance study
    if ((fIsSelectedCandidate == 1 || fIsSelectedCandidate == 3) && fFillOnlyD0D0bar < 2)
    { //D0
      isD0sel = true;
      if (fReadMC)
      {
        if (labD0 >= 0)
        {
          ...
          { // D0
            if (fDebug > 2)
              cout << "MC: D0 candidate" << endl;
            isrealD0 = true;
            if (fUsedMassWindow && (part->InvMassD0() < 1.7 || part->InvMassD0() > 2.1))
              invmassD0 = 0;
          }
          else
          { // it was a D0bar
            if (fDebug > 2)
              cout << "MC: D0bar candidate selected as D0 --> reflection" << endl;
          }
        }
        else
        { // background
    ...
    { //D0bar
      ...
      if (fReadMC)
      {
        if (labD0 >= 0)
        {
          ...
          { // D0bar
            if (fDebug > 2)
              cout << "MC: D0bar candidate" << endl;
            isrealD0bar = true;
            if (fUsedMassWindow && (part->InvMassD0bar() < 1.7 || part->InvMassD0bar() > 2.1))
              invmassD0bar = 0;
          }
          else
          {
            if (fDebug > 2)
              cout << "MC: D0 candidate selected as D0bar --> reflection" << endl;
          }
        }
        else
        {
    ...
    if (!fUsedMassWindow)
      AliWarning("WARNING: Mass window selection NOT used!");

    if (fReadMC && fSelectTrueD0)
    {
      if (!isrealD0 && !isrealD0bar)
        return;
      if (isD0sel && isrealD0)
        selCand = 0;
      if (isD0barsel && isrealD0bar)
        selCand = 1;
    }
    else
    {
    ...
    {
      if (isPrimary)
        fCandidateVariables[16] = 4;
      else
        fCandidateVariables[16] = 5;
    }
    fVariablesTree->Fill();
  }
}
...
void AliAnalysisTaskSED0Mass::Terminate(Option_t * /*option*/)
{
  ...
  //
  if (fDebug > 1)
    printf("AnalysisTaskSED0Mass: Terminate() \n");
  ...
  if (fReadMC)
  {
    ...
    if (lab >= 0)
    {
      ...
      if (orig == 4)
        ...
        signalType = 2; //feed down
      //pt, ptB, normImpParTrk1, normImpParTrk2, decLXY, normDecLXY, iscut, ispid
      if (ispid)
        fCuts->SetUsePID(kFALSE); // if PID on, switch it off
      Int_t isCuts = fCuts->IsSelected(part, AliRDHFCuts::kAll, aod);
      if (ispid)
        fCuts->SetUsePID(kTRUE); //if PID was on, switch it on
      if (!isCuts)
        return;
      ...
      if (signalType == 1)
        fhStudyImpParSingleTrackSign->Fill(pointD0MC);
      if (signalType == 2)
      {
