AliAnalysisVertexingHF* ConfigVertexingHF() {
  ...
  printf("MYCONFIGPID from PbPb Call to AliAnalysisVertexingHF parameters setting :\n");
  ...
  vHF->SetV0TypeForCascadeVertex(AliRDHFCuts::kOnlyOfflineV0s); //All V0s 0, Offline 1, OnTheFly 2 //as in Config_Pb_AllCent
   ...
   // set PID
   // from PWGHF/vertexingHF/upgrade/ConfigVertexingHF_ITSUpgrade_D0Lc_wPID.C
   ...
   vHF->SetnSigmaTOFforProtonSel(5.,5.);
   vHF->SetnSigmaTPCforProtonSel(5., 5.);
   vHF->SetnSigmaTPCforPionSel(5., 5.);
   vHF->SetnSigmaTOFforPionSel(5.,5.);
...
//--- set cuts for single-track selection  
 ...
 //5-999
  ...
  cutsDStartoKpipi->SetUseCentrality(kFALSE);
