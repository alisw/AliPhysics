...
class AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP;    // your analysis class

ClassImp(AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP) // classimp: necessary for root

AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP() :
  AliAnalysisTaskSE(),
  fIsMC(kFALSE),
  fPID(0),
  fAnaCuts(0),
  ...
  fHTrigger(0),
  fHCentrality(0),
  fWriteXiccPPTree(kFALSE),
  fWriteXiccPPMCGenTree(kFALSE)
{
    // default constructor, don't allocate memory here!
    // this is used by root for IO purposes, it needs to remain empty
}
//_____________________________________________________________________________
AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP(const char* name, AliRDHFCutsKFP* cuts) :
  ...
  fWriteXiccPPMCGenTree(kFALSE)
{
  ...
  DefineOutput(4, TTree::Class()); // XiccPP
  DefineOutput(5, TTree::Class()); // XiccPP MCGen
  DefineOutput(6, TList::Class()); // XiccPP event & trigger
...
AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::~AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP()
{
    ...
    if (fTree_XiccPPMCGen) {
      delete fTree_XiccPPMCGen;
      fTree_XiccPPMCGen = 0;
    }

    if (fVar_XiccPPMCGen) {
      delete fVar_XiccPPMCGen;
      fVar_XiccPPMCGen = 0;
    }
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::Init()
{
  // Initialization
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::UserCreateOutputObjects()
{
  ...
  DefineTreeRecXiccPP();
  PostData(4, fTree_XiccPP);

  DefineTreeGenXiccPP();
  PostData(5, fTree_XiccPPMCGen);
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::UserExec(Option_t *)
{
  // user exec
  ...
  if (fIsMC) {
    ...
    if ( !mcHeader ) {
      AliError("AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::UserExec: MC header branch not found!\n");
      return;
    }
...
//------------------------------------------------
  ...
  MakeAnaXiccPPFromCasc(AODEvent, mcArray, PV);
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::Terminate(Option_t *)
{
    // terminate
...
//_____________________________________________________________________________
Bool_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MakeMCAnalysis(TClonesArray *mcArray)
{
  ...
  for(Int_t iPart=0; iPart<mcArray->GetEntriesFast(); iPart++) {
    ...
    if ( TMath::Abs(pdg)!=4422 ) { // Check Xicc++ pdg
      AliDebug(2, Form("MC particle %d is not a Xicc++: its pdg code is %d", iPart, pdg));
      continue;
    }
    AliDebug(2, Form("Step 0 ok: MC particle %d is a Xicc++: its pdg code is %d", iPart, pdg));
    if ( mcPart->GetNDaughters()!=2 ) {
      AliDebug(2, "Xicc++ does not have 2 daughters");
      continue;
    }
    Int_t index_FirstDau = mcPart->GetDaughterFirst();
    if (index_FirstDau<0) continue;
    AliAODMCParticle* mcXiccPPDau_0 = dynamic_cast<AliAODMCParticle*>(mcArray->At(index_FirstDau));
    AliAODMCParticle* mcXiccPPDau_1 = dynamic_cast<AliAODMCParticle*>(mcArray->At(index_FirstDau+1));

    if ( TMath::Abs(mcXiccPPDau_0->GetPdgCode())!=211 && TMath::Abs(mcXiccPPDau_0->GetPdgCode())!=4232 ) {
      AliDebug(2, Form("First daughter of Xicc++ is not pi and Xic+: its pdg code is %d", mcXiccPPDau_0->GetPdgCode()));
      continue;
    }
    if ( TMath::Abs(mcXiccPPDau_1->GetPdgCode())!=211 && TMath::Abs(mcXiccPPDau_1->GetPdgCode())!=4232 ) {
      AliDebug(2, Form("First daughter of Xicc++ is not pi and Xic+: its pdg code is %d", mcXiccPPDau_1->GetPdgCode()));
      continue;
    }

    AliAODMCParticle* mcXicPlus = NULL;
    if ( TMath::Abs(mcXiccPPDau_0->GetPdgCode())==4232 ) mcXicPlus = mcXiccPPDau_0;
    if ( TMath::Abs(mcXiccPPDau_1->GetPdgCode())==4232 ) mcXicPlus = mcXiccPPDau_1;

    if ( mcXicPlus->GetNDaughters()!=3 ) {
      AliDebug(2, "Xic+ does not have 3 daughters");
      continue;
    }
    index_FirstDau = mcXicPlus->GetDaughterFirst();
    if (index_FirstDau<0) continue;
    ...
    if ( TMath::Abs(mcXicPlusDau_0->GetPdgCode())==3312 && TMath::Abs(mcXicPlusDau_1->GetPdgCode())==211 && TMath::Abs(mcXicPlusDau_2->GetPdgCode())==211 ) { // First is Xi
      ...
      if ( pifromXi_flag && v0_flag && pifromLam_flag && prfromLam_flag ) {
        if ( TMath::Abs(mcPart->Y()) < 0.8 ) { // Acceptance
          ...
          FillTreeGenXiccPP(mcPart, CheckOrigin);
        }
      }
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::FillTreeGenXiccPP(AliAODMCParticle *mcpart, Int_t CheckOrigin)
{
  // Fill histograms or tree depending

  for(Int_t i=0;i<4;i++){
    fVar_XiccPPMCGen[i] = -9999.;
  }

  fVar_XiccPPMCGen[0] = mcpart->Y();
  fVar_XiccPPMCGen[1] = mcpart->Pt();
  fVar_XiccPPMCGen[2] = CheckOrigin;
  fVar_XiccPPMCGen[3] = mcpart->GetPdgCode();

  if (fWriteXiccPPMCGenTree && fVar_XiccPPMCGen[1]>0.9999) fTree_XiccPPMCGen->Fill();

//  fVar_XiccPPMCGen[ 0] = fCentrality;
//  fVar_XiccPPMCGen[ 1] = decaytype;
//  if (mcpart->IsPrimary() && (!mcpart->IsPhysicalPrimary())) fVar_XiccPPMCGen[2] = 1;
//  if (mcpart->IsPhysicalPrimary()) fVar_XiccPPMCGen[2] = 2;
//  if (mcpart->IsSecondaryFromWeakDecay()) fVar_XiccPPMCGen[2] = 3;
//  if (mcpart->IsSecondaryFromMaterial()) fVar_XiccPPMCGen[2] = 4;
//  if (mcpart->IsFromSubsidiaryEvent()) fVar_XiccPPMCGen[2] = 5;
//  fVar_XiccPPMCGen[ 3] = mcpart->Eta();
//  fVar_XiccPPMCGen[ 4] = mcpart->Y();
//  fVar_XiccPPMCGen[ 5] = mcpart->Px();
//  fVar_XiccPPMCGen[ 6] = mcpart->Py();
//  fVar_XiccPPMCGen[ 7] = mcpart->Pz();
//  fVar_XiccPPMCGen[ 8] = mcpipart->Px();
//  fVar_XiccPPMCGen[ 9] = mcpipart->Py();
//  fVar_XiccPPMCGen[10] = mcpipart->Pz();
//  fVar_XiccPPMCGen[11] = mccascpart->Px();
//  fVar_XiccPPMCGen[12] = mccascpart->Py();
//  fVar_XiccPPMCGen[13] = mccascpart->Pz();
//  fVar_XiccPPMCGen[14] = mcpart->GetPdgCode();
//  fVar_XiccPPMCGen[15] = mcpipart->GetPdgCode();
//  fVar_XiccPPMCGen[16] = mccascpart->GetPdgCode();
//  fVar_XiccPPMCGen[17] = fRunNumber;
//  fVar_XiccPPMCGen[18] = fEvNumberCounter;
//  fVar_XiccPPMCGen[19] = CheckOrigin;
...
//  Double_t piE  = sqrt(pipx*pipx+pipy*pipy+pipz*pipz+0.000511*0.000511);
  ...
  Double_t contXiccPPMC[3];
  contXiccPPMC[0] = mcpart->Pt();
  contXiccPPMC[1] = mcpart->Y();
  contXiccPPMC[2] = fCentrality;
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MakeAnaXiccPPFromCasc(AliAODEvent *AODEvent, TClonesArray *mcArray, KFParticle PV)
{
...
//  std::cout.precision(3);
  ...
  const Float_t massXicPlus = TDatabasePDG::Instance()->GetParticle(4232)->Mass();
...
//          if ( kfpXicPlus.GetPt() < fAnaCuts->GetPtMinXicPlus() ) continue;
          ...
          // mass window cut of XicPlus
          if ( TMath::Abs(massXicPlus_Rec-massXicPlus) > (fAnaCuts->GetProdMassTolXicPlus()) ) continue;

          KFParticle kfpXicPlus_m = kfpXicPlus;
          kfpXicPlus_m.SetNonlinearMassConstraint(massXicPlus);

          if ( !AliVertexingHFUtils::CheckKFParticleCov(kfpXicPlus_m) || TMath::Abs(kfpXicPlus_m.GetE()) <= TMath::Abs(kfpXicPlus_m.GetPz()) ) continue;


          // === reconstruction of Xicc++ ===
          for (Int_t itrkBP_trk3=0; itrkBP_trk3<flag_trkP; itrkBP_trk3++) { // Loop for third bachelor pion+

            if ( trackP[itrkBP_trk3]->GetID() == ptrack->GetID() || trackP[itrkBP_trk3]->GetID() == trackP[itrkBP_trk1]->GetID() || trackP[itrkBP_trk3]->GetID() == trackP[itrkBP_trk2]->GetID() ) continue;

            if ( !fAnaCuts->PassedTrackQualityCuts_PrimaryPion(trackP[itrkBP_trk3]) ) continue;

            KFParticle kfpBP_trk3 = AliVertexingHFUtils::CreateKFParticleFromAODtrack(trackP[itrkBP_trk3], 211);

            KFParticle kfpXiccPP;
            const KFParticle *vXiccPPDs[2] = {&kfpXicPlus_m, &kfpBP_trk3};
            kfpXiccPP.Construct(vXiccPPDs, 2);

            // chi2>0 && NDF>0
            if ( kfpXiccPP.GetNDF()<=0 || kfpXiccPP.GetChi2()<=0 ) continue;

            // Prefilter
            if ( kfpXiccPP.GetChi2()/kfpXiccPP.GetNDF() >= fAnaCuts->GetKFPXiccPP_Chi2geoMax() ) continue;
            if ( kfpXiccPP.GetPt() < fAnaCuts->GetPtMinXiccPP() ) continue;

            // check rapidity of XiccPP
            if ( TMath::Abs(kfpXiccPP.GetE())<=TMath::Abs(kfpXiccPP.GetPz()) ) continue;

            // check covariance matrix
            if ( !AliVertexingHFUtils::CheckKFParticleCov(kfpXiccPP) ) continue;

            // err_massXiccPP > 0
            Float_t massXiccPP_Rec, err_massXiccPP;
            kfpXiccPP.GetMass(massXiccPP_Rec, err_massXiccPP);
            if ( err_massXiccPP<=0 ) continue;

            if (fWriteXiccPPTree) {
              Int_t lab_XiccPP = -9999;
              if (fIsMC) {
                lab_XiccPP = MatchToMCXiccPP(ptrack, ntrack, btrack, trackP[itrkBP_trk1], trackP[itrkBP_trk2], trackP[itrkBP_trk3], mcArray);
                if (lab_XiccPP>=0) FillTreeRecXiccPPFromCasc(kfpXiccPP, kfpXicPlus, trackP[itrkBP_trk3], kfpBP_trk3, trackP[itrkBP_trk1], kfpBP_trk1, kfpXiMinus, kfpXiMinus_m, kfpPionOrKaon, btrack, kfpK0Short, kfpGamma, kfpLambda, kfpLambda_m, ptrack, ntrack, trackP[itrkBP_trk2], kfpBP_trk2, kfpProton, kfpPionMinus, PV, mcArray, lab_XiccPP);
              }
              if (!fIsMC) {
                FillTreeRecXiccPPFromCasc(kfpXiccPP, kfpXicPlus, trackP[itrkBP_trk3], kfpBP_trk3, trackP[itrkBP_trk1], kfpBP_trk1, kfpXiMinus, kfpXiMinus_m, kfpPionOrKaon, btrack, kfpK0Short, kfpGamma, kfpLambda, kfpLambda_m, ptrack, ntrack, trackP[itrkBP_trk2], kfpBP_trk2, kfpProton, kfpPionMinus, PV, mcArray, lab_XiccPP);
              }
            }
            kfpBP_trk3.Clear();
          }
...
//          if ( kfpAntiXicPlus.GetPt() < fAnaCuts->GetPtMinXicPlus() ) continue;
          ...
          // mass window cut of AntiXicPlus
          if ( TMath::Abs(massAntiXicPlus_Rec-massXicPlus) > (fAnaCuts->GetProdMassTolXicPlus()) ) continue;

          KFParticle kfpAntiXicPlus_m = kfpAntiXicPlus;
          kfpAntiXicPlus_m.SetNonlinearMassConstraint(massXicPlus);

          if ( !AliVertexingHFUtils::CheckKFParticleCov(kfpAntiXicPlus_m) || TMath::Abs(kfpAntiXicPlus_m.GetE()) <= TMath::Abs(kfpAntiXicPlus_m.GetPz()) ) continue;


          // === reconstruction of anti-Xicc++ ===
          for (Int_t itrkBP_trk3=0; itrkBP_trk3<flag_trkN; itrkBP_trk3++) { // Loop for third bachelor pion-

            if ( trackN[itrkBP_trk3]->GetID() == ntrack->GetID() || trackN[itrkBP_trk3]->GetID() == trackN[itrkBP_trk1]->GetID() || trackN[itrkBP_trk3]->GetID() == trackN[itrkBP_trk2]->GetID() ) continue;

            if ( !fAnaCuts->PassedTrackQualityCuts_PrimaryPion(trackN[itrkBP_trk3]) ) continue;

            KFParticle kfpBP_trk3 = AliVertexingHFUtils::CreateKFParticleFromAODtrack(trackN[itrkBP_trk3], -211);

            KFParticle kfpAntiXiccPP;
            const KFParticle *vAntiXiccPPDs[2] = {&kfpAntiXicPlus_m, &kfpBP_trk3};
            kfpAntiXiccPP.Construct(vAntiXiccPPDs, 2);

            // chi2>0 && NDF>0
            if ( kfpAntiXiccPP.GetNDF()<=0 || kfpAntiXiccPP.GetChi2()<=0 ) continue;

            // Prefilter
            if ( kfpAntiXiccPP.GetChi2()/kfpAntiXiccPP.GetNDF() >= fAnaCuts->GetKFPXiccPP_Chi2geoMax() ) continue;
            if ( kfpAntiXiccPP.GetPt() < fAnaCuts->GetPtMinXiccPP() ) continue;

            // check rapidity of Anti-XiccPP
            if ( TMath::Abs(kfpAntiXiccPP.GetE())<=TMath::Abs(kfpAntiXiccPP.GetPz()) ) continue;

            // check covariance matrix
            if ( !AliVertexingHFUtils::CheckKFParticleCov(kfpAntiXiccPP) ) continue;

            // err_massAntiXiccPP > 0
            Float_t massAntiXiccPP_Rec, err_massAntiXiccPP;
            kfpAntiXiccPP.GetMass(massAntiXiccPP_Rec, err_massAntiXiccPP);
            if ( err_massAntiXiccPP<=0 ) continue;

            if (fWriteXiccPPTree) {
              Int_t lab_AntiXiccPP = -9999.;
              if (fIsMC) {
                lab_AntiXiccPP = MatchToMCAntiXiccPP(ntrack, ptrack, btrack, trackN[itrkBP_trk1], trackN[itrkBP_trk2], trackN[itrkBP_trk3], mcArray);
                if (lab_AntiXiccPP>=0) FillTreeRecXiccPPFromCasc(kfpAntiXiccPP, kfpAntiXicPlus, trackN[itrkBP_trk3], kfpBP_trk3, trackN[itrkBP_trk1], kfpBP_trk1, kfpXiPlus, kfpXiPlus_m, kfpPionOrKaon, btrack, kfpK0Short, kfpGamma, kfpAntiLambda, kfpAntiLambda_m, ntrack, ptrack, trackN[itrkBP_trk2], kfpBP_trk2, kfpAntiProton, kfpPionPlus, PV, mcArray, lab_AntiXiccPP);
              }
              if (!fIsMC) {
                FillTreeRecXiccPPFromCasc(kfpAntiXiccPP, kfpAntiXicPlus, trackN[itrkBP_trk3], kfpBP_trk3, trackN[itrkBP_trk1], kfpBP_trk1, kfpXiPlus, kfpXiPlus_m, kfpPionOrKaon, btrack, kfpK0Short, kfpGamma, kfpAntiLambda, kfpAntiLambda_m, ntrack, ptrack, trackN[itrkBP_trk2], kfpBP_trk2, kfpAntiProton, kfpPionPlus, PV, mcArray, lab_AntiXiccPP);
              }
            }
            kfpBP_trk3.Clear();
          }
          kfpAntiXicPlus_m.Clear();
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCXiccPP(AliAODTrack *trackProton, AliAODTrack *trackPionMinus_2, AliAODTrack *trackPionMinus_1, AliAODTrack *trackPionPlus_trk1, AliAODTrack *trackPionPlus_trk2, AliAODTrack *trackPionPlus_trk3, TClonesArray *mcArray)
{
  ...
  Int_t labelPionPlus_trk3  = fabs(trackPionPlus_trk3->GetLabel());
  if (labelPionPlus_trk3<0) return -1;
  AliAODMCParticle* mcPionPlus_trk3 = static_cast<AliAODMCParticle*>(mcArray->At(labelPionPlus_trk3));

  if ( mcProton->GetPdgCode() != 2212 || mcPionMinus_2->GetPdgCode() != -211 || mcPionMinus_1->GetPdgCode() != -211 || mcPionPlus_trk1->GetPdgCode() != 211 || mcPionPlus_trk2->GetPdgCode() != 211 || mcPionPlus_trk3->GetPdgCode() != 211 ) return -1; // check pdg
  ...
  if ( mcMother->GetPdgCode() != 4232 || mcMother->GetNDaughters()!=3 ) return -1; // check mother is Xic+
  // ==================

  // === check Xicc++ ===
  IndexMother[0] = mcMother->GetMother(); // mother of XicPlus
  IndexMother[1] = mcPionPlus_trk3->GetMother();
  if ( IndexMother[0]<0 || IndexMother[1]<0 ) return -1; // check mother exist
  if ( IndexMother[0] != IndexMother[1] ) return -1; // check the same mother
  mcMother = static_cast<AliAODMCParticle*>(mcArray->At(IndexMother[0]));
  if ( mcMother->GetPdgCode() != 4422 || mcMother->GetNDaughters()!=2 ) return -1; // check mother is Xicc++
  // ==================
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCAntiXiccPP(AliAODTrack *trackAntiProton, AliAODTrack *trackPionPlus_2, AliAODTrack *trackPionPlus_1, AliAODTrack *trackPionMinus_trk1, AliAODTrack *trackPionMinus_trk2, AliAODTrack *trackPionMinus_trk3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
  ...
  Int_t labelPionMinus_trk3  = fabs(trackPionMinus_trk3->GetLabel());
  if (labelPionMinus_trk3<0) return -1;
  AliAODMCParticle* mcPionMinus_trk3 = static_cast<AliAODMCParticle*>(mcArray->At(labelPionMinus_trk3));

  if ( mcAntiProton->GetPdgCode() != -2212 || mcPionPlus_2->GetPdgCode() != 211 || mcPionPlus_1->GetPdgCode() != 211 || mcPionMinus_trk1->GetPdgCode() != -211 || mcPionMinus_trk2->GetPdgCode() != -211 || mcPionMinus_trk3->GetPdgCode() ) return -1; // check pdg
  ...
  if ( IndexMother_XicPlusDau[0] != IndexMother_XicPlusDau[1] || IndexMother_XicPlusDau[0] != IndexMother_XicPlusDau[2] ) return -1; // check the same mother
  mcMother = static_cast<AliAODMCParticle*>(mcArray->At(IndexMother_XicPlusDau[0]));
  if ( mcMother->GetPdgCode() != -4232 || mcMother->GetNDaughters()!=3 ) return -1; // check mother is Anti-Xic+
  // ==================

  // === check Anti-Xicc++ ===
  IndexMother[0] = mcMother->GetMother(); // mother of XicPlus
  IndexMother[1] = mcPionMinus_trk3->GetMother();
  if ( IndexMother[0]<0 || IndexMother[1]<0 ) return -1; // check mother exist
  if ( IndexMother[0] != IndexMother[1] ) return -1; // check the same mother
  mcMother = static_cast<AliAODMCParticle*>(mcArray->At(IndexMother[0]));
  if ( mcMother->GetPdgCode() != -4422 || mcMother->GetNDaughters()!=2 ) return -1; // check mother is Anti-Xicc++
  // ==================
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::SelectTrack(AliVEvent *event, Int_t trkEntries, Int_t &nSeleTrks, Bool_t *seleFlags)
{
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCXiMinus(AliAODTrack *trackProton, AliAODTrack *trackPion3, AliAODTrack *trackPion2, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCXiPlus(AliAODTrack *trackAntiProton, AliAODTrack *trackAntiPion3, AliAODTrack *trackAntiPion2, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCLambda(AliAODTrack *trackProton, AliAODTrack *trackPion3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCLambdaFromXi(AliAODTrack *trackProton, AliAODTrack *trackPion3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCAntiLambda(AliAODTrack *trackAntiProton, AliAODTrack *trackAntiPion3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCAntiLambdaFromXi(AliAODTrack *trackAntiProton, AliAODTrack *trackAntiPion3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToMCPion(AliAODTrack *track, TClonesArray *mcArray)
{
...
/*
//_____________________________________________________________________________
Int_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::MatchToXiccPPMC(TClonesArray *mcArray, Int_t PDGXiccPP, const Int_t nDaughters, const Int_t *daughterIndex, const Int_t *daughterPDG)
{
  ///
  /// Check if this candidate is matched to a MC signal XiccPP
  ...
  for(Int_t i=0; i<nDaughters; i++) {
    ...
    while ( mother->GetMother()>=0 ) {
      ...
      if ( pdgMother==PDGXiccPP ) { // check mother is XiccPP
        IndexMom[i]=IndexMother;
        break;
      } else if( pdgMother>PDGXiccPP || pdgMother<10 ) {
        break;
      }
    }
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::DefineEvent()
{
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::DefineTreeRecXiccPP()
{
  // This is to define tree variables

  const char* nameoutput = GetOutputSlot(4)->GetContainer()->GetName();
  fTree_XiccPP = new TTree(nameoutput, "XiccPP variables tree");
  Int_t nVar = 56;
  fVar_XiccPP = new Float_t[nVar];
  TString *fVarNames = new TString[nVar];
  ...
  fVarNames[46] = "nSigmaTPC_PiFromXiccPP"; // TPC nsigma for pion coming from Xicc++
  fVarNames[47] = "nSigmaTOF_PiFromXiccPP"; // TOF nsigma for pion coming from Xicc++
  fVarNames[48] = "pt_PiFromXiccPP"; // pt of pion coming from Xicc++
  fVarNames[49] = "pt_XiccPP"; // pt of Xicc++
  fVarNames[50] = "rap_XiccPP"; // rapidity of Xicc++
  fVarNames[51] = "mass_XiccPP"; // mass of Xicc++
  fVarNames[52] = "chi2geo_XiccPP"; // chi2_geometry of Xicc++
  fVarNames[53] = "chi2prim_PiFromXiccPP"; // chi2_topo of pion from Xicc++
  fVarNames[54] = "DCAxy_PiFromXiccPPToPV_KF"; // DCA of pion coming from Xicc++ in x-y plane

  fVarNames[55] = "Source_XiccPP"; // flag for XiccPP MC truth (“4” prompt, "5" feed-down, “<0” background)
...
//  fVarNames[40] = "DCA_XicPlusDau_KF"; // DCA of XicPlus's daughters (calculated from KF after Xi mass constraint)
  ...
  for (Int_t ivar=0; ivar<nVar; ivar++) {
    fTree_XiccPP->Branch(fVarNames[ivar].Data(), &fVar_XiccPP[ivar], Form("%s/F", fVarNames[ivar].Data()));
  }
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::DefineTreeGenXiccPP()
{
  const char* nameoutput = GetOutputSlot(5)->GetContainer()->GetName();
  fTree_XiccPPMCGen = new TTree(nameoutput,"XiccPP MC variables tree");
  Int_t nVar = 4;
  fVar_XiccPPMCGen = new Float_t[nVar];
  TString *fVarNames = new TString[nVar];
  
  fVarNames[0] = "rap_XiccPP";
  fVarNames[1] = "pt_XiccPP";
  fVarNames[2] = "Source_XiccPP";
  fVarNames[3] = "PDG_XiccPP";
  ...
  */
  ...
  for (Int_t ivar=0; ivar<nVar; ivar++) {
    fTree_XiccPPMCGen->Branch(fVarNames[ivar].Data(),&fVar_XiccPPMCGen[ivar],Form("%s/F",fVarNames[ivar].Data()));
  }
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::InvMassV0atPV(AliAODTrack *trk1, AliAODTrack *trk2, Int_t pdg1, Int_t pdg2)
{
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::DefineAnaHist()
{
...
//_____________________________________________________________________________
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::FillEventROOTObjects()
{
...
void AliAnalysisTaskSEXiccPPToXicPlusPiPlusfromKFP::FillTreeRecXiccPPFromCasc(KFParticle kfpXiccPP, KFParticle kfpXicPlus, AliAODTrack *trackPiFromXiccPP, KFParticle kfpPiFromXiccPP, AliAODTrack *trackPiFromXicPlus_trk1, KFParticle kfpBP_trk1, KFParticle kfpXiMinus, KFParticle kfpXiMinus_m, KFParticle kfpPionOrKaon, AliAODTrack *trackPiFromXiOrKaonFromOmega, KFParticle kfpK0Short, KFParticle kfpGamma, KFParticle kfpLambda, KFParticle kfpLambda_m, AliAODTrack *trkProton, AliAODTrack *trkPion, AliAODTrack *trackPiFromXicPlus_trk2, KFParticle kfpBP_trk2, KFParticle kfpProtonFromLam, KFParticle kfpPionFromLam, KFParticle PV, TClonesArray *mcArray, Int_t lab_XiccPP)
{
  ...
  Float_t nSigmaTPC_PiFromXiccPP       = fPID->NumberOfSigmasTPC(trackPiFromXiccPP,AliPID::kPion);
  Float_t nSigmaTOF_PiFromXiccPP       = fPID->NumberOfSigmasTOF(trackPiFromXiccPP,AliPID::kPion);
  ...
  if ( fabs(nSigmaTPC_PiFromXiccPP)>=4. || fabs(nSigmaTPC_PiFromXicPlus_trk1)>=4. || fabs(nSigmaTPC_PiFromXicPlus_trk2)>=4. || fabs(nSigmaTPC_PiFromXi)>=4. || fabs(nSigmaTPC_PrFromLam)>=4. || fabs(nSigmaTPC_PiFromLam)>=4. ) return;
...
//  }
  ...
  KFParticle kfpXiccPP_PV = kfpXiccPP;
  kfpXiccPP_PV.SetProductionVertex(PV);
...
//  kfpXiMinus_pv.SetProductionVertex(PV);
  ...
  kfpPiFromXiccPP.GetDistanceFromVertexXY(PV);
  ...
  const Float_t PDGmassXiccPP = TDatabasePDG::Instance()->GetParticle(4422)->Mass();
  Float_t mass_XiccPP_PV, err_mass_XiccPP_PV;
  kfpXiccPP_PV.GetMass(mass_XiccPP_PV, err_mass_XiccPP_PV);
  fVar_XiccPP[51] = mass_XiccPP_PV; // mass of Xicc++

  if ( fabs(mass_XiccPP_PV-PDGmassXiccPP) > fAnaCuts->GetProdMassTolXiccPP() ) return;

  fVar_XiccPP[0]  = nSigmaTPC_PiFromXicPlus_trk1; // TPC nsigma for pion_0 coming from XicPlus
  fVar_XiccPP[1]  = nSigmaTOF_PiFromXicPlus_trk1; // TOF nsigma for pion_0 coming from XicPlus
  fVar_XiccPP[2]  = nSigmaTPC_PiFromXicPlus_trk2; // TPC nsigma for pion_1 coming from XicPlus
  fVar_XiccPP[3]  = nSigmaTOF_PiFromXicPlus_trk2; // TOF nsigma for pion_1 coming from XicPlus
  fVar_XiccPP[4]  = nSigmaTPC_PiFromXi; // TPC nsigma for pion coming from Xi
  fVar_XiccPP[5]  = nSigmaTOF_PiFromXi; // TOF nsigma for pion coming from Xi
  fVar_XiccPP[6]  = nSigmaTPC_PiFromLam; // TPC nsigma for pion coming from Lambda
  fVar_XiccPP[7]  = nSigmaTPC_PrFromLam; // TPC nsigma for proton coming from Lambda

  fVar_XiccPP[8] = kfpLambda.GetChi2()/kfpLambda.GetNDF(); // chi2geo_Lam
  fVar_XiccPP[9] = l_Lambda/dl_Lambda; // l/dl of Lambda
  fVar_XiccPP[10] = kfpLambda_PV.GetChi2()/kfpLambda_PV.GetNDF(); // chi2_topo of Lambda (with mass constraint) to PV

  fVar_XiccPP[11] = kfpXiMinus.GetChi2()/kfpXiMinus.GetNDF(); // chi2_geometry of Xi (with Lambda mass const.)
  fVar_XiccPP[12] = l_Xi/dl_Xi; // l/dl of Xi (with Lambda mass const.)
  fVar_XiccPP[13] = kfpXiMinus_PV.GetChi2()/kfpXiMinus_PV.GetNDF(); // chi2_topo of Xi (with mass constraint) to PV
  fVar_XiccPP[14] = kfpXiMinus_m.GetChi2()/kfpXiMinus_m.GetNDF(); // chi2_MassConst of Xi

  Float_t DecayLxy_Lam, err_DecayLxy_Lam;
  kfpLambda_Xi.GetDecayLengthXY(DecayLxy_Lam, err_DecayLxy_Lam);
  fVar_XiccPP[15] = DecayLxy_Lam; // decay length of Lambda in x-y plane
  Float_t ct_Lam=0., err_ct_Lam=0.;
  kfpLambda_Xi.GetLifeTime(ct_Lam, err_ct_Lam);
  fVar_XiccPP[16] = ct_Lam; // life time of Lambda

  Float_t DecayLxy_Xi, err_DecayLxy_Xi;
  kfpXiMinus_PV.GetDecayLengthXY(DecayLxy_Xi, err_DecayLxy_Xi);
  fVar_XiccPP[17] = DecayLxy_Xi; // decay length of Xi in x-y plane
  Float_t ct_Xi=0., err_ct_Xi=0.;
  kfpXiMinus_PV.GetLifeTime(ct_Xi, err_ct_Xi);
  fVar_XiccPP[18] = ct_Xi; // life time of Xi
  ...
  fVar_XiccPP[19] = TMath::ACos(cosPA_v0toXi); // pointing angle of Lmabda (pointing back to Xi)
  fVar_XiccPP[20] = TMath::ACos(cosPA_v0toPV); // pointing angle of Lambda (pointing back to PV)
  fVar_XiccPP[21] = TMath::ACos(cosPA_XiToPV); // pointing angle of Xi (pointing back to PV)

  Float_t mass_Lam, err_mass_Lam;
  kfpLambda.GetMass(mass_Lam, err_mass_Lam);
  fVar_XiccPP[22] = mass_Lam; // mass of Lambda (without mass const.)

  Float_t mass_Xi, err_mass_Xi;
  kfpXiMinus.GetMass(mass_Xi, err_mass_Xi);
  fVar_XiccPP[23] = mass_Xi; // mass of Xi (without mass const.)

  fVar_XiccPP[24] = trackPiFromXicPlus_trk1->Pt(); // pt of pion_0 coming from XicPlus
  fVar_XiccPP[25] = trackPiFromXicPlus_trk2->Pt(); // pt of pion_1 coming from XicPlus

  fVar_XiccPP[26] = kfpXicPlus_PV.GetPt(); // pt of XicPlus
  if ( TMath::Abs(kfpXicPlus_PV.GetE())>TMath::Abs(kfpXicPlus_PV.GetPz()) ) {
    fVar_XiccPP[27] = kfpXicPlus_PV.GetRapidity(); // rapidity of XicPlus
  }

  Float_t mass_XicPlus_PV, err_mass_XicPlus_PV;
  kfpXicPlus_PV.GetMass(mass_XicPlus_PV, err_mass_XicPlus_PV);
  fVar_XiccPP[28] = mass_XicPlus_PV; // mass of XicPlus

  fVar_XiccPP[29] = kfpXicPlus.GetChi2()/kfpXicPlus.GetNDF(); // chi2_geometry of XicPlus

  // --- chi2_prim of Pion to PV ---
  KFParticle kfpBP_trk1_PV = kfpBP_trk1;
  kfpBP_trk1_PV.SetProductionVertex(PV);
  fVar_XiccPP[30] = kfpBP_trk1_PV.GetChi2()/kfpBP_trk1_PV.GetNDF(); // chi2_topo of pion_0 to PV
  KFParticle kfpBP_trk2_PV = kfpBP_trk2;
  kfpBP_trk2_PV.SetProductionVertex(PV);
  fVar_XiccPP[31] = kfpBP_trk2_PV.GetChi2()/kfpBP_trk2_PV.GetNDF(); // chi2_topo of pion_1 to PV
  KFParticle kfpPiFromXiccPP_PV = kfpPiFromXiccPP;
  kfpPiFromXiccPP_PV.SetProductionVertex(PV);
  fVar_XiccPP[53] = kfpPiFromXiccPP_PV.GetChi2()/kfpPiFromXiccPP_PV.GetNDF(); // chi2_topo of pion_2 to PV
  // -------------------------------
  
  // --- DCA of Pion to PV ---
  fVar_XiccPP[32] = kfpBP_trk1.GetDistanceFromVertexXY(PV); // DCA of pion_0 coming from XicPlus in x-y plane
  fVar_XiccPP[33] = kfpBP_trk2.GetDistanceFromVertexXY(PV); // DCA of pion_1 coming from XicPlus in x-y plane
  fVar_XiccPP[54] = kfpPiFromXiccPP.GetDistanceFromVertexXY(PV); // DCA of pion_2 coming from Xicc++ in x-y plane
  // -------------------------

  Float_t massK0S_Rec, err_massK0S;
  kfpK0Short.GetMass(massK0S_Rec, err_massK0S);
  fVar_XiccPP[34] = massK0S_Rec; // mass of Ks0
  Float_t massGamma_Rec, err_massGamma;
  kfpGamma.GetMass(massGamma_Rec, err_massGamma);
  fVar_XiccPP[35] = massGamma_Rec; // mass of e+e-

  fVar_XiccPP[36] = kfpPionFromLam.GetDistanceFromParticleXY(kfpProtonFromLam); // DCA of Lam's daughters
  fVar_XiccPP[37] = kfpPionOrKaon.GetDistanceFromParticleXY(kfpLambda_m); // DCA of Xi's daughters (calculated from KF after Lambda mass constraint)
  fVar_XiccPP[38] = kfpXiMinus_m.GetDistanceFromVertexXY(PV); // DCA of Xi to PV in x-y plane (calculated from KF after Xi mass constraint)
  fVar_XiccPP[39] = kfpBP_trk1.GetDistanceFromParticleXY(kfpBP_trk2); // DCA of pi to pi in x-y plane
  fVar_XiccPP[40] = kfpBP_trk1.GetDistanceFromParticle(kfpBP_trk2); // DCA of pi to pi
  fVar_XiccPP[41] = kfpBP_trk1.GetDistanceFromParticleXY(kfpXiMinus_m); // DCA of pi_trk1 to Xi in x-y plane
  fVar_XiccPP[42] = kfpBP_trk2.GetDistanceFromParticleXY(kfpXiMinus_m); // DCA of pi_trk2 to Xi in x-y plane

  // --- CosPointingAngle ---
  Double_t cosPA_XicPlusToPV = AliVertexingHFUtils::CosPointingAngleFromKF(kfpXicPlus, PV);
  fVar_XiccPP[43] = TMath::ACos(cosPA_XicPlusToPV); // pointing angle of XicPlus (pointing back to PV)
  // -------------------------
  Float_t DecayLxy_XicPlus, err_DecayLxy_XicPlus;
  kfpXicPlus_PV.GetDecayLengthXY(DecayLxy_XicPlus, err_DecayLxy_XicPlus);
  fVar_XiccPP[44] = DecayLxy_XicPlus; // decay length of XicPlus in x-y plane
  fVar_XiccPP[45] = kfpXicPlus_PV.GetChi2()/kfpXicPlus_PV.GetNDF(); // chi2_topo of XicPlus to PV

  fVar_XiccPP[46] = nSigmaTPC_PiFromXiccPP;
  fVar_XiccPP[47] = nSigmaTOF_PiFromXiccPP;
  fVar_XiccPP[48] = trackPiFromXiccPP->Pt(); // pt of pion coming from Xicc++
  fVar_XiccPP[49] = kfpXiccPP_PV.GetPt(); // pt of Xicc++
  if ( TMath::Abs(kfpXiccPP_PV.GetE())>TMath::Abs(kfpXiccPP_PV.GetPz()) ) {
    fVar_XiccPP[50] = kfpXiccPP_PV.GetRapidity(); // rapidity of Xicc++
  }

  fVar_XiccPP[52] = kfpXiccPP.GetChi2()/kfpXiccPP.GetNDF(); // chi2_geometry of Xicc++

  // --- chi2_prim of Pion to PV ---
  // -------------------------------


//  fVar_XiccPP[26] = AliVertexingHFUtils::CosThetaStarFromKF(0, 4132, 211, 3312, kfpXicPlus, kfpBP_XicPlus, kfpXiMinus_XicPlus);
//  fVar_XiccPP[27] = AliVertexingHFUtils::CosThetaStarFromKF(1, 4132, 211, 3312, kfpXicPlus, kfpBP_XicPlus, kfpXiMinus_XicPlus);


//  fVar_XiccPP[33] = kfpLambda_Xi.GetChi2()/kfpLambda_Xi.GetNDF();
//  fVar_XiccPP[34] = kfpXiMinus_XicPlus.GetChi2()/kfpXiMinus_XicPlus.GetNDF();

//  Float_t ct_XicPlus=0., err_ct_XicPlus=0.;
//  kfpXicPlus_PV.GetLifeTime(ct_XicPlus, err_ct_XicPlus);
//  fVar_XiccPP[36] = ct_XicPlus;
//  fVar_XiccPP[38] = casc->DcaV0Daughters(); // DCA_LamDau
//  fVar_XiccPP[40] = kfpBP_trk1.GetDistanceFromParticle(kfpXiMinus_m); // DCA_XicPlusDau_KF

  if (fIsMC) {
    fVar_XiccPP[55] = lab_XiccPP;
    // === weight ===
    /*
    if (lab_XiccPP>0) {
  ...
  // pt(XicPlus)>=1
  if (fVar_XiccPP[49]>0.9999) fTree_XiccPP->Fill();



//  fVar_XiccPP[10] = casc->DcaXiDaughters(); // DCA_XiDau
//  fVar_XiccPP[32] = AliVertexingHFUtils::CosThetaStarFromKF(0, 4132, 211, 3312, kfpXicPlus, kfpBP_trk1, kfpXiMinus_m);
//  fVar_XiccPP[33] = AliVertexingHFUtils::CosThetaStarFromKF(1, 4132, 211, 3312, kfpXicPlus, kfpBP_trk1, kfpXiMinus_m);

//  fVar_XiccPP[3]  = TMath::Prob(kfpLambda.GetChi2(), kfpLambda.GetNDF());
//  fVar_XiccPP[7]  = kfpLambda.GetPz();
//  fVar_XiccPP[8]  = kfpLambda.GetX();
//  fVar_XiccPP[9]  = kfpLambda.GetY();
//  fVar_XiccPP[10] = kfpLambda.GetZ();
//  fVar_XiccPP[8] = kfpLambda_pv.GetChi2()/kfpLambda_pv.GetNDF();
//  fVar_XiccPP[9] = TMath::Prob(kfpLambda_pv.GetChi2(), kfpLambda_pv.GetNDF());
//  fVar_XiccPP[15] = alpha_FirstDaugPos;
//  fVar_XiccPP[16] = v0->PtArmV0();

//  fVar_XiccPP[17] = trackPiFromXi->Charge();
//  fVar_XiccPP[20] = TMath::Prob(kfpXiMinus.GetChi2(), kfpXiMinus.GetNDF());
//  fVar_XiccPP[21] = kfpXiMinus_pv.GetChi2()/kfpXiMinus_pv.GetNDF();
//  fVar_XiccPP[22] = TMath::Prob(kfpXiMinus_pv.GetChi2(), kfpXiMinus_pv.GetNDF());
//  Float_t mass_Xi_PV, err_mass_Xi_PV;
//  kfpXiMinus_pv.GetMass(mass_Xi_PV, err_mass_Xi_PV);
//  fVar_XiccPP[23] = mass_Xi_PV;
//  if ( TMath::Abs(kfpXiMinus_pv.GetE())>TMath::Abs(kfpXiMinus_pv.GetPz()) ) {
//    fVar_XiccPP[24] = kfpXiMinus_pv.GetRapidity();
//  }
//  fVar_XiccPP[25] = kfpXiMinus_pv.GetPt();
//  fVar_XiccPP[26] = kfpXiMinus_pv.GetPz();
//  fVar_XiccPP[27] = kfpXiMinus_pv.GetX();
//  fVar_XiccPP[28] = kfpXiMinus_pv.GetY();
//  fVar_XiccPP[29] = kfpXiMinus_pv.GetZ();
//  fVar_XiccPP[33] = kfpXiMinus.GetPz();
//  fVar_XiccPP[34] = kfpXiMinus.GetX();
//  fVar_XiccPP[35] = kfpXiMinus.GetY();
//  fVar_XiccPP[36] = kfpXiMinus.GetZ();

//  fVar_XiccPP[39] = trackPiFromXicPlus_trk1->Charge();
//  fVar_XiccPP[41] = trackPiFromXicPlus_trk1->Pz();

//  fVar_XiccPP[20] = kfpXicPlus.GetChi2()/kfpXicPlus.GetNDF();
//  fVar_XiccPP[45] = TMath::Prob(kfpXicPlus.GetChi2(), kfpXicPlus.GetNDF());
//  fVar_XiccPP[21] = kfpXicPlus_PV.GetChi2()/kfpXicPlus_PV.GetNDF();
//  fVar_XiccPP[47] = TMath::Prob(kfpXicPlus_PV.GetChi2(), kfpXicPlus_PV.GetNDF());
//  fVar_XiccPP[51] = kfpXicPlus_PV.GetPz();
//  fVar_XiccPP[52] = kfpXicPlus_PV.GetX();
//  fVar_XiccPP[53] = kfpXicPlus_PV.GetY();
//  fVar_XiccPP[54] = kfpXicPlus_PV.GetZ();
//  Float_t mass_Xic, err_mass_Xic;
//  kfpXicPlus.GetMass(mass_Xic, err_mass_Xic);
//  fVar_XiccPP[55] = mass_Xic;
//  fVar_XiccPP[56] = kfpXicPlus.GetRapidity();
//  fVar_XiccPP[57] = kfpXicPlus.GetPt();
//  fVar_XiccPP[58] = kfpXicPlus.GetPz();
//  fVar_XiccPP[59] = kfpXicPlus.GetX();
//  fVar_XiccPP[60] = kfpXicPlus.GetY();
//  fVar_XiccPP[61] = kfpXicPlus.GetZ();

//  fVar_XiccPP[27] = cosPA_XiToXic;
...
//  kfpXiMinus_pv.GetLifeTime(CT_Xi, err_CT_Xi);
//  kfpXicPlus_PV.GetLifeTime(CT_XicPlus, err_CT_XicPlus);
//  fVar_XiccPP[38] = CT_Lam;
//  fVar_XiccPP[39] = CT_Xi;
//  fVar_XiccPP[40] = CT_XicPlus;

//  if (fIsMC) {
//    fVar_XiccPP[78] = lab_XiccPP;
//  }
