...
/// \cond CLASSIMP
ClassImp(AliAnalysisTaskSED0MassTMVA);
/// \endcond
...
//________________________________________________________________________
AliAnalysisTaskSED0MassTMVA::AliAnalysisTaskSED0MassTMVA(const char *name,AliRDHFCutsD0toKpi* cuts):
...

{
  ...
  if(!inputArray || !aod) {
    printf("AliAnalysisTaskSED0MassTMVA::UserExec: input branch not found!\n");
    return;
  }
  ...
  if(fReadMC) {
    ...
    if(!mcArray) {
      printf("AliAnalysisTaskSED0MassTMVA::UserExec: MC particles branch not found!\n");
      return;
    }
    ...
    if(!mcHeader) {
      printf("AliAnalysisTaskSED0MassTMVA::UserExec: MC header branch not found!\n");
      return;
    }
  }
...
void AliAnalysisTaskSED0MassTMVA::DrawDetSignal(AliAODRecoDecayHF2Prong *part, TList *ListDetSignal)
{
  //
...
//____________________________________________________________________________
void AliAnalysisTaskSED0MassTMVA::FillVarHists(AliAODEvent* aod,AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliRDHFCutsD0toKpi *cuts, TList *listout){
  //
...
//____________________________________________________________________________
void AliAnalysisTaskSED0MassTMVA::FillMassHists(AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliAODMCHeader *mcHeader, AliRDHFCutsD0toKpi* cuts, TList *listout){
  //
...
void AliAnalysisTaskSED0MassTMVA::FillCandVariables(AliAODEvent *aodev, AliAODRecoDecayHF2Prong *part, TClonesArray *arrMC, AliAODMCHeader *mcHeader, AliRDHFCutsD0toKpi *cuts){
...
Float_t AliAnalysisTaskSED0MassTMVA::GetTrueImpactParameter(AliAODMCHeader *mcHeader, TClonesArray* arrayMC, AliAODMCParticle *partD0) const {
  /// true impact parameter calculation

  //printf(" AliAnalysisTaskSED0MassTMVAV1::GetTrueImpactParameter() \n");
...
//_________________________________________________________________________________________________
Float_t AliAnalysisTaskSED0MassTMVA::ComputeTopomatic(AliAODEvent *aod, AliAODRecoDecayHF2Prong *part) {
  Float_t dd0max = 0.;
  for (Int_t ipr = 0; ipr < 2; ipr++) {
...
//_________________________________________________________________________________________________
Int_t AliAnalysisTaskSED0MassTMVA::CheckOrigin(TClonesArray* arrayMC, AliAODMCParticle *mcPartCandidate) const {
  //  obsolete method
  /// checking whether the mother of the particles come from a charm or a bottom quark
  //
  printf(" AliAnalysisTaskSED0MassTMVA V1::CheckOrigin() \n");
...
void AliAnalysisTaskSED0MassTMVA::FillMCAcceptanceHistos(TClonesArray *arrayMC, AliAODMCHeader *mcHeader){
...
void AliAnalysisTaskSED0MassTMVA::NormIPvar(AliAODEvent *aod, AliAODRecoDecayHF2Prong *part,TClonesArray *arrMC){
    ...
    if(fReadMC){  // MC THnSparse
        ...
        if(lab>=0){
            ...
            if((fIsSelectedCandidate==1 || fIsSelectedCandidate==3) && fFillOnlyD0D0bar<2){
                tmp[8] = invmassD0;
                if(pdgcode!=421) fNtupleRefl->Fill(tmp); // Reflection
            }
    ...
    else  // data
    {
        ...
        Int_t pid=fCuts->IsSelectedPID(part);      // returns 0 if both D0 and D0bar are rejectecd
        ...
        if((fIsSelectedCandidate==1 || fIsSelectedCandidate==3) && fFillOnlyD0D0bar<2){
            ...
            tmp[5] = (Double_t)IsSelectedPIDoff;
            tmp[6] = (Double_t)pid;
            tmp[7] = 1;
            tmp[8] = invmassD0;
