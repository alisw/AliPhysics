...
#include "AliAnalysisTaskSEXic0pp13TeV.h"
...
AliAnalysisTaskSEXic0RunTable::AliAnalysisTaskSEXic0RunTable() :
  fCollisionType(kUnknownCollType){;}   //defult constructor
...
AliAnalysisTaskSEXic0pp13TeV::AliAnalysisTaskSEXic0pp13TeV() :
  AliAnalysisTaskSE("AliAnalysisTaskSEXic0pp13TeV"),
  fOption(){
...
//___________________________________________________________________
AliAnalysisTaskSEXic0pp13TeV::AliAnalysisTaskSEXic0pp13TeV( const char *name, const char *option ) :
...
//___________________________________________________________________
AliAnalysisTaskSEXic0pp13TeV::AliAnalysisTaskSEXic0pp13TeV( const AliAnalysisTaskSEXic0pp13TeV& ap ) :
  fOption(ap.fOption) {;}

//___________________________________________________________________
AliAnalysisTaskSEXic0pp13TeV& AliAnalysisTaskSEXic0pp13TeV::operator = ( const AliAnalysisTaskSEXic0pp13TeV& ap ) {
  // assignment operator
  this->~AliAnalysisTaskSEXic0pp13TeV();
  new(this) AliAnalysisTaskSEXic0pp13TeV(ap);
  return *this;
}
//___________________________________________________________________
AliAnalysisTaskSEXic0pp13TeV::~AliAnalysisTaskSEXic0pp13TeV(){
...
//___________________________________________________________________
void AliAnalysisTaskSEXic0pp13TeV::UserCreateOutputObjects(){
...
//___________________________________________________________________
void AliAnalysisTaskSEXic0pp13TeV::UserExec(Option_t* ){
  // Pointer to a event----------------------------------------------------
  ...
  if(!fPIDResponse) printf("AliAnalysisTaskSEXic0pp13TeV No PIDd\n");
 ...
 fHistos->FillTH1("NumOfEvtperRun",fRunNumber-RunOffset);
 ...
 if (IsMC && fEvt->IsA()==AliAODEvent::Class())
 {
   ...
   for (Int_t iTracks = 0; iTracks < (fMC->GetNumberOfTracks()); iTracks++)
   {
     ...
     if(IsFromMCXib(MCparticle)) continue;
   }
 }
...
void AliAnalysisTaskSEXic0pp13TeV::CheckXic0Info(AliAODTrack *trk, AliAODcascade *casc){
...
///////////////////////////////Filtering///////////////////////////////////
Bool_t AliAnalysisTaskSEXic0pp13TeV::FilterTrack(AliAODTrack * track, Int_t FillHistosFlag){
...
Bool_t AliAnalysisTaskSEXic0pp13TeV::FilterElectron(AliAODTrack *etrk, Double_t &minmass, Double_t &minmass_ss, Bool_t IsSameSign, Bool_t IsLoose, Int_t FillHistosFlag){
  ...
  if((FillHistosFlag==100) && StandardCutFlag(etrk,casc,1,1,0,0)) fHistos->FillTH1("NumOfe",fRunNumber-RunOffset);
...
Bool_t AliAnalysisTaskSEXic0pp13TeV::FilterCascade(AliAODcascade *casc){
  Bool_t isparticle = kTRUE;
  if(casc->ChargeXi()>0) isparticle = kFALSE;
  ...
  if((fabs(massXi-mxiPDG)<0.025) &&StandardCutFlag(pion,casc,0,0,1,1)) fHistos->FillTH1("NumOfXi",fRunNumber-RunOffset);  //dummy track
...
void AliAnalysisTaskSEXic0pp13TeV::FillMCXic0(AliAODMCParticle *mcpart){
...
Bool_t AliAnalysisTaskSEXic0pp13TeV::IsFromMCXib(AliAODMCParticle *mcpart){
...
void AliAnalysisTaskSEXic0pp13TeV::FillPairEleXi(AliAODcascade *casc, AliAODTrack *trk, Int_t FillHistosFlag){
  for(Int_t i=0; i<38; i++) fPaireXiTreeVariable[i] = -9999.;
  for(Int_t i=0; i<8; i++) fMCTreeVariable[i] = -9999.;
...
void AliAnalysisTaskSEXic0pp13TeV::FillXiHistFromPromptNonPrompt(Bool_t IsMC, AliAODcascade *casc){
  if(!IsMC) return;
  Bool_t isparticle = kTRUE;
  if(casc->ChargeXi()>0) isparticle = kFALSE;
...
Int_t AliAnalysisTaskSEXic0pp13TeV::MatchToMCXic0(AliAODcascade *casc, AliAODTrack *trk){
...
Int_t AliAnalysisTaskSEXic0pp13TeV::MatchToMCXic0(AliAODcascade *casc){
  Int_t Xi_label = MatchToMCXi(casc); if(!Xi_label) return -1;
  AliAODMCParticle* mcXi = (AliAODMCParticle*) fMC->GetTrack(Xi_label);
...
Int_t AliAnalysisTaskSEXic0pp13TeV::MatchToMCXi(AliAODcascade* casc){
...
Int_t AliAnalysisTaskSEXic0pp13TeV::MatchToMCele(AliAODTrack* trk){
...
Bool_t AliAnalysisTaskSEXic0pp13TeV::StandardCutFlag(AliAODTrack *track, AliAODcascade *casc, Bool_t e_reco, Bool_t e_pid, Bool_t Xi_reco, Bool_t Xi_pid){
  //-------------------------------------eletron-------------------------------------
  if(e_reco){
...
//___________________________________________________________________
void AliAnalysisTaskSEXic0pp13TeV::FinishTaskOutput() {}
                //fOutput->Write();
//___________________________________________________________________
void AliAnalysisTaskSEXic0pp13TeV::Terminate(Option_t*) {}

//___________________________________________________________________
//_____________________Define Tree Valuable___________________________
//___________________________________________________________________

void AliAnalysisTaskSEXic0pp13TeV::DefineMCCutTree(){
...
void AliAnalysisTaskSEXic0pp13TeV::DefinePaireXiTree(){
...
void AliAnalysisTaskSEXic0pp13TeV::DefineMCPaireXiTree(){
  fMCTree = new TTree("MCTree","MCTree");
  vector <TString> fTreeVariableName = {"mcpTe","mcecharge","mcpTv","mcvcharge","mcpTXic0","mcpTeXi","c_flag","b_flag"};
...
void AliAnalysisTaskSEXic0pp13TeV::DefineEventTree(){
...
unsigned int AliAnalysisTaskSEXic0pp13TeV::GetEvID() {
