...
//--------------------------------------------------------------------------
AliRDHFCutsDstoK0sK::AliRDHFCutsDstoK0sK(const char* name) :
AliRDHFCuts(name)
   , fExcludedCut(-1)
   , fV0Type(0)
   , fV0daughtersCuts(0)
   ...
   , fMaxPtStrongPid(0)
{
   ...
   SetNVars(nvars);
   TString varNames[nvars] = { "Inv. Mass Ds -> K0s+K [GeV/c2]",                 //  0
                               ...
                               "max cosThetaStar Ds daughters",                  // 11
...
//---------------------------------------------------------------------------
void AliRDHFCutsDstoK0sK::GetCutVarsForOpt(AliAODRecoDecayHF* obj, Float_t* vars, Int_t nvars, Int_t *pdgdaughters, AliAODEvent *aod)
{
   //
   ...
   if (fVarsForOpt[0]) {
      iter++;
      if (TMath::Abs(pdgdaughters[0]==431)) vars[iter] = TMath::Abs(d->InvMassDstoK0sK()-mDsPDG);
   }
   ...
   if (fVarsForOpt[11]) {
      iter++;
      vars[iter] = TMath::Abs(d->CosThetaStar(0, 431, 321, 310));
   }
...
Int_t AliRDHFCutsDstoK0sK::IsSelected(TObject* obj, Int_t selectionLevel, AliAODEvent* aod)
{
   ...
   //___ 2./ Check for cascade Ds  ___________________
   //_________________________________________________
   ...
   if (selectionLevel == AliRDHFCuts::kAll ||
      ...
      // - Check invariant mass of cascade Ds
      if (TMath::Abs(mDs - mDsPDG) > fCutsRD[GetGlobalIndex(0,ptbin)] && fExcludedCut!=0) {
         AliDebug(4, "cascade not compatible with Ds invariant mass");
         return 0;
      }
      ...
      // - Cut on Ds-daughters cosThetaStar
      if (TMath::Abs(d->CosThetaStar(0, 431, 321, 310)) > fCutsRD[GetGlobalIndex(11,ptbin)] && fExcludedCut!=11) {
         AliDebug(4, " Ds daughter cosThetaStar doesn't pass the cut");
         CleanOwnPrimaryVtx(d, aod, origownvtx);
         return 0;
      }
      ...
      if (TMath::Abs(d->DecayLengthXY()) < fCutsRD[GetGlobalIndex(14,ptbin)] && fExcludedCut!=14) {
         AliDebug(4, " Ds normalised decay length (XY) doesn't pass the cut");
         CleanOwnPrimaryVtx(d, aod, origownvtx);
         return 0;
      }
...
{
   //
   /// Apply PID selections on the bachelor track (Ds->K0s+K)
   /// Return value:  0: not compatible with Kaon
   ///                1: compatible with Kaon
   //
...
//---------------------------------------------------------------------------
Bool_t AliRDHFCutsDstoK0sK::IsInFiducialAcceptance(Double_t pt, Double_t y) const
{
   //
...
Int_t AliRDHFCutsDstoK0sK::GetV0Type()
{
   ...
   if (sVarNames[nvars-1].Contains("V0 type")) {
      ...
      AliInfo("AliRDHFCutsDstoK0sK Last variable is not the V0 type!!!");
      return -999;
   }
}
