...
/////////////////////////////////////////////////////////////
// Author: Jianhui Zhu (INFN-Padova)
// E-mail: jianhui.zhu@cern.ch
/////////////////////////////////////////////////////////////
...
class AliHFMLXicZeroToXiPifromKFP;    // your analysis class

ClassImp(AliHFMLXicZeroToXiPifromKFP) // classimp: necessary for root
...
AliHFMLXicZeroToXiPifromKFP::AliHFMLXicZeroToXiPifromKFP() :
  AliAnalysisTaskSE(),
  fIsMC(kFALSE),
  fIsPbPb(kTRUE),
  fIsAnaOmegac0(kFALSE),
  fIsStoreOnlyMLoutput(kTRUE),
  fIsStoreLS(kFALSE),
  fPID(0),
  fAnaCuts(0),
  fpVtx(0),
...
AliHFMLXicZeroToXiPifromKFP::AliHFMLXicZeroToXiPifromKFP(const char* name, AliRDHFCutsKFP* cuts) :
...
AliHFMLXicZeroToXiPifromKFP::~AliHFMLXicZeroToXiPifromKFP()
{
    ...
    */
    ...
    if (fTree_MLoutput) {
      delete fTree_MLoutput;
      fTree_MLoutput = 0;
    }
  
    if (fVar_MLoutput) {
      delete fVar_MLoutput;
      fVar_MLoutput = 0;
    }

}
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::Init()
{
  // Initialization
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::UserCreateOutputObjects()
{
...
//  fCounterRec_Cuts_AntiLambda->GetXaxis()->SetBinLabel(12,"sec. #bar{#Lambda} cut");
  ...
  f2DHistArmenterosPodolanski = new TH2F("f2DHistArmenterosPodolanski", "f2DHistArmenterosPodolanski", 1000, -1., 1., 1000, 0., 1.);
...
*/
  ...
  // Initialize ML response
  if (fConfigPath != "") {
    if (!fIsAnaOmegac0) fMLResponse = new AliHFMLResponse("Xic0ToPiXiMLResponse", "Xic0ToPiXiMLResponse", fConfigPath.Data());
    if (fIsAnaOmegac0)  fMLResponse = new AliHFMLResponse("Omegac0ToPiOmegaMLResponse", "Omegac0ToPiOmegaMLResponse", fConfigPath.Data());
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::UserExec(Option_t *)
{
  // user exec
  ...
  if (fIsMC) {
    ...
    if ( !mcHeader ) {
      AliError("AliHFMLXicZeroToXiPifromKFP::UserExec: MC header branch not found!\n");
      return;
    }
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::Terminate(Option_t *)
{
    // terminate
...
//_____________________________________________________________________________
Bool_t AliHFMLXicZeroToXiPifromKFP::MakeMCAnalysis(TClonesArray *mcArray)
{
...
//    if ( mcpart->GetNDaughters() != NDaughters ) continue;
    ...
    if ( fIsAnaOmegac0 && TMath::Abs(mcpart->GetPdgCode())==4332 && mcpart->GetNDaughters()==NDaughters ) { // 4332: Omegac0
      ...
      for (Int_t idau=mcpart->GetDaughterFirst();idau<=mcpart->GetDaughterLast();idau++) {
        ...
        if (TMath::Abs(mcDau_Omegac0->GetPdgCode())==211) {pifromOmegac0_flag = kTRUE; mc_pifromOmegac0=mcDau_Omegac0;} // 211: pion
        if (TMath::Abs(mcDau_Omegac0->GetPdgCode())==3334) { // 3312: Omega
          Omega_flag = kTRUE;
          mc_Omega=mcDau_Omegac0;
          if (mcDau_Omegac0->GetNDaughters()==NDaughters) {
            for (Int_t jdau=mcDau_Omegac0->GetDaughterFirst(); jdau<=mcDau_Omegac0->GetDaughterLast(); jdau++) {
              ...
              if (TMath::Abs(mcDau_Omega->GetPdgCode())==321) {kafromOmega_flag = kTRUE; mc_kafromOmega=mcDau_Omega;}
              if (TMath::Abs(mcDau_Omega->GetPdgCode())==3122) {
                ...
                if (mcDau_Omega->GetNDaughters()==NDaughters) {
                  for (Int_t kdau=mcDau_Omega->GetDaughterFirst(); kdau<=mcDau_Omega->GetDaughterLast(); kdau++) {
                    ...
                    if (TMath::Abs(mcDau_Lam->GetPdgCode())==211 && (kdau!=idau)) {pifromLam_flag = kTRUE; mc_pifromLam=mcDau_Lam;}
                    if (TMath::Abs(mcDau_Lam->GetPdgCode())==2212) {prfromLam_flag = kTRUE; mc_prfromLam=mcDau_Lam;}
                  }
                }
              }
            }
      ...
      if ( pifromOmegac0_flag && Omega_flag && kafromOmega_flag && Lam_flag && pifromLam_flag && prfromLam_flag ) {
        ...
        FillTreeGenXic0(mcpart, CheckOrigin, MLoverP, mcArray, mc_pifromOmegac0, mc_kafromOmega, mc_pifromLam, mc_prfromLam, mc_Omega, mc_Lam);
      }
    }
    ...
    if ( !fIsAnaOmegac0 && TMath::Abs(mcpart->GetPdgCode())==4132 && mcpart->GetNDaughters()==NDaughters ) { // 4132: Xic0
      ...
      for (Int_t idau=mcpart->GetDaughterFirst();idau<=mcpart->GetDaughterLast();idau++) {
        ...
        if (TMath::Abs(mcDau_Xic0->GetPdgCode())==211) {pifromXic0_flag = kTRUE; mc_pifromXic0=mcDau_Xic0;} // 211: pion
        if (TMath::Abs(mcDau_Xic0->GetPdgCode())==3312) { // 3312: Xi
          ...
          if (mcDau_Xic0->GetNDaughters()==NDaughters) {
            for (Int_t jdau=mcDau_Xic0->GetDaughterFirst(); jdau<=mcDau_Xic0->GetDaughterLast(); jdau++) {
              ...
              if (TMath::Abs(mcDau_Xi->GetPdgCode())==211 && (jdau!=idau)) {pifromXi_flag = kTRUE; mc_pifromXi=mcDau_Xi;}
              if (TMath::Abs(mcDau_Xi->GetPdgCode())==3122) {
                ...
                if (mcDau_Xi->GetNDaughters()==NDaughters) {
                  for (Int_t kdau=mcDau_Xi->GetDaughterFirst(); kdau<=mcDau_Xi->GetDaughterLast(); kdau++) {
                    if (kdau<0) break;
                    AliAODMCParticle *mcDau_Lam = (AliAODMCParticle*) mcArray->At(kdau);
                    if (TMath::Abs(mcDau_Lam->GetPdgCode())==211 && (kdau!=jdau) && (kdau!=idau)) {pifromLam_flag = kTRUE; mc_pifromLam=mcDau_Lam;}
                    if (TMath::Abs(mcDau_Lam->GetPdgCode())==2212) {prfromLam_flag = kTRUE; mc_prfromLam=mcDau_Lam;}
                  }
                }
              }
      ...
      if ( pifromXic0_flag && Xi_flag && pifromXi_flag && Lam_flag && pifromLam_flag && prfromLam_flag ) {
        ...
        Double_t MLoverP = sqrt( pow(mcpart->Xv()-mcdau_0->Xv(),2.)+pow(mcpart->Yv()-mcdau_0->Yv(),2.)+pow(mcpart->Zv()-mcdau_0->Zv(),2.) ) * mcpart->M() / mcpart->P()*1.e4; // c*(proper lifetime) in um
        Int_t CheckOrigin = AliVertexingHFUtils::CheckOrigin(mcArray,mcpart,kFALSE);
        FillTreeGenXic0(mcpart, CheckOrigin, MLoverP, mcArray, mc_pifromXic0, mc_pifromXi, mc_pifromLam, mc_prfromLam, mc_Xi, mc_Lam);
      }
    } // for Xic0
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::FillTreeGenXic0(AliAODMCParticle *mcpart, Int_t CheckOrigin, Double_t MLoverP, TClonesArray *mcArray, AliAODMCParticle *mc_pifromXic0, AliAODMCParticle *mc_pifromXi, AliAODMCParticle *mc_pifromLam, AliAODMCParticle *mc_prfromLam, AliAODMCParticle *mc_Xi, AliAODMCParticle *mc_Lam)
{
  // Fill histograms or tree depending
  ...
  while (mother>=0) {
    AliAODMCParticle* mcMother = static_cast<AliAODMCParticle*>(mcArray->At(mother));
    if ( (fabs(mcMother->GetPdgCode())>500 && fabs(mcMother->GetPdgCode())<600) || (fabs(mcMother->GetPdgCode())>5000 && fabs(mcMother->GetPdgCode())<6000) ) {
      fVar_Xic0MCGen[6] = mcMother->GetPdgCode();
      fVar_Xic0MCGen[7] = mcMother->Y();
      fVar_Xic0MCGen[8] = mcMother->Pt();
    }
    mother = mcMother->GetMother();
  }

  if (mc_pifromXic0) fVar_Xic0MCGen[9] = mc_pifromXic0->Eta();
  if (mc_pifromXi) fVar_Xic0MCGen[10] = mc_pifromXi->Eta();
  if (mc_pifromLam) fVar_Xic0MCGen[11] = mc_pifromLam->Eta();
  if (mc_prfromLam) fVar_Xic0MCGen[12] = mc_prfromLam->Eta();
  if (mc_Xi) fVar_Xic0MCGen[13] = mc_Xi->Eta();
  if (mc_Lam) fVar_Xic0MCGen[14] = mc_Lam->Eta();
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::MakeAnaXicZeroFromV0(AliAODEvent *AODEvent, TClonesArray *mcArray, KFParticle PV)
{
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::MakeAnaXicZeroFromCasc(AliAODEvent *AODEvent, TClonesArray *mcArray, KFParticle PV)
{
  // Main analysis called from "UserExec"
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCXic0(AliAODTrack *trackProton, AliAODTrack *trackPion3, AliAODTrack *trackPion2, AliAODTrack *trackAntiPion1, TClonesArray *mcArray)
{
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCAntiXic0(AliAODTrack *trackAntiProton, AliAODTrack *trackAntiPion3, AliAODTrack *trackAntiPion2, AliAODTrack *trackPion1, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCOmegac0(AliAODTrack *trackProton, AliAODTrack *trackPionMinus, AliAODTrack *trackKaon, AliAODTrack *trackPionPlus, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCAntiOmegac0(AliAODTrack *trackAntiProton, AliAODTrack *trackPionPlus, AliAODTrack *trackKaon, AliAODTrack *trackPionMinus, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::SelectTrack(AliVEvent *event, Int_t trkEntries, Int_t &nSeleTrks, Bool_t *seleFlags)
{
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCOmegaMinus(AliAODTrack *trackProton, AliAODTrack *trackPion, AliAODTrack *trackKaon, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCOmegaPlus(AliAODTrack *trackProton, AliAODTrack *trackPion, AliAODTrack *trackKaon, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCXiMinus(AliAODTrack *trackProton, AliAODTrack *trackPion3, AliAODTrack *trackPion2, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCXiPlus(AliAODTrack *trackAntiProton, AliAODTrack *trackAntiPion3, AliAODTrack *trackAntiPion2, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCLambda(AliAODTrack *trackProton, AliAODTrack *trackPion3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCLambdaFromXi(AliAODTrack *trackProton, AliAODTrack *trackPion3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCAntiLambda(AliAODTrack *trackAntiProton, AliAODTrack *trackAntiPion3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCAntiLambdaFromXi(AliAODTrack *trackAntiProton, AliAODTrack *trackAntiPion3, TClonesArray *mcArray)
{
  // Check if all of the tracks is matched to a MC signal
  // If no, return -1;
  // If yes, return label (>=0) of the AliAODMCParticle
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToMCPion(AliAODTrack *track, TClonesArray *mcArray)
{
...
//_____________________________________________________________________________
Int_t AliHFMLXicZeroToXiPifromKFP::MatchToXicZeroMC(TClonesArray *mcArray, Int_t PDGXicZero, const Int_t nDaughters, const Int_t *daughterIndex, const Int_t *daughterPDG)
{
  ///
...
//  fVarNames[27] = "CosThetaStar_Xi"; // CosThetaStar of Xi coming from Xic0 //////////
  ...
  fVarNames[45] = "nSigmaTOF_PiFromLam"; // TOF nsigma for pion coming from Lambda
  ...
  fVarNames[48] = "OA_XiToPi"; // Opening angle of Xi and pion
  fVarNames[49] = "pt_Xic0_gen"; // pt of Xic0 at gen. level
  fVarNames[50] = "pt_Xic0Mom_gen"; // pt of Xic0 mom at gen. level
  fVarNames[51] = "PDG_Xic0Mom_gen"; // PDG of Xic0 mom at gen. level
  }
...
//    fVarNames[27] = "CosThetaStar_Omega"; // CosThetaStar of Omega coming from Omegac0 /////////
    ...
    fVarNames[46] = "nSigmaTOF_PrFromLam"; // TOF nsigma for proton coming from Lambda
    fVarNames[47] = "armenteros"; // Armenteros-Podolanski plot (qT/|alpha|)
    fVarNames[48] = "OA_OmegaToPi"; // Opening angle of Omega and pion
    fVarNames[49] = "pt_Omegac0_gen"; // pt of Omegac0 at gen. level
    fVarNames[50] = "pt_Omegac0Mom_gen"; // pt of Omegac0 mom at gen. level
    fVarNames[51] = "PDG_Omegac0Mom_gen"; // PDG of Omegac0 mom at gen. level
  }

  fVarNames[52] = "fCentrality";
  fVarNames[53] = "fNtracklets";
...
/*
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::DefineVarTreePiPlus()
{
  // This is to define tree variables
...
/*
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::DefineVarTreePiMinus()
{
  // This is to define tree variables
...
void AliHFMLXicZeroToXiPifromKFP::DefineTreeGenXic0()
{
  ...
  if (!fIsAnaOmegac0) {
    ...
    fVarNames[8] = "pt_Xic0Mom";
    fVarNames[9] = "eta_pifromXic0";
    fVarNames[10] = "eta_pifromXi";
    fVarNames[11] = "eta_pifromLam";
    fVarNames[12] = "eta_prfromLam";
    fVarNames[13] = "eta_Xi";
    fVarNames[14] = "eta_Lam";
  }
  if (fIsAnaOmegac0) {
    ...
    fVarNames[8] = "pt_Omegac0Mom";
    fVarNames[9] = "eta_pifromOmegac0";
    fVarNames[10] = "eta_kafromOmega";
    fVarNames[11] = "eta_pifromLam";
    fVarNames[12] = "eta_prfromLam";
    fVarNames[13] = "eta_Omega";
    fVarNames[14] = "eta_Lam";
  }
...
//_____________________________________________________________________________
Double_t AliHFMLXicZeroToXiPifromKFP::InvMassV0atPV(AliAODTrack *trk1, AliAODTrack *trk2, Int_t pdg1, Int_t pdg2)
{
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::DefineAnaHist()
{
...
/*
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::FillPiPlusROOTObjects(AliAODTrack *track, TClonesArray *mcArray)
{
  for (Int_t i=0; i<5; i++) {
    fVar_PiPlus[i] = -9999.;
  }
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::FillPiMinusROOTObjects(AliAODTrack *track, TClonesArray *mcArray)
{
  for (Int_t i=0; i<5; i++) {
    fVar_PiMinus[i] = -9999.;
  }
...
void AliHFMLXicZeroToXiPifromKFP::FillTreeRecXic0FromCasc(Int_t flagUSorLS, KFParticle kfpXic0, AliAODTrack *trackPiFromXic0, KFParticle kfpBP, KFParticle kfpXiMinus, KFParticle kfpXiMinus_m, KFParticle kfpPionOrKaon, AliAODTrack *trackPiFromXiOrKaonFromOmega, AliAODcascade *casc, KFParticle kfpK0Short, KFParticle kfpGamma, KFParticle kfpLambda, KFParticle kfpLambda_m, AliAODTrack *trkProton, AliAODTrack *trkPion, KFParticle PV, TClonesArray *mcArray, Int_t lab_Xic0)
{
  ...
  Double_t nSigmaTOF_PiFromXic0 = -999., nSigmaTOF_PiFromXi = -999., nSigmaTOF_PrFromLam = -999., nSigmaTOF_PiFromLam = -999.;
  AliAODPidHF *Pid_HF = fAnaCuts->GetPidHF();
  if (Pid_HF) {
    Pid_HF->GetnSigmaTOF(trackPiFromXic0, AliPID::kPion, nSigmaTOF_PiFromXic0);
    Pid_HF->GetnSigmaTOF(trkProton, AliPID::kProton, nSigmaTOF_PrFromLam);
...
//  Float_t nSigmaTOF_PiFromXic0 = fPID->NumberOfSigmasTOF(trackPiFromXic0,AliPID::kPion);
  ...
  Float_t nSigmaTPC_PrFromLam  = fPID->NumberOfSigmasTPC(trkProton,AliPID::kProton);
  Float_t nSigmaTPC_PiFromLam  = fPID->NumberOfSigmasTPC(trkPion,AliPID::kPion);
//  Float_t nSigmaTOF_PrFromLam  = fPID->NumberOfSigmasTOF(trkProton,AliPID::kProton);
//  Float_t nSigmaTOF_PiFromLam  = fPID->NumberOfSigmasTOF(trkPion,AliPID::kPion);
...
//    nSigmaTOF_PiFromXi = fPID->NumberOfSigmasTOF(trackPiFromXiOrKaonFromOmega,AliPID::kKaon);
  ...
  // PID selections
  if ( fabs(nSigmaTPC_PiFromXic0) >= fAnaCuts->GetCut_nSigmaTPC_PiFromXic0_or_PiFromOmegac0() ||
       fabs(nSigmaTPC_PiFromXi) >= fAnaCuts->GetCut_nSigmaTPC_PiFromXi_or_KaFromOmega() ||
       fabs(nSigmaTPC_PrFromLam) >= fAnaCuts->GetCut_nSigmaTPC_PrFromLam() ||
       fabs(nSigmaTPC_PiFromLam) >= fAnaCuts->GetCut_nSigmaTPC_PiFromLam() ||
       nSigmaTOF_PiFromXic0 >= fAnaCuts->GetCut_nSigmaTOF_PiFromXic0_or_PiFromOmegac0() || (nSigmaTOF_PiFromXic0 != -999. && nSigmaTOF_PiFromXic0 <= (0.-fAnaCuts->GetCut_nSigmaTOF_PiFromXic0_or_PiFromOmegac0())) ||
       nSigmaTOF_PiFromXi >= fAnaCuts->GetCut_nSigmaTOF_PiFromXi_or_KaFromOmega() || (nSigmaTOF_PiFromXi != -999. && nSigmaTOF_PiFromXi <= (0.-fAnaCuts->GetCut_nSigmaTOF_PiFromXi_or_KaFromOmega())) ||
       nSigmaTOF_PrFromLam >= fAnaCuts->GetCut_nSigmaTOF_PrFromLam() || (nSigmaTOF_PrFromLam != -999. && nSigmaTOF_PrFromLam <= (0.-fAnaCuts->GetCut_nSigmaTOF_PrFromLam())) ||
       nSigmaTOF_PiFromLam >= fAnaCuts->GetCut_nSigmaTOF_PiFromLam() || (nSigmaTOF_PiFromLam != -999. && nSigmaTOF_PiFromLam <= (0.-fAnaCuts->GetCut_nSigmaTOF_PiFromLam())) ) return;

  // Armenteros-Podolanski (qT/|alpha|) selection
  if ( (casc->PtArmV0()/fabs(casc->AlphaV0())) >= fAnaCuts->GetCut_Armenteros() ) return;
...
//  const Float_t PDGmassOmegac0 = TDatabasePDG::Instance()->GetParticle(4332)->Mass();
  ...
  fVar_Xic0[4]  = nSigmaTPC_PiFromLam; 
  fVar_Xic0[5]  = nSigmaTPC_PrFromLam;
  fVar_Xic0[45] = nSigmaTOF_PiFromLam; 
  fVar_Xic0[46] = nSigmaTOF_PrFromLam;
  fVar_Xic0[47] = casc->PtArmV0() / fabs(casc->AlphaV0());

  Double_t cosOA_XiToPi = AliVertexingHFUtils::CosOpeningAngleFromKF(kfpXiMinus_m, kfpBP);
  fVar_Xic0[48] = TMath::ACos(cosOA_XiToPi);
...
//    fVar_Xic0[27] = AliVertexingHFUtils::CosThetaStarFromKF(1, 4332, 211, 3334, kfpXic0, kfpBP_Xic0, kfpXiMinus_Xic0);
    ...
    if (lab_Xic0>=0) {
      ...
      while (mom>=0) {
        ...
        if ( (fabs(mcMom->GetPdgCode())>500 && fabs(mcMom->GetPdgCode())<600) || (fabs(mcMom->GetPdgCode())>5000 && fabs(mcMom->GetPdgCode())<6000) ) {
          fVar_Xic0[50] = mcMom->Pt();
          fVar_Xic0[51] = mcMom->GetPdgCode();
        }
        mom = mcMom->GetMother();
      }
    }
  ...
  fVar_Xic0[52] = fCentrality;
  fVar_Xic0[53] = fNtracklets; 
  ...
  if (fIsPbPb) {
    ...
    if (!fIsAnaOmegac0) {
      ...
      fVars_MLmap["DCA_Xic0Dau_KF"]    = fVar_Xic0[37]; // DCA of Xic0's daughters (calculated from KF after Xi mass constraint)
      ...
      // Xi
      fVars_MLmap["DCA_XiDau"]         = fVar_Xic0[6];  // DCA of Xi's daughters (calculated from AOD cascade)
      fVars_MLmap["chi2geo_Xi"]        = fVar_Xic0[10]; // chi2_geometry of Xi (with Lambda mass const.)
      fVars_MLmap["ldl_Xi"]            = fVar_Xic0[11]; // l/dl of Xi (with Lambda mass const.)
      fVars_MLmap["chi2topo_XiToPV"]   = fVar_Xic0[12]; // chi2_topo of Xi to PV
      fVars_MLmap["DecayLxy_Xi"]       = fVar_Xic0[14]; // decay length of Xi in x-y plane
      fVars_MLmap["PA_XiToXic0"]       = fVar_Xic0[17]; // pointing angle of Xi (pointing back to Xic0)
      fVars_MLmap["PA_XiToPV"]         = fVar_Xic0[18]; // pointing angle of Xi (pointing back to PV)
      ...
      fVars_MLmap["chi2topo_LamToPV"] = fVar_Xic0[9];  // chi2_topo of Lambda to PV
      fVars_MLmap["DecayLxy_Lam"]     = fVar_Xic0[13]; // decay length of Lambda in x-y plane
      fVars_MLmap["PA_LamToXi"]       = fVar_Xic0[15]; // pointing angle of Lmabda (pointing back to Xi)
      fVars_MLmap["PA_LamToPV"]       = fVar_Xic0[16]; // pointing angle of Lambda (pointing back to PV)
      fVars_MLmap["chi2topo_LamToXi"] = fVar_Xic0[31]; // chi2_topo of Lambda to Xi
      fVars_MLmap["DCA_LamDau"]       = fVar_Xic0[35]; // DCA of Lambda's daughters (calculated from AOD cascade)
      fVars_MLmap["mass_Lam"]         = fVar_Xic0[19]; // mass of Lambda (without mass const.)
    }
    if (fIsAnaOmegac0) {
      ...
      fVars_MLmap["DCA_Omegac0Dau_KF"]    = fVar_Xic0[37]; // DCA of Omegac0's daughters (calculated from KF after Xi mass constraint)
      ...
      // Omega
      fVars_MLmap["DCA_OmegaDau"]            = fVar_Xic0[6]; // DCA of Omega's daughters (calculated from AOD cascade)
      fVars_MLmap["chi2geo_Omega"]           = fVar_Xic0[10]; // chi2_geometry of Omega (with Lambda mass const.)
      fVars_MLmap["ldl_Omega"]               = fVar_Xic0[11]; // l/dl of Omega (with Lambda mass const.)
      fVars_MLmap["chi2topo_OmegaToPV"]      = fVar_Xic0[12]; // chi2_topo of Omega to PV
      fVars_MLmap["DecayLxy_Omega"]          = fVar_Xic0[14]; // decay length of Omega in x-y plane
      fVars_MLmap["PA_OmegaToOmegac0"]       = fVar_Xic0[17]; // pointing angle of Omega (pointing back to Omegac0)
      fVars_MLmap["PA_OmegaToPV"]            = fVar_Xic0[18]; // pointing angle of Omega (pointing back to PV)
      ...
      fVars_MLmap["chi2topo_LamToPV"]    = fVar_Xic0[9];  // chi2_topo of Lambda to PV
      fVars_MLmap["DecayLxy_Lam"]        = fVar_Xic0[13]; // decay length of Lambda in x-y plane
      fVars_MLmap["PA_LamToOmega"]       = fVar_Xic0[15]; // pointing angle of Lmabda (pointing back to Omega)
      fVars_MLmap["PA_LamToPV"]          = fVar_Xic0[16]; // pointing angle of Lambda (pointing back to PV)
      fVars_MLmap["chi2topo_LamToOmega"] = fVar_Xic0[31]; // chi2_topo of Lambda to Omega
      fVars_MLmap["DCA_LamDau"]          = fVar_Xic0[35]; // DCA of Lambda's daughters (calculated from AOD cascade)
      fVars_MLmap["mass_Lam"]            = fVar_Xic0[19]; // mass of Lambda (without mass const.)
    }
    ...
    if (
        ...
        fVar_Xic0[42]<5. && fVar_Xic0[42]>0. && // ldl_Xic0
        fVar_Xic0[43]>-0.4 && fVar_Xic0[43]<0.4 && // ct_Xic0
        
        // pion (from Xic0) cuts
        fVar_Xic0[21]>fAnaCuts->GetPtMinPiFromXic0ForML() && // pt_PiFromXic0
        fVar_Xic0[25]>-1. && fVar_Xic0[25]<1. && // CosThetaStar_PiFromXic0
        fVar_Xic0[26]<10. && fVar_Xic0[26]>0. && // chi2prim_PiFromXic0
        fVar_Xic0[27]>-0.03 && fVar_Xic0[27]<0.03 && // DCAxy_PiFromXic0_KF

        // Xi cuts
        fVar_Xic0[6]<1.4 && fVar_Xic0[6]>0. && // DCA_XiDau
        fVar_Xic0[10]<10. && fVar_Xic0[10]>0. && // chi2geo_Xi
        fVar_Xic0[11]>1. && // ldl_Xi
        fVar_Xic0[12]<10. && fVar_Xic0[12]>0. && // chi2topo_XiToPV
        fVar_Xic0[14]>1. && // DecayLxy_Xi
        fVar_Xic0[17]<0.1 && fVar_Xic0[17]>0. && // PA_XiToXic0
        fVar_Xic0[18]<0.1 && fVar_Xic0[18]>0. && // PA_XiToPV
        fVar_Xic0[32]<10. && fVar_Xic0[32]>0. && // chi2topo_XiToXic0
        fVar_Xic0[38]>-0.4 && fVar_Xic0[38]<0.4 && // DCAxy_XiToPV_KF
        ...
        fVar_Xic0[31]<10. && fVar_Xic0[31]>0. && // chi2topo_LamToXi
  ...
  f2DHistNsigmaTPCTOF_PrFromLam->Fill(fVar_Xic0[5], fVar_Xic0[46]);
    ...
    if (fIsMC && (lab_Xic0>=0)) {
      ...
      Int_t IndexXic0 = mcPiFromXic0->GetMother();
      AliAODMCParticle *mcXic0 = static_cast<AliAODMCParticle*>(mcArray->At(IndexXic0));
      ptGen_Xic0 = mcXic0->Pt();
    }
    fVars_MLmap["pt_Gen"] = ptGen_Xic0; // mass of Xic0 (in generation)
    */
...
//_____________________________________________________________________________
void AliHFMLXicZeroToXiPifromKFP::DefineTreeMLoutput()
{
  // This is to define tree variables
