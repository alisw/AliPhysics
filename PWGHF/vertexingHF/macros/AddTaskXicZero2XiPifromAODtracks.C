...

{
  ...
  printf("CREATE TASK\n");
  AliAnalysisTaskSEXicZero2XiPifromAODtracks *task = new AliAnalysisTaskSEXicZero2XiPifromAODtracks("AliAnalysisTaskSEXicZero2XiPifromAODtracks",RDHFCutsXic2XiPianal,writeVariableTree);
  task->SetMC(theMCon);
  ...
  outputfile += ":PWG3_D2H_XicZero2XiPi_";
  ...
  AliAnalysisDataContainer *coutputXic2 = mgr->CreateContainer(Form("XicZero2XiPiCuts%s", outputFileName.Data()),TList::Class(),AliAnalysisManager::kOutputContainer,outputfile.Data()); // cuts
  mgr->ConnectOutput(task,2,coutputXic2);
  if (writeVariableTree) {
    AliAnalysisDataContainer *coutputXic3 = mgr->CreateContainer(Form("XicZerovariables%s", outputFileName.Data()),TTree::Class(),AliAnalysisManager::kOutputContainer,outputfile.Data()); 
    mgr->ConnectOutput(task,3,coutputXic3);
  }else{
    AliAnalysisDataContainer *coutputXic3 = mgr->CreateContainer(Form("XicZeroAll%s", outputFileName.Data()),TList::Class(),AliAnalysisManager::kOutputContainer,outputfile.Data()); // general histos
    mgr->ConnectOutput(task,3,coutputXic3);
  }
