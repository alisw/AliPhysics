...
AliCFTaskVertexingHFEMCal *AddTaskCFVertexingHFCascadeEMCal(const char* cutFile = "DStartoKpipiCuts010.root", TString cutObjectName="DStartoKpipiCuts", TString suffix="suf", Int_t configuration = AliCFTaskVertexingHFEMCal::kCheetah, Bool_t isKeepDfromB=kFALSE, Bool_t isKeepDfromBOnly=kFALSE, Int_t pdgCode = 413, Char_t isSign = 2, Bool_t useWeight=kTRUE, Bool_t useFlatPtWeight=kFALSE, Bool_t useZWeight=kFALSE, Bool_t useNchWeight=kFALSE, Bool_t useNtrkWeight=kFALSE, TString estimatorFilename="", Int_t multiplicityEstimator = AliCFTaskVertexingHFEMCal::kNtrk10, Bool_t isPPData=kFALSE, Bool_t isPPbData=kFALSE, Double_t refMult=9.26, Bool_t isFineNtrkBin=kFALSE,Bool_t bUseEMCalTrigger = kFALSE, TString nameEMCalTrigger = "G1", Bool_t bCheckEMCalAcceptance = kFALSE, Int_t nCheckEMCalAcceptanceNumber = 0, Bool_t bApplyEMCALClusterEventCut = kFALSE)
{
...
const Float_t multmax_100_400 = 400;
  ...
  else if (configuration == AliCFTaskVertexingHFEMCal::kRT) {
	...
	Info("AliCFTaskVertexingHFEMCal","SETUP CONTAINER");
	const Double_t phimax = 2*TMath::Pi();
	UInt_t nstep = 11; //number of selection steps: MC with limited acceptance, MC, Acceptance, Vertex, Refit, Reco (no cuts), RecoAcceptance, RecoITSClusters (RecoAcceptance included), RecoPPR (RecoAcceptance+RecoITSCluster included), RecoPID 
	...
	container -> SetStepTitle(1, "MC");
        ...
        container -> SetStepTitle(3, "MCEMCalTrigger");
        container -> SetStepTitle(4, "RecoVertex");
        container -> SetStepTitle(5, "RecoRefit");
        container -> SetStepTitle(6, "Reco");
        container -> SetStepTitle(7, "RecoAcc");
	container -> SetStepTitle(8, "RecoITSCluster");
	container -> SetStepTitle(9, "RecoCuts");
	container -> SetStepTitle(10, "RecoPID");
	...
	man->SetParticleCutsList(3 , emptyList); // EMCal trigger 
	man->SetParticleCutsList(4 , emptyList); // Vertex 
	man->SetParticleCutsList(5 , emptyList); // Refit 
	man->SetParticleCutsList(6 , emptyList); // AOD
	man->SetParticleCutsList(7 , emptyList); // AOD in Acceptance
	man->SetParticleCutsList(8 , emptyList); // AOD with required n. of ITS clusters
	man->SetParticleCutsList(9 , emptyList); // AOD Reco (PPR cuts implemented in Task)
	man->SetParticleCutsList(10 , emptyList); // AOD Reco PID
	...
	// create the task
	AliCFTaskVertexingHFEMCal *task = new AliCFTaskVertexingHFEMCal("AliCFTaskVertexingHFEMCal",cutsDStartoKpi);
