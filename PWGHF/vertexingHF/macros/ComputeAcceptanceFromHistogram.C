...
//-------------------------------------------------------------------------------------------------------------------
//
//  ComputeAcceptanceFromHistogram:
//    Compute the acceptance correction factor of open-charm hadrons with a toy MC.
//    The pt shape can be taken from PYTHIA or FONLL.
//
//    The macro is similar to ComputeAcceptance.C but use histogram (TH1D) instead of function (TF1)
//    for the random trial according to pt shape.
//    This is motivated by the fact that no efficient function has been found to match both the low- and high-pt
//    part of the D-meson spectra.
//
//-------------------------------------------------------------------------------------------------------------------
...
enum EPtShape{ kFlat, kFONLL13TeV, kPythia13TeV };
...
void ComputeAcceptanceFromHistogram()
{
  // main function
  ...
  // Load pt-shape histogram
  TH1D* funcPt = 0x0;

  if (fPtShape==kFONLL13TeV) {

    if (fDDecay==kDplusKpipi) {
      funcPt = LoadFONLL13TeV_Dplus();
    } else if (fDDecay==kDstarD0pi) {
      funcPt = LoadFONLL13TeV_Dstar();
    } else {
      funcPt = LoadFONLL13TeV_D0();
    }
    ...
    if (fDDecay==kDplusKpipi) {
      funcPt = LoadPYTHIA13TeV_Dplus();
    } else if (fDDecay==kDstarD0pi) {
      funcPt = LoadPYTHIA13TeV_Dstar();
    } else if (fDDecay==kDsKKpi) {
      funcPt = LoadPYTHIA13TeV_Ds();
    } else {
    ...
    funcPt = new TH1D("fFlat", "", 40, 0., 40.);
    for (Int_t ibin=1; ibin<=40; ++ibin) funcPt->SetBinContent(ibin, 1.e+7);
    outFileName.Append("flatpt.root");

  }


  printf("\n\tYou are using the toy MC with the following settings:\n");
  printf("\t%s\n\n", outFileName.Data());
...
TH1D* LoadPYTHIA13TeV_D0()
{
  ...
  for (Int_t ibin=0; ibin<80; ++ibin) hPYTHIA13->SetBinContent(ibin+1, val[ibin]);
