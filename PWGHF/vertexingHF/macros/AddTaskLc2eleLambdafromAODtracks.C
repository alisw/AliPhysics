...

{
  ...
  if(domixing)
	...
	if(ispp){
		...
		task->SetNumberOfEventsForMixing(400);//pp
	}else{
  ...
  AliAnalysisDataContainer *coutputLc6 = mgr->CreateContainer(Form("v0variables%1d",nTour),TTree::Class(),AliAnalysisManager::kOutputContainer,outputfile.Data()); // variables tree
