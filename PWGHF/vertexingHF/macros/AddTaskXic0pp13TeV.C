...
//------------------------------------------------------------------
AliAnalysisTaskSEXic0pp13TeV *AddTaskXic0pp13TeV(
		...
		)
{
 ...
 if (!mgr) {
    ...
    return NULL;
		...
		AliAnalysisTaskSEXic0pp13TeV *task = new AliAnalysisTaskSEXic0pp13TeV(taskname, Form("%s%s_task",taskname,option));
		task->SetMC(ismc);
