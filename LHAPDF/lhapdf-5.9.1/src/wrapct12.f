! -*- F90 -*-
      ...
      integer nset 
      Character Line*80 
      PARAMETER (MXX = 204, MXQ = 40, MXF = 6, MaxVal=4, nhess = 52) 
      PARAMETER (MXPQX = (MXF+1+MaxVal) * MXQ * MXX) 
      Common                                                            &
     & / CTPar1nhess12 /                                               &
     &  Al(nmxset), QV(0:MXX,nmxset), XV(0:MXX,nmxset),TV(0:MXQ,nmxset),           &
     &  UPD(0:nhess,MXPQX,nmxset), AlsCTEQ(0:MXQ,nmxset)                 &
...
!        
      ...
      U =         X * CtLhCT12Pdf(imem,1,X,Q) 
      D =         X * CtLhCT12Pdf(imem,2,X,Q) 
      USEA =      X * CtLhCT12Pdf(imem,-1,X,Q) 
      DSEA =      X * CtLhCT12Pdf(imem,-2,X,Q) 
      STR =       X * CtLhCT12Pdf(imem,-3,X,Q) 
      CHM =       X * CtLhCT12Pdf(imem,-4,X,Q) 
      BOT =       X * CtLhCT12Pdf(imem,-5,X,Q) 
      GLU  =      X * CtLhCT12Pdf(imem,0,X,Q) 
!                                                                       
      pdf(0)  = glu 
      pdf(1)  = d 
      pdf(-1) = dsea 
      ...
      return 
!                                                                       
      entry CT12getgrid(nset,ngridx,ngridq,gridx,gridq)
      do jx=0,nx(nset)
          gridx(jx+1)=xv(jx,nset)
      enddo
      do jq=0,nt(nset)
          gridq(jq+1)=qv(jq,nset)*qv(jq,nset)
      enddo
      ngridx=nx(nset)
      ngridq=nt(nset)
      return        
!                                                                       
      ...
      if(Line(1:11) .eq. '  ipk, Ordr') then
        ipdsformat = 10
        Read(1, *) ipk, Dr, QQalfa, alfaQ, (Amass(I,nset),I=1,6)
        Iorder(nset) = Nint(Dr)
        Read(1, '(A)') Line
        if(Line(1:7) .eq. '  IMASS' ) then
          ipdsformat = 11
          read(1, *) aimass, fswitch, N0, N0, N0, Nfmx(nset), MxVal
        else
          Read (1, *) N0, N0, N0, NfMx(nset), MxVal
        endif
      else
        ipdsformat = 6
      ...
      Read  (1, '(A)') Line 
      Read  (1, *) NX(nset),  NT(nset), N0, NG, N0
      if(NG.gt.0) Read(1,'(A)') (line, i=1,NG+1) 
      Read  (1, '(A)') Line 
      if(ipdsformat.eq.11) then 
        Read  (1, *)                                                      &
     &   QINI(nset), QMAX(nset), (QV(I,nset),TV(I,nset),AlsCTEQ(I,nset), I =0, NT(nset))         
        qbase1=QV(1,nset)/Exp(Exp(Tv(1,nset)))
        qbase2=QV(NT(nset),nset)/Exp(Exp(Tv(NT(nset),nset)))
        if(abs(qbase1-qbase2).gt.1e-5) then
           print *,'something wrong with qbase'
           print *,'qbase1, qbase2=',qbase1,qbase2
           stop
        else
          Al(nset)=(qbase1+qbase2)/2.0d0
          Alambda(nset) = Al(nset) 
        endif
      else
        Read  (1, *)                                                      &
...
!                                                                       
!                                                                       
      entry CT12alfa(alfas,Qalfa)
      if(ipdsformat.eq.11) then
          alfas = CtLhCT12Alphas(Qalfa)
      else
          alfas = pi*CtLhALPI(Qalfa)
      endif 
      return 
!                                                                       
      entry CT12init(Eorder,Q2fit) 
      return 
!                                                                       
      entry CT12pdf(mem) 
!        imem = mem                                                     
...
!                                                                       
      ...
      subroutine LHCT12set 
      Implicit Double Precision (A-H,O-Z) 
      common /CT12co/ xlast, qlast, nxsave 
      nxsave = -1000 
      xlast = -2. 
      qlast = -2. 
      return 
...
!=======================================================================
      Function CtLhPartonX12 (imem,IPRTN, XX, QQ) 
...
!                                                                       
      If     (JLx .LE. -1) Then 
        Print '(A,1pE12.4)','Severe error: x <= 0 in CtLhPartonX12 x=',x 
        Stop 
...
!                                      Keep at least 4 points >= Jx     
         Jx = JLx - 2 
      Else 
        Print '(A,1pE12.4)','Severe error: x > 1 in CtLhPartonX12 x=',x 
        Stop 
      Endif 
...
!                         the full range QV(0:Nt)  (in contrast to XV)  
     ...
     &    +  (tmp1*ty2-t12*ty3)*(fvec(4)-g4)/t34)                       
      ...
      END                                           
!=======================================================================
      Function CtLhCt12Pdf (imem,Iparton, X, Q) 
      Implicit Double Precision (A-H,O-Z) 
      include 'parmsetup.inc' 
      Logical Warn 
      Common                                                            &
     ...
     & / QCDtable /  Alambda(nmxset), Nfl(nmxset), Iorder(nmxset)       
      ...
      If (X .lt. 0D0 .or. X .gt. 1D0) Then 
        Print *, 'X out of range in CtLhCt12Pdf: ', X 
        Stop 
      Endif 
      If (Q .lt. Alambda(iset)) Then 
        Print *, 'Q out of range in CtLhCt12Pdf: ', Q 
        Stop 
      Endif 
...
!        put a warning for calling extra flavor.                        
             Warn = .false. 
             Print *, 'Warning: Iparton out of range in CtLhCt12Pdf: ' &
     &              , Iparton                                           
         Endif 
         CtLhCt12Pdf = 0D0 
         Return 
      ...
      CtLhCt12Pdf = CtLhPartonX12 (imem,Iparton, X, Q) 
      if(CtLhCt12Pdf.lt.0.D0)  CtLhCt12Pdf = 0.D0 
...
!                                 This is the interpolation variable in Q
      Call CtLhPolint4 (TV(jq,iset), AlsCTEQ(jq,iset),4,tt,Alsout,Dfq)
      CtLhCT12Alphas = Alsout
      Return
!                                       ********************
      End
!======================================================================
