      subroutine ABKM09evolve(xb,Q,fset)
      implicit none
      include 'parmsetup.inc'
      integer npdf,npar,kschem,i,k,n,m,kx,nxbb
      integer nxb,nq,np,nvar,nvar2,pdfmem,imem
      parameter(nxb=99,nq=20,np=9,nvar=30,nvar2=25)
...
!
      ...
      return
!
      entry ABKM09getgrid(nset,ngridx,ngridq,gridx,gridq)
...
!...X GRID
      ...
      do i=0,npdf
        do m=1,nq
          ...
          call ABKM09spline (nxb,xx,fsp,bs,cs,ds)
          do n=1,nxb
            bsp(k,n,m,i)=bs(n)
            csp(k,n,m,i)=cs(n)
            dsp(k,n,m,i)=ds(n)
...
!
     ...
     &      dlog(dlog(qsqmin/0.04d0)))/dble(nq-1)
      ...
      return
!
      entry ABKM09pdf(imem)
      pdfmem=imem
      if ((pdfmem.lt.0).or.(pdfmem.gt.npar)) then
         write(*,*) 'ABKM09 PDF set:'
...
! ---------------------------------------------------------------------
      SUBROUTINE ABKM09SPLINE(N,X,Y,B,C,D)
! ---------------------------------------------------------------------
