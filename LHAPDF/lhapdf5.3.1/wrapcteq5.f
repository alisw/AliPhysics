...
*
      ...
      return
*
      entry CTEQ5pdf(mem)		!error corrected (jcp)
c        imem = mem
        call getnset(iset)
...
c note, this routine doesn't have the features of not recalculating 
c the x or q point values if they have not changed since the last call.
c that makes it slower than cteq6, but since cteq5 is already obsolete,
c speed is not so important for it. 
C                                                 Work with Log (Q)
      QG  = LOG (Q/AL)
...
C                           Find lower end of interval containing X
 ...
 11   If (JU-JL .GT. 1) Then
      ...
      Jx = JL - (M-1)/2
c crude treatment if outside the defined interval.
      If (Jx .LT. 0) then
         Jx = 0
