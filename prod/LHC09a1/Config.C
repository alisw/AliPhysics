...
// -- Pythia Mb
  kPyMbNoHvq, kPyOmegaPlus, kPyOmegaMinus, kPyJetJet, kPyJetJetFixed,
...
AliGenPythia* Blubb(Int_t proc) {
  ...
  case kPyJetJet:
    comment = comment.Append(Form("pp->jet + jet at %3.0f TeV, pt hard %3.0f - %3.0f",eCMS,ptHardMin,ptHardMax));
  ...
  case kPyGammaBremsEMCAL:
      ...
      break;
  case kPyJetJetFixed:
      comment = comment.Append(" pp->jet + jet+jet in Fixed bin for testing");
      gener = new AliGenPythia(nEvts);
      gener->SetEnergyCMS(14000);//        Centre of mass energy
      gener->SetProcess(kPyJets);//        Process type
      gener->SetJetEtaRange(-1.1, 1.1);//  Final state kinematic cuts
      gener->SetJetPhiRange(0., 360.);
      gener->SetJetEtRange(95., 105.);
      gener->SetPtHard(80, 1000);// Pt transfer of the hard scattering
      gener->SetStrucFunc(kCTEQ5L);
...
void Config()
{
  ...
  if (proc <=   kBeautypp14000wmi) {
    ...
    AliGenPythia *pythia = Blubb((Int_t) proc ); 
   ...
   if(proc == kPyJetJetPHOSv2)
    ...
    pythia->SetPtKick(3.); // set the intrinsic kt to 5 GeV/c
    pythia->SetGluonRadiation(1,1);
    gener = pythia;
  }
