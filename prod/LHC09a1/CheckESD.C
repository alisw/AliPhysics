...
// check the content of the ESD
  ...
  for (Int_t iEvent = 0; iEvent < runLoader->GetNumberOfEvents(); iEvent++) {
    ...
    cout<<"CaloClusters "<<nCaloCluster<<endl;
    for (Int_t iCluster=0; iCluster<nCaloCluster; iCluster++){
      if(esd->GetCaloCluster(iCluster)->IsPHOS()) 
	hEPHOS->Fill(esd->GetCaloCluster(iCluster)->E());
      if(esd->GetCaloCluster(iCluster)->GetClusterType()==AliESDCaloCluster::kEMCALClusterv1) {
	hEEMCAL->Fill(esd->GetCaloCluster(iCluster)->E());
	//cout<<esd->GetCaloCluster(iCluster)->E()<<endl;
      }
    }
    }
  }
