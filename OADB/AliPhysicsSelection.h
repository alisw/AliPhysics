...
public:
  ...
  virtual AliBits GetSelectionBits(const TObject* obj) { return IsCollisionCandidate((const AliESDEvent*) obj); }
  ...
  AliBits IsCollisionCandidate(const AliESDEvent* aEsd);
  ...
  void AddCollisionTriggerClass(const char* /*className*/) { AliFatal("This method is deprecated. Please use SetCustomOADBObjects() instead."); }
  void AddBGTriggerClass(const char* /*className*/)        { AliFatal("This method is deprecated. Please use SetCustomOADBObjects() instead."); }
...
protected:
  AliBits CheckTriggerClass(const AliESDEvent* aEsd, const char* trigger, Int_t& triggerLogic) const;
  ...
  Float_t fBIFactorAC;                // ratio of interacting over non interacting bunch intensities for beam 1&2: 
                                      // for some trigger classes we do not distinguish between 1 and 2
  ...
  Bool_t fUseBXNumbers; // Explicitly select "good" bunch crossing numbers (exclude pilot, afterpulses and fakes). If you enable this you can only process runs within the same filling scheme
  ...
  ClassDef(AliPhysicsSelection, 17)
