  ...
  for(Int_t iRun=0; iRun<lNRuns; iRun++) {
    ...
    for(Int_t iEst=0; iEst<lNEstimators; iEst++) {
      ...
      for( Long64_t iEntry=0; iEntry<sTree[iRun]->GetEntries(); iEntry++) {
        ...
        sTree[iRun]->GetEntry(iEntry); //Get all variables
        fSelection->Evaluate ( fInput ); //Evaluate based on current variables
        l2dTrackletVsEstimatorData[iRun][iEst] -> Fill( fSelection->GetEstimator(iEst)->GetValue() , fSelection->GetEstimator(iEstSPDtracklets)->GetValue() );
      }
      cout<<endl;
      ...
      for( Long64_t iEntry=0; iEntry<sTreeMC[iRun]->GetEntries(); iEntry++) {
        ...
        fInput->GetVariable("fZncFired")->SetValue(1);
        fInput->GetVariable("fZpaFired")->SetValue(1);
        fInput->GetVariable("fZpcFired")->SetValue(1);
        fSelection->Evaluate ( fInput ); //Evaluate based on current variables
        l2dTrackletVsEstimatorMC[iRun][iEst] -> Fill( fSelection->GetEstimator(iEst)->GetValue() , fSelection->GetEstimator(iEstSPDtracklets)->GetValue() );
      }
      cout<<endl;
    }
  }
