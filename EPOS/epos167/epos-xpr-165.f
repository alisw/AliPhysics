c----------------------------------------------------------------------
      double precision function xDfit(i1,i2,s,xp,xm,b)
c----------------------------------------------------------------------
      ...
      do i=i1,i2
        ...
        xDfit=xDfit+dble(alp*xp**(bet+corp)*xm**(betp+cort)*s**(-epss))
      enddo
      return
      end
...
c----------------------------------------------------------------------
     ...
     &       /(x**dble(-alppar)*dble(chad(iclpro)*chad(icltar)))
      ...
      do i=0,nptg
        x=xminr
        if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
        write(ifhi,*) x,2.d0*om51p(real(x)*smaxDf,x,y,biniDf,1)
     &       /(x**dble(-alppar)*dble(chad(iclpro)*chad(icltar)))
      enddo
      ...
      do i=0,nptg
        x=xminr
        if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
        write(ifhi,*) x,2.d0*(om51p(real(x)*smaxDf,x,y,biniDf,2)
     ...
     &       /(x**dble(-alppar)*dble(chad(iclpro)*chad(icltar)))
      ...
      do i=0,nptg
        ...
        write(ifhi,*) x,xDfit(0,0,smaxDf,xp,xm,biniDf)
      enddo
      write(ifhi,'(a)')    '  endarray'
      write(ifhi,'(a)')    'closehisto plot 0-'
      ...
      do i=0,nptg
        ...
        xp=sqrt(real(x))*exp(real(y))
        xm=sqrt(real(x))*exp(-real(y))
        write(ifhi,*) x,xDfit(1,1,smaxDf,xp,xm,biniDf)
      enddo
      write(ifhi,'(a)')    '  endarray'
      write(ifhi,'(a)')    'closehisto plot 0-'
      ...
      do i=0,nptg
        ...
        xp=sqrt(real(x))*exp(real(y))
        xm=sqrt(real(x))*exp(-real(y))
        write(ifhi,*) x,xDfit(0,1,smaxDf,xp,xm,biniDf)
      enddo
      write(ifhi,'(a)')    '  endarray'
      write(ifhi,'(a)')    'closehisto plot 0'
...
c     & ,omYuncut,omNpuncut
     ...
     &               *(1-xm)**alplea(icltar)*(1-xp)**alplea(iclpro)
      ...
      do i=0,nptg
        ...
        xp=sqrt(real(x))*exp(real(y))
        xm=sqrt(real(x))*exp(-real(y))
          z(i)=xDfit(0,imax,engy**2,xp,xm,biniDf)
...
c      write(ifhi,'(a)')       'yrange auto auto'
      ...
      do i=0,nptg
        ...
        write(ifhi,*) b,Dsoftshval(real(x)*smaxDf,x,y,b,0)
      enddo
...
c      write(ifhi,'(a)')       'yrange -.01 auto'
      ...
      do i=0,nptg
        b=-bmax+2.*real(i)/real(nptg)*bmax
        write(ifhi,*) b,2.d0*om51p(real(x)*smaxDf,x,y,b,0)
     &       /(x**dble(-alppar)*dble(chad(iclpro)*chad(icltar)))
      enddo
...
c      write(ifhi,'(a)')       'yrange -.01 auto'
      ...
      do i=0,nptg
        b=-bmax+2.*real(i)/real(nptg)*bmax
        write(ifhi,*) b,2.d0*om51p(real(x)*smaxDf,x,y,b,1)
     &       /(x**dble(-alppar)*dble(chad(iclpro)*chad(icltar)))
      enddo
...
c      write(ifhi,'(a)')       'yrange -.01 auto'
      ...
      do i=0,nptg
        b=-bmax+2.*real(i)/real(nptg)*bmax
        write(ifhi,*) b,2.d0*(om51p(real(x)*smaxDf,x,y,b,2)+
...
c      write(ifhi,'(a)')       'yrange -.01 auto'
      ...
      do i=0,nptg
        b=-bmax+2.*real(i)/real(nptg)*bmax
        write(ifhi,*) b,Dsoftshval(real(x)*smaxDf,x,y,b,0)/Dint
      enddo
...
c      write(ifhi,'(a)')       'yrange -.01 auto'
      ...
      Dint=xDfit(0,imax,engy**2,
     &     real(dsqrt(x)*dexp(y)),real(dsqrt(x)*dexp(-y)),0.)
      
      do i=0,nptg
        b=-bmax+2.*real(i)/real(nptg)*bmax
        z=xDfit(0,imax,engy**2,
     &       real(dsqrt(x)*dexp(y)),real(dsqrt(x)*dexp(-y)),b)
...
c      iqq=int(xpar7)
      ...
      if(xpar6.eq.1.)t=real(xDfit(0,imax,smaxDf,xp,xm,0.))
      if(abs(t).lt.1.d-8)t=1.
      do i=0,nptg
        b=-bmax+2.*real(i)/real(nptg)*bmax
         z(i)=xDfit(0,imax,smaxDf,xp,xm,b)/dble(t)
         write(ifhi,*) b,z(i)
       enddo
...
c**********************************************************************
     ...
     &   /real(idxD0+1)
      ...
      do i=0,nptg
        b=-bmax+2.*real(i)/real(nptg)*bmax
        write(ifhi,*) b,(alpDs(idxD0,iclegy,iclpro,icltar)/t
     &   *(smaxDf*real(x))**(betDs(idxD0,iclegy,iclpro,icltar)
...
c**********************************************************************
     ...
     &         /(x**dble(-alppar)*dble(chad(iclpro)*chad(icltar))))
      ...
      do i=0,nptg
        b=-bmax+2.*real(i)/real(nptg)*bmax
        write(ifhi,*) b,2*real(om5s(real(x)*smaxDf,x,y,b,iqq1,iqq2)
...
c----------------------------------------------------------------------
      ...
      double precision x,omJ1(0:nptg),xtmp,w(0:nptg)
      double precision z(0:nptg),om51!,omYuncut,omYuncutJ
     ...
     &,omJ5(0:nptg)!,omJ6(0:nptg),omJ7(0:nptg)
      ...
      do i=0,nptg
        ...
        z(i)=0.d0
        do j=0,5
          z(i)=z(i)+om51(x,y,biniDf,j,j)
        enddo
        w(i)=om51(x,y,biniDf,-1,-1)
...
c*************************************************************************
      ...
      do i=0,nptg
        x=xminr
        if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
        omJ1(i)=om51(x,y,biniDf,0,0)
        if(xpar6.eq.0)t=z(i)/w(i)
        if(xpar6.eq.1)t=z(i)
        write(ifhi,*) x,omJ1(i)/t
      enddo
...
c*************************************************************************
      ...
      do i=0,nptg
        x=xminr
        if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
        omJ2(i)=om51(x,y,biniDf,1,1)
        if(xpar6.eq.0)t=z(i)/w(i)
        if(xpar6.eq.1)t=z(i)
        write(ifhi,*) x,omJ2(i)/t
      enddo
...
c*************************************************************************
      ...
      do i=0,nptg
        x=xminr
        if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
        omJ3(i)=om51(x,y,biniDf,2,3)
        if(xpar6.eq.0)t=z(i)/w(i)
        if(xpar6.eq.1)t=z(i)
        write(ifhi,*) x,omJ3(i)/t
      enddo
...
c*************************************************************************
      ...
      do i=0,nptg
        x=xminr
        if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
        omJ4(i)=om51(x,y,biniDf,4,4)
        if(xpar6.eq.0)t=z(i)/w(i)
        if(xpar6.eq.1)t=z(i)
        write(ifhi,*) x,omJ4(i)/t
      enddo
...
c*************************************************************************
      ...
      do i=0,nptg
        x=xminr
        if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
        omJ5(i)=om51(x,y,biniDf,5,5)
        if(xpar6.eq.0)t=z(i)/w(i)
        if(xpar6.eq.1)t=z(i)
        write(ifhi,*) x,omJ5(i)/t
      enddo
...
c*************************************************************************
      ...
      do i=0,nptg
        ...
        if(xpar6.eq.0)t=z(i)/w(i)
        if(xpar6.eq.1)t=z(i)
        write(ifhi,*) x,(omJ1(i)+omJ2(i)+omJ3(i)+omJ4(i)+omJ5(i))
     &                  /t 
      enddo
...
c----------------------------------------------------------------------
      ...
      double precision x,xminr,y,om5s,xtmp
c     & ,t,omYuncut
...
c**********************************************************************
      ...
      do i=0,nptg
        ...
        egy=sqrt(real(x))*engy
        iclegy=1+int((log(egy)-log(egylow))/log(egyfac))
      write(ifhi,*) x,2*om5s(real(x)*smaxDf,1.d0,0.d0,biniDf,iqq1,iqq2)
...
c----------------------------------------------------------------------
      ...
      double precision x,om5s,xDfit,z(0:200),ymax,y
c     & ,omYuncut,omNpuncut
...
c**********************************************************************
      ...
      do i=0,nptg
        y=-ymax+(ymax+ymax)*(dble(i)/dble(nptg))
        write(ifhi,*) y,2*om5s(real(x)*smaxDf,x,y,biniDf,iqq1,iqq2)
     &         /(x**dble(-alppar)*dble(chad(iclpro)*chad(icltar)))
      enddo
...
c----------------------------------------------------------------------
      ...
      double precision x,om5s,xDfit,y,enh!,omNpuncut
...
c      endif
      ...
      do i=0,nptg
        ...
        enh=0.d0
       write(ifhi,*) b,dble(b)*(2*om5s(real(x)*smaxDf,x,y,b,-1,-1)
     &                +enh)
     &         /(x**dble(-alppar)*dble(chad(iclpro)*chad(icltar)))
      enddo
...
c      endif
      ...
      do i=0,nptg
        ...
        xp=sqrt(real(x))*exp(real(y))
        xm=sqrt(real(x))*exp(-real(y))
        write(ifhi,'(2e14.6)') b,dble(b)*
     &       xDfit(0,imax,engy**2,xp,xm,b)
      enddo
...
c----------------------------------------------------------------------
      ...
      do nj=1,14  
       ...
       write(ifhi,'(a)')     'htyp '//ch//' xmod lin ymod log'
...
c        x=xminr+(xmaxDf-xminr)*(dble(i)/dble(nptg))
        y=0.d0
        if(engy**2..lt.5.e06)
     &  y=PhiExact(.5,dsqrt(x),dsqrt(x),engy**2,biniDf)
...
c********************** blue = PhiMExpo ******************************
      ...
      do i=0,nptg
        ...
        if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
c        x=xminr+(xmaxDf-xminr)*(dble(i)/dble(nptg))
        y=PhiExpo(.5,dsqrt(x),dsqrt(x),engy**2,biniDf)
     &       *dsqrt(x)**dble(-alplea(iclpro))
     &       *dsqrt(x)**dble(-alplea(icltar))
        write(ifhi,*) x,y
      enddo
...
c     &  y=PhiExact(1.,dsqrt(x),dsqrt(x),engy**2,biniDf)
...
c       ! if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
c        x=xminr+(xmaxDf-xminr)*(dble(i)/dble(nptg))
c        y=PhiExpo(1.,dsqrt(x),dsqrt(x),engy**2,biniDf)
c  !   &       *dsqrt(x)**dble(-alplea(iclpro))
c  !   &       *dsqrt(x)**dble(-alplea(icltar))
c        write(ifhi,*) x,y
c      enddo
...
c        v(i)=0.d0
c        if(engy**2..lt.5.e06)
c     *  v(i)=PhiExact(1.,1.d0-x,1.d0-xm,engy**2,biniDf)
cc     *  v(i)=PhiExact(1.,z,z,engy**2,biniDf)
...
c        endif
cc        z=1.d0-dsqrt(x)
cc        w(i)=PhiExpo(1.,z,z,engy**2,biniDf)
c        w(i)=PhiExpo(1.,1.d0-x,1.d0-xm,engy**2,biniDf)
c        y=u(i)*w(i)
c        if(xpar5.ne.0.)y=dabs(y)
c        write(ifhi,*)x,y
c      enddo
...
c********************** full-red = PhiExact ***************************
     ...
     * 'txt  "title E=',nint(engy),' b=',biniDf,'"'
      ...
      do i=0,nptg
        ...
        y=PhiExact(1.,dsqrt(x),dsqrt(x),engy**2,biniDf)
     &       *dsqrt(x)**dble(-alplea(iclpro))
     &       *dsqrt(x)**dble(-alplea(icltar))
        write(ifhi,*)x,y
      enddo
...
c******************** blue = PhiExpo ***************************
      ...
      do i=0,nptg
        ...
        !if (i.ne.0) x=x*(xmaxDf/xminr)**(dble(i)/dble(nptg))
        x=xminr+(xmaxDf-xminr)*(dble(i)/dble(nptg))
        z(i)=PhiExpo(1.,dsqrt(x),dsqrt(x),engy**2,biniDf)
     &       *dsqrt(x)**dble(-alplea(iclpro))
     &       *dsqrt(x)**dble(-alplea(icltar))
        write(ifhi,*) x,z(i)
      enddo
...
c.....xminr=0.d0   !value of xminr for plotting the function 
      ...
      if(iomega.eq.2)imax=1
      do i=idxDmin,imax
        call GfunPar(1,i,biniDf,smaxDf,alpx,betx,betpx,epsp,epst,epss
     &               ,gamv)
      enddo
...
c************************ dotted = gauss integration ******************
      ...
      do k=1,nbib
        b=b1+(k-0.5)*db   
        do i=idxDmin,imax
          call GfunPar(1,i,b,smaxDf,alpx,betx,betpx,epsp,epst,epss,gamv)
          call GfunPar(2,i,b,smaxDf,alpx,betx,betpx,epsp,epst,epss,gamv)
        enddo
        y=HPhiInt(smaxDf,b)
        write(ifhi,*)b,y
      enddo
...
c********************* full-red = 1-PhiExact **************************
      ...
      do k=1,nbib
        b=b1+(k-0.5)*db   
        y=1.d0-PhiExact(1.,1.d0,1.d0,engy**2,b)
        write(ifhi,*)b,y
      enddo
      write(ifhi,'(a)')    '  endarray'
      write(ifhi,'(a)')    'closehisto plot 0-'
...
c************************** blue-dashed = 1-PhiExpo *******************
      ...
      do k=1,nbib
        b=b1+(k-0.5)*db   
        y=1.d0-PhiExpo(1.,1.d0,1.d0,engy**2,b)
        write(ifhi,*)b,y
      enddo
      write(ifhi,'(a)')    '  endarray'
      write(ifhi,'(a)')    'closehisto plot 0-'
...
c********************* full-red = 1-PhiExact **************************
      ...
      do k=1,nbib
        b=b1+(k-0.5)*db   
        y=1.d0-PhiExact(.5,1.d0,1.d0,engy**2,b)
        write(ifhi,*)b,y
      enddo
      write(ifhi,'(a)')    '  endarray'
      write(ifhi,'(a)')    'closehisto plot 0-'
...
c************************** blue-dashed = 1-PhiExpo *******************
      ...
      do k=1,nbib
        b=b1+(k-0.5)*db   
        y=1.d0-PhiExpo(.5,1.d0,1.d0,engy**2,b)
        write(ifhi,*)b,y
      enddo
      write(ifhi,'(a)')    '  endarray'
      write(ifhi,'(a)')    'closehisto plot 0'
...
c$$$      if(xp.ne.0.d0)xm=xh/xp
      ...
      write(ifhi,'(a)')       'htyp lin'
      write(ifhi,'(a)')       'xmod lin ymod log'
      write(ifhi,'(a,2e11.3)')'xrange',0.,min(10.,xcut+1.)
      write(ifhi,'(a)')'yrange auto auto'
...
c----------------------------------------------------------------------
      ...
      sfsh=xDfit(0,1,smaxDf,xp,xm,0.)
      if(dabs(sfsh).ge.1.d-5)then
      do i=0,nptf-1
        bf(i+1)=dble(-bmaxDf+real(i)*2.*bmaxDf/real(nptf-1))
        Db(i+1)=xDfit(0,1,smaxDf,xp,xm,real(bf(i+1)))/sfsh
      enddo
...
c----------------------------------------------------------------------
      ...
      enddo
      do j=1,n
        if(mod(j,10000).eq.0)write(*,*)"y1",j
        x= 0 !  om1yr(xh,b)
