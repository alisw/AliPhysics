...
c     *,xprh,xmrh
     ...
     *,qmin(2),iqc(2),nqc(2),ncc(2,2),amqt(4)
     ...
     &,idaprt(2,ntim)
      ...
      integer icp(2),ict(2),icini(2)
      integer jcp(nflav,2),jct(nflav,2),nemis(2)
      ...
      q2finsave=q2fin
      zzzz=zzremn(ip,1)+zzremn(it,2)
      zz=min( 1+zoemax , 1+zoeinc  *zzzz ) !<-----
      q2fin=q2fin*zz
...
c      xmrh=xmt(jh)
      ...
      nj0=nj
      if(ish.ge.6)write(ifch,*)'iptl,nptl,wp0,wm0,z,iqq,bx:'
      if(ish.ge.6)write(ifch,*) iptl,nptl,wp0,wm0,z,iqq,bx
...
c          endif
        ...
        if(aks.lt.wwuu)then
          if(iclpro.ne.4)then
            ...
            iq1=4*(2.*int(.5+rangen())-1.)
          endif
          iq2=1
        elseif(aks.lt.wwuu+wwdd)then
          if(iclpro.eq.2)then
            iq1=2
          elseif(iclpro.eq.1)then
            iq1=-2
          ...
          iq2=2
        elseif(aks.lt.wwuu+wwdd+wwud)then
          if(iclpro.ne.4)then
            iq1=1
          else
            iq1=4*(2.*int(.5+rangen())-1.)
 ...
 4        continue
     ...
     *        *(r2had(iclpro)+r2had(icltar))/z
          ...
          if(gbyj.gt.1.d0.and.ish.ge.2)write(ifmt,*)'gbyj',gbyj
...
c-------------------------------------------------------------------------
  ...
  6   continue
      ...
      if((iqq-1)*(iqq-3).eq.0)then
        ...
        if(iabs(idptl(ip)).eq.1220)iq1=3-iq1      !proj=neutron
        iqc(1)=iq1                                !proj=particle
        if(idptl(ip).lt.-1000)iqc(1)=-iq1         !proj=antiparticle
        nj=nj+1
        ifl1=iabs(iq1)

        if(iqc(1).gt.0)then               
          call idsufl(ifl1,1,jcp,iret1)   !remove valence quark from remnant
          if(iret1.ne.1)ifl=idrafl(iclpro,jcp,2,'s',iret2)     !take sea antiquark
        elseif(iqc(1).lt.0)then
          call idsufl(ifl1,2,jcp,iret1)
          if(iret1.ne.1)ifl=idrafl(iclpro,jcp,1,'s',iret2)
        else
        ...
        if(iret1.eq.1.or.iret2.eq.1)then
          ifl=ifl1
         if(ish.ge.3)write(ifmt,*)'Not enough place in rem (psahot 1)'
         if(ish.ge.5)write(ifch,*)'Not enough place in rem (psahot 1)'
          call iddeco(icp,jcp)
          iret1=0
          iret2=0
        endif

        if(ish.ge.5)write(ifch,*)'flavor vq+,sqb+',iq1,-ifl
     ...
     *    eqj(2,nj))*(eqj(1,nj)+eqj(2,nj))-eqj(3,nj)**2-eqj(4,nj)**2 
        ...
        nj=nj+1
        if(idfpomr.ne.3)then
          iflq=idrafl(iclpro,jcp,1,'s',iret1)       !Choose flavor of sea quark.
          if(iret1.ne.1)iflqb=idrafl(iclpro,jcp,2,'s',iret2)  !Choose antiquark.
        else
          iflq=jdrafl(iclpro,jcp,1,iret1)       !Choose flavor of sea quark.
          iflqb=iflq                   !antiquark=quark (vertex end)
        endif

        if(iret1.eq.1.or.iret2.eq.1)then
          iflqb=iflq
         if(ish.ge.3)write(ifmt,*)'Not enough place in rem (psahot 2)'
         if(ish.ge.5)write(ifch,*)'Not enough place in rem (psahot 2)'
          call iddeco(icp,jcp)
          iret1=0
          iret2=0
        endif
...
7           zg=1.d0-dble(rangen())*(1.d0-xp)
     ...
     *      eqj(2,nj)**2-eqj(3,nj)**2-eqj(4,nj)**2 
      ...
      if((iqq-2)*(iqq-3).eq.0)then
        if(iabs(idptl(maproj+it)).eq.1220)iq2=3-iq2     !targ=neutron
        iqc(2)=iq2             !tar=particle (can not be pion or kaon !)
        if(idptl(maproj+it).lt.-1000)iqc(2)=-iq2  !targ=antiparticle
        ifl2=iabs(iq2)

        nj=nj+1
        if(iqc(2).gt.0)then               
          call idsufl(ifl2,1,jct,iret1)   !remove valence quark from remnant
          if(iret1.ne.1)ifl=idrafl(icltar,jct,2,'s',iret2)    !take sea antiquark
        elseif(iqc(2).lt.0)then
          call idsufl(ifl2,2,jct,iret1)
          if(iret1.ne.1)ifl=idrafl(icltar,jct,1,'s',iret2)   
        else
          call utstop('No quark for hard Pomeron- in psahot!&')  
        endif
        ...
        if(iret1.eq.1.or.iret2.eq.1)then
          ifl=ifl2
         if(ish.ge.3)write(ifmt,*)'Not enough place in rem (psahot 3)'
         if(ish.ge.5)write(ifch,*)'Not enough place in rem (psahot 3)'
          call iddeco(ict,jct)
          iret1=0
          iret2=0
        endif

        if(ish.ge.5)write(ifch,*)'flavor vq-,sqb-',iq2,-ifl
     ...
     *    +eqj(2,nj))*(eqj(1,nj)-eqj(2,nj))-eqj(3,nj)**2-eqj(4,nj)**2 
        ...
        nj=nj+1
        if(idfpomr.ne.2)then
          iflq=idrafl(icltar,jct,1,'s',iret1)       !Choose flavor of sea quark.
          if(iret1.ne.1)iflqb=idrafl(icltar,jct,2,'s',iret2) !Choose  antiquark.
        else
          iflq=jdrafl(icltar,jct,1,iret1)       !Choose flavor of sea quark.
          iflqb=iflq                   !antiquark=quark (vertex end)
        endif

        if(iret1.eq.1.or.iret2.eq.1)then
          iflqb=iflq
         if(ish.ge.3)write(ifmt,*)'Not enough place in rem (psahot 4)'
         if(ish.ge.5)write(ifch,*)'Not enough place in rem (psahot 4)'
          call iddeco(ict,jct)
          iret1=0
          iret2=0
        endif
...
c        write(*,*)'sem:',wpt(jj),pt2,q2mass,wpt(3-jj),jj
     ...
     *(qt2+dble(q2part))/(1.d0-x)/wplc))
      ...
      if(ish.ge.5)write(ifch,*)'qq,eprt,iq2ini',qq,eprt,iq2ini,
     *eprt**2-qt2
      ntest=0
11    ntest=ntest+1
      call timsh1(qp2max,sngl(eprt),iq2ini)
      amprt=pprt(5,1)**2
...
c      enddo
      if(ish.ge.3)then
        ...
        do k=1,nj
          ...
          write(ifch,'(i3,2x,4e10.3,2x,2i3)')iqj(k)
     *    ,(eqj(j,k),j=1,4),(ncj(j,k),j=1,2)
      ...
      enddo
      nptl0=nptl
      if(ish.ge.5)write (ifch,*)'nptl,nj',nptl,nj
          ...
          if(nptl-nptl0.lt.nj)then
            ...
            if(ish.ge.3)then
              ...
              do k=nptl0,nptl
                ...
                write(ifch,'(i5,2x,5e10.3)')idptl(k)
     *          ,(pptl(j,k),j=1,5)
              enddo
            endif
            jfl=1
...
c-----------------------------------------------------------------------
      ...
      if(nptl.ge.mxptl)then
       ...
       call alist('Error in pspawr: nptl out of bounds &',1,nptl)
       stop'nptl out of bounds'
      endif 
      
      if(ifrptl(1,iptl).eq.0)ifrptl(1,iptl)=nptl+1
      ifrptl(2,iptl)=nptl+1
...
clight cone momentum share for the first parton
     ...
     $   .or.(jt.eq.13.and.jm.eq.2))z=1-z
      ...
      call timsh2(qq1,qq2,sqrt(si),iq2ini1,iq2ini2)  !final state cascade
...
1     continue
     ...
     *  ncc(1,iprt)
      ...
      if(idau1.ne.0.)then         !virtual parton
        ...
        if(ish.ge.6)then
          write (ifch,*)'jet-nj,iprt,icp,iqj(nj),ncj',
     *    nj,iprt,icp,iqj(nj),ncj(1,nj)
