...
c Variables used for xparg (only graphical application)
      ...
      sfshlim=5.*spmin          !transition energy for soft->hard
...
c iclpro=1 or 3 and iclpro=2
     ...
     &          ,iscreen,iclpro,icltar,iclegy,engy,sfshlim
...
c starting values
        ...
        if(smaxDf.ge.3.*sfshlim)then
          call paramx           !alpD and betD

          call pompar(alpsf,betsf,-1) !soft (taking into account hard)
          parDf(1,3)=alpsf
          parDf(2,3)=max(-0.95+alppar,betsf)
...
c        alpdifs=alpdif
c        alpdif=0.99
        betDp(2,iclpro,icltar)=-alpdif+alppar
        betDpp(2,iclpro,icltar)=-alpdif+alppar
c        alpD(2,iclpro,icltar)=(alpsf+alpsh)*wdiff(iclpro)*wdiff(icltar)
        alpD(2,iclpro,icltar)=wdiff(iclpro)*wdiff(icltar)
     &            /utgam1(1.-alpdif)**2
     &            *utgam1(2.-alpdif+alplea(iclpro))
     &            *utgam1(2.-alpdif+alplea(icltar))
...
c for plots record alpDs, betDs, etc ...        
      ...
      if(ish.ge.4)then             !check PhiExact value for x=1
        y=PhiExact(1.,1.d0,1.d0,smaxDf,0.)
...
c----------------------------------------------------------------------
      ...
      if(iqq.le.0) then
        ...
        do i=0,nptf-1
          ...
          D=max(1.d-10,Dsoftshval(real(X)*smaxDf,X,0.d0,0.,iscr))
          if(D.eq.1.d-10)then 
            write(ifch,*) 
     &    "Warning in pompar ! Dsoftshval(0) could be negative"
            sigma(i+1)=1.e5
          endif
          xlnXs(i+1)=real(dlog(X*dble(smaxDf)))
          xlnD(i+1)=real(dlog(D))
        enddo
...
c Fit of D(X) between X0 and xmax
        ...
        alpha=real(Dsoftshval(real(X0)*smaxDf,X0,0.d0,0.,iscr))
     &       *(real(X0)*smaxDf)**(-beta)   
...
c Definition of D0=D(X0)
        ...
        D1=Dsoftshval(real(xmax)*smaxDf,xmax,0.d0,0.,iscr)
...
c Fit of D(X) between X0 and xmax
        ...
        alpha=real(Dsoftshval(real(xmax)*smaxDf,xmax,0.d0,0.,iscr))
     &       *(real(xmax)*smaxDf)**(-beta)   
...
c----------------------------------------------------------------------
      ...
      k=0
      x0=min(xfitmin,100.d0*xminDf)
      x1=xmax
 5    d2=dabs(Dsoftshval(real(x0)*smaxDf,x0,0.d0,0.,iscr))
      if(d2.lt.1.d-10.and.x0.lt.x1)then
        x0=dsqrt(x0*x1)
c        write(ifch,*)"droot",x0,x1,d0,d1,d2
        goto 5
        elseif(d2.gt.d0)then
        droot=max(x0,xfitmin)
c        write(ifch,*)"droot",x0,x1,d0,d1,d2
        return
      endif
      f0=d2-d0
 ...
 10   x2=dsqrt(x0*x1)
      d2=dabs(Dsoftshval(real(x2)*smaxDf,x2,0.d0,0.,iscr))
      f2=d2-d0
      k=k+1
c        write (ifch,*) '******************* droot **************'
...
c----------------------------------------------------------------------
      ...
      elseif(iqq.eq.2)then
        ...
        X0=.1d0/dlog(max(dexp(2.d0),dble(smaxDf)/1.d3)) 
        if(smaxDf.lt.100.*q2min)X0=.95d0
        xmax=xmaxDf
      endif
...
c----------------------------------------------------------------------
      ...
      if(iqq.eq.0)then
        range=sig2
        sfsh=om51p(real(x)*smaxDf,x,0.d0,0.,0)
        if (dabs(sfsh).gt.eps) then
        do i=0,nptf-1
          bf(i+1)=dble(bmin+real(i)*(bmax-bmin)/real(nptf-1))
          Db(i+1)=om51p(real(x)*smaxDf,x,0.d0,real(bf(i+1)),0)/sfsh
        enddo
        else
          ierro=1
        endif
      elseif(iqq.eq.1.and.xshmin.lt..95d0)then
        ...
        sfsh=0.d0 
        do j=1,4
          sfsh=sfsh+om51p(real(x)*smaxDf,x,0.d0,0.,j)
        enddo
        if (dabs(sfsh).gt.eps) then
        do i=0,nptf-1
          bf(i+1)=dble(bmin+real(i)*(bmax-bmin)/real(nptf-1))
          Db(i+1)=0.d0
          do j=1,4
           Db(i+1)=Db(i+1)+om51p(real(x)*smaxDf,x,0.d0,real(bf(i+1)),j)
          enddo
          Db(i+1)=Db(i+1)/sfsh
        enddo
        else
        ...
        range=sig2
        iscr=0 
        sfsh=Dsoftshval(real(x)*smaxDf,x,0.d0,0.,iscr)
        if (dabs(sfsh).gt.eps) then
        do i=0,nptf-1
          bf(i+1)=dble(bmin+real(i)*(bmax-bmin)/real(nptf-1))
          Db(i+1)=Dsoftshval(real(x)*smaxDf,x,0.d0,real(bf(i+1)),iscr)
     &              /sfsh
        enddo
        else
          ierro=1
...
c----------------------------------------------------------------------
      ...
      X0=max(xminDf,dble(sfshlim/smaxDf))           
      X1=xmaxDf
      Xseuil=.0001d0
...
c Fit of G(X) between X0 and X1
     ...
     &                  real(Dsoftshval(real(X)*smaxDf,X,0.d0,0.,1)))
        datafitD(i+1,1)=real(X)
        datafitD(i+1,3)=1.
        if (X.gt.Xseuil) datafitD(i+1,3)=exp((Xseuil/X)-1.)
      enddo
...
c----------------------------------------------------------------------
      ...
      iscr=0
      Dint=Dsoftshval(real(x)*smaxDf,x,0.d0,0.,iscr)

      sigma1i=0.
      if(Dint.ne.0.)
     &sigma1i=real(-1.d0/dlog(Dsoftshval(real(x)*smaxDf,x,0.d0,1.,iscr)
     &   /Dint))
...
c      enddo
      ...
      enddo
      err=abs(err)/real(numdataDf)
      
c metropolis iteration

      do i=1,nmc 
c        if(mod(i,int(real(nmc)/1000.)).eq.0)then
          betac=betac*(1.+1./real(nmc))!1.05
          betae=betae*(1.+1./real(nmc))!1.05
c        endif
c        if(mod(i,int(real(nmc)/20.)).eq.0)write(ifch,*)i,chi2,err
        ...
        n=max(n,numminDf)
        n=min(n,numparDf)
c              if(mod(i,int(real(nmc)/20.)).eq.0)write(ifch,*)n
        
 10     parDf(n,3)=parDf(n,1)+rangen()*(parDf(n,2)-parDf(n,1))
        chi2=0
        err=0
        do j=1,numdataDf
          x=dble(datafitD(j,1))
...
c        write(ifch,*) '$$$$$$$$$$ fit : a,b,siga,sigb,chi2,q $$$$$$$$$$$'
c        write(ifch,*) a,b,siga,sigb,chi2!???????????????
...
c----------------------------------------------------------------------
      ...
      Dsoftshpar=dble(
     &        parDf(1,3)*(real(x)*smaxDf)**parDf(2,3)
     &       +parDf(3,3)*(real(x)*smaxDf)**parDf(4,3) )
      xp=dsqrt(x)
      xm=xp
