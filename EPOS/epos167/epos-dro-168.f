...
c----------------------------------------------------------------------
      include 'epos.inc'
      common/drop9/mkeu,mked,mkes,mkec,mkeb,mket
     $     ,iwkeu,iwked,iwkes,iwkec,iwkeb,iwket
...
ctp060829      taus=sngl(ttaus)
     ...
     *' keu ked kes kec keb ket:',keu,ked,kes,kec,keb,ket
      ...
      jc(i,2)=0
      enddo
 10   if(mkeu.ne.0.or.iwkeu.ne.0)then
 12     x=-log(rangen())*iwkeu+mkeu
        if(rangen().lt.0.5) x=real(mkeu)-(x-real(mkeu))
        if( exp(-((x-mkeu)/iwkeu)**2) / exp(-abs(x-mkeu)/iwkeu)/2
     $       .lt. rangen() ) goto 12
        keu=nint(x)
      endif
      if(mked.ne.0.or.iwked.ne.0)then
 13     x=-log(rangen())*iwked+mked
        if(rangen().lt.0.5) x=real(mked)-(x-real(mked))
        if( exp(-((x-mked)/iwked)**2) / exp(-abs(x-mked)/iwked)/2
     $       .lt. rangen() ) goto 13
        ked=nint(x)
      endif
      if(mkes.ne.0.or.iwkes.ne.0)then
 15     x=-log(rangen())*iwkes+mkes
        if(rangen().lt.0.5) x=real(mkes)-(x-real(mkes))
        if( exp(-((x-mkes)/iwkes)**2) / exp(-abs(x-mkes)/iwkes)/2
     $       .lt. rangen() ) goto 15
        kes=nint(x)
      endif
...
c       if(ireten.eq.1)call utstop('ametro: idenco ret code = 1&')
     ...
     *       +mod(jc(1,2)+jc(2,2)+jc(3,2)+jc(4,2),10**4)
      ...
      istptl(nptl)=istptl(nptl)+1
      ifrptl(1,nptl)=nptlb+1
      ifrptl(2,nptl)=nptl
      x=xorptl(1,nptl)
      y=xorptl(2,nptl)
      z=xorptl(3,nptl)
      ...
      if(ioceau.eq.1)call xhnbte(0)
      if(iociau.eq.1)call xhnbti(0)
...
c----------------------------------------------------------------------
      subroutine hnbxxx(ip,iret)
c----------------------------------------------------------------------
c  decays droplet very fast ... and hopefully not too badly
c----------------------------------------------------------------------
      include 'epos.inc'
      integer jc(nflav,2),jc1(nflav,2)
      integer ifl(nflav)
...
ctp060829      parameter (mxidh=3331)
      real wbar(-1:1)

      call utpri('hnbxxx',ish,ishini,4)
      iret=0
...
c...fill wspecs
      ...
      e=pptl(5,ip)
      k=1
      do while(k.lt.mxe.and.ee(k).lt.e)
        k=k+1
      enddo
      k=max(k,2)
      xi=(e-ee(k-1))/(ee(k)-ee(k-1))
      do i=1,nspecs
        ii=i
      ...
      enddo
            ...
            write(ifch,'(9x, 9f6.3)')(wspecs(i),i=10,18)
            write(ifch,'(9x, 8f6.3)')(wspecs(i),i=19,26)
            write(ifch,'(9x, 8f6.3)')(wspecs(i),i=27,34)
            write(ifch,'(9x,10f6.3)')(wspecs(i),i=35,44)
            write(ifch,'(9x,10f6.3)')(wspecs(i),i=45,54)
            endif
...
c...initializations
      ...
      wbar(-1)=0
      do i=19,53,2
       wbar(1)=wbar(1)+wspecs(i)
       wbar(-1)=wbar(-1)+wspecs(i+1)
      enddo
      wbar(1)=wbar(1)  /wtot
      wbar(-1)=wbar(-1)/wtot
      w12=0
...
c...print    
           ...
           write(ifch,*)'jc=',jc
           write(ifch,*)'nq=',nq,'  na=',na,'  nbar=',nbar
           write(ifch,*)'wtot=',wtot
           endif
...
c...generate number of hadrons
 ...
 776    nhad=0
        ...
        do while (sum.lt.rr)
          nhad=nhad+1
          if(nhad.gt.10*aa)goto776
          pr=pr*aa/nhad
          sum=sum+pr
            !print*,'r:',rr,'  n:',nhad,'  sum pr:',sum
        enddo
        nhad=max(2,nhad)
      else
 778    nhad=aa-30
        ...
        do while (sum.lt.rr)
          nhad=nhad+1
          if(nhad.gt.10*aa)goto778
          sum=sum+exp(nhad-aa)*(aa/nhad)**nhad/sqrt(2*pi*nhad)
            !print*,'r:',rr,'  n:',nhad,'  sum pr:',sum
        enddo
        nhad=max(2,nhad)
      endif
...
c...generate first n-2 hadrons 
  ...
  1     sum=0
     ...
     *  .and.nbari*(nbar-nbari).lt.0.and.rangen().gt.wbar(-nbari))then
          if(ish.ge.5)
     *    write(ifch,*)'-----',nptl,nbar,ispecs(i),wbar(-nbari),rr
          goto1
...
c...last two hadrons
     ...
     *       write(ifch,*)'nptl=',nptl,'  baryon=',id,'  jc=',jc   
      ...
      do while (nqu.gt.0.or.nptl.eq.nptlb)
        if(nqu.eq.0)then
          i1=1.5+rangen()
          i2=1.5+rangen()
          jc(i1,1)=jc(i1,1)+1
...
c... generate momenta 
      ...
      do n=nptlb+1,nptl
        ...
        b=0.9
        b=max(b,am+0.1)
        !relative weights ... consider here f1/exp(7), f2/exp(7)
        c1=(b-am)*b**2*exp(-a*b)
        c2=(b**2/a+2*b/a**2+2/a**3)*exp(-a*b)
            if(ish.ge.5)write(ifch,*)'c1 c2:',c1,c2
        rrr=1
        fff=0
        do while(rrr.gt.fff)
          !proposal
          i=2
          if(rangen().lt.c1/(c1+c2))i=1
               if(ish.ge.5)write(ifch,*)'i=',i
          if(i.eq.1)then
            x=am+rangen()*(b-am)
            fx=b**2*exp(-a*b)  *exp(7.)
          else
            r=alog(rangen())
            !find root of log((x**2/a+2*x/a**2+2/a**3)*exp(-a*x)/c2)-r
            x=20
            do k=1,4
              f=alog(x**2/a+2*x/a**2+2/a**3)-a*x-alog(c2)-r
              fp=(2*x/a+2/a**2)/(x**2/a+2*x/a**2+2/a**3)-a
              xb=x
              x=x-f/fp
                   if(ish.ge.5)write(ifch,*)'k,xb,f,fp,x: ',k,xb,f,fp,x
            enddo
            fx=x**2*exp(-a*x)   *exp(7.)
          endif
          !acceptance
          rrr=rangen()
          if(x.le.2)then
            fff=sqrt(x-am)*x*exp(-4*x-1.75*x**2)  /fx
          else
            fff=sqrt(x-am)*x*exp(7-11*x)          /fx
          endif
               if(ish.ge.5)write(ifch,*)'fff,rrr',fff,rrr
        enddo
        e=x
        pa=sqrt(e**2-am**2)
        u(3)=2.*rangen()-1.
        ...
        do j=1,3
         pptl(j,n)=pa*u(j)
         pout(j)=pout(j)+pptl(j,n)
        enddo
        pptl(4,n)=e
        pptl(5,n)=am
        pout(4)=pout(4)+e
        if(ish.ge.5)
     *  write(ifch,*)pptl(1,n),pptl(2,n),pptl(3,n),pptl(4,n),pptl(5,n)
      enddo

c...check total energy ... rescale (maybe)

            if(ish.ge.5)then
            write(ifch,*)'pout(cms)=',pout
            write(ifch,*)'energy_in(cms)=  ',sngl(c(5))
            endif
...
c...boost
      ...
      do n=nptlb+1,nptl
        ...
        do j=1,4
         pptl(j,n)=p(j)
         poutx(j)=poutx(j)+pptl(j,n)
        enddo
      enddo
           if(ish.ge.5)then
           write(ifch,*)'pout(lab)=',poutx
           write(ifch,*)'pin(lab)= ',pin
           endif
...
c----------------------------------------------------------------------
      ...
      common/xxxspecsy/ndrop(-4:4,-4:4,-4:4)
      common/xxxspecsx/ee(mxe),wwspecs(mxdrop,mxe,mspecs)
      inquire(file=fndr(1:nfndr),exist=lcalc)
      if(lcalc)then
        if(inicnt.eq.1)then
          do ku=-4,4
            do kd=-4,4
          ...
          do n=1,mxdrop
            ...
            if(abs(kd).gt.4)stop'hnbxxxini: kd out of range'
            if(abs(ks).gt.4)stop'hnbxxxini: ks out of range'
            ndrop(ku,kd,ks)=n
            ndrop(-ku,-kd,-ks)=-n
          enddo
          read(1,*)(ittspecs(i),i=1,nspecs)
          do i=1,nspecs
            if(ittspecs(i).ne.ispecs(i))stop'hnbxxxini: wrong id table'
          enddo
          read(1,*)ee
          do n=1,mxdrop
            read(1,*)((wwspecs(n,k,i),i=1,nspecs),k=1,mxe)
          enddo
          close(1)
        endif
      else
...
c----------------------------------------------------------------------
     ...
     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
      ...
      common/yrad/yrad(maxp),phifop(maxp),radfop(maxp),taufop(maxp)
      common/xxxspecsy/ndrop(-4:4,-4:4,-4:4)
  ...
  140 format(/' ----------------------------------'/
    ...
    !~~~~~~~~~read in freeze out surface properties from hydro~~~~~~~~~~~~       
      if(iorsdf.eq.3.and.ityptl(ip).eq.60)then 
       ...
       if(icnthnb.eq.1)then
        ...
        read(3,*)txt
        read(3,*)maprojx,matargx,engyx,epscrix
        if(maprojx.ne.maproj)stop'hnbaaa: maprojx.ne.maproj.       '
        if(matargx.ne.matarg)stop'hnbaaanew: matargx.ne.matarg.       '
        if(engyx.ne.engy)stop'hnbaaanew: engyx.ne.engy.       '
        ...
        if(ntauhyx.ne.ntauhy)stop'hnbaaanew: ntauhyx.ne.ntauhy.       '
        if(nphihyx.ne.nphihy)stop'hnbaaanew: nphihyx.ne.nphihy.       '
        if(nradhyx.ne.nradhy)stop'hnbaaanew: nradhyx.ne.nradhy.       '
        read(3,*)ntauhoc
        read(3,*)centhy,etahy,phihy,radhy
        read(3,*)tauhoc
	read(3,*)epsii
	read(3,*)rom,yom,wom
        close(3)
    ...
    !~~~~~~~~~define womi yomi romi~~~~~~~~~~~~     
      if(iorsdf.eq.3.and.icnthnb.eq.1)then 
       if(ioclude.eq.3)then
	    ...
	    do i=1,2
	       womi(ncent,neta,ntau,i)=wom(ncent,neta,ntau,i)
	       yomi(ncent,neta,ntau,i)=yom(ncent,neta,ntau,i)
	       romi(ncent,neta,ntau,i)=rom(ncent,neta,ntau,i)
	    enddo
	   enddo
	 enddo
	enddo
    ...
    !~~phi partition function pauf(ncent,neta,ntau,nphi)
        ...
        do neta=1,netahy
         ...
         do ntau=1,ntauhoc(ncent)  
          if(womi(ncent,neta,ntau,1).gt.womax )then     
           ...
           !print*,ncent,neta,ntau,ntauhac(ncent,neta)
          !. ,    womi(ncent,neta,ntau,1)
         enddo
         paut(ncent,neta,1)=0
         do ntau=2,ntauhac(ncent,neta)
           paut(ncent,neta,ntau)=womi(ncent,neta,ntau,1)/womax
           !print*,ncent,neta,ntau,paut(ncent,neta,ntau)
         enddo
         !~~phi~~~~
	 do ntau=2,ntauhac(ncent,neta)
           pauf(ncent,neta,ntau,1)=0
	   do nphi=2,nphihy
            dphi=phihy(nphi)-phihy(nphi-1)
     ...
     .	     =pauf(ncent,neta,ntau,nphi-1)+0.5*(w1+w2)*dphi
	   ...
	   do nphi=2,nphihy
            pauf(ncent,neta,ntau,nphi)=pauf(ncent,neta,ntau,nphi)/w
	   enddo
         enddo
        enddo
       enddo
    ...
    !~~~~~~~~~determine netar~~~~~~~~~~~~
      if(iorsdf.eq.3.and.ityptl(ip).eq.60)then 
       ...
       if(t+z.le.0..or.t-z.le.0.)then
         zetaor=1000.
       else
         zetaor=abs(0.5*log((t+z)/(t-z)))
       endif
       netar=0
       detamin=1000.
       do neta=1,netahy
        deta=abs(zetaor-etahy(neta))
    ...
    !~~~~~redefine energy in case of radial flow~~~~~~~~~~~~~~~~       
     ...
     ..and.ityptl(ip).eq.60)then
       am=0.
       do ntau=2,ntauhac(ncentr,netar)
        d=paut(ncentr,netar,ntau)-paut(ncentr,netar,ntau-1)
	am=am+d
       enddo
       en=0.
       do ntau=2,ntauhac(ncentr,netar)
          d=paut(ncentr,netar,ntau)-paut(ncentr,netar,ntau-1)
	  y=( yomi(ncentr,netar,ntau-1,1)
     .       +yomi(ncentr,netar,ntau,1)) / 2
	  en=en+d*cosh(y)
       enddo
       fradflo=1.
...
1     continue
 ...
 301    continue      
    ...
    !~~~~~~~~~~radial flow -> particles~~~~~~~~~~~~~~~~~~
	  ...
	  r=rangen()
	  ntau=1
	  do while(paut(ncentr,netar,ntau).lt.r)
	   ntau=ntau+1
	  enddo
          if(ntau.eq.1)ntau=2
	  ntau1=ntau-1
	  ntau2=ntau
	  tau1=tauhoc(ncentr,ntau1)
	  tau2=tauhoc(ncentr,ntau2)
	  f1=paut(ncentr,netar,ntau1)-r
	  f2=paut(ncentr,netar,ntau2)-r
	  f=f1/(f1-f2)
	  fx=f
	  tau= tau1*(1-f) + tau2*f
          taufop(n)=tau      
     ...
     .    .and.pauf(ncentr,netar,ntau,nphihy).gt.0.)then
	    nphi=1
	    do while((pauf(ncentr,netar,ntau1,nphi)*(1-fx)
     .               + pauf(ncentr,netar,ntau2,nphi)*fx)   .lt.r)
	     nphi=nphi+1
	    enddo
            if(nphi.eq.1)nphi=2
	    nphi1=nphi-1
	    ...
	    phi1=phihy(nphi1)
	    phi2=phihy(nphi2)
	    f1=pauf(ncentr,netar,ntau,nphi1)-r
	    f2=pauf(ncentr,netar,ntau,nphi2)-r
	    f=f1/(f1-f2)
	    phi=phi1+f*(phi2-phi1)
          else
            phi=rangen()*pi/2.
          endif
          if(px.ge.0..and.py.ge.0.)then
           continue
 	  elseif(px.lt.0..and.py.gt.0.)then
           phi=pi-phi
	  elseif(px.lt.0..and.py.lt.0.)then
           phi=pi+phi
	  elseif(px.gt.0..and.py.lt.0.)then
           phi=2*pi-phi
	  endif
          phifop(n)=phi
	  !~~determine yrad~~~~
     ...
     .        +romi(ncentr,netar,ntau1,2)*cos(2*phi)
          rad2=romi(ncentr,netar,ntau2,1)
     .        +romi(ncentr,netar,ntau2,2)*cos(2*phi)
          rad= rad1*(1-fx) + fx*rad2
          radfop(n)=rad
        enddo
        energ=0.
        do n=1,np 
          ...
          !px=pcm(1,n)
          !py=pcm(2,n)
          !vv2=vv2+(px**2-py**2)/(px**2+py**2)
          call utlob3(-1,uu(1),uu(2),uu(3),uu(4),1e0
     *         , pcm(1,n), pcm(2,n), pcm(3,n), pcm(4,n))
          energ=energ+pcm(4,n)
        !?????????????????????????????????????????????????
        !  px=pcm(1,n)
        !  py=pcm(2,n)
        !  vv3=vv3+(px**2-py**2)/(px**2+py**2)
        !  nvv2=nvv2+1
        !  if(mod(nvv2,100).eq.0)
        !.   print*,'++++',nvv2,vv2/nvv2,vv3/nvv2
        !?????????????????????????????????????????????????
        enddo
        esoll=tecm
        scal=1.
...
c------------------------------------------------------------------------------
      subroutine xFreezeOutTauX(neta)
c------------------------------------------------------------------------------
      include 'epos.inc'
      include 'epos.inchy'
      common/cxyzt/xptl(mxptl),yptl(mxptl),zptl(mxptl),tptl(mxptl)
     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
      common/cen/ncentr /ctauhac/ntauhac(ncenthy,netahy)
      character *8 cbim,cbimhy
      call centrality(bimevt        ,cbim)
      call centrality(centhy(ncentr),cbimhy)
    !..........................................................................
      nhis=1
      npl=0
      nplx=0
      deleta=etahy(2)-etahy(1)
      eta1=etahy(neta)-deleta/2
      eta2=etahy(neta)+deleta/2
      etaav=etahy(neta)
      taumax=tauhoc(ncentr,ntauhac(ncentr,neta))+4
      do n=1,nptl
        if(ityptl(n).eq.60
     ...
     *        write(ifhi,'(a)')       '  endarray closehisto plot 0-'
              ...
              write(ifhi,'(a,f4.1,a)')'text 0.55 0.90 "[c]=',etaav,'"'
              write(ifhi,'(a)')    'text 0.02 0.90 ""-2fm"L#y"L#2fm""'
...
c------------------------------------------------------------------------------
     ...
     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
      ...
      call centrality(bimevt        ,cbim)
      call centrality(centhy(ncentr),cbimhy)
      taumax=tauhoc(ncentr,ntauhac(ncentr,1))+4
    !..........................................................................
      nhis=1
      npl=0
      do n=1,nptl
...
c------------------------------------------------------------------------------
      ...
      character *8 cbim
      call centrality(bimevt        ,cbim)
      taumax=tauhoc(ncentr,ntauhac(ncentr,neta))+2
      netahyxx=netahy
      write(ifhi,'(a)')    '!----------------------------------------'
      write(ifhi,'(a,i3)') '!   hydro freeze out w_0 w_2     '
      write(ifhi,'(a)')    '!----------------------------------------'
      write(ifhi,'(a)')       '!newpage'
      do ii=1,2
        ...
        do ntau=2,ntauhac(ncentr,neta)
         dy=(wom(ncentr,neta,ntau,ii)-wom(ncentr,neta,ntau-1,ii))
...
c------------------------------------------------------------------------------
      ...
      netahyxx=netahy
      write(ifhi,'(a)')    '!----------------------------------------'
      write(ifhi,'(a,i3)') '!        hydro freeze out r_0 r_2     '
      write(ifhi,'(a)')    '!----------------------------------------'
      write(ifhi,'(a)')    '!newpage'
      do ii=1,2
...
c------------------------------------------------------------------------------
      ...
      netahyxx=netahy
      write(ifhi,'(a)')    '!----------------------------------------'
      write(ifhi,'(a,i3)') '!        hydro freeze out y_0 y_2      '
      write(ifhi,'(a)')    '!----------------------------------------'
      write(ifhi,'(a)')    '!newpage'
      do ii=1,2
        write(ifhi,'(a,3i1)')'openhisto htyp lin name y-',neta,nn,ii
	if(ii.eq.1)then !----------------------
         write(ifhi,'(a,f4.1)')'xmod lin xrange 0. ',taumax
         write(ifhi,'(a)')'txt "xaxis [t] (fm/c)"'
         write(ifhi,'(a)') 'ymod lin yrange auto auto '
...
c------------------------------------------------------------------------------
      subroutine centrality(b,cbim)
c------------------------------------------------------------------------------
      include 'epos.inc'
      character *8 cbim
      parameter (maxb=16)
      dimension iperc(maxb),bim(maxb)
     ...
     &               ,w1define(mxdefine),w2define(mxdefine)
      ...
      enddo
      do m=1,ndefine
        if(w1define(m)(1:l1define(m)).eq.'bim05')then
          read(w2define(m)(1:l2define(m)),*)bim(2)
        elseif(w1define(m)(1:l1define(m)).eq.'bim10')then
          read(w2define(m)(1:l2define(m)),*)bim(3)
        elseif(w1define(m)(1:l1define(m)).eq.'bim15')then
          read(w2define(m)(1:l2define(m)),*)bim(4)
        elseif(w1define(m)(1:l1define(m)).eq.'bim20')then
          read(w2define(m)(1:l2define(m)),*)bim(5)
        elseif(w1define(m)(1:l1define(m)).eq.'bim25')then
          read(w2define(m)(1:l2define(m)),*)bim(6)
        elseif(w1define(m)(1:l1define(m)).eq.'bim30')then
          read(w2define(m)(1:l2define(m)),*)bim(7)
        elseif(w1define(m)(1:l1define(m)).eq.'bim35')then
          read(w2define(m)(1:l2define(m)),*)bim(8)
        elseif(w1define(m)(1:l1define(m)).eq.'bim40')then
          read(w2define(m)(1:l2define(m)),*)bim(9)
        elseif(w1define(m)(1:l1define(m)).eq.'bim45')then
          read(w2define(m)(1:l2define(m)),*)bim(10)
        elseif(w1define(m)(1:l1define(m)).eq.'bim50')then
          read(w2define(m)(1:l2define(m)),*)bim(11)
        elseif(w1define(m)(1:l1define(m)).eq.'bim60')then
          read(w2define(m)(1:l2define(m)),*)bim(12)
        elseif(w1define(m)(1:l1define(m)).eq.'bim70')then
          read(w2define(m)(1:l2define(m)),*)bim(13)
        elseif(w1define(m)(1:l1define(m)).eq.'bim80')then
     ...
     .       ,' #define bim?? ??? missing. *******  '
      ...
      enddo
      n=max(2,n)
      write(cbim,'(a,i2,a,i2,a)')' ',iperc(n-1),'-',iperc(n),'% '
      !print*,cbim
...
c-----------------------------------------------------------------------
     ...
     *,optl(mxptl),uptl(mxptl),sptl(mxptl),rptl(mxptl,3)
      ...
      cbim='        '
      cbimhy='        '
      if(ioclude.gt.1)call centrality(bimevt        ,cbim)
      if(ioclude.gt.1)call centrality(centhy(ncentr),cbimhy)

      phi=ranphi+phievt
      !print*,'RandomPhi=',ranphi,'   EventPhi=',phievt
      rapmax=6
      radmax=10
      r1=0.0
      ...
      write(ifhi,'(a,2e11.3)')'xrange',-a*1.5,a*1.5
      write(ifhi,'(a,2e11.3)')'yrange',-a,a
      write(ifhi,'(a)')    'text 1.02 -0.15 " x"'
      write(ifhi,'(a)')    'txt "yaxis y"'
   ...
   !........................................................................................
      ...
      do n=1,nptl
        ...
        routp=-sin(phievt)*xptl(n)+cos(phievt)*yptl(n)
        rinp = cos(phievt)*xptl(n)+sin(phievt)*yptl(n)
