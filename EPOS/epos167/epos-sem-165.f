...
c     *  *gamhad(iclt)*rr**2*(xpp*xpm)**(1.-alppar)
c     *  /(1.+alplea(iclp))/(1.+alplea(iclt))*pi/8.*factk
c      else
c        omi5pp=omi5pp*chad(iclp)*chad(iclt)*rr*gamhad(iclt)
...
c-----------------------------------------------------------------------
     ...
     &     +bmxdif(iclpro,icltar)/4./0.0389
      ...
      do m=1,2
        ...
        vv21=sngl(PhiExpo(1.,1.d0,1.d0,engy**2,b1)) 
        vv22=sngl(PhiExpo(1.,1.d0,1.d0,engy**2,b2))
...
c iqq=1 : projectile, iqq=2 : target
c-----------------------------------------------------------------------
      include 'epos.inc'
      common /ar3/ x1(7),a1(7)
      external func
...
c-----------------------------------------------------------------------
      ...
      do i=1,2
        b=bm+x5(i)*difn
        ptgau1=ptgau1+ptfau(b,iqq)*a5(i)*exp(x5(i))*b*2.*pi*difn
      enddo
      return
      end
c------------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      do i=1,2
        b=bm+x5(i)*difn
        ptgau2=ptgau2+ptfauAA(b)*a5(i)*exp(x5(i))*b*2.*pi*difn
      enddo
      return
      end
...
c      p=sqrt(max(0.,egy**2-ampro**2))   
      ...
      if(isetcs.le.1)then
        ...
        elseif(mapr.eq.1)then
          ...
          gin=(ptgau(ptfau,bm,2)+ptgau1(bm,2))*10. !sig_in
        elseif(matg.eq.1)then
          ...
          gin=(ptgau(ptfau,bm,1)+ptgau1(bm,1))*10. !sig_in
        else
          rad=radnuc(matg)+1.    
          radp=radnuc(mapr)+1.  
          bm=rad+radp+2.
          ...
          anorm=1.5/pi/rrr**3/(1.+(pi/rrr)**2)/difnuc(matg)**2
          anormp=1.5/pi/rrrp**3/(1.+(pi/rrrp)**2)/difnuc(mapr)**2
          gin=(ptgau(ptfauAA,bm,2)+ptgau2(bm))*10.
        endif
        pscrse=gin
        maproj=maprojsave
        matarg=matargsave
...
c------------------------------------------------------------------------------
      ...
      if(id.eq.0)then
        do k=1,3
          mt=int(airanxs(k))
          eposcrse=eposcrse+airwnxs(k)*pscrse(ek,mapro,mt)
        enddo
      else
        eposcrse=pscrse(ek,mapro,matar)
      endif
...
c numerical integration over z1
     ...
     *    +(sdtq+sdnq)*fzeroSeaZZ(z1,iclpro0))
      ...
      psdsh=dqsh+dsh*rr*4.*pi*gamhad(iclpro0)/4.5  !*ccorr(1,1,iclpro0)
      return
      end

c------------------------------------------------------------------------
      function psdsh1(s,qq,iclpro0,dqsh,long)
c-----------------------------------------------------------------------
c psdsh - semihard interaction eikonal
c s - energy squared for the interaction (hadron-hadron),
c iclpro0 - hadron class,
...
c-----------------------------------------------------------------------
      ...
      include 'epos.inc'
      include 'epos.incsem'
c      double precision psuds

      psdsh1=0.       !only for plotting in psaevp : not use any more

c$$$      xd=qq/s
c$$$      write(ifch,*)'Psdsh1 for xd,qq',xd,qq
c$$$      if(long.eq.0.and.(idisco.eq.0.or.idisco.eq.1))then 
c$$$        dqsh=psftist(xd)/4.5*sngl(psuds(qq,1)/psuds(q2min,1))
c$$$     *  *4.*pi**2*alfe/qq
c$$$      else
c$$$        dqsh=0.
c$$$      endif
c$$$
c$$$      if(long.eq.0)then
c$$$        s2min=qq/(1.-q2ini/qq)
c$$$      else
c$$$        s2min=qq+4.*max(q2min,qcmass**2)        
c$$$      endif
c$$$      xmin=s2min/s
c$$$      xmin=xmin**(delh-dels)
c$$$      dsh=0.
c$$$      if(xmin.lt.1.)then
c$$$c numerical integration over z1
c$$$        do i=1,7
c$$$        do m=1,2
c$$$          z1=(.5*(1.+xmin-(2*m-3)*x1(i)*(1.-xmin)))**(1./
c$$$     *    (delh-dels))
c$$$          call psdint(z1*s,qq,sdsg,sdng,sdbg,sdtg,sdrg,0,long)
c$$$          call psdint(z1*s,qq,sdsq,sdnq,sdbq,sdtq,sdrq,1,long)
c$$$          dsh=dsh+a1(i)/z1**delh*(sdtg*psftigt(z1)
c$$$     *    +(sdtq+sdnq)*psftist(z1))*z1**dels
c$$$        enddo
c$$$        enddo
c$$$        dsh=dsh*(1.-xmin)/(delh-dels)/2.
c$$$      endif
c$$$      psdsh1=dqsh+dsh/4.5
      return
      end
...
c-----------------------------------------------------------------------
      ...
      do kk=1,2
       ...
       if(kk.eq.2)spmax=spmin*(epmax/2./spmin)**(1./(ksmax-1.)) 
...
c-----------------------------------------------------------------------
     ...
     *    psdsh1(ww,qq,2,dqsh,0)+psdsh1(ww,qq,2,dqsh,1)
     *    )/(4.*pi**2*alfe)*qq
...
c      double precision om5p,xh,yh,v3pom(4),om2p
     ...
     *,alplea0(nclha),asect1(7,4,7),asect2(7,4,7),asect3(7,7,7)
     *,asect4(7,7,7)!,cgam(idxD)
      common /psar2/  edmax,epmax
      common /psar4/  fhgg(11,10,8),fhqg(11,10,80)
     *,fhgq(11,10,80),fhqq(11,10,80),fhgg0(11,10),fhgg1(11,10,4)
     *,fhqg1(11,10,40),fhgg01(11),fhgg02(11),fhgg11(11,4)
...
c      write(6,*)'===========',ffrr
     ...
     *  utgam1(2.-alppar)*utgam1(3.+alplea(2)-alppar)
      gamsoft=rr*4.*pi
ckkkkkkk-------------------------------
        if(ish.ge.4)write (ifch,*)'rr,qnorm',rr,qnorm
     ...
     *  write (ifch,*)'gamhad(4),gamhads(4)',gamhad(4),gamhads(4)
        ...
        enddo
        enddo
        gnorm=gnorm/(1.-dels)*2.*pi*gamhad(4)*rr
        alvc=6./(1.-gnorm)-4.
        if(ish.ge.4) write (ifch,*)'rr,qnorm,gnorm,alvc',
     *  rr,qnorm,gnorm,alvc

c        write (*,*)'rr-c,qnorm,gnorm,alvc',rr,qnorm,gnorm,alvc
      endif
...
c---------------------------------------
 ...
 6    continue
     ...
     *   .or.epscrp0.ne.epscrp)then
        ...
        if(isetcs.eq.2)then
          read (1,*)asect,asect2,asectn,asect4
        elseif(isetcs.eq.3)then
          read (1,*)asect1,asect,asect3,asectn
        else
      ...
      bminim=0.
      bmaxim=10000.
      fctrmx=100.         !to get stable pA and AA cross section, this number has to be large
      ...
      do iclpro=1,4
       if(iclpro.lt.iclpro1.or.iclpro.gt.iclpro2)then
         do ie=1,7    
           do iia=1,7    
             asect1(ie,iclpro,iia)=0.
             asect2(ie,iclpro,iia)=0.
           enddo
         enddo
       else
         do ie=1,7    
           ...
           do iia=1,7
            ...
            if(ish.ge.3)write (ifch,226)gin
226         format(2x,'psaini: hadron-nucleus cross sections:'/
     *       4x,'gin=',e10.3)
            asect1(ie,iclpro,iia)=log(gin)
            asect(ie,iclpro,iia)=asect1(ie,iclpro,iia)
            write(ifmt,*)'  matarg,gin :'
     *       ,matarg,gin
           enddo
         enddo
...
c if random sign for projectile, set it here
                ...
                call emsaaa(iret)          
                if(iret.gt.0)goto 222
              else
                ntevt=ntry
              endif
            enddo
            a=pi*bmax**2
            if(a.gt.0..and.ntevt.gt.0.)then
             xs=float(nevent)/float(ntevt)*a*10.
             write(ifmt,*)'  matarg,nevent,ntevt,bmax,xs :'
     .       ,matarg,nevent,ntevt,bmax,xs
             write(ifch,*)'  matarg,nevent,ntevt,bmax,xs :'
     .       ,matarg,nevent,ntevt,bmax,xs
             asect2(ie,iclpro,iia)=log(xs)
            else
             write(ifmt,*)' Problem ? ',iclpro,matarg,bmax,ntevt
             asect2(ie,iclpro,iia)=0.
            endif
           enddo
         enddo
      ...
      do ie=1,7
        ...
        do iib=1,7
          ...
          if(ish.ge.3)write (ifch,227)gin
 227      format(2x,'psaini: nucleus-nucleus cross sections:'/
     *       4x,'gin=',e10.3)
            asect3(ie,iia,iib)=log(gin)
            asectn(ie,iia,iib)=asect3(ie,iia,iib)
            write(ifmt,*)'  maproj,matarg,gin :'
     *       ,maproj,matarg,gin
 ...
 223        ntevt=ntevt+1
          ...
          if(a.gt.0..and.ntevt.gt.0.)then
            xs=float(nevent)/float(ntevt)*a*10.
          write(ifmt,*)'  maproj,matarg,nevent,ntevt,bmax,xs :'
     &                         ,maproj,matarg,nevent,ntevt,bmax,xs
          write(ifch,*)'  maproj,matarg,nevent,ntevt,bmax,xs :'
     &                         ,maproj,matarg,nevent,ntevt,bmax,xs
            asect4(ie,iia,iib)=log(xs)
          else
            write(ifmt,*)' Problem ? ',maproj,matarg,bmax,ntevt
            asect4(ie,iia,iib)=0.
          endif
        enddo
      enddo
     ...
     *,egylow,egymax,iomega,egyscr,epscrw,epscrp
      write (1,*)asect1,asect2,asect3,asect4
