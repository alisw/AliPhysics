...
processFilterLog(){
    ...
    cat $logFile | grep "TestStatusAll_0" |gawk '{print $2}' >$inputList.Good
    cat $logFile | grep "TestStatusAll_" | grep -v "TestStatusAll_0" |gawk '{print $2}' >$inputList.Bad
}
...
makeFilteredList(){
    ...
    logFile=$3
    timeOut=$4
    testFileList $1 $2 $timeOut|tee $logFile
    processFilterLog $1 $logFile
}
