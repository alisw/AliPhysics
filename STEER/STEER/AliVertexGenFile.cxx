...
//_____________________________________________________________________________
TVector3 AliVertexGenFile::GetVertex(Bool_t &isGood)
{
// get the vertex from the event header tree
  ...
  TString genHeadName=fHeader->GenEventHeader()->GetName();
  if(genHeadName.Contains("Pileup") && genHeadName.Contains("NoGenTrig")){
    AliInfo("Trigger event not generated by AliGenPileup");
    isGood=kFALSE;
    return TVector3(0,0,0);
  }
  TArrayF vertex(3);
  ...
  isGood=kTRUE;
  AliInfo(Form("Primary vertex position from external header: (%f %f %f) cm  time = %f ns",vertex[0],vertex[1],vertex[2],fLastTime*1e9));
