...
{
	HLTImportant("Initializing ASYNC Processor");
...
{
	HLTImportant("Deinitializing ASYNC Processor");
	...
	fBackend = NULL;
	fQueueDepth = 0;
	HLTImportant("Deinitialization of ASYNC Processor done");
}
...
{
	HLTImportant("Async Thread Running");
	while (true)
	{
	...
	HLTImportant("Async Thread Terminating");
}
...
int AliHLTAsyncProcessor::QueueAsyncTask(void* (*processFunction)(void*), void* data)
{
	...
	HLTImportant("Queuing task (Queue Fill Status %d %d %d)", fInputQueueUsed, (int) fAsyncThreadProcessing, fOutputQueueUsed);
	fBackend->LockMutex(1);
...
{
	HLTImportant("%d results ready for retrieval", fOutputQueueUsed);
...
void* AliHLTAsyncProcessor::RetrieveQueuedTaskResult()
{
	...
	HLTImportant("Retrieving Queued Result");
...
void AliHLTAsyncProcessor::WaitForTasks(int n)
{
	...
	HLTImportant("Waiting for %d tasks", n);
	if (n <= fOutputQueueUsed)
	{
		fBackend->UnlockMutex(1);
		HLTImportant("%d Tasks already ready, no need to wait", n);
		return;
	}
	fWaitingForTasks = n;
	fBackend->UnlockMutex(1);
	fBackend->LockMutex(3);
	HLTImportant("Waiting for %d tasks finished", n);
}
