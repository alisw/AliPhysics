...
{
	return(a.pad < b.pad || (a.pad == b.pad && a.time < b.time));
}
...
unsigned int truncate(int j, unsigned int val)
{
	if (truncate_bits && field_bits[j] != significant_bits[j] && val)
	{
		...
		if (ldz > significant_bits[j]) val &= ((1 << ldz) - 1) ^ ((1 << (ldz - significant_bits[j])) - 1);
	}
	return(val);
}
...
int main(int argc, char** argv)
{
	...
	for (int islice = 0;islice < 36;islice++) for (int ipatch = 0;ipatch < 6;ipatch++)
	{
		if (separate_slices) printf("SLICE %d ", islice);
		if (separate_patches) printf("PATCH %d", ipatch);
		if (separate_slices || separate_patches) printf("\n");
		for (int i = SLICE;i < nFields;i++)
		{
			if (i == CLUSTER_ID || i == PATCH) continue;
		...
		for (size_t i = 0;i < nClusters;i++)
		{
			...
			if (full_row_numbers) cluster.row += fgRows[cluster.patch][0];

			if ((slice_diff || res_diff || track_diffqtot || track_diffqmax) && cluster.trackID != -1 && track_based)
			{
				if (lastTrack != cluster.trackID) lastSlice = lastResPad = lastResTime = lastQTot = lastQMax = lastSigmaPad = lastSigmaTime = 0;
				
				if (slice_diff) do_diff(cluster.slice, lastSlice, field_bits[SLICE]);
			...
			if (cluster.row != lastRow) lastPad = lastTime = 0;
			...
			if (approximate_qtot && (!track_based || cluster.trackID == -1 || (track_avgtot == 0 && track_diffqtot == 0)))
			{
				...
				if (cluster.qtot < 0) cluster.qtot = -truncate(QTOT, -cluster.qtot);
				else cluster.qtot = truncate(QTOT, cluster.qtot);
			...
			if (track_avgtot && cluster.trackID != -1)
			{
				...
				if (newTrack) cluster.qtot = truncate(QTOT, cluster.avgtot);
				cluster.avgtot = tmp & ((1 << field_bits[QTOT]) - 1);
			}
			if (track_avgmax && cluster.trackID != -1)
			{
				int tmp = cluster.qmax - cluster.avgmax;
				if (newTrack) cluster.qmax = cluster.avgmax;
				cluster.avgmax = tmp & ((1 << field_bits[QMAX]) - 1);
			}

			//Copy qmax / qtot to combined track avg... slot, to use for combine_maxtot
			if ((((combine_maxtot && (track_avgtot || track_diffqtot)) || track_separate_q) && track_avgmax == 0 && track_diffqmax == 0) && cluster.trackID != -1) cluster.avgmax = cluster.qmax;
			if ((((combine_maxtot && (track_avgmax || track_diffqmax)) || track_separate_q) && track_avgtot == 0 && track_diffqtot == 0) && cluster.trackID != -1) cluster.avgtot = cluster.qtot;

			for (int j = 0;j < sizeof(cluster_struct) / sizeof(unsigned int);j++)
			{
				if (approximate_qtot && (j == QTOT || j == AVG_TOT)) continue;
				if (track_avgtot &&  (j == QTOT || j == AVG_TOT)) continue;
			...
			for (int j = SLICE;j < nFields;j++)
			{
				...
				if (j == CLUSTER_ID || j == PATCH) continue;

				if (j == SLICE && (track_based == 0 || cluster.trackID == -1)) continue;
				...
				{
					if (j == SIGMA_PAD || j == SIGMA_TIME) continue;
					if (j == DIFF_SIGMA_PAD)
					{
		...
		for (int i = SLICE;i < nFields;i++)
		{
			...
			if (i <= FLAG_PADTIME) rawBits += field_bits[i]; 

			if (combine_maxtot && (i == QMAX || i == QTOT)) continue;
		...
		if (separate_sides && !separate_slices && islice == 0) islice = 17;
		else if (!separate_slices) islice = 9999999;

		if (!separate_patches) ipatch = 9999999;
	}

	if (separate_slices || separate_patches || separate_sides) printf("Total Compression: %f\n", rawtotalbytes / entrototalbytes );
