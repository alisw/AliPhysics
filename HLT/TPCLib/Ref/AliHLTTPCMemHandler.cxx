...
void AliHLTTPCMemHandler::Free()
{
  ...
  if(!fPt){
    LOG(AliHLTTPCLog::kWarning,"AliHLTTPCMemHandler::Free","Memory")
...
void AliHLTTPCMemHandler::SetRandomCluster(Int_t maxnumber)
{
  ...
  fNDigits = 0;
  if(fDigits) delete [] fDigits;
  fDigits = new AliHLTTPCRandomDigitData[fNRandom*9];
  if(fDPt) delete [] fDPt;
...
UInt_t AliHLTTPCMemHandler::GetRandomSize()
{
  Int_t nrandom = 0;
...
Bool_t AliHLTTPCMemHandler::Memory2Binary(UInt_t nrow,AliHLTTPCDigitRowData *data)
{
  ...
  AliHLTTPCDigitRowData *row_pt = data; 
  Int_t outsize = 0;
  for(UInt_t i=0;i<nrow;i++){
    Int_t size = sizeof(AliHLTTPCDigitData) * row_pt->fNDigit 
      + sizeof(AliHLTTPCDigitRowData);
    outsize += size;
    fwrite(row_pt,size,1,fOutBinary);
    Byte_t  *byte_pt =(Byte_t *) row_pt;
    byte_pt += size;
    row_pt = (AliHLTTPCDigitRowData *) byte_pt;
  }
...
Bool_t AliHLTTPCMemHandler::Binary2Memory(UInt_t & nrow,AliHLTTPCDigitRowData *data)
{
  ...
  while(!feof(fInBinary)){
    ...
    if(fread(row_pt,sizeof(AliHLTTPCDigitRowData),1,fInBinary)!=1) break;

    byte_pt += sizeof(AliHLTTPCDigitRowData);
...
UShort_t AliHLTTPCMemHandler::Read(UInt_t *comp, UInt_t & index, UInt_t & subindex)
{
  UInt_t shift[3] = {0,10,20};
...
UInt_t AliHLTTPCMemHandler::GetCompMemorySize(UInt_t nrow,AliHLTTPCDigitRowData *data)
{
...
UInt_t AliHLTTPCMemHandler::GetMemorySize(UInt_t nrow,UInt_t *comp){
  if(!comp){
...
Bool_t AliHLTTPCMemHandler::Memory2Binary(UInt_t ntrack,AliHLTTPCTrackSegmentData *data)
{
  ...
  AliHLTTPCTrackSegmentData *track_pt = data;
  for(UInt_t i=0;i<ntrack;i++){
    Int_t size=sizeof(AliHLTTPCTrackSegmentData)+track_pt->fNPoints*sizeof(UInt_t); 
    fwrite(track_pt,size,1,fOutBinary);
    Byte_t *byte_pt = (Byte_t*) track_pt;
    byte_pt += size; 
    track_pt = (AliHLTTPCTrackSegmentData*) byte_pt;
  }
...
Bool_t AliHLTTPCMemHandler::Binary2Memory(UInt_t & ntrack,AliHLTTPCTrackSegmentData *data)
{
  ...
  while(!feof(fInBinary)){
    if(fread(track_pt,sizeof(AliHLTTPCTrackSegmentData),1,fInBinary)!=1) break;
    Int_t size=track_pt->fNPoints*sizeof(UInt_t);
    if(fread(track_pt->fPointIDs,size,1,fInBinary)!=1) break;
    Byte_t *byte_pt = (Byte_t*) track_pt;
    byte_pt += sizeof(AliHLTTPCTrackSegmentData)+size;
    track_pt = (AliHLTTPCTrackSegmentData*) byte_pt;
    ntrack++; 
  }
  LOG(AliHLTTPCLog::kDebug,"AliHLTTPCMemHandler::Binary2Memory","File")
...
Bool_t AliHLTTPCMemHandler::TrackArray2Memory(UInt_t & ntrack,AliHLTTPCTrackSegmentData *data,AliHLTTPCTrackArray *array)
{
...
Bool_t AliHLTTPCMemHandler::Memory2TrackArray(UInt_t ntrack,AliHLTTPCTrackSegmentData *data,AliHLTTPCTrackArray *array)
{
...
Bool_t AliHLTTPCMemHandler::Memory2TrackArray(UInt_t ntrack,AliHLTTPCTrackSegmentData *data,AliHLTTPCTrackArray *array,Int_t slice)
{
