...
#ifndef AliHLTTPC_Modeller
#define AliHLTTPC_Modeller
...
class AliHLTTPCModeller {
 ...
 protected:
  ...
  AliHLTTPCDigitRowData *fRowData;//!
  Digit *fRow; //!
  Char_t fPath[1024];
  ...
  void FillCluster(AliHLTTPCModelTrack *track,Cluster *cluster,Int_t row,Int_t npads);
  void FillZeros(AliHLTTPCDigitRowData *digPt,Bool_t reversesign=kFALSE);
  void LocateCluster(AliHLTTPCModelTrack *track,ClusterRegion *region,Int_t &padmin,Int_t &padmax);
  void GetTrackID(Int_t pad,Int_t time,Int_t *trackID);
  
 public:
  
  AliHLTTPCModeller();
  virtual ~AliHLTTPCModeller();
  ...
  void SetInputData(AliHLTTPCDigitRowData *digits) {fRowData = digits;}
  ...
  ClassDef(AliHLTTPCModeller,1) //Modeller class
