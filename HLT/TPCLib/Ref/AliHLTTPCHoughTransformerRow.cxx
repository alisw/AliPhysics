...
AliHLTTPCHoughTransformerRow::AliHLTTPCHoughTransformerRow(Int_t slice,Int_t patch,Int_t netasegments,Bool_t /*DoMC*/,Float_t zvertex) : AliHLTTPCHoughBaseTransformer(slice,patch,netasegments,zvertex)
{
  //Normal constructor
  fParamSpace = 0;
						...
						Int_t nybin,Float_t ymin,Float_t ymax)
{
...
#ifdef do_mc
  {
	...
	LOG(AliHLTTPCLog::kInformational,"AliHLTTPCHoughTransformerRow::CreateHistograms()","")
	  <<"Transformer: Allocating "<<GetNEtaSegments()*ncells*sizeof(AliHLTTPCTrackIndex)<<" bytes to fTrackID"<<ENDLOG;
	fTrackID = new AliHLTTPCTrackIndex*[GetNEtaSegments()];
	for(Int_t i=0; i<GetNEtaSegments(); i++)
	  fTrackID[i] = new AliHLTTPCTrackIndex[ncells];
      }
  }
#endif
  AliHLTTPCHistogram *hist = fParamSpace[0];
  ...
  if(!fTrackNRows)
	...
	<<"Transformer: Allocating "<<ncells*sizeof(UChar_t)<<" bytes to fInitialGapCount"<<ENDLOG;
      ...
      AliHLTTPCTrackLength *tracklength = new AliHLTTPCTrackLength[nxgrid*nygrid];
      memset(tracklength,0,nxgrid*nygrid*sizeof(AliHLTTPCTrackLength));
      ...
      for(Int_t ybin=ymin-1; ybin<=(ymax+1); ybin++)
	{
	  for(Int_t xbin=xmin-1; xbin<=xmiddle; xbin++)
		...
		for(Int_t deltax = 0; deltax <= 1; deltax++) {
		  ...
		  AliHLTTPCTrackLength *curtracklength = &tracklength[(xbin + deltax) + (ybin + deltay)*nxgrid];
  ...
  if(!fStartPadParams)
    {
      ...
      LOG(AliHLTTPCLog::kInformational,"AliHLTTPCHoughTransformerRow::CreateHistograms()","")
	<<"Transformer: Allocating about "<<nrows*100*sizeof(AliHLTTPCPadHoughParams)<<" bytes to fStartPadParams"<<ENDLOG;
      fStartPadParams = new AliHLTTPCPadHoughParams*[nrows];
      LOG(AliHLTTPCLog::kInformational,"AliHLTTPCHoughTransformerRow::CreateHistograms()","")
	<<"Transformer: Allocating about "<<nrows*100*sizeof(AliHLTTPCPadHoughParams)<<" bytes to fEndPadParams"<<ENDLOG;
      fEndPadParams = new AliHLTTPCPadHoughParams*[nrows];
      LOG(AliHLTTPCLog::kInformational,"AliHLTTPCHoughTransformerRow::CreateHistograms()","")
      ...
      for(Int_t i=AliHLTTPCTransform::GetFirstRow(0); i<=AliHLTTPCTransform::GetLastRow(5); i++)
	{
	  ...
	  fStartPadParams[i] = new AliHLTTPCPadHoughParams[npads];
	  fEndPadParams[i] = new AliHLTTPCPadHoughParams[npads];
...
void AliHLTTPCHoughTransformerRow::Reset()
{
...
#ifdef do_mc
  {
    ...
    Int_t ncells = ncellsx*ncellsy;
    for(Int_t i=0; i<GetNEtaSegments(); i++)
      memset(fTrackID[i],0,ncells*sizeof(AliHLTTPCTrackIndex));
  }
#endif
  AliHLTTPCHistogram *hist = fParamSpace[0];
  Int_t ncells = (hist->GetNbinsX()+2)*(hist->GetNbinsY()+2);
...
void AliHLTTPCHoughTransformerRow::TransformCircleFromDigitArray()
{
  ...
  AliHLTTPCEtaRow *etaclust = new AliHLTTPCEtaRow[netasegments];
  ...
  for(UChar_t i=AliHLTTPCTransform::GetFirstRow(ipatch); i<=AliHLTTPCTransform::GetLastRow(ipatch); i++)
    {
      ...
      memset(etaclust,0,netasegments*sizeof(AliHLTTPCEtaRow));  
...
#ifdef do_mc
			   ,AliHLTTPCEtaRow etaclust,AliHLTTPCTrackIndex *trackid
#endif
			   )
{
...
inline void AliHLTTPCHoughTransformerRow::FillCluster(UChar_t i,Int_t etaindex,AliHLTTPCEtaRow *etaclust,Int_t ilastpatch,Int_t firstbinx,Int_t lastbinx,Int_t nbinx,Int_t firstbiny)
{
  // The method is a part of the fast hough transform.
...
#ifdef do_mc
  AliHLTTPCTrackIndex *trackid = fTrackID[etaindex];
#endif

  //Do the transformation:
  AliHLTTPCPadHoughParams *startparams = &fStartPadParams[(Int_t)i][etaclust[etaindex].fStartPad]; 
  AliHLTTPCPadHoughParams *endparams = &fEndPadParams[(Int_t)i][etaclust[etaindex].fEndPad];
  ...
  if(ilastpatch == -1) {
	...
	AliHLTTPCTrackIndex *trackid2 = trackid + tempbin2;
#endif
...
#ifdef do_mc
inline void AliHLTTPCHoughTransformerRow::FillClusterMCLabels(AliHLTTPCDigitData digpt,AliHLTTPCEtaRow *etaclust)
{
  // The method is a part of the fast hough transform.
