...
ClassImp(AliHLTTPCModeller)

AliHLTTPCModeller::AliHLTTPCModeller()
{
  fMemHandler=0;
  fTracks=0;
  fRow=0;
...
void AliHLTTPCModeller::Init(Int_t slice,Int_t patch,Char_t *trackdata,Char_t *path,Bool_t houghtracks,Bool_t binary)
{
  fSlice = slice;
  fPatch = patch;
  ...
  Char_t fname[100];
  AliHLTTPCMemHandler *file = new AliHLTTPCMemHandler();
  if(!houghtracks)
  ...
  if(!file->SetBinaryInput(fname))
    {
      cerr<<"AliHLTTPCModeller::Init : Error opening trackfile: "<<fname<<endl;
      return;
    }
...
#ifdef use_aliroot
  fMemHandler = new AliHLTTPCFileHandler();
  ...
  if(binary == kFALSE)
	...
	{
	  cerr<<"AliHLTTPCModeller::Init : Error opening file "<<fname<<endl;
	  return;
	}
      digits=(AliHLTTPCDigitRowData*)fMemHandler->CompBinary2Memory(ndigits);
    }
#else
  fMemHandler = new AliHLTTPCMemHandler();
  fMemHandler->Init(slice,patch);
  if(binary == kFALSE)
    {
      cerr<<"AliHLTTPCModeller::Init : Compile with AliROOT if you want rootfile as input"<<endl;
      return;
    }
  else
    {
...
void AliHLTTPCModeller::FindClusters()
{
  if(fDebug)
      {
#if 0
    cout<<"AliHLTTPCModeller::FindClusters : Processing slice "<<fSlice<<" patch "<<fPatch<<endl;
#endif
      }
  if(!fTracks)
    {
      cerr<<"AliHLTTPCModeller::Process : No tracks"<<endl;
      return;
    }
  if(!fRowData)
    {
      cerr<<"AliHLTTPCModeller::Process : No data "<<endl;
      return;
    }
  ...
  for(Int_t i=AliHLTTPCTransform::GetFirstRow(fPatch); i<=AliHLTTPCTransform::GetLastRow(fPatch); i++)
	...
	{
	  cerr<<"AliHLTTPCModeller::FindClusters : Mismatching rownumbering "<<i<<" "<<rowPt->fRow<<endl;
	  return;
	}
      fCurrentPadRow = i;
	...
	{
	  AliHLTTPCModelTrack *track = (AliHLTTPCModelTrack*)fTracks->GetCheckedTrack(k);
	  if(!track) continue;
	  ...
	  for(pad=minpad; pad<=maxpad; pad++)
		...
		{
		  charge = fRow[(AliHLTTPCTransform::GetNTimeBins()+1)*pad+time].fCharge;
		  if(!charge) continue;
		  if(fRow[(AliHLTTPCTransform::GetNTimeBins()+1)*pad+time].fUsed == kTRUE)
		    continue;
		  ntimes++;
		  ...
		  fRow[(AliHLTTPCTransform::GetNTimeBins()+1)*pad+time].fUsed = kTRUE;
		}
	      if(ntimes)
		npads++;
	    }
  ...
  for(Int_t i=0; i<fTracks->GetNTracks(); i++)
    {
      AliHLTTPCModelTrack *track = (AliHLTTPCModelTrack*)fTracks->GetCheckedTrack(i);
      if(!track) continue;
      if(track->GetNClusters() != AliHLTTPCTransform::GetNRows(fPatch))
	cerr<<endl<<"Mismatching hitcounts; nclusters: "<<track->GetNClusters()<<" nrows "<<AliHLTTPCTransform::GetNRows(fPatch)<<endl<<endl;
    }
  
}


void AliHLTTPCModeller::LocateCluster(AliHLTTPCModelTrack *track,ClusterRegion *region,Int_t &padmin,Int_t &padmax)
{
  ...
  while(1)
      ...
      if(pad < 0)
	{
      ...
      else if(pad >= AliHLTTPCTransform::GetNPads(row))
	{
	  padmax = AliHLTTPCTransform::GetNPads(row)-1;
	  break;
	}
      ...
      while(1)
	{
	  ...
	  else if(time >= AliHLTTPCTransform::GetNTimeBins())
	    {
	      //timemax = AliHLTTPCTransform::GetNTimeBins()-1;
	      break;
	    }
	  charge = fRow[(AliHLTTPCTransform::GetNTimeBins()+1)*pad+time].fCharge;
	  //if(row==0)
...
void AliHLTTPCModeller::FillCluster(AliHLTTPCModelTrack *track,Cluster *cluster,Int_t row,Int_t npads)
{
...
void AliHLTTPCModeller::FillZeros(AliHLTTPCDigitRowData *rowPt,Bool_t reversesign)
{
...
void AliHLTTPCModeller::WriteRemaining()
{
  //Write remaining (nonzero) digits to file.
  
  AliHLTTPCDigitRowData *rowPt;
  rowPt = (AliHLTTPCDigitRowData*)fRowData;
  Int_t digitcount=0;
  Int_t ndigits[(AliHLTTPCTransform::GetNRows(fPatch))];
  for(Int_t i=AliHLTTPCTransform::GetFirstRow(fPatch); i<=AliHLTTPCTransform::GetLastRow(fPatch); i++)
    {
      AliHLTTPCDigitData *digPt = (AliHLTTPCDigitData*)rowPt->fDigitData;
      ndigits[(i-AliHLTTPCTransform::GetFirstRow(fPatch))]=0;
      for(UInt_t j=0; j<rowPt->fNDigit; j++)
	{
	  if(digPt[j].fCharge==0) continue;
	  digitcount++;
	  ndigits[(i-AliHLTTPCTransform::GetFirstRow(fPatch))]++;
	}
      //cout<<"Difference "<<(int)ndigits[(i-AliHLTTPCTransform::GetFirstRow(fPatch))]<<" "<<(int)rowPt->fNDigit<<endl;
      fMemHandler->UpdateRowPointer(rowPt);
    }
  
  Int_t size = digitcount*sizeof(AliHLTTPCDigitData) + AliHLTTPCTransform::GetNRows(fPatch)*sizeof(AliHLTTPCDigitRowData);
  Byte_t *data = new Byte_t[size];
  memset(data,0,size);
  AliHLTTPCDigitRowData *tempPt = (AliHLTTPCDigitRowData*)data;
  rowPt = (AliHLTTPCDigitRowData*)fRowData;
  ...
  for(Int_t i=AliHLTTPCTransform::GetFirstRow(fPatch); i<=AliHLTTPCTransform::GetLastRow(fPatch); i++)
    {
      Int_t localcount=0;
      tempPt->fRow = i;
      tempPt->fNDigit = ndigits[(i-AliHLTTPCTransform::GetFirstRow(fPatch))];
      AliHLTTPCDigitData *digPt = (AliHLTTPCDigitData*)rowPt->fDigitData;
      for(UInt_t j=0; j<rowPt->fNDigit; j++)
	{
	  if(digPt[j].fCharge==0) continue;
	  if(localcount >= ndigits[(i-AliHLTTPCTransform::GetFirstRow(fPatch))])
	    {
	      cerr<<"AliHLTTPCModeller::WriteRemaining : Digitarray out of range!!"<<endl;
	      return;
	    }
      ...
      if(ndigits[(i-AliHLTTPCTransform::GetFirstRow(fPatch))] != localcount)
	{
	  cerr<<"AliHLTTPCModeller::WriteRemaining : Mismatch in digitcount"<<endl;
	  return;
	}
      fMemHandler->UpdateRowPointer(rowPt);
      Byte_t *tmp = (Byte_t*)tempPt;
      Int_t size = sizeof(AliHLTTPCDigitRowData) + ndigits[(i-AliHLTTPCTransform::GetFirstRow(fPatch))]*sizeof(AliHLTTPCDigitData);
      tmp += size;
  ...
  mem->Memory2CompBinary((UInt_t)AliHLTTPCTransform::GetNRows(fPatch),(AliHLTTPCDigitRowData*)data);
  mem->CloseBinaryOutput();
  delete mem;
  delete [] data;
}

void AliHLTTPCModeller::RemoveBadTracks()
{
...
void AliHLTTPCModeller::CalculateCrossingPoints()
{
  if(fDebug)
      {
#if 0
  ...
  if(!fTracks)
    {
      cerr<<"AliHLTTPCModeller::CalculateCrossingPoints(): No tracks"<<endl;
      return;
    }
  Float_t hit[3];
  
  Int_t sector,row;
  for(Int_t i=AliHLTTPCTransform::GetLastRow(fPatch); i>=AliHLTTPCTransform::GetFirstRow(fPatch); i--)
    {
      for(Int_t j=0; j<fTracks->GetNTracks(); j++)
	{
	  AliHLTTPCModelTrack *track = (AliHLTTPCModelTrack*)fTracks->GetCheckedTrack(j);
	  if(!track) continue;

	  if(!track->GetCrossingPoint(i,hit)) 
	    {
	      //cerr<<"AliHLTTPCModeller::CalculateCrossingPoints : Track "<<j<<" does not intersect row "<<i<<" :"<<endl<<
	      //	" pt "<<track->GetPt()<<
	  ...
	  AliHLTTPCTransform::Local2Raw(hit,sector,row);
	  //cout<<"Pad "<<hit[1]<<" time "<<hit[2]<<" in sector "<<sector<<" row "<<row<<endl;
	  if(hit[1]<0 || hit[1]>AliHLTTPCTransform::GetNPads(i) ||
	     hit[2]<0 || hit[2]>AliHLTTPCTransform::GetNTimeBins())
...
void AliHLTTPCModeller::CheckForOverlaps(Float_t dangle,Int_t *rowrange)
{
  ...
  for(Int_t k=AliHLTTPCTransform::GetFirstRow(fPatch); k<=AliHLTTPCTransform::GetLastRow(fPatch); k++)
    {
      if(rowrange)
	{
      ...
      for(Int_t i=0; i<fTracks->GetNTracks(); i++)
	{
	  AliHLTTPCModelTrack *track1 = (AliHLTTPCModelTrack*)fTracks->GetCheckedTrack(i);
	  if(!track1) continue;
	  if(track1->GetPadHit(k)<0 || track1->GetTimeHit(k)<0) continue;
	  
	  for(Int_t j=i+1; j<fTracks->GetNTracks(); j++)
	    {
	      AliHLTTPCModelTrack *track2 = (AliHLTTPCModelTrack*)fTracks->GetCheckedTrack(j);
	      if(!track2) continue;
...
void AliHLTTPCModeller::CalcClusterWidth(Cluster *cl,Float_t &sigmaY2,Float_t &sigmaZ2)
{
  
  Float_t padw,timew;
  
  padw = AliHLTTPCTransform::GetPadPitchWidth(fPatch);
  ...
  */
  ...
  timew = AliHLTTPCTransform::GetZWidth();
...
void AliHLTTPCModeller::GetTrackID(Int_t pad,Int_t time,Int_t *trackID)
{
#ifdef do_mc
  AliHLTTPCDigitRowData *rowPt = (AliHLTTPCDigitRowData*)fRowData;
