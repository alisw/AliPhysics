// $Id: AliHLTEMCALOnlineDisplayCalibTab.cxx 31683 2009-03-24 21:17:03Z odjuvsla $
...
#include <iostream>
#include "AliHLTEMCALRcuCellAccumulatedEnergyDataStruct.h"
...
AliHLTEMCALOnlineDisplayCalibTab::AliHLTEMCALOnlineDisplayCalibTab()
{
  // See header file for documentation
  cout << "AliHLTEMCALOnlineDisplayCalibTab:ERROR: You cannot create a onlinedisplay Tab without arguments" << endl;
}

AliHLTEMCALOnlineDisplayCalibTab::AliHLTEMCALOnlineDisplayCalibTab(TGTab  *tabPtr, HOMERReader *homerSyncPtr, HOMERReader *homerPtrs[MAXHOSTS], int nHosts)
{
  // See header file for documentation
...
void
AliHLTEMCALOnlineDisplayCalibTab::ReadBlockData(HOMERReader *homerReaderPtr)
{
  /*
  ...
  while ( blk != ~(unsigned long)0 ) 
    {
      cout << "AliHLTEMCALOnlineDisplayCalibTab::ReadBlockDat:GetHistogram: updating block " << endl;
      AliHLTUInt16_t moduleID;
      const AliHLTEMCALRcuCellAccumulatedEnergyDataStruct* accCellEnergiesPtr = (const AliHLTEMCALRcuCellAccumulatedEnergyDataStruct*)homerReaderPtr->GetBlockData( blk ); 
...
int 
AliHLTEMCALOnlineDisplayCalibTab::GetNextEvent()
{
  // See header file for documentation
  ResetDisplay();
  DoGetNextEvent();
...
void
AliHLTEMCALOnlineDisplayCalibTab::ResetDisplay() const
{
  // See header file for documentation
}

void 
AliHLTEMCALOnlineDisplayCalibTab::InitDisplay(TGTab *tabPtr)
{
  // See header file for documentation
  //  gStyle->SetOptLogy();
  ...
  for(int gain = 0; gain< NGAINS; gain ++)
   ...
   TGCompositeFrame  *tf = tabPtr->AddTab("Calibration data zzz");
	   ...
	   fgEventButtPtr = new  AliHLTEMCALGetEventButton(fSubF4, "update histograms z", 'h');
}


void 
AliHLTEMCALOnlineDisplayCalibTab::UpdateDisplay()
{
  // See header file for documentation
  // gStyle->SetOptLogy();
  //  gStyle->SetOptStat(true);
