...
ClassImp(AliTRDtrackerHLT)

const  Float_t  AliTRDtrackerHLT::fgkMinClustersInTrack =  0.5;  
const  Float_t  AliTRDtrackerHLT::fgkLabelFraction      =  0.8;  // ??
const  Double_t AliTRDtrackerHLT::fgkMaxChi2            = 12.0; 
const  Double_t AliTRDtrackerHLT::fgkMaxSnp             =  0.95; // Corresponds to tan = 3
const  Double_t AliTRDtrackerHLT::fgkMaxStep            =  2.0;  // Maximal step size in propagation 

//_____________________________________________________________________________
AliTRDtrackerHLT::AliTRDtrackerHLT()
  :AliTracker()
  ,fHBackfit(0x0)
  ,fHClSearch(0x0)
  ,fHRefit(0x0)
  ...
  ,fDebugStreamer(0)
{
  ...
  for (Int_t i = 0; i < AliTRDtrackingHLT::kTrackingSectors; i++) {
    fTrSec[i] = 0;
  }
  for (Int_t j = 0; j <  5; j++) {
...
//_____________________________________________________________________________
AliTRDtrackerHLT::AliTRDtrackerHLT(const AliTRDtrackerHLT &t)
  :AliTracker(t)
  ,fHBackfit(0x0)
  ,fHClSearch(0x0)
  ,fHRefit(0x0)
...
//_____________________________________________________________________________
AliTRDtrackerHLT::AliTRDtrackerHLT(const TFile *geomfile)
  :AliTracker()
  ,fHBackfit(0x0)
  ,fHClSearch(0x0)
  ,fHRefit(0x0)
  ...
  ,fDebugStreamer(0)
{
  ...
  for (Int_t geomS = 0; geomS < AliTRDtrackingHLT::kTrackingSectors; geomS++) {
    Int_t trS   = geomS;
    fTrSec[trS] = new AliTRDtrackingSectorHLT(fGeom,geomS);
...
{
  //
  // Destructor of AliTRDtrackerHLT 
  //
...
//_____________________________________________________________________________
Int_t  AliTRDtrackerHLT::LocalToGlobalID(Int_t lid)
{
  //
...
//_____________________________________________________________________________
Int_t  AliTRDtrackerHLT::GlobalToLocalID(Int_t gid)
{
  //
...
//_____________________________________________________________________________
Bool_t  AliTRDtrackerHLT::Transform(AliTRDcluster *cluster)
{
  //
...
//_____________________________________________________________________________
Bool_t AliTRDtrackerHLT::AdjustSector(AliTRDtrack *track) 
{
  //
...
//_____________________________________________________________________________
AliTRDcluster *AliTRDtrackerHLT::GetCluster(AliTRDtrack *track, Int_t plane
                                       , Int_t timebin, UInt_t &index)
{
  ...
  for (UInt_t i = 0; i < AliTRDtrackingHLT::kMaxTimeBinIndex; i++) {
    if (indexes[i] == 0) {
      break;  
    }
...
//_____________________________________________________________________________
Int_t  AliTRDtrackerHLT::GetLastPlane(AliTRDtrack *track)
{
  //
...
//_____________________________________________________________________________
Int_t AliTRDtrackerHLT::FollowProlongation(AliTRDtrack &t)
{
  //
  ...
  for (Int_t iplane = lastplane; iplane >= 0; iplane--) {
    ...
    for (Int_t itime = 0 ; itime < GetTimeBinsPerPlane(); itime++) {
      ...
      AliTRDpropagationLayerHLT &timeBin = *(fTrSec[sector]->GetLayer(ilayer));
      AliTRDcluster *cl = 0;
      UInt_t   index   = 0;
...
//_____________________________________________________________________________
Int_t AliTRDtrackerHLT::FollowBackProlongation(AliTRDtrack &t)
{
  //  
...
//_____________________________________________________________________________
Int_t AliTRDtrackerHLT::PropagateToX(AliTRDtrack &t, Double_t xToGo, Double_t maxStep)
{
  //
  // Starting from current radial position of track <t> this function
...
//_____________________________________________________________________________
Int_t AliTRDtrackerHLT::LoadClusters(TTree *cTree)
{
  //
...
void AliTRDtrackerHLT::MakeSeedsMI(Int_t /*inner*/, Int_t /*outer*/, AliESDEvent *esd)
{
  ...
  Int_t layers[6][2];
  AliTRDpropagationLayerHLT *reflayers[6];
  for (Int_t i = 0; i < 6; i++) {
    layers[i][0] = 10000; 
    layers[i][1] = 0;
  }
  for (Int_t ns = 0; ns < maxSec; ns++) {
    for (Int_t ilayer = 0; ilayer < fTrSec[ns]->GetNumberOfLayers(); ilayer++) {
      AliTRDpropagationLayerHLT &layer = *(fTrSec[ns]->GetLayer(ilayer));
      if (layer == 0) {
        continue;
      }
  ...
  //for (Int_t ns = 0; ns < 5; ns++) { // Loop over sectors
	...
	xcl[ilayer]       = reflayers[ilayer]->GetX();
      ...
      AliTRDpropagationLayerHLT &layer0 = *reflayers[sLayer+0];
      AliTRDpropagationLayerHLT &layer1 = *reflayers[sLayer+1];
      AliTRDpropagationLayerHLT &layer2 = *reflayers[sLayer+2];
      AliTRDpropagationLayerHLT &layer3 = *reflayers[sLayer+3];

      Int_t maxn3 = layer3;
      for (Int_t icl3 = 0; icl3 < maxn3; icl3++) {

	//AliInfo(Form("icl3 %d", icl3));
	...
	for (Int_t icl0 = layer0.Find(yymin0); icl0 < maxn0; icl0++) {
	  ...
	  for (Int_t icl1 = layer1.Find(yymin1); icl1 < maxn1; icl1++) {
	    ...
	    AliDebug(2, Form("Fit Seeding part"));
		...
		for (Int_t iTime = 2; iTime < 20; iTime++) { 
		  ...
		  AliTRDpropagationLayerHLT &layer = *(fTrSec[ns]->GetLayer(layers[sLayer+jLayer][1]-iTime));
	    ...
	    AliDebug(2, Form("never here? FULL TRACK FIT PART START"));
		...
		for (Int_t iTime = 2; iTime < 20; iTime++) { 
		  AliTRDpropagationLayerHLT &layer = *(fTrSec[ns]->GetLayer(layers[jLayer][1]-iTime));
		...
		for (Int_t iTime = 2; iTime < 20; iTime++) { 
		  ...
		  AliTRDpropagationLayerHLT &layer = *(fTrSec[ns]->GetLayer(layers[bLayer][1]-iTime));
...
// 	    }
    ...
    AliDebug(2, Form("N registered = %d", registered));
	...
	AliDebug(2, Form("Register seed %d", index));
	...
	AliTRDtrack  dummy;
	if (!track) {
	  AliWarning(Form("register seed returned 0%x", track));
          track = &dummy;
	}
	else {
...
//_____________________________________________________________________________
Int_t AliTRDtrackerHLT::ReadClusters(TObjArray *array, TTree *clusterTree) const
{
  //
...
//_____________________________________________________________________________
Bool_t AliTRDtrackerHLT::GetTrackPoint(Int_t index, AliTrackPoint &p) const
{
  //
...
//_____________________________________________________________________________
void AliTRDtrackerHLT::CookLabel(AliKalmanTrack *pt, Float_t wrong) const 
{
  //
...
//_____________________________________________________________________________
void AliTRDtrackerHLT::UseClusters(const AliKalmanTrack *t, Int_t from) const 
{
  //
...
//_____________________________________________________________________________
Double_t AliTRDtrackerHLT::ExpectedSigmaY2(Double_t , Double_t , Double_t ) const
{
  //
...
//_____________________________________________________________________________
Double_t AliTRDtrackerHLT::ExpectedSigmaZ2(Double_t , Double_t ) const
{
  //
...
//_____________________________________________________________________________
Double_t AliTRDtrackerHLT::GetX(Int_t sector, Int_t plane, Int_t localTB) const 
{
  //
...
{ 
  //
  // AliTRDpropagationLayerHLT constructor
  //

  for (Int_t i = 0; i < (Int_t) AliTRDtrackingHLT::kZones; i++) {
    fZc[i]   = 0; 
    fZmax[i] = 0;
  }

  if (fTimeBinIndex >= 0) { 
    fClusters = new AliTRDcluster*[AliTRDtrackingHLT::kMaxClusterPerTimeBin];
    fIndex    = new UInt_t[AliTRDtrackingHLT::kMaxClusterPerTimeBin];
  }
...
//_____________________________________________________________________________
void AliTRDpropagationLayerHLT
...
{
  //
  // AliTRDtrackingSectorHLT Constructor
  //

  AliTRDpadPlane *padPlane = 0;
  AliTRDpropagationLayerHLT *ppl = 0;
  ...
  AliTRDCommonParam *commonParam = AliTRDCommonParam::Instance();
  if (!commonParam) {
    AliErrorGeneral("AliTRDtrackingSectorHLT::Ctor"
  ...
  for (Int_t plane = 0; plane < AliTRDgeometry::Nplan(); plane++) {
    ...
    for (Int_t iTime = 0; iTime < nTimeBins; iTime++) {
      ...
      ppl     = new AliTRDpropagationLayerHLT(x,dx,rho,radLength,tbIndex,plane);
                  ...
                  ::MapTimeBinLayers() 
{
  ...
  for (Int_t i = 0; i < fN; i++) {
    ...
    if (index >= (Int_t) AliTRDtrackingHLT::kMaxTimeBinIndex) {
      //AliWarning(Form("Index %d exceeds allowed maximum of %d!\n"
      //               ,index,AliTRDtrackingHLT::kMaxTimeBinIndex-1));
      continue;
    }
                   ...
                   ::GetNumberOfTimeBins() const 
{
  ...
  for (tb = AliTRDtrackingHLT::kMaxTimeBinIndex - 1; tb >= 0; tb--) {
...
//_____________________________________________________________________________
void AliTRDtrackingSectorHLT
                  ::InsertLayer(AliTRDpropagationLayerHLT *pl)
{ 
  //
  ...
  if (fN == ((Int_t) AliTRDtrackingHLT::kMaxLayersPerSector)) {
  ...
  memmove(fLayers+i+1,fLayers+i,(fN-i)*sizeof(AliTRDpropagationLayerHLT*));
...
//_____________________________________________________________________________
void AliTRDpropagationLayerHLT::SetHoles(Bool_t *holes)
{
  //
  // set centers and the width of sectors
  //
                  ...
                  ::InsertCluster(AliTRDcluster *c, UInt_t index) 
{
  ...
  if (fN == (Int_t) AliTRDtrackingHLT::kMaxClusterPerTimeBin) {
...
//_____________________________________________________________________________
Int_t AliTRDpropagationLayerHLT::Find(Float_t y) const 
{
  //
...
//_____________________________________________________________________________
Int_t AliTRDpropagationLayerHLT
...
//_____________________________________________________________________________
Double_t AliTRDtrackerHLT::GetTiltFactor(const AliTRDcluster *c) 
{
  //
...
//_____________________________________________________________________________
void AliTRDtrackerHLT::CookdEdxTimBin(AliTRDtrack &TRDtrack)
{
  //
...
//_____________________________________________________________________________
Int_t AliTRDtrackerHLT::FindClusters(Int_t sector, Int_t t0, Int_t t1
                                , AliTRDtrack *track
                                , Int_t *clusters, AliTRDtracklet &tracklet)
{
  ...
  for (Int_t it = 0; it < t1-t0; it++) {
    ...
    AliTRDpropagationLayerHLT &timeBin = *(fTrSec[sector]->GetLayer(it+t0));
    if (timeBin == 0) {
...
//_____________________________________________________________________________
Int_t AliTRDtrackerHLT::Freq(Int_t n, const Int_t *inlist
...
//_____________________________________________________________________________
AliTRDtrack *AliTRDtrackerHLT::RegisterSeed(AliTRDseed *seeds, Double_t *params)
{
  //
  // Register a seed
  //
  ...
  AliDebug(2, Form("FollowBackProlongation"));

  Int_t rc = FollowBackProlongation(*track);
  if (rc < 30) {
    AliWarning("rc < 30 deleting the track");
    delete track;
    track = 0;
  }
  else {
...
void AliTRDtrackerHLT::InitLogHists() {
...
void AliTRDtrackerHLT::SaveLogHists() {
