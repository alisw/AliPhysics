...
for hist in ${histograms[@]}; do
  ...
  ZMQhistSource name=${hist} entries=1 nbins=500 sleep=10 distribution="exp(-0.5*((x-0.)/0.1)**2)" range="'-0.5,0.5'" out="PUSH>ipc:///tmp/example_proxyin" &>/dev/null &
  sourcePID+=(${!})
done

ZMQproxy  in="PULL@ipc:///tmp/example_proxyin" out="PUSH@ipc:///tmp/example_proxyout"&
sourcePID+=(${!})
ZMQROOTmerger in="PULL>ipc:///tmp/example_proxyout" out="PUB@ipc:///tmp/example_mergerout" MaxObjects=10 pushback-period=1000&
sourcePID+=(${!})
