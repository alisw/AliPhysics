...
AliHLTCaloChannelDataStruct* AliHLTCaloSharedMemoryInterfacev2::NextChannel()
{
  // Comment
    ...
    if (fHasRawData == true)
      if (false) {
        ...
        if (fRawData.fChannelID == fRawDataPtr[0]) {
          ...
          // cout << __FILE__ << __LINE__ << "ERROR! fRawData.fChannelID = "<< fRawData.fChannelID << " but
          // fRawDataPtr[0] = " << fRawDataPtr[0] << endl;
        }
...
void AliHLTCaloSharedMemoryInterfacev2::NextRawChannel()
{
...
void
  // AliHLTCaloSharedMemoryInterfacev2::SetMemory(AliHLTCaloChannelDataHeaderStruct* channelDataHeaderPtr,  const
...
void AliHLTCaloSharedMemoryInterfacev2::Reset()
{
  // Shutting up rule checker
  fCurrentCnt = 0;
  fIsSetMemory = false;
