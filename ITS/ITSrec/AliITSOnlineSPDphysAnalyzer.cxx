...
UInt_t AliITSOnlineSPDphysAnalyzer::ProcessNoisyPixels() {
	...
	//UInt_t minHitsNoisy = 0.0005*GetNrEvents();
...
UInt_t AliITSOnlineSPDphysAnalyzer::ProcessNoisyPixelsFast() {
	// process noisy pixel data , returns number of chips with enough statistics
	if (fPhysObj==NULL) {
		Warning("AliITSOnlineSPDphysAnalyzer::ProcessNoisyPixels","No data!");
		return 0;
	...
	if (fHandler==NULL) {
        ...
        UInt_t nrNoisyPixels = 0;
	UInt_t nrEvts = GetNrEvents();
	UInt_t eq = GetEqNr();
	UInt_t minHitsNoisy = fDefinitelyNoisyRatio*nrEvts;
	//UInt_t minHitsNoisy = 0.0005*GetNrEvents();
	for (UInt_t hs=0; hs<6; hs++) {
		if(fPhysObj->GetAverageMultiplicityTot(hs)<1e-6) continue;
		for (UInt_t chip=0; chip<10; chip++) {
...
UInt_t AliITSOnlineSPDphysAnalyzer::ProcessDead() {

	Printf("AliITSOnlineSPDphysAnalyzer::ProcessDead()");
	// process dead pixel data , returns number of dead pixels
	if (fPhysObj==NULL) {
				...
				if (!fHandler->IsActiveChip(GetEqNr(),hs,chip)) {
					...
					for (UInt_t col=0; col<32; col++) {
						for (UInt_t row=0; row<256; row++) {
							...
							else fHandler->UnSetDeadPixel(GetEqNr(),hs,chip,col,row);
						}//end loop on row
					}//end loop on col 
				}
			}//end loop on chips
