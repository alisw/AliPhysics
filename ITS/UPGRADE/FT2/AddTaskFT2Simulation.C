AliAnalysisTask* AddTaskFT2Simulation(Bool_t ft2OutputTree = 0, Bool_t tuneOnDataOrMC=0,Bool_t simMat=0,TString tpcParameterizationFile="", TString crossSectionFile="",Bool_t usePID=0,Double_t maxStepTGeo=0,Double_t dNdY=0,Bool_t useKalman=0,Bool_t allowDecay=0, Bool_t allowAbsorbtion=0,Bool_t allowConversions=0,TString outputFile="",Int_t runnumber=0,Int_t debugLevel=0,Int_t level=0,TString taskname=""){
  ...
  if(ft2OutputTree)AliAnalysisDataContainer *coutputmassD26 = mgr->CreateContainer("filteredTree", TTree::Class(), AliAnalysisManager::kOutputContainer, outputFile.Data());
  ...
  if(ft2OutputTree)mgr->ConnectOutput(TPResidualTask,7,coutputmassD26);
