...
void run_mcgenFT2(Int_t nEvents =0, Int_t run_number=0, Bool_t ft2OutputTree = 0, Int_t seed =0)
{
  ...
  Bool_t allowConversions      	  = kFALSE;
  ...
  //	gROOT->ProcessLine(Form(".x sim.C(%i)",nEvents));
  ...
  if (doCDBconnect && !useTender) {
    ...
    AliTaskCDBconnect *taskCDB = AddTaskCDBconnect("raw://",run_number);
    if (!taskCDB) return;
