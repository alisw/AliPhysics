...
void CreateITSUv1_tdr7()
{
  ...
  //  With this layout, the two middle layers are shifted in radius closer to the outer layers.
  // ----------------------------------------------------------------------------------------------------

  const Double_t dltR =4.94;  // radial shift wrt the TDR6  
  const Double_t dltZ =21.1;  // increment in the lenght (for the corresponding wrapper volume) 
  ...
  const double tdr5dat[kNLr][kNPar] = { 
    ...
    {-1,  19.6 +dltR, -1, 4.+dltNmod, 0., 24+dltNsta},  // for others: -, rMid, -, NMod/Stave, phi0, nStave // 24 was 49
  ...
  //      virtual void   DefineLayerTurbo(const Int_t nlay, const Double_t r,  const Double_t zlen, const Int_t nstav,   const Int_t nmod, const Double_t width,
