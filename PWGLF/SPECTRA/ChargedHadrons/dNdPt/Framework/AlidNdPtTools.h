...
public:
  ...
  static Long64_t FillHist(THnBase* s, Double_t x0, Double_t x1 = 0, Double_t x2 = 0,
                           Double_t x3 = 0, Double_t x4 = 0, Double_t x5 = 0, Double_t x6 = 0,
                           Double_t x7 = 0, Double_t x8 = 0, Double_t x9 = 0, Double_t x10 = 0,
                           Double_t x11 = 0);
  static Long64_t FillHist(Double_t w, THnBase* s, Double_t x0, Double_t x1 = 0, Double_t x2 = 0,
                           Double_t x3 = 0, Double_t x4 = 0, Double_t x5 = 0, Double_t x6 = 0,
                           Double_t x7 = 0, Double_t x8 = 0, Double_t x9 = 0, Double_t x10 = 0,
                           Double_t x11 = 0);
  static Long64_t FillHistWeighted(THnBase* s, std::vector<double> const& val, double weight);
  static Int_t AddAxis(const char* label, Int_t nbins, Double_t xmin, Double_t xmax,
                       const char* option = 0); // options: <none>
  static Int_t AddAxis(const char* label, const char* title, Int_t nbins, Double_t xmin,
                       Double_t xmax,
                       const char* option = 0); // options: <none>
  static Int_t AddAxis(const char* label, Int_t nbins, Double_t* xbins,
                       const char* option = 0); // options: <none>
  static Int_t AddAxis(const char* label, const char* title, Int_t nbins, Double_t* xbins,
                       const char* option = 0); // options: <none>
  static Int_t AddAxis(const char* label, const char* title,
                       const char* option);                    // options: pt
  ...
  static void Log(TH1* h, const char* name)
  {
    if(h) h->Fill(name, 1);
  }
  static Double_t MCScalingFactor(
    ProductionType prod, ParticleType part,
    Double_t pt); // this is a temp solution, to be replace by MCSpectraWeights
                  // quick and dirty hard coded solution to be replaced
                  // periodindex 0: pp 13 TeV, LHC18b  -- this is the (new)
                  // default periodindex 1: pp 5TeV, LHC17pq   -- previoulsy that
                  // was the default! be aware!
  static Double_t MCScalingFactor(AliMCParticle* part, AliMCEvent* event, Int_t systflag = 0,
                                  Int_t periodindex = 0); // access to the scaling

  static ParticleType ParticleTypeFromPDG(Int_t pdgCode);
  static void Range2Pi(Double_t& val)
  {
    while(val >= 2 * TMath::Pi())
      val -= 2 * TMath::Pi();
    while(val < 0)
      val += 2 * TMath::Pi();
  } // change range: 0 <= val < 2Pi
  static void Range1Pi(Double_t& val)
  {
    while(val >= TMath::Pi())
      val -= 2 * TMath::Pi();
    while(val < -TMath::Pi())
      val += 2 * TMath::Pi();
  } // change range: -Pi <= val < Pi
