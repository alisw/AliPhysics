#ifndef ALIMPTANALYSIS_H
#define ALIMPTANALYSIS_H
...
// 
// renamed and modified for mean pt analysis: 2013-01-28 by M. Marquard
// 
...
class AlimPtAnalysis : public AlidNdPt {
	public :
		AlimPtAnalysis(); 
		AlimPtAnalysis(Char_t* name, Char_t* title);
		~AlimPtAnalysis();
		...
		//Configure centrality selection
		void SetCentSelection(Bool_t centOn=kTRUE) {fCentSelection = centOn;}
		...
		TFolder *CreateFolder(TString folder = "foldermPtAnalysis",TString title = "Analysed dNdPt histograms");
		...
		void SetmultTrackCuts(AliESDtrackCuts* esdmultTrackCuts) {fmultTrackCuts = esdmultTrackCuts;}
		void SetmultAccCuts(AlidNdPtAcceptanceCuts* dndptmultAccCuts) {fmultAccCuts = dndptmultAccCuts;}
	...
	private:
		...
		AliESDtrackCuts *fmultTrackCuts;
		AlidNdPtAcceptanceCuts *fmultAccCuts;
		...
		AlimPtAnalysis(const AlimPtAnalysis&); // not implemented
		AlimPtAnalysis& operator=(const AlimPtAnalysis&); // not implemented

		ClassDef(AlimPtAnalysis,4);
};
