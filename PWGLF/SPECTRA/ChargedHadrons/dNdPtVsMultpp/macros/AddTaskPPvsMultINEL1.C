...
// Edited by Aditya Nath Mishra, amishra@cern.ch


AliAnalysisTask* AddTaskPPvsMultINEL1(Bool_t is13TeV = kTRUE)
{
	...
	if (!mgr) {
		Error("AddTaskPPvsMultINEL1", "No analysis manager to connect to.");
		return NULL;
	}  
	...
	if (!mgr->GetInputEventHandler()) {
		Error("AddTaskPPvsMultINEL1", "This task requires an input event handler");
		return NULL;
	}  

        gROOT->LoadMacro("$(ALICE_PHYSICS)/PWGLF/SPECTRA/ChargedHadrons/dNdPtVsMultpp/macros/CreateTrackCutsPWGLF0.C");
