...
public:
  ...
  enum AliMESbaseSteering{
    ...
    ,kTriggerHM   = BIT(21)     // request HM trigger (kFALSE = request MB trigger)
  };
  enum EMEScontainers{
     kQA        = 1
    ,kEventInfo = 1
  ...
  Bool_t         RequestTriggerHM() const   { return TestBit(kTriggerHM);};
  ...
  virtual void   SetTriggerHM(Bool_t hm = kFALSE)  { SetBit(kTriggerHM, hm);};
