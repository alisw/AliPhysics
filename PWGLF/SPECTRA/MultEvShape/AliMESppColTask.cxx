...
// 	if(!fEvInfo->HasTriggerHM()) return ; //High Multiplicity Trigger
  ...
  for(Int_t it(0); it<fTracks->GetEntries(); it++){
    ...
    do{
      ...
      if((pTlead>=1. && pTlead<=2.) && mult_comb08>40 && mult_comb08<=150 && TMath::Abs(fEvInfo->GetVertexZ())<10.0 && sfer>0.0 && sfer<=0.3){
//         TObjArray *selectedTracks1=FindLeadingObjects(fTracks, 0);
      ...
      if((pTlead>=1. && pTlead<=2.) && mult_comb08>40 && mult_comb08<=150 && TMath::Abs(fEvInfo->GetVertexZ())<10.0 && sfer>0.3 && sfer<=0.6){
//         TObjArray *selectedTracks2=FindLeadingObjects(fTracks, 0);
        ...
        FillCorrelationMixing(mult_comb08, fEvInfo->GetVertexZ(), 150., 0., selectedTracks2, 6, 0);
      }
      if((pTlead>=1. && pTlead<=2.) && mult_comb08>40 && mult_comb08<=150 && TMath::Abs(fEvInfo->GetVertexZ())<10.0 && sfer>0.6 && sfer<=1.0){
//         TObjArray *selectedTracks3=FindLeadingObjects(fTracks, 0);
        ...
        FillCorrelationMixing(mult_comb08, fEvInfo->GetVertexZ(), 150., 0., selectedTracks3, 9, 0);
      }
        ESD=0;
    }while(ESD==1);
	...
	if( HasMCdata()){// run only on MC  
      // NOTE: the intervals are considered half-closed: (a,b]
      if((pTMClead>=1.0 && pTMClead<=2.0) && MC_mult_glob08>40 && MC_mult_glob08<=150 && TMath::Abs(fMCevInfo->GetVertexZ())<10.0 && MC_sfer>0.0 && MC_sfer<=0.3){
// 		TObjArray *selectedTracksMC1=FindLeadingObjects(fMCtracks, 1);
      ...
      if((pTMClead>=1.0 && pTMClead<=2.0) && MC_mult_glob08>40 && MC_mult_glob08<=150 && TMath::Abs(fMCevInfo->GetVertexZ())<10.0 && MC_sfer>0.3 && MC_sfer<=0.6){
// 		TObjArray *selectedTracksMC2=FindLeadingObjects(fMCtracks, 1);
        ...
        FillCorrelationMixing(MC_mult_glob08, fMCevInfo->GetVertexZ(), 150., 0., selectedTracksMC2, 6, 1);
      }
      if((pTMClead>=1.0 && pTMClead<=2.0) && MC_mult_glob08>40 && MC_mult_glob08<=150 && TMath::Abs(fMCevInfo->GetVertexZ())<10.0 && MC_sfer>0.6 && MC_sfer<=1.0){
// 		TObjArray *selectedTracksMC3=FindLeadingObjects(fMCtracks, 1);
        ...
        FillCorrelationMixing(MC_mult_glob08, fMCevInfo->GetVertexZ(), 150., 0., selectedTracksMC3, 9, 1);
      }
    }
}
