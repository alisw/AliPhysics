...
//Charged Multiplicity and Sphericity Event Shape analysis
//new multiplicity domains and larger eta range for particles
...
AliMESppColTask::~AliMESppColTask()
{
  ...
  if(fPoolMgr4) {delete fPoolMgr4; fPoolMgr4=0;}
  ...
  if(fPoolMgrMC4) {delete fPoolMgrMC4; fPoolMgrMC4=0;}
}
...
// 	hNoEvts->Fill(vec_hNoEvts);
  ...
  for(Int_t it(0); it<fTracks->GetEntries(); it++){
    if(!(t = (AliMEStrackInfo*)fTracks->At(it))) continue;
	 ...
	 if( TMath::Abs(t->Eta())>= 0.9 ) continue;
  ...
  THnSparseD *hbTrk = (THnSparseD *)fHistosQA->At(3);
  Double_t vec_hbMCTrk[7];
  THnSparseD *hbMCTrk = (THnSparseD *)fHistosQA->At(4);
...
//	ESD tracks - two-particle correlations 
    ...
    do{
      ...
      if ((pTlead >= 1. && pTlead <= 2.) && mult_comb08 > 0 && mult_comb08 <= 80 && TMath::Abs(fEvInfo->GetVertexZ()) < 10.0 && vec_hbTrk[8] == 0 && sfer>0.0)
      {
        TObjArray *selectedTracks4 = SelectedTracks(fTracks, 0, idLead, -1, mult_comb08);
        if(!selectedTracks4) break;
        selectedTracks4->SetOwner(kTRUE);
        FillCorrelationSE(mult_comb08, selectedTracks4, 4, 0, sfer);
        FillCorrelationMixing(mult_comb08, fEvInfo->GetVertexZ(), 80., 0., selectedTracks4, 4, 0);
      }
        ESD=0;
    }while(ESD==1);
	...
	if( HasMCdata()){// run only on MC  
      ...
      if ((pTMClead >= 1.0 && pTMClead <= 2.0) && MC_mult_glob08 > 0 && MC_mult_glob08 <= 80 && TMath::Abs(fMCevInfo->GetVertexZ()) < 10.0 && vec_hbTrk[8] == 0 && MC_sfer>0.0)
      {TObjArray *selectedTracksMC4 = SelectedTracks(fMCtracks, 1, -1, idMCLead, MC_mult_glob08);
        if (!selectedTracksMC4)
          return;
        selectedTracksMC4->SetOwner(kTRUE);
        FillCorrelationSE(MC_mult_glob08, selectedTracksMC4, 4, 1, MC_sfer);
        FillCorrelationMixing(MC_mult_glob08, fMCevInfo->GetVertexZ(), 80., 0., selectedTracksMC4, 4, 1);}
    }
...
TObjArray*  AliMESppColTask::SelectedTracks(TObjArray *obj, Int_t MC, Int_t idL, Int_t idLMC, Double_t MultipOrCent)
{
	...
	const Int_t nMult(5);
	Double_t multBin[nMult+1] = {10., 15., 20., 30., 46., 80.};

	Double_t pTtrigMin[nMult] = {1., 1., 1., 1., 1.};
	Double_t pTtrigMax[nMult] = {2., 2., 2., 2., 2.};
    ...
    if(HasMCdata() && MC==1 && idLMC!=-1 && ((AliMEStrackInfo*)obj->At(idLMC))->Pt() >= pTtrigMin[jm] && ((AliMEStrackInfo*)obj->At(idLMC))->Pt() <= pTtrigMax[jm]){
        ...
        // Accept tracks in a limited pT & rapidity range
          if( TMath::Abs(partMC->Eta())>= 0.9 ) continue;
...
//  Int_t PoolMaxNEvents = 200;
 // Int_t PoolMinNTracks = 20000;
  ...
  Int_t NMultBins = 5;
  Double_t MultBins1[] = {10., 15., 20., 30., 46., 80.};
...
//   Double_t ZvtxBins3[] = {-10, -8., -6., -4, -2., 0., 2., 4., 6., 8., 10.};
    ...
    fPoolMgrMC4 = new AliEventPoolManager(PoolMaxNEvents, PoolMinNTracks, NMultBins, MultBins1, NzVtxBins1, ZvtxBins1);
    fPoolMgrMC4 -> SetTargetValues(PoolMinNTracks, 0.1, 5);
    fPoolMgrMC4->SetDebug(0);
//     if(!fPoolMgrMC) return kFALSE;
  ...
  fPoolMgr4 = new AliEventPoolManager(PoolMaxNEvents, PoolMinNTracks, NMultBins, MultBins1, NzVtxBins1, ZvtxBins1);
  fPoolMgr4 -> SetTargetValues(PoolMinNTracks, 0.1, 5);
  fPoolMgr4->SetDebug(0);
...
{
    Double_t vec_hTrk[3];
	THnSparseD *hTrk = (THnSparseD*)fHistosQA->At(1);
    Double_t vec_hMCTrk[3];
    THnSparseD *hMCTrk = (THnSparseD*)fHistosQA->At(2);
	...
	if( TMath::Abs(trigger->Eta())>= 0.9 ) return;
	...
	for (Int_t j=1; j<selectedArray->GetEntriesFast(); j++){
		...
		if( TMath::Abs(associate->Eta())>= 0.9 ) continue;
...
// 				vec_hMCTrk[1]=2;
    ...
    if(MC==0 && d==3 && assAux==111) ((TH2*)fHistosQA->At(85))->Fill(jm+0.5, 0.5);
    if(MC==0 && d==6 && assAux==111) ((TH2*)fHistosQA->At(85))->Fill(jm+0.5, 1.5);
    if(MC==0 && d==9 && assAux==111) ((TH2*)fHistosQA->At(85))->Fill(jm+0.5, 2.5);
    if(MC==0 && d==4 && assAux==111) ((TH2*)fHistosQA->At(85))->Fill(jm+0.5, 3.5);

    if(HasMCdata() && MC==1 && d==3 && assAuxMC==111) ((TH2*)fHistosQA->At(86))->Fill(jm+0.5, 0.5);
    if(HasMCdata() && MC==1 && d==6 && assAuxMC==111) ((TH2*)fHistosQA->At(86))->Fill(jm+0.5, 1.5);
    if(HasMCdata() && MC==1 && d==9 && assAuxMC==111) ((TH2*)fHistosQA->At(86))->Fill(jm+0.5, 2.5);
    if(HasMCdata() && MC==1 && d==4 && assAuxMC==111) ((TH2*)fHistosQA->At(86))->Fill(jm+0.5, 3.5);
...
{
	const Int_t nMult(5);
    Int_t multBin[nMult+1] = {10, 15, 20, 30, 46, 80};
	Double_t pTtrigMin[nMult] = {1., 1., 1., 1., 1.};
	Double_t pTtrigMax[nMult] = {2., 2., 2., 2., 2.};
  ...
  // 		pool1->SetTargetEvents(5);
        ...
        if (pool1->IsReady() || pool1->GetCurrentNEvents() > 5){
          ...
          for (Int_t jMix=0; jMix<nMix; jMix++){
            ...
            if( TMath::Abs(trigger->Eta())>= 0.9 ) continue;
  ...
  // 		pool3->SetTargetEvents(5);
      ...
      if(d==4 ){
          // AliInfo("Bucla de reconstruite!!!!!!!!");
        AliEventPool* pool4 = fPoolMgr4->GetEventPool(MultipOrCentMix, Zvtx);
        if (!pool4){
          AliInfo(Form("No pool found for mult/cent = %f, zVtx = %f", MultipOrCentMix, Zvtx));
          return;
        }

        if (pool4->IsReady() || pool4->GetCurrentNEvents() > 5){
          //	AliInfo("Pool este Ready!!!!!!!!");
          Int_t nMix = pool4->GetCurrentNEvents();
          for (Int_t jMix=0; jMix<nMix; jMix++){
            TObjArray* mixEvents = pool4->GetEvent(jMix);
            AliBasicParticle* trigger = (AliBasicParticle*)selectedArray->At(0);
            if(!trigger) continue;
            Double_t ptL  = trigger->Pt();
            Double_t phiL = trigger->Phi();
          ...
          TObjArray* tracksClone4 = new TObjArray;
          tracksClone4->SetOwner(kTRUE);
          for (Int_t i=0; i<selectedArray->GetEntriesFast(); i++){
            AliBasicParticle* particle4 = (AliBasicParticle*) selectedArray->At(i);
            tracksClone4->AddLast(particle4);
          }
            pool4->UpdatePool(tracksClone4);
      }
    }
...
//         poolMC3->PrintInfo();
  // 		poolMC3->SetTargetEvents(5);
      ...
      if(d==4){
        AliEventPool* poolMC4 = fPoolMgrMC4->GetEventPool(MultipOrCentMix, Zvtx);
        if (!poolMC4){
          AliInfo(Form("No pool found for mult/cent = %f, zVtx = %f", MultipOrCentMix, Zvtx));
          return;
        }

        if (poolMC4->IsReady() || poolMC4->GetCurrentNEvents() > 5){
          Int_t nMix = poolMC4->GetCurrentNEvents();
          for (Int_t jMix=0; jMix<nMix; jMix++){
            TObjArray* mixEvents = poolMC4->GetEvent(jMix);
            AliBasicParticle* trigger = (AliBasicParticle*)selectedArray->At(0);
            if(!trigger) continue;
            Double_t ptL  = trigger->Pt();
            Double_t phiL = trigger->Phi();
          ...
          TObjArray* tracksCloneMC4 = new TObjArray;
          tracksCloneMC4->SetOwner(kTRUE);
          for (Int_t i=0; i<selectedArray->GetEntriesFast(); i++){
            AliBasicParticle* particleMC4 = (AliBasicParticle*) selectedArray->At(i);
            tracksCloneMC4->AddLast(particleMC4);
          }
            poolMC4->UpdatePool(tracksCloneMC4);
      }
    }
}
...
// used for scaling
  ...
  const Int_t cldNbinsNoEvts[ndimNoEvts]   = {5, 150, 40, 100, 150, 30, 100};
  const Double_t cldMinNoEvts[ndimNoEvts]  = {-0.5, 0.5, 0., 0., 0.5, 0., 0.}, cldMaxNoEvts[ndimNoEvts]  = {4.5, 150.5, 1., 10., 150.5, 1., 10.};
  ...
  const Int_t ndimTrk(3);
  const Int_t cldNbinsTrk[ndimTrk]   = { 5, 4, 3};
  const Double_t cldMinTrk[ndimTrk]  = { 0., 0., -1.},
					  cldMaxTrk[ndimTrk]  = {5., 4., 1.};
  THnSparseD *hTrk = new THnSparseD("infoTrk","infoTrk; MultBin; SferBin;as;",ndimTrk, cldNbinsTrk, cldMinTrk, cldMaxTrk);
  fHistosQA->AddAt(hTrk, 1);

  THnSparseD *hMCTrk = new THnSparseD("infoMCTrk","infoMCTrk; MultBinMC; SferBinMC;asMC;",ndimTrk, cldNbinsTrk, cldMinTrk, cldMaxTrk);
  fHistosQA->AddAt(hMCTrk, 2);

  const Int_t ndimbTrk(9);
  const Int_t cldNbinsbTrk[ndimbTrk]   = { 100, 40, 100, 100, 36, 30, 3, 160, 2};
  const Double_t cldMinbTrk[ndimbTrk]  = { 0.5, 0., 0., 0.,-1.8, -0.5*TMath::Pi(), -1.5, 0., 0.},
					  cldMaxbTrk[ndimbTrk]  = {100.5, 1., 10., 10., 1.8, 1.5*TMath::Pi(), 1.5, 800., 2.};

  const Int_t ndimbTrkMC(7);
  const Int_t cldNbinsbTrkMC[ndimbTrkMC] = {100, 40, 100, 100, 36, 30, 3};
  const Double_t cldMinbTrkMC[ndimbTrkMC] = {0.5, 0., 0., 0., -1.8, -0.5 * TMath::Pi(), -1.5},
                 cldMaxbTrkMC[ndimbTrkMC] = {100.5, 1., 10., 10., 1.8, 1.5 * TMath::Pi(), 1.5};
...
//   Int_t multBin[nMult+1] = {1, 4, 7, 10, 15, 20, 25, 30, 40, 50, 60, 70, 80};
  ...
  for(Int_t sel(0); sel<4; sel++){
    for(Int_t im(0); im<nMult; im++){
        hSE[im+5*sel] = new TH2F(Form("hS%02d", im+5*sel), Form("Mult[%2d-%2d]Sfer[%d]", multBin[im], multBin[im+1]-1, selBin[sel]), 18, -1.8, 1.8, 30, -0.5*TMath::Pi(), 1.5*TMath::Pi());
        fHistosQA->AddAt(hSE[im+5*sel], im+5+5*sel);

        hME[im+5*sel] = new TH2F(Form("hE%02d", im+5*sel), Form("MultE[%2d-%2d]Sfer[%d]", multBin[im], multBin[im+1]-1, selBin[sel]), 18, -1.8, 1.8, 30, -0.5*TMath::Pi(), 1.5*TMath::Pi());
        fHistosQA->AddAt(hME[im+5*sel], im+25+5*sel);
    }
  }
  ...
  TH2F *hEvts (NULL); TH2F *hEvtsMC (NULL);
  for(Int_t sel(0); sel<4; sel++){
    for(Int_t im(0); im<nMult; im++){
        hSEMC[im+5*sel] = new TH2F(Form("hSMC%02d", im+5*sel), Form("MultSMC[%2d-%2d]Sfer[%d]", multBin[im], multBin[im+1]-1, selBin[sel]), 18, -1.8, 1.8, 30, -0.5*TMath::Pi(), 1.5*TMath::Pi());
        fHistosQA->AddAt(hSEMC[im+5*sel], im+5*sel+45);
        
        hMEMC[im+5*sel] = new TH2F(Form("hEMC%02d", im+5*sel), Form("MultEMC[%2d-%2d]Sfer[%d]", multBin[im], multBin[im+1]-1, selBin[sel]), 18, -1.8, 1.8, 30, -0.5*TMath::Pi(), 1.5*TMath::Pi());
        fHistosQA->AddAt(hMEMC[im+5*sel], im+5*sel+65);
      }
  }
    hEvts = new TH2F("Evts", "Evts", 5, 0, 5, 4, 0, 4);
        fHistosQA->AddAt(hEvts, 85);

        hEvtsMC = new TH2F("EvtsMC", "EvtsMC", 5, 0, 5, 4, 0, 4);
        fHistosQA->AddAt(hEvtsMC, 86);
