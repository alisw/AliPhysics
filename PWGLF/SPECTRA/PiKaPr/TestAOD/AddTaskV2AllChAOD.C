AliAnalysisTaskV2AllChAOD* AddTaskV2AllChAOD(
						       Bool_t mc=kFALSE,
  ...
  if (!mgr) 
    {
      ::Error("AddAliAnalysisTaskV2AllChAOD", "No analysis manager to connect to.");
      return NULL;
    }   
  ...
  if (!mgr->GetInputEventHandler()) 
    {
      ::Error("AliAnalysisTaskV2AllChAOD", "This task requires an input event handler");
      return NULL;
    }   
  ...
  if(type.Contains("ESD"))
    {
      ::Error("AliAnalysisTaskV2AllChAOD", "This task requires to run on AOD");
      return NULL;
    }
  ...
  AliAnalysisTaskV2AllChAOD *task = new AliAnalysisTaskV2AllChAOD(Form("TaskAODSpectraCent%.0fto%.0f_QVec%.1fto%.1f_Eta%.1fto%.1f_TrBit%d%s",	
										 CentCutMin,
										 CentCutMax,
										 QvecCutMin,
										 QvecCutMax,
  ...
  outputFileName += Form(":V2_ESE_%s%s",typeofdata.Data(),opt.Data());
  ...
  AliAnalysisDataContainer *coutputpt4 = mgr->CreateContainer(Form("chist_lq%s",opt.Data()),                TList::Class(),    AliAnalysisManager::kOutputContainer,outputFileName);
  AliAnalysisDataContainer *coutputpt5 = mgr->CreateContainer(Form("chist_sq%s",opt.Data()),                TList::Class(),    AliAnalysisManager::kOutputContainer,outputFileName);
  mgr->AddTask(task);
  ...
  mgr->ConnectOutput(task, 5, coutputpt5);
