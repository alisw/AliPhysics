void AddTaskSpectraBothManyTask(Bool_t mc=kFALSE,
					     ...
					     Int_t minrun=-1,
					     Int_t maxrun=-1,
					     Bool_t usedAdditionalCuts=kTRUE;	
...
{
	gROOT->LoadMacro("$ALICE_PHYSICS/PWGLF/SPECTRA/PiKaPr/TestAOD/AddTaskSpectraBoth.C");
	...
	for (int =0;i<7;ntask++)
	{
		...
		AliAnalysisTaskSpectraBoth* task1=AddTaskSpectraBoth(mc,CentCutMin,CentCutMax,QvecCutMin,QvecCutMax,EtaMin,EtaMax,Nsigmapid,pt,p,ymin,ymax,ptTofMatch,trkbit,trkbitQVector,UseCentPatchAOD049,DCA,minNclsTPC,nrebin,centestimator,pidmethod,opt);
