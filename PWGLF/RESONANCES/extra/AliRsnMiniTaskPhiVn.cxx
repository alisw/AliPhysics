...
//
   ...
   TComplex FlowVecQneg[iDiffHarm] = {0};
   TComplex FlowVecQpos[iDiffHarm] = {0};
   TComplex FlowVecQProduct[iDiffHarm] = {0};

   Double_t    MQPos[iDiffHarm] = {0};
   Double_t    MQNeg[iDiffHarm] = {0};

   
   Double_t FlowVecQnegRe[iDiffHarm] = {0};
   Double_t FlowVecQnegIm[iDiffHarm] = {0};
   Double_t FlowVecQposRe[iDiffHarm] = {0};
   Double_t FlowVecQposIm[iDiffHarm] = {0};
   ...
   for(int j = 0; j < iDiffHarm; j++) {
      ...
      Double_t MPPos[iDiffHarm][iPtBins][iInvMass] = {0};
      Double_t MPNeg[iDiffHarm][iPtBins][iInvMass] = {0};

      Double_t MPPosCharged[iDiffHarm][iPtBins] = {0};
      Double_t MPNegCharged[iDiffHarm][iPtBins] = {0};

      TComplex FlowPVecPosCharged[iDiffHarm][iPtBins] = {0};
      TComplex FlowPVecNegCharged[iDiffHarm][iPtBins] = {0};

      TComplex PVecPosCharged[iDiffHarm][iPtBins] = {0};
      TComplex PVecNegCharged[iDiffHarm][iPtBins] = {0};
      ...
      TComplex FlowPVecPos[iDiffHarm][iPtBins][iInvMass] = {0};
      TComplex FlowPVecNeg[iDiffHarm][iPtBins][iInvMass] = {0};
      ...
      Bool_t sameEvent = (fMiniEvent->ID() == fMiniEvent->ID());
      ...
      //AliDebugClass(1, Form("[%10s] Part #1: [%s] -- evID %6d -- charge = %c -- cut ID = %d --> %4d tracks (%s)", GetName(), (event1 == event2 ? "def" : "mix"), event1->ID(), fCharge[0], fCutID, n1, selList1.Data()));
      ...
      //AliDebugClass(1, Form("[%10s] Part #2: [%s] -- evID %6d -- charge = %c -- cut ID = %d --> %4d tracks (%s)", GetName(), (event1 == event2 ? "def" : "mix"), event2->ID(), fCharge[1], fCutID, n2, selList2.Data()));
      if (!n1 || !n2) {
         AliDebugClass(1, "No pairs to mix");
         //cout << "No pairs to mix" << endl;
         return;
      }
      ...
      for (i1 = 0; i1 < n1; i1++) {
         ...
         for (i2 = start; i2 < n2; i2++) {
            ...
            if (pairPt > 9.9 || pairPt < 0.0)
            {
              continue;
            }

            if (pairInvMass > 1.5 || pairInvMass < 0.0)
            {
               continue;
            }
