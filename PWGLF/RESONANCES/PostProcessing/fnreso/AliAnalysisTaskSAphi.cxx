...
#include "AliAnalysisTaskSAphi.h"
...
ClassImp(AliAnalysisTaskSAphi)
ClassImp(AliCompphiTrack)

AliAnalysisTaskSAphi::AliAnalysisTaskSAphi(): 
AliAnalysisTaskSE(), 
fOutput(0), 
fPoolMgr(0X0), 
fPIDResponse(0), 
...
AliAnalysisTaskSAphi::AliAnalysisTaskSAphi(const char *name): 
...
void AliAnalysisTaskSAphi::UserCreateOutputObjects()
{
  ...
  Int_t bins[4]={250, 10, 200, 5};
  Double_t xmin[4]={0.95, 0.0, 0.0, 0.0};
  Double_t xmax[4]={1.2, 100.0, 20.0, 1.0};
...
//________________________________________________________________________
void AliAnalysisTaskSAphi::UserExec(Option_t *)
{
  ...
  for(Int_t itr = 0; itr < ntracks; itr++)
      ...
      if(IsKaon(track))
	{
	  ...
	  selectedpiontracks->Add(new AliCompphiTrack(track->Px(),track->Py(),track->Pz(),aodtrack->Charge()));
	  pion.charge=aodtrack->Charge();
	  pion.trkid=itr;
	  pion.particle.SetXYZM(track->Px(), track->Py(), track->Pz(), kaonmass);
  ...
  if (pool && pool->IsReady())
      ...
      for (Int_t jMix = 0; jMix < nMix; jMix++)
	{
	  ...
	  for (const auto& bgTrack : *bgTracks)
	    {
	      AliCompphiTrack* piontrackmix = dynamic_cast<AliCompphiTrack*>(bgTrack);
	      if (!piontrackmix)
		{
...
//________________________________________________________________________
void AliAnalysisTaskSAphi::Terminate(Option_t *)
{
...
Bool_t AliAnalysisTaskSAphi::GoodEvent(const AliVVertex *vertex) 
...
//-----------------------------------------------
Bool_t AliAnalysisTaskSAphi::IsPion(AliVTrack *vtrack)
{
...
Bool_t AliAnalysisTaskSAphi::IsKaon(AliVTrack *vtrack)
{
...
Bool_t AliAnalysisTaskSAphi::HasTOF(AliAODTrack *track)
{
...
void AliAnalysisTaskSAphi::EventMixing()
{
...
Double_t AliAnalysisTaskSAphi::CosThetaStar(TLorentzVector mother, TLorentzVector daughter0, TLorentzVector daughter1)
{
...
Double_t AliAnalysisTaskSAphi::CosThetaStarHel(TLorentzVector mother, TLorentzVector daughter0, TLorentzVector daughter1)
...
/*
Double_t AliAnalysisTaskSAphi::CosThetaStarEP(TLorentzVector mother, TLorentzVector daughter0, TLorentzVector daughter1)
...
void AliAnalysisTaskSAphi::GetNUACorrectionHist(Int_t run, Int_t kParticleID)
{
...
void AliAnalysisTaskSAphi::GetV0MCorrectionHist(Int_t run, Int_t kParticleID)
{
...
void AliAnalysisTaskSAphi::GetMCCorrectionHist(Int_t run,Float_t centr){
