...
TH1D* GetSpectrafromName(TString inputfilename);
void Xi1530PlotSystematics() {
    vector<double> multibinsloop = {0, 10, 30, 50, 70, 100};
    //vector<double> multibinsloop = {0, 100};
    ...
    auto hDef = Xi1530PlotSystematics_multi(0, 100);
    fout_systematic->cd();
    ...
    for (int imultibin = 0; imultibin < multibincheck.size(); imultibin++) {
        ...
        auto htemp = MakeHistfromArray(Form("h_%.2f-%.2f_sys", multibincheck[imultibin][0],
                               multibincheck[imultibin][1]),
                                       temp_y, temp_e, ptbin_final);
    ...
    for (int imultibin = 0; imultibin < multibincheck.size(); imultibin++) {
        auto temp_spectra_multi = GetSpectra(multibincheck[imultibin][0],
                                             multibincheck[imultibin][1]);
        vector<double> temp_y;
        vector<double> temp_e;
        for (int bin = 0; bin <= ptbin.size(); bin++) {
        ...
        auto htemp = MakeHistfromArray(Form("h_%.2f-%.2f_stat", multibincheck[imultibin][0],
                               multibincheck[imultibin][1]),
                                       temp_y, temp_e, ptbin_final);
    ...
    */
    TCanvas* cCanvas_barlow =
        new TCanvas("cCanvas_barlow", "cCanvas_barlow", w, h);
    ...
    for (int topological_cutvar = 0; topological_cutvar < TopologicalCutSystematic_bins.size(); topological_cutvar++) {
        ...
        if (hbarlowcheck->GetStdDev() < 1.3)
            Stdtext = Form("#bf{StdDev: %.3f / Pass!}", hbarlowcheck->GetStdDev());
        else
            Stdtext = Form("#bf{StdDev: %.3f / Fail!}", hbarlowcheck->GetStdDev());

        t->DrawLatex(0.65, 0.77, Meantext);
        t->DrawLatex(0.65, 0.72, Stdtext);
        t->DrawLatex(0.65, 0.67, I1text);
        t->DrawLatex(0.65, 0.62, I2text);
        if ((I1value > 0.55) && (I2value > 0.75) &&
            (abs(hbarlowcheck->GetMean()) < 0.8) &&
            (hbarlowcheck->GetStdDev() < 1.3))
            t->DrawLatex(0.65, 0.57, "Consistency Check pass!");
        ...
        cCanvas_barlow->SaveAs(
            Form("./check_%s.pdf", TopologicalCutSystematic_bins[topological_cutvar].Data()));
    }
    ...
    for (int topological_cutvar = 0; topological_cutvar < FitSystematic_bins.size(); topological_cutvar++) {
        ...
        //
        cCanvas_barlow->SaveAs(
            Form("./check_%s.pdf", FitSystematic_bins[topological_cutvar].Data()));
    }
...
TH1D* Xi1530PlotSystematics_multi(double multi_start,
    ...
    auto hStatError = (TH1D*)MakeHistfromArray("hStatError", baseStatError, ErrorArray,
                                               ptbin, "NOERROR");
    ...
    auto hFitSysSum = (TH1D*)MakeHistfromArray("FitSys", ErrorArray, ErrorArray,
                                               ptbin, "NOERROR");
    TString fvarfile;
    ...
    */
    ...
    if ((fUseCutSysfrom0100) && (multi_end - multi_start > 99)){
        ...
        hMultiplicityIndependentTopologicalCutSysSum = (TH1D*)MakeHistfromArray("CutSys", ErrorArray, ErrorArray,
                                              ptbin, "NOERROR");
        ...
        hMultiplicityIndependentTopologicalCutSysSum = (TH1D*)MakeHistfromArray("TopologicalCutSys", TopolCutSysSum, ErrorArray,
                                              ptbin, "NOERROR");
    }
    fout->cd();
    ...
    if ((fUseCutSysfrom0100) && (multi_end - multi_start > 99)){
        cout << "Check Sys Error for Cut sys " << endl;
        hMultiplicityIndependentPIDCutSysSum = (TH1D*)MakeHistfromArray("PIDCutSys", ErrorArray, ErrorArray,
                                              ptbin, "NOERROR");
    ...
    */
        ...
        hMultiplicityIndependentPIDCutSysSum = (TH1D*)MakeHistfromArray("PIDCutSys", PIDCutSysSum, ErrorArray,
                                              ptbin, "NOERROR");
    }
    fout->cd();
    if (!(multi_end - multi_start > 99))
    ...
    auto hSysMaterialBudget = (TH1D*)MakeHistfromArray("hSysMaterialBudget", materialbudget, ErrorArray,
                                               ptbin, "NOERROR");
    ...
    auto hSysMultiIndepEffi = (TH1D*)MakeHistfromArray("hSysMultiIndepEffi", multiindepeffi, ErrorArray,
                                               ptbin, "NOERROR");
    ...
    auto hSysTrackingEffi = (TH1D*)MakeHistfromArray(
        "hSysTrackingEffi", trackingeffi, ErrorArray, ptbin, "NOERROR");
    //------------------------------------------------------------------------------
    // Total Uncertainty
    ...
    for (int bin = 0; bin <= ptbin.size(); bin++) {
        ...
        TotError +=
            pow(hFitSysSum->GetBinContent(bin + 1) *
                    base->GetBinContent(bin + 1),
                2) +
            pow(hMultiplicityIndependentTopologicalCutSysSum->GetBinContent(
                    bin + 1) *
                    base->GetBinContent(bin + 1),
                2) +
            pow(hMultiplicityIndependentPIDCutSysSum->GetBinContent(bin + 1) *
                    base->GetBinContent(bin + 1),
                2) +
            pow(hSysMaterialBudget->GetBinContent(bin + 1) *
                    base->GetBinContent(bin + 1),
                2) +
            pow(hSysMultiIndepEffi->GetBinContent(bin + 1) *
                    base->GetBinContent(bin + 1),
                2) +
            pow(hSysTrackingEffi->GetBinContent(bin + 1) *
                    base->GetBinContent(bin + 1),
                2);
    ...
    for (int bin = 0; bin <= ptbin.size(); bin++) {
        double temp;
        temp = pow(hFitSysSum->GetBinContent(bin + 1), 2) +
               pow(hMultiplicityIndependentTopologicalCutSysSum->GetBinContent(
                       bin + 1),
                   2) +
               pow(hMultiplicityIndependentPIDCutSysSum->GetBinContent(bin + 1),
                   2) +
               pow(hSysMaterialBudget->GetBinContent(bin + 1), 2) +
               pow(hSysMultiIndepEffi->GetBinContent(bin + 1), 2) +
               pow(hSysTrackingEffi->GetBinContent(bin + 1), 2);
        totalSysError.push_back(sqrt(temp));
    }
    auto hSysErrorSum = (TH1D*)MakeHistfromArray("hSysErrorSum", totalSysError, ErrorArray,
                                               ptbin, "NOERROR");
    ...
    auto hSpectra_7TeV_syserr = MakeHistfromArray(
        ...
        CorrectedYeild_syserr_7TeV, ptbin);
    auto hSpectra_7TeV_staterr = MakeHistfromArray(
        "7TeV Spectra with statistical error", CorrectedYeild_7TeV,
        CorrectedYeild_staterr_7TeV, ptbin);
...
vector<TH1D*> Xi1530SysCheck(TString fdeafultinputfile,
    ...
    bool fastbarlowcheck = false; // true: will check barlow cut every systematics/pT bin.
    ...
    for (int bin = 1; bin <= nbins; bin++) {  // pt bins
        ...
        double errorFraction =  // abs(Variation - Default)/Deafult - >"Error
                                // fraction"
            abs(1 - variation->GetBinContent(bin) / base->GetBinContent(bin));
        ...
        if (fastbarlowcheck) {  // Barlow check in dhevan case!
            if (error > deltasigma) {
                cout << "Barlow pass! errorFraction: " << errorFraction
                    << ", deltasigma:" << deltasigma
                    << ", check: " << error / deltasigma << endl;
                VarError.push_back(error);
                VarErrorFraction.push_back(errorFraction);
            } else {
                cout << "Barlow fail... errorFraction: " << errorFraction
                    << ", deltasigma:" << deltasigma
                    << ", check: " << error / deltasigma << endl;
    ...
    // Drawing QA Plots
    auto hSystematics =
        (TH1D*)MakeHistfromArray(Form("Var_%s", options), VarErrorFraction,
                                 VarErrorFraction, ptbin, "NOERROR");
    ...
    auto hVarStatError =
        (TH1D*)MakeHistfromArray(Form("VarStat_%s", options), StatError_var,
                                 StatError_var, ptbin, "NOERROR");
    auto hVarError =
        (TH1D*)MakeHistfromArray(Form("VarStat_%s", options), VarErrorCheck,
                                 VarErrorCheck, ptbin, "NOERROR");
...
TH1D* Xi1530SysFitVar(TString nameFitVar = "FitVar",
    ...
    auto temphist =
        (TH1D*)MakeHistfromArray(Form("%s", nameFitVar.Data()), ErrorArray2,
                                 ErrorArray2, ptbin, "NOERROR");
    ...
    auto hstderror =
        (TH1D*)MakeHistfromArray("StdError", ErrorArray2,
                                 ErrorArray2, ptbin, "NOERROR");
    for (int bin = 0; bin < ptbin.size(); bin++) {  // for eacbin
...
vector<TH1D*> Xi1530SysFitVarCheck(TString nameFitVar) {
    ...
    */
    ...
    if (savecheck)
        ratio_firvar->SaveAs(Form("./hRatio%s_(%2.f-%2.f).pdf",
                                  nameFitVar.Data(), multibin[0], multibin[1]));
    return fReturnHist;
}
TH1D* GetSpectra(double multi_start, double multi_end) {
    ...
    TH1D* hbase = (TH1D*)inputfile->Get("hXiSpectrum");
