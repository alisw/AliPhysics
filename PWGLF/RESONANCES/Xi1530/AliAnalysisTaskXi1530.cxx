...
AliAnalysisTaskXi1530RunTable::AliAnalysisTaskXi1530RunTable() : fCollisionType(kUnknownCollType)
{
    ;
}
...
AliAnalysisTaskXi1530::AliAnalysisTaskXi1530(
    ...
    : fOption(ap.fOption), goodtrackindices(ap.goodtrackindices), goodcascadeindices(ap.goodcascadeindices), fEMpool(ap.fEMpool)
{
}
//___________________________________________________________________
...
void AliAnalysisTaskXi1530::UserCreateOutputObjects()
{
    ...
    if (!IsMC)
    {
        if (IsAA && !IsHighMult)
            ...
            binCent = AxisVar("Cent", {0, 1, 5, 10, 15, 20, 30, 40, 50, 70, 100}); // for kINT7 study
        else
            binCent = AxisFix("Cent", 100, 0, 0.1); // for HM study.
    }
    else
        binCent = AxisFix("Cent", 1000, 0, 100); // for MC, including HM/Int7
...
void AliAnalysisTaskXi1530::UserExec(Option_t *)
{
    ...
    if (!IsMC)
    {
        if (!IsHighMult)
            ...
            IsSelectedTrig = (inputHandler->IsEventSelected() & AliVEvent::kHighMultV0);
    }
    else
        IsSelectedTrig = (inputHandler->IsEventSelected() & AliVEvent::kINT7);
    ...
    ftrackmult = AliESDtrackCuts::GetReferenceMultiplicity(((AliESDEvent *)fEvt), AliESDtrackCuts::kTracklets, 0.8); // tracklet in eta +_0.8
    ...
    Bool_t IsGoodVertex = spdVtx->GetStatus() && SelectVertex2015pp(((AliESDEvent *)fEvt), kTRUE // spdVertex->GetDispersion() < 0.04 cm
    ...
    Bool_t IsTrackletinEta1 =
        (AliESDtrackCuts::GetReferenceMultiplicity(((AliESDEvent *)fEvt), AliESDtrackCuts::kTracklets, 1.0) >= 1);
    ...
    if (IsMC && IsINEL0True)
        fHistos->FillTH1("hEventNumbers", "INELg0True", 1); // For trigger efficiency
    // -----------------------------------------------------------------------
...
Bool_t AliAnalysisTaskXi1530::GoodTracksSelection()
{
    ...
    for (UInt_t it = 0; it < ntracks; it++)
    {
        if (fEvt->IsA() == AliESDEvent::Class())
        {
            ...
            // Z vertex cut
            if (abs(track->GetZ() - fZ) > fXi1530PionZVertexCut)
                continue;
...
Bool_t AliAnalysisTaskXi1530::GoodCascadeSelection()
{
    ...
    for (UInt_t it = 0; it < ncascade; it++)
    {
        ...
        { // ESD case
            ...
            if (TMath::Abs(Xicandidate->GetPindex()) == TMath::Abs(Xicandidate->GetNindex()))
                continue;
            if (TMath::Abs(Xicandidate->GetPindex()) == TMath::Abs(Xicandidate->GetBindex()))
                continue;
            if (TMath::Abs(Xicandidate->GetNindex()) == TMath::Abs(Xicandidate->GetBindex()))
                continue;
            ...
            if (StandardXi)
            { // Save only the Xi is good candidate
...
void AliAnalysisTaskXi1530::FillTracks()
{
    ...
    for (UInt_t i = 0; i < ncascade; i++)
    {
        ...
        for (UInt_t j = 0; j < ntracks; j++)
        {
            ...
            if (track1->GetID() == pTrackXi->GetID() || track1->GetID() == nTrackXi->GetID() || track1->GetID() == bTrackXi->GetID())
                continue;
            ...
            auto sign = kAllType;
            if ((Xicandidate->Charge() == -1 && track1->Charge() == +1) || (Xicandidate->Charge() == +1 && track1->Charge() == -1))
            ...
            if (IsMC)
            {
                ...
                { // ESD case
                    ...
                    { // MC Association, if it comes from True Xi1530
...
void AliAnalysisTaskXi1530::FillMCinput(AliStack *fMCStack, Bool_t PS)
{
    ...
    for (Int_t it = 0; it < fMCStack->GetNprimary(); it++)
    {
        ...
        if (PS)
            FillTHnSparse("hInvMass", {(double)kMCTruePS, fCent, mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
        else
            FillTHnSparse("hInvMass", {(double)kMCTrue, fCent, mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
    }
}
void AliAnalysisTaskXi1530::FillMCinputdXi(AliStack *fMCStack, Bool_t PS)
{
    ...
    for (Int_t it = 0; it < fMCStack->GetNprimary(); it++)
    {
        ...
        if (!(abs(mcInputTrack->GetPdgCode()) == kXiCode))
            continue;
        if (PS)
            FillTHnSparse("hInvMass_dXi", {(double)kMCTruePS, fCent, mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
        else
            FillTHnSparse("hInvMass_dXi", {(double)kMCTrue, fCent, mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
    }
}
