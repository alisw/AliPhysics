...
//
Bool_t ConfigKStarPlusMinusRun2_mul
(
...
//kTPCpidphipp2015
{
    ...
    Int_t   cutID2  [6] = { iCutPi         ,iCutPi           ,iCutPi             ,iCutPi              ,iCutPi           ,iCutPi            };
     TString outputh [11]={"HIST","HIST","HIST","HIST","HIST","HIST","HIST","HIST","HIST","HIST" ,"HIST"}; //Suman
	...
	//out->AddAxis(SpherocityID,1000,0.,1.); //Suman
        if(isGT)  out->AddAxis(sdpt,100,0.,10.);
	...
	outps->AddAxis(centID,400,0.5,400.5);//ORIGINAL---->else outps->AddAxis(centID,400,0.5,400.5);//ST
	...
	outpsf->AddAxis(centID,400,0.5,400.5);//ORIGINAL----->outpsf->AddAxis(centID,400,0.5,400.5);
