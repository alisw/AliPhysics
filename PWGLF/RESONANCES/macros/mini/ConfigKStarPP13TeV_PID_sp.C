...
)
{
  ...
  TString outputh [12]=  {"HIST"    ,"HIST"     , "HIST"   ,"HIST"    ,"HIST"   ,"HIST"   ,"HIST"   ,"HIST"   ,"HIST"   ,"HIST"  ,"HIST"     ,"HIST"       };
  ...
  if (isMC){   
   ...
   if(useHIST)   AliRsnMiniOutput *outm = task->CreateOutput(Form("Ks_Mother%s", suffix), "HIST", "MOTHER");//AK
    ...
    if(!useHIST)  AliRsnMiniOutput *outreflex = task->CreateOutput(Form("Ks_reflex%s", suffix), "SPARSE", "TRUE");
    if(useHIST)  AliRsnMiniOutput *outreflex = task->CreateOutput(Form("Ks_reflex%s", suffix), "HIST", "TRUE");//AK
