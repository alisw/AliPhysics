...
AliRsnMiniAnalysisTask * AddAnalysisTaskTPCKStarTestNew
(
   Bool_t      isMC,
   Bool_t      isPP,
...
{  
  ...
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
   if (!mgr) {
      ::Error("AddAnalysisTaskTPCKStarTestNew", "No analysis manager to connect to.");
      return NULL;
   } 
   ...
   // -- CONFIG ANALYSIS --------------------------------------------------------------------------
   if(!isMC){
     gROOT->LoadMacro("$ALICE_PHYSICS/PWGLF/RESONANCES/macros/mini/ConfigTPCanalysisKStarTestNew.C");
     //gROOT->LoadMacro("ConfigTPCanalysisKStarTestNew.C");
     if (!ConfigTPCanalysisKStarTestNew(task, isMC, isPP, "", cutsPair, nsigmaPi, nsigmaKa, enableMonitor, isMC&IsMcTrueOnly,313,optSys.Data())) return 0x0;
   }
   else {
     gROOT->LoadMacro("$ALICE_PHYSICS/PWGLF/RESONANCES/macros/mini/ConfigTPCanalysisKStarTestNew.C");
     //gROOT->LoadMacro("ConfigTPCanalysisKStarTestNew.C");
     if (!ConfigTPCanalysisKStarTestNew(task, isMC, isPP, "Part", cutsPair, nsigmaPi, nsigmaKa, enableMonitor, isMC&IsMcTrueOnly, 313,optSys.Data())) return 0x0; //K*
     if (!ConfigTPCanalysisKStarTestNew(task, isMC, isPP, "AntiPart", cutsPair, nsigmaPi, nsigmaKa, enableMonitor, isMC&IsMcTrueOnly, -313,optSys.Data())) return 0x0; //anti-K* 
   }
   ...
   //  outputFileName += ":Rsn";
   Printf("AddAnalysisTaskTPCKStarTestNew - Set OutputFileName : \n %s\n", outputFileName.Data() );
