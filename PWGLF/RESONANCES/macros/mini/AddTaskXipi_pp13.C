/***************************************************************************
 ...
 Modified(Minimized) only for Xi+pi test
 ****************************************************************************/
...
Bool_t Config_Xipi(
    ...
    if(!cutSetPi){cerr<<"Error in AddTaskXipi_pp13::Config_Lambdapi(): missing cutSetPi"<<endl; return kFALSE;}
    ...
    AliRsnCutCascade* cutXi=new AliRsnCutCascade("cutXi",kXiMinus,AliPID::kProton,AliPID::kPion,AliPID::kPion);
    ...
    for(i=0;i<16;i++){
        ...
        if(comp.EqualTo("PAIR")) out->SetPairCuts(cutsPairSame);
        else out->SetPairCuts(cutsPairMix);
        
        // axis X: invmass or resolution
        if(!xID) out->AddAxis(imID,1050,1.45,2.5);
        else if(xID==1) out->AddAxis(mmID,1050,1.45,2.5);
    ...
    if(isMC){ //phase-space histograms
        //Xi(1530)0
        out=task->CreateOutput(Form("Xistar0p_mother_ps%s", suffix),"HIST","TRUE");
        out->SetDaughter(0,AliRsnDaughter::kXi);
        ...
        //anti-Xi(1530)0
        out=task->CreateOutput(Form("Xistar0a_mother_ps%s", suffix),"HIST","TRUE");
        out->SetDaughter(0,AliRsnDaughter::kXi);
...
AliRsnMiniAnalysisTask* AddTaskXipi_pp13(
                                         TString lname,
                                         Bool_t isMC,
                                         Int_t system,
    ...
    if(!mgr){
        ::Error("AddTaskXipi_pp13", "No analysis manager to connect to.");
        return NULL;
    }
    ...
    ::Info("AddTaskXipi_pp13", "%s", Form("Event mixing configuration: \n events to mix = %i \n max diff. vtxZ = cm %5.3f \n max diff multi = %5.3f", nmix, maxDiffVzMix, maxDiffMultMix));
    ...
    if(isPP && (!isMC) && cutVertex){
        ...
        ::Info("AddTaskXipi_pp13", "%s", Form(":::::::::::::::::: Pile-up rejection mode: %s", (rejectPileUp)?"ON":"OFF"));
    }
    ...
    if(d1==AliRsnDaughter::kXi && d2==AliRsnDaughter::kPion){
    ...
    Printf("AddTaskXipi_pp13 - Set OutputFileName : \n %s\n", outputFileName.Data() );
