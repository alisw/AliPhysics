...
//
Bool_t ConfigKStarPlusMinusMultiplicity
(
...
//kTPCpidphipp2015
{
    ...
    for (Int_t i = 0; i < 6; i++) {
        ...
        //if(isPP) out->AddAxis(centID, 400, 0.5, 400.5);//binning used for multiplicity
        //Binning in centrality not Multiplicity
        //Used also for PP collision in multiplicity study
    ...
    if (isMC) {
        ...
        //if(isPP) out->AddAxis(centID, 400, 0., 400.5);
        //if(isPP) out->AddAxis(centID, 100, 0., 100.);
        //else out->AddAxis(centID, 100, 0.0, 100.);
        if(isHiMult)out->AddAxis(centID, 100, 0.0, 1.);
        else out->AddAxis(centID, 400, 0.0, 100.);
