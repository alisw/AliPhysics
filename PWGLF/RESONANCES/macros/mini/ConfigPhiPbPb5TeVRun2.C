   ...
   )
{
  ...
  //------------Mass from PDG database--------------------
  Int_t pdgParticle = 333; //for phi meson
  TDatabasePDG *database = TDatabasePDG::Instance();
  TParticlePDG *particle = database->GetParticle(pdgParticle);
  Double_t pdgMass = particle->Mass();
  ...
  // --------------Claculate n-sgmacut for TPC and TOF ------------
  ...
  Int_t cosThSIDAbs;
  if(isMC == kTRUE)
    cosThSIDAbs  = task->CreateValue(cosThetaType , kTRUE);
  else
    cosThSIDAbs = task->CreateValue(cosThetaType ,kFALSE);
  ...
  TString name   [11]={"UnlikePM"   ,"MixingPM"   ,"LikePP"     ,"LikeMM"     ,"Trues" ,"TruesFine","TruesMM","TruesFineMM","Res"   ,"MixingPP","MixingMM"};
  ...
  for(Int_t i=0;i<9;i++){
    ...
    if(!isPP) out->AddAxis(centID,101,0.,101.);
    ...
    // axis 3: CosThetaStarAbs
    if (!isPP)
      out->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);
    else 
      out->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);
  ...
  if(isMC){   
    ...
    outm->SetMotherMass(pdgMass);
    outm->SetPairCuts(cutsPair);

    //axis 0 - invariant mass
    outm->AddAxis(imID,215,0.985,1.2);
    //axis 1 - phi pt
    outm->AddAxis(ptID,200,0.,20.);
    //axis 2 - cent for PbPb
    if(!isPP) outm->AddAxis(centID,101,0.,101.);
    else outm->AddAxis(centID,161,-0.5,160.5);
    //axis 3 - Costheta
    if (!isPP)
      outm->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);
    else 
      outm->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);
    //axis 4 - eta
    outm->AddAxis(etaID, 20, -1.0, 1.0);
    ...
    if(!isPP) outmf->AddAxis(centID,101,0.,101.);
    else outmf->AddAxis(centID,161,-0.5,160.5);

    if (!isPP)
      outmf->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);
    else 
      outmf->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);

    outmf->AddAxis(etaID, 20, -1.0, 1.0);
    ...
    if (!isPP)
      outps->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);
    else 
      outps->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);
    ...
    if (!isPP)
      outpsf->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);
    else 
      outpsf->AddAxis(cosThSIDAbs, 10, 0.0, 1.0);

    outpsf->AddAxis(etaID, 20, -1.0, 1.0);
