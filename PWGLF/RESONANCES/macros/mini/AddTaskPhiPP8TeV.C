...
AliRsnMiniAnalysisTask * AddTaskPhiPP8TeV
(
 Bool_t      isMC=kFALSE,
 Bool_t      isPP=kTRUE,
...
{  
  ...
  if(!mgr){
    ::Error("AddTaskPhiPP8TeV", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  ::Info("AddTaskPhiPP8TeV", Form("Event mixing configuration: \n events to mix = %i \n max diff. vtxZ = cm %5.3f \n max diff multi = %5.3f", nmix, maxDiffVzMix, maxDiffMultMix));
  ...
  if(isPP && (!isMC) && cutVertex){ 
    ...
    ::Info("AddTaskPhiPP8TeV", Form(":::::::::::::::::: Pile-up rejection mode: %s", (rejectPileUp)?"ON":"OFF"));
  }
  ...
  //  outputFileName += ":Rsn";
  Printf("AddAnalysisTaskPhiPP8TeV - Set OutputFileName : \n %s\n",outputFileName.Data());
