/*=================================================================================
  ...
  *** Configuration script for K*+-->K0Short-Pi analysis ***
  Modified for Spherocity analysis by Suman Deb (suman.deb@cern.ch) 0n 03-12-2019
  Modified for Spherocity analysis by Suman Deb (AOD format) (suman.deb@cern.ch) 0n 10-11-2021
  =======================================================================================*/
...
Bool_t ConfigKStarPlusMinusRun2sp_bin_new_AOD
(
...

{
  ...
  for (Int_t i = 0; i < 6; i++) {
    ...
    if(!isPP) {
      out->AddAxis(centID, 100, 0.0, 100.0);//change-SD-[03-12-2019]
    } else {
      out->AddAxis(centID, mult_binning, mult_min, mult_max);
  ...
  if (isMC) {
    ...
    TString mode = "SPARSE";//==============SD=====10-NOV-21
    //TString mode = "HIST";
    //if (collSyst) mode = "SPARSE";

    // create output

    AliRsnMiniOutput *out = task->CreateOutput(Form("KStarPlusMinus_MotherMC%s", suffix), mode.Data(), "MOTHER");//=====ORIGINAL
    ...
    if(!isPP){
      ...
      out->AddAxis(centID, mult_binning, mult_min, mult_max);//out->AddAxis(centID, 100, 0.0, 100.);
    }
    ...
    AliRsnMiniOutput *out_SD = task->CreateOutput(Form("AKStarPlusMinus_MotherMC%s", suffix), mode.Data(), "MOTHER");//======ORIGINAL
 
    
    // selection settings
    out_SD->SetDaughter(0, AliRsnDaughter::kKaon0);
    out_SD->SetDaughter(1, AliRsnDaughter::kPion);
    out_SD->SetMotherPDG(-323);
    out_SD->SetMotherMass(0.89166);
    
    // pair cuts
    
    out_SD->SetPairCuts(PairCutsSame);
    
    // binnings
    
    out_SD->AddAxis(imID, 90, 0.6, 1.5);
    out_SD->AddAxis(ptID, 300, 0.0, 30.0);
    //out->AddAxis(k0sDCA, 10, 0.0, 1.0);
    //if (collSyst) out->AddAxis(centID, 10, 0.0, 100.0);

    // if(isPP) out->AddAxis(centID, 400, 0.5, 400.5);
    if(!isPP) {
      out_SD->AddAxis(centID, 100, 0.0, 100.0);//change-SD-[03-12-2019]
    }  else {
      out_SD->AddAxis(centID, mult_binning, mult_min, mult_max);//out->AddAxis(centID, 100, 0.0, 100.);
    }
    
    out_SD->AddAxis(SpherocityID, binning_sphero,min_sphero, max_sphero);//out->AddAxis(SpherocityID,1000,0.,1.); //ST
    if(isGT)  out->AddAxis(sdpt,100,0.,10.);
    ...
    //========added by SD========================================================================================
    if(!isPP) {
      outps->AddAxis(centID, 100, 0.0, 100.0);//change-SD-[03-12-2019]
    }  else {
      outps->AddAxis(centID, mult_binning, mult_min, mult_max);//out->AddAxis(centID, 100, 0.0, 100.);
    }
    ...
    //========added by SD=========================================================================================
    if(!isPP) {
      outpsf->AddAxis(centID, 100, 0.0, 100.0);//change-SD-[03-12-2019]
    }  else {
      outpsf->AddAxis(centID, mult_binning, mult_min, mult_max);//out->AddAxis(centID, 100, 0.0, 100.);
    }

    outpsf->AddAxis(SpherocityID, binning_sphero,min_sphero, max_sphero);//out->AddAxis(SpherocityID,1000,0.,1.); //ST
...
Bool_t SetCustomQualityCut(AliRsnCutTrackQuality * trkQualityCut, Int_t customQualityCutsID = 0, Int_t customFilterBit = 0)
{
  ...
  if(customQualityCutsID>=1 && customQualityCutsID<100 && customQualityCutsID!=2){
    ...
    //Printf(Form("::::: SetCustomQualityCut:: using TPC-only track quality cuts"));//=====>>>ORIGINAL
    Printf("::::: SetCustomQualityCut:: using TPC-only track quality cuts");//=====>>>CHANGED ON 10-NOV-21
  ...
  // Printf(Form("::::: SetCustomQualityCut:: using custom track quality cuts #%i",customQualityCutsID));//====ORIGINAL
  Printf("::::: SetCustomQualityCut:: using custom track quality cuts #%i",customQualityCutsID);//CHANGED ON 10-NOV-21
