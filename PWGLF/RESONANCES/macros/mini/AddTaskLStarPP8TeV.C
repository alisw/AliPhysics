...
AliRsnMiniAnalysisTask * AddTaskLStarPP8TeV
(
 Bool_t      isMC           = kFALSE,
 Bool_t      isPP           = kTRUE,
 Int_t       trCut         = 2011,
...
{  
  ...
  // cutY->SetRangeD(-0.9, 0.9);
   ...
   // -- CONFIG ANALYSIS --------------------------------------------------------------------------
   //
    gROOT->LoadMacro("$ALICE_PHYSICS/PWGLF/RESONANCES/macros/mini/ConfigLStarPP8TeV.C");
   //   gROOT->LoadMacro("ConfigLStarPP8TeV.C");
   if (!ConfigLStarPP8TeV(task, isMC, isPP, "", cutsPair, trCut, customQualityCutsID,cutKaCandidate,cutPrCandidate, nsigmaP,nsigmaK,nsigmatofP,nsigmatofK, enableMonitor)) return 0x0;
