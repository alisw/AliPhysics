...
if(isPP && (!isMC) && cutVertex){ 
  ...
  ::Info("AddTaskKStarPlusMinusRun2sp_bin", Form(":::::::::::::::::: Pile-up rejection mode: %s", (rejectPileUp)?"ON":"OFF"));
 }
...
TH2F* hsp=new TH2F("hSpherocityVsCent","",110,0.,110., 500.,0.,1.0);//change-SD-[03-12-2019]
    ...
    //
    // -- CONFIG ANALYSIS --------------------------------------------------------------------------
    gROOT->LoadMacro("$ALICE_PHYSICS/PWGLF/RESONANCES/macros/mini/ConfigKStarPlusMinusRun2sp_bin_new.C");
...
if(!useHIST)//change-SD-[03-12-2019]
      {
