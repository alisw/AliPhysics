...
void AddMonitorOutput_KPt(TObjArray *mon=0,TString opt="",AliRsnLoopDaughter *lpt=0)
{

  // Pt                                                                                                                                     
  AliRsnValueDaughter *axisKPt = new AliRsnValueDaughter("kaon_transversemomentum", AliRsnValueDaughter::kPt);
  axisKPt->SetBins(0.,30.0, 0.1);

  // output: 2D histogram                                                                                                                   
  AliRsnListOutput *outMonitorKTrMom = new AliRsnListOutput("Kaon_TransverseMomentum", AliRsnListOutput::kHistoDefault);
  outMonitorKTrMom->AddValue(axisKPt);

  // add outputs to loop                                                                                                                    
  if (mon) mon->Add(outMonitorKTrMom);
  if (lpt) lpt->AddOutput(outMonitorKTrMom);

}

void AddMonitorOutput_PiPt(TObjArray *mon=0,TString opt="",AliRsnLoopDaughter *lpt=0)
{

  // Pt                                                                                                                                     
  AliRsnValueDaughter *axisPiPt = new AliRsnValueDaughter("pion_transversemomentum", AliRsnValueDaughter::kPt);
  axisPiPt->SetBins(0.,30.0, 0.1);

  // output: 2D histogram                                                                                                                   
  AliRsnListOutput *outMonitorPiTrMom = new AliRsnListOutput("Pion_TransverseMomentum", AliRsnListOutput::kHistoDefault);
  outMonitorPiTrMom->AddValue(axisPiPt);

  // add outputs to loop                                                                                                                    
  if (mon) mon->Add(outMonitorPiTrMom);
  if (lpt) lpt->AddOutput(outMonitorPiTrMom);
    ...
    */
    ...
    if (isMC) {
      ...
      */
      ...
      out = task->CreateOutput("Kposgenerated", "HIST", "SINGLE");
      out->SetDaughter(0, AliRsnDaughter::kUnknown);
      ...
      out = task->CreateOutput("Pionposreconstructed", "HIST", "SINGLEREC");
      out->SetDaughter(0, AliRsnDaughter::kUnknown);
      //out->SetDaughter(1, AliRsnDaughter::kUnknown);
      out->SetMotherPDG(211);
      out->SetMotherMass(0.139);
