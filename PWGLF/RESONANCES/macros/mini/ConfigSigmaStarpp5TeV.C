...
Bool_t ConfigSigmaStarpp5TeV (AliRsnMiniAnalysisTask *task, 
		Bool_t                 isMC, 
		...
		Int_t                  pidCUT=4,             //pion PID cut set, 1 for nominal, 2 for systematic check
...
//  RSNPID  d2 = AliRsnDaughter::kLambda;
  ...
  if (pidCUT==2) {
    ...
    if (pidCUT==4) { //pp @5 TeV
    //for setting PID cuts in given pT ranges
      iCutTPCNSigma->SinglePIDRange(nsigma);
