...
#include <PWGLF/RESONANCES/macros/mini/ConfigureLstar13TeVpp_AOD.C>
#endif
/***************************************************************************
...
AliRsnMiniAnalysisTask * AddTaskLstar13TeVpp_AOD
(
 Bool_t      isMC=kFALSE,
 Bool_t      isPP=kTRUE,
...
{  
  ...
  if(!mgr){
    ::Error("AddTaskLstar13TeVpp_AOD", "No analysis manager to connect to.");
    return NULL;
  }
  ...
  ::Info("AddTaskLstar13TeVpp_AOD","%s", Form("Event mixing configuration: \n events to mix = %i \n max diff. vtxZ = cm %5.3f \n max diff multi = %5.3f", nmix, maxDiffVzMix, maxDiffMultMix));
  ...
  if(isPP && (!isMC) && cutVertex){
    ...
    ::Info("AddTaskLstar13TeVpp_AOD","%s", Form(":::::::::::::::::: Pile-up rejection mode: %s", (rejectPileUp)?"ON":"OFF"));
  }
  ...
  // -- CONFIG ANALYSIS --------------------------------------------------------------------------
#if !defined (__CINT__) || defined (__CLING__)
  if (!ConfigureLstar13TeVpp_AOD(task, isMC, isPP, "", cutsPair, aodFilterBit, customQualityCutsID, cutPrCandidate, cutKaCandidate, nsigmaPr, nsigmaKa,nsigmaTOFPr, nsigmaTOFKa,  enableMonitor,triggerMask)) return 0x0;
#else
  gROOT->LoadMacro("$ALICE_PHYSICS/PWGLF/RESONANCES/macros/mini/ConfigureLstar13TeVpp_AOD.C");
  if (!ConfigureLstar13TeVpp_AOD(task, isMC, isPP, "", cutsPair, aodFilterBit, customQualityCutsID, cutPrCandidate, cutKaCandidate, nsigmaPr, nsigmaKa,nsigmaTOFPr, nsigmaTOFKa,  enableMonitor,triggerMask)) return 0x0;
