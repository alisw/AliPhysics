...
ClassImp(AliAnalysisTaskMultspec)

AliAnalysisTaskMultspec::AliAnalysisTaskMultspec() : AliAnalysisTaskSE(),
...
AliAnalysisTaskMultspec::AliAnalysisTaskMultspec(const char *name, int particle, TString lExtraOptions, bool ismc, bool removePythiaGen) : AliAnalysisTaskSE(name),
fHistos_misc(nullptr), 
fTree(nullptr), 
fpart(particle),
fisMC(ismc),
fRemovePythiaGen(removePythiaGen),
fPIDResponse(0), 
//AliEventCuts object
fEventCuts(0),
//pile-up rejection flag
...
AliAnalysisTaskMultspec::~AliAnalysisTaskMultspec()
{
    //------------------------------------------------
    // DESTRUCTOR
    //------------------------------------------------
...
void AliAnalysisTaskMultspec::UserCreateOutputObjects()
{
  ...
  //MC-related TH
  if(fisMC) {
      fHistos_misc->CreateTH1("hPtGen", "", 400, -20, 20, "s");  //histogram PtGen_distrib
      ...
      fHistos_misc->CreateTH2("hR0genVSpT", "", 400, -20, 20, 100, 0, 100, "s");  //histogram Gen_distrib for zero-Rec events for given part VS pT
      fHistos_misc->CreateTH2("hR0genBothVSpT", "", 400, -20, 20, 100, 0, 100, "s");  //same as before, but for particle and anti-particle (for cascades)
...
//________________________________________________________________________
void AliAnalysisTaskMultspec::UserExec(Option_t *)
{
  ...
  if (!MultSelection) { 
    ...
    if(fpart<kxi_Mult) ffillV0->Mult = lMultiplicity;
  ...
  if(fpart<kxi_Mult){
    ...
    if(fisMC){
      for (int i_MCtrk = 0;  i_MCtrk < lMCev->GetNumberOfTracks(); i_MCtrk++){
        ...
        if(fRemovePythiaGen){
         genindex = lPart->GetGeneratorIndex();  
         if(genindex==0) continue;  
        }
        
        if( fpart==kk0s_Mult && lPart->PdgCode()==310   ) {
         ngen++;
         fHistos_misc->FillTH1("hPtGen", lPart->Pt());
         hPt_Gen_Ev->Fill(lPart->Pt());
        }
        if( fpart==klam_Mult && lPart->PdgCode()==3122  ) {
         ngen++;
         fHistos_misc->FillTH1("hPtGen", lPart->Pt());
         hPt_Gen_Ev->Fill(lPart->Pt());
        }
        if( fpart==kalam_Mult && lPart->PdgCode()==-3122) {
         ngen++;
         fHistos_misc->FillTH1("hPtGen", lPart->Pt());
         hPt_Gen_Ev->Fill(lPart->Pt());
        }
    ...
    for (int iV0 = 0; iV0 < nv0s; iV0++) {
      ...
      if(fRemovePythiaGen){
       if(fisMC){
        indexRecP = ((AliMCParticle*) lMCev->GetTrack((int)TMath::Abs(pTrack->GetLabel())))->GetGeneratorIndex();
        indexRecN = ((AliMCParticle*) lMCev->GetTrack((int)TMath::Abs(nTrack->GetLabel())))->GetGeneratorIndex();
      
        if(indexRecP==0 || indexRecN==0) continue;
       }
      }
      ...
      if(fisMC) {
        ...
        if((labMothPosDaught==labMothNegDaught) && (pdgV0==PDGcodes_Mult[fpart])) ffillV0->IsPDGmatched = kTRUE;
         else ffillV0->IsPDGmatched = kFALSE;
        
        //MC-related PhysicalPrimary matching flag
        if((labMothPosDaught==labMothNegDaught) && (pdgV0==PDGcodes_Mult[fpart]) && ((AliMCParticle*) lMCev->GetTrack((int)TMath::Abs(labMothPosDaught)))->IsPhysicalPrimary()) ffillV0->PhysicalPrimary = kTRUE;
        else ffillV0->PhysicalPrimary = kFALSE;
      }
    ...
    if(!isZeroV0) {
        ...
        if(fisMC) {
         fHistos_misc->FillTH2("hR0gen", lMultiplicity, ngen);
         for (int ibinx = hPt_Gen_Ev->FindBin(0.05); ibinx < hPt_Gen_Ev->GetNbinsX();ibinx++) fHistos_misc->FillTH2("hR0genVSpT", hPt_Gen_Ev->GetBinCenter(ibinx+1), ngen,hPt_Gen_Ev->GetBinContent(ibinx+1));
        }
    }    
    ...
    if(fisMC){
      for (int i_MCtrk = 0;  i_MCtrk < lMCev->GetNumberOfTracks(); i_MCtrk++){
        ...
        if( fpart==kxi_Mult && lPart->PdgCode()==-3312  ) {
         ngenP++;
         fHistos_misc->FillTH1("hPtGen", lPart->Pt());
         hPt_Gen_Ev->Fill(lPart->Pt());
        }
        ...
        if( fpart==kom_Mult && lPart->PdgCode()==-3334  ){
         ngenP++;
         fHistos_misc->FillTH1("hPtGen", lPart->Pt());
         hPt_Gen_Ev->Fill(lPart->Pt());
        }
    ...
    for (int i_casc = 0; i_casc < ncasc; i_casc++) {
      ...
      if(fRemovePythiaGen){
       if(fisMC){
        indexRecP = ((AliMCParticle*) lMCev->GetTrack((int)TMath::Abs(pTrackCasc->GetLabel())))->GetGeneratorIndex();
        indexRecN = ((AliMCParticle*) lMCev->GetTrack((int)TMath::Abs(nTrackCasc->GetLabel())))->GetGeneratorIndex();
        indexRecB = ((AliMCParticle*) lMCev->GetTrack((int)TMath::Abs(bTrackCasc->GetLabel())))->GetGeneratorIndex();
       
        if(indexRecP==0 || indexRecN==0 ||indexRecB==0) continue;
       }
      }
      ...
      if(fisMC) {
        ...
        //MC-related PDG matching flag
        if((labMothPosDaught!=labMothNegDaught) || (pdgV0!=PDGcodes_Mult[(casc->ChargeXi()<0 ? klam_Mult : kalam_Mult)])) ffillCasc->IsPDGmatched = kFALSE;
        else if((mothV0!=mothBac) || (pdgCasc!=(casc->ChargeXi()<0 ? PDGcodes_Mult[fpart] : -PDGcodes_Mult[fpart]))) ffillCasc->IsPDGmatched = kFALSE;
        else ffillCasc->IsPDGmatched = kTRUE;
        
        //MC-related PhysicalPrimary matching flag
        if((labMothPosDaught!=labMothNegDaught) || !(((AliMCParticle*) lMCev->GetTrack(labelCasc))->IsPhysicalPrimary()) || (pdgV0!=PDGcodes_Mult[(casc->ChargeXi()<0 ? klam_Mult : kalam_Mult)])) ffillCasc->PhysicalPrimary = kFALSE;
        else if((mothV0!=mothBac) || !(((AliMCParticle*) lMCev->GetTrack(labelCasc))->IsPhysicalPrimary()) || (pdgCasc!=(casc->ChargeXi()<0 ? PDGcodes_Mult[fpart] : -PDGcodes_Mult[fpart]))) ffillCasc->PhysicalPrimary = kFALSE;
        else ffillCasc->PhysicalPrimary = kTRUE;
      }
    ...
    if(isZeroCascM) {
      ...
      if(fisMC){
       fHistos_misc->FillTH2("hR0gen", -lMultiplicity-1e-6, ngenM);
       for (int ibinx = 0; ibinx < hPt_Gen_Ev->FindBin(0.05);ibinx++) fHistos_misc->FillTH2("hR0genVSpT", hPt_Gen_Ev->GetBinCenter(ibinx+1), ngenM ,hPt_Gen_Ev->GetBinContent(ibinx+1));
      }
    }
    if(isZeroCascP) {
      fHistos_misc->FillTH1("hZeros",lMultiplicity+1e-6);
      if(fisMC){
        fHistos_misc->FillTH2("hR0gen", lMultiplicity+1e-6, ngenP);
        for (int ibinx = hPt_Gen_Ev->FindBin(0.05); ibinx < hPt_Gen_Ev->GetNbinsX();ibinx++) fHistos_misc->FillTH2("hR0genVSpT", hPt_Gen_Ev->GetBinCenter(ibinx+1), ngenP ,hPt_Gen_Ev->GetBinContent(ibinx+1));
    ...
    if(isZeroBoth) {
      ...
      if(fisMC){
        ...
        for (int ibinx = 0; ibinx < hPt_Gen_Ev->FindBin(0.05);ibinx++) fHistos_misc->FillTH2("hR0genBothVSpT", hPt_Gen_Ev->GetBinCenter(ibinx+1), ngenM ,hPt_Gen_Ev->GetBinContent(ibinx+1));
        for (int ibinx = hPt_Gen_Ev->FindBin(0.05); ibinx < hPt_Gen_Ev->GetNbinsX();ibinx++) fHistos_misc->FillTH2("hR0genBothVSpT", hPt_Gen_Ev->GetBinCenter(ibinx+1), ngenP ,hPt_Gen_Ev->GetBinContent(ibinx+1));
      }
    }
...
//________________________________________________________________________
void AliAnalysisTaskMultspec::Terminate(Option_t *)
{

}

//________________________________________________________________________
void AliAnalysisTaskMultspec::DataPosting() {
