...
#include "AliAnalysisTaskStrVsMult_SelectionStudies.h"

#include "TMath.h"

ClassImp(AliAnalysisTaskStrVsMult_SelectionStudies)

AliAnalysisTaskStrVsMult_SelectionStudies::AliAnalysisTaskStrVsMult_SelectionStudies() : AliAnalysisTaskSE(),
//outputs
fHistos_eve(nullptr),
fHistos_K0S(nullptr),
fHistos_Lam(nullptr),
...
AliAnalysisTaskStrVsMult_SelectionStudies::AliAnalysisTaskStrVsMult_SelectionStudies(const char *name, TString lExtraOptions) : AliAnalysisTaskSE(name),
//outputs
fHistos_eve(nullptr),
fHistos_K0S(nullptr),
fHistos_Lam(nullptr),
...
AliAnalysisTaskStrVsMult_SelectionStudies::~AliAnalysisTaskStrVsMult_SelectionStudies()
{
    //------------------------------------------------
    // DESTRUCTOR
    //------------------------------------------------
...
void AliAnalysisTaskStrVsMult_SelectionStudies::UserCreateOutputObjects()
{
  ...
  if (fParticleAnalysisStatus[kxip]) {
    ...
    for (int icut=0; icut<kCasccutsnum+1; icut++) {
      if (icut==kCasc_y || icut==kCasc_etaDaugh || icut==kCasc_PropLifetOm || icut== kCasc_DcaBarToPV) continue;
      fHistos_XiMin->CreateTH3(Form("h3_ptmasscent_sel[%d]", icut), "", fnptbins[kXi], fptbinning[kXi], fnmassbins[kXi], fmassbinning[kXi], fncentbins[kXi], fcentbinning[kXi]);
      fHistos_XiPlu->CreateTH3(Form("h3_ptmasscent_sel[%d]", icut), "", fnptbins[kXi], fptbinning[kXi], fnmassbins[kXi], fmassbinning[kXi], fncentbins[kXi], fcentbinning[kXi]);
    }
  }
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::UserExec(Option_t *)
{
  ...
  if (fParticleAnalysisStatus[kk0s] || fParticleAnalysisStatus[klam]) {
    ...
    for (Int_t iV0 = 0; iV0 < nv0s; iV0++) {
      ...
      if (fParticleAnalysisStatus[kk0s]) {
        if ( physprim && assFlag[kk0s] && ApplyCuts(kk0s, lPercentile)) fHistos_K0S->FillTH3("h3_ptmasscent_def", fV0_Pt, fV0_InvMassK0s, lPercentile);
      }
      if (fParticleAnalysisStatus[klam]) {
        if ( physprim && assFlag[klam] && ApplyCuts(klam, lPercentile)) fHistos_Lam->FillTH3("h3_ptmasscent_def", fV0_Pt, fV0_InvMassLam, lPercentile);
        if ( physprim && assFlag[kalam] && ApplyCuts(kalam, lPercentile)) fHistos_ALam->FillTH3("h3_ptmasscent_def", fV0_Pt, fV0_InvMassALam, lPercentile);
        if(fisMC){
          //Feeddown matrix filling
          //numerator
          if ( fdmtx_ptxi<0  && ApplyCuts(klam, lPercentile)) fHistos_Lam->FillTH3("h3_FDmtxNUM_def", fV0_Pt, TMath::Abs(fdmtx_ptxi), lPercentile);
          else if( fdmtx_ptxi>0 && ApplyCuts(kalam, lPercentile)) fHistos_ALam->FillTH3("h3_FDmtxNUM_def", fV0_Pt, TMath::Abs(fdmtx_ptxi), lPercentile);
          //denominator
          if ( assFlag[klam] && ApplyCuts(klam, lPercentile)) fHistos_Lam->FillTH2("h2_FDmtxDEN_def", fV0_Pt, lPercentile);
          if ( assFlag[kalam] && ApplyCuts(kalam, lPercentile)) fHistos_ALam->FillTH2("h2_FDmtxDEN_def", fV0_Pt, lPercentile);
        }
      }
  ...
  if (fParticleAnalysisStatus[kxip] || fParticleAnalysisStatus[komp]) {
    ...
    for (int i_casc = 0; i_casc < ncasc; i_casc++) {
      ...
      if (fParticleAnalysisStatus[kxip]) {
        if( physprim && assFlag[kxim] && ApplyCuts(kxim, lPercentile)) fHistos_XiMin->FillTH3("h3_ptmasscent_def", fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
        if( physprim && assFlag[kxip] && ApplyCuts(kxip, lPercentile)) fHistos_XiPlu->FillTH3("h3_ptmasscent_def", fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
      }
      if (fParticleAnalysisStatus[komp]) {
        if( physprim && assFlag[komm] && ApplyCuts(komm, lPercentile)) fHistos_OmMin->FillTH3("h3_ptmasscent_def", fCasc_Pt, fCasc_InvMassOmMin, lPercentile);
        if( physprim && assFlag[komp] && ApplyCuts(komp, lPercentile)) fHistos_OmPlu->FillTH3("h3_ptmasscent_def", fCasc_Pt, fCasc_InvMassOmPlu, lPercentile);
      }
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::Terminate(Option_t *)
{

}

//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::SetCutVal(bool defchange, bool iscasc, int cutnum, double cval) {
  if (!iscasc) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::SetParametricBacBarCosPA(int nbins, float *ptbins, float *values, int cent_limit) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::SetDefCutVals() {
  //V0 part
  if (fParticleAnalysisStatus[kk0s] || fParticleAnalysisStatus[klam]) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::SetCutVariation(bool iscasc, int cutnum, int nvar, double lowval, double highval) {
  if (!iscasc) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::SetDefCutVariations() {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::SetDefOnly(bool isdefonly) {
  fDefOnly = isdefonly;
}

//________________________________________________________________________
bool AliAnalysisTaskStrVsMult_SelectionStudies::ApplyCuts(int part, double lPercentile) {
  ...
  if (part<kxip) { //we are checking V0s
    ...
    // 21) only very basic selections
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasccutsnum), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasccutsnum), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);

    // Start filling histograms for progressive selections

    // 0) check candidate daughter's DCA between them (needs to be small because they have to come from the same secondary vertex)
    if(fCasc_DcaCascDaught>cutval_Casc[kCasc_DcaCascDaught]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaCascDaught), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaCascDaught), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 1) check the cosine of the Pointing Angle for cascade (angle between candidate's momentum and vector connecting Primary and secondary vertices)
    if(fCasc_CascCosPA<cutval_Casc[kCasc_CascCosPA]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_CascCosPA), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_CascCosPA), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 2) check candidate's 2D decay distance from PV (if it is too small, then it's not a weak decay)
    if(fCasc_CascRad<cutval_Casc[kCasc_CascRad]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_CascRad), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_CascRad), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 3) check PID for all daughters (particle hypothesis' dependent)
    ...
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_NSigPID), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_NSigPID), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 4) check candidate daughters' crossed TPC raws (note that the checked value is the lowest among the daughters)
    if(fCasc_LeastCRaws<cutval_Casc[kCasc_LeastCRaws]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_LeastCRaws), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_LeastCRaws), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 5) check candidate daughters' crossed TPC raws over findable
    if(fCasc_LeastCRawsOvF<cutval_Casc[kCasc_LeastCRawsOvF]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_LeastCRawsOvF), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_LeastCRawsOvF), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 6) check candidate daughters' TPC clusters
    if(fCasc_LeastTPCcls<cutval_Casc[kCasc_LeastTPCcls]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_LeastTPCcls), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_LeastTPCcls), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 7) check candidate V0 daughter's mass difference from nominal Lambda mass
    if(TMath::Abs(fCasc_InvMassLam-1.115683)>cutval_Casc[kCasc_InvMassLam]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_InvMassLam), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_InvMassLam), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 8) check V0 daughter's daughters DCA between them (needs to be small because they have to come from the same secondary vertex)
    if(fCasc_DcaV0Daught>cutval_Casc[kCasc_DcaV0Daught]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaV0Daught), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaV0Daught), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 9) check the cosine of the Pointing Angle for V0 (angle between candidate's momentum and vector connecting Primary and secondary vertices)
    if(fCasc_V0CosPA<cutval_Casc[kCasc_V0CosPA]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_V0CosPA), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_V0CosPA), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 10) check candidate daughters' DCA to Primary Vertex (needs to be large because decay is far from the Primary Vertex)
    if(fCasc_DcaV0ToPV<cutval_Casc[kCasc_DcaV0ToPV]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaV0ToPV), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaV0ToPV), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 11)
    if(fCasc_DcaBachToPV<cutval_Casc[kCasc_DcaBachToPV]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaBachToPV), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaBachToPV), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 12) check if at least one of candidate's daughter has a hit in the TOF or has ITSrefit flag (removes Out Of Bunch Pileup)
    if (fCasc_ITSTOFtracks<cutval_Casc[kCasc_ITSTOFtracks]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_ITSTOFtracks), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_ITSTOFtracks), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 15) check candidate's proper lifetime (particle hypothesis' dependent). Remember: c*tau = L*m/p
    ...
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_PropLifetXi), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_PropLifetXi), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 17) check candidate V0 daughter's 2D decay distance from PV (if it is too small, then it's not a weak decay)
    if(fCasc_V0Rad<cutval_Casc[kCasc_V0Rad]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_V0Rad), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_V0Rad), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 18) check V0 daughters' DCA to Primary Vertex. Different cut for meson and baryon daughters, so different conditions for + and - candidates
    ...
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaMesToPV), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_DcaMesToPV), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
    // 20) check DCA bachelor-baryon. If it is too small --> bump structure in Inv Mass
    if(fCasc_BacBarCosPA>cutval_Casc[kCasc_BacBarCosPA]) return kFALSE;
    if (part==kxim) fHistos_XiMin->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_BacBarCosPA), fCasc_Pt, fCasc_InvMassXiMin, lPercentile);
    if (part==kxip) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent_sel[%d]", kCasc_BacBarCosPA), fCasc_Pt, fCasc_InvMassXiPlu, lPercentile);
...
void AliAnalysisTaskStrVsMult_SelectionStudies::SetParticleAnalysisStatus(bool k0s, bool lambda, bool xi, bool omega) {
...
bool AliAnalysisTaskStrVsMult_SelectionStudies::GetParticleAnalysisStatus(int part) {
  if (part<0 || part>=ksignednumpart) {
    ::Error("AliAnalysisTaskStrVsMult_SelectionStudies::GetParticleAnalysisStatus", "Wrong particle selected: accepted values from 0 to 3");
    return false;
  }
  return fParticleAnalysisStatus[part];
}

void AliAnalysisTaskStrVsMult_SelectionStudies::SetCentbinning(int ipart, int numcentbins, double *centbins) {
...
void AliAnalysisTaskStrVsMult_SelectionStudies::SetPtbinning(int ipart, int numptbins, double *ptbins) {
...
void AliAnalysisTaskStrVsMult_SelectionStudies::SetMassbinning(int ipart, int nummassbins, double valminmass, double valmaxmass) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::DataPosting() {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_SelectionStudies::FillHistCutVariations(bool iscasc, double perc, bool phypri, bool *associFlag, double ptassxi) {
      ...
      for(int i_var=0; i_var<nvarcut_V0[i_cut]; i_var++){
        ...
        if (i_cut!=kV0_PropLifetLam) {
          if (fParticleAnalysisStatus[kk0s]) {
            ...
            if (phypri && associFlag[kk0s] && ApplyCuts(kk0s, perc)) fHistos_K0S->FillTH3(Form("h3_ptmasscent[%d][%d]", i_cut, i_var), fV0_Pt, fV0_InvMassK0s, perc);
          }
        }
        ...
        if (i_cut!=kV0_PropLifetK0s) {
          if (fParticleAnalysisStatus[klam]) {
            SetCutVal(kFALSE, kFALSE, i_cut, varlowcut_V0[i_cut]+i_var*(varhighcut_V0[i_cut]-varlowcut_V0[i_cut])/(nvarcut_V0[i_cut]-1));
            if (phypri && associFlag[klam] && ApplyCuts(klam, perc)) fHistos_Lam->FillTH3(Form("h3_ptmasscent[%d][%d]", i_cut, i_var), fV0_Pt, fV0_InvMassLam, perc);
            if (phypri && associFlag[kalam] && ApplyCuts(kalam, perc)) fHistos_ALam->FillTH3(Form("h3_ptmasscent[%d][%d]", i_cut, i_var), fV0_Pt, fV0_InvMassALam, perc);
            if (fisMC) {
              //Feeddown matrix filling
              //numerator
              if (ptassxi<0 && ApplyCuts(klam, perc)) fHistos_Lam->FillTH3(Form("h3_FDmtxNUM[%d][%d]", i_cut, i_var), fV0_Pt, TMath::Abs(ptassxi), perc);
              else if (ptassxi>0 && ApplyCuts(kalam, perc)) fHistos_ALam->FillTH3(Form("h3_FDmtxNUM[%d][%d]", i_cut, i_var), fV0_Pt, TMath::Abs(ptassxi), perc);
              //denominator
              if (associFlag[klam] && ApplyCuts(klam, perc)) fHistos_Lam->FillTH2(Form("h2_FDmtxDEN[%d][%d]", i_cut, i_var), fV0_Pt, perc);
              if (associFlag[kalam] && ApplyCuts(kalam, perc)) fHistos_ALam->FillTH2(Form("h2_FDmtxDEN[%d][%d]", i_cut, i_var), fV0_Pt, perc);
            }
          }
        }
      }
      ...
      for (int i_var=0; i_var<nvarcut_Casc[i_cut]; i_var++) {
        ...
        if (i_cut!=kCasc_PropLifetOm) {
          if (fParticleAnalysisStatus[kxip]) {
            ...
            if(phypri && associFlag[kxim] && ApplyCuts(kxim, perc)) fHistos_XiMin->FillTH3(Form("h3_ptmasscent[%d][%d]", i_cut, i_var), fCasc_Pt, fCasc_InvMassXiMin, perc);
            if(phypri && associFlag[kxip] && ApplyCuts(kxip, perc)) fHistos_XiPlu->FillTH3(Form("h3_ptmasscent[%d][%d]", i_cut, i_var), fCasc_Pt, fCasc_InvMassXiPlu, perc);
          }
        }
        //Om filling
        if (i_cut!=kCasc_PropLifetXi) {
          if (fParticleAnalysisStatus[komp]) {
            SetCutVal(kFALSE, kTRUE, i_cut, varlowcut_Casc[i_cut]+i_var*(varhighcut_Casc[i_cut]-varlowcut_Casc[i_cut])/(nvarcut_Casc[i_cut]-1));
            if(phypri && associFlag[komm] && ApplyCuts(komm, perc)) fHistos_OmMin->FillTH3(Form("h3_ptmasscent[%d][%d]", i_cut, i_var), fCasc_Pt, fCasc_InvMassOmMin, perc);
            if(phypri && associFlag[komp] && ApplyCuts(komp, perc)) fHistos_OmPlu->FillTH3(Form("h3_ptmasscent[%d][%d]", i_cut, i_var), fCasc_Pt, fCasc_InvMassOmPlu, perc);
          }
        }
      }
      SetDefCutVals(); //reset defaults
