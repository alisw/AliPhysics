...
#include "AliAnalysisTaskStrVsMult_PileUpStudies.h"

#include "TMath.h"

ClassImp(AliAnalysisTaskStrVsMult_PileUpStudies)

AliAnalysisTaskStrVsMult_PileUpStudies::AliAnalysisTaskStrVsMult_PileUpStudies() : AliAnalysisTaskSE(),
//outputs
fHistos_eve(nullptr),
fHistos_K0S(nullptr),
fHistos_Lam(nullptr),
...
AliAnalysisTaskStrVsMult_PileUpStudies::AliAnalysisTaskStrVsMult_PileUpStudies(const char *name, TString lExtraOptions) : AliAnalysisTaskSE(name),
//outputs
fHistos_eve(nullptr),
fHistos_K0S(nullptr),
fHistos_Lam(nullptr),
...
fCasc_Cuts{1., 0.99, 1., 4., 80., 0.8, 0.005, 1., 0.99, 0.1, 0.1, 1., 0.5, 0.8, 3., 3., 3., 0.2, 0.2, 1.},
//particle to be analysed
fParticleAnalysisStatus{true, true, true, true, true, true, true},
//variables for V0 cuts
fV0_DcaV0Daught(0),
...
fCentLimit_BacBarCosPA(0)
{
  ...
  DefineOutput(9, TList::Class()); // Topology Histograms
}

AliAnalysisTaskStrVsMult_PileUpStudies::~AliAnalysisTaskStrVsMult_PileUpStudies()
{
    //------------------------------------------------
    // DESTRUCTOR
    //------------------------------------------------
...
void AliAnalysisTaskStrVsMult_PileUpStudies::UserCreateOutputObjects()
{
  ...
  fHistos_eve->CreateTH1("hcent_nopileup", "", 100, 0, 100, "s");  //storing #events in bins of centrality for events w/o pile-up
  ...
  if (!fDefOnly && (fParticleAnalysisStatus[kxip] || fParticleAnalysisStatus[komp])) {
    for (int icut=0; icut<kCasccutsnum; icut++) {
      if (icut==kCasc_y || icut==kCasc_etaDaugh || icut==kCasc_ITSTOFtracks) continue;
  ...
  if (fParticleAnalysisStatus[kxip] || fParticleAnalysisStatus[komp]) {
    fHistos_CascTopology = new THistManager("histos_CascTopology");
    fHistos_CascTopology->CreateTH1("CascCosPA_pileup","", 200, 0.99, 1.);
    ...
    fHistos_CascTopology->CreateTH1("DcaV0ToPV_pileup","", 90, 0.1, 1.);
    fHistos_CascTopology->CreateTH1("DcaBachToPV_pileup","", 90, 0.1, 1.);
    fHistos_CascTopology->CreateTH1("DcaPosToPV_pileup","", 80, 0.2, 1.);
    fHistos_CascTopology->CreateTH1("DcaNegToPV_pileup","", 80, 0.2, 1.);
    fHistos_CascTopology->CreateTH1("InvMassLambdaDaughter_pileup", "", 100, 1.11, 1.121);

    fHistos_CascTopology->CreateTH1("CascCosPA_nopileup","", 200, 0.99, 1.);
    ...
    fHistos_CascTopology->CreateTH1("DcaV0ToPV_nopileup","", 90, 0.1, 1.);
    fHistos_CascTopology->CreateTH1("DcaBachToPV_nopileup","", 90, 0.1, 1.);
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::UserExec(Option_t *)
{
  ...
  //MC truth
  if(fisMC){
    if(isESD) fispileupevent = AliAnalysisUtils::IsPileupInGeneratedEvent(lMCev, "Hijing");
        else fispileupevent = AliAnalysisUtils::IsPileupInGeneratedEvent(header, "Hijing");
    if(!fispileupevent) fHistos_eve->FillTH1("hcent_nopileup", lPercentile);
    for (int i_MCtrk = 0;  i_MCtrk < lMCev->GetNumberOfTracks(); i_MCtrk++){
      ...
      if (!fispileupevent) {
        if(fParticleAnalysisStatus[kxim] && lPart->PdgCode()==3312  ) fHistos_XiMin->FillTH2("h2_gen_nopileupevts",lPart->Pt(),lPercentile);
        if(fParticleAnalysisStatus[kxip] && lPart->PdgCode()==-3312 ) fHistos_XiPlu->FillTH2("h2_gen_nopileupevts",lPart->Pt(),lPercentile);
        if(fParticleAnalysisStatus[komm] && lPart->PdgCode()==3334  ) fHistos_OmMin->FillTH2("h2_gen_nopileupevts",lPart->Pt(),lPercentile);
  ...
  if (fParticleAnalysisStatus[kxip] || fParticleAnalysisStatus[komp]) {
    ...
    for (int i_casc = 0; i_casc < ncasc; i_casc++) {
      ...
      if(isESD){
        ...
        fCasc_nITSTOFtracks = 0;
        if((fCasc_NegTrackStatus & AliESDtrack::kITSrefit) || (fCasc_NegTOFBunchCrossing > -95.)) fCasc_nITSTOFtracks++;
        if((fCasc_PosTrackStatus & AliESDtrack::kITSrefit) || (fCasc_PosTOFBunchCrossing > -95.)) fCasc_nITSTOFtracks++;
        if((fCasc_BacTrackStatus & AliESDtrack::kITSrefit) || (fCasc_BacTOFBunchCrossing > -95.)) fCasc_nITSTOFtracks++; 
        ...
        if(fisMC){
          ...
          fisParticleFromOOB = AliAnalysisUtils::IsParticleFromOutOfBunchPileupCollision((int)TMath::Abs(labMothBach), lMCev);
        ...
        if(fisMC){
          ...
          physprim = ((AliMCParticle*) lMCev->GetTrack((int)TMath::Abs(labMothBach)))->IsPhysicalPrimary();
          fisParticleFromOOB = AliAnalysisUtils::IsParticleFromOutOfBunchPileupCollision((int)TMath::Abs(labMothBach), header, MCTrackArray);
          if(fisMCassoc && (pdgPosDaught!=2212 || pdgNegDaught!=-211 || pdgBachelor!=-211 || labMothPosDaught!=labMothNegDaught || pdgV0!=3122 || labMothV0!=labMothBach || pdgCasc!=3312)) assFlag[kxim] = kFALSE;
          if(fisMCassoc && (pdgPosDaught!=211 || pdgNegDaught!=-2212 || pdgBachelor!=211 || labMothPosDaught!=labMothNegDaught || pdgV0!=-3122 || labMothV0!=labMothBach || pdgCasc!=-3312)) assFlag[kxip] = kFALSE;
          if(fisMCassoc && (pdgPosDaught!=2212 || pdgNegDaught!=-211 || pdgBachelor!=-321 || labMothPosDaught!=labMothNegDaught || pdgV0!=3122 || labMothV0!=labMothBach || pdgCasc!=3334)) assFlag[komm] = kFALSE;
          if(fisMCassoc && (pdgPosDaught!=211 || pdgNegDaught!=-2212 || pdgBachelor!=321 || labMothPosDaught!=labMothNegDaught || pdgV0!=-3122 || labMothV0!=labMothBach || pdgCasc!=-3334)) assFlag[komp] = kFALSE;
        ...
        if( physprim && assFlag[kxim] && ApplyCuts(kxim)) {
          ...
          if (fisMC) {
            ...
            if (fisParticleFromOOB) {
              ...
              fHistos_CascTopology->FillTH1("V0Radius_pileup", fCasc_V0Rad);
              fHistos_CascTopology->FillTH1("CascyXi_pileup", fCasc_yXi);
              fHistos_CascTopology->FillTH1("CascCtauXi_pileup", 1.32171*fCasc_DistOverTotP);
              fHistos_CascTopology->FillTH1("CascPt_pileup", fCasc_Pt);
              fHistos_CascTopology->FillTH1("DcaV0Daughters_pileup", fCasc_DcaV0Daught);
              fHistos_CascTopology->FillTH1("DcaCascDaughters_pileup", fCasc_DcaCascDaught);
              fHistos_CascTopology->FillTH1("DcaV0ToPV_pileup", fCasc_DcaV0ToPV);
              fHistos_CascTopology->FillTH1("DcaBachToPV_pileup", fCasc_DcaBachToPV);
            ...
            if (!fisParticleFromOOB) {
              ...
              fHistos_CascTopology->FillTH1("V0Radius_nopileup", fCasc_V0Rad);
              fHistos_CascTopology->FillTH1("CascyXi_nopileup", fCasc_yXi);
              fHistos_CascTopology->FillTH1("CascCtauXi_nopileup", 1.32171*fCasc_DistOverTotP);
              fHistos_CascTopology->FillTH1("CascPt_nopileup", fCasc_Pt);
              fHistos_CascTopology->FillTH1("DcaV0Daughters_nopileup", fCasc_DcaV0Daught);
              fHistos_CascTopology->FillTH1("DcaCascDaughters_nopileup", fCasc_DcaCascDaught);
              fHistos_CascTopology->FillTH1("DcaV0ToPV_nopileup", fCasc_DcaV0ToPV);
              fHistos_CascTopology->FillTH1("DcaBachToPV_nopileup", fCasc_DcaBachToPV);
        ...
        if( physprim && assFlag[komm] && ApplyCuts(komm)) {
          ...
          if (fisMC) {
            ...
            if (fisParticleFromOOB) {
              ...
              fHistos_CascTopology->FillTH1("CascRadius_pileup", fCasc_CascRad);
              fHistos_CascTopology->FillTH1("V0Radius_pileup", fCasc_V0Rad);
              fHistos_CascTopology->FillTH1("CascyOm_pileup", fCasc_yOm);
              fHistos_CascTopology->FillTH1("CascCtauOm_pileup", 1.67245*fCasc_DistOverTotP);
              fHistos_CascTopology->FillTH1("CascPt_pileup", fCasc_Pt);
              fHistos_CascTopology->FillTH1("DcaV0Daughters_pileup", fCasc_DcaV0Daught);
              fHistos_CascTopology->FillTH1("DcaCascDaughters_pileup", fCasc_DcaCascDaught);
              fHistos_CascTopology->FillTH1("DcaV0ToPV_pileup", fCasc_DcaV0ToPV);
            ...
            if (!fisParticleFromOOB) {
              ...
              fHistos_CascTopology->FillTH1("CascRadius_nopileup", fCasc_CascRad);
              fHistos_CascTopology->FillTH1("V0Radius_nopileup", fCasc_V0Rad);
              fHistos_CascTopology->FillTH1("CascyOm_nopileup", fCasc_yOm);
              fHistos_CascTopology->FillTH1("CascCtauOm_nopileup", 1.67245*fCasc_DistOverTotP);
              fHistos_CascTopology->FillTH1("CascPt_nopileup", fCasc_Pt);
              fHistos_CascTopology->FillTH1("DcaV0Daughters_nopileup", fCasc_DcaV0Daught);
              fHistos_CascTopology->FillTH1("DcaCascDaughters_nopileup", fCasc_DcaCascDaught);
              fHistos_CascTopology->FillTH1("DcaV0ToPV_nopileup", fCasc_DcaV0ToPV);
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::Terminate(Option_t *)
{

}

//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::SetCutVal(bool defchange, bool iscasc, int cutnum, double cval) {
  if (!iscasc) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::SetParametricBacBarCosPA(int nbins, float *ptbins, float *values, int cent_limit) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::SetDefCutVals() {
  //V0 part
  if (fParticleAnalysisStatus[kk0s] || fParticleAnalysisStatus[klam]) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::SetCutVariation(bool iscasc, int cutnum, int nvar, double lowval, double highval) {
  if (!iscasc) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::SetDefCutVariations() {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::SetDefOnly(bool isdefonly) {
  fDefOnly = isdefonly;
}

//________________________________________________________________________
bool AliAnalysisTaskStrVsMult_PileUpStudies::ApplyCuts(int part) {
  ...
  if (part<kxip) { //we are checking V0s
    ...
    //    (fCasc_NegTOFBunchCrossing < cutval_Casc[kCasc_ITSTOFtracks]) &&
    //    (fCasc_PosTOFBunchCrossing < cutval_Casc[kCasc_ITSTOFtracks]) &&
    //    (fCasc_BacTOFBunchCrossing < cutval_Casc[kCasc_ITSTOFtracks])) fCasc_isRecoOOBpileup = kTRUE; //OOB
    // TPC refit, should be already verified for Offline V0s
...
void AliAnalysisTaskStrVsMult_PileUpStudies::SetParticleAnalysisStatus(bool k0s, bool lambda, bool xi, bool omega) {
...
bool AliAnalysisTaskStrVsMult_PileUpStudies::GetParticleAnalysisStatus(int part) {
  if (part<0 || part>=ksignednumpart) {
    ::Error("AliAnalysisTaskStrVsMult_PileUpStudies::GetParticleAnalysisStatus", "Wrong particle selected: accepted values from 0 to 3");
    return false;
  }
  return fParticleAnalysisStatus[part];
}

void AliAnalysisTaskStrVsMult_PileUpStudies::SetCentbinning(int ipart, int numcentbins, double *centbins) {
...
void AliAnalysisTaskStrVsMult_PileUpStudies::SetPtbinning(int ipart, int numptbins, double *ptbins) {
...
void AliAnalysisTaskStrVsMult_PileUpStudies::SetMassbinning(int ipart, int nummassbins, double valminmass, double valmaxmass) {
...
//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::DataPosting() {
  ...
  if (fParticleAnalysisStatus[kxip] || fParticleAnalysisStatus[komp]) {
    histnumber++;
    PostData(histnumber, fHistos_CascTopology->GetListOfHistograms());
  }
  
}

//________________________________________________________________________
void AliAnalysisTaskStrVsMult_PileUpStudies::FillHistCutVariations(bool iscasc, double perc, bool phypri, bool *associFlag, double ptassxi) {
  ...
  if (!iscasc) {
    ...
    for(int i_cut=0; i_cut<kCasccutsnum; i_cut++) {
      if(i_cut==kCasc_y || i_cut==kCasc_etaDaugh || i_cut==kCasc_ITSTOFtracks) continue;
