AliAnalysisTaskStrangenessVsMultVsEffEnergyAODRun2 *AddTaskStrangenessVsMultVsEffEnergyAODRun2( Bool_t lSaveEventTree = kTRUE, Bool_t lSaveV0 = kTRUE, Bool_t lSaveCascade = kTRUE, TString lExtraOptions = "", const TString lMasterJobSessionFlag = "", TString lExtraOutputName = "")
{
    ...
    if (!mgr) {
        ::Error("AddTaskStrangenessVsMultVsEffEnergyAODRun2", "No analysis manager to connect to.");
        return NULL;
    }
    ...
    if (!mgr->GetInputEventHandler()) {
        ::Error("AddTaskStrangenessVsMultVsEffEnergyAODRun2", "This task requires an input event handler");
        return NULL;
    }
    TString type = mgr->GetInputEventHandler()->GetDataType(); // can be "ESD" or "AOD"
    
    // Create and configure the task
    AliAnalysisTaskStrangenessVsMultVsEffEnergyAODRun2 *taskAuxiliary = new AliAnalysisTaskStrangenessVsMultVsEffEnergyAODRun2(lSaveEventTree, lSaveV0, lSaveCascade, Form("taskAuxiliary%s",lExtraOutputName.Data()), lExtraOptions);
