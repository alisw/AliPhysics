...
To see this a study was made, selecting the \ppCol{} at
...
\begin{align}
  ...
  \left(\frac1{\Nch}\diffFrac{^{2}N_{i}}{\pT{}\mathrm{d}y}\right)_{j}
\end{align}.

\FIXME{We should fix up \figref{fig:weights}}
\begin{figure}[]
  \centering
  \figinput[.7\textwidth]{310weights}
  \caption{The $K^{0}_{s}$ weights for \PbPbCol{} calculated based on
    \eqref{eq:weights}.  The bottom panels show the averaged weights
    for some pseudorapidity and transverse momentum slices.}
  \label{fig:weights}
\end{figure} 
...
particle species.  \figref {fig:secmapweighted:all} shows an example
of the weights for $K^{0}_{s}$ for \PbPbCol{}.
\figref{fig:secmapweighted:all} shows the comparison of the secondary
maps from the weighting procedure for all the system. It is clear that
all discrepancies observed in Figure \ref{fig:secmap:all} are
compressed by the weighting procedure.

\FIXME{Conclusion on this, and why does it affect FMD1 the most. Also,
words on pending weights from `real' data, and the final effect on the
systematics.} 
...
\FIXME{Perhaps something on AliROOT/AliPhysics release used.} 
