...
function \FuncSty{SignalInStrip}($t$) returns the properly
path--length corrected signal in strip $t$.  Two cuts are used in this
algorithm:
\begin{description}
\item[\FuncSty{GetLowCut}] Signals below this cut are considered
...
\FuncSty{GetHighCut} and how those values are obtained can be varied
in the analysis (see also \secref{sec:cuts}).
...
location).  See also \figref{fig:density:corners}
...
For this method, we simply sum up $n'_t$ for all strips within an
$\etaphi$ bin.
\begin{align}
  \label{eq:density1}
...
likewise be varied (see also \secref{sec:cuts}).   Note, this does not
...
energies agree within at least 5\% (see \figref{fig:secmap:all}) ---
except for \PbPbCol{} and \PbpCol{} which deviates more in the region
of \FMD{1}.  This effect was studied in detail using the above
...
To see this, a study was performed, selecting the \ppCol{} reaction at
...
\begin{figure}[h!tbp]
  \centering
  \figinput[.9\textwidth]{310weights}
...
\figref{fig:secmap:all} are systematically reduced by the weighting
...
Again, the sub--script $i$ denotes this is per--event information. 
...
\begin{figure}[]
  ...
  \caption{A cartoon of the step--by--step evolution of $\dndeta$ in
    ...
    \secref{sec:sub:sub:secmap}). The figure should be read from the
    top-left, downward, continuing on the top right, and finally
    ending up in the lower right corner. Dark blue markers are for
    \FMD{3i}, light blue is \FMD{3o}, light green \FMD{2o}, dark green
    \FMD{2i}, and red \FMD{1i}.}
  \label{fig:steps}
\end{figure} 
