...
class AliTrackletTaskMulti : public AliAnalysisTaskSE {
 public:
  ...
  enum {kCentV0M,kCentV0A,kCentV0C,kCentFMD,kCentTRK,kCentTKL,kCentCL0,kCentCL1,kCentV0MvsFMD,kCentZNA,kCentTKLvsV0,kCentZEMvsZDC,kCentV0A123,kCentV0A0,kCentV0S,kCentMB,kCentV0Mplus05,kCentV0Mminus05,SPDClustersCorr,kNCentTypes}; // what is used to define centrality
  ...
  enum { // define here id's of any custom histos to be added to fHistosCustom
    ...
    kHPdgPrimMC,       // PDG code of primary species 
    //
    ...
    kHPtPrimMC    =kHZVResMC+50            // pT distribution
  }; // custom histos
  ...
  Bool_t     FillHistosSet(TObjArray* histos, double eta, /*double phi,double theta,*/double dphi,double dtheta,double dthetaX,double dist,double weight);
  // RS
 ...
 protected:
  ...
  void       ReweightStack(double weight); // R+HACK
  float      GetPrimaryWeight(Int_t i); // R+HACK
  //
 protected:
  ...
  Float_t      fPhiMin;                    // histos filled only for this phi range
  Float_t      fPhiMax;                    // histos filled only for this phi range
  ...
  Int_t fReweightStack;                    // flag to reweight stack
  Int_t fReweightFlag;                     // reweight scaling factor
  Double_t fEtaBinWidth;                   // Bin width in Eta
  //
