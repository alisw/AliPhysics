 ...
 */
void
TunedConfig(AliForwardMultiplicityBase* task)
{
  if (!task) return;
  ...
  task->SetAddMask(AliAODForwardMult::kV0AND|
  ...
  Bool_t use3 = false;
  AliFMDMultCuts cSharingLow(AliFMDMultCuts::kFixed,.15);
  AliFMDMultCuts cSharingHigh(AliFMDMultCuts::kLandauSigmaWidth,
	                      1.2,1.0,1.9,0.7,1.4);
  AliFMDMultCuts cDensity(cSharingHigh); 
  ...
  task->GetSharingFilter().SetAllow3Strips(use3);
  ...
  task->GetDensityCalculator().SetMaxOutliers(0.05); // 1.0);//Disable filter
