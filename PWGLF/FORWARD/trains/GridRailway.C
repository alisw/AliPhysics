...
struct GridRailway : public PluginRailway
{
	       ...
	       Bool_t /*asShellScript*/) 
  {
    ...
    d << "// Generated by GridRailway\n"
      ...
      << "{\n"
      << "  TString base = \"" << fUrl.GetProtocol()<<"://"<< outDir << "\";\n"
      << "  TString runs = \"" << runs << "\";\n\n"
    ...
    if (!a) { 
      Error("GridRailway::AuxSave", "Failed to open DownloadAODs.C");
      return;
    }
    a << "// Generated by GridRailway\n"
      << "void DownloadAODs(Bool_t verbose=false,Bool_t force=false)\n"
      << "{\n"
      << "  gROOT->LoadMacro(\"" << macDir << "/GridDownloadAODs.C\");\n"
      << "  GridDownloadAODs(\"" << outDir << "\",verbose,force);\n"
      << "}\n"
      << "// EOF\n"
      << std::endl;
    a.close();

    std::ofstream e("downloadAODs.sh");
    if (!e) { 
      Error("GridRailway::AuxSave", "Failed to open downloadAODs.sh");
      return;
    }
    e << "#!/bin/sh\n"
      << "# Generated by GridRailway\n"
      << "exec " << macDir << "/downloadAODs.sh -s 0 -e 10000 -f AliAOD.root "
      << "-d " << outDir << " $@\n"
      << "# EOF\n" << std::endl;
    e.close();
    gSystem->Exec("chmod a+x downloadAODs.sh");
    
    std::ofstream c("MakeIndex.C");
    if (!c) {
      Error("GridRailway::AuxSave", "Failed to open MakeIndex.C");
      return;
    }
    c << "// Generated by GridRailway\n"
      << "void MakeIndex()\n"
      << "{\n"
      << "  TString libs = \"" << libs << "\";\n"
      << "  TString pars = \"" << pars << "\";\n"
      ...
      << "  gSystem->AddIncludePath(\"-I$ALICE_ROOT/include\");\n\n"
      << "  gSystem->AddIncludePath(\"-I$ALICE_PHYSICS/include\");\n\n"
      << "  gROOT->LoadMacro(\"" << macDir << "/CreateIndex.C\");\n"
