...
void PlotMatchedTracksDepositsFromData(TString datafilename="rootFiles/LHC10hPass2/Et.ESD.realPbPb.EMCal.LHC10hPass2.Run139465.root",TString simfilename="rootFiles/LHC11a10a_bis/Et.ESD.simPbPb.EMCal.LHC11a10a_bis.Run139465.root", TString det = "EMCal",Bool_t allCells = kTRUE){
...
//   TH1D *data[10] = {NULL,NULL,NULL,NULL,NULL, NULL,NULL,NULL,NULL,NULL};
  ...
  for(int bin = 1; bin<=nbins;bin++){
    ...
    simulation[bin] = fHistMatchedTracksEvspTBkgdvsCent->ProjectionY("y");
    ...
    simulationEffCorr[bin] = fHistMatchedTracksEvspTBkgdvsCentEffCorr->ProjectionY("y");
    ((TH1D*)simulationEffCorr[bin])->SetName(Form("Sim%i",bin));
    SetStyles((TH1D*)simulationEffCorr[bin],markersSim[bin-1],colors[bin-1]);
    //
    //   TObjArray trackHit(nbins+1);
    //   TObjArray trackDep(nbins+1);
    //   TObjArray trackEffVsPt(nbins+1);
    ...
    cout<<"fraction bin "<<bin<<" "<<belowCut[bin]/total[bin];
    cout<<" <et> low pT "<<((TH1D*)etForLowPtTracks[bin])->GetMean();
    cout<<" <et> high pT "<<((TH1D*)simulation[bin])->GetMean();
    cout<<" low/high "<<((TH1D*)etForLowPtTracks[bin])->GetMean()/((TH1D*)simulation[bin])->GetMean();
  ...
  for(int bin = 1; bin<=nbins;bin++){
    ...
    ((TH1D*)ratiosEffCorr[bin])->Divide((TH1D*)simulationEffCorr[bin]);
  }
  ...
  for(int bin = 1; bin<=nbins;bin++){
    ...
    simulationEffCorr[bin]->Draw("same");
    myfile2<< ((TH1D *)dataEffCorr[bin])->GetMean() <<"\t"<< ((TH1D *)simulationEffCorr[bin])->GetMean()<<endl;
  }
  myfile2.close();
  leg->Draw();
