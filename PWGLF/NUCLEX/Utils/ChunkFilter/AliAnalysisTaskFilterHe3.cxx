...
void AliAnalysisTaskFilterHe3::UserExec(Option_t*) {
  ...
  if (eventHandlerMC) mcTrue = kTRUE;
  ...
  for (Int_t j = 0; j < jTracks; j++) { // start loop over tracks
    ...
    if (mcTrue) {
      ...
      if (particleGen) pdgCode = particleGen->GetPdgCode();
    }
    //
    ...
    //
    if (track->GetTPCsignalN() > 50 && fESDtrackCutsPrimary->AcceptTrack(track)) fHistdEdxData->Fill(ptot*sign,tpcSignal);
    //
    ...
    Float_t hel3Exp = -999;
    //
    if (ptot > 0.3) { // protection against numerical instabilities
      if (!mcTrue) {
