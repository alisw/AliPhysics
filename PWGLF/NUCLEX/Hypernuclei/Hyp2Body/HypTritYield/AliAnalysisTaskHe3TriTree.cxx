...
#include "AliAnalysisTask.h"
#include "AliAnalysisManager.h"
#include "AliAnalysisTaskHe3TriTree.h"



class AliAnalysisTaskHe3TriTree;   
using namespace std;           

ClassImp(AliAnalysisTaskHe3TriTree) 

AliAnalysisTaskHe3TriTree::AliAnalysisTaskHe3TriTree() 
: AliAnalysisTaskSE(), 
fESDevent(0),
mcEvent(0),
fStack(0),
...
AliAnalysisTaskHe3TriTree::AliAnalysisTaskHe3TriTree(const char* name) //member initialization
...
tPdg(-1)
{
    ...
    DefineOutput(1, TList::Class());            //Input und output definieren
...
AliAnalysisTaskHe3TriTree::~AliAnalysisTaskHe3TriTree() {
    if(fOutputList) {
        delete fOutputList;    //löschen der outputlist 
    }
}
//_____________________________________________________________________________
void AliAnalysisTaskHe3TriTree::Terminate(Option_t *){
}
//_____________________________________________________________________________
void AliAnalysisTaskHe3TriTree::UserCreateOutputObjects() {
  ...
  matching = new AliTRDonlineTrackMatching();
	...
	fHistEvents->GetXaxis()->SetBinLabel(2,"MB");                       //Histogramme 
	...
	for (int i = 0; i < 2; i++) {   
	  ...
	  fHistEventsTRD[i]->GetXaxis()->SetBinLabel(4,"HQUsimMB");         //Weiteres Histogramm
	...
	fTree->Branch("tPdg"			 , &tPdg			 , "tPdg/I");

	fTreeGenHe = new TTree("treeGenHe","fTreeGenHe");
	fTreeGenHe->Branch("tGenCharge"  , &tGenCharge  , "tGenCharge/D" );
	fTreeGenHe->Branch("tGenPt"      , &tGenPt      , "tGenPt/D"     );
	fTreeGenHe->Branch("tGenY"       , &tGenY       , "tGenY/D"      );
	fTreeGenHe->Branch("tGenPrimary" , &tGenPrimary , "tGenPrimary/I");
	fTreeGenHe->Branch("tMagField"   , &tMagField   , "tMagField/D"  );
	fTreeGenHe->Branch("pdgCode"     , &pdgCode     , "pdgCode/I"    );
	fTreeGenHe->Branch("tTrigMB"     , &tTrigMB     , "tTrigMB/I"    );
	fTreeGenHe->Branch("tTrigHMV0"   , &tTrigHMV0   , "tTrigHMV0/I"  );
	fTreeGenHe->Branch("tTrigHMSPD"  , &tTrigHMSPD  , "tTrigHMSPD/I" );
	fTreeGenHe->Branch("tTrigHNU"    , &tTrigHNU    , "tTrigHNU/I"   );
	fTreeGenHe->Branch("tTrigHQU"    , &tTrigHQU    , "tTrigHQU/I"   );


	fTreeGenTri = new TTree("treeGenTri", "fTreeGenTri");
	fTreeGenTri->Branch("tGenCharge"  , &tGenCharge  , "tGenCharge/D" );
	fTreeGenTri->Branch("tGenPt"      , &tGenPt      , "tGenPt/D"     );
	fTreeGenTri->Branch("tGenY"       , &tGenY       , "tGenY/D"      );
	fTreeGenTri->Branch("tGenPrimary" , &tGenPrimary , "tGenPrimary/I");
	fTreeGenTri->Branch("pdgCode"     , &pdgCode     , "pdgCode/I"    );
	fTreeGenTri->Branch("tMagField"   , &tMagField   , "tMagField/D"  );
	fTreeGenTri->Branch("tTrigMB"     , &tTrigMB     , "tTrigMB/I"    );
	fTreeGenTri->Branch("tTrigHMV0"   , &tTrigHMV0   , "tTrigHMV0/I"  );
	fTreeGenTri->Branch("tTrigHMSPD"  , &tTrigHMSPD  , "tTrigHMSPD/I" );
	fTreeGenTri->Branch("tTrigHNU"    , &tTrigHNU    , "tTrigHNU/I"   );
	fTreeGenTri->Branch("tTrigHQU"    , &tTrigHQU    , "tTrigHQU/I"   );
	...
	fTreeTri->Branch("tY"               , &tY               , "tY/D"               );
	fTreeTri->Branch("tEta"             , &tEta             , "tEta/D"             );
	fTreeTri->Branch("tPhi"             , &tPhi             , "tPhi/D"             );
	fTreeTri->Branch("tP"               , &tP               , "tP/D"               );
	fTreeTri->Branch("tPx"              , &tPx              , "tPx/D"              );
	fTreeTri->Branch("tPy"              , &tPy              , "tPy/D"              );
	fTreeTri->Branch("tPz"              , &tPz              , "tPz/D"              );
	fTreeTri->Branch("tTriSigma"        , &tTriSigma        , "tSigma/D"           );
	fTreeTri->Branch("tTriDedx"         , &tTriDedx         , "tDedx/D"            );
	...
	fTreeTri->Branch("tTrigHMSPD"       , &tTrigHMSPD       , "tTrigHMSPD/I"       );
	fTreeTri->Branch("tTrigHNU"         , &tTrigHNU         , "tTrigHNU/I"         );
	fTreeTri->Branch("tTrigHQU"         , &tTrigHQU         , "tTrigHQU/I"         );
	fTreeTri->Branch("tMagField"        , &tMagField        , "tMagField/D"        );
	fTreeTri->Branch("tTRDvalid"        , &tTRDvalid        , "tTRDvalid/I"        );
	fTreeTri->Branch("tTRDPid"          , &tTRDPid          , "tTRDPid/I"          );
	fTreeTri->Branch("tTRDnTracklets"   , &tTRDnTracklets   , "tTRDnTracklets/I"   );
	fTreeTri->Branch("tTRDPt"           , &tTRDPt           , "tTRDPt/I"           );
	fTreeTri->Branch("tTRDLayerMask"    , &tTRDLayerMask    , "tTRDLayerMask/I"    );
	fTreeTri->Branch("tTRDSagitta"      , &tTRDSagitta      , "tTRDSagitta/D"      );
	fTreeTri->Branch("tTRDStack"        , &tTRDStack        , "tTRDStack/I"        );
	fTreeTri->Branch("tTRDSector"       , &tTRDSector       , "tTRDSector/I"       );
	fTreeTri->Branch("tTRDPID0"         , &tTRDPID0         , "tTRDPID0/i"         );
	fTreeTri->Branch("tTRDPID1"         , &tTRDPID1         , "tTRDPID1/i"         );
	fTreeTri->Branch("tTRDPID2"         , &tTRDPID2         , "tTRDPID2/i"         );
	fTreeTri->Branch("tTRDPID3"         , &tTRDPID3         , "tTRDPID3/i"         );
	fTreeTri->Branch("tTRDPID4"         , &tTRDPID4         , "tTRDPID4/i"         );
	fTreeTri->Branch("tTRDPID5"         , &tTRDPID5         , "tTRDPID5/i"         );
	fTreeTri->Branch("tnTPCcluster"     , &tnTPCcluster     , "tnTPCcluster/D"     );
	fTreeTri->Branch("tnITScluster"     , &tnITScluster     , "tnITScluster/D"     );
	fTreeTri->Branch("tTPCchi2"         , &tTPCchi2         , "tTPCchi2/D"         );
	fTreeTri->Branch("tSPDFiredChips0"  , &tSPDFiredChips0  , "tSPDFiredChips0/D"  );
	fTreeTri->Branch("tSPDFiredChips1"  , &tSPDFiredChips1  , "tSPDFiredChips1/D"  );
	fTreeTri->Branch("tSPDTracklets"    , &tSPDTracklets    , "tSPDTracklets/D"    );
	fTreeTri->Branch("tSPDCluster"      , &tSPDCluster      , "tSPDCluster/D"      );
	fTreeTri->Branch("tV0Multiplicity"  , &tV0Multiplicity  , "tV0Multiplicity/D"  );
	fTreeTri->Branch("tNTracks"         , &tNTracks         , "tNTracks/D"         );
	fTreeTri->Branch("tMultV0M"         , &tMultV0M         , "tMultV0M/D"         );
	fTreeTri->Branch("tMultOfV0M"       , &tMultOfV0M       , "tMultOfV0M/D"       );
	fTreeTri->Branch("tMultSPDTracklet" , &tMultSPDTracklet , "tMultSPDTracklet/D" );
	fTreeTri->Branch("tMultSPDCluster"  , &tMultSPDCluster  , "tMultSPDCluster/D"  );
	fTreeTri->Branch("tMultRef05"       , &tMultRef05       , "tMultRef05/D"       );
	fTreeTri->Branch("tMultRef08"       , &tMultRef08       , "tMultRef08/D"       );
	fTreeTri->Branch("tKink"            , &tKink            , "tKink/I"            );
	fTreeTri->Branch("tTPCrefit"        , &tTPCrefit        , "tTPCrefit/I"        );
	fTreeTri->Branch("tITSrefit"        , &tITSrefit        , "tITSrefit/I"        );
	fTreeTri->Branch("tTOFSignalTri"    , &tTOFSignalTri    , "tTOFSignalTri/D"    );
	fTreeTri->Branch("tPrimary"         , &tPrimary         , "tPrimary/I"         );
	fTreeTri->Branch("tWeak"            , &tWeak            , "tWeak/I"            );
	fTreeTri->Branch("tMaterial"        , &tMaterial        , "tMaterial/I"        );
	fTreeTri->Branch("tPdg"			 , &tPdg			 , "tPdg/I");


	PostData(1, fOutputList);
	PostData(2, fTree);
	PostData(3, fTreeGenHe);  
	PostData(4, fTreeTri); 
	PostData(5, fTreeGenTri);           
}
//_____________________________________________________________________________
void AliAnalysisTaskHe3TriTree::UserExec(Option_t *) {
  // MC
	fMCtrue = kTRUE;
	...
	trackCutsV0.SetRequireTPCRefit(kFALSE);                         //Cuts
	...
	//********************************************
	//* loop over all tracks, identify He3, Tri  *
	//********************************************
	...
	for(Int_t i(0); i < iTracks; i++) {                
		...
		if (TMath::Abs(Bethe(*track, AliPID::ParticleMass(AliPID::kTriton), TMath::Abs(tChargeTri), fBetheParamsT)) < 5) {
  			tRunNumber = runNumber;
			tTrigMB = MB;		
			tTrigHMV0 = HMV0;
			tTrigHMSPD = HMSPD;
			...
			Double_t momvectTri[3];
  			track->PxPyPz(momvectTri);
  			TLorentzVector TriVector(momvectTri[0], momvectTri[1], momvectTri[2],0);
  			TriVector.SetVect(TMath::Abs(tChargeTri) * TriVector.Vect());
  			tE = TMath::Sqrt(AliPID::ParticleMass(AliPID::kTriton) * AliPID::ParticleMass(AliPID::kTriton) + TriVector.Vect().Mag2());
  			TriVector.SetE(tE);
  			...
  			tPz = momvectTri[2];
  			tPt = TriVector.Pt();
  			tY = TriVector.Rapidity();
  			tEta = track->Eta();
  			tPhi = track->Phi();

  			tP = track->GetInnerParam()->GetP();
			tTriDedx = track->GetTPCsignal();
			tTriSigma = Bethe(*track, AliPID::ParticleMass(AliPID::kTriton), TMath::Abs(tChargeTri), fBetheParamsT);
			tTOFSignalTri = GetTOFSignalTri(*track, tP);

			Float_t dcaTri[2], covTri[3];
			track->GetImpactParameters(dcaTri, covTri);
			tDcaXY = dcaTri[0];
			tDcaZ = dcaTri[1];  //distance of closest approach
			tSigmaYX = covTri[0];
			tSigmaXYZ = covTri[1];
			tSigmaZ = covTri[2];
			TRDtrack(track);
			...
			Int_t labelMother = mcEvent->GetLabelOfParticleMother(TMath::Abs(label));
			AliMCParticle *particleMother = new AliMCParticle(mcEvent->GetTrack(TMath::Abs(labelMother))->Particle()); 
			tMCtrue = TMath::Abs(particle->PdgCode()) == 1000010030;	
			tPrimary = mcEvent->IsPhysicalPrimary(TMath::Abs(label));
			tWeak = mcEvent->IsSecondaryFromWeakDecay(TMath::Abs(label)); 
			tMaterial = mcEvent->IsSecondaryFromMaterial(TMath::Abs(label));	
			}
...
//_____________________________________________________________________________
void AliAnalysisTaskHe3TriTree::ProcessMCParticles() {
	// fills tree with generated He3/3H
	...
	for (Int_t istack = 0; istack < mcEvent->GetNumberOfTracks(); istack++) {
		...
		if  (TMath::Abs(pdgCode) == 1000010030){
			tGenPt = particle->Pt();
			tGenY = particle->Y();
			tGenCharge = -1;
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskHe3TriTree::GetInvPtDevFromBC(Int_t b, Int_t c) {
...
//_____________________________________________________________________________
void AliAnalysisTaskHe3TriTree::TRDnEvents() {
	//*   get trigger information  *
	Bool_t MB = kFALSE;		// minimum bias
	Int_t HNU = 0;			// TRD nuclei
	Int_t HQU = 0;			// TRD quarkonia
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskHe3TriTree::Bethe(const AliESDtrack& track, Double_t mass, Int_t charge, Double_t* params){
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskHe3TriTree::GetTOFSignalHe3(AliESDtrack& trackHe, Double_t tP) {
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskHe3TriTree::GetTOFSignalTri(AliESDtrack& trackTri, Double_t tP){
	Double_t mass = 0, time = -1, beta = 0, gamma = 0, length = 0, time0 = 0;
	length = trackTri.GetIntegratedLength();
	time0 = fPIDResponse->GetTOFResponse().GetStartTime(trackTri.P());
	time = trackTri.GetTOFsignal() - time0;
	if (time > 0 && length > 0) {
		beta = length / (2.99792457999999984e-02 * time);
		gamma = 1/ TMath::Sqrt(1- beta*beta);
		mass = (tP/ TMath::Sqrt(gamma*gamma -1));
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskHe3TriTree::TRDtrack(AliESDtrack* esdTrack) {
