/// \class AliAnalysisTaskHypTritTest
...
#ifndef ALIANALYSISTASKHypTritTestAOD_H
#define ALIANALYSISTASKHypTritTestAOD_H
...
class KFParticle;
//class KFParticle3LHN;
class KFVertex;
class AliITStrackerMI;
class AliAODMCHeader;

class KFParticle3LHJ : public KFParticle {
  public:
    Bool_t CheckDaughter(KFParticle3LHJ daughter) {
      Float_t m[8], mV[36], D[3][3];
      if(KFParticleBase::GetMeasurement(daughter, m, mV, D)) return kTRUE;
      return kFALSE;    
    }
  
    Bool_t AddDaughters(KFParticle3LHJ daughter1, KFParticle3LHJ daughter2, int constructMethod) {
      if (constructMethod == 2) {
        KFParticle3LHJ KFCheck1;
        KFCheck1.AddDaughter(daughter1);
        if (!KFCheck1.CheckDaughter(daughter2)) return kFALSE;
        KFParticle3LHJ KFCheck2;
...
class AliAnalysisTaskHypTritTestAOD : public AliAnalysisTaskSE {
 public:
  AliAnalysisTaskHypTritTestAOD();
  AliAnalysisTaskHypTritTestAOD(const char *name);
  virtual ~AliAnalysisTaskHypTritTestAOD();
  virtual void UserCreateOutputObjects();
  virtual void UserExec(Option_t *option);
  virtual void Terminate(const Option_t*);


  void SetPeriod(Int_t period = 2018) {fPeriod = period;};
 ...
 private:
  ...
  AliPIDResponse              *fPID;                 //!<! ESD pid
  AliAODEvent            *fESDevent;            //!<! ESD event
  ...
  KFParticle3LHJ CreateKFParticle(AliExternalTrackParam& track, float Mass, Int_t Charge);
  KFVertex CreateKFVertex(const AliVVertex& vertex);
  Double_t GetTOFSignal(const AliAODTrack& track);  
  AliAnalysisTaskHypTritTestAOD(const AliAnalysisTaskHypTritTestAOD&);
  AliAnalysisTaskHypTritTestAOD &operator=(const AliAnalysisTaskHypTritTestAOD&);

  /// \cond CLASSIMP
  ClassDef(AliAnalysisTaskHypTritTestAOD, 1);
  /// \endcond
};
#endif
