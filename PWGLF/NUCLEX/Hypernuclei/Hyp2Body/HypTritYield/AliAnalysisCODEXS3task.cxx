...
ClassImp(AliAnalysisCODEXS3task);

using namespace AliAnalysisCODEX;

AliAnalysisCODEXS3task::AliAnalysisCODEXS3task(const char* name) :
...
AliAnalysisCODEXS3task::~AliAnalysisCODEXS3task() {
  delete mPIDresponse;
  delete mTree;
  delete mOutput;
}

void AliAnalysisCODEXS3task::UserCreateOutputObjects() {
  ...
  handler = dynamic_cast<AliInputEventHandler*> (man->GetInputEventHandler());
  if(!handler)
  ...
  fHistEvents = new TH1D("fHistEvents", "fHistEvents", 6, 0, 6);     
	fHistEvents->GetXaxis()->SetBinLabel(1,"Events");
	fHistEvents->GetXaxis()->SetBinLabel(2,"HMV0");
	fHistEvents->GetXaxis()->SetBinLabel(3,"HMSPD");
	fHistEvents->GetXaxis()->SetBinLabel(4,"HNU");
	fHistEvents->GetXaxis()->SetBinLabel(5,"HQU");

	mOutput->Add(fHistEvents);  
...
void AliAnalysisCODEXS3task::UserExec(Option_t *){
  ...
  if (!event) return;
  if (mTriggerMask) {
  	mEventCuts.OverrideAutomaticTriggerSelection(mTriggerMask);
  }
  ...
  fHistEvents->Fill(1);
  if ((handler->IsEventSelected() & AliVEvent::kHighMultV0)) {
  	fHistEvents->Fill(2);
 		mHeader.mEventMask |= kHMV0; 
  }
  if ((handler->IsEventSelected() & AliVEvent::kHighMultSPD)) {
   fHistEvents->Fill(3);
 		mHeader.mEventMask |= kHMSPD;   
  }
  if ((handler->IsEventSelected() & AliVEvent::kTRD)) {
  	TString classes = event->GetFiredTriggerClasses();   
  	if (classes.Contains("HNU")) {
  		fHistEvents->Fill(4);
 			mHeader.mEventMask |= kTRDNHU;     		
  	}
		if (classes.Contains("HQU")) {
  		fHistEvents->Fill(5);
 			mHeader.mEventMask |= kTRDHQU;     		
  	}
  }
...
long AliAnalysisCODEXS3task::GetParticleMask(TParticle *part) {
...
void AliAnalysisCODEXS3task::Discard(){
