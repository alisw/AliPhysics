...
// This is intended only for testing purpose
// Author:
...
ClassImp(AliAnalysisTaskHypertriton3Dev)
...
AliAnalysisTaskHypertriton3Dev::AliAnalysisTaskHypertriton3Dev(TString taskname):
...
//________________________________________________________________________
AliAnalysisTaskHypertriton3Dev::~AliAnalysisTaskHypertriton3Dev(){
  //Destructor
  if(fOutput){
    delete fOutput;
    fOutput = 0;
...
//________________________________________________________________________
Double_t AliAnalysisTaskHypertriton3Dev::GetDCAcut(Int_t part, Double_t dca)const{
  Double_t cut;
...
//________________________________________________________________________
void AliAnalysisTaskHypertriton3Dev::SetConvertedAODVertices(AliESDVertex *ESDvtxp, AliESDVertex *ESDvtxs)const{
...
//________________________________________________________________________
Bool_t AliAnalysisTaskHypertriton3Dev::PassTriggerSelection(UInt_t PhysSelMask){
...
//________________________________________________________________________
Bool_t AliAnalysisTaskHypertriton3Dev::PassCentralitySelection(){
...
//________________________________________________________________________
Bool_t AliAnalysisTaskHypertriton3Dev::PassTrackSelection(AliESDtrack *trk){
...
//________________________________________________________________________
Bool_t AliAnalysisTaskHypertriton3Dev::HasTOF(AliESDtrack *trk, float &beta_tof){
    // TOF signal
    Bool_t TOFon = kFALSE;
...
//________________________________________________________________________
Bool_t AliAnalysisTaskHypertriton3Dev::PassPIDSelection(AliESDtrack *trk, int specie, Bool_t isTOFin, Float_t nsigma_cut){
    //PID selection
...
Double_t AliAnalysisTaskHypertriton3Dev::ComputeSigma(Double_t dc[2], Double_t dc_cov[3]){
...
//________________________________________________________________________
void AliAnalysisTaskHypertriton3Dev::CombineThreeTracks(Bool_t isMatter, TArrayI arrD, TArrayI arrP, TArrayI arrPi, Bool_t cent0, Bool_t cent1){
...
Double_t labelM_deu,labelM_pro,labelM_pio =0;
...
for(Int_t j=0; j<arrD.GetSize(); j++){ // candidate deuteron loop cdeuteron.size()
  ...
  for(Int_t m=0; m<arrP.GetSize(); m++){ // candidate proton loop cproton.size()
    ...
    for(Int_t s=0; s<arrPi.GetSize(); s++ ){ // candidate pion loop cpion.size()
      ...
      if((fCutMass == kTRUE) && (Hypertriton.M() > 3.18)) continue; 
...
//________________________________________________________________________
void AliAnalysisTaskHypertriton3Dev::UserCreateOutputObjects(){
  // Create a TList with Histograms
  // Called once
  //printf("**************************************************\n");
  //printf("AliAnalysisTaskHypertriton3Dev::CreateOutputObjects()\n");
  //printf("**************************************************\n");
...
//________________________________________________________________________
void AliAnalysisTaskHypertriton3Dev::UserExec(Option_t *){
  // Main loop
  ...
  if(!fESDevent){
    printf("AliAnalysisTaskHypertriton3Dev::Exec(): bad ESD\n");
    PostData(1,fOutput);
    return;
  }
...
//________________________________________________________________________
void AliAnalysisTaskHypertriton3Dev::Terminate(Option_t *){
  // Merge output
