AliAnalysisTask *AddTask_jditzel_DoubleHypNucTreeLS(UInt_t triggerMask = (AliVEvent::kAny), Bool_t pidch = kFALSE) {
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    Error("AddTask_jditzel_DoubleHypNucTreeLS", "No analysis manager found.");
    return 0;
  }
  ...
  AliAnalysisTaskDoubleHypNucTreeLS *task = new AliAnalysisTaskDoubleHypNucTreeLS("jditzelTaskDoubleHypNucTreeLS");
  if(!task) return 0x0;
