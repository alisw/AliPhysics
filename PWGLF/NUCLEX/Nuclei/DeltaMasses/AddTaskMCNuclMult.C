AliAnalysisTaskSE *AddTaskMCNuclMult(Bool_t kAOD=kFALSE, Bool_t isMC=kTRUE){
  ...
  snprintf(mytaskName,100,"AliAnalysisMCNuclMult");
  
  AliAnalysisMCNuclMult *task_0[Ntask_0];
  AliAnalysisMCNuclMult *task_1[Ntask_1];
  for(Int_t i=0;i<Ntask_0;i++) {
    task_0[i] = new AliAnalysisMCNuclMult(mytaskName);
    mgr->AddTask(task_0[i]);
  }
  for(Int_t i=0;i<Ntask_1;i++) {
    task_1[i] = new AliAnalysisMCNuclMult(mytaskName);
  ...
  for(Int_t i=0;i<Ntask_0;i++) {
    task_0[i]->SetTrackFilter(trackFilter);
  ...
  for(Int_t i=0;i<Ntask_1;i++) {
    task_1[i]->SetTrackFilter(trackFilter);
