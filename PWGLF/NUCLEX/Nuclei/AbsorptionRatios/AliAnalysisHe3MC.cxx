...
void AliAnalysisHe3MC::UserExec(Option_t *)
{
  ...
  {
    ::Error("AliAnalysisHe3MC::UserExec", "MC handler not initialised.");
    PostData(1, fOutputList);
    return;
  ...
  if (!fVEvent)
  {
    ::Error("AliAnalysisHe3MC::UserExec", "AliVEvent not found.");
    PostData(1, fOutputList);
    return;
  }
  ...
  if (!fAODEvent)
  {
    ::Error("AliAnalysisHe3MC::UserExec", "AliAODEvent not found.");
    PostData(1, fOutputList);
    return;
  }
  mcEvent = MCEvent();
  ...
  if (!mcEvent)
  {
    ::Error("AliAnalysisHe3MC::UserExec", "MCEvent not found.");
    PostData(1, fOutputList);
    return;
  }
  else
  ...
  for (int i = 0; i < nMCtracks; i++)
  {
    ...
    if (not(pdgCode == -2212 or pdgCode == 2212 or pdgCode == -1000010030 or pdgCode == 1000010030 or pdgCode == -1000020030 or pdgCode == 1000020030 or pdgCode == 1000010020))
      continue;
    ...
    case 1000020030:
      ...
      if (isPrimary)
      {
        FillHistosMC((TList *)fMCOutputHe3->At(0), part, fAODEvent, i);
      }
      if (isSecondaryFromWeak)
        FillHistosMC((TList *)fMCOutputHe3->At(1), part, fAODEvent, i);
      if (isSecondaryFromMaterial)
      {
        FillHistosMC((TList *)fMCOutputHe3->At(2), part, fAODEvent, i);
        FillNtuple((TNtuple *)((TList *)((TList *)fMCOutputHe3->At(2)))->At(44), part, fAODEvent, i);
      }
      break;
    case -1000020030:
      //cout<<"anti-3He"<<endl;
      if (isPrimary)
        FillHistosMC((TList *)fMCOutputAHe3->At(0), part, fAODEvent, i);
      if (isSecondaryFromWeak)
        FillHistosMC((TList *)fMCOutputAHe3->At(1), part, fAODEvent, i);
      if (isSecondaryFromMaterial)
        FillHistosMC((TList *)fMCOutputAHe3->At(2), part, fAODEvent, i);
      break;
      /*
        case 1000010020:
            //cout<<"Deuteron"<<endl;
