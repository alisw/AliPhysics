...
#include "AliAnalysisHe3MC.h"

class AliAnalysisHe3MC;
using namespace std;

const char *GetID(int pdgCode);
ClassImp(AliAnalysisHe3MC)

    //________________________________________________________________________
    AliAnalysisHe3MC::AliAnalysisHe3MC()
    : AliAnalysisTaskSE(), fVEvent(0), fAODEvent(0), fOutputList(0), fOutputEvent(0), fOutputProtons(0), fOutputAProtons(0), fOutputDeuterons(0), fOutputADeuterons(0), fOutputHe3(0), fOutputAHe3(0), fMCOutputList(0), fMCOutputEvent(0), fMCOutputProtons(0), fMCOutputAProtons(0), fMCOutputDeuterons(0), fMCOutputADeuterons(0), fMCOutputHe3(0), fMCOutputAHe3(0), fHistTrackCuts(0), fEventStat(0), fVertexZ(0), fVtxContrib(0), fSPDVtxResol(0), fVtxDisplacement(0), fMultV0(0), fCentV0M(0), fHistsProton(), fHistsAProton(), fHistsDeuteron(), fHistsADeuteron(), fPIDResponse(0), fFilterBit(256), fLowPCut(0.0), fHighPCut(1e30), fEtaCut(1.0), fMinClIts(0), fMaxDCAxyCut(10.), fMaxDCAxyAna(10.), fMaxDCAz(10.), fMaxTPCnSigma(10.), fMaxTOFnSigma(10.), fUseTOFPidCut(kFALSE), fMomTOFProt(10.), fMomTOFDeut(10.), fTrackCuts(0)
{
...
//________________________________________________________________________
AliAnalysisHe3MC::AliAnalysisHe3MC(const char *name)
...
//________________________________________________________________________
void AliAnalysisHe3MC::UserCreateOutputObjects()
{
    ...
    fMCOutputDeuterons->SetName("Tritons");
    fMCOutputDeuterons->SetOwner(kTRUE);
    
    fMCOutputADeuterons = new TList();
    fMCOutputADeuterons->SetName("ATritons");
    fMCOutputADeuterons->SetOwner(kTRUE);

	fMCOutputHe3 = new TList();
	fMCOutputHe3->SetName("He3");
	fMCOutputHe3->SetOwner(kTRUE);

	fMCOutputAHe3 = new TList();
	fMCOutputAHe3->SetName("AHe3");
	fMCOutputAHe3->SetOwner(kTRUE);

	fMCOutputTriton = new TList();
	fMCOutputTriton->SetName("Deuterons");
	fMCOutputTriton->SetOwner(kTRUE);
	...
	fMCOutputHe3->Add(primaries->Clone());
	fMCOutputHe3->Add(secondariesW->Clone());
	fMCOutputHe3->Add(secondariesM->Clone());

	fMCOutputAHe3->Add(primaries->Clone());
	fMCOutputAHe3->Add(secondariesW->Clone());
	fMCOutputAHe3->Add(secondariesM->Clone());

	fMCOutputTriton->Add(primaries->Clone());
	...
	fMCOutputList->Add(fMCOutputHe3);
	fMCOutputList->Add(fMCOutputAHe3);
	fMCOutputList->Add(fMCOutputTriton);
...
//________________________________________________________________________
void AliAnalysisHe3MC::UserExec(Option_t *)
{
    // Main loop
	...
	if (!testMCEvent and kFALSE) {
			cout <<"no MC Event found from handler"<<endl;
...
//  
	...
	for (int i=0; i<nMCtracks; i++) {
		...
		if (!part) continue;
		if (part->Pt() <0.2 or part->Eta()>0.8 or part->Eta()<-0.8) continue;//Gets rid of weird particles
		Int_t pdgCode = part->PdgCode();
		if (not (pdgCode==-2212 or pdgCode==2212 or pdgCode==-1000010030 or pdgCode==1000010030 or pdgCode ==-1000020030 or pdgCode==1000020030 or pdgCode==1000010020)) continue;
		// if (not( antiproton 	or proton	 	 or antitriton 		 or triton			or antihelium3			 or helium3 or deuteron)) continue; Pseudocode for legibility
		...
		if (not (isPrimary or isSecondaryFromWeak or isSecondaryFromMaterial)) continue;
		...
		switch (pdgCode) {
				...
				case 2212://changed to \Dueterons for analysis
						...
						if (isPrimary) {FillHistosMC((TList*)fMCOutputProtons->At(0), part, fAODEvent, i); if (testbool) {FillNtuple((TNtuple*)((TList*)((TList*)fMCOutputHe3->At(2)))->At(44), part, fAODEvent, i);}}
				...
				case 1000020030:
						//cout<<"3He"<<endl;
						if (isPrimary) {FillHistosMC((TList*)fMCOutputHe3->At(0), part, fAODEvent, i);}
						if (isSecondaryFromWeak) FillHistosMC((TList*)fMCOutputHe3->At(1), part, fAODEvent, i);
						if (isSecondaryFromMaterial) {FillHistosMC((TList*)fMCOutputHe3->At(2), part, fAODEvent, i);FillNtuple((TNtuple*)((TList*)((TList*)fMCOutputHe3->At(2)))->At(44), part, fAODEvent, i); }
						break;
				case -1000020030:
						//cout<<"anti-3He"<<endl;
						if (isPrimary) FillHistosMC((TList*)fMCOutputAHe3->At(0), part, fAODEvent, i);
						if (isSecondaryFromWeak) FillHistosMC((TList*)fMCOutputAHe3->At(1), part, fAODEvent, i);
						if (isSecondaryFromMaterial) FillHistosMC((TList*)fMCOutputAHe3->At(2), part, fAODEvent, i);
						break;
						/*
				case 1000010020:
						//cout<<"Deuteron"<<endl;
...
//________________________________________________________________________
void AliAnalysisHe3MC::Terminate(Option_t *)
{
...
//________________________________________________________________________
void AliAnalysisHe3MC::CreateHistosTrack(vector<TH1 *> &histos)
{
...
//________________________________________________________________________
void AliAnalysisHe3MC::FillHistosTrack(vector<TH1 *> &histos, AliAODTrack *track)
{
...
void AliAnalysisHe3MC::FillHistosTrack(vector<TH1*> &histos, AliAODTrack *track)
{
    ...
    // if DCAxy > cut for final analysis -> fill only 3d histogram
    if (kFALSE and (TMath::Abs(DCAxy) > fMaxDCAxyAna)){//currently disabled to check what is going on with MC
        ...
        histos.at(0)->Fill(trackP);
		/*
>>>>>>> a17b7ce538fe24d5ba30a9c05b0bb48934283f92
...
//________________________________________________________________________
Float_t AliAnalysisHe3MC::GetTOFBeta(AliAODTrack *track)
{
...
//________________________________________________________________________
Float_t AliAnalysisHe3MC::GetMass2TOF(Float_t beta, AliAODTrack *track)
{
    Float_t p = track->P();
    Float_t mass2sq = -999;
    if(!(beta==0)){
...
void AliAnalysisHe3MC::CreateMCLists(TList* list) {
		...
		cout<<endl<<"HistsVectorSize = "<<fMCHistVector.size()<<endl;
...
void AliAnalysisHe3MC::CreateHistosMC(vector<TH1*> &hists) {
		/* MC histograms. 
		 * 
...
void AliAnalysisHe3MC::FillHistosMC(TList* list, AliAODMCParticle* part, AliAODEvent* fAODEvent, Int_t iMC) {
		...
		if (kFALSE and part->IsSecondaryFromMaterial()) {cout<<part->GetMother()<<endl;}//need to acess and identify mother particle, that should give more insight. 
		...
		for (int itrack=0; itrack<fAODEvent->GetNumberOfTracks(); itrack++) {
				...
				if (reachedTOF) {histos.at(i+9)->Fill(track->P());}
		...
		if (GotSelected==0) {histos.at(i+5)->Fill("Labels not found", 1);}
		if (GotSelected>1) {
...
const char* GetID(int pdgCode) {
		if (pdgCode==111 or pdgCode==-111) return "Pi0";
		if (pdgCode==211) return "Pi+";
		if (pdgCode==-211) return "Pi-";

		if (pdgCode==2212) return "Proton";
		if (pdgCode==-2212) return "AProton";

		if (pdgCode==1000010020) return "Deuteron";
		if (pdgCode==-1000010020) return "ADeuteron";

		if (pdgCode==1000020030) return "He3";
		if (pdgCode==-1000020030) return "AHe3";

		if (pdgCode==1000020040) return "He4";
		if (pdgCode==1000020040) return "AHe4";

		return (const char*)pdgCode;
}

void AliAnalysisHe3MC::FillNtuple(TNtuple* nt, AliAODMCParticle* part, AliAODEvent* fAODEvent, Int_t iMC) {
		AliMCParticle* mcpart = (AliMCParticle*)mcEvent->GetTrack(iMC);
		...
		for (int itrack=0; itrack<fAODEvent->GetNumberOfTracks(); itrack++) {
				...
				//For reference //fNtupleHe3 = new TNtuple("fNtupleHe3", "fNtupleHe3", "p:pt:TPCSignal:TPCnSigmaHe3:DCAxy:DCAz:TOFm2:TPCNClusters:ITSNClusters:TPCClusters4dEdx:Eta:Chi2TPC:Chi2ITS:TPCCrossedRows:label");
				float vars [16];
				vars[0] = track->P();
				vars[1] = track->Pt();
