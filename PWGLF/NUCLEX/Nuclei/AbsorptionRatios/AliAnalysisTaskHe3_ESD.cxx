...
#include "AliAnalysisTaskHe3_ESD.h"

class AliAnalysisTaskHe3_ESD;
using namespace std;

ClassImp(AliAnalysisTaskHe3_ESD)

//________________________________________________________________________
AliAnalysisTaskHe3_ESD::AliAnalysisTaskHe3_ESD()
: AliAnalysisTaskSE()
,fVEvent(0)
,fAODEvent(0)
,fESDEvent(0)
...
//________________________________________________________________________
AliAnalysisTaskHe3_ESD::AliAnalysisTaskHe3_ESD(const char* name)
...
//________________________________________________________________________
void AliAnalysisTaskHe3_ESD::UserCreateOutputObjects()
{
	...
	if (!fPIDResponse) {
			cout<<"PIDResponse not found"<<endl;
	} else {cout<<"PIDResonse found"<<endl;}
	...
	fNtupleHe3 = new TNtuple("fNtupleHe3", "fNtupleHe3", "p:pt:TPCSignal:TPCnSigmaHe3:Charge:TOF_beta:DCAxy:DCAz:TOFm2:TPCNClusters:ITSNClusters:TPCClusters4dEdx:Eta:ITSnSigmaHe3:Chi2TPC:Chi2ITS:TPCCrossedRows:pTPC:TOFdx:TOFdz");
	fNtupleAHe3 = new TNtuple("fNtupleAHe3", "fNtupleAHe3", "p:pt:TPCSignal:TPCnSigmaHe3:Charge:TOF_beta:DCAxy:DCAz:TOFm2:TPCNClusters:ITSNClusters:TPCClusters4dEdx:Eta:ITSnSigmaHe3:Chi2TPC:Chi2ITS:TPCCrossedRows:pTPC:TOFdx:TOFdz");
    ...
    fHe3TOFCutVariation->GetYaxis()->SetBinLabel(1, "TOF distance < 1cm");	
    fHe3TOFCutVariation->GetYaxis()->SetBinLabel(2, "TOF distance < 2cm");	
    fHe3TOFCutVariation->GetYaxis()->SetBinLabel(3, "TOF distance < 3cm");	
    fHe3TOFCutVariation->GetYaxis()->SetBinLabel(4, "TOF distance < 5cm");	
    fHe3TOFCutVariation->GetYaxis()->SetBinLabel(5, "TOF distance < 10cm");	
    fHe3TOFCutVariation->GetYaxis()->SetBinLabel(6, "TOF distance default");	
	fHe3TOFCutVariation->GetXaxis()->SetTitle("p_TPC/Z GeV/c");
...
//________________________________________________________________________
void AliAnalysisTaskHe3_ESD::UserExec(Option_t *)
{
    // Main loop
	...
	*/
	...
	fESDEvent = dynamic_cast<AliESDEvent*>(fVEvent);
	if (!fESDEvent) {

			printf("fESDEvent not available\n");
			return;
	} 
    ...
    if (!isINT7selected){
        PostData(1, fOutputList);
		cout<<"Event selection triggered"<<endl;
        return;
    }
    ...
    if (ClustersVsTrackletBG) {
        PostData(1, fOutputList);
		cout<<"Event selectuin SPD cluster cut"<<endl;
        return;
    }
...
//    }
	...
	*/
    ...
    for (Int_t iTrack = 0; iTrack < fESDEvent->GetNumberOfTracks(); iTrack++){
        ...
        AliESDtrack copy(*track);
        const AliVVertex *ESDeventVtx = copy.GetESDEvent()->GetPrimaryVertex();
        Double_t ESDeventMagneticF = copy.GetESDEvent()->GetMagneticField();
        if (copy.PropagateToDCA(ESDeventVtx, ESDeventMagneticF,10, dcaVals, covar)){
            DCAxy = dcaVals[0];
            DCAz  = dcaVals[1];
        } else {
            DCAxy = -99.; // track->DCA();
		...
		Bool_t isHelium3Selected = (nSigmaTPCHe3 > -7.0) and (nSigmaTPCHe3 < 5); //Helium 3 is triggered in TPC only with a relaxed condition of 5, EDIT: cut reduced to 3 sigma due to large includion of background in the 5 sigma cut. See 16qt set from 260919. Edit2: considered relaxing condition to -10<nSigma<5 in order to better fit the TPCnSigma plots, but it is not necesary is the TPC Signal is cut at 100.
	...
	fVertexZ->Fill(vertex->GetZ());//temporarily here to check if this gets called. 
...
//________________________________________________________________________
void AliAnalysisTaskHe3_ESD::Terminate(Option_t *)
{
    // terminate
...
//________________________________________________________________________
void AliAnalysisTaskHe3_ESD::CreateHistosTrack(vector<TH1*> &histos)
{
...
//________________________________________________________________________
void AliAnalysisTaskHe3_ESD::FillHistosTrack(vector<TH1*> &histos, AliESDtrack *track)
{
    Double_t trackP   = track->P();
    ...
    if (TMath::Abs(DCAxy) > fMaxDCAxyAna){
        ...
        histos.at(10)->Fill(0);//track->Chi2perNDF());
...
//________________________________________________________________________
Float_t AliAnalysisTaskHe3_ESD::GetTOFBeta(AliESDtrack *track)
{
    float beta = -999;
...
//________________________________________________________________________
Float_t AliAnalysisTaskHe3_ESD::GetMass2TOF(Float_t beta, AliESDtrack *track)
{
    Float_t p = track->P();
    Float_t mass2sq = -999;
    if(!(beta==0)){
