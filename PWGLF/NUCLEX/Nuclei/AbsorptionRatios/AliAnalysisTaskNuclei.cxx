...
ClassImp(AliAnalysisTaskNuclei)
...
//________________________________________________________________________
AliAnalysisTaskNuclei::AliAnalysisTaskNuclei(const char* name)
...
void AliAnalysisTaskNuclei::UserCreateOutputObjects()
{
	...
	/*fOutputATriton = new TList();
	fOutputATriton->SetName("ATritons");
	fOutputATriton->SetOwner(kTRUE);

	fOutputTriton = new TList();
	fOutputTriton->SetName("Tritons");
	fOutputTriton->SetOwner(kTRUE);
	...
	*/
	...
	fHe3TOFCutVariation = new TH2F("fHe3TOFCutVariation", "He3 yield as a function of geometric TOF cut", 1000, 0, 10, 6, 0, 6);
    

	fAHe3TOFCutVariation = new TH2F("fAHe3TOFCutVariation", "AHe3 yield as a function of geometric TOF cut", 1000, 0, 10, 6, 0, 6);
	...
	fOutputHe3->Add(fHe3TOFCutVariation);
	...
	fOutputProtons->Add(fNtupleTriton);
	fOutputAProtons->Add(fNtupleATriton);
	/*CreateHistosTrack(fHistsATriton);
	for (Int_t i=0;i<(int)(fHistsATriton.size()); i++){
		fOutputATriton->Add(fHistsATriton.at(i));
	}*/
	//CreateHistosTrack(fHistsTriton);
	/*for (Int_t i=0;i<(int)(fHistsTriton.size()); i++){
		fOutputTriton->Add(fHistsTriton.at(i));
	}*/
...
//________________________________________________________________________
void AliAnalysisTaskNuclei::UserExec(Option_t *)
{
    // Main loop
...
//________________________________________________________________________
void AliAnalysisTaskNuclei::CreateHistosTrack(vector<TH1*> &histos)
{
    ...
    TH2F* fHistTPCnSigmaProt = new TH2F("fHistTPCnSigmaTriton","TPC nSigma Triton;#it{p}/Z, GeV/#it{c};TPCn#sigma_{prot}",100,0.,10.0,100,-5.,5.);
    histos.push_back(fHistTPCnSigmaProt);
    
    TH2F* fHistTPCnSigmaDeut = new TH2F("fHistTPCnSigmaHe4","TPC nSigma He4;#it{p}/Z, GeV/#it{c};TPCn#sigma_{deut}",100,0.,10.0,100,-5.,5.);
    ...
    TH3F* fHistTOFmass2_DCAxy_p = new TH3F("fHistTOFmass2_DCAxy_p","TOF mass2 vs DCA_{xy} vs #it{p}/Z;#it{p}/Z, GeV/#it{c};TOF mass^{2} / Z^{2}, (GeV/#it{c}^{2})^{2};DCA_{xy}, cm;",120,0.,12.,1000,0.0,10.0,200,-1.0,1.0);
...
/*
	TH2F* fHistTPCnSigmaTriton = new TH2F("fHistTPCnSigmaTriton", "fHistTPCnSigmaTriton", 200, 0, 10, 100, -5, 5);
	histos.push_back(fHistTPCnSigmaTriton);

	TH3F* fHistTPCnSigmaTriton_TOF = new TH3F("fHistTPCnSigmaTriton_TOF", "fHistTPCnSigmaTriton_TOF", 200, 0, 10, 100, -5, 5, 100, 0, 10);
	histos.push_back(fHistTPCnSigmaTriton_TOF);
*/
...
//________________________________________________________________________
void AliAnalysisTaskNuclei::FillHistosTrack(vector<TH1*> &histos, AliAODTrack *track)
{
    Double_t trackP   = track->P();
    ...
    // PID information
    Float_t nSigmaTPCprot = fPIDResponse->NumberOfSigmasTPC(track, AliPID::kTriton);
    Float_t nSigmaITSprot = fPIDResponse->NumberOfSigmasITS(track, AliPID::kProton);
    Float_t nSigmaTOFprot = fPIDResponse->NumberOfSigmasTOF(track, AliPID::kProton);
    
    Float_t nSigmaTPCdeut = fPIDResponse->NumberOfSigmasTPC(track, AliPID::kAlpha);
    Float_t nSigmaITSdeut = fPIDResponse->NumberOfSigmasITS(track, AliPID::kDeuteron);
    Float_t nSigmaTOFdeut = fPIDResponse->NumberOfSigmasTOF(track, AliPID::kDeuteron);

    Float_t nSigmaTPCHe3 = fPIDResponse->NumberOfSigmasTPC(track, AliPID::kHe3);
    Float_t nSigmaITSHe3 = fPIDResponse->NumberOfSigmasITS(track, AliPID::kTriton);
    Float_t nSigmaTOFHe3 = fPIDResponse->NumberOfSigmasTOF(track, AliPID::kTriton);
    
    Float_t nSigmaTOFpion = fPIDResponse->NumberOfSigmasTOF(track, AliPID::kPion);

    Float_t nSigmaTPCTriton = fPIDResponse->NumberOfSigmasTPC(track, AliPID::kPion);
    ...
    if (TMath::Abs(DCAxy) > fMaxDCAxyAna){
		...
		//histos.at(28)->Fill(trackP, nSigmaTPCTriton);
		//((TH3F*)histos.at(29))->Fill(trackP, nSigmaTPCTriton, fmass2TOF);
		/*
...
//________________________________________________________________________
Float_t AliAnalysisTaskNuclei::GetTOFBeta(AliAODTrack *track)
{
    float beta = -999;
...
//________________________________________________________________________
Float_t AliAnalysisTaskNuclei::GetMass2TOF(Float_t beta, AliAODTrack *track)
{
    Float_t p = track->P();
    Float_t mass2sq = -999;
    if(!(beta==0)){
