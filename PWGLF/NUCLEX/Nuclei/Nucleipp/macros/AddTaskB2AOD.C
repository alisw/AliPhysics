...
AliAnalysisTaskB2AOD* AddTaskB2AOD(  const TString& species
...
//
	...
	if (!mgr)
	{
		cerr << "AddTaskB2AOD: no analysis manager to connect to" << endl;
		return 0;
	}
	
	if (!mgr->GetInputEventHandler())
	{
		cerr << "AddTaskB2AOD: this task requires an input event handler" << endl;
		return 0;
	}
	
	// Create and configure the task
	
	AliAnalysisTaskB2AOD* task = new AliAnalysisTaskB2AOD(Form("B2.%s",containername.Data()));
	...
	AliLnAODtrackCuts* trkCuts = new AliLnAODtrackCuts();
	
	trkCuts->SetSelectionCriteria(trksel);
	trkCuts->SetMaxDCAxy(maxDCAxy);
	trkCuts->SetMaxDCAz(maxDCAz);
	trkCuts->SetMaxNSigmaToVtx(kMaxNSigma);
	trkCuts->SetMaxEta(maxEta);
	trkCuts->SetTPCXRowsCut(xRowsTPC);
	trkCuts->SetMinTPCnClsOrXRows(minTPCnClsOrXRows);
	
	task->SetTrackCuts(trkCuts);
	...
	AliAnalysisDataContainer* output = mgr->CreateContainer(containername.Data(), TList::Class(), AliAnalysisManager::kOutputContainer, Form("%s:AliAnalysisTaskB2AOD", AliAnalysisManager::GetCommonFileName()));
