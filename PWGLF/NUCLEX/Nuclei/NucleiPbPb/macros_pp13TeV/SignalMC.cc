...
void SignalMC(bool useMBsignal=true, bool use_extended=true) {
  ...
  TFile input_file(Form("%sMCasData/MCasData.root",kBaseInputDir.data()));
  ...
  for (auto list_key : *input_file.GetListOfKeys()) {
    ...
    for (int iS = 1; iS>=0; iS--) {
      for (int iC = 0; iC < kCentLength; ++iC) {
        ...
        hTPC3sigma[iS][iC] = new TH1D(Form("hTPC3sigma%c%i",kLetter[iS],iC),";p_{T} GeV/c; TPC raw counts (3#sigma)",n_pt_bins,pt_labels.GetArray());
      ...
      for (int iS = 1; iS>=0; iS--) {
        ...
        for (int iC = kCentLength-1; iC>=kCentLength-1;iC--) {
          ...
          if(pt_axis->GetBinCenter(iB+1) < kTPCmaxPt){
            ...
            hTPC3sigma[iS][iC]->SetBinContent(iB + 1, sValTPC);
            hTPC3sigma[iS][iC]->SetBinError(iB + 1, sErrTPC);
    ...
    for (int iS = 0; iS < 2; ++iS) {
      for (int iC = 0; iC < kCentLength; ++iC) {
        ...
        base_dir->cd(Form("%s/TPC3sigma",kNames[iS].data()));
        hTPC3sigma[iS][iC]->Write();
