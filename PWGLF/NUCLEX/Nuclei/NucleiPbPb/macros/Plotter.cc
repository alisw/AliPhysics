...
void Plotter(bool bTPC = false, bool short_mode = true){
  ...
  string output_name = (bTPC) ? "/Users/lbariogl/cernbox/Deuterons13TeV/results/plotsTPC.root" : "/Users/lbariogl/cernbox/Deuterons13TeV/results/plots.root";
    ...
    for(int iS=0; iS<2; iS++){
      ...
      for(int iC=0; iC<kCentLength; iC++){
        ...
        for(int iB=1; iB<=kPtBinLimit[iC]; iB++){
          ...
          fPlot->Draw();
          if((iPad+1)%6 == 0){
            fCanvas->Print(Form("/Users/lbariogl/cernbox/Deuterons13TeV/results/plots/cent_%d_%c_%d.pdf",iC,kLetter[iS],page_counter));
            fCanvas->Write();
            page_counter++;
          }
          iPad++;
