...
// with infinities.
const double JunctionSplitting::MINDELTAR = 1e-7; 
  ...
  for (int i = 0; i < int(junChains.size()); ++i) {
    if (junChains[i].size() == 2) {
      ...
      // Split into string if two legs are combined.
        ...
        if ( REtaPhi(pJunLeg0,pJunLeg1)   < MINDELTAR ||
             REtaPhi(pAntiLeg0,pAntiLeg1) < MINDELTAR ||
             REtaPhi(pJunLeg0,pAntiLeg0)  < MINDELTAR ||
             REtaPhi(pJunLeg0,pAntiLeg1)  < MINDELTAR ||
             REtaPhi(pJunLeg1,pAntiLeg0)  < MINDELTAR ||
             REtaPhi(pJunLeg1,pAntiLeg1)  < MINDELTAR) {
      ...
      for (int iJunLocal = 0; iJunLocal < 2; ++iJunLocal) {
        ...
        do {
          ...
          double wt2 = -1e5; 
	  if(-pInRF[2].e()/eNormJunction < 10) wt2 = 1. - exp( -pInRF[2].e() / eNormJunction);
          double wt3 = -1e5;
	  if(-pInRF[3].e()/eNormJunction < 10) wt3 = 1. - exp( -pInRF[3].e() / eNormJunction);
      ...
      double vJvAe2y = vJvA;
      if ((vJvA*vJvA - 1.) > 0.) vJvAe2y += sqrt(vJvA*vJvA - 1.);
