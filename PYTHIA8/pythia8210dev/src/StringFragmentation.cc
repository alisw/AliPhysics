...
Vec4 StringEnd::kinematicsHadron( StringSystem& system) {
  ...
  for (int iStep = 0; ; ++iStep) {
    ...
    double denom = cM2 + cM4 * xInvHad;
    if (abs(denom) < TINY) return Vec4(0., 0., 0., -1.);
    xDirHad      = (cM0 - cM3 * xInvHad) / denom;
...
RotBstMatrix StringFragmentation::junctionRestFrame(Vec4& p0, Vec4& p1,
  ...
  for (int iTry = 0; iTry < 3; ++iTry) {
    ...
    // avoid too small or negative values
    if (pipj < 1e-6)
      pipj = 1e-6;
    if (pipk < 1e-6)
      pipk = 1e-6;
    ...
    // Else find three-momentum range for parton i and values at extremes.
      ...
      double eiMax = (pipj + pipk) / sqrt(max(0., m2j) + max(0., m2k) + 2. * pjpk);
      ...
      double pkMax = (eiMax * sqrtpos( pipk*pipk - m2k * temp )
        - 0.5 * piMax * pipk) / temp;
      double ejMax = 1e-06;
      if(pjMax*pjMax + m2j > 1e-06) ejMax = sqrt(pjMax*pjMax + m2j);
      double ekMax = 1e-06;
      if(pkMax*pkMax + m2k > 1e-06) ekMax = sqrt(pkMax*pkMax + m2k);
      ...
      for (int iter = 0; iter < NTRYJRFEQ; ++iter) {
        ...
        double pk = (ei * sqrtpos( pipk*pipk - m2k * temp )
          - 0.5 * pi * pipk) / temp;
        ej = sqrt(pj*pj + max(0., m2j));
        ek = sqrt(pk*pk + max(0., m2k));
  ...
  double denom     = max(1e-6, pDiff01 * pDiff02 - pDiff0102 * pDiff0102);
