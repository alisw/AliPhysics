...
double StringLength::getJuncLength(Vec4 p1, Vec4 p2, Vec4 p3) {
  ...
  if( p1*p2 > 1e-5 || p2*p3 > 1e-5 || p3*p1 > 1e-5) return 1e9;
...
double StringLength::getJuncLength(Vec4 p1, Vec4 p2, Vec4 p3, Vec4 p4) {
  ...
  double returnValue = getLength(p1, v1, true) + getLength(p2, v1, true)
    + getLength(p3, v2, true) + getLength(p4, v2, true);
  
  if (pow2(v1*v2)-1 > 0)
    returnValue += log(v1*v2 + sqrt(pow2(v1*v2)-1));
  else 
    returnValue += log(v1*v2);
