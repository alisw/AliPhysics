...
Int_t AliTPCMonitor::ReadData(Int_t secid)
{
  ...
  else         {return  ReadDataDATE(secid,format);}
} 
//__________________________________________________________________
Int_t AliTPCMonitor::ReadDataDATE(Int_t secid, Int_t format ) 
{
  ...
  if(MON)         { if(fReaderDATE)    { delete fReaderDATE    ; fReaderDATE   =0; }}
  else            { if(fReaderDATEMon) { delete fReaderDATEMon ; fReaderDATEMon=0; }}
  ...
  // Create objects ///
  if( !MON     && (fReaderDATE==0 || ( fReaderDATE!=0 && (strcmp(GetLastProcFile(),GetFile())!=0) ))  ) 
    {
  ...
  else if(MON && (fReaderDATEMon==0 || ( fReaderDATEMon!=0 && (strcmp(GetLastProcFile(),GetFile())!=0) )))
    {
      if(fReaderDATEMon!=0) 
	{
      ...
      cout << " Creating new Date monitoring Object "<< endl;
  ...
  if(!MON) {  eventPtr  = fReaderDATE->GetMemoryPointer();         }
  else     {  eventPtr  = fReaderDATEMon->GetEventPointerasChar(); }
  ...
  if(!MON && fReaderDATE->IsLastEvent()) fLastEv =1;
  if(DateForm->IsEventEndOfRun()) {  AliInfo("Event is end of run: eventType END_OF_RUN "); }
  
  if( !MON && fReaderDATE->IsEventValid() == false )   {AliInfo("Skipping Event because invalid");                     return 10;   } 
  ...
  delete DateForm;
  if(fVerb) cout << " last rcu " << last_rcu_id << endl; 
  return last_rcu_id;
}
