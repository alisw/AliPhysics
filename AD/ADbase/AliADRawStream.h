...
public :
  ...
  ULong64_t GetBBScalers(Int_t channel)                   const  { return fBBScalers[channel]; }
  ULong64_t GetBGScalers(Int_t channel)                   const  { return fBGScalers[channel]; }
  UInt_t    GetTriggerScalers(Int_t num_scaler)           const  { return fScalers[num_scaler]; }
  UInt_t    GetBunchNumbersMB(Int_t num_bunch)            const  { return fBunchNumbers[num_bunch]; }
  UShort_t  GetChargeMB(Int_t channel, Int_t num_bunch)   const  { return fChargeMB[channel][num_bunch]; }
  Bool_t    GetIntMBFlag(Int_t channel, Int_t num_bunch)  const  { return fIsIntMB[channel][num_bunch]; }
  Bool_t    GetBBMBFlag(Int_t channel, Int_t num_bunch)   const  { return fIsBBMB[channel][num_bunch]; }
  Bool_t    GetBGMBFlag(Int_t channel, Int_t num_bunch)   const  { return fIsBGMB[channel][num_bunch]; }

  // Getters of ADC signals, ADC pedestals, time information and corresponding flags :
  Short_t   GetADC(Int_t channel)                         const { return TMath::MaxElement(kNEvOfInt, fADC[channel]); }    // maximum value instead of central clock
  //    { return fADC[channel][kNEvOfInt/2]; }

  Short_t   GetPedestal(Int_t channel, Int_t event)       const  { return fADC[channel][20-event]; }
  Bool_t    GetIntegratorFlag(Int_t channel, Int_t event) const  { return fIsInt[channel][20-event]; }
  Bool_t    GetBBFlag(Int_t channel, Int_t event)         const  { return fIsBB[channel][20-event]; }
  Bool_t    GetBGFlag(Int_t channel, Int_t event)         const  { return fIsBG[channel][20-event]; }
  Short_t   GetTime(Int_t channel)                        const  { return fTime[channel]; }
  Short_t   GetWidth(Int_t channel)                       const  { return fWidth[channel]; }

  UShort_t  GetTriggerInputs()                            const  { return fTrigger; }
  UShort_t  GetTriggerInputsMask()                        const  { return fTriggerMask; }
