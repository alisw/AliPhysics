...
- Output Files: local files  AD0_HistosMON.root, AD0_PedestalsMON_On.dat (Online mapping)
                FXS file     AD0_PedestalsMON_Off.dat (Offline mapping)
...
* It computes calibration parameters, populates local "./AD0_PedestalsMON_On.dat"   *            
...
// Get cuts from AD0_PedestalMON_DA.config file
  ...
  status = daqDA_DB_getFile("AD0_PedestalMON_DA.config","./AD0_PedestalMON_DA.config");
  if (status) {
      printf("Failed to get Config file (AD0_PedestalMON_DA.config) from DAQ DB, status=%d\n", status);
      ...
      FILE *fpConfig = fopen("AD0_PedestalMON_DA.config","r");
      int res = fscanf(fpConfig,"%d %d %d %d %d %d %f",&kClockMin,&kClockMax,&kLowCut,&kHighCut,&kClockMinRef,&kClockMaxRef,&kChi2Max);
      if(res!=7) {
	    printf("Failed to get values from Config file (AD0_PedestalMON_DA.config): wrong file format - 7 integers are expected - \n");
	    kClockMin  =  16; 
        kClockMax  =  20; 
    	kLowCut    =  60;   
        kHighCut   =  50; 
...
//___________________________________________________ 
  ...
  FILE *fpLocal=NULL;
  fpLocal=fopen("./AD0_PedestalsMON_On.dat","w");
  if (fpLocal==NULL) {
  ...
  /* open result file to be exported to FES */
  FILE *fp=NULL;
  fp=fopen("./AD0_PedestalsMON_Off.dat","w");
  if (fp==NULL) {
...
// Write root file with histos for users further check - just in case - 
  ...
  TFile *histoFile = new TFile("AD0_PedestalsMON.root","RECREATE");
...
//________________________________________________________________________
  ...
  status=daqDA_FES_storeFile("./AD0_PedestalsMON_Off.dat","AD0da_results");
  if (status)    {
      printf("Failed to export file : %d\n",status);
      return -1; }

  /* store result file into Online DB */
  status=daqDA_DB_storeFile("./AD0_PedestalsMON_On.dat","AD0da_results");
  if (status)    {
