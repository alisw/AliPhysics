...
#include <THashList.h>
#include <Tracks/AliAnalysisTaskEmcalOfflinePatchesRef.h>
#include <TString.h>
...
void AliAnalysisTaskEmcalOfflinePatchesRef::UserExec(Option_t *){
  ...
  for(TIter patchiter = TIter(patches).Begin(); patchiter != TIter::End(); ++patchiter){
    ...
    //if(mypatch->IsLevel0Simple()) FillTriggerPatchHistos("EL0", mypatch, supermoduleID, sector);
    if(mypatch->IsGammaHighSimple()) FillTriggerPatchHistos("EG1", mypatch, supermoduleID, sector);
    if(mypatch->IsGammaLowSimple()) FillTriggerPatchHistos("EG2", mypatch, supermoduleID, sector);
  }
 ...
 */
void AliAnalysisTaskEmcalOfflinePatchesRef::FillTriggerPatchHistos(const char *patchtype, const AliEmcalTriggerPatchInfo * const recpatch, Int_t supermoduleID, Int_t sector){
