...
Bool_t loadRunOptionsPWGJE(Bool_t verb,const char *path) {
  ifstream optionsfile;
  TString currline;

  if (bOptionsLoaded) CleanOptions();

  TString szTheFileName = "runoptionsPWGJE.txt";
  ...
  if (currline.BeginsWith("Is Run2 dataset")) {
    ...
    if (currline.Contains("yes"))
      ...
      { printf("ERROR: wrong Is Run2 dataset option in options file %s\n", filename); return kFALSE; }
  }
  else
    //    { printf("ERROR: wrong Is 2015 dataset option in options file %s\n", filename); return kFALSE; }
    { printf("ERROR: wrong Is Run2 dataset option in options file %s\n", filename); return kFALSE; }
  //  printf("  Is 2015 dataset: %s\n", b2015DataSet ? "yes" : "no");
  printf("  Is Run2 dataset: %s\n", bRun2DataSet ? "yes" : "no");
...
//     }
      ...
      if (currline.BeginsWith("Use AliEventCuts: ")) {
        currline.Remove(0, strlen("Use AliEventCuts: "));
        if (currline.Contains("yes"))
          bUseAliEventCuts = kTRUE;
        else if (currline.Contains("no"))
          bUseAliEventCuts = kFALSE;
        else
          { printf("ERROR: wrong Use AliEventCuts option in options file %s\n", filename); return -1; }
        printf ("      Use AliEventCuts: %s\n", bUseAliEventCuts ? "yes" : "no");
        currline.ReadLine(optionsfile);
        while (currline.BeginsWith("#") || currline.IsWhitespace()) currline.ReadLine(optionsfile);
      } /* end use only events validated for centrality calibration */
    }    
