#ifndef ALIANALYSISTASKCHECKDEADCONE_H
#define ALIANALYSISTASKCHECKDEADCONE_H
...
class AliAnalysisTaskCheckDeadCone : public AliAnalysisTaskEmcalJet {
public:
  enum JetShapeType {
  ...
  AliAnalysisTaskCheckDeadCone();
  AliAnalysisTaskCheckDeadCone(const char *name);
  virtual ~AliAnalysisTaskCheckDeadCone();
...
protected:
  ...
  void IterativeParentsMCAveragePP(AliEmcalJet *fJet, Int_t km);
...
private:
  AliAnalysisTaskCheckDeadCone(
      const AliAnalysisTaskCheckDeadCone &); // not implemented
  AliAnalysisTaskCheckDeadCone &
  operator=(const AliAnalysisTaskCheckDeadCone &); // not implemented

  ClassDef(AliAnalysisTaskCheckDeadCone, 10)
};
#endif
