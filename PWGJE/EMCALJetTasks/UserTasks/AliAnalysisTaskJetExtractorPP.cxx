 ...
 *                                                                        *
 * Author: R. Haake with modifications by S. Bysiak.                      *
...
#include "AliAnalysisTaskJetExtractorPP.h"

/// \cond CLASSIMP
ClassImp(AliEmcalJetTreePP)
/// \endcond

/// \cond CLASSIMP
ClassImp(AliAnalysisTaskJetExtractorPP)
/// \endcond
...
Int_t AliHFJetsTaggingVertex::FindVertices(const AliEmcalJet* jet,
                                           ...
                                           Int_t&             nDauRejCount)
{
  std::cout << "My FindVertices()" << std::endl;
...
AliEmcalJetTreePP::AliEmcalJetTreePP() : TNamed("CustomTree", "CustomTree"), fJetTree(0), fInitialized(0), fExtractionPercentages(), fExtractionPercentagePtBins(), fExtractionJetTypes_HM(), fExtractionJetTypes_PM()
{
  // For these arrays, we need to reserve memory
  fBuffer_Track_Pt         = new Float_t[kMaxNumConstits];
  fBuffer_Track_Eta        = new Float_t[kMaxNumConstits];
  fBuffer_Track_Phi        = new Float_t[kMaxNumConstits];
  fBuffer_Track_Charge     = new Float_t[kMaxNumConstits];
  fBuffer_Track_Label      = new Int_t  [kMaxNumConstits];
  fBuffer_Track_ProdVtx_X  = new Float_t[kMaxNumConstits];
  fBuffer_Track_ProdVtx_Y  = new Float_t[kMaxNumConstits];
  fBuffer_Track_ProdVtx_Z  = new Float_t[kMaxNumConstits];

  fBuffer_Cluster_Pt       = new Float_t[kMaxNumConstits];
  fBuffer_Cluster_E        = new Float_t[kMaxNumConstits];
  fBuffer_Cluster_Eta      = new Float_t[kMaxNumConstits];
  fBuffer_Cluster_Phi      = new Float_t[kMaxNumConstits];
  fBuffer_Cluster_M02      = new Float_t[kMaxNumConstits];
  fBuffer_Cluster_Time     = new Float_t[kMaxNumConstits];
  fBuffer_Cluster_Label    = new Int_t[kMaxNumConstits];
}

//________________________________________________________________________
AliEmcalJetTreePP::AliEmcalJetTreePP(const char* name) : TNamed(name, name), fJetTree(0), fInitialized(0), fExtractionPercentages(), fExtractionPercentagePtBins(), fExtractionJetTypes_HM(), fExtractionJetTypes_PM()
{
  // For these arrays, we need to reserve memory
  fBuffer_Track_Pt         = new Float_t[kMaxNumConstits];
  fBuffer_Track_Eta        = new Float_t[kMaxNumConstits];
...
//________________________________________________________________________
Bool_t AliEmcalJetTreePP::AddJetToTree(AliEmcalJet* jet, Bool_t saveConstituents, Bool_t saveConstituentsIP, Bool_t saveCaloClusters, Double_t* vertex, Float_t rho, Float_t rhoMass, Float_t centrality, Int_t multiplicity, Long64_t eventID, Float_t magField)
{
  if(!fInitialized)
...
//________________________________________________________________________
void AliEmcalJetTreePP::FillBuffer_TriggerTracks(std::vector<Float_t>& triggerTrackPt, std::vector<Float_t>& triggerTrackDeltaEta, std::vector<Float_t>& triggerTrackDeltaPhi)
{
...
//________________________________________________________________________
void AliEmcalJetTreePP::FillBuffer_ImpactParameters(std::vector<Float_t>& trackIP_d0, std::vector<Float_t>& trackIP_z0, std::vector<Float_t>& trackIP_d0cov, std::vector<Float_t>& trackIP_z0cov)
{
...
//________________________________________________________________________
void AliEmcalJetTreePP::FillBuffer_PID(std::vector<Float_t>& trackPID_ITS, std::vector<Float_t>& trackPID_TPC, std::vector<Float_t>& trackPID_TOF, std::vector<Float_t>& trackPID_TRD, std::vector<Short_t>& trackPID_Reco, std::vector<Int_t>& trackPID_Truth)
{
...
//________________________________________________________________________
void AliEmcalJetTreePP::FillBuffer_JetShapes(AliEmcalJet* jet, Double_t leSub_noCorr, Double_t angularity, Double_t momentumDispersion, Double_t trackPtMean, Double_t trackPtMedian)
{
...
//________________________________________________________________________
void AliEmcalJetTreePP::FillBuffer_Splittings(std::vector<Float_t>& splittings_radiatorE, std::vector<Float_t>& splittings_kT, std::vector<Float_t>& splittings_theta, Bool_t saveSecondaryVertices, std::vector<Int_t>& splittings_secVtx_rank, std::vector<Int_t>& splittings_secVtx_index)
{
...
//________________________________________________________________________
void AliEmcalJetTreePP::FillBuffer_SecVertices(std::vector<Float_t>& secVtx_X, std::vector<Float_t>& secVtx_Y, std::vector<Float_t>& secVtx_Z, std::vector<Float_t>& secVtx_Mass, std::vector<Float_t>& secVtx_Lxy, std::vector<Float_t>& secVtx_SigmaLxy, std::vector<Float_t>& secVtx_Chi2, std::vector<Float_t>& secVtx_Dispersion)
{
...
//________________________________________________________________________
AliAnalysisTaskJetExtractorPP::AliAnalysisTaskJetExtractorPP() :
  AliAnalysisTaskEmcalJet("AliAnalysisTaskJetExtractorPP", kTRUE),
  ...
  fSimpleSecVertices()
{
  ...
  fJetTree = new AliEmcalJetTreePP(GetName());
  DefineOutput(2, TTree::Class());
}

//________________________________________________________________________
AliAnalysisTaskJetExtractorPP::AliAnalysisTaskJetExtractorPP(const char *name) :
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::UserCreateOutputObjects()
{
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::ExecOnce()
{
...
//________________________________________________________________________
Bool_t AliAnalysisTaskJetExtractorPP::Run()
{
  // ################################### EVENT SELECTION
  // For debugging
...
//________________________________________________________________________
Bool_t AliAnalysisTaskJetExtractorPP::IsTriggerTrackInEvent()
{
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::CalculateJetShapes(AliEmcalJet* jet, Double_t& leSub_noCorr, Double_t& angularity, Double_t& momentumDispersion, Double_t& trackPtMean, Double_t& trackPtMedian)
{
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::GetTrueJetPtFraction(AliEmcalJet* jet, Double_t& truePtFraction, Double_t& truePtFraction_mcparticles)
{
  // #################################################################################
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::GetMatchedJetObservables(AliEmcalJet* jet, Double_t& matchedJetPt, Double_t& matchedJetDistance, Double_t& matchedJetMass, Double_t& matchedJetAngularity, Double_t& matchedJetpTD, Bool_t isPartLevelMatching)
{
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::GetJetType(AliEmcalJet* jet, Int_t& typeHM, Int_t& typePM, Int_t& typeIC)
{
  if(!fMCParticleArray)
    return;
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::GetTrackImpactParameters(const AliVVertex* vtx, const AliAODTrack* track, Float_t& d0, Float_t& d0cov, Float_t& z0, Float_t& z0cov)
{
  if (track)
  {
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::ReconstructSecondaryVertices(const AliVVertex* primVtx, const AliEmcalJet* jet, std::vector<Float_t>& secVtx_X, std::vector<Float_t>& secVtx_Y, std::vector<Float_t>& secVtx_Z, std::vector<Float_t>& secVtx_Mass, std::vector<Float_t>& secVtx_Lxy, std::vector<Float_t>& secVtx_SigmaLxy, std::vector<Float_t>& secVtx_Chi2, std::vector<Float_t>& secVtx_Dispersion)
{
  if(!primVtx)
    return;
...
//________________________________________________________________________
Bool_t AliAnalysisTaskJetExtractorPP::IsStrangeJet(AliEmcalJet* jet)
{
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::FillEventControlHistograms()
{
  fMultiplicity = 0;
  for(Int_t iCont=0; iCont<fParticleCollArray.GetEntriesFast(); iCont++)
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::FillJetControlHistograms(AliEmcalJet* jet)
{
  // ### Jet control plots
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::FillTrackControlHistograms(AliVTrack* track)
{
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::AddPIDInformation(const AliVParticle* particle, Float_t& sigITS, Float_t& sigTPC, Float_t& sigTOF, Float_t& sigTRD, Short_t& recoPID, Int_t& truePID)
{
  truePID = 9;
  if(!particle) return;
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::GetJetSplittings(AliEmcalJet* jet, std::vector<Float_t>& splittings_radiatorE, std::vector<Float_t>& splittings_kT, std::vector<Float_t>& splittings_theta, std::vector<Int_t>& splittings_secVtx_rank, std::vector<Int_t>& splittings_secVtx_index)
{
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::PrintConfig()
{
  std::cout << "#########################################\n";
...
//________________________________________________________________________
inline Bool_t AliAnalysisTaskJetExtractorPP::IsTrackInCone(const AliVParticle* track, Double_t eta, Double_t phi, Double_t radius)
{
...
//________________________________________________________________________
inline Bool_t AliAnalysisTaskJetExtractorPP::IsClusterInCone( TLorentzVector clusterMomentum, Double_t eta, Double_t phi,Double_t radius)
{
...
//________________________________________________________________________
inline void AliAnalysisTaskJetExtractorPP::FillHistogram(const char * key, Double_t x)
{
...
//________________________________________________________________________
inline void AliAnalysisTaskJetExtractorPP::FillHistogram(const char * key, Double_t x, Double_t y)
{
  TH1* tmpHist = static_cast<TH1*>(fOutput->FindObject(key));
  if(!tmpHist)
  {
...
//________________________________________________________________________
inline void AliAnalysisTaskJetExtractorPP::FillHistogram(const char * key, Double_t x, Double_t y, Double_t add)
{
...
//________________________________________________________________________
inline void AliAnalysisTaskJetExtractorPP::FillHistogram3D(const char * key, Double_t x, Double_t y, Double_t z, Double_t add)
{
...
//________________________________________________________________________
template <class T> T* AliAnalysisTaskJetExtractorPP::AddHistogram1D(const char* name, const char* title, const char* options, Int_t xBins, Double_t xMin, Double_t xMax, const char* xTitle, const char* yTitle)
{
...
//________________________________________________________________________
template <class T> T* AliAnalysisTaskJetExtractorPP::AddHistogram2D(const char* name, const char* title, const char* options, Int_t xBins, Double_t xMin, Double_t xMax, Int_t yBins, Double_t yMin, Double_t yMax, const char* xTitle, const char* yTitle, const char* zTitle)
{
...
//________________________________________________________________________
template <class T> T* AliAnalysisTaskJetExtractorPP::AddHistogram3D(const char* name, const char* title, const char* options, Int_t xBins, Double_t xMin, Double_t xMax, Int_t yBins, Double_t yMin, Double_t yMax, Int_t zBins, Double_t zMin, Double_t zMax, const char* xTitle, const char* yTitle, const char* zTitle)
{
...
//________________________________________________________________________
void AliAnalysisTaskJetExtractorPP::Terminate(Option_t *)
{
...
// ### ADDTASK MACRO
//________________________________________________________________________
AliAnalysisTaskJetExtractorPP* AliAnalysisTaskJetExtractorPP::AddTaskJetExtractorPP(TString trackArray, TString clusterArray, TString jetArray, TString rhoObject, Double_t jetRadius, AliRDHFJetsCutsVertex* vertexerCuts, const char* taskNameSuffix)
{
  ...
  // ###### Task name
  TString name("AliAnalysisTaskJetExtractorPP");
  if (jetArray != "") {
    name += "_";
    name += jetArray;
  }
  ...
  AliAnalysisTaskJetExtractorPP* myTask = new AliAnalysisTaskJetExtractorPP(name);
