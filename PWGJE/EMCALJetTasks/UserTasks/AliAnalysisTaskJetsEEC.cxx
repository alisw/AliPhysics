    ...
    //(jet, jet container, vector of jets, vector of constituents within each jet?)
{
    ...
    for (auto part: fJet->GetParticleConstituents())
    {
        ...
        if(fUnfolding==1)
        {
            ...
            {
                fJet_pt_det = jet_pt;
                fTrack_eta_det = fConstituents[j].eta();
                fTrack_phi_det = fConstituents[j].phi();
                fTrack_pt_det = fConstituents[j].pt();
                fJet_pt_tru = 0;
                fTrack_eta_tru = 0;
                fTrack_phi_tru = 0;
                fTrack_pt_tru = 0;
