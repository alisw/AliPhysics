...
void AliAnalysisTaskJetChem::UserCreateOutputObjects()
{
  ...
  */
  ...
  fhnK0sEmbConeStandard             = new THnSparseF("fhnK0sEmbConeStandard","emb jet pT; K0s inv. mass, emb. UE V0 subtraction; particle pT; particle #eta",4,binsK0sEmbConeStandard,xminK0sEmbConeStandard,xmaxK0sEmbConeStandard);
  ...
  */
  ...
  if(saveLevel>0){
    ...
    if(fUseExtraTracks == 1){fCommonHistList->Add(fhnK0sEmbConeRef);
    ...
    if(fUseExtraTracks == 1){fCommonHistList->Add(fhnLaEmbConeRef);
    ...
    if(fUseExtraTracks == 1){fCommonHistList->Add(fhnALaEmbConeRef);
      fCommonHistList->Add(fhnLaEmbConeStandard);
    }
 ...
 //standard OCs method:
  ...
  for(Int_t ij=0; ij<nRecJetsCuts; ++ij){                               // ij is an index running over the list of the reconstructed jets after most of the cuts, but not yet the leading track bias, all jets in event are looped
	...
	continue;//rejection of current jet
      ...
      if(!(fUseExtraTracks == 0)){//this following big block is used only for Embedding study
	...
	if(ptFractionEmbedded >= fCutFractionPtEmbedded && deltaREmbedded <= fCutDeltaREmbedded) // end: cut embedded ratio
	  {
	    ...
	    FillEmbeddedHistos(jet, embeddedJet, nK0s, nLa, nALa, mclist);//fetch V0s for matched jets and fill embedding histos
	...
	for(Int_t ic=0; ic<jetConeALalist->GetSize(); ++ic){//loop over all reconstructed ALa
	  ...
	  TList *listmc = fAOD->GetList();
  	  if(!listmc)continue;
	  Bool_t isinjected;
	  Bool_t mclabelcheck = MCLabelCheck(v0c, kAntiLambda, trackMCNeg, trackMCPos, listmc, negDaughterpdg, posDaughterpdg, motherType, v0Label, MCPt, fPhysicalPrimary, MCv0PDGCode, generatorName, isinjected);
  ...
  if((fBranchGenJets.Length())&&(fMatchMode == 2)){
	...
	if(ptFractionEmbeddedMC>=fCutFractionPtEmbedded && deltaREmbeddedMC <= fCutDeltaREmbedded && 
	  ...
	  FillEmbeddedHistos(jet, embeddedJet, nK0s, nLa, nALa, mclist);
...
//_____________________________________________________________________
void AliAnalysisTaskJetChem::FillEmbeddedHistos(const AliAODJet* jet, const AliAODJet* embeddedJet, Int_t nK0s, Int_t nLa, Int_t nALa, TList* mclist)
...
{//Jet matching cuts (FractionPtEmbedded(MC) = 0.5, DeltaREmb(MC) = 0.75*R) are already applied
  ...
  Double_t dRadiusExcludeCone = 2*GetFFRadius(); //2 times jet radius
	  ...
	  Float_t jetPt = jet->Pt();//embedded true jet pt
	...
	jetConeK0Emblist->Clear();
	jetConeK0EmbStlist->Clear();//standard V0s in embedded jets, take smeared jet energy
