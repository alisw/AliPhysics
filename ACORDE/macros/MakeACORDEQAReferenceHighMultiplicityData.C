...
**************************************************************************/
void MakeACORDEQAReferenceHighMultiplicityData()
{
	...
	AliACORDECalibData *calibda = new AliACORDECalibData("QAHighMultiplicity");
	...
	TObjString str("ACORDE QA Reference HighMultiplicity Data");      // object that will be stored

	AliCDBMetaData *md= new AliCDBMetaData(); // metaData describing the object

	AliCDBId id("ACORDE/QA/HighMultiplicity",0,9999999);

	md->SetResponsible("Mario Rodriguez");
	md->SetBeamPeriod(0);
	md->SetAliRootVersion("Current trunk version");
	md->SetComment("Version 1.0 of QA Reference HighMultiplicity Data for ACORDE");
	md->PrintMetaData();
