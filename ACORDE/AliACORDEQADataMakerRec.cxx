...
//---
// Last update: May 5th. 2011 (by Mario RC: mrodrigu@mail.cern.ch) -->Creates QA expert histograms and QA-shifter histograms also with threshold lines and visual alarm
  ...
  // do the QA checking
	...
	for (Int_t specie = 0 ; specie < AliRecoParam::kNSpecies ; specie++) 
	{
		if ( !AliQAv1::Instance()->IsEventSpecieSet(specie) ) 
			continue ; 
		if (!GetRawsData(0)||!GetRawsData(1)) continue;
		Double_t integral = GetRawsData(0)->Integral();
		if (integral==0) 
		{
			...
			GetRawsData(0)->GetListOfFunctions()->Add(acoBoxFatal);
			...
			GetRawsData(1)->GetListOfFunctions()->Add(acoMultiBoxFatal);
		...
		// alarms
		Double_t max = GetRawsData(0)->GetMaximum();
		if (max == 0)
		{
		...
		Double_t maxMulti = GetRawsData(1)->GetMaximum();
		if (maxMulti == 0)
		{
		...
		{
			if ((GetRawsData(0)->GetBinContent(iModule))/max < 0.5) flagACO_DQM++;
		}
		if (flagACO_DQM < 15)
		{
			...
			GetRawsData(0)->GetListOfFunctions()->Add(acoBox);
		...
		}else if (flagACO_DQM > 15 && flagACO_DQM<30)
		{
			...
			GetRawsData(0)->GetListOfFunctions()->Add(acoBox1);
			isACOWarning=kTRUE;
		...
		}else if (flagACO_DQM > 30)
		{
			...
			GetRawsData(0)->GetListOfFunctions()->Add(acoBox2);
			isACOOk=kFALSE;
		...
		if (GetRawsData(1)->Integral()==0 || isACOOk==kFALSE)
		{
			...
			GetRawsData(1)->GetListOfFunctions()->Add(acoMultiBox);
		}
...
*/
		...
		if (!GetRawsData(2)||!GetRawsData(3)) continue;
		Double_t integral1 = GetRawsData(2)->Integral();
		if (integral1==0) 
		{
			printf("No entries in ACORDE Hits histograms --> fatal error, please check !!!\n");
			...
			GetRawsData(2)->GetListOfFunctions()->Add(acoBoxFatalAMU);
			...
			GetRawsData(3)->GetListOfFunctions()->Add(acoMultiBoxFatalAMU);
		...
		// alarms
		Double_t max1 = GetRawsData(2)->GetMaximum();
		if (max1 == 0)
		{
			printf("Maximum of hits equal to ZERO, please check the status of ACORDE !!\n");
			continue;
		...
		// Multiplicity histogram threshold
		Double_t maxMulti1 = GetRawsData(3)->GetMaximum();
		if (maxMulti1 == 0)
		{
			printf("Maximum of entries equal to ZERO, please check the status of ACORDE !!\n");
			continue;
		...
		for(Int_t iModule=0;iModule<60;iModule++)
		{
			if ((GetRawsData(2)->GetBinContent(iModule))/max1 < 0.5) flagACO_DQMAMU++;
		}
		if (flagACO_DQMAMU < 15)
		{
			...
			GetRawsData(2)->GetListOfFunctions()->Add(acoBoxAMU);
		...
		}else if (flagACO_DQMAMU > 15 && flagACO_DQMAMU<30)
		{
			...
			GetRawsData(2)->GetListOfFunctions()->Add(acoBox1AMU);
		...
		}else if (flagACO_DQMAMU > 30)
		{
			...
			GetRawsData(2)->GetListOfFunctions()->Add(acoBox2AMU);
			isACOOkAMU=kFALSE;
		...
		if (GetRawsData(3)->Integral()==0 || isACOOkAMU==kFALSE)
		{
			...
			GetRawsData(3)->GetListOfFunctions()->Add(acoMultiBoxAMU);
		}
/*		if (GetRawsData(5)->GetBinContent(1) > 0 || isACOOk && GetRawsData(5)->Integral()!=0 && isACOOk==kTRUE){
			acoMultiBox->SetFillColor(kYellow);
			acoMultiBox->SetLineColor(kYellow);
...
*/
		...
		for (Int_t iModule = 0; iModule < 60; iModule++)
		{
			if (GetRawsData(2)->GetBinContent(iModule)==0) 
			{
				eff = 0.0;
				continue;
			}else
			{
				eff = GetRawsData(0)->GetBinContent(iModule)/GetRawsData(2)->GetBinContent(iModule);
				GetRawsData(4)->Fill(iModule,eff);
				if (eff!=1) effFlag++;
			}
		}
		...
		if (effFlag == 0)
		{
			...
			GetRawsData(4)->GetListOfFunctions()->Add(checkTriggerBox);
		}else
		{
			...
			GetRawsData(4)->GetListOfFunctions()->Add(checkTriggerBox1);
	...
	for(Int_t r=0;r<60;r++)
		{
				if(kroSingle[r]==1)
				{
					GetRawsData(0)->Fill(r);
					//GetRawsData(4)->Fill(r);
					contSingle++;
				}
		}GetRawsData(1)->Fill(contSingle);//GetRawsData(5)->Fill(contSingle);
		contMulti=0;
        for(Int_t r=0;r<60;r++)
        {
			if(kroMulti[r]==1)
			{
				GetRawsData(2)->Fill(r);
				//GetRawsData(6)->Fill(r);
				contMulti++;
			}

        }GetRawsData(3)->Fill(contMulti);//GetRawsData(7)->Fill(contMulti);
}
}
//____________________________________________________________________________
