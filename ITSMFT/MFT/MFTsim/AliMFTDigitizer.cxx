...
void AliMFTDigitizer::Digitize(Option_t*) {
  ...
  TClonesArray sDigits[AliMFTConstants::kNDisks];
  for (Int_t iPlane=0; iPlane<AliMFTConstants::kNDisks; iPlane++) sDigits[iPlane].SetClass("AliMFTDigit");  // tmp storage for sdigits sum up from all input files
  ...
  for (Int_t iFile=0; iFile<fDigInput->GetNinputs(); iFile++) {
    ...
    if (!pInMFTLoader) {
      AliDebug(1,"no MFT loader, checking in the other input \n");
      continue;
    }
  ...
  for (Int_t iPlane=0; iPlane<AliMFTConstants::kNDisks; iPlane++) sDigits[iPlane].Clear();   // remove all tmp sdigits
...
void AliMFTDigitizer::SDigits2Digits(TClonesArray *pSDigitList, TObjArray *pDigitList) {   
  ...
  TClonesArray *myDigitList[AliMFTConstants::kNDisks] = {0};
  ...
  for (Int_t iPlane=0; iPlane<AliMFTConstants::kNDisks; iPlane++) {
    ...
    TClonesArray *digitsPerDetElem[nDetElem];
    ...
    for (Int_t iSDig=0; iSDig<pSDigitList[iPlane].GetEntries(); iSDig++) {
      ...
      Int_t localDetElemID = mftGeo->GetDetElemLocalID(newDig->GetDetElemID());
      AliDebug(1,Form("Unique ID = %d Local ID = %d  max = %d",newDig->GetDetElemID(),localDetElemID,nDetElem));
