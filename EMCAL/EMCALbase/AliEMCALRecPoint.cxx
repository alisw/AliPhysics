    ...
    fDistToBadTower(rp.fDistToBadTower), fSharedCluster(rp.fSharedCluster)
{
  ...
  for(Int_t i = 0; i < rp.fMulTrack; i++) 
    fDETracksList[i] = rp.fDETracksList[i];
...
void AliEMCALRecPoint::EvalLocalPosition(Float_t logWeight, TClonesArray * digits)
{
  ...
  for(Int_t iDigit=0; iDigit<fMulDigit; iDigit++)
  {
    ...
    if(logWeight > 0.0)  
      ...
      w = fEnergyList[iDigit]; // just energy
  ...
  { 
    ...
    for(i=0; i<3; i++ ) 
    {
      ...
      if(nstat>1) 
      {
        ...
        if(clRmsXYZ[i] > 0.0) clRmsXYZ[i] =  TMath::Sqrt(clRmsXYZ[i]);
        else                  clRmsXYZ[i] = 0;
      } 
      else        
        clRmsXYZ[i] = 0;
...
//_____________________________________________________________________________
Bool_t AliEMCALRecPoint::EvalLocalPositionFromDigits(const Double_t esum, const Double_t deff,
                                                     const Double_t w0, TClonesArray *digits,
                                                     TArrayD &ed, TVector3 &locPos)
{
  AliEMCALDigit *digit=0;
...
///
/// \return the position of the cluster in the local reference system of the sub-detector
//____________________________________________________________________________
...
///
/// \return the position of the cluster in the global reference system of ALICE
...
/// See ALICE PPR Chapter 5 p.18 for "tracking" c.s. definition, or just ask Jouri Belikov. :) 
//_____________________________________________________________________________
void AliEMCALRecPoint::EvalLocal2TrackingCSTransform()
{
  ...
  if(AliLog::GetGlobalDebugLevel()>0) 
  {
    ...
    Float_t gxyz[3];
    GetGlobalXYZ(gxyz);
    AliInfo(Form("lCS-->(%.3f,%.3f,%.3f), tCS-->(%.3f,%.3f,%.3f), gCS-->(%.3f,%.3f,%.3f), "
                 " gCScalc->(%.3f,%.3f,%.3f), supermodule %d",
                 fLocPos.X(),fLocPos.Y(),fLocPos.Z(),
                 GetX(),GetY(),GetZ(),
...
///
/// \return the primary track index in TreeK which deposits the most energy in Digits which forms RecPoint. 
//____________________________________________________________________________
