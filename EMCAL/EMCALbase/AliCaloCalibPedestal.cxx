...
///
/// \param detectorType: specify the detector defined in kDetType
...
AliCaloCalibPedestal::AliCaloCalibPedestal(kDetType detectorType) :  
  ...
  fHotSigma(5)
{
  // First we set the detector-type related constants.
...
{
  if (!fPedestalLowGain.IsEmpty()) return; 
  // The profiles already exist. We just check one, because they're all created at
  // the same time
...
///
/// \param parameterFile: external file path and name
//_____________________________________________________________________
...
{  
  ...
  while ((in.rdstate() & ios::failbit) == 0 ) 
  {
    ...
    while ( ( s.rdstate() & ios::failbit ) == 0 ) 
    {
      ...
      if ( (key == "fFirstPedestalSample") || (key == "fLastPedestalSample") || (key == "fDeadThreshold") || 
           (key == "fWarningThreshold")    || (key == "fWarningFraction")    || (key == "fHotSigma") ) 
      {
        printf("AliCaloCalibPedestal::SetParametersFromFile - key %s value %s\n", key.c_str(), value.c_str());
...
///
/// \param rawReader: reader pointer
//_____________________________________________________________________
...
///
/// \param in: AliCaloRawStreamV3 pointer to data stream
//_____________________________________________________________________
...
{ 
  ...
  while (in->NextDDL()) 
  {
    while (in->NextChannel()) 
    {
      ...
      while (in->NextBunch()) 
      {
        ...
        for (i = 0; i < in->GetBunchLength(); i++) 
        {
          ...
          if ( (fFirstPedestalSample<=time && time<=fLastPedestalSample) || // sample time in range
              !fSelectPedestalSamples ) 
          { // or we don't restrict the sample range.. - then we'll take all 
      ...
      if (nsamples > 0) 
      { 
        // this check is needed for when we have zero-supp. on, but not sparse readout
        ...
        {
          AliError(Form("n modules %d larger than expected %d",fModules,arrayPos));
          return kFALSE;
        }     
        ...
        if ( in->IsLowGain() ) 
        {
          ...
          if (nPed>0) 
          { 
            // only fill pedestal info in case it could be calculated
            for ( i=0; i<nPed; i++) 
            {
...
///
/// Compute the difference and ratio into the histo's in memory
...
///
/// Compute the dead tower values
/// Compute the number of dead towers etc etc into memory, after this you can call the GetDead... -functions
//_____________________________________________________________________
void AliCaloCalibPedestal::ComputeDeadTowers(const char * deadMapFile)
{
  ...
  for (int i = 0; i < fModules; i++) 
  {
    if (GetPeakProfileHighGain(i)->GetEntries() > 0) 
    {
      //don't care about empty histos
      for (int j = 1; j <= fColumns; j++)
      {
        for (int k = 1; k <= fRows; k++) 
        {
          
          if (GetPeakProfileHighGain(i)->GetBinContent(j, k) < fDeadThreshold) 
          {
            //It's dead
