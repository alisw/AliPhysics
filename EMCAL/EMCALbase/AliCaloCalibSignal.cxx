...
static double fAmp        = 0; ///< amplitude
static double fAvgAmp     = 0; ///< average amplitude
static double fRMS        = 0; ///< RMS

///
/// Constructor; initialize everything in order to avoid compiler warnings,
...
///
/// Check fraction of signals to check for LED events
...
///
/// Check if LED Ref is also above cut
...
///
/// \param fitAlgo: algorithm flag, check AliCaloRawAnalyzerFactory for options
//_____________________________________________________________________
...
{ 
  // note/FIXME: we are not yet adding correctly the info for fN{HighGain,LowGain,Ref} 
  // here - but consider this a feature for now (20080905): 
  // we'll do Analyze() unless entries were found for a tower in this original object.
...
{ 
  ...
  if (fStartTime == 0) 
  { // if start-timestamp wasn't set,we'll pick it up from the first event we encounter
  ...
  for(int i=0; i<fModules; i++)
  {
    for(int j=0; j<fColumns; j++)
    {
      ...
      if (checkResultArray[i*fColumns + j]>0  && checkResultArrayLEDRef[i*fLEDRefs + iStrip]>0) 
      {
        // column passed check 
        for(int k=0; k<fRows; k++)
        {
    ...
    for(int j=0; j<fLEDRefs; j++)
    {
      ...
      if ( ((checkResultArray[i*fColumns + iColFirst]>0) || (checkResultArray[i*fColumns + iColFirst + 1]>0)) && // at least one column in strip passed check 
          (checkResultArrayLEDRef[i*fLEDRefs + j]>0) ) 
      { // and LED Ref passed checks
...
///
/// \param fileName: string with designated file
//_____________________________________________________________________
...
Bool_t AliCaloCalibSignal::Analyze()
{
  ...
  for (fChannelNum = 0; fChannelNum<(fgkMaxTowers*2); fChannelNum++) 
  {
    if (profile[fChannelNum]) { // profile was created
      if (profile[fChannelNum]->GetEntries() > 0) 
      { 
        // profile had some entries
  ...
  for (fRefNum = 0; fRefNum<(fgkMaxRefs*2); fRefNum++) 
  {
    if (profileLED[fRefNum])
    { // profile was created
      if (profileLED[fRefNum]->GetEntries() > 0)
      { // profile had some entries
        for(int it=0; it<numProfBins; it++) {
