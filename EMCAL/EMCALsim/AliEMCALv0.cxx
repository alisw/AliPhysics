...
///
/// Initialize, main job done in AliEMCAL::Init()
/// Here, it just prints information messages
//______________________________________________________________________
...
void AliEMCALv0::Trd1Tower3X3(const Double_t *parSCM0)
{
  ...
  { // 3X3
    ...
    { // central part is box but we treat as trapesoid due to numbering
      parTRAP[1] = 0.;
...
/// For final "ALICE" geometry.
//______________________________________________________________________
void AliEMCALv0::AddAlignableVolumesInALICE() const
{
  Float_t pars[] = 
  {
    GetGeometry()->GetSuperModulesPar(0),
    GetGeometry()->GetSuperModulesPar(1),
    GetGeometry()->GetSuperModulesPar(2)
  };
  ...
  for (Int_t smodnum = 0; smodnum < nSMod; ++smodnum) 
  {
    ...
    if     (geom->GetSMType(smodnum) == AliEMCALGeometry::kEMCAL_Standard )      
    { SMPathName = "SMOD";  SMName = "FullSupermodule";}
    else if(geom->GetSMType(smodnum) == AliEMCALGeometry::kEMCAL_Half )     
    { SMPathName = "SM10";  SMName = "HalfSupermodule";}
    else if(geom->GetSMType(smodnum) == AliEMCALGeometry::kEMCAL_3rd )      
    { SMPathName = "SM3rd"; SMName = "OneThrdSupermodule";}
    else if( geom->GetSMType(smodnum) == AliEMCALGeometry::kDCAL_Standard ) 
    { SMPathName = "DCSM";  SMName = "DCALSupermodule";}
    else if( geom->GetSMType(smodnum) == AliEMCALGeometry::kDCAL_Ext )      
    { SMPathName = "DCEXT"; SMName = "DCALExtensionSM";}
    else AliError("Unkown SM Type!!");
