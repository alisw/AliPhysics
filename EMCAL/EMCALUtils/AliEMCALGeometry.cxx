...
{   
  ...
  // Init the proper trigger mapping, depending the presence of DCal
...
///
//________________________________________________________________________________________________
void AliEMCALGeometry::GetGlobal(const Double_t *loc, Double_t *glob, int iSM) const
{	
...
///
//________________________________________________________________________________________________
void AliEMCALGeometry::GetGlobal(const TVector3 &vloc, TVector3 &vglob, int iSM) const
{
...
///
/// \param absId: cell absolute id. number.
/// \param pri: if >0 more verbosity
/// \param tit: unclear use
///
//______________________________________________________________________
...
///
/// Print Trd1 parameters
/// For comparing with numbers from drawing (obsolete?)
///
/// \param pri: if >0 more verbosity
///
//______________________________________________________________________
...
Bool_t AliEMCALGeometry::GetAbsCellIdFromEtaPhi(Double_t eta, Double_t phi, Int_t &absId) const
{
  ...
  if(SuperModuleNumberFromEtaPhi(eta, phi, nSupMod)) 
  {
   ...
   //patch for mapping following alice convention  
    if(nSupMod%2 == 0) 
    {// 47 + 16 -ieta for DCSM, 47 - ieta for others, revert the ordering on A side in order to keep convention.
      ieta = (neta -1)-ieta;
...
{
  if( fEMCGeometry->GetEMCSystem()[iSupMod] == kDCAL_Standard || fEMCGeometry->GetEMCSystem()[iSupMod] == kDCAL_Ext ) 
    return kTRUE;
...
{
  if( fEMCGeometry->GetEMCSystem()[iSupMod] == kDCAL_Ext ) 
    return kTRUE;
