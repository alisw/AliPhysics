...
namespace CaloConstants
{
  ...
  const double TIMEBINMAX  = TIMEBINS*TIMEBINWITH; ///< maximum bin in time sample
  //  const double TAU = 2.35;
  //  const int  ORDER = 2;
  
  const int    OVERFLOWCUT = 950; ///< sample overflow
  const double HGLGFACTOR  = 16;  ///< energy transion low/high gaing
...
//const int MAXHWADDRESSES   = 4096;    ///< Highest possible harware address
  ...
  namespace EMCALConstants
  {
    ...
    const int MAXHWADDR    = 3279;   ///< Max harware address,
    const int MAXCHANNELS  = 1408;   ///< Max number of channels
  ...
  {
    const int MAXHWADDR      = 4096;  ///< Max harware address
//  const int MAXHARDWAREADDRESS = 4096; ///< Max harware address,  ( its to high )
  ...
  namespace ReturnCodes
  {
    ...
    enum kReturnCode { kFitPar=1, kDummy=-1, kCrude=-9, kNoFit=-99, kInvalid=-9999 } ;  
  }
