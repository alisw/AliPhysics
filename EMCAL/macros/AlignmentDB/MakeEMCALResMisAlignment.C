///
/// \file  MakeEMCALResMisAlignment.C
/// \ingroup EMCAL_AlignDB
/// \brief Make OCDB residual alignment matrices
///
...
void MakeEMCALResMisAlignment(TString geoname = "EMCAL_COMPLETE12SMV1_DCAL_8SM")
{
  ...
  ("AliAlignObjParams",emcalGeom->GetNumberOfSuperModules());
  ...
  if( TString(gSystem->Getenv("TOCDB")) == TString("kTRUE") )
  {
    ...
    storage = cdb->GetStorage(storageName.Data());
    if(!storage)
    {
  ...
  for(i=0; i<nSM; i++)
  {
    ...
    if(     emcalGeom->GetSMType(i) == AliEMCALGeometry::kEMCAL_Standard) pathstr=fbasepath;
    else if(emcalGeom->GetSMType(i) == AliEMCALGeometry::kEMCAL_Half    ) pathstr=hbasepath;
    else if(emcalGeom->GetSMType(i) == AliEMCALGeometry::kEMCAL_3rd     ) pathstr=tbasepath;
    else if(emcalGeom->GetSMType(i) == AliEMCALGeometry::kDCAL_Standard ) pathstr=dbasepath;
    else if(emcalGeom->GetSMType(i) == AliEMCALGeometry::kDCAL_Ext      ) pathstr=debasepath;
