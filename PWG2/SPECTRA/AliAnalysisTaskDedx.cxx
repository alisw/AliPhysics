...
#include "AliAnalysisTaskDedx.h"

ClassImp(AliAnalysisTaskDedx)

//________________________________________________________________________
AliAnalysisTaskDedx::AliAnalysisTaskDedx()
  : AliAnalysisTaskSE(), fAnalysisType("ESD"), fCollidingSystems(0), fListHist(), 
    fHistPtot(0), fHistMultiplicity(0), fHistTPCDedxVsMomentum(0), fHistITSDedxVsMomentum(0),
    ...
    fAllConstrainedFlag(0), fMidPseudoRapidityFlag(0),

    fSelTrackRemoveKink(0), fSelTrackWithOnTheFlyV0(0),
    fSelTrackMinClustersTPC(0), fSelTrackMinClustersITS(0),
    fSelTrackMaxChi2PerClusterTPC(0), fSelTrackMaxChi2PerClusterITS(0),
    fSelTrackMaxCov11(0), fSelTrackMaxCov22(0), fSelTrackMaxCov33(0),
    fSelTrackMaxCov44(0), fSelTrackMaxCov55(0),
    fSelV0MaxDcaDaughters(0), fSelV0MinDecayLength(0)
{
  // Dummy constructor
}
//________________________________________________________________________
AliAnalysisTaskDedx::AliAnalysisTaskDedx(const char    *rName,
  ...
  : AliAnalysisTaskSE(rName), fAnalysisType("ESD"), fCollidingSystems(0), fListHist(), 
    fHistPtot(0), fHistMultiplicity(0), fHistTPCDedxVsMomentum(0), fHistITSDedxVsMomentum(0),
    fHistMassK0(0), fHistMassLambda(0), fHistMassAntiLambda(0),
    fHistTPCDedxVsMomPosK0(0), fHistTPCDedxVsMomNegK0(0),
    fHistTPCDedxVsMomPosLambda(0), fHistTPCDedxVsMomNegLambda(0),
...
//________________________________________________________________________
void AliAnalysisTaskDedx::UserCreateOutputObjects()
{
  // Create histograms
  // Called once
  ...
  if (!fHistMultiplicity) {
    if (fCollidingSystems)
      fHistMultiplicity = new TH1F("fHistMultiplicity", "Multiplicity distribution;Number of tracks;Events", 200, 0, 40000);
    else
  ...
  if (!fHistMultiplicityCuts) {
    if (fCollidingSystems)
      fHistMultiplicityCuts = new TH1F("fHistMultiplicityCuts", "Multiplicity distribution;Number of tracks;Events", 200, 0, 40000);
    else
      fHistMultiplicityCuts = new TH1F("h1HistMultiplicityCuts","Number of Good tracks;Number of tracks;Event", 250, 0, 250);
...
//________________________________________________________________________
void AliAnalysisTaskDedx::UserExec(Option_t *) 
{
  // Main loop
  ...
  Int_t    ntracks        = lEvent->GetNumberOfTracks();
  Printf("dE/dx analysis task: There are %d tracks in this event", ntracks);
  ...
  for (Int_t iTracks = 0; iTracks < ntracks; iTracks++) {
    AliESDtrack* track = ((AliESDEvent*)lEvent)->GetTrack(iTracks);
    if (!track) {
  ...
  fHistMultiplicity->Fill(ntracks);
  ...
  for (Int_t iV0 = 0; iV0 < nv0s; iV0++) 
    {// This is the V0 loop
      ...
      const AliESDVertex *lPrimaryVertex = ((AliESDEvent*)lEvent)->GetPrimaryVertex();
      ...
      AliESDtrack* trackPos =((AliESDEvent*)lEvent)->GetTrack(lIndexTrackPos);
      AliESDtrack* trackNeg =((AliESDEvent*)lEvent)->GetTrack(lIndexTrackNeg);
...
//________________________________________________________________________
void AliAnalysisTaskDedx::Terminate(Option_t *) 
{
  ...
  fHistPtot = dynamic_cast<TH1F*> (((TList*)GetOutputData(1))->FindObject("fHistPtot"));
  if (!fHistPtot) {
  ...
  if(fHistPtot->GetMaximum() > 0.) cTaskDedx->cd(1)->SetLogy();
  fHistPtot->DrawCopy("E");
}
//____________________________________________________________________//
Bool_t AliAnalysisTaskDedx::IsAccepted(AliESDtrack* track) {
